...
const scripts = ["place13.mjs", "land13.mjs", "m0110.mjs", "sched13.mjs", "fix278.mjs", "led7b1.mjs", "debtpeek.mjs", "drafts-branch.mjs", "wtclean.mjs"];
const drafts = readdirSync(S).filter((f) => /^new-(M0-11[456]|D-278|D-148|D-149|D-134)\.md$/.test(f)).sort();
const sTree = tree(scripts.map((f) => ["100644", "blob", blob(join(S, f)), f]));
const dTree = tree(drafts.map((f) => ["100644", "blob", blob(join(S, f)), f]));
const parent = git(["rev-parse", process.env.PARENT || "origin/scheduler13/row-drafts"]);
...
const msg = process.env.MSG || "scheduler #13: the lane scripts of this session and the drafts it placed (never merged) — lane-scripts-13/ (place13 with --place and --cut apart, land13, m0110, sched13, fix278) beside SCHEDULER #12's lane-scripts/; row-drafts-13/ the full drafts of M0-114, M0-115, M0-116, D-278, D-148 and D-149 as placed\n\nCo-Authored-By: Claude Opus 5 <noreply@anthropic.com>\n";
