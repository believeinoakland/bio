...
- **D-349** — every path in the CLAIM and its ADDENDUM above, plus `docs/DECIDED.md` (REGENERATED by `node tools/decided.mjs`, which `plancheck` requires of any turn that rules — it dropped D-349's ROUTED row and gained IC-96's correction; a generated index, no judgment).  
