...
**Incomplete sections** ·
- §3 — the STATE column is now RENDERED from `construct-status.json` and verified against the code at every push (2026-09-18); it replaced a 2026-09-14 hand reading that three independent reads found wrong both ways, mostly calling BUILT things absent. Each cell is a claim's first sentence (M0-138; §3's preamble). **What it still cannot say:** a probe proves a named thing is PRESENT, or ABSENT under the names searched — never that it WORKS (the battery's job), and an absence is only as good as the names tried. Three claims rest on the code's own statement of an absence (5.reextract, 8.contradiction, 9.internet) and say so.
...
| 10 | **Standing intent and monitoring** | named requests and ratified sweeps; the frontier of deferred links; monitoring contracts; change detection through the digests; what the instance does unattended | the record stays current without anyone calling an op | capture (2), scheduler (14) | `BIO_Intake_Doctrine_v1_1.md` §4; `BIO_Content_Framework_v0_10.md` Part I §6 [`SCHEDULER.md`, `NOTIFICATIONS.md`] | **BUILT:** monitoring on the alarm scheduler, with digest-based change detection; which Drive-linked bundles hold a baseline captured from Google's application shell rather than the export (D-525): `op=driveshells` is a READ, under op=index's classes and D-15 stamp, that walks every visible bundle whose projected …; op=monitor asks `assess` through the capture's handler and content type, answers with the layer it stopped at and graded events, logs each look, and says which cadence governs (D-65); the monitor cadence schedules an ADDRESS, not a bundle, and a document authoring no frequency by its content type's contract (REC-191; D-220's ruled intent; BOB #31's 22:03Z ruling); the frontier of deferred links; a member's LEAD (D-194; MEMBER-KNOWLEDGE-DESIGN.md §5), plane half (MK-4): the `leads` table and op=lead, leadlook, leadread and leadshare. … — verified at the code: `node tools/status.mjs 10` **Designed in:** the LEAD (10.lead) — `docs/development/MEMBER-KNOWLEDGE-DESIGN.md` §5 (2026-09-18). |
...
release and installs the fleet (D-297); DIST deploys it at every cut under Bob's standing
permission of 2026-09-18 (BOB #25 corrected, 2026-09-22, a line saying it waited on Bob). Several instances in one account
