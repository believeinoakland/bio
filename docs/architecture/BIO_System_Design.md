...
| 10 | **Standing intent and monitoring** | named requests and ratified sweeps; the frontier of deferred links; monitoring contracts; change detection through the digests; what the instance does unattended | the record stays current without anyone calling an op | capture (2), scheduler (14) | `BIO_Intake_Doctrine_v1_1.md` §4; `BIO_Content_Framework_v0_10.md` Part I §6 [`SCHEDULER.md`, `NOTIFICATIONS.md`] | **BUILT:** monitoring on the alarm scheduler, with digest-based change detection; the frontier of deferred links; a member's LEAD (D-194; MEMBER-KNOWLEDGE-DESIGN.md §5), plane half (MK-4): the `leads` table and op=lead, leadlook, leadread and leadshare. … — verified at the code: `node tools/status.mjs 10` **Designed in:** the LEAD (10.lead) — `docs/development/MEMBER-KNOWLEDGE-DESIGN.md` §5 (2026-09-18). |
