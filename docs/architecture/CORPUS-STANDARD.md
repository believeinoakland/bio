...
**Incomplete sections** ·
- §5 — the design documents under `docs/development/` are listed and six are governed as of 2026-09-14; the rest join the governed table as each is retrofitted (an act per owner, routed through the queue). CONDUCT's group is done, BOB's, RECORD's, CAPTURE's, UI's, SKILL's and FRAMEWORK's are not.
