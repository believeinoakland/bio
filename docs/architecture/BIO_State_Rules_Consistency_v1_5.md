...
info finding rather than present a key-order walk as write order. The
I-17 divergence ladder walks the same order (D-718, 2026-09-25): C-17.2's
classifier anchors a pending package's base, and collects the promotions
intervening since it, in `seq` order, so a promotion written after the
base is never dropped from the intervening set for carrying an earlier
key; on an image without `seq` it classifies in snap-key order and C-17.2
says so in an info finding.
