...
- §6 — I-20's write-order rule is kept by the store's manifest reads (REC-182, D-674) but NOT yet by its audit: `readImage` sorts the gate image's `_history/manifest.json` entries by snap key and C-20.1 walks them in snap-key order, so a caller choosing keys against write order is audited against the wrong prior snapshot. The plane's own writers key by timestamp, so this needs a caller-chosen key. The fix: the image carries write order and C-20.1 walks it (found by REC-182, 2026-09-23; minted D-700 by D-674, which found no plan row for it).
