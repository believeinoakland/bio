...
- §8 — an inline file's stated `bytes` is not verified at promote (REC-175's finding: several writers send `text.length`, UTF-16 units, not UTF-8 bytes); `op=digestcensus` counts the disagreement and nothing refuses it. What a deployed instance's census reads is not known until an administrator runs it there.
