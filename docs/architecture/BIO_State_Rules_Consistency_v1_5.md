...
- §4.1 — `op=reinstate` onto a retired item is refused `RETIRED_NOT_CITABLE` since REC-183 (2026-09-23), closing the door REC-181 found, and the PROJECT side of the `reinstate` affordance is narrowed to match since D-444 (2026-09-24), so the residue stated here is closed. What remains: the retired question is spelled in THREE places — the one `#retiredNotCitable` predicate the refusal and the pre-flight share, D-168's copy inside the `is-cite-retired` DEC-49 region at `op=cite`, and a type-blind, viewer-gated one at the suggest path's `SUGGEST_LEG_UNREACHABLE` — and folding them is not a rename, because the cite copy sits inside a governed region with a `regionLines` floor and the suggest copy asks a different column set. A confirmed edge onto a retired item that predates REC-181 survives in the record and is not swept. The withdrawal door for a question's leg is a new basis version, not a sever (ruled below, BOB #31).
...
- §4.7 — C-4.2's corroborated reading dates the record's move by the manifest's `created`, which is the writer's `last_updated` where it sent one, so a writer who misdated its own promotion moves which side of the fence the record's move reads (D-674, placed). Which PAIRS exist does not depend on it: they are taken in write order and only where the chain joins. A move dated ON the fence's day reads "not before the fence", because the fence is a date and which side of it that day's writes fell is not recorded.
...
- §6 — I-20's tie rule is kept by the store's manifest reads (REC-182) but NOT yet by its audit: `readImage` sorts the gate image's `_history/manifest.json` entries by snap key and C-20.1 walks them in snap-key order, so a caller choosing keys against write order is audited against the wrong prior snapshot. The plane's own writers key by timestamp, so this needs a caller-chosen key. The fix: the image carries write order and C-20.1 walks it (found by REC-182, 2026-09-23; sent to SCHEDULER).
...
- **A document's own history is read against the record's (RULED 2026-09-25 08:00Z by BOB #35, option (b); D-673).**
  The `state_history` a document carries in its bytes is the writer's claim about moves, and its timestamps are the
  writer's: a writer can backdate them, so a timestamp in the bytes never buys the pre-fence reading on its own (a
  provenance hop a caller can hand us is one a caller can invent). The gate's C-4.2 therefore reads an undeclared edge
  in those bytes in the sentence above ONLY where the RECORD's own history corroborates the same move: the pair
  appears among `op=statemovecensus`'s chain-joined moves for that bundle, dated before the fence. Otherwise C-4.2
  keeps the ERROR. As built:
  - The corroboration is the census's own computation, not a second one: the store's `recordedMovesFor` returns the
    bundle's joined moves from the pairing the census counts, and it is injected into the ratification gate's facts and
    into `op=audit`'s sweep. A caller with no view of the record (the cli over a filesystem) injects nothing, and then
    nothing is corroborated.
  - "Before the fence" is the census's one predicate (`stateMoveBeforeFence`), so the gate never places a move before
    the fence that the census would not; a move dated on the fence's own day is not before it.
  - Each recorded move corroborates at most ONE entry, so the reading is never larger than the record's count: bytes
    claiming a move twice that the record holds once read one entry stated and one in error.
  - The corroborated reading is neither valid nor invalid, as D-546 ruled: the gate passes it as STATED (an `info`
    finding in the sentence, naming the record's promotion and its writer's date), not as legal. `op=audit` lists
    them beside `offenders` in `stated_moves`, always present, bounded at 20 with the whole count.
  - M-179 measured 11 in-bytes entries and 0 undeclared on the live register, so nothing live changes.
...
it is not a separate permission but a consequence of writing at all. "Prior" is defined on a tie (REC-182, 2026-09-23): a
promotion's `created` is the document's own last_updated, so two can
carry the same one, and on a `created` tie the immediately prior
recorded snapshot is the one written first — the store's write order
(`rowid`), never the caller-chosen snap key, whose lexical order is not
a clock (D-171's precedent). The store's manifest reads order by
`created`, then write order.
