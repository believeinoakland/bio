...
**A BEARER REACHING `op=memberadd` IS NOT REFUSED — PROVISIONAL, REC-156's decision and BOB's to
rule.** Once stamped, a bearer's `by` names nobody, and what the op still does for one — the §6
...
- **BUILT 2026-09-19 by REC-151 (IC-164, C-59.5), the remaining gated prefixes.** `Store#mintOpaqueId` is the ONE
  ...
  `Math.random` or counter-derived suffix passes every behavioural arm and fails the source pins). An opaque id can be
  drawn again after a whole-store purge (D-432, fix named).
