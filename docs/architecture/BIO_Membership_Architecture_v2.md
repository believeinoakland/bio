...
- §4 (the 4.10 block) — REC-155's seven session routes are RULED (BOB #19, 2026-09-21) and NOT BUILT: all seven still answer every session `SESSION_ROUTE_NOT_RECORDED`, and the provenance pair's bearer write stays open until the ruling's second landing.
...
- §7 (item 7.14) — DISCOVERABLE or HIDDEN and the request to join: DESIGNED (BOB #16, 2026-09-19); step 1 BUILT by REC-149 (IC-231, C-70, 2026-09-23), and every existing project reads HIDDEN. NOT BUILT: the request to join (step 2 — until it lands, EVERY act at EXISTENCE is refused, and the directory's `request` is null and says why); a creation or fork CARRYING its setting (the plane creates every project HIDDEN, and the item does not say how a setting travels on a creation, nor whose it is when a machine credential creates a project with no owner); the surfaces (steps 3 and 4). UNDECIDED (REC-149's DESIGN GAP): a READ that names a project by id (`projectparticipants`, `strengthbarof`, `image`, …) still answers a caller at EXISTENCE as for a project that does not exist, as "Record reads do not widen" says — while the positional refusal's own argument (*a "does not exist" answer is false about a project the directory has just shown*) applies to those answers too.
- §4 (the 4.7 block) — `governorconfig` is RULED the OPERATOR's, not an administrator's (BOB #23, §4.9): it stays the founder's session's and the bearer's, and since REC-159 moved the four custodial acts into both session sets it is the ONE op the founder's session alone reaches; its refusal still calls the op an administrator's and an enrolled administrator's role `member` — that sentence's correction is REC-162's and NOT BUILT. (The four custodial acts themselves are CLOSED by REC-159, 2026-09-23, graded by `bio-plane/test/adminvote.test.mjs` §9.) A `status_by` recorded before REC-159 does not exist: every member and key row changed before it reads `not recorded`, which is stated rather than back-filled.
...
- §7 (item 7.9) — COUNTS: BUILT by D-464 (2026-09-24, IC-254, MEASUREMENTS M-124; first measured by D-447, M-122). `op=stats` (and its `selftest` and `livefire` relays), `op=searchindexcheck`'s `counts.indexed` and `op=selectionlist`'s `bytes` are taken through the caller's `viewerPredicate`: a row naming a bundle the caller cannot see is not counted, so creating and revising a hidden project leaves a member's answers byte-identical (`project-sight.test.mjs` §8); an unfiltered credential's figures are unchanged. NOT decided and stated: whether what a hidden project's RUN produced at the evidence levels (its observation-log rows, proposed readings, capture requests) is the shared corpus's or the project's — `observationsNonLead`, `aiRunLog` and the frontier's tallies still count it; and `#queueSharedInquiryCandidates` bounds its candidates over the UNGATED `refs`, so at more than 64 shared questions a hidden project's citations can take a slot and move `inquiries_truncated` (routed with its fix, D-464's report). The `rows=passage` snippet over `capture_text_fts` is the same statistics-free FTS5 function as the search snippet and its rows order by identity, not by score — reasoned from the code, NOT driven by a hidden-revision arm.
...
credential, never a person. The two plane comments that justified `governorconfig`'s class by
