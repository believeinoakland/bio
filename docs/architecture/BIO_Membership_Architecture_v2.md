...
- §4 (4.9) — the member SURFACE for the four custodial acts is BUILT by D-134 (2026-09-25; the paragraph closing 4.9's REC-159 block). Still without a member surface: the §4.7 votes (`adminendorse`, `adminremove`), `membercaps` and `expertiseconfirm`. An ORDINARY invitation records no inviter (`members.status_by` stays NULL); whether it should is BOB's question, routed by D-134.
...
C-96.9). WHAT THE RECORD DOES NOT SAY: an ordinary invitation, and the second administrator's, record no
inviter, so `status_by` reads `not recorded` on such a row; a §4.7 proposal names its proposer through the
proposer's own endorsement.
