...
- §7 — *"HOW the plane mints a project id"* is BUILT in the plane (REC-141, IC-158, C-59) and on the surface (UI-66: the Add surface and the fork form send no id and show the plane's). Two residuals are named, not closed: a project created before REC-141 under a NON-canonical id (not `PROJ-`-prefixed) still answers `EXISTS` to a creation of another type at that id; and the minted id's sequence number tells a creator how many project ids were minted before theirs, hidden projects included (as `op=allocid` already did) — RULED a §7.9 disclosure by BOB #16 (2026-09-19); the opaque-id rule is DECIDED and NOT BUILT on this tree (the bullet *"A MINTED ID CARRIES NO COUNT"*).
...
- **BUILT 2026-09-18 by REC-141 (IC-158, C-59), the plane half.** `Store#promote`: a `base: null` creation typed
  ...
  plane mints `PROJ-<year>-<seq>-<slug of the name>` from `allocId`'s sequence inside the promote transaction, writes
