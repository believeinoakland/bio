...
- §4 (the 4.7 block) — `op=memberadd` reaches the FOUNDER'S session alone (`SESSION_OPS.admin`), so an enrolled administrator cannot propose an addition or invite a member from her own session, and is refused with a sentence calling the op an administrator's; `memberset`, `signeradd` and `signerset` stand the same way against §4.9's list. Routed by REC-156 with its fix named (D-136's shape), not taken; pinned as KNOWN-OPEN by `bio-plane/test/adminvote.test.mjs` §8f. An operator's BEARER keeps reaching all four, its `by` stamped `class:admin` and endorsing for nobody (RULED by BOB #22, §4.7). `governorconfig` is RULED the OPERATOR's, not an administrator's (BOB #23, §4.9): it stays the founder's session's and the bearer's, and its refusal, which calls the op an administrator's, is corrected when the four move — NOT BUILT.
