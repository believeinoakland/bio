...
- §7 — DEC-63's run verdict: RULED by Bob (2026-09-18) and BUILT by REC-145 (IC-162). Who may TICK and CLOSE a run (the gap REC-145 pinned as built at ARM H6) was DECIDED by BOB #16 — the run's principal alone — and is BUILT by REC-152 (IC-165, C-22.12). And `aiRunOpen` does not check the context KIND against the bundle it names, so a run labelled `inquiry` over a project's id consults nothing (pre-existing, REC-69's unfenced kind).
