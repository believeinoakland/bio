...
- **BUILT 2026-09-18 by REC-134 (IC-152).** One helper, `Store#projectAuthority`, asks the POSITIONAL identity (never the
  ...
  **`linkproject` JOINS THIS LIST (D-722, BOB #36's final ruling 2026-09-25 11:15Z).** It sat in `EDGE_ACTIONS` beside
  `cite`, `sever` and `reinstate` and this per-act table never reached it. Where the capture's source bundle is a PROJECT
  (registered there, or named by `bundle=`), the edges it gains are that project's citation edges, so the actor must be a
  JOINED participant — `#projectAuthority(..., "joined", "linkproject")`, asked after sight. A `links_to` edge on shared
  Information changes no project, and the `contribute` capability governs it as before. The same ruling holds "not its
  existence" at this act: it resolves ONCE through the caller's sight and writes exactly the edges its answer names, and a
  `bundle=` the caller cannot see answers `NO_SUCH_BUNDLE` as an id naming nothing does (`LINK-FIDELITY.md` §The write is
  the viewer's; `bio-plane/test/d706-linkproject.test.mjs`).
