...
- §7 — DEC-63's run verdict: RULED by Bob (2026-09-18) and BUILT by REC-145 (IC-162). What the ruling does not say: the three run verbs share ONE gate, so over a question any member holding contribute may also TICK and CLOSE another member's run — whether a run is its opener's to drive is UNDECIDED (pinned as built, `airun-projectgate.test.mjs` ARM H6). The context KIND is checked at the open (REC-153, IC-163); what remains: the runs stored before it under a mislabelled kind are UNCOUNTED (MEASUREMENTS M-68 — no op enumerates runs), and tick and close still read such a run by its stored kind.
