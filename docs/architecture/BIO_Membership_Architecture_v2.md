...
- §7 — DEC-63's run verdict: RULED by Bob (2026-09-18) and BUILT by REC-145 (IC-162). What the ruling does not say: the three run verbs share ONE gate, so over a question any member holding contribute may also TICK and CLOSE another member's run — whether a run is its opener's to drive is UNDECIDED (pinned as built, `airun-projectgate.test.mjs` ARM H6). The context KIND is now checked at the open (REC-153, IC-163); what remains open: a kind outside the two-word vocabulary that EQUALS a seen bundle's type (`information` over an Information bundle) still opens (REC-69's unfenced word, narrowed not closed), and an `ai` credential, which sees as its principal member but is not asked for participation, may open a run labelled `inquiry` over a project hidden from that member, because it must answer as for an absent id — no authority is gained (a machine is not gated under `project` either), but the record then holds a run whose kind is not the thing it names.
- §7 — a legacy project's caller-chosen id still answers `EXISTS` to a creation of another type at it: a stated LIMITATION with its closing path (the bullet "The legacy residue"); the opaque-id rule is DECIDED and NOT BUILT.
...
- **RULED BY BOB, 2026-09-18 — a project does not own a line of inquiry.** *"Anybody can ask a question related to
  ...
  **AND THE CONTEXT KIND IS CHECKED (the same day, same source):** `aiRunOpen` never compared `contextType` with the
  ...
  must equal the named bundle's type; a mismatch is refused, and an id the caller cannot see answers as absent. **BUILT
  2026-09-19 by REC-153 (IC-163, C-22.11 `AI_RUN_NO_SUCH_CONTEXT`):** `airun.mjs checkRunContextKind` decides it at the
  open, before the gate, from the named bundle's type AS THE CALLER CAN SEE IT (`Store#inSight`); the kind is compared
  exactly (`Project` is not `project`). One refusal carries the mismatch, the absent id and the hidden one, built only
  from what the caller sent. What an unseen id answers follows REC-138: under `project` the joined gate answers it,
  unchanged; under any other kind a MEMBER is refused; a MACHINE credential is not (it has no participation to walk
  around, and a run's context need not be a bundle the store holds). Driven in `bio-plane/test/airun-contextkind.test.mjs`;
  control in `airun-contextkind.control.mjs`.
