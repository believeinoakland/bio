...
- §7 (item 7.9) — COUNTS taken over the whole store still reach a member: `op=stats` moves when a project the member cannot see is created (`bundles`, `indexed`) or revised (`history`), and `op=searchindexcheck`'s `counts.indexed` moves on a hidden creation. MEASURED by D-447 on 2026-09-23 (MEASUREMENTS M-122), NOT fixed there; the fix named is to count through the caller's `viewerPredicate` for every class but the operator's. The `rows=passage` snippet over `capture_text_fts` is the same statistics-free FTS5 function as the search snippet and its rows order by identity, not by score — reasoned from the code, NOT driven by a hidden-revision arm.
