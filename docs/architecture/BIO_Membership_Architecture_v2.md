...
- §7 — DEC-63's run verdict: RULED by Bob (2026-09-18) and BUILT by REC-145 (IC-162). What the ruling does not say: the three run verbs share ONE gate, so over a question any member holding contribute may also TICK and CLOSE another member's run — whether a run is its opener's to drive is UNDECIDED (pinned as built, `airun-projectgate.test.mjs` ARM H6). And `aiRunOpen` does not check the context KIND against the bundle it names, so a run labelled `inquiry` over a project's id consults nothing (pre-existing, REC-69's unfenced kind).
