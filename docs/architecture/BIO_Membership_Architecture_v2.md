...
- §7 — *"HOW the plane mints a project id"* is BUILT in the plane (REC-141, IC-158, C-59) and on the surface by UI-66 (landed together: the Add surface and the fork form send no id and show the minted one). The minted PROJ id carries NO count (BOB #16): its suffix is random from the CSPRNG, never `allocId`'s counter. The opaque-id rule for `CASE`, `DRAFT`, `RVG` and `op=allocid`'s refusal of gated prefixes is NOT built by REC-141 (a separate SCHEDULER item).
