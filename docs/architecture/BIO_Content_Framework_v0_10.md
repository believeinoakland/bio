...
| no recorded via (a provenance document's bytes, a member's upload) | UNDETERMINED, and STATED: the entry carries `fetch: { route: "unrecorded" }` with `undetermined_because: CAPTURE_ROUTE_UNRECORDED`; a leg on it reads as the letter its author gave, under the ceiling, and the read says the letter is authored, never measured | BOB #35 (1); BOB #35, 2026-09-25 10:05Z (D-709) |

**An unrecorded route is stated on every entry, never left silent** (BOB #35, 2026-09-25 10:05Z;
D-709). CLAUDE.md §4 makes undetermined first-class and stated: an entry with no `fetch` key
cannot be told apart from "the route was measured and is absent", so every capture entry
none of whose captures has a recorded route carries `fetch: { route: "unrecorded" }` — the
same `unrecorded` naming the partial case (some captures routed, some not) counts under — and
its capture grade reads as the authored letter under the ceiling, never as a measured
letter. The suites that pinned such entries whole encoded that silence; they were corrected,
not exempted.
