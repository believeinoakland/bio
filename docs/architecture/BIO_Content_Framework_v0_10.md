...
- **A PAGE AN IMAGE FILLS WHILE ITS TEXT IS A FOLIO SAYS SO, AND IS ROUTED TO OCR (D-627, 2026-09-25, M-178; BOB #35's
  ...
  **NOT BUILT, and each named (item (2) is corrected in place: BUILT by D-633 the same day).** (1) A chart painted as an image under a text title has the same two figures as a photo
  page with captions, so it reads no marker. BOB #35 ruled at 06:25Z that the true statement is per IMAGE: every painted
  image above a size floor carries `image_unread` with its rect and area share, whatever it depicts (D-665). Only the
  OCR route needs a signal, and for that class it stays off until D-665 measures one over M-178's 49 pages. (2) A page
