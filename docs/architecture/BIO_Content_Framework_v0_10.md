...
- §16 — **TIER 3 NOW READS A SCAN PAST ITS FIRST PAGE (D-606, 2026-09-25), within a per-request budget of 24 member calls** (the tier table's row 3). NOT BUILT: reading the pages past that budget on a later request (D-616), so a scan longer than 24 image-only pages stays partly unread, stated; the budget rests on Cloudflare's claim of 32 invocations per request, unmeasured on the deployed runtime (M-175).
