...
was read. `content.chain_kind` is the kind of the unit's page: each PART covering the page answers its last
derivation step (a part's `pixels` then `ocr` is one reading, `ocr`), and the page reads that kind when every
part agrees. On a mixed document a text-layer page's unit therefore reads `layer` and an OCR'd page's reads
`ocr`. **A page two parts share** (D-635: the folio decoded from the text layer, the transcription appended)
reads **`mixed`** (BOB #36, 2026-09-25 11:05Z; BUILT 2026-09-25, D-723) — D-686's rule that it reads the part
appended last is SUPERSEDED for this case only, because `ocr` called the layer's text machine-read; a page read
one way keeps its one kind, and an unreadable extent behind a covering part is undetermined (it could be a
second part). **A unit with no page** (a whole document, an office
...
first. `content:mixed` is a search word, and the published search grammar is the first such reader: it
says a `mixed` passage CONTAINS machine-read text and that `content:ocr` names its pages, not it (D-710).
Where the chain cannot say which pages which kind read — an unscoped step BEFORE scoped parts, since
the pages only it read need a page count the chain does not hold — the unit is undetermined (NULL),
never the parts' kind (D-710: a caller's `text_source` can carry that shape; `mergedChain` never
builds it). One
