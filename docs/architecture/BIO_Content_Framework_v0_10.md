...
- §14.2 — **the capture grade BELOW the ceiling is MEASURED only for a DIRECT fetch (D-177, 2026-09-25; `node tools/status.mjs 2.capture-grade`)**. BUILT: `earnedBasisRegistry` reads each capture's `captured_locators.via`; a document fetched `direct` by this instance carries a `fetch` entry whose `earned` letter is the byte grade bounded by its chain (DEC-4, DEC-75), and every reader of a leg's capture letter reads a weaker authored one at it. NOT BUILT, and UNDETERMINED by any ruling found: what a NON-direct capture (an archive replay) earns — `op=acquire`'s typed archive letter is open by decision (REC-50), so such a capture is named `CAPTURE_GRADE_VIA_UNRULED` and a leg on it keeps its authored letter under the ceiling; routed to BOB. A capture with NO locator row (a provenance document's bytes, a member's upload) has no recorded fetch path and reads exactly as before. The ISSUING authority (D-97) is not read: this section's reading of DEC-75 is that capture grade tracks who SERVED the bytes, not who issued them — stated here for BOB to confirm with the same question.
