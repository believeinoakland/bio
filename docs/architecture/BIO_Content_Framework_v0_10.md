...
| the CONNECTION's determining reference pair | BUILT 2026-09-14 (FW-17, D-161, Bob's 5.4) — the reference on each end that determined the grade, with each end's position where the reading could say, so a PORTION answers a connection grade (Bob's 5.1) instead of the UNDETERMINED it was owed. The ON-POINT choice among several mentions is a member's act and is BUILT at the plane on 2026-09-18 (REC-86, IC-123): `op=narrowcandidates` lists the machine's proposals for one leg — reading positions marked by whether they name the question's subject, EXTRACT proposals, machine-marked rows — every one labelled machine work; `op=narrow` is the member's act, writing a NEW basis version one leg narrower with the old retained. The surface affordance is UI's and not yet built |
...
specificity is worked for rather than merely permitted — is a member's authored act, BUILT AT
...
the LIVE `basis:` block, which is not a version and carries no partition to copy.
