...
**Incomplete sections** ·
- §8.4 — THEMES (Bob's ruling, D-162, 2026-09-21): BUILT IN BOTH HALVES — the plane (D-162, 2026-09-23, IC-241) and the member surface (UI-76, 2026-09-25: the Themes screen and `#theme/<id>`; `node tools/status.mjs 6.themes`). The fence-1 DESIGN GAP this line carried is RULED (BOB #32, 2026-09-24: Membership v2 §3 governs — a handle for every reader, the member id and cover for administrators) and fence 1's text below already says so. NOT BUILT: a "place in a theme" entry on the DOCUMENT page — a member places from the theme's own page by naming a document's id or a passage's content id, which is typing an identifier; and no surface proposes (a member's hunch is reachable only through the API).
...
- §16 — the Google Drive paragraphs are SILENT on the one thing CAP-8 measured that matters most: the export is Google's conversion at fetch time and the conversion is NOT reproducible — three consecutive exports of one unchanged document produced three distinct `capture_sha` (D-351, MEASUREMENTS.md 2026-09-14 §4) — which puts C-18.3's corroboration fold and `resolveLinks`' identity bracket out of reach for Drive material; a design that makes the export the harvest owes a sentence about what that costs the record (a DESIGN GAP reported by CAP-8, folded here at integration, unwritten). **NARROWED 2026-09-24 by D-351 (built, not yet written into §16's body):** C-18.3's fold now reaches `.ods` exports — `profile.digests.evidentiary` is the sha256 of the package's `content.xml`, measured byte-stable across exports (M-123), with `capture_sha` still the envelope's (§5's three digests, the container arm of `substanceDigests`). **NARROWED AGAIN 2026-09-25 by D-473:** `.odt` now carries the digest over content.xml with Google's per-export `text:list` `xml:id` relabelled in document order (`text:continue-list` rewritten to match), measured over a fresh population of public government Docs, two rounds apart (M-167); **and D-612 (2026-09-25) lets it reach the field**: every real Doc export M-167 read embeds its fonts (`Fonts/fontN.ttf`, named by `svg:font-face-uri`), and the member rule no longer counts an href on a `font-face-uri` element — a font face is presentational, the same §5 judgment that discounts `styles.xml` — while a `Pictures/` or `Object N/` reference still refuses. Read off M-167's recorded `refs` column (not re-measured: its population is not re-addressable), 4 of its 8 Docs carry no image and move from UNDETERMINED to a determined digest, all 4 normalised-stable; the other 4 reference `Pictures/` and stay refused. STILL OUT OF REACH: `.odp` (M-167: content.xml raw byte-stable on every pair read, no census target; recorded UNDETERMINED with the reason); `resolveLinks`' bracket (keys on the raw capture sha, D-59). **CORRECTED IN PLACE 2026-09-24 BY D-472** (this bullet's last clause read *and `op=monitor` over a Drive LINK, which fetches the link rather than composing its export address*, and that became false at D-472's landing): the tick now routes `source.locator` through the same recogniser `op=acquire` uses and fetches the OpenDocument export under the governor, so an unchanged `.ods` reads `unchanged` on the container digest where it read `modified` on every tick before; Google's application shell is refused BY NAME on the declared type (C-48.8) and on the bytes (C-48.9) with the look logged LOOKED_INDETERMINATE and the document's own status untouched, and a folder, a kind-less file id and an unread Drive shape are refused before the network. WHAT THE CORRECTION DOES NOT REACH, so the frontier is not read as closed: `.odp` still carries no measured container digest, and `.odt`'s (D-473, reaching the field with D-612) is refused on every Doc that references an image or embedded object, so a Slides deck, or a Doc carrying an image, compares RAW and still reads `modified` whenever Google's conversion differs — which is this bullet's own unreproducible-conversion finding, unchanged.
...
- §16 — **A PAGE AN IMAGE FILLS WHILE ITS TEXT IS A FOLIO: RULED (BOB #35, 2026-09-25 05:50Z) AND BUILT (D-627, M-178).** D-608's reading stands (the page bears text, so no `no_text_layer`), and a second marker, `image_content_unread`, says its content is unread and routes it to OCR. The FY23-25 budget book's 9 pages are routed again. **Still open:** a chart painted as an image under a text TITLE (22 to about 380 glyphs) cannot be told from a photo page by the two figures, so it reads no marker. **BOB #35 RULED at 06:25Z:** the true statement is per IMAGE. **BUILT 2026-09-25 (D-665, M-182):** every painted image above a measured size floor (0.001 of the page; 781 bullets sit under it, every other image over it) carries `image_unread` with its rect and area share, whatever it depicts. Routing that class to OCR stays OFF: D-665 measured both signals BOB named over M-178's 49 pages and NEITHER separates a chart under a title from a photo page. Two consequences downstream were minted: tier 2's merge dropped the marker on a page it wins, BUILT by D-633 (2026-09-25: the base page's image-content markers are carried onto it and re-graded against tier 2's glyph count, BOB #35 08:05Z), and the tier-3 merge will not fill a routed page whose folio decoded (D-635; 8 of the 17 measured pages). BOB ruled D-635 APPEND: that page keeps its layer text, gets the transcription appended, and is listed in both derivation parts. **D-635 is BUILT (2026-09-25, M-180)**, and the body's item (3) names each reader that assumed the parts partition pages. **Closed 2026-09-25 by D-686:** `content.chain_kind` is the unit's own (it had labelled every unit of a mixed document with the whole chain's last step); `capture_text.chain_kind` stays the document's, as ruled.
- §16 — **POSITIONAL TEXT (REC-206, 2026-09-25) is PARTIAL**: tier-1 lines, anchors and the derived membership are BUILT (its bullet in §16); NOT BUILT: position on tier-2 and tier-3 text, a STORED membership, and any item shape but Legistar's gateway addresses. I2's additions are PROVISIONAL until the integrator's IC settles them.
...
- §16 — **TIER 3 NOW READS A SCAN PAST ITS FIRST PAGE (D-606, 2026-09-25), within a per-request budget of 24 member calls** (the tier table's row 3). **The pages past that budget are read on a later request (D-616, 2026-09-25)**: `op=pdfstructure&ocr=1` asks only for the pages the stored reading has not transcribed, advancing by up to the budget each time, and a page kept from the earlier reading stays under the chain of the engine build that read it. NOT BUILT: nothing re-reads the tail automatically (no deferred task); a member asks. The budget rests on Cloudflare's claim of 32 invocations per request, unmeasured on the deployed runtime (M-175).
...
- §18.1 — **the cross-version relation's PROPOSED half is BUILT AT THE PLANE, 2026-09-23 (D-394, IC-239)**: `op=versionnotice` states a newer capture at a cited document's address with certainty where the version chain was read, and offers the extent-match CANDIDATE or UNDETERMINED with its reason, writing nothing (`node tools/status.mjs 4.cross-version`). Its member ASK surface is BUILT 2026-09-25 (UI-96, `4.cross-version-ui`): one control per passage leg on the question page, the four states rendered in the plane's words, asked and never on load. NOT built: the proactive notice to a PUBLISHED case's OWNERS (Bob's ruling of 2026-09-24, option D), which is REC-209's delivery; the office extent arms are reached by the same checker and are UNDRIVEN by its suite; and the section's former open question — whether the notice reaches a member whose case is already PUBLISHED — was RULED 2026-09-24 (owners only, once; §18.1's "What this does not decide"). The build added one state the design's table does not list, *the chain could not be read*, recorded under §18.1's "As built". **REC-220 (2026-09-25) pins a question's basis leg at `op=cite` and states each leg's version on `op=earnedbasis`; a CASE's citation edge, an ACTION's basis leg and a run's suggested version legs still name only a bundle (D-579), and the resolver's earliest-capture order mixes the server's registration instant with a reading's own date (D-580).**
...
- §8.3 — the identifier spaces are **PARTIAL (REC-203, 2026-09-25; C-91; `node tools/status.mjs 6.identifier-spaces`)**: the four recognisers and rule 1–3's counting are BUILT and reached by `op=idmatch`. NOT BUILT: crosswalk capture, any assessor vintage held in the record, a referent read by the plane, a SHARED verdict raising a resolution's grade, and a per-instance system table. **DESIGN GAPS, routed to BOB:** rule 3 names the roll's `apn_sort` as the canonical key, and M-157 measured that column spelled three ways, so the build keys on M-157's parsed-integer key instead; and rule 1's "independent system" has no home in the record — the build derives it from each capture's addresses through a measured Oakland table, and a host serving many offices' publications (www.oaklandca.gov and its storage, where M-157's 24 CIP matches were read) names NO system, so those pairs read SYSTEM_UNDETERMINED.
...
- §14.2 — **the capture grade is MEASURED from the fetch path for BOTH ruled routes (D-177 direct, D-693 archive, 2026-09-25; `node tools/status.mjs 2.capture-grade`)**. BUILT: `earnedBasisRegistry` reads each capture's `captured_locators.via`; a `direct` fetch earns the byte grade bounded by its chain, an archive-only capture earns `ARCHIVE_CAPTURE_GRADE` (C, derived one rank below the ceiling) bounded by the same chain, and every reader of a leg's capture letter reads it at the measured one (§14.2's table). Both questions D-177 raised are RULED (BOB #35, 2026-09-25 07:55Z) and folded into §14.2. UNDETERMINED BY RULING, and named: a via no ruling names (`CAPTURE_GRADE_VIA_UNRULED`) and a capture with no recorded via. ONE DEFINITION (D-698, 2026-09-25): `ARCHIVE_CAPTURE_GRADE` is exported from `checks/bio-checks.mjs` beside the ceiling, and the store's measurement and the letter `op=acquire` STAMPS on an archive capture both read it.
...
- §8.2, §12 — **a decided finding CARRIES its decision on the progression reads and is never hidden by it (D-552, 2026-09-24, IC-290)**: `op=instance` (with its `op=thread` / `op=discharge` echoes) and `op=captureprogressions` publish, on every finding, the decision about its (progression, stage) — state, reason, decider, instant, the version it judged, whether it governs — or `null`, and `op=instance` counts `open_finding_count` beside `finding_count`. The PROGRESSION page paints it (UI-108, 2026-09-25, `progPaintInstance`): each finding keeps its place and carries its decision beside it in the record's words; and so does the DOCUMENT page (D-617, 2026-09-25, `docInstanceHtml`), in the same sentence, for a missing finding and an overdue one alike. NOT BUILT: the document page states no open count, because `op=captureprogressions` publishes none.
- §6 — **WHICH FREQUENCY A MONITOR RUNS AT is BUILT by REC-191 (2026-09-25) but for its first clause, which is a DESIGN GAP**: BOB #31's ruling has an address's OWN frequency govern first, and the record holds no address-level setting (no column, and no op writes one), so `#monitorCadencePlan` reads the current version's authored `monitoring.frequency` and then its contract's default; where an address's own setting would live and who authors it is undesigned (`node tools/status.mjs 10.cadence-address`).
...
BUILT 2026-09-25 by REC-191 (`node tools/status.mjs 10.cadence-address`): `#monitorCadencePlan` schedules each
ADDRESS once, against its current version; that version's authored frequency governs, and where it authored none, the
contract of the type the last tick read at the address (`monitor_address_type`, kept per ADDRESS rather than per bundle)
through `Store.CONTRACT_FREQUENCY`. NOT BUILT: an ADDRESS-LEVEL frequency setting — no column or op holds one, so the
ruling's first clause has nothing to read (a DESIGN GAP, in Incomplete sections above).
...
**BUILT, 2026-09-25 (REC-203, C-91): the recognisers and the counting rule, as PARTIAL.** `bio-plane/src/idspaces.mjs`
holds a recogniser per space (the C.M.S. number with Legistar's floor, reading OUTSIDE THE RECORD'S REACH below it;
the project number's concurrent forms told apart by shape; the fund code; the Alameda APN on M-157's key) and rules
1–3 above as one judgement, `op=idmatch`. Each end's SYSTEM is read from where the record retrieved the capture, never
from the caller; a republication is judged as what it republishes. A fund's name is compared by the plane; every other
referent is a reading the caller supplies, and the answer says it is theirs. Two forms stay UNJOINED (no crosswalk is
captured). An APN is RETIRED only on the assessor's own lineage (BOB #34, 2026-09-25 01:50Z, on M-157; cite until
folded); the record holds no vintage yet, so `op=idmatch` reads every APN UNDETERMINED over none searched.
`node tools/status.mjs 6.identifier-spaces` is the status authority, and the Incomplete sections above name what is not built.
...
connected THROUGH a member's declared lens, visibly theirs. **BUILT, plane half (D-162, 2026-09-23; IC-241):** `op=themedeclare`, `op=themeplace` (membership, graded D), `op=themepropose` (a hunch, graded C, never membership until a member places it), `op=themeread`, the `themes` and `theme_placements` tables, and C-81.1 `THEME_NOT_EVIDENCE` at the basis, version and action-basis leg grammars; `theme` is not an entity kind. **BUILT, member surface (UI-76, 2026-09-25):** the Themes screen lists every theme with its test and its declarer as the plane projected them for the reader, and declares one (nothing prefilled, fence 2's refusal in the plane's words); a theme's page (`#theme/<id>`) shows its test, its declarer, its members and its hunches APART — every hunch under its own label, never as membership — places a document or a passage, and confirms a hunch through `op=themeplace`. `node tools/status.mjs 6.themes` is the status authority.
...
**The capture grade is MEASURED from the fetch path, and only from the fetch path** (D-177,
D-693; DEC-75; BOB #35's two rulings of 2026-09-25 07:55Z, made from doctrine on record). The
record's own fact about who SERVED a capture's bytes — `captured_locators.via`, written by the
fetch that received them — grades it, bounded by its chain as above (DEC-4):

| route the record holds | capture grade | ruling applied |
| --- | --- | --- |
| `direct` — this instance fetched the document's own address | the direct ceiling (`EARNED_CAPTURE_CEILING`), measured | R2-g; DEC-75 |
| an archive replay only (`archive.org`) | one rank below it, `ARCHIVE_CAPTURE_GRADE` (C), measured, the same shape as the direct case and strictly below it | `ARCHIVE-FALLBACK.md`'s two-hop GRADE-C chain; `AUTHORITY-AND-TRUST.md`'s transitive trust "with disclosure and grade adjustment"; grade tracks directness |
| a via no ruling names | UNDETERMINED, named `CAPTURE_GRADE_VIA_UNRULED` | none — never guessed |
| no recorded via (a provenance document's bytes, a member's upload) | UNDETERMINED; the entry reads as it did before D-177 | BOB #35 (1) |

A leg is READ at the measured letter — a weaker authored letter from below, and, where every
capture of the document sits on a measured route, a stronger one from above; the member's
authored letter is kept as written and the reads state the measured one (BOB #35: the write
gate needs no row). **The grade is about the fetch path only** (BOB #35 (2), confirming this
section's reading of DEC-75): the issuing authority's D-97 state is a different axis, one that
gates PUBLICATION, and CPDF-10 forbids composing axes, so a direct capture with an undetermined
authority still earns its measured letter.
...
| the CONNECTION's determining reference pair | BUILT 2026-09-14 (FW-17, D-161, Bob's 5.4) — the reference on each end that determined the grade, with each end's position where the reading could say, so a PORTION answers a connection grade (Bob's 5.1) instead of the UNDETERMINED it was owed. **The pair is the STRONGEST-GRADED mention, not the ON-POINT one Bob ruled** (FW-21, 2026-09-18, driven): the ON-POINT choice is a member's act, is NOT BUILT and is NOT REC-86's (REC-86 narrows a leg, not a connection). **REC-120 (2026-09-18, IC-129) built D-161's acts (1) and (2), NOT the choice:** the pair now SAYS it is the strongest-graded mention (`determining_pair.selection`, `chosen: false`, its tie-break named and recorded on the row as `connections.pair_rule`), and a portion's connection grade answers UNDETERMINED (C-49.4) wherever a definite answer would rest on a machine selection among several mentions — a pair outside the part while another mention is inside or unplaced, or a pair inside it that won only a tie. **REC-122 (2026-09-23, IC-232) built act 3, THE CHOICE:** a member records WHICH mention of the subject is on point on one end of one connection (`op=connectionchoose`, a signed-in member only — a machine credential is refused by name, C-74.1; a mention the document does not carry, C-74.3; a re-choice retains the old), and a portion's connection grade answers FROM THE CHOSEN MENTION where one exists — a definite reach at the chosen mention's grade, a definite outside, or UNDETERMINED under C-49.2 when the chosen mention has no position — while a connection nobody chose on keeps REC-120's UNDETERMINED byte for byte. The machine's strongest-graded pair is kept beside the choice and never rewritten (`on_point` is the member's, `determining_pair` the machine's). **UI-91 (2026-09-25) built the member SURFACE** on the document page's connection display: the machine's pair renders first and always, the member's `on_point` beside it and never in its place, and a signed-in member is offered the document's mentions of the subject; the id/sha arm it read stated no LAPSE, and `selection.says` read *which nobody has chosen* beside a current choice, until **D-575 (2026-09-25)**: every reader of a connection (`op=connections` by id, sha256 and content, and `op=connect`'s answer) now resolves each end's choice through ONE function against what the document reads now, `on_point[side]` carries `lapsed` (and, lapsed, its `why`) on the id/sha arm as on the content arm, and `selection.says` names the chooser, the mention and where it was read, or states the lapse with its why — keeping REC-120's sentence byte for byte only where nobody has chosen (`chosen` stays false: it is about the machine's pair). Still NOT built: a choice from the member SURFACE (UI's, owed). **D-454 (2026-09-25) built the choice subject view, and any way to choose between OCCURRENCES of one reference string**, which this row stated as not built while `reading_refs` held one position per `(capture, ref)`: every reader that kept a reference's first sighting keeps every sighting, `reading_refs` is keyed `(capture_sha, ref, occurrence)` with the reading order in `seq` (a migration re-keys an existing store keeping every row, `measurements/M-155.md`), `op=connectionchoose` names the occurrence and refuses naming only a string read at several places (C-74.4), and the unchosen-mention check (C-49.4) now counts another read of the pair's OWN string as another mention — before it, a page holding a second read of that string answered a definite outside. A choice whose place a re-read drops LAPSES rather than sliding to another occurrence, and a choice made before D-454 is stated AMBIGUOUS once its reference has several places. **D-625 (2026-09-25) made the UNPLACED occurrence choosable**: every unplaced read is the one `''` occurrence and the record lists it by that key, but the act read an `occurrence=` present and empty as none named, so that read could never be chosen and a member was refused C-74.4 for a place the plane itself listed; now an ABSENT `occurrence=` is none named and a PRESENT empty one (in the body or the query) names the unplaced read, whose portion grade answers C-49.2, and an empty one where the document holds no unplaced read is refused C-74.3. **UI-112 (2026-09-25) made the member SURFACE choose an OCCURRENCE**: the document page offers each place `op=readingref` lists for the document's mentions of the subject and sends it as `occurrence=`; C-74.4 renders in its canned words followed by the plane's own keyed `occurrences`; a choice the plane states AMBIGUOUS renders as it says; nothing is preselected. Not built, stated: the page does not offer the unplaced occurrence, which the act accepts since D-625 (D-676), and the id/sha arm's `on_point` carries no position, so the page marks no place of a many-place string as the current choice. **REC-86 (2026-09-18, IC-123) built the LEG-side narrow — `op=narrowcandidates` / `op=narrow`, a new basis version one leg narrower — and that does NOT choose a connection's pair; merged beside FW-21's correction by CONDUCT #4, which declined REC-86's draft of this cell because it claimed the connection-side choice built** |
...
| 3 | the `ocr-worker` fleet member | when a page has no text layer, unless the file is encrypted | renders one page per invocation, transcribes it, answers per-line regions each carrying its image rectangle; **the plane asks again for each page the member DEFERRED, one call per page and in order (D-606, 2026-09-25), up to 24 calls per request — Cloudflare's stated limit of 32 Worker invocations per request (their claim, not measured on the deployed runtime; M-175) less a reserve for the request's other bindings — and a page past that budget, a page whose call throws, a refused page and a page answered under a different engine build each keep their `no_text_layer` marker, stated in the reading's basis, with the capture left a tier-3 candidate. That tail is read on a later request (D-616, 2026-09-25): the re-read (`op=pdfstructure&ocr=1`) keeps every page the stored reading's chain says a tier-3 part transcribed and whose whole text the record holds, asks the member only for the rest — up to the same budget — and states how many remain; a kept page stays under the chain of the build that read it, so a changed engine build yields two scoped tier-3 parts rather than one page filed under another build's provenance. Nothing drives that re-read on its own: a member asks**; the plane merges page-wise and composes the chain from the parts. **WHETHER A PAGE HOLDS TEXT AT ALL IS A GLYPH QUESTION AT EVERY SITE THAT ASKS IT (D-514, 2026-09-24), and two of those sites were making the record claim what it does not hold.** A whitespace-only page was attributed to the `layer` part of the chain, so the record named a tier-1 derivation for a page from which nothing was derived — `legistar-73550` p1 is the real witness, 39 characters and ZERO glyphs (M-140); it now belongs to neither part, which is what this section's own rule already said about a page nobody produced anything for. And both page-wise merges refused a whitespace-only base while SAYING it *already holds N decoded character(s)*, a false sentence in the record's own words; a base holding no glyph now has nothing to lose and takes the tier, and the refusal, where it still fires, is counted and worded in glyphs. **AND WHETHER A PAGE HAS A TEXT LAYER IS WHETHER IT SHOWS TEXT, ASKED BY ONE PREDICATE (D-585, 2026-09-25).** Tier 1's `no_text_layer` marker required "no font declared" and the OCR member's renderer counted a bare `BT` as text, so a scan carrying a font dictionary and an empty `BT … ET` failed both: `0201-cafr-2002` held 161 of its 175 scanned pages as zero characters of TEXT — read and empty — and the member refused them `PAGE_HAS_TEXT_LAYER` (M-157). Both now ask `pageShowsText` (`pdfstructure.mjs`): does a text-SHOWING operator (`Tj`, `TJ`, `'`, `"`) run on the page or in a Form XObject it draws. No font declared stays sufficient on its own; a stream or form that cannot be read answers UNDETERMINED, which marks nothing. Re-read (M-160): 175 of 175 pages marked and admitted |
...
- **Positional text, designed (CPDF-3's gap; M-120).** Tier-1 text units carry no position, and a LinkRecord carries
  ...
  presented as the publisher's own link. **BUILT 2026-09-25 by REC-206 (I2 PROVISIONAL; `node tools/status.mjs
  5.pdf-positional`), in three parts.** (1) **The unit is the tier-1 LINE** — D-481's baseline line, D-502's runs
  inside it — so `text.pages[].lines[]` (`{page, text, rect}`) rejoins to the page's `text` character for character,
  or the page says `lines: null` with `linesWhy`. The `rect` is the union of the line's glyph EM BOXES in the page's
  default user space — the space and `[x0,y0,x1,y1]` order of an annotation's /Rect and CPDF-18's image rect — the
  pen's advance across and one em up from the baseline: not the glyph outline, no descender, no text rise, the font's
  own bbox unread. It is NULL on a line with a glyph the pen could not place (D-502's unknown pen), never a box round
  the glyphs that could be. (2) **The anchor** (`links[].anchor`, `{text, why, tier:1}`) is the glyphs whose ink point
  lies inside the annotation's /Rect, in the order the page showed them; `why` is null only for a complete reading
  and otherwise names `no_rect`, `text_not_read`, `no_text_in_rect`, `positions_unknown`, `partly_unplaced`,
  `undecodable` or `partly_undecodable`. A page tier 2 or tier 3 replaces carries no `lines`, and its links' anchors
  stay tier 1's reading and say so. (3) **The membership** is a SIBLING of `links[]` on `op=pdfstructure`'s answer,
  never inside it (`bio-plane/src/membership.mjs`): item and file links are told apart by the source's OWN address
  shapes (Legistar's gateway today), each item link opens a region from its top edge to the next item link's across
  pages, and a file link whose top edge the region contains is its member; one above the first item is `unplaced`,
  never assigned. The answer and every pair carry `derived: containment, work: machine, asserted_by: system, grade:
  C, standing: inferred, established: false` — §8.1's grade C (correspondence, not identity) and the connections
  table's `system` author are how this document already says *inferred*. On M-120's agenda it reproduces that
  measurement's independent grouping exactly: 41 items, 118 files, 0 unplaced. NOT BUILT, stated: position on tier-2
  and tier-3 text; a STORED membership (a read derives it, nothing persists it); an item shape other than
  Legistar's; a two-column agenda or one painting attachments above their item would be grouped wrongly by the
  top-down rule, which the grade already says is to be confirmed.
...
- **TEXT INSIDE A FORM XOBJECT IS READ AT TIER 1 (D-608, 2026-09-25, M-174).** Until this landing the tier-1 text
  walk interpreted no `Do`, so text a page draws through a Form XObject went unread and nothing said so: ACFR
  FY2023-24 page index 38 read 80 glyphs, its running header, where tier 2 read 546 (M-166). The walk now paints a
  form the way ISO 32000-1 §8.10.1 does and the way CPDF-18's image walk already descended: state saved, the CTM
  premultiplied by the form's `/Matrix`, its content read with its own `/Resources` (the enclosing ones when it has
  none), state restored. That page now reads 546. Over the three held documents M-166 diagnosed, **all 9 pages
  where tier 2 read 10% or more beyond an unflagged tier 1 now read what tier 2 reads**, and of the 1,455 pages under
  10%, 826 now read tier 2's glyph count, 605 are flagged instead (their form text is in a font with no
  `/ToUnicode`), and the 24 left over are ligatures: tier 1 keeps `ﬃ` where tier 2 writes `ffi`. **What the walk
  cannot enter it says, and counts**: a form nested past the depth limit or painting itself is `form_text_unread`,
  with the bytes its text-showing operators carry; an undecodable form stream is `form_stream_undecodable`, count 0.
  `Td` with a zero `ty` now asks the device baseline as `Tm` always has, because a form enters through `cm` and
  `/Matrix` and would otherwise glue its first line to the page's. It moved 0 of 1,840 real pages. **NOT BUILT:**
  `/BBox` clipping, so text a form clips away is read.
- **A PAGE AN IMAGE FILLS WHILE ITS TEXT IS A FOLIO SAYS SO, AND IS ROUTED TO OCR (D-627, 2026-09-25, M-178; BOB #35's
  ruling of 05:50Z).** Two facts, two markers. A page that bears text carries no `no_text_layer` (D-608's reading,
  kept: a false absence is worse than a missing pass). But when the images a page PAINTS (CPDF-18's `images`, stored
  as D-420's `container_extent.images`) cover at least **0.18** of its visible box, and the text it SHOWS (decoded
  glyphs plus the characters its undetermined markers count) is at most **4** glyphs, tier 1 adds
  `image_content_unread` with both figures (`image_share`, `glyphs`), count 0. `needsTier3` and `tier3Pages` route it
  exactly as a no-text page, and `needsTier2` counts it as a scan marker. OCR output stays machine-read and never
  raises a grade (DEC-4). **The thresholds are measured, not chosen** (M-178): over the FY23-25 budget book, the
  adopted budget book and the ACFR (1,788 pages), all 17 pages that paint an image and show at most a folio are images of
  content (tables, a certificate, a screenshot, a chart), shares 0.1897 to 0.6542, and every other page that paints an
  image shows at least 22 glyphs. **The gaps read UNDETERMINED, never forced**: 5 to 21 glyphs, a share under 0.18
  on a folio page, or an unreadable page box gives `image_content_undetermined`, which is NOT routed. No measured page
  falls there. INFO-2026-0301's pages 633, 634 and 645–651 carry the marker and are routed; the text pages are not.
  **NOT BUILT, and each named (item (2) is corrected in place: BUILT by D-633 the same day).** (1) A chart painted as an image under a text title has the same two figures as a photo
  page with captions, so it reads no PAGE marker. BOB #35 ruled at 06:25Z that the true statement is per IMAGE, and
  D-665 built it (its bullet below); only the OCR route for that class stays off, because no measured signal separates
  it. (2) A page
  tier 2 wins lost the marker in `mergeTier2Text`, so it routed nowhere. **BUILT 2026-09-25 by D-633:** the merge
  carries the base page's `image_content_*` markers onto a page tier 2 wins, because they are facts about its images
  and not about the decode; tier 2's own markers are otherwise unchanged, one tier 2 already states is not doubled,
  and a marker counts 0 undetermined characters, so no award moves. Through `op=acquire` with a tier-2 member that
  decodes INFO-2026-0301's folios and wins the pages, every folio page is still sent to OCR. **A CARRIED MARKER IS
  RE-GRADED (BOB #35, 2026-09-25 08:05Z):** its image share is a fact about the images and carries over unchanged,
  but its glyph count must be that of the text the record HOLDS, which is the winning tier's. So the marker is
  re-graded against tier 2's page (decoded glyphs plus the characters its undetermined markers count, D-627's
  measure) by D-627's own thresholds: at most 4 keeps `image_content_unread`, 5 to 21 reads
  `image_content_undetermined` and is no longer routed, and 22 or more drops the marker. The marker never says the
  text is a folio beside a page holding more: a tier-2 page of 30 glyphs is not sent to OCR. (3) `mergeTier3Text`
  refused to fill a routed page whose folio decoded, because the page held a glyph (D-635). BOB ruled APPEND: the
  page keeps its layer text, the transcription is appended, and the page is listed in BOTH derivation parts, because
  D-252's guarantee outranks the parts' partition. **BUILT 2026-09-25 (D-635, M-180).**
- **A ROUTED PAGE WHOSE FOLIO DECODED KEEPS IT AND GAINS THE TRANSCRIPTION (D-635, 2026-09-25, M-180; BOB #35's
  ruling of 06:25Z).** `mergeTier3Text` still FILLS a selected page with no glyph. A selected page that holds a glyph
  now keeps every glyph, and the transcription is APPENDED after its layer text. The routing marker is dropped,
  because the engine has read the content, and the page's other markers stay. A page nobody asked about is still
  refused. The page is listed in the layer part AND the engine's part. When parts overlap, `mergedChain` stamps
  each part's index as `extent.part`, because two parts can share a whole page list (a scanned book with a decoded
  folio on every page) and a page list can no longer name a part. A chain whose parts partition is recorded byte
  for byte as before. **The readers that assumed a partition, each named and driven in `d635-append.test.mjs`:**
  `derivationCap` asked about a page answers by the partition rule when two parts cover it, so a folio page is
  UNDETERMINED (its layer is unmeasured) and never the engine's letter; `derivationCap` about the document names
...
- **EVERY PAINTED IMAGE ABOVE A SIZE FLOOR SAYS ITS CONTENT IS UNREAD, AND THE CHART UNDER A TITLE IS STILL NOT ROUTED
  (D-665, 2026-09-25, M-182; BOB #35's ruling of 06:25Z).** The true statement is per IMAGE and needs no classifier: an
  image is content whose text, if it has any, is unread until a pass reads it, a photo as much as a chart. So tier 1
  adds `image_unread` for every placement CPDF-18 finds, with the placement's `rect` exactly as `images` carries it
  (the same `image {page, rect}` reference) and its `area_share` of the page's visible box, count 0. **The floor is
  measured:** over the 1,104 placements in the three held documents, 781 are 12x12-pixel bullets of about 5.5 points,
  shares at most 0.0000624, and the smallest other image has 0.0079. The floor, 0.001, sits in that gap, nearer the
  bullets, so an image smaller than any measured one is stated rather than hidden. With no readable page box the share
  is null and the image is still stated. The marker is not an undecoded character, so the two readers that judge the
  DECODE by the marker count (`needsTier2` and `readText`'s partial-decode line) are handed the text through
  `decodeView`, which takes these markers out: a page of photos with short captions neither escalates to tier 2 nor
  reads as undecodable. `op=pdfstructure` returns the markers. When OCR fills a page, the page takes the engine's
  markers and the statement is discharged. **ROUTING STAYS OFF, and that is measured, not deferred:** over M-178's 49
  classified pages (17 charts or tables under a title, 12 memo images beside text, 20 photo pages), neither signal BOB
  named separates the charts from the photos. The image's pixels or dpi against the page, its pixels per glyph of the
  page's text, its area against the text's bounding box, and the glyph density outside the painted rects each
  misclassify at least 5 of the 37 chart and photo pages at their best single threshold. So `image_unread` is not a
  tier-3 reason and routes nothing. A signal that would separate them reads the image's PIXELS (a chart's flat colours
  against a photo's continuous tone), which tier 1 does not decode. **NOT BUILT:** a page tier 2 wins drops these
  markers as it drops D-627's, and D-633's carry must take `image_unread` with the `image_content_*` markers.
...
- **Until this landed, M-143's rule governed and it still governs any figure taken before it:** compare two
  ...
  the census judged `op=pdfstructure`'s plain text, which never carries tier 3. **Since D-557 (2026-09-25, M-173)** the
  census judges the acquire reading's own text (its `text_units`, whose digest it checks against the reading's
  `text_sha256`) and names the reader from the producers of the pages judged, never from `text_tier`.
...
§18.1 — its plane half BUILT 2026-09-23, its member ASK surface 2026-09-25, the owners' notice REC-209's), and the TRANSCRIBE surface
...
the cross-version read D-394 built (on its way to `main`). NOT BUILT: the owners' notice (REC-209). The ASK is BUILT (UI-96, 4.cross-version-ui; "As built — 2026-09-25" below). Nothing above changes §14.4, the row's identity,
or the rule that an authored edge moves only by a member's act.

**RULED 2026-09-25 by Bob (00:40Z), in his words:** *"When content from a document is referenced by a basis, claim, or
other construct, that reference continues to refer to that version of the content/document. If an updated version of
the document is found, the member should be notified that an update exists when the update affects the referenced
content, but it's up to the user as to whether they want their reference updated to the new version or to stay
referring to the earlier version."* Three rules follow, and they GENERALISE this section from basis legs to EVERY
reference kind (a leg, a cite onto a case or a question, a claim). (1) **PINNED:** every reference records the capture
it was made against. A reference whose capture cannot be known reads "version undetermined" and is never guessed
(REC-220). (2) **NOTIFIED ONLY WHEN AFFECTED:** the cross-version relation is GRADED as A byte-identical at the extent,
B the same text at a new position, C similar text, NOT FOUND, or UNDETERMINED with its reason (REC-221). A and B are
UNAFFECTED: no notice is raised, and they stay readable here. C and NOT FOUND are AFFECTED, and UNDETERMINED is
notified as undetermined, never passed over in silence. One notice goes to each holder per (reference, newer capture),
and it can be muted (REC-222). Option D above now applies THROUGH THE GRADE: a published case's owners are told once
when a cited part is AFFECTED or UNDETERMINED, not for every newer version (REC-209 is superseded by REC-222).
(3) **THE MEMBER'S CHOICE:** ADOPT writes a new version of the reference pinned to the newer capture, and the old one
stays readable. KEEP records "stays on the earlier version" with who, when and an optional why. Either act closes the
notice, and a machine may take neither (REC-223, surface UI-111). The record never moves a reference by itself.
...
**The grade — 2026-09-25 (REC-221; Bob's 00:40Z version doctrine, rule 2: notify only when the change
touches the referenced PART, and say "undetermined" rather than stay silent).** Each candidate also carries
`grade` and `affects`, and each notice an `affects` roll-up, on CONTENT-EXTENT-DESIGN-SPACE §5.8's
vocabulary: **A** the text at the extent is byte-identical in the newer capture, **B** the identical text is
at another position (`found_at` names it), **C** similar text (a word-multiset Dice at or above a published
floor, `similarity` stated), **NOT FOUND**, and **UNDETERMINED** with `grade_reason`. A and B read
UNAFFECTED, C and NOT FOUND AFFECTED. **The grade is computed from the TEXT the record holds for both
captures (`capture_text`), never from the extent test**: page 3 exists in a revision that rewrote page 3,
so a grade read off extent-match would be the silence the rule forbids. The asymmetry is the design:
UNAFFECTED only on positive evidence; NOT FOUND only where the newer capture's text is held WHOLE (index
PRESENT, no unit truncated) — a partial index, a capture nobody read, a cited extent no unit carries text
for (a sub-page rect, a table, a sheet range) or an image cited as its bytes is UNDETERMINED, by name. The
floor separates only C from NOT FOUND, which are both AFFECTED, so it cannot move whether anyone is
notified. Additive on the wire: `state` and `matched` are what they were. Nothing is pushed yet (REC-222).

#### As built — 2026-09-25 (UI-96, the member ASK surface)

On the question page, every leg whose referent is a passage carries ONE control where the member meets
the citation; using it asks `op=versionnotice` for that passage (`content=`), and nothing is asked when
the page is drawn — asking on load would be the surface telling everyone, the option the ruling did not
take. Every sentence rendered is the plane's own (DEC-8): a matched candidate as the plane's CANDIDATE;
`newer_capture_undetermined` and `chain_unread` drawn in the undetermined primitive, never as the first
row's earned silence; a newer capture nobody has read named by the candidate's own reason, which says it
is a fact about the record; the chains' scope (`visible_to`) on every answer; a refusal in its
translation; a state the surface does not know by its code and sentence. The suite
(`civicos-ui/test/version-notice-surface.test.mjs`) drives the real plane; its control collapses *not
read* into *unchanged* and fails by name. Not reached: the owners' proactive notice (REC-209), and the
published case's public surface, which by the ruling tells the public nothing.

#### As built — 2026-09-25 (REC-220): the reference is PINNED, and the record says whether it can

**Bob, 2026-09-25 00:40Z, rule 1: a reference is PINNED to the version it was made against, and the system never
moves it.** Measured before building (`measurements/M-158.md`): a leg that named only a BUNDLE rested on whatever
`#captureForContent` answered when its question was FIRST projected, and was held there since by REC-82's
carry-forward in `inquiry_basis` — a DERIVED table. The same bytes projected after a later capture rested on
different bytes. So the pin was a fact about the projection's history, not about the record.

- **At the act.** `op=cite` onto a question writes `extent_capture` — the capture every read of the document
  presents at that instant — into each new leg onto a captured document. Not on a leg naming a `content_id` (the id
  hashes its capture), not on a question target (no bytes), not where no capture is held. The receipt names it per
  leg as `pinned_capture`.
- **At the write.** An authored `extent_capture` WINS over a carried referent: the carry-forward is keyed by capture
  for a leg that names one, so a member re-pointing a leg is not overridden by the row about the bytes it left.
- **At the read.** `op=earnedbasis` gives every leg onto a document a `version`: `pinned` (by `extent_capture` or
  `content_id`), `only_capture` (no pin, one capture held), or `undetermined` (no pin, several held), naming the
  capture the projection resolved as `resolved_capture` and never calling it the one the member read. Nothing is
  back-filled.
- **Not pinned yet (D-579):** a CASE's citation edge (`references[]`, `rel: cites`), an ACTION's basis leg, and a
  run's suggested version legs (`op=suggest`) name only a bundle and have no slot for a capture.
