...
- §16 — **A SCAN WHOSE ONLY TEXT IS A FOLIO IN A FORM (D-608, M-174): DESIGN GAP, routed to BOB.** Nine pages of the FY23-25 budget book are page images with a three-digit page number drawn as text inside a Form XObject. Tier 1 used to mark them `no_text_layer`, and `needsTier3` sent them to OCR, because it never saw the form's font. It now sees that text, and it is right that the pages bear some text, so the marker is gone and the tier-1 path no longer routes them to OCR. Tier 2 already treated them this way. No rule yet says whether a page whose text layer is only a folio over a full-page image wants OCR.
