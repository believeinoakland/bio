...
**Status** · The record of the case-making design pass: the frame in Bob's words, the observations, the collapse of focus/finding/case into one recursive INQUIRY, naming and division, what a CLAIM is, the action plan, and the R1–R4 resolutions the adversarial pass forced. "Opened 2026-08-01 (session BOB) as the D-127 design pass Bob directed … Nothing here is settled unless it says so." Its 2026-08-10 status correction rules the file non-authoritative: the collapse HAS SHIPPED, and "what is authoritative is no longer this file. It is the plane, and the rulings the pass produced — DEC-15 through DEC-32 and the R1–R4 resolutions." Superseded as design authority (by the plane, `docs/development/DECISIONS.md`, and DEC-72 for the case section); complete as the reasoning record. Read it for WHY; several body sentences are now false as written (below). **One of them was CORRECTED IN PLACE 2026-09-17 (REC-117) rather than left to the list — *the falsifier is REQUIRED* in §What a CLAIM is, which Bob overruled the same day; a non-authoritative reasoning record may carry stale sentences, but a sentence that states the OPPOSITE of a live ruling misleads a reader about what was decided, and it was load-bearing in an argument.** §CONTRADICTION's IDENTIFY is designed at level 2 (BOB #16, 2026-09-19, `CONTRADICTION-IDENTIFY-DESIGN.md`). **§2 GAINED TWO RULINGS OF BOB'S ON 2026-09-22 (D-148, D-149), folded by BOB #26: a fee quote is EVIDENCE, and a records request names EVERY law that governs the agency asked, by citation. The fee quote is BUILT (D-148, 2026-09-23: a quote on a received correspondence entry, projected to `action_quotes`, read by op=actionquotes); D-149 is BUILT ON BOTH HALVES: the plane's (2026-09-23: `op=actionlaws`) and the MEMBER SURFACE (UI-90, 2026-09-24: the action page's governing-laws list and act, with no default level and the plane's own undetermined sentence) — `construct-status.json` 8.governing-laws — and the MACHINE PROPOSAL, its last NOT BUILT half, is BUILT 2026-09-24 (REC-195): `op=actionlawspropose` stores a proposal of citations and levels apart from the member's list, read labelled machine work, and the list moves only through the member's own act. AND THE MEMBER SURFACE FOR THAT PROPOSAL IS BUILT 2026-09-24 (UI-102), which was 8.governing-laws' last NOT BUILT half: the action page renders each proposal beside the list under the plane's own label, and offers NO control that would set the list from one — adopting is not an act the record has.** §2's `risk_tier` ruling (D-182) is BUILT in the plane, 2026-09-23: `node tools/status.mjs 8.risk-tier`; its write-side fence (a machine credential may not set or change a tier, MACHINE_CANNOT_SET_RISK_TIER, REC-189) is BUILT 2026-09-24, and its member-facing chooser is built on BOTH intake surfaces — `civicos-ui/app.html`'s action intake (UI-85) and the plane's setup page (D-483). **That write-side fence stopped trusting the CALLER'S envelope on 2026-09-24 (D-505): it opened on `meta.object_type` while every reader of the tier derives it from the promoted DOCUMENT's own `object_type`, so a machine credential's action, sent under a `meta` naming another type, landed a stated tier 1 — measured through `op=promote`, with the tier in the bytes, in `action_risk_tier` and findable at `op=search q=risk:1`. The guard now asks both types, as a UNION (IC-266, PROPOSED I3 MAJOR: a refusal where an answer stood, no new code or check). Its declared residue: the `replay` exemption is caller-asserted, pinned by an arm rather than closed, and routed to BOB.** **THE SAME ENVELOPE, ONE STEP OUT, CLOSED 2026-09-24 (D-510, D-505's finding 3): `promote` wrote `bundles.object_type` from the CALLER'S envelope and gated the `action_basis`, `correspondence` and `action_quotes` projections on it, while `#projectRow`'s action columns were already read off the promoted DOCUMENT's own front matter — so an action promoted under an `information` envelope landed TYPED INFORMATION, with its risk tier in the indexed column and its basis and correspondence PROJECTED NOWHERE: the record holding an action it does not index as one. The promoted type is now the DOCUMENT's own through `normalizeType`, at every site that decides what the record SAYS about those bytes, and an envelope stating a type that contradicts it is REFUSED by name (ENVELOPE_TYPE_DISAGREES, C-86.1, a new DEC-49 family; IC-279, PROPOSED I3 MAJOR). A `replay` is exempt from the refusal and NOT from the derivation — the record's own history must be holdable verbatim, but what the record says about those bytes is the bytes' own word — and that exemption remains caller-asserted, which is D-511's.** **AND EVERY FENCE ASKS THE SAME TYPE, 2026-09-24 (D-526):** D-510 parsed the document BELOW the fences, so REC-141's mint, D-85's surfacing gate, 7.1's name scan, REC-181's retirement arm, D-149's carry-forward and C-26's bias gate in the store — and `create_projects`, the migration-replay admission and D-78's `surfaced_by` restamp on the control plane — still asked the ENVELOPE; WHICH refusal a caller met depended on it, and an envelope stating NO type (legal, and outside D-510's refusal) skipped them, so an action creation filling in its own governing laws, a machine's question outside any run, a project created without `create_projects` and owned by nobody, a question keeping the `surfaced_by` its bytes claimed and a malformed bias set all LANDED (measured on 8bdf20e6). The type is now derived ONCE at the top of each plane's promote path, the document's own through `normalizeType`, the envelope only where the document states none, and every one of those reads it; `#projectRow`'s action columns ask through `normalizeType` too. **§2 GAINED BOB #33's RISK-TIER REVISION RULING (2026-09-24), folded and BUILT by REC-214: a member revises a tier by `op=actionrisktier`, an authored, append-only act with a REQUIRED reason, and every earlier tier, its author and its reason stay readable in `risk_tier_history`; `promote` refuses any other writer that moves either (C-90.1, IC-291) — `node tools/status.mjs 8.risk-tier-revision`.** **AND THE LABELLED MACHINE PROPOSAL OF A TIER IS BUILT on the plane 2026-09-25 (REC-215): `op=actionriskpropose` stores a tier and its basis apart from the member's tier and history, read labelled machine work, and never sets it — `node tools/status.mjs 8.risk-tier-proposal`; its surface is not built.** **THE DOCUMENT'S TWO DATES ARE ITS OWN TOO (D-615, 2026-09-25): `promote` derives `created` and `last_updated` from the bytes and refuses a contradicting label (C-86.7) — `node tools/status.mjs 8.action`.** **ITS MEMBER SURFACE IS BUILT 2026-09-25 (UI-104):** the action page shows the tier history and offers the revise act with a required reason, nothing preselected. **§2's RECORDS-REQUEST LIFECYCLE IS BUILT IN THE PLANE 2026-09-25 (D-147):** stages, outcomes and a member-stated due date with its citation on correspondence entries (C-94), read back as one dated chain on `op=projection`'s action block, a due date nobody stated reading UNDETERMINED — `node tools/status.mjs 8.records-lifecycle`. **§2 GAINED BOB #32's LAW-NEUTRAL RECORDS-REQUEST RULING (2026-09-23 23:08Z), folded and BUILT in the plane by REC-201: a `records_request` kind carrying a `law` field read verbatim, `cpra_request` read as written — `node tools/status.mjs 8.records-request`.** **ITS INTAKE CONTROL IS BUILT ON BOTH SURFACES (UI-119, 2026-09-25).** **§2 GAINED BOB #35's ruling (2026-09-25 08:25Z) that a machine may only PROPOSE the law a records request is made under, folded and BUILT by D-689 (C-32.20; pre-fence machine rows read MACHINE-STATED) — `node tools/status.mjs 8.records-law-fence`.** **§2's C-2.10 `law` refusal RUNS AT THE ACT since D-695 (2026-09-25): REC-201 had run it only in the audit sweep, so `op=promote` landed a 250-character law and a law on another kind; `promote` now refuses both as RECORDS_LAW_REFUSED (C-73.6).** as of 2026-09-25.
...
**Incomplete sections** ·
- §2. `action` IS the impact substrate — the fee quote as evidence (D-148) and the governing laws of a records request (D-149), both RULED 2026-09-22, are BUILT in the plane. The fee quote (2026-09-23): C-2.10's quote grammar, `action_quotes`, op=actionquotes; the records-request lifecycle that extends its grammar is BUILT (D-147, 2026-09-25): see §2's lifecycle paragraph; its member surface is not built. The governing laws (2026-09-23): a member sets the list by `op=actionlaws`, a machine is refused by name, `promote` sets it no other way, and an action with none reads UNDETERMINED. The MEMBER SURFACE is BUILT (UI-90, 2026-09-24): the action page lists the governing laws at the levels the plane publishes, offers the act with NO default level, and renders the plane's own undetermined sentence for an empty list — this bullet said it was not built, which was true when written and is corrected here rather than left standing. The MACHINE PROPOSAL of a list is BUILT (REC-195, 2026-09-24): `op=actionlawspropose` writes `action_law_proposals` — citations and levels under the proposer's own name, keyed apart from the member's list and never in the action's bytes — and `op=projection`'s action block serves it beside `governing_laws` under the catalogue's own label, so a machine's proposal is read AS machine work and the list stays the member's until the member acts. This bullet said it was not built, which was true when written and is corrected here rather than left standing. The PROPOSAL'S MEMBER SURFACE is BUILT (UI-102, 2026-09-24): the action page renders `governing_laws_proposals` beside the list, each proposal under the plane's own label sentence and never one of the page's own, and it offers NO control that would set the list from a proposal — the form that act opens still opens on an EMPTY row with proposals standing, because adopting is not an act the record has and a list a member did not author is not the member's. Driven against a real plane by `civicos-ui/test/ui102-laws-proposals.test.mjs`. This bullet said that surface was still owed, which was true when written and is corrected here rather than left standing. The law-neutral records request is RULED (BOB #32, 2026-09-23 23:08Z) and BUILT in the plane (REC-201, 2026-09-25): a `records_request` kind carrying a `law` field, read verbatim; `cpra_request` reads as written — this bullet said it was proposed and not decided, which was true when written and is corrected here rather than left standing. The intake control for `law` is BUILT on both intake surfaces (UI-119, 2026-09-25) — this bullet said it was owed, which was true when written and is corrected here rather than left standing. C-2.10's `law` arm, which UI-119 measured judging only in the audit sweep, RUNS AT THE ACT since D-695 (RECORDS_LAW_REFUSED, C-73.6), and a machine stating `law` is refused since D-689 (C-32.20). Owed and NOT built: the action page reads `action.law` only inside governing_laws' undetermined sentence (D-696). The `risk_tier` ruling (D-182) is built on BOTH intake surfaces: the member UI's action intake offers the published words as a chooser since UI-85 (2026-09-24), and the plane's own setup page since D-483 (2026-09-24) — a radio group over the published vocabulary, unset by default, where a form nobody touched still writes UNDETERMINED. (A machine credential's promote setting or changing a tier is refused by name since REC-189, and since D-505 that refusal reads the PROMOTED DOCUMENT's own `object_type` as well as the caller's envelope — the envelope-only guard let a machine set the tier by calling its action something else. Since D-510, 2026-09-24, the divergent envelope does not reach that fence at all: it is refused above it (ENVELOPE_TYPE_DISAGREES, C-86.1), and `bundles.object_type` and the action projections are derived from the document, so the disagreement D-505 measured — `action_risk_tier` read off the bytes beside an `object_type` read off the envelope — is unreachable for any promotion that is not a replay. D-505's union stays and is still reached where the envelope states no type at all. Owed and NOT built: the `replay` exemption is caller-asserted, so a machine asserting `replay` still lands a stated tier; D-505 pins it in `risk-tier.test.mjs` §7 arm (ix) and routes the doctrine question — may a caller-asserted flag exempt a machine fence? — to BOB.) **A REVISION MAY NOT RETYPE ITS BUNDLE (D-547, 2026-09-25):** `promote` wrote `bundles.object_type` from the revision's derived type with no comparison to the head's, so a revision could turn an action into an inquiry in place and every type-scoped fence then asked the wrong machine; a non-replay revision whose derived type differs from the head's (both through `normalizeType`, so `focus` is not a retype) is now refused REVISION_RETYPES_BUNDLE (C-86.2) before the first write. Replay stays exempt, for D-510's reason. The corpus held NO retyped bundle to report (M-156: 0 of 31 in `bio`, 0 of 17 in `scratch`, 2026-09-25). **A TYPE IS NEVER LOST BY A REVISION, NOR INVENTED FOR A CREATION (D-578, 2026-09-25):** a promotion whose document and envelope both stated no type reached the NOT NULL `bundles.object_type` and answered a raw error with a stack; a revision now carries the head's type (the one C-86.2 admits) and says so (`type_carried`), and a creation is refused PROMOTED_TYPE_UNSTATED (C-86.5). A blank or non-string `meta.object_type` is no statement (before, a blank one landed a bundle typed '  '; M-177: none in the live record). The same raw error on `current_state`, `created`, `last_updated` is closed the same way by D-628 (2026-09-25): a revision stating one nowhere carries the head's (`fields_carried`), a creation is refused PROMOTED_FIELD_UNSTATED (C-86.8) naming each, a blank or non-string envelope value is no statement, and a `meta` sent as a string states nothing (State Rules v1.5 §3.1, §4). **THE TITLE AND THE STATE ARE THE DOCUMENT'S TOO (D-563, 2026-09-25):** D-510's derivation extended to what a document is CALLED and where it STANDS — `promote` derives `title`, `current_state`, `prior_state` (and the `closed_reason` 7.11 reads) from the promoted bytes, the envelope only where they state none, and 7.1's name scan, 7.11's owner test, REC-181's retirement arm, the bias state edge and the projection all read the derived values; a non-replay envelope contradicting the document is refused by name (ENVELOPE_TITLE_DISAGREES C-86.3, ENVELOPE_STATE_DISAGREES C-86.4). (M-172 measured the head rows: 0 title, 0 state and 11 `prior_state` drifts in `bio`.) **AND THE TWO DATES (D-615, 2026-09-25):** `promote` derives `created` and `last_updated` from the promoted bytes too, the envelope only where they state none, for `bundles.created`, `bundles.last_updated`, the manifest row's `created` (the document's `last_updated`) and a minted content row's `at`; a non-replay envelope contradicting either is refused by name (ENVELOPE_DATES_DISAGREE, C-86.7), two spellings of one instant agreeing, and `op=projectfork` now writes the fork's own `created` into its bytes, where before only its label said it. This bullet said the dates were NOT derived, which was true when written and is corrected here rather than left standing. M-181 measured the head rows first: 0 `created` and 0 `last_updated` drifts of 31 in `bio`. The manifest's `created` is still the WRITER's own date, but since D-674 (2026-09-25) no reader orders by it — REC-182's order is write order (`rowid`) — and a promotion stating no date anywhere is refused by name since D-628 (C-86.8, above), where it met the raw NOT NULL error.
BOB.) The member's REVISION of a tier after intake (BOB #33, 2026-09-24) is BUILT in the plane by REC-214 — `op=actionrisktier`, append-only with a required reason. The member SURFACE is BUILT (UI-104, 2026-09-25): the action page shows the tier history beside the governing-laws list and offers the act with a required reason and no tier preselected. The labelled machine PROPOSAL of a reconsideration is BUILT in the plane (REC-215, 2026-09-25: `op=actionriskpropose`, read beside the history, never setting the tier); owed and NOT built: its member surface, beside the tier with no adopt control — this bullet said the proposal was not built, which was true when written and is corrected at the c22-batch30 union. An intake tier's author is not recorded, so the history states it UNDETERMINED.
...
an overclaim on the one field that carries legal exposure. Only a member's authored act sets 1, 2 or 3, and **once a member has set a tier, no machine credential may change it,
not even back to `undetermined`**: erasing an authored assessment is itself an act on it (BOB #32, 2026-09-24,
REC-189's finding). A machine may PROPOSE that a tier be reconsidered, as labelled machine work. **BUILT 2026-09-23 (D-182):** `RISK_TIERS` and `riskTierState()` in the check catalogue, published as `vocabularies.risk_tiers`; an action's read carries `risk_tier` and `risk_tier_words`, and an absent tier reads UNDETERMINED. Actions written at the old default are not back-filled: they read 1, as their bytes say. **FENCED 2026-09-24 (REC-189):** *only a member's authored act* is now enforced at the write — `promote` refuses a machine credential stating a tier of 1, 2 or 3 that differs from the version it replaces (`MACHINE_CANNOT_SET_RISK_TIER`, C-32.19); carrying a member's tier forward unchanged is not refused. **BOB #32 (2026-09-24 01:44Z):** once a member has set a tier a machine may not write it at all — dropping it to `undetermined` is refused by the same name; where no member ever set one, the machine may leave it undetermined. **The member's chooser, BUILT 2026-09-24 ON BOTH INTAKE SURFACES.** *(UI-85)* the member UI's action intake offers `vocabularies.risk_tiers` in the plane's words, `undetermined` preselected and no number a default, and writes a tier only when the member chose a key of the published map. *(D-483)* the plane's SETUP PAGE offers a radio group over `vocabularies.risk_tiers` as published — the labels are the vocabulary's own sentences and the settable keys are derived through `riskTierState`, so `undetermined` is not offered as a choice — with NOTHING preselected, and a form nobody touched still writes `undetermined`. D-483's own note that `civicos-ui/app.html` has no such control is CORRECTED HERE: UI-85 built it on the other side of this integration, and the two landings crossed.

**A MEMBER REVISES A TIER AS AN AUTHORED, APPEND-ONLY ACT, RULED 2026-09-24 by BOB #33** (the "Risk-tier revision"
ruling, sent 21:18Z and recorded in the BOB inbox entry of 21:55Z; folded here by REC-214, which built it). Before
it, no member could set or revise a tier after intake, and a revision by any other route would have left no trace.
The ruling, in full: a member-class act, `actionrisktier`, writes through the ONE front-matter path every reader
derives the tier from; a machine credential is refused `MACHINE_CANNOT_SET_RISK_TIER`; a member may revise ANY tier,
up or down, as an AUTHORED act recording who, when and a REQUIRED reason; and it is APPEND-ONLY — the prior tier,
its author and its reason stay readable in the action's tier history, and nothing is ever silently overwritten.
The reason is the field's legal exposure: the record must show that a "do not file without counsel" tier was
changed, and by whom. **BUILT 2026-09-24 (REC-214):** `op=actionrisktier` (member class, `contribute`) states 1, 2
or 3 with a reason of 1 to 500 characters, writes `risk_tier` in the action's own bytes — so the read, the
`action_risk_tier` column and `op=search q=risk:` agree without a second writer — and APPENDS one entry `{tier,
prior, by, at, reason}` to `risk_tier_history[]`, recording it in the Session Log as well. `riskTierHistoryOf` in
the check catalogue is the one reader: `op=projection`'s action block serves it beside `risk_tier`, oldest first,
and C-2.10 judges the same chain on bytes (each `prior` the tier before it, the last entry the tier stated).
**No other writer moves either:** `promote` refuses a revision that changes the tier or edits the history without
the act, and a creation that states a history (`RISK_TIER_REWRITTEN`, C-90.1; IC-291). A creation may still state
a tier — that is intake — and carrying the tier and history forward unchanged is not refused, for a member or a
machine. `undetermined` is not a tier the act sets: it is what an action reads when nobody has assessed it, the
rule `actionlaws` keeps for an empty list; and a revision to the tier already held is refused, since the history
records changes. **What the record cannot say, stated rather than inferred:** an intake writes its tier with no
author of its own, so the history reads the INTAKE tier's author as UNDETERMINED, in words — naming the action's
creator would be an inference, not a record. A machine may PROPOSE a reconsideration only as labelled machine work,
read BESIDE this history and never written into it (REC-215, a read and never an act in ACTS, as every `*propose`
op is NON_ACTS per REC-195). The member surface that shows the history and offers the act is BUILT
(UI-104, 2026-09-25): the action page renders `risk_tier_history` in the plane's words beside the governing-laws
list and offers `actionrisktier` with a REQUIRED reason and nothing preselected; this sentence said it was not built,
which was true when written. **The
PROPOSAL is BUILT on the plane 2026-09-25 (REC-215):** `op=actionriskpropose` (any credential that may contribute;
the ai class through its minted writes) states a tier of 1, 2 or 3 — judged by the member's act's own grammar, so a
tier outside them meets `BAD_RISK_TIER` (C-90.2) at either door — and a REQUIRED basis of 1 to 500 characters, what
the proposer read the tier from (`RISK_PROPOSAL_BASIS_REFUSED`, C-90.6). It writes `action_risk_proposals`, one
standing proposal per proposer, and never the action's bytes, its `risk_tier` or its `risk_tier_history`;
`op=projection`'s action block serves `risk_tier_proposals` beside the two under the catalogue's own label
(`riskProposalLabel`: machine work, a member's proposal, or unstated), and nothing relates a proposal to the tier,
even when a member later revises to the same number. It is `NON_ACTS` in `affordances.mjs`. A proposal of the tier
already held is not refused: the ruling fences setting the tier, not agreeing with it. **Not built:** the surface
that shows the proposal beside the tier with no adopt control (UI-102's shape for governing laws). A promote under `replay` is exempt from the fence with the whole action block, which is D-511's residue.
...
labelled as machine work, and never sets it. **THE PROPOSAL IS BUILT 2026-09-24 (REC-195),** the half
D-149's own landing left: `op=actionlawspropose` stores citations and levels in `action_law_proposals` under the
credential that proposed them, apart from the member's list and never in the action's bytes, and the action's read
carries them beside the list under the plane's own label (machine work, or a member's proposal, with the published
sentence). **THE MEMBER SURFACE FOR THE PROPOSAL IS BUILT 2026-09-24 (UI-102):** the action page renders each proposal beside the list, inside the same section, under the plane's own label sentence and with no sentence of the page's own — and it offers NO CONTROL on a proposal at all. That absence is the item rather than an omission: there is no act that adopts a proposal, so a "use this" control could only prefill the member's authored statement with a machine's citations, which is the surface making the legal claim the act exists to have a member make. The form that act opens therefore still opens on an EMPTY row with proposals standing against the action. It never sets the list — structurally, because nothing on that path writes bytes at all — and
**nothing relates a proposal to the list a member later states**: that a member acted BECAUSE of a proposal is a
claim about a person that the record cannot support. What a proposal is derived FROM is the proposer's own business:
the plane maps no agency to a law, which is D-149's own rule that it encodes none. An empty list is UNDETERMINED and stated, never a default (the
...
the interface protocol. **RULED 2026-09-23 23:08Z by BOB #32, on D-149's builder's proposal — *DESIGN DIRECTION
ADOPTED — a law-neutral `records_request` kind carrying a `law` field; `cpra_request` stays readable as written* —
because a sovereign group may sit outside California, and a kind that names California's law cannot be its records
request.** Folded here by REC-201, which built it. **BUILT 2026-09-25 (REC-201):** `records_request` is a kind in
`ACTION_KINDS`, published in `vocabularies.action_kind`; the law it is made under is the action's own `law` field, a
citation read VERBATIM by `recordsLawOf` onto the action's read (`law`: stated, or UNDETERMINED in words where none is
written, never a default). The `law` field is the KIND's statement one layer out, exactly as `cpra_request`'s name is:
it is NOT the governing-laws list, which stays undetermined until a member states it by `op=actionlaws`, and whose
undetermined sentence names the stated `law` as its author's statement and infers nothing else. `cpra_request` is not
migrated and no read rewrites it: its `law` answers `kind`, no citation is synthesised from it, and D-149's sentence
stands byte for byte. C-2.10 refuses a `law` on any other kind (it would state a law no read shows) and one longer than
a citation — AT THE ACT: `op=promote` refuses either as RECORDS_LAW_REFUSED (C-73.6, D-695), carrying the arm's own
sentences, for a member's write and a machine's alike (REC-201 ran the arm only in the audit sweep, and the write landed it). **THE INTAKE CONTROL IS BUILT 2026-09-25 (UI-119):** the app's action intake draws a `law` field when a member
chooses `records_request`, empty and naming no law (DEC-69), and writes it on that kind only; the setup page, which
wrote every action as `other`, gains a kind chooser over the catalogue's own list and the same field. (UI-119 measured
the arm running only in the audit sweep, which is the defect D-695 above fixed; a surface now shows the refusal the act makes.) **A MACHINE MAY ONLY PROPOSE THE LAW, RULED 2026-09-25
08:25Z by BOB #35, (b) FENCE BOTH, from DEC-24 and D-149:** *which law governs a request is a characterization the
member makes and answers for* — so a machine credential may neither state `law` on a `records_request` nor create a
`cpra_request`; it may only PROPOSE either, as REC-195 does for the list, and a member's act adopts the proposal.
Existing rows are not rewritten: a `cpra_request` a machine created reads MACHINE-STATED from its recorded author
class, never as a member's statement (D-546's history rule). **BUILT 2026-09-25 (D-689):** `promote`'s action block
refuses a machine credential (REC-46's one predicate) whose write CHANGES what the action states about the law its
...
or `undetermined`), read from `manifest.author` of the version that INTRODUCED the current statement — not the
creator, not the last writer: a member revising a machine `cpra_request`'s plan carries its statement and does not
adopt it — and a machine-stated `cpra_request` or `law` says MACHINE-STATED in both `law.stated` and
`governing_laws.stated`; a member's reads exactly as before. The walk is bounded at 64 versions and says
`undetermined` past it. A quote is read "by" the action's own counterparty name,
matched EXACTLY; when nothing matches, it is stated as undetermined (D-148's gap).
...
judged against the action's governing laws WHEN IT IS STATED, at the op only: a document-level rule would make a
later change to the list refuse the action's own history. Not built: a member surface, and a cross-action index of
passed due dates (the chain is read per action from its bytes).
