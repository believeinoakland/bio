...
there reads undetermined; and whether a machine credential may state `law` at intake is not ruled — it may state
`cpra_request` at intake today, which is the same statement. A quote is read "by" the action's own counterparty name,
