...
**Incomplete sections** ·
- §2. `action` IS the impact substrate — the fee quote as evidence (D-148) and the governing laws of a records request (D-149), both RULED 2026-09-22, are BUILT in the plane. The fee quote (2026-09-23): C-2.10's quote grammar, `action_quotes`, op=actionquotes; the records-request lifecycle that extends its grammar (D-147) is not built. The governing laws (2026-09-23): a member sets the list by `op=actionlaws`, a machine is refused by name, `promote` sets it no other way, and an action with none reads UNDETERMINED. Still owed under D-149: the machine PROPOSAL of a list (labelled machine work) is not built, the member surface is not built, and whether the kinds gain a law-neutral records request is PROPOSED, not decided (`cpra_request` is unchanged). The `risk_tier` ruling (D-182) is built except the chooser on the plane's own setup page, which writes UNDETERMINED and offers no control; the member UI's action intake offers the published words as a chooser since UI-85 (2026-09-24). (A machine credential's promote setting or changing a tier is refused by name since REC-189.)
