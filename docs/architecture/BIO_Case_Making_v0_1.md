...
**Incomplete sections** ·
- §2. `action` IS the impact substrate — the fee quote as evidence (D-148) and the governing laws of a records request (D-149) were both RULED 2026-09-22, and **this bullet's claim that neither is built is CORRECTED 2026-09-24 (UI-90) rather than left standing: it was already false for D-148 and D-149's plane half when they landed on 2026-09-23, and a front-matter line saying NOT BUILT over built work misleads every reader who comes here to find out what is missing.** D-148 is BUILT (a quote on a received correspondence entry, projected to `action_quotes`, read by `op=actionquotes`). D-149 is BUILT on BOTH halves: the plane's `op=actionlaws` (2026-09-23) and the member surface (UI-90, 2026-09-24 — the action page lists the governing laws at the levels the plane publishes, offers the act with NO default level, and renders the plane's own undetermined sentence for an empty list). What remains unbuilt under D-149 is the MACHINE PROPOSAL of the list, which the ruling licenses and nobody has built; `node tools/status.mjs 8.governing-laws` is the live answer. The `risk_tier` ruling (D-182) is built, and its member-facing chooser now on ONE of the two intake surfaces (D-483, 2026-09-24): `bio-plane/src/setup.mjs` offers the published words as a radio group, unset by default, and unset still writes UNDETERMINED. `civicos-ui/app.html` still writes UNDETERMINED and offers no control.
...
REC-189's finding). A machine may PROPOSE that a tier be reconsidered, as labelled machine work. **BUILT 2026-09-23 (D-182):** `RISK_TIERS` and `riskTierState()` in the check catalogue, published as `vocabularies.risk_tiers`; an action's read carries `risk_tier` and `risk_tier_words`, and an absent tier reads UNDETERMINED. Actions written at the old default are not back-filled: they read 1, as their bytes say. **THE MEMBER'S CHOOSER IS BUILT ON THE SETUP PAGE 2026-09-24 (D-483):** a radio group over `vocabularies.risk_tiers` as published — the labels are the vocabulary's own sentences and the settable keys are derived through `riskTierState`, so `undetermined` is not offered as a choice — with NOTHING preselected, and a form nobody touched still writes `undetermined`. `civicos-ui/app.html` has no such control yet.
