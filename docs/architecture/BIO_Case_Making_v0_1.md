...
**Incomplete sections** ·
- §2. `action` IS the impact substrate — the governing laws of a records request (D-149), RULED 2026-09-22, are NOT BUILT: an action names no law. The `risk_tier` ruling (D-182) is built except its member-facing chooser: both intake surfaces write UNDETERMINED and neither yet offers the published words as a control. The fee quote as evidence (D-148) is BUILT as of 2026-09-23 (C-2.10's quote grammar, `action_quotes`, op=actionquotes); the records-request lifecycle that extends its grammar (D-147) is not.
