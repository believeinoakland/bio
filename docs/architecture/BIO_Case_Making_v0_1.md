...
a citation. **THE INTAKE CONTROL IS BUILT 2026-09-25 (UI-119):** the app's action intake draws a `law` field when a member
chooses `records_request`, empty and naming no law (DEC-69), and writes it on that kind only; the setup page, which
wrote every action as `other`, gains a kind chooser over the catalogue's own list and the same field. MEASURED BY
UI-119 and NOT BUILT: C-2.10's `law` arm runs only in the audit sweep, so `op=promote` LANDS a law on another kind or
longer than a citation and no surface can show a refusal the act never makes (D-695). And whether a machine credential may state `law` at intake is not ruled — it may state
