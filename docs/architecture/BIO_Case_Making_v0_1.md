...
**Incomplete sections** ·
- §2. `action` IS the impact substrate — the fee quote as evidence (D-148) and the governing laws of a records request (D-149), both RULED 2026-09-22, are NOT BUILT: a quote is still correspondence prose and an action names no law. The `risk_tier` ruling (D-182) is built, and its member-facing chooser now on ONE of the two intake surfaces (D-483, 2026-09-24): `bio-plane/src/setup.mjs` offers the published words as a radio group, unset by default, and unset still writes UNDETERMINED. `civicos-ui/app.html` still writes UNDETERMINED and offers no control.
