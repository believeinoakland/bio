...
an overclaim on the one field that carries legal exposure. Only a member's authored act sets 1, 2 or 3. **BUILT 2026-09-23 (D-182):** `RISK_TIERS` and `riskTierState()` in the check catalogue, published as `vocabularies.risk_tiers`; an action's read carries `risk_tier` and `risk_tier_words`, and an absent tier reads UNDETERMINED. Actions written at the old default are not back-filled: they read 1, as their bytes say. **FENCED 2026-09-24 (REC-189):** *only a member's authored act* is now enforced at the write — `promote` refuses a machine credential stating a tier of 1, 2 or 3 that differs from the version it replaces (`MACHINE_CANNOT_SET_RISK_TIER`, C-32.18); carrying a member's tier forward unchanged, or stating none, is not refused.
