...
sentences, for a member's write and a machine's alike (REC-201 ran the arm only in the audit sweep, and the write landed it). NOT BUILT: an intake control for `law` on the app's intake or the setup page, so a `records_request` filed
there reads undetermined (UI-119 owns the member's control). **A MACHINE MAY ONLY PROPOSE THE LAW, RULED 2026-09-25
