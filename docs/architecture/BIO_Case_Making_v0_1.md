...
**Incomplete sections** ·
- §2. `action` IS the impact substrate — the fee quote as evidence (D-148) and the governing laws of a records request (D-149), both RULED 2026-09-22, are NOT BUILT: a quote is still correspondence prose and an action names no law. The `risk_tier` ruling (D-182) is built except the chooser on the plane's own setup page, which writes UNDETERMINED and offers no control; the member UI's action intake offers the published words as a chooser since UI-85 (2026-09-24).
