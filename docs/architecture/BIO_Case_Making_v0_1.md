...
op is NON_ACTS per REC-195); the member surface that shows the history and offers the act is UI-104's. **The
...
that shows the proposal beside the tier with no adopt control (UI-102's shape for governing laws), and UI-104's. A promote under `replay` is exempt from the fence with the whole action block, which is D-511's residue.
