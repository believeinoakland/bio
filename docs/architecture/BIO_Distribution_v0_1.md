...
- §6 rung 4 — the plane's `limits.subrequests` (D-54, 2026-09-23; `construct-status.json` 15.subrequest-limit, PARTIAL) is stated in `bio-plane/wrangler.jsonc` with its reason and carried by `deploy.mjs`, which reads it back from the script's settings; NO deployed plane carries it yet (biosmoke7's settings read 2026-09-23 state no `limits`), whether those settings report the field once set is UNDETERMINED until the first deploy, and the installer's plane upload sends no `limits`, so a group's instance runs at Cloudflare's default until DIST carries it there.
