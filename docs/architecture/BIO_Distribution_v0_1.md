...
**Status** · v0.1 DRAFT, written 2026-09-14 by session BOB #11 as the level-1 home of construct 15 of `BIO_System_Design.md` §3 (distribution: installer, releases, fleet, multi-instance), which that map named as having no level-1 document describing the deployed topology. Awaiting Bob's review. It RULES NOTHING: the rules below are restated from where they were ruled — D-297 (closed), D-292, D-106, D-108, D-118, DEC-42, `CLAUDE.md`'s account and release sections, `PARALLELISM.md`'s fleet rules — and the mechanisms are those `newgroup`, `deploy.mjs`, the fleet build and `release/RELEASE.json` implement at 0.58.0. Completeness: complete at its level for the topology as deployed; multi-instance isolation is PLANNED NOT BUILT and sequenced after the member surfaces (§7); the fleet reaching a sovereign group is Bob's gate (§4). The one caveat: `BIO_Technical_Architecture_Decisions_v10.md` still carries the general rules this construct runs under (integrity, the manifest contract, promotion posture) and explicitly disclaims its own retired mechanisms; this document is their live replacement for distribution and cites the rules rather than restating them. §6 rung 6 gained what the scratch sweep takes (BOB #16, 2026-09-19) and the namespace limitation D-325 leaves as (BOB #22, 2026-09-21); §6's D-260 bullet now points at the assistant's ruling. §5 gained the update path's D-436 telling — the one act an update leaves its operator (DIST #4, 2026-09-22). §6 rung 8 and §8 gained the RUNTIME half of the fleet's version authority — the DO's and each member's own build read back and a lagging part named (D-116, IC-182, 2026-09-23). §5 and §8 gained the installed plane's BINDING to its members and the three-act upload order a binding cycle forces (DIST-6, 2026-09-23). §6 rung 4 gained the plane's SUBREQUEST CEILING as a stated decision carried and read back by the release path (D-54, 2026-09-23). §6's D-260 bullet now names the secret the plane reads, `INSTANCE_AI_TOKEN`, since the plane's caller landed (D-260, 2026-09-23); carrying it through install and update is BUILT (DIST-9, 2026-09-24): carried when the operator supplies it, never generated. §6 rung 6 now carries the MINTED credential's sticky confinement to `scratch`, which supersedes the limitation BOB #22 stated there on that ruling's own condition (D-463, 2026-09-24; C-78.3, C-29.10); BOB's ruling on whether it is doctrine is owed. §6 rung 6 gained the RELEASE'S OWN CANARY speaking its verdict: `op=livefire` answers `ok` for the op having ANSWERED, with the result in `verdict` and the failed assertions named in `failing` (D-506, IC-265, on BOB #32's ruling of 2026-09-24 06:07Z of the question D-495 raised; `construct-status.json` `15.livefire-verdict`). as of 2026-09-24
...
- §6 rung 6 — a MINTED credential can be bound to the scratch namespace for its whole life (D-463, 2026-09-24), on the condition BOB #22's 2026-09-21 limitation named, met twice (D-456, D-461); BOB owes the ruling on whether it is doctrine, and the design for it is this rung rather than a document written ahead of the landing. The four BINDING credentials still cannot be confined, so for them this rung's no-write guarantee remains the per-call `store=scratch` plus the witness.
...
- §5 — the installed plane's member bindings (DIST-6, 2026-09-23; `construct-status.json` 15.member-bindings, BUILT in the installer's SOURCE) are verified against the wizard suite's stateful fake account only, which models Cloudflare's 10143 refusal as this project measured it (`tools/deploy-fleet.mjs`, 2026-08-10 / 2026-09-10). Two acts are NOT LIVE-VERIFIED and are read back at the first real install and update after DIST's next cut: that the three-act order is accepted end to end, and that the install's closing re-PUT, in the update's `keep_bindings` shape, keeps the passwords and the Durable Object it does not restate. Until that cut no group's installer carries the bindings.
- §6 rung 4 — the plane's `limits.subrequests` (D-54, 2026-09-23; `construct-status.json` 15.subrequest-limit, BUILT) is stated in `bio-plane/wrangler.jsonc` with its reason and carried by `deploy.mjs`, which reads it back from the script's settings: biosmoke7 carries it since 0.79.0 (`/settings` read back 10000, 2026-09-24). The installer sends it on install and update since DIST-7 (2026-09-24), pinned to the plane's config by the wizard suite; it reaches a group's instance with the next release. NOT BUILT: carrying it at RUNTIME from the signed release — the manifest has no plane-limits field.
- §6 — D-260 is RULED (BOB #22, 2026-09-21; `BIO_Assistant_and_AI_Roles_v0_1.md` §6): an instance may hold ONE organisation-principal `ai` credential as a deploy secret, and it resumes only the runs that credential opened. Carrying that secret through install and update, as `DAEMON_TOKEN` is carried, is this construct's half, BUILT 2026-09-24 (DIST-9; `construct-status.json` `15.instance-ai-secret`): the installer's update page takes an OPTIONAL operator-supplied value, both upload paths bind it only when one is supplied, and `deploy.mjs` sends it only when its environment holds it. It is NEVER GENERATED, because a member mints it on the copy (DS-3): none supplied sends none, a fresh install holds none and says so, and an update keeps a value the copy already holds (`keep_bindings`: `secret_text`). The PLANE half is built (2026-09-23, IC-242): the plane reads the Worker secret `INSTANCE_AI_TOKEN` (`tokens.mjs`, denylisted on publication) and, on a copy holding none, every wake says `NO_INSTANCE_AI_CREDENTIAL`. It reaches a group only with the next release (none until Bob asks).
...
`newgroup` is a Worker a group opens in a browser. It carries the release embedded (the embed step, D-106's refusal) and, step by step, with each step an emitted id: `auth` (OAuth into the group's account), `acct`, `fresh` (refuse if a plane of that name already exists), `plan` (probe and require Workers Paid — refuse rather than half-install, DIST-3), `r2` (the two buckets, per instance), `gen` (credentials), `install` (the plane, byte-verified on read-back), `fleet` (each member fetched, verified against the signed statement, uploaded — §4), `bind` (the plane re-uploaded bound to each member now present), `addr` (the subdomain), `verify` (the whole read back). An update path mirrors it.

**The plane is bound to its members in three acts, and the order is forced** (DIST-6, 2026-09-23). Cloudflare refuses an upload whose service binding names a worker the account does not hold (code 10143 — measured by this project, `tools/deploy-fleet.mjs`, 2026-08-10 and 2026-09-10), and the plane and `agent-worker` bind each other (`AGENT_WORKER` one way, `PLANE` the other), so neither can go first holding the binding. So: (1) the plane is uploaded bound to the members the account ALREADY holds — none on a fresh account, all of them on an update of a copy that has them, which is what keeps an update from un-binding them (`service` is not kept by `keep_bindings`); (2) the members are uploaded, their `PLANE` binding now resolving; (3) the plane is re-uploaded, in the update's shape, bound to every member now present. The binding names are the plane's own (`FLEET_BINDINGS`), the target each member's own script name. A member whose upload fails is left unbound — never bound to nothing — and the verify step names it; neither act reports success over it. Its contract with the account is deliberately minimal and stated: the installer's own configuration carries `bindings: []` — it holds nothing of the group's, and the wizard suite asserts it — and it is deployed by dashboard paste (`newgroup/DEPLOY.md`), the structural guarantee being that empty binding list.

**The one act an update leaves to its operator** (DIST #4, 2026-09-22, D-436's DELEGATION to DIST, item 3). Since IC-172 a
copy records which group produces its record ONCE — at its store's first boot from the `INSTANCE_NAME` the install PUT
binds, or, for a store that already held a record when the value arrived, by its root of trust's `op=instancegroupseed`.
An update of a copy that ran a release before 0.71.0 therefore TELLS the operator that act (for the record and for
scratch), what its copy refuses until it is done (`GROUP_UNDETERMINED`), and the installed name as a SUGGESTION only —
a copy's worker name need not be its group's slug (this project's copy is `biosmoke7`; its group, `believe-in-oakland`).
It never performs the act (D-436's decision (b)), and it decides from the version it reads before the upload rather than
from `op=instancegroup`, which answers only a credential an update does not hold; when that version is unknown the
telling is conditional and says so.
...
4. **The bytes are read back and hashed against the signed asset**; *"this never reports success from what the API said … That comparison is the only thing here that decides anything."* Byte-identical and already SERVING the target version is *nothing to do*; byte-identical alone is a metadata deploy and proceeds. **The metadata includes the subrequest ceiling** (D-54): `limits.subrequests` is a figure the plane states with its reason in `wrangler.jsonc`, never Cloudflare's default of the month; `deploy.mjs` refuses a config without it, reads the deployed script's settings back, refuses success on a figure not its own, and states UNDETERMINED — never a match — when the settings do not say.
...
6. **Live verification in the scratch namespace, never the real record, swept after; `op=audit` clean** — then the version is serving.
   **What "swept after" means (BOB #16, 2026-09-19, on FLEET's measurement at biosmoke7; row M0-69):** the sweep is a
   whole-store `op=purge` with `confirm=scratch`, and on the SCRATCH store it takes the scratch store's IDENTITY as well —
   `members`, `admin_votes` and every member-keyed table in `schema.mjs`, enumerated from the schema and pinned against it
   the way the record list is. Why: `#activeAdmins` and the administrator-consensus count read `members`, so identity left
   behind by one live run changes the next run's membership arithmetic, and a live verification stops measuring the same
   subject twice (FLEET found 7 VF-4 member rows, 6 `proposed`, surviving a purge). **A purge of the RECORD store never
   touches identity, structurally** — the control plane passes the identity flag only when the resolved store is scratch —
   ...
   **THE RELEASE'S OWN CANARY SPEAKS ITS VERDICT, AND `ok` IS NOT IT (D-506, IC-265; BOB #32, 2026-09-24
   06:07Z).** `op=livefire` is the battery this rung fires at a deployment, and until this ruling its `ok`
   was a VERDICT over its own assertions (`ok: A.every(...) && r2.ok`, dispatched `200 : 500`). So a canary
   that found a real defect answered `ok:false` carrying no `reason`, no `code` and no `error` sentence of
   any kind — one layer further out than D-270's bare-sentence residue, which at least carried words — and
   every consumer that reads `ok:false` as a refusal, the catalogue-free `agent-worker` included, received
   from the one op whose job is to SPEAK something it could neither name nor translate. D-495 found it and
   raised it as the interface question D-270's boundary makes it, and did not settle it. It is settled here:
   **`ok` says THE OP ANSWERED, and `ok:false` is reserved for a catalogued refusal**; the canary's result is
   **`verdict`** (`pass` / `fail`) and every failed assertion is NAMED in **`failing`**. The HTTP status is
   keyed to the verdict and is byte-for-byte what it was for every outcome, so a gate reading the status
   alone is not silenced by the move. The rung's reading is unchanged in substance and exact in form: this
   rung is satisfied by `verdict: "pass"`, never by `ok`, and a `fail` is read from the names in `failing`.

   **For the four BINDING credentials, which namespace a call lands in is chosen PER CALL (D-325; BOB #17 ruled the
   posture SUFFICIENT 2026-09-19, and BOB #22 stated its residue 2026-09-21).** `scopeFor`
   (`bio-plane/src/index.mjs`) confines only the PROBE class, by refusal; ADMIN, MEMBER and DAEMON — the last on purpose
   (REC-33) — land in `bio` unless that call names `store=scratch`. So for those four this rung's no-write guarantee is
   the naming on every call plus the WITNESS, the record's counters read before and after every arm (`CLAUDE.md` §5).
   **A MINTED credential CAN now be bound to scratch for its whole life (D-463, 2026-09-24; C-78.3, C-29.10;
   `construct-status.json` `1.namespace-confined`), AND THAT IS THIS RULE'S OWN CONDITION BEING MET RATHER THAN A
   REVERSAL.** BOB #22 stated the limitation and named the one condition on which a sticky confinement would be raised:
   *"raised only if a live verification is measured writing the real record despite the naming and the witness"*. It was
   measured twice — D-456 (`store=biosmoke-pdf`, a namespace that has never existed, answered from `bio`) and D-461
   (`op=knock&store=scratch` filed a knock in the REAL record's inbox) — each found by a worker whose every call was
   disciplined, which is to say the discipline was not the thing that failed. So `op=aicredentialmint` takes an optional
   `confinedTo`, the record carries it in `ai_credentials.confined_to` (nullable, never back-filled), and
   `confinedNamespaceGate` holds every call that credential makes to `scratch` at the front door — refusing a named
   `bio` BY NAME and setting an absent `store=` to `scratch` — with `scopeFor` untouched, so there is ONE decider and
   not two that can age apart. **The four binding classes still cannot be confined:** they are values an operator sets
   in the hosting dashboard, with no row to carry the property, so the per-call obligation above binds every caller
   holding one. **BOB'S RULING ON WHETHER THIS IS NOW DOCTRINE IS OWED** (routed by D-463's worker, 2026-09-24): the
   ruling's condition is met and the mechanism runs, but the ruling itself said a sticky confinement would come *"with
   its own design"*, and this rung is where that design now sits — written by the landing rather than ahead of it.
7. **Per member, the same ladder** (`tools/deploy-fleet.mjs`, one at a time), because each member rolls out on its own.
8. **Each part's OWN build, read where it runs** (D-116, IC-182, 2026-09-23). `op=bootstrap`'s `version` is the ROUTING isolate's; the Durable Object reports its own as `storeVersion`, from its own env and never from the request or the isolate; and `op=bootstrap&members=1` adds `memberVersions`, each member's own `/version` asked THROUGH the plane's binding (`SERVING` / `UNBOUND` / `SILENT` / `MISNAMED`). The installer reads all three after an install and after an update and names the part not on the release; neither reports success while one lags. A release whose plane predates the fields states those builds UNDETERMINED — never a match.
...
| which build the DO and each member SERVE, read back and a lag named — the runtime half of the version authority | BUILT (D-116, IC-182, 2026-09-23; §6 rung 8); reaches a group only with the next cut that carries it |
| the installed plane bound to the members installed beside it | BUILT in the installer's source (DIST-6, 2026-09-23; 15.member-bindings; §5's three-act order), found ABSENT by D-116's worker the same day; reaches a group only with the next cut, and NOT LIVE-VERIFIED until then |
...
| an instance-held `ai` credential; a woken run re-entered | RULED 2026-09-21 (D-260; the assistant's §6); the plane READS `INSTANCE_AI_TOKEN` and re-enters the runs it opened (2026-09-23, IC-242); install and update CARRY an operator-supplied `INSTANCE_AI_TOKEN` and never generate one (DIST-9, 2026-09-24, `15.instance-ai-secret`); reaches a group with the next release |
