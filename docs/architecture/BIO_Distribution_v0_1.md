...
- §6 — D-260 is RULED (BOB #22, 2026-09-21; `BIO_Assistant_and_AI_Roles_v0_1.md` §6): an instance may hold ONE organisation-principal `ai` credential as a deploy secret, and it resumes only the runs that credential opened. Carrying that secret through install and update, as `DAEMON_TOKEN` is carried, is this construct's half, and it is NOT BUILT (`construct-status.json` `15.instance-ai-secret`, ABSENT, whose `none` search is the tripwire). The PLANE half is built (2026-09-23, IC-242): the plane reads the Worker secret `INSTANCE_AI_TOKEN` (`tokens.mjs`, denylisted on publication) and, until an install places it, every wake says `NO_INSTANCE_AI_CREDENTIAL`.
...
| an instance-held `ai` credential; a woken run re-entered | RULED 2026-09-21 (D-260; the assistant's §6); the plane READS `INSTANCE_AI_TOKEN` and re-enters the runs it opened (2026-09-23, IC-242); NOT BUILT: the deploy carries no such secret yet (`15.instance-ai-secret`) |
