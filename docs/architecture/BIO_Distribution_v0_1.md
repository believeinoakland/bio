...
- §4 / §5 — the plane the installer uploads holds NO service binding to the members it uploads beside it (found by D-116's worker, 2026-09-23; `construct-status.json` 15.member-bindings, ABSENT): a sovereign copy's members are installed but unreachable from its plane. Since D-116 the verify step NAMES each such member rather than reporting success; binding them — and the upload order a binding to a not-yet-uploaded script needs — is DIST's to build.
...
| the installed plane bound to the members installed beside it | ABSENT (found by D-116's worker, 2026-09-23; 15.member-bindings): uploaded but unreachable from the plane — NAMED at verify since D-116 |
