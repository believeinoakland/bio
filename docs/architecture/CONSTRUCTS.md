...
**Status** · A dated inventory of the document-profile and content constructs — where each lives, what consumes it, eight overlaps and conflicts, the connection-table brief, and a ten-step consumer-driven plan. "Written 2026-07-30, at Bob's direction, after he asked whether the development work had become too diffused"; "It is not a design document. It is the inventory that has to exist before a design document can be written honestly." One RULED marker (Bob, 2026-07-30, Step 0's scope). Completeness: an inventory at a date — the tables are the 2026-07-30 measurement and were not re-measured as the FW steps landed; read the plan section's BUILT markers, not the tables, for what is true. Step 5a's measurement was taken once on 2026-09-23 (D-74, M-119); nothing it feeds is built. as of 2026-09-23.
...
| confirmation (identical_bytes / same_substance / intact counts) | pipeline + content type | `op=monitor`'s `assessment` (D-65) and the observation log's `PRESENT unchanged` row | answered and logged |
| monitor frequency by kind | `CONTRACT_FREQUENCY` in `bio-plane/src/index.mjs` (D-65, provisional intervals) | `op=monitor`'s `cadence` answer | built at the tick; not read by REC-26's scheduler |
...
*Measured once, 2026-09-23 (D-74, `docs/development/measurements/M-119.md`)*: from the record's own
...
not read. A first sample, not a census; M-119 names the four measurements still owed.

**Step 6 — BUILT AT THE TICK (D-65, 2026-09-23; the cadence scheduler's half is not). Monitoring adopts the contracts,** with frequency by document kind, and
