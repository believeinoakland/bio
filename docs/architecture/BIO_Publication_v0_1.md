...
- §3 rule 2 / §4 — `op=ratify` still publishes three things OUTSIDE a case (an information bundle, an inquiry in no case, a finding prepared into a case not yet ratified). DECIDED 2026-09-19 by BOB #16 (rule 2's second note: nothing crosses except a finding a ratified case pins, or evidence such a finding rests on) and NOT BUILT (D-431).
...
- **Who may ratify, and what (REC-137 / REC-140, IC-154 / IC-157, 2026-09-18):** `caseratify` and `ratify` of a finding a ratified case pins ask ONE helper, `Store#caseAuthority` — an OWNER of the publishing project signs (C-57.1), the founder or a JOINED member delivers (C-56.1); `ratify` refuses a project bundle by type (C-58.1); both answer a caller who cannot see the project as for one that does not exist. `ratify` of anything no ratified case pins is unchanged and is D-431's.
