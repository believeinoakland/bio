...
- §7.3 — the role's productions are designed (`EXTRACTION-BREADTH-DESIGN.md` §4) and its RUNTIME, SCOPE and BOUND are **DECIDED 2026-09-14** (D-358 answered: the run, not the pilot). The plane's half is built and was, for one day, a **DOOR WITH NO CALLER** (SK-7): it admits a machine-minted citable row, labels it on every projection and on the one surface that shows one, and refuses the minter an attestation. What remains undesigned and is named in place: the member-facing surface on which a run is requested and its productions reviewed (Program B's), and whether a project may stand an EXTRACT run unattended (OPEN, running under the stated provisional: no standing EXTRACT).
...
| **EXTRACT** | document → CONTENT: propose passages, readings, structure; resolve what a document names to the registry — "the role that makes everything else addressable" | named at DEC-24; its productions designed 2026-09-14 (`EXTRACTION-BREADTH-DESIGN.md` §4); machine-minted citable rows ruled by Bob (5.7) and **BUILT ON THE PLANE SIDE 2026-09-14 (SK-7): the act, the label and the fence exist and NO ASSISTANT CALLS THEM**; the `ai(function)` chain step DESIGNED and emitted by nothing |
...
**What this unblocks:** SK-8 (AI-proposed readings, `EXTRACTION-BREADTH-DESIGN.md` item 7) has its placement; the pilot's §5 exclusion 1 is corrected by SKILL through the delegation this session filed, never exempted.
...
| machine-minted citable rows (5.7) | RULED; **PLANE HALF BUILT 2026-09-14 (SK-7)** — `op=contentmint`, the four-state MINT LABEL on every content-row projection and on the leg surface, the attest fence made reachable; the ASSISTANT half unbuilt; D-358 ANSWERED at §7.3 — the caller is a RUN, not the pilot, whose exclusion is corrected rather than lifted; SK-8 is the item |
...
| the EXTRACT role's member-facing surface; AI-proposed readings (`ai(function)` emitted) | the surface is Program B's; SK-8 placed and queued |
