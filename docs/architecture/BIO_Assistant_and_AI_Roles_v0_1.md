...
- §7.3 — the EXTRACT role's surface and loop are not designed; only its productions are (`EXTRACTION-BREADTH-DESIGN.md` §4). **From 2026-09-14 the role also has a DOOR AND NO CALLER** (SK-7, D-358): the plane admits a machine-minted citable row, labels it on every projection and on the one surface that shows one, and refuses the minter an attestation — and no assistant anywhere in this repository invokes any of it, because the surface and loop this bullet names are what would invoke it.
...
| machine-minted citable rows (5.7) | RULED; **PLANE HALF BUILT 2026-09-14 (SK-7)** — `op=contentmint`, the four-state MINT LABEL on every content-row projection and on the leg surface, the attest fence made reachable; the ASSISTANT half unbuilt, because `ASSISTANT-PILOT.md` §5 exclusion 1 excludes EXTRACT from the pilot BY NAME and nothing in `agent-worker/` runs an EXTRACT pass (D-358) |
