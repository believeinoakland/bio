...
**AMENDED 2026-09-23 by D-241 — §4.3's *truncation stated* NOW REACHES `op=connections`, under `IC-236` on I3 (PROPOSED, ADDITIVE).** REC-95 recorded each connection derivation's extent (`partial` when the pair bound cut it) and no read consulted it: `connectionsFor`'s `truncated` is whether its PAGE was cut at `limit`, so a subject derived over 32 of its 40 documents read back `truncated: false` over 496 connections. The entity arm now publishes `derivation {state, cut, at, documents, derived, says}` from the LATEST meaning-level entity row, and no row is stated in words through `OBSERVATION-LOG-DESIGN.md` §5.1's cause — `never_derived` only when the log carried the level over the subject's whole lifetime, else `pre_log` or `undetermined` — never an absent key. No column: `documents` is read back out of the row's own detail beside its writer (`derivationStatement` in `airun.mjs`). Evidence `bio-plane/test/d241-derivation-stated.test.mjs`; its control (`noread`, the two liars, `overstrict`) six of six as declared.

**AMENDED 2026-09-25 by UI-95 — THE MEMBER SEES IT.** Until today `civicos-ui/app.html`'s subject view read the entity arm and dropped `derivation`, so a cut derivation still read to a member as every connection through the subject. `connectionsDerivationHtml` renders the plane's `says` VERBATIM (DEC-8) beside the list when `cut` is true or `derived` is not `derived` (never derived, pre-log, undetermined); a whole recorded derivation adds nothing; an entity-arm answer WITHOUT the key is stated undetermined rather than read as complete. The document page (the capture arm) states nothing, per the Incomplete bullet below. Evidence `civicos-ui/test/subject-view.test.mjs`, its fixtures drawn from the plane's own `derivationStatement`; its NEGATIVE CONTROL line records both arms as declared.

**AMENDED 2026-09-25 by D-672 — §4.1'S WORKBOOK UNIT IS BUILT, AND THE INCOMPLETE BULLET THAT SAID IT "WAITS ON THE `sheet-range` ARM" WAS STALE FOR A WEEK.** The extent arm landed with FW-19 (IC-124/IC-125) and every spreadsheet producer — `.xlsx`, `.ods`, `.csv` — already emitted each sheet's `range` through the one builder (`usedSheetRange`), so what was missing was a READER: `textUnitsFor` (`index.mjs`) recognised `pages[]`, `paragraphs[]` and `slides[]` and returned nothing for `sheets[]`, and the store's container set left the workbooks out. Both now admit them, by SHAPE: one unit per sheet, extent `{kind: "sheet-range", sheet, range}` from the producer's own `range` (the USED range, A1 to the far corner the cells reach, never the grid), text the sheet's own stream (rows newline-joined, cells tab-joined), `seq` the producer's sheet position; a sheet whose `range` is null is not emitted, because its extent cannot be named. **A cell is still not a passage** — the unit is the sheet, which is what this section designed. Driven through `op=meaningrows&rows=passage` in `capture-text-index.test.mjs` X1/X2b (a cell's words found in ONE row labelled `sheet-range`, on an `.xlsx` and an `.ods`), with its control's three arms as declared (`nc-rec91.mjs` `d672nosheets`, `d672storeset`, `d672strict`). **Three things this landing does not do, stated:** (a) a workbook's NAMED units (D-415's defined names, table parts, named and database ranges) are extents with no text, and are NOT indexed — cutting a rectangle's text out of its sheet, and deciding whether a table and its sheet may both answer for the same words, is not designed here (Incomplete list); (b) a CSV captured through `op=acquire` is read as profile TEXT and never reaches the format wire, so it still has no unit although `csv` is in the store's set by shape — a defect, minted D-684 (CLOSED 2026-09-25 by D-684: such a capture is now handed to its format entry and indexed; its sheet list on that path is D-694); (c) the acquire wire DROPS a unit whose text is over its budget rather than carrying the per-unit-capped prefix the store would keep, which a workbook's sheet reaches far more often than a page or a paragraph — minted D-685, diagnosed at the code and not measured (CLOSED 2026-09-25 by D-685, MEASURED first at M-184: one 600,599 B sheet reached the store as NO unit; the wire now carries a unit over the per-unit cap as its capped prefix, marked `truncated` — see §4.1). **A workbook promoted before D-672 keeps its `none: no unit arm` observation until it is re-promoted**; how many the live record holds is UNDETERMINED.
...
- §4.1 — **a workbook's NAMED units are not indexed, and nothing designs how they would be** (D-672, 2026-09-25). D-415 emits each defined name, table part, named range and database range that names one rectangle as a `sheet-range` EXTENT (`rangeUnits`), with no text. Indexing one needs its rectangle's text cut out of the sheet — a producer change — and a ruling on overlap: a table and the sheet it sits on would both answer for the same words, and the index's primary key is the address, not the text. The SHEET is the unit this section designs and is built; the named unit is a finer grain nobody has designed.
- §4.1 — **a CSV read as text at intake carries its units but not its SHEET LIST** (D-684, 2026-09-25; minted D-694). `op=acquire`'s `container_extent` is projected inside the FORMAT wire's own block, which a document read as text at intake never enters; D-684 hands such a capture to its entry's `text()` for the units and the container key only, so `reading.container_extent` is ABSENT for it — a `sheet-range`/`sheet-cell` citation naming a sheet the CSV does not have is skipped as undetermined rather than refused, and a version notice reads *the record holds no sheet list*. Measured in `capture-text-index.test.mjs` at D-684's landing (the key absent on the acquired document). The fix is the projection lifted into one function both paths call, as `textUnitsFor` was (CPDF-19).
...
*(§4.1's workbook bullet — "the indexing unit for a sheet waits on `EXTRACTION-BREADTH-DESIGN.md` §3.2's `sheet-range` arm" — is STRUCK 2026-09-25 by D-672: the arm landed with FW-19 and the unit is now written, one per sheet; see the D-672 amendment above. M-20's measurement of the gap — 288 workbooks, 72,651,441 bytes of text over 1,056 sheets, not one indexable unit — is the size of what it closed, and stays in `MEASUREMENTS.md`.)*
...
**A unit with no GLYPH is not a unit (D-531, 2026-09-25).** Both emission sites — the acquire wire's unit arm in `index.mjs` and the writer's ordering filter in `store.mjs` — judge "this unit has text" by `glyphCount(text) > 0` (D-514's counter, non-whitespace code points), never by `text.length`. Until D-531 both read the length, so a page, paragraph or slide of pure whitespace was emitted, written and counted toward `full`, and a capture whose every unit was blank read `indexed_full` where it now reads `none` (M-154). The store's filter is not redundant with the wire's: a caller-authored `data/provenance.json` reaches the writer directly. A dropped unit keeps the producer's numbering of the survivors (a gap, never a re-count), and a unit with any glyph is carried verbatim — the rule is "no glyph", never "trim". Units written before D-531 stand until their capture is re-promoted; how many the live record holds is UNDETERMINED (M-154).
...
**Workbooks** are not indexed per cell — a 63 MB sheet inflates to millions of cells and a cell is not a passage. A sheet's unit is a `sheet-range`, which is piece 4's arm. **BUILT 2026-09-25 (D-672):** one unit per sheet, its extent the sheet's USED range as the producer emits it (`sheets[].range`), its text the sheet's own stream; a sheet whose range the producer could not name is not emitted. Until D-672 the arm existed and no reader used it, and the per-capture `indexed` state said `none: no unit arm for this container` rather than staying silent — which a workbook promoted before D-672 still says until it is re-promoted. A workbook's NAMED units (D-415) are extents without text and are not indexed (Incomplete list). **A CSV READ AS TEXT AT INTAKE IS ITEMISED TOO (D-684, 2026-09-25).** `op=acquire`'s FORMAT wire ran only for a document NOT read as text at intake, so a `text/csv` body under the profile bound took the content-type reader's branch and reached no unit and no `text_container` — its cells unsearchable though the csv entry emits `sheets[]`. The acquire path now hands such a capture to its format entry's `text()` and `textUnitsFor` beside the profile reading, which is kept as it was (the reader still reads, and the provenance still digests, the intake text); the reading gains only `text_container`. An entry that declares no `text()` — HTML — is unmoved. What that path still does not carry is the sheet LIST (`container_extent`), so a citation naming a sheet a CSV does not have is skipped as undetermined rather than refused (D-694, Incomplete list). **A UNIT OVER THE PER-UNIT CAP IS CARRIED AS ITS PREFIX, NOT DROPPED (D-685, 2026-09-25, M-184).** The acquire wire's budget (§4.3's table: 524,288 B, charged per unit with a 128 B envelope) charged a unit's WHOLE text and dropped the unit when that did not fit, though the store keeps only a unit's first `CAPTURE_TEXT_UNIT_CAP` (131,072) characters — so, measured, a workbook of one 600,599 B sheet reached the store as NO unit and read `partial` over nothing, and the second of two 300,299 B sheets was dropped while the first spent 300,427 B of budget on text the store discarded. The wire now cuts a unit to that cap, in the store's own unit (characters), charges the prefix's UTF-8 bytes, and marks it `truncated: true`; the writer ORs that mark with its own cut, because a prefix arriving AT the cap reads whole to its comparison. A unit under the cap is carried byte for byte and unmarked. **What is still dropped, stated:** a unit whose capped prefix plus envelope does not fit what REMAINS of the budget — at most three full-cap units fit one capture (3 × 131,200 B leaves 130,688), so a fourth is dropped whole, counted, and the capture reads `partial`; a later unit that fits is still carried, so a `partial` capture's units are not a strict reading-order prefix — and since D-724 the dropped unit is NAMED, not only counted (§4.3). The capture-level state of a capture whose units were all carried but one was CUT stays `full`, with the cut said on the unit's row (`rows=passage`'s `truncated`) and in the observation's detail — the same answer a unit cut by the store's own cap has always had.
...
**Stated correction (D-391 part 2; BOB #32, 2026-09-23).** The 257 ms window converts the 40M ceiling at the MINIFLARE
rate, but the deployed window is 1,071 ms, so every percentage here is up to **4.16×** too PESSIMISTIC (84.8% → 20.4%).
The promote itself was also timed in miniflare, and deployed SQLite promote speed is UNMEASURED, so the two errors
partly cancel by an amount nobody has bounded. **The honest reading is UNDETERMINED between ~20% and ~85%**, and the
conclusion that the ceiling is near rests on the miniflare end of that range. REC-111's 4,096-unit budget already
removed the consequence. One deployed measurement of the same promote ladder would settle it.
...
office containers, 18 workbooks in the census are over it, and a workbook had no unit arm anyway
(true when written; since D-672 a workbook has one unit per sheet, and those 18 are refused upstream of it).

**WHAT A `partial` INDEX HOLDS, CORRECTED 2026-09-25 by D-724 (BOB #36, 11:20Z, option (b)): EVERY UNIT
THAT FIT, IN READING ORDER, WITH GAPS — AND EVERY GAP NAMED.** This section and the writer's docblock said a
partial index is indexed *TO the bound in reading order*, and the writer's said it *must be a PREFIX*. Neither
loop ever did that: the acquire wire's (`textUnitsFor`) and the store's (`#writeCaptureText`) both go ON past
a unit over the bound and index a later unit that fits (measured, M-184 Z5: a workbook's fourth sheet dropped,
its fifth indexed). **Breaking at the first over-bound unit (option (a)) was REJECTED**: it would discard
readable text to protect a prefix property nothing reads — `rows=passage` carries `seq`, so a gap is visible.
What binds instead is `CLAUDE.md` §2: a member whose search finds nothing in sheet 4 must be able to learn that
...
A run can end only at a carried unit, and after the first skip what is carried is bounded by what remained of
the budget (under 131,200 B, at 129 B the smallest unit), so the wire sends at most ~1,018 runs. The index
observation's sentence says how many skipped units the read names, and how many it does not (a caller that
carried a count without keys). Under the UNIT bound alone every later unit is skipped, so there the index IS a
prefix; under either byte budget it is not. A unit CUT to the per-unit cap is not a skipped unit: its prefix
is indexed and found, and it is said on the unit's row (`truncated`). Driven in `capture-text-index.test.mjs`
G6b/G6c/G7b and Z3b/Z5c–Z5f; control `nc-rec91.mjs` `d724nowrite`, `d724wire`, `d724nodelete`,
`d724cutisskip`.
