...
- §4.1 — decks: the section names `slide-shape` as a deck's unit and says the units come from the I2 shape the acquire path already holds. **They do not.** `pptx.mjs` emits one text string per SLIDE, and its slide walk keeps a shape index for hyperlinks that it discards at the line where it collects a paragraph's text — so a `capture_text` row at `slide-shape` grain cannot be written from I2 today. `textchain.mjs` sharpens the choice rather than settling it: `readingSource()` REQUIRES both `slide` and `shape` for a `slide-shape` reading position, while `covers()` accepts a shape-omitted extent as covering a whole slide, so a slide-grain unit is expressible as an EXTENT but not as a reading POSITION. **Either `pptx.mjs` emits text per shape, or this section adopts the slide as the deck's unit and says so.** Found by M0-31's measurement, which reported the per-slide figure and named the substitution; it is §7 item 4's to resolve.
