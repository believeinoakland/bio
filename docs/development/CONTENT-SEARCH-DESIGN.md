...
**Incomplete sections** ·
- §5 — the storage measurement (text bytes per captured page and per document; index bytes per text byte on workerd's SQLite; promote-time CPU per indexed unit) is named and not taken; §4.3's cap is provisional until it is.
- §4.1 — workbooks: the indexing unit for a sheet waits on `EXTRACTION-BREADTH-DESIGN.md` §3.2's `sheet-range` arm; until it exists a workbook is searchable at document grain only, and the answer says so.
...
| **D-225's caps land BEFORE any new content surface** — an uncapped legacy read beside a capped new one is the inconsistency a caller builds against | `DEBT.md` D-225; §14c "Related finding" |
...
Two bounds, both stated where they bite:

- **Per unit:** a page, a paragraph or a shape never approaches 128 KB in the census; the bound exists so that one pathological unit cannot make a promote unbounded. Provisionally `TEXT_CAP` (128 KB), the number the existing index already carries; the measurement in §5 may lower it. A unit over the bound is stored to the bound with `truncated = 1`, and `rows=passage` carries the flag — never a silent prefix.
- **Per capture:** the total indexed bytes per document. For office containers the extraction bound already exists — 20 MiB of declared uncompressed text-part bytes (COFF-2, `MEASUREMENTS.md`), over which the document is `text-undetermined` and nothing is extracted, so nothing is indexed and the reason is already recorded. For PDFs no such bound exists; §5's measurement sets one. Over the bound, the capture is indexed to the bound in reading order and its `indexed` state reads `partial`.
...
| --- | --- | --- | --- | --- | --- |
| 1 | RECORD | **D-225's caps** — `concerns`, `resolutionsForCapture`, `connectionsFor` onto REC-57's envelope (limit applied and named, truncation said, no bare array); no new vocabulary | — | I3, additive, an IC (IC-3's rule: a break recorded as additive teaches the registry to lie, so an addition is recorded too) | §2; `INVESTIGATIVE-SESSION.md` §14c "Related finding" |
