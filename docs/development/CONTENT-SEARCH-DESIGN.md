...
**Incomplete sections** ·
- §4.4 — **THE `levels` STATEMENT AND THE `axis` STATEMENT DISAGREE ABOUT WHAT *IN SCOPE* MEANS,
  AND THE HALF THAT FEEDS THE MEMBER-FACING SENTENCE IS THE WRONG ONE. THE DESIGN IS RIGHT AND
  THE CODE IS HALF-CORRECTED.** This section's *the query's OTHER arms* is honoured by
  `meaning({mode:"axis"})`, which builds its scope from `cte(false, armSet(rowArm))` with the
  passage arm STRIPPED — REC-92's own correction. `meaning({mode:"levels"})` uses the ORDINARY
  cte with the arm still applied, **while its own comment says it counts "how many documents the
  query's other arms put in scope at all"** — a comment describing a constraint nothing enforces.
  So `scope.documents` collapses to 0 on any passage MISS while `captures_counted` correctly
  does not, and because `#meaningLevels` tests `documents === 0` BEFORE `searchable === 0`, the
  two honest branches of `says` are **UNREACHABLE for every passage miss**. Measured 2026-09-17
  by UI-62 against the live plane (`MEASUREMENTS.md` M-43): a miss over a scope holding two
  documents, one of them fully indexed, publishes *"nothing matched, and no document was in scope
  to match in — this is an empty DOCUMENT level, not an empty record"*, which is **the false
  absence this construct exists to refuse, produced by the mechanism itself for the second time
  and through the second statement.** REC-92's suite did not catch it because its S8 asserts
  `levels.content.why` — composed from the TALLY, which is correct — and never compares the
  three empties' `says` to each other. **This is RECORD's to fix in `query.mjs` and is DELEGATED
  in `CLAIMS.md`; UI-62 did not work around it, because DEC-8 forbids a surface rewording the
  record's sentence and a surface adjudicating between two of the plane's own numbers would be
  making a judgement it cannot support.** Found and reported by UI-62 in the building of item 6.
