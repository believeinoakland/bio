...
**Incomplete sections** ·
- §4.4 — **THE TALLY IS BOUNDED AND THE SECTION STILL DESCRIBES AN UNBOUNDED ONE IN PROSE.**
  REC-92 shipped `MEANING_AXIS_CAP` = 500 with `captures_counted` / `captures_truncated` /
  `captures_bound` published, and the section's worked example ("0 hits over 412 indexed
  captures") reads as though the tally always ranges over the whole scope. The NUMBER is
  measured (`MEASUREMENTS.md` M-40) and the envelope is honest; what is incomplete is that
  this section does not yet say what a member sees when the bound bites, which is a
  presentation question and UI's (item 6), not this section's to settle alone. Found and
  reported by REC-92 in the building of it.
