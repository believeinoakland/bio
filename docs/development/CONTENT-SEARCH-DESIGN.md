...
**AMENDED 2026-09-25 by D-686 (BOB #35's ruling of 09:05Z) — `content.chain_kind` IS THE UNIT'S EXTRACTION METHOD, NOT THE DOCUMENT'S, AND IS NO LONGER A GENERATED COLUMN.** REC-104's column was the whole chain's last step, so on a mixed document every unit read `ocr`, including its text-layer pages. It is now the kind of the last derivation step covering the unit's page, written at mint by `textchain.mjs` `chainKindFor` (the one computation; `#migrate` recomputes older rows with it), so `content:layer` and `content:ocr` each find the units that were read that way, and a unit with no page read more than one way (a mixed document's whole-document unit) holds `mixed`, askable as `content:mixed` (BOB #35, 09:35Z). §4.1's `capture_text.chain_kind` is KEPT document-level: the last step of this document's chain, not how any given page was read. The filter, its index and every predicate below are unchanged. Content Framework Part II §14.2 holds the rule.
...
`capture_text` — one row per indexed unit of one capture's text under its CURRENT chain:
    ...
    chain_kind     TEXT NOT NULL      -- the last step of this document's chain, not how any given page was read (D-686), so "every unit of an OCR-ending document" is a predicate and not a parse
...
**The `chain` filter reads a column (REC-104).** The table row above says the `content:` arm filters on the chain's last step. `content` stores the whole chain as JSON, so that last step is held in `content.chain_kind` with an index `(chain_kind, bundle_id)` — the same answer §4.1 gives `capture_text`, and for the same reason: *every OCR'd region* is a predicate and not a parse. **Since D-686 the column is the UNIT's:** the last derivation step covering its page, written at mint by `chainKindFor`, where REC-104's GENERATED column held the whole chain's last step. `chain:undetermined` is `chain IS NULL`, the record holding no chain at all — **over TEXT rows only (REC-121)**: an image cited as its own bytes has no chain BY MEANING and answers `chain:does-not-apply` (`cited_as = 'bytes'`), with `chain_last` saying the same word, so every row answers exactly one of a step, undetermined, or does-not-apply. **The `cap` filter has the same three answers (REC-127):** `cap:undetermined` is `derivation_cap IS NULL` over TEXT rows only, `cap:does-not-apply` is `cited_as = 'bytes'`, and `rows=content`'s `derivation_cap` reads `does-not-apply` on such a row — in the column's own slot, so no row gains a column.
