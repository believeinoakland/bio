...
- §Content type is a SEPARATE axis from the stack — written when the axis carried one measured type. SEVEN are registered as of 2026-09-23: `meeting_calendar`, `meeting_minutes`, `meeting_agenda` (FW-15, the first type reached through text a non-HTML container produced), `staff_report`, `regulation` (FW-18), `staff_directory` (FW-20), and the `generic` fallback — six measured, the last four in `EXTRACTION-BREADTH-DESIGN.md` §2's census order, which is now complete through its fourth class (row 5, a budget or dataset, is owed and unmeasured). The section still says nothing about how many types exist or what each one cost to add, which is precisely the evidence for framework §4's uniformity claim and is recorded only in the source. **Two things FW-18 measured are NOT in this document's body and belong to this axis rather than to one type.** (1) THE ENGINE CANNOT REPORT A MULTI-CLASS DOCUMENT: `makeRegistry().recognise` breaks on the first CERTAIN detection, so `considered` is truncated at whatever won, while M0-32 measured that 52 of 600 sampled documents satisfy more than one class. `doctypes/registry.mjs` answers this beside the engine with an additive `also` pass that asks every type independently and changes no verdict; whether the ENGINE should instead be able to say it is an open question for this axis and the stack axis together, and nothing here decides it. (2) A CLASS'S SELF-NAMING IS A RATE, NOT A PRESENCE — a masthead is page furniture and recurs once per page, a reference occurs once or twice — which is the mechanism by which the content axis resists M0-32's one defect class (a REFERENCE read as MEMBERSHIP). It is implemented as `selfNaming`/`FURNITURE_RECURS` in `docprofile/doctypes/index.mjs` with its measurements at the site.
- §Content type is a SEPARATE axis from the stack — a SECOND gap in the same section, **its premise CORRECTED 2026-09-23 by FW-20 (`measurements/M-121.md`) and the gap NARROWED rather than closed.** FW-18 withheld a STAFF DIRECTORY type because 30 name-matched directory PDFs were Tier-1 undecodable, and read that as "a capture-tier gap wearing a content-type gap's clothes" to be closed by tier 3. Re-taken through the plane with its fleet bound, the markers on those documents are `no_tounicode` / `unmapped_code` — fonts with no Unicode map, TIER 2's case, so tier 3 is never asked — and 56 of 57 name-matched documents read from text (tier 1 alone: 5). The type is now written (`docprofile/doctypes/staff-directory.mjs`) from real directories read at tier 2. **What remains open on this axis, stated:** (1) the type recognises a directory by addresses at ONE organisation's domain, so a directory with NO email addresses (phone-only, or names and titles only — the Council's committee roster and the org charts are of this shape) is not recognised and falls to `generic`; (2) a 300-document fixed-seed walk found NO staff directory against ~3.8 expected at M0-32's rate, which M-121 records unresolved — M0-32's class is broader than this type and was counted with a different threshold over a different sample. The body of this section has not been rewritten to describe the directory type; its header and the registry carry it.
- §Content type is a SEPARATE axis from the stack — a STATED LIMITATION of the STACK axis run over EXTRACTED text (D-167, LED-7 batch S14-1, SCHEDULER #14, 2026-09-22). The stack question is about the SERVED bytes, which the fetch path profiles (FW-3); `docprofile/readtext.mjs` runs `identify()` over I2's text for the doctype pass, and there the stack handlers can speak only weakly — plain Tier-1 PDF text with no anchors trips `client_rendered`'s structural test at `possible` confidence. So `stack` on a text reading is ADVISORY context for the doctype pass, never a verdict about the document and never a substitute for the capture's own profile; no plane reader takes it off the result (verified at the code). Making the axis text-provenance-aware is the next stack-axis item's, unscheduled.
...
evidentiary digest intra-bundle; `op=monitor` compares the evidentiary digest when both
sides earned one (D-60, 2026-09-23); `resolveLinks`' bracket is still raw and is what
survives of D-60 (§Known gaps). `civicos-ui/app.html` carries a flattened copy
...
- **CORRECTED 2026-09-14 (M0-27). This bullet read "The plane has not adopted it —
  ...
  cross-bundle sweep exists anywhere. MONITORING is **DISCHARGED (D-60, 2026-09-23)**: `op=monitor` compares the
  EVIDENTIARY digest when the baseline's register row recorded one as `determined` and the
  fetched bytes normalise under the SAME handler (key and version) with certainty, through
  `substanceDigests` in `bio-plane/src/index.mjs` — the one function `op=acquire` records
  the baseline's with; otherwise it compares raw, and its answer carries `compared`
  (`evidentiary` | `raw`) and `compared_basis` saying why (`monitor-substance.test.mjs`).
  It does NOT read the kind's contract (§Monitoring is a different contract per kind).
  The remaining site is **STILL RAW**: `resolveLinks`' contemporaneity bracket keys on raw `capture_sha`, so
