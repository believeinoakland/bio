...
- §11 item 5 — RULED (BOB #25, 2026-09-21), BUILT: rules 2 and 3 were BUILT on 2026-09-23 by D-85 (IC-181, `bio-plane/test/d85-surface-run.test.mjs`, sixteen-arm control), which supersedes this bullet's "NOT BUILT" sentence below, kept as the record. Rule 2: `op=promote` creating an inquiry under an `ai` credential asks sight, `runPrincipalGate`, status and the run's declared `surfaces` bound (C-66.1..4), writes `inquiry_run_surfacings` keyed by the inquiry (both purge arms, never its bytes), and `op=projection` states `surfaced_in` with the run's lens block, `not recorded` where none; a member's creation is untouched. Rule 3: `ai_runs.lens_at_open`, and the lens block's `at_open`, `hand` and `moved`. STILL OPEN, routed with their fixes in D-85's report: ~~DESIGN GAP (D-85) — rule 2 names the `ai` credential, and the instance's other machine classes (admin, member and probe tokens) still create questions stamped `surfaced_by: agent` outside any run (BOB's: does rule 2 reach them?)~~ ANSWERED by BOB #30 and BUILT 2026-09-23 by REC-171 (IC-186, see the bullet on rule 2's reach below); and `op=airuntick`'s `consume` takes a negative delta, so a run's principal can refund a plane-counted bound (`mints`, `surfaces`). Rule 1 and its target (BOB #28) are BUILT by REC-165 (IC-176, 2026-09-22): `op=suggest` and `op=extractpropose` take REC-152's one `principal` stamp and ask, in order, sight (an unseen run's context answers as a never-minted id), position (`runPrincipalGate`, C-22.12) and status (`op=suggest` refuses an ended run, `SUGGEST_RUN_NOT_RUNNING` C-27.18); a suggestion outside its run's context is refused `SUGGEST_OUTSIDE_RUN_CONTEXT` (C-27.19) after sight and position (`bio-plane/test/rec165-production-principal.test.mjs`, nine-row control). NOT BUILT: rule 2 (an `ai` credential still creates an inquiry outside any run, with no lens and no bound) and rule 3 (a run records only the manifest it was handed). MEASURED BY REC-165, RULED BY BOB #28 AND BUILT 2026-09-23 BY REC-168 (IC-178): `op=capturerequest` took a `run` and copied its principals into the request row without asking whose run it was; it now takes the same stamp, asks sight, `runPrincipalGate` and status in that order, and records the CALLER's principal (`bio-plane/test/rec168-capturerequest-principal.test.mjs`, eleven-row control). DESIGN GAP (REC-168): the ruling's *"a request naming no run is the member's own"* describes no path the plane has — the door has always refused a request naming no run (`CAPTURE_REQUEST_NO_RUN`, DEC-47), so there is no run-less request, and nothing was built for one. Still routed: `agent-worker`'s one `suggest` site does not bound a candidate's target to its run's context (BOB #28's builder measurement), its table-made empty-level candidates carrying no target at all. ~~Still routed~~ BUILT 2026-09-23 by FL-11+FL-12 for a run over a QUESTION (fleet member only, no IC: `agent-worker/test/harness.test.mjs` FT0-FT4, `harness.control.mjs` T1/T2): the member seeds its run's target from the run read's context, a candidate naming none and every table-made empty-level candidate land there, `dedup` reads it, and a candidate aimed outside is refused C-27.19 and routed to ADJUST; and its `op=capturerequest` names the locator `address` (it sent `url`, which the plane never reads). STILL OPEN, stated: a run over a PROJECT has no target the member can name — the run read does not publish the questions the project confirmed-cites — so its table-made empty-level candidates are refused `SUGGEST_NO_TARGET` (routed with its fix).
- §12 (c) — D-195's shared-origin disclosure at the member's own elicitation is RULED (BOB #22, 2026-09-21) and BUILT IN PART on 2026-09-23 by REC-161 (IC-221, C-71): `op=partitionindependence` computes independence over a PROPOSED partition of the question's basis legs through `Store#independenceOf`, gated as `op=versionstrength` is, and equals it once the partition is written (`bio-plane/test/partitionindependence.test.mjs`). ~~NOT BUILT: UI-27's read-back (`elicFalsifier`) does not call it, so no member is shown a shared origin yet.~~ The SURFACE half is BUILT 2026-09-25 by UI-75 (`civicos-ui/test/elicitation.test.mjs`, six-arm control): the read-back asks it once over the proposed partition and names each shared origin, refusing and reordering nothing; a refused read renders as not told and is asked again on return. Nothing open under (c) but the gap below. DESIGN GAP: the clause names the parts by what the elicitation lists and does not say what a group is called on the wire; the read takes the caller's names or files a group by its position (`part N`), and never invents one.
- §12 (a)–(b) — the ACCEPT CEREMONY is BUILT at the member surface (UI-74, 2026-09-23; `civicos-ui/test/accept-ceremony.test.mjs`, real plane): D-195's derived shared origin (`op=versionstrength`'s `independence`) is shown before the per-set affirmation, which is sent as `affirmed=` and refused by the plane if incomplete (D-271). Two gaps stay: ~~`independence` is published only beside the strength pair~~ the WIRE half is BUILT 2026-09-24 by REC-192 (IC-248 PROPOSED, C-71.8/.9; BOB #31's 22:22Z ruling): `op=partitionindependence&version=` answers a stored version's `independence` on its own, no strength key, equal to `op=versionstrength`'s; ~~STILL OPEN, stated: the page still reads it off `op=versionstrength` and drops the pair, so clause 5's ordering is held by the page until it switches (a UI row)~~ the PAGE half is BUILT 2026-09-25 by UI-88: `acerOriginsRead` asks the version arm, and the suite judges the network log before the affirmation to hold no strength-bearing answer; STILL OPEN: a lens that MOVED (DEC-46) is rendered but not driven by a fixture.
- §14c — options graded 2026-08-06; option A LANDED (PL-8, the MEANING arm) while D-222's debt row still reads open; stage C (content-grain search) is BUILT (`node tools/status.mjs 9`: 9.content-search, the `content:` and `passage:` arms; corrected 2026-09-23 by BOB #32).
...
- §What the session sees — the bias object is BUILT (PL-12) and D-220's version join landed (PL-10); the text still says UNBUILDABLE TODAY. D-220's consumer (3), the session reading versions as versions, was BUILT on 2026-09-23 by D-220 (IC-213): see the BUILT note under the paragraph. STILL OPEN, stated: the consumer NUMBERING disagrees — this section calls the session consumer (3) and D-220's own ledger row numbers it (2), with search rendering (3) and monitoring (4); the QUEUE row's "consumer (2), monitoring" follows neither. Monitoring was checked at the code and NOT driven: no monitoring path calls `versionChain`, and its one join of the same two tables (`#conditionBundlesForHost`) selects a host's distinct bundles as queue-condition subjects rather than counting documents.
...
- §14b — the pre-write checks landed plane-side (PL-3) and F10's denied-means-adjust row landed (FL-3); §14b.2 still frames D-222 as a precondition. §14b.6's bound takes no refund and no caller figure for a plane-counted bound since REC-169 (IC-184, 2026-09-23); since REC-172 (IC-188, 2026-09-23, PROPOSED) a figure lands only on a bound the run HAS — a `consume` that is not a map or names no bound, a `bounds` that is not a list or names none, is refused (C-22.15), `lease` is refused as a consumable (C-22.14), and a member's `allowed` is a whole number of zero or more (C-22.13). ~~STILL OPEN: an ABSENT `allowed` opens a bound at 0, which `finishedBound` reads as no ceiling — whether a declared bound may omit its allowance is not ruled here.~~ RULED by BOB #30 (the §14b item 6 bullet below) and BUILT 2026-09-23 by REC-177 (IC-195, PROPOSED): an absent or zero `allowed` is refused at the open (C-22.16).
...
- §11 — item 5, rule 2's REACH, RULED 2026-09-23 by BOB #30 (D-85's builder's question): rule 2 binds every creation D-78 stamps `surfaced_by: agent`, not the `ai` class alone. D-85 built it for `ai`; ~~the admin, member and probe deploy tokens are NOT BUILT (a row, placed after D-85)~~ BUILT 2026-09-23 by REC-171 (IC-186, `bio-plane/test/rec171-surface-token.test.mjs`, nine-arm control): `op=promote`'s stamp is set for every non-session caller, `class:<cls>` for a deploy token. ~~STILL OPEN~~ ANSWERED by BOB #30 and BUILT 2026-09-23 by REC-173 (IC-190, the next bullet); the finding as it stood, brought to BOB rather than exempted (the ruling's own instruction): `bio-plane/migrate/migrate.mjs` replays a Drive mirror's questions under a deploy token and cannot honestly name a run (a replayed question was not surfaced by a run opened today), so a migration of any inquiry is now refused `SURFACE_NO_RUN` — and D-78 already restamps a migrated member's question `agent`, which predates this item.
- §11 — item 5, a MIGRATION REPLAY is not a surfacing: RULED 2026-09-23 by BOB #30 (REC-171's finding); ~~NOT BUILT (a row)~~ BUILT 2026-09-23 by REC-173 (IC-190, `bio-plane/test/rec173-migration-replay.test.mjs`, eleven-arm control; `migrate.test.mjs` migrates a Drive-era question clean): see the BUILT note under the paragraph. STILL OPEN, stated: DESIGN GAP (REC-173) — the paragraph asks for a capture registered "already in the record", but the register has ONE writer (`promote`) and a row names the bundle it was registered against, so nothing can register a capture against a bundle before its creation; built as bytes HELD before the creation and REGISTERED by the creation itself (BOB's, if a prior registration act is wanted). The root of trust's own honesty stays out of scope (Membership §DEC-2).
- §11 — item 5, `replay` IS THE SERVER'S WORD (BOB #33, 2026-09-24, D-505's finding): ~~step (1), the admin-class fence on a caller's `replay`, … NOT BUILT (sent to SCHEDULER to row)~~ **step (1) BUILT 2026-09-24 by D-511** (IC-278 PROPOSED; `bio-plane/test/risk-tier.test.mjs` §8 and its inverted arm (ix), five-arm control `d511-replay-server-word.control.mjs`): `op=promote`'s stamp block deletes a caller's `replay` unless the call arrives under the ADMIN class with no session — see the BUILT note under the paragraph. ~~**STEP (2) IS NOT BUILT** and is the residue BOB #33 states: an admin-class caller can still ASSERT a replay it cannot show, since only an inquiry's CREATION is server-verified (REC-173's `migrationReplayOf`), and every other type and every revision is taken on the root of trust's word. §8's arm (δ) and its residue arm PIN that, so closing it turns them red rather than letting it disappear.~~ **STEP (2) BUILT 2026-09-24 by D-512** (`bio-plane/test/d512-replay-verified.test.mjs`, eight-arm control; C-66.6 `REPLAY_UNVERIFIED`; §8's arm (δ) and its residue arm INVERTED): see the BUILT note under the paragraph. STILL OPEN, stated: two provisional choices D-512 made where the ruling is silent — an ADMIN's unverifiable claim is REFUSED rather than downgraded, and a revision must RE-REGISTER the capture it names — are recorded there with their alternatives. The root of trust's own honesty stays out of scope (Membership §DEC-2). STILL OPEN, and FOUND by this build, stated rather than parked: `test/surfacing-run.mjs` cannot supply a surfacing run for a SECOND deploy token in one store — its fixture project is created BY TITLE and the second token is refused `NAME_TAKEN`, after which the wrapper passes the creation through un-run and the suite reads `SURFACE_NO_RUN`. Measured 2026-09-24 by D-511 in `reopen.test.mjs`; the fix is a title unique per (token, store), and it is not taken here because that fixture is shared by dozens of suites (routed to SCHEDULER).
- §11 — item 5, rule 2's reach: a REVISION carries `surfaced_by` forward — BUILT 2026-09-23 by REC-179 (IC-197 PROPOSED, `bio-plane/test/rec179-surfaced-by.test.mjs`, seven-arm control; C-66.5 `SURFACED_BY_REWRITTEN`): see the BUILT note under the migration paragraph. STILL OPEN, stated: (a) a migration REVISION whose Drive-era history changes the value is refused too — REC-173 verified only a creation as a replay (since D-512, 2026-09-24, every revision's replay is VERIFIED, and a verified replay is STILL refused C-66.5 when it changes the value — ARM F4 drives exactly that), and whether the Drive corpus holds such a history is UNDETERMINED (brought to BOB); (b) D-78's creation restamp rewrites only the FIRST `surfaced_by:` line, so a creation carrying the key twice lands with the caller's value as the one the catalog's parser reads (measured by REC-179, routed with its fix).
- §14b — item 6, a declared bound states its allowance: RULED 2026-09-23 by BOB #30 (REC-172's finding); ~~NOT BUILT (a row)~~ BUILT 2026-09-23 by REC-177 (IC-195, PROPOSED; `bio-plane/test/rec177-allowance.test.mjs`, five-arm control): `op=airunopen` refuses an entry whose `allowed` is absent, `null` or 0 (`AI_RUN_BOUND_NO_ALLOWANCE`, C-22.16), nothing written; a present figure of the wrong form stays C-22.13. The one caller that declared such a bound was a suite's over-strictness arm (`rec172-bounds.test.mjs` ARM G2), corrected.
- §11 — item 5, a project run's target: BUILT 2026-09-25 by D-451 for a project that confirmed-cites ONE question (`bio-plane/test/airun.test.mjs` ARM PQ; `agent-worker/test/harness.test.mjs` FT2-FT2g; `harness.control.mjs` T3). DESIGN GAP, not ruled: a project citing SEVERAL questions has no seeded target, and which question a table-made level-empty candidate of such a run belongs to is UNDETERMINED — one candidate per cited question, or none, is a design choice this document has not made (D-572, to BOB). Its candidates are refused `SUGGEST_NO_TARGET` meanwhile.
...
- §What a SUGGESTION is — the five kinds are built (PL-3); D-324 (VF-4) found `new-version` is not one of the five, unrecorded here; a version's kind is published by BOTH readers since D-235 (2026-09-25): `op=basisversions` carries `kind` on every version, `null` on a reading a member composed by hand, and `bio-plane/test/suggest.test.mjs`'s cross-op arm compares it with `op=suggest`'s.
...
**BUILT 2026-09-23 (D-220, IC-213).** The run's `collect` row resolves every citation a
sub-session returns to its document — `op=search` finds the cited bundle and the source
address its own bytes name; a citation naming no held bundle is read as the address it is —
and asks `op=versionchain` for that address. `documentHoldings` (agent-worker
`subsession.mjs`) groups by the `address_norm` the record answers with and NEVER by a title
or a text, so the run's published `holdings` count a document ONCE with its versions; an item
the record holds no captured version for counts as itself; a refused read makes that
citation's document UNDETERMINED and is published. The skill doctrine's
`versions-are-versions` clause (`skilldoctrine.mjs`) tells the model the same and is
instruction-only: nothing refuses a model's prose that calls sixty captures sixty sources.
`agent-worker/test/versions.test.mjs` drives it through `POST /run` against the REAL plane's
chain, over a fixture carrying two DIFFERENT documents that share a title and a summary.
...
**A PROJECT RUN'S TARGET — BUILT 2026-09-25 by D-451.** FL-11 seeded a run's target from its context, but a run over a
project could not be seeded: `op=airun` published only `context: { type, id }`, so the member could not name the question the
project confirmed-cites and every table-made level-empty candidate was refused `SUGGEST_NO_TARGET`. `aiRunRead` now publishes a
project run's `context.questions` (`#runContextQuestions`, `store.mjs`): `#runContextProjects`' set read the other way round —
every question `#citesInto` says the project CONFIRMED-cites, kept where `op=suggest` admits the target (an inquiry by id that the
viewer can see) — so the set published is the set the target rule admits, never a second answer to it. A run over a question
publishes no `questions`; its context id is its one question. `runContextTarget` (`agent-worker`) seeds exactly ONE published
question as the target; several are left to each candidate's own target (the member does not pick one of several), and none, or a
plane publishing no set, stays UNDETERMINED and stated. **What remains, stated:** a project citing SEVERAL questions still has no
seeded target, so its table-made level-empty candidates are refused `SUGGEST_NO_TARGET`; which question an empty level of such a
run belongs to is undetermined (DESIGN GAP D-572, Incomplete sections).
...
**`replay` IS THE SERVER'S WORD, NEVER THE CALLER'S — RULED 2026-09-24 by BOB #33, on D-505's finding.** `op=promote` exempts a
promotion carrying `replay: true` from every shape fence it has: the gathering grammar, the inquiry and action basis arms, the
correspondence arms, the bias arm, the group stamp, and C-32.19's rule that no machine writes a member's `risk_tier`. The
exemption is right for what it is for, because a replay re-states the record's own past verbatim and that past predates the fences.
But the flag arrives in the request body and nothing removes it. Measured by D-505 (`risk-tier.test.mjs` §7 arm (ix)): a machine
credential that sends `replay: true` lands `risk_tier: 1` on a creation, and `op=projection` publishes it. `CLAUDE.md` §5 already
answers this: a provenance hop a caller can hand us is one a caller can invent. The migration ruling above already applies the
same principle one field over, because `migrationReplay` is deleted for every caller and set only by the server. **So `replay`
takes the same shape, in two steps.**
(1) **Now, as a fence:** the plane deletes a caller's `replay` unless the call arrives under the ADMIN class with no session, which
is the only class `migrate.mjs` uses (it narrowed to admin at REC-173). A member session, a member, probe or `ai` token, and any
other class has the flag removed before the store sees it, so every fence applies to it. This closes the measured hole without
touching migration. D-505's arm (ix) is INVERTED, not deleted.
(2) **The end state, a build:** `replay` is honoured only where the server VERIFIES it, as REC-173 does for an inquiry's creation.
Every replayed promotion, of any type and any revision, names its drive-provenance capture, and the plane checks that the held
bytes' preserved promotion record lists this bundle and this revision's `bundle.md` SHA-256. `migrate.mjs` already carries that
register. After this step even the root of trust cannot assert a replay it cannot show, and step (1)'s class test is kept as a
second condition, not removed. Until step (2) is built, an admin-class caller can still assert `replay` without proof. The record
does not model the root of trust's honesty (Membership §DEC-2, deferred), so that is the residue both steps leave, and it is stated.

**STEP (1) BUILT 2026-09-24 by D-511 (IC-278, PROPOSED).** `index.mjs`'s `op=promote` stamp block deletes a
caller's `replay` unless the call arrives under the ADMIN class with no session — one statement, placed before the
`migrationReplay` block, which remains the only writer of `b.replay` that is the SERVER'S. Every other caller's
promotion is judged by the fences it tried to skip, so D-505's measured hole answers `MACHINE_CANNOT_SET_RISK_TIER`
(C-32.19) by name with nothing written, and `migrate.mjs` — which refuses to run under any class but admin since
REC-173 — is byte-unchanged (`migrate.test.mjs` and `rec173-migration-replay.test.mjs` green, unedited). No code,
check, region or translation is minted: `check-refusal-codes.mjs --strict` reads every ratchet key at slack 0,
unmoved, and what moves is the REACH of refusals that already existed. **WHICH SESSION THE CLASS TEST CANNOT SEE,
MEASURED AND NOT ASSUMED:** the FOUNDER'S session — stored role the literal `admin`, `Store.ROOT_ADMIN`,
`rootOfTrust: true` — arrives as `cls === "admin"` exactly as the deploy token does, which is why `!viaSession` is
half the rule; an ADMIN-ROLE MEMBER's session does NOT (a member login stores `member:<id>`, and her `role` decides
only her capabilities), and the first draft of the build asserted that it did. This section's own negative control
caught it: the `any-session` arm came back GREEN and `op=whoami` then measured the classes. Four suites that drove
a replay under a caller that may no longer assert one were CORRECTED and never exempted — `gathering`, `reopen`,
`suggest` and `testimonyaxis` now drive the same subjects under the root of trust, and two of them became genuine
REVISIONS, which is what their own labels always said. ~~Step (2) is not built, and its residue is stated above.~~ Step
(2) was built by D-512 — the next paragraph.

**STEP (2) BUILT 2026-09-24 by D-512.** `index.mjs`'s `op=promote` stamp block reads the caller's `replay` ONCE, after
step (1)'s class delete, and deletes it; the only write of `b.replay` after that line is the server's, on a replay it
verified. The verification is REC-173's `migrationReplayOf`, unchanged and now asked of EVERY promotion that claims a
replay, of any type and any revision (nothing in it was particular to an inquiry or a creation): the promotion names a
capture (`provenanceCapture`) that it registers at `migration/drive-provenance.json`, whose bytes are HELD — read back
from R2 and re-hashed to the sha named — and one of whose preserved promotion records has `target` = this bundle and
lists `bundle.md` at the SHA-256 computed from the text being promoted. Step (1)'s class test is kept as the SECOND
condition, as ruled: it runs first and removes every non-admin flag, and the verification is asked only for the admin
class with no session. An ADMIN that claims a replay the held bytes do not show is refused **`REPLAY_UNVERIFIED`
(C-66.6)** before the store is called, so nothing is written; an inquiry CREATION that claims nothing is still asked as
REC-173 built it (verified, a migration replay; unverified, an ordinary creation under rule 2 and D-78). `migrate.mjs`
now names and registers its provenance capture on EVERY revision (a re-registration under the same bundle is the
register's upsert), and `migrate.test.mjs` migrates clean. **Superseded, and said so:** the migration ruling's "refused
`SURFACE_NO_RUN` as today" now holds for a creation that claims NO replay; one in `migrate.mjs`'s shape, which claims
one, is refused `REPLAY_UNVERIFIED` — `rec173-migration-replay.test.mjs`'s arms were CORRECTED to that, and ARM N7 was
split so the ordinary-creation half (N7b, no claim) is still driven. And REC-179's "`replay: true` on a revision is the
caller's word" is no longer true: a revision's replay is verified now, and a verified replay is STILL no exemption from
C-66.5 (ARM F4, made stronger). `risk-tier.test.mjs` §8's arm (δ) and its residue arm are INVERTED, never deleted: the
root of trust's bare `replay: true` is refused by name and no tier is published; its replay over a verified capture
lands as `promotion-replay`; a MEMBER token carrying the same proof is still refused C-32.19. Seven further suites that
drove an admin replay on the caller's word were CORRECTED to carry the proof (`test/replay-proof.mjs`), none exempted.
**TWO PROVISIONAL CHOICES, where the ruling is silent, each with its alternative.** (i) REFUSE, NOT DOWNGRADE: an
admin's unverifiable claim could instead have its flag deleted, as step (1) does for every other class, and be judged
by the fences. That is wrong for the one caller that sends the flag honestly — an ordinary creation is rewritten on the
way in (D-436's group stamp, D-78's restamp), so `migrate.mjs` would report success over bytes the Drive record does not
list — and the QUEUE row's accepts-when asks for a refusal by name. Reversing it is one statement and one row. (ii)
RE-REGISTER ON EVERY REVISION: the verification keeps REC-173's condition that THIS promotion registers the capture,
rather than accepting a capture already registered against the bundle, because the register has one writer and a
revision can always carry the entry; the alternative asks the store, a second read across the trust boundary. **What
remains is the residue both steps leave, stated:** the provenance capture is itself uploaded by the root of trust, whose
honesty the record does not model (Membership §DEC-2, deferred) — after this step no caller can ASSERT a replay the held
bytes do not list, and an admin can still FABRICATE the bytes.

**BUILT 2026-09-23 by REC-173 (IC-190).** `index.mjs`'s `op=promote` stamp block deletes the server's `migrationReplay` stamp
for every caller, and for a non-session ADMIN caller creating an inquiry asks `migrationReplayOf`: the named capture
(`provenanceCapture`) is registered by this creation at `migration/drive-provenance.json`, its bytes are HELD (read back from
R2 and hashed), and one preserved record has `target` = this bundle id and lists `bundle.md` at the SHA-256 computed from the
text being promoted (the file's `sha256` must equal it). A verified replay carries no `assistantPrincipal` (REC-171's stamp
stands, and a line after it removes the stamp), skips D-78's restamp, and writes one `inquiry_migration_replays` row that
`op=projection`'s `surfaced_in` reads as `not recorded (migrated from the Drive era)`. **The order, measured:** `migrate.mjs`
uploaded every capture before any promote but registered the provenance only on the LAST revision — AFTER the creation — so
it was reordered: the creation registers and names it (and the last revision still registers it, for a resume). Because the
register has one writer, `promote`, "already in the record" is built as bytes held before the creation and registered by the
creation itself — a DESIGN GAP stated in Incomplete sections. `migrate.mjs`'s token narrows to admin.

**A REVISION CARRIES `surfaced_by` FORWARD — BUILT 2026-09-23 by REC-179 (IC-197, PROPOSED).** Rule 2's reach rests on `surfaced_by` recording the SURFACING ACT, decided once at the trust boundary; D-78's restamp runs only on a creation, and its own comment states that a revision carries the value forward. `store.mjs` `promote` now asks it of every revision of a bundle whose current version is an inquiry, after the compare-and-swap and before any write (region `is-promote-surfaced-by`): the value the catalog's parser reads out of each version's `bundle.md` must be the same — an absent or unreadable value counts as a value, so a revision can neither drop the origin nor supply one its creation did not record — else `SURFACED_BY_REWRITTEN` (C-66.5), in either direction and whoever the caller. `replay: true` on a revision is the caller's word and exempts nothing; a migration revision whose Drive-era history changes the value would be refused, and that case is brought to BOB rather than built.
...
- **The anti-gaming keystone survives machine composition, and two mechanisms carry it**
  ...
  *"your answer fails only if ALL of these fail"* — and the member AFFIRMS independent
  ...
  authored before strength is shown — holds. BUILT IN PART (REC-161, 2026-09-23, IC-221):
  `op=partitionindependence` is `#independenceOf`'s THIRD consumer — beside `op=suggest`'s check
  and `op=versionstrength`'s read of a STORED version — computing it over a partition not yet
  written. ~~NOT BUILT: the elicitation's read-back does not call it yet.~~ BUILT 2026-09-25 by UI-75: the
  read-back asks `op=partitionindependence` over the partition the member's answers propose, once, and
  names each shared origin between the sets it lists beside the falsifier — the accept ceremony's
  rendering, called (`originsHtml`). It keeps only `independence` from the answer, offers the control
  to record while the read is in flight, and `doElicit` never reads it, so the answers are written as
  given (`civicos-ui/test/elicitation.test.mjs`, six-arm control).
...
- **A leg marked as a HUNCH** (§6.7 — a member marking, always) is visible as such and
  ...
  **RULED 2026-09-23 by BOB #31 (UI-74's finding 5; S17-1 Q4):** a read answers `independence` ON ITS OWN, apart from
  ...
  BUILT on the plane (one RECORD row, with a UI row behind it if the page must switch).~~ BUILT on the plane 2026-09-24 by
  REC-192 (IC-248): `op=partitionindependence` takes `version=<name>` and answers that stored version's `independence`
  through the same `#independenceOf`, with no strength key and not state-gated, equal to `op=versionstrength`'s for the same
  version (`bio-plane/test/partitionindependence.test.mjs` block H). ~~NOT BUILT: the UI row — the accept ceremony still asks
  `op=versionstrength` and drops the pair at the fetch.~~ BUILT 2026-09-25 by UI-88: `acerOriginsRead` asks
  `op=partitionindependence&version=`, naming no state, so no code path fetches a strength-bearing answer before the
  affirmation; `civicos-ui/test/accept-ceremony.test.mjs`'s PRE-AFFIRMATION FETCH arms judge the network log itself.
...
**A declared bound STATES its allowance — RULED 2026-09-23 by BOB #30, on REC-172's builder's finding. BUILT 2026-09-23 by REC-177 (IC-195, PROPOSED): `AI_RUN_BOUND_NO_ALLOWANCE`, C-22.16, in `airun.mjs checkConsume`'s allowance arm; a present figure of the wrong form (a string, a fraction, a negative) stays C-22.13.** `op=airunopen`
