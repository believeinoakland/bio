...
- §12 (a)–(b) — the ACCEPT CEREMONY is BUILT at the member surface (UI-74, 2026-09-23; `civicos-ui/test/accept-ceremony.test.mjs`, real plane): D-195's derived shared origin (`op=versionstrength`'s `independence`) is shown before the per-set affirmation, which is sent as `affirmed=` and refused by the plane if incomplete (D-271). Two gaps stay: `independence` is published only beside the strength pair, so clause 5's ordering is held by the page dropping the pair rather than by the wire; and a lens that MOVED (DEC-46) is rendered but not driven by a fixture.
