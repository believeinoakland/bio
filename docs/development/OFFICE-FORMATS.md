...
change to the content extent census and goes through an IC. **BUILT 2026-09-25 by REC-204** (RECORD): the `envelope`
kind with its four item kinds is in the check catalogue, `op=acquire` asks each office entry's `structure()` for the
envelope and indexes each item as one unit after the body, labelled ENVELOPE in its extent kind, its `ref` and its text,
and a minted envelope row carries the capture's chain and cap with `cited_as: envelope` (`search.test.mjs` Phase 4). NOT
BUILT and stated at `construct-status.json` 5.envelope: a leg citing an envelope item, the producers' formula and
hidden-* kinds, and re-projection of captures acquired before the landing. The IC for the census is the integrator's.
