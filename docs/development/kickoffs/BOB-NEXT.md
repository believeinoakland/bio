...
- **21:06Z ROOT CAUSE AND STRUCTURAL FIX (Bob: WHY is the count always below 16?).** ROOT CAUSE: CONDUCT did both the long work (trains, 30-60 min) and the short work (flip finished rows, spawn). Every refill took 3 handoffs between 2 lanes that act only when woken, so slots emptied whenever CONDUCT was busy training. And the finished rows kept reading 'running', so the cache looked full. RULED by BOB #33: DISPATCH (flip finished rows, refill, spawn) is SCHEDULER's end to end, using list_sessions' status_bucket at every SCHEDULER wake. CONDUCT keeps verification, integration, trains, archiving and answering workers. BOB's 20-min slot probe (slots.py) is the backstop; the timer quota forbids a faster one. OWED: correct kickoffs/SCHEDULER.md and CONDUCT.md for the split on BOB's next batch branch.
