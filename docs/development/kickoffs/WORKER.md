...
- **KEEP EVERY SCRATCH FILE OUT OF YOUR WORKTREE — in the SESSION SCRATCHPAD your harness names:**
  ...
  **RULED by BOB #32, 2026-09-24**, superseding "inside your own worktree": a file in the worktree
  ...
  GREEN verdict (three items paid for it on 2026-09-24: REC-185, D-487, D-486). **The scratchpad is neither
  ISOLATED between sessions** (two workers reported that) **nor DURABLE** — name every file there for YOUR item,
  never generically (and see the `provenance:` rule and the `/tmp` section below).
