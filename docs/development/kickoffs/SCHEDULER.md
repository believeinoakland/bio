...
6. **Keep the cache ahead of CONDUCT.** It holds 20 (`CACHE_ROWS`, sized to CONDUCT's capacity plus spare — Bob, `WORK-PIPELINE.md`); CONDUCT spawns continuously. If the cache holds fewer than 4
...
- **NEVER ASK A LANE TO HOLD `main` FOR YOUR GATE** (Bob, 2026-09-22; `CLAUDE.md` §6). Gate a CLEAN, committed tree so its verdict records; if `main` moved, rebase onto a PINNED sha, run `gates.mjs --since <measured commit>`, and push under a NEW branch name.
