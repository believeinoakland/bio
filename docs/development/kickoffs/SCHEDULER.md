...
- **ASK FOR `main` BEFORE YOU GATE A DOCS LANDING when other lanes are landing** (SCHEDULER #9, 2026-09-21): `main` moved three times under one gated landing (BOB twice, DIST once), each move a rebase and a ~4-minute re-gate. CONDUCT, BOB and DIST each held on request until "landed", and said so. Ask before the gate, not after it. **And ask EVERY live lane that lands on `main`, one mid-refresh included** (SCHEDULER #10): BOB #24's and DIST #3's handoffs each moved `main` under a gate; on a non-fast-forward, fetch, rebase, re-gate, and push the rebased commit under a NEW branch name.
