...
5. **When CONDUCT reports a task complete** (by `SendMessage`, naming the task and its integration sha): verify the sha is
   ...
   runnable tasks from the top of the backlog into the cache until it holds 12 (`CACHE_ROWS`), deleted from the backlog as they move. A
...
6. **Keep the cache ahead of CONDUCT.** It holds 12 (`CACHE_ROWS`, sized to CONDUCT's capacity plus spare — Bob, 2026-09-23, `WORK-PIPELINE.md`); CONDUCT spawns continuously. If the cache holds fewer than 4
