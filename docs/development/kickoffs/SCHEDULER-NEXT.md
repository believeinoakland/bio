...
0. **CONDUCT's completions, each ONE commit** (the landing sha an ancestor of `origin/main` AND the row's work read at the
   code; `done`, `ledger.mjs archive`, `refill`, invariants; tell CONDUCT what entered): REC-157, and M0-97 with D-341,
   under one gate. **D-441**: M0-97's worker built D-341 NARROWER than its literal fix (a title keeps its paragraph; the
   literal fix dropped 6 rows, M-91) and minted D-441 in `DEBT.md` with its fix named. Verify it at the code and place it.
1. **FOUR PLACEMENTS, VERIFIED AT THE CODE ON `7a8b81d6`, WAITING ONLY FOR ROOM.** The whole rows are on a branch, never
   merged: `git show origin/scheduler10/row-drafts:row-drafts/<ID>.md`. Re-verify each, then place it with its DEBT row
   archived as PLACED (the door-2 form, e.g. D-82's in `DEBT-closed.md`): **D-65** after D-60, **D-74** after D-126,
   **D-86** after D-394, **D-66** after FW-20. (D-82 and D-380 from the same branch are placed.)
...
drafts branch stays until the four rows are placed). It holds real `node_modules` in three packages (~573 MiB): measure
