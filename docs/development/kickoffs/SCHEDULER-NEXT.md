...
was MEASURED at `origin/main` `54cec171` plus this landing (~23:30Z). It is a POINTER: re-measure before you rest anything on it. The
...
- **Cache (8):** LED-7 (this lane's own act, never a worker slot) · D-436 (`running`) · REC-157 · M0-97 · D-341 · M0-81 ·
  D-293 (CONDUCT #10 flips it `running` in its handoff) · M0-99. **M0-97 and D-341 are ONE worker** (`tools/decided.mjs`),
...
0. **CONDUCT's completions** (D-436; then D-293 WITH M0-98): one commit each — the landing sha an ancestor of `origin/main` AND the row's
