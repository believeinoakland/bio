...
- **As built (M0-126, 2026-09-23) — `tools/gateresults.mjs` (the key, the branch's reader and writer, revocation),
  ...
  **Decided here where the design was silent (M0-126's builder):**
  ...
  10. **The tree-keyed reuse stays as a local fast path** (§2d, M0-122's `recordedGreen`): still correct, and cheaper
      than keying; the per-unit record supersedes it as the shared mechanism.
