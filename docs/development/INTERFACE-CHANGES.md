...
- **Interface:** I3 (plane → UI, the op contracts). **Base read off THIS TREE: 34.0.0** (33.0.0 at spawn on `dd52609b`;
  IC-155, REC-138, moved it while this was built and was merged in before close-out). **Proposed MAJOR — 34.0.0 → 35.0.0**,
  by IC-137's precedent: a refusal where none stood. Read the base AT RESOLUTION.
...
- **Interface:** I3 (plane → UI, the op contracts). **Version read off this tree's `docs/development/INTERFACES.md`
  (base `0cb784ab`): 41.1.0; RE-READ at this branch's merge with origin/main `a04e04dc` (REC-153's IC-163 accepted):
  42.0.0. Proposed as MAJOR — 42.0.0 → 43.0.0.** Read the base AT RESOLUTION. **Why not additive:** a
...
**RESOLUTION · 2026-09-19 · ACCEPTED by CONDUCT #7 as MAJOR — I3 43.0.0 → 44.0.0.** Base RE-READ at resolution off the integration tree: 43.0.0. The row proposed 42.0.0 → 43.0.0 and IC-165 landed underneath it, so the proposed base was stale — read at RESOLUTION, by this file's own rule. Breaking by IC-137: a call that SUCCEEDED is now refused — `op=allocid` with `PROJ`, `CASE`, `DRAFT`, `RVG` or `TASK` answers `ALLOCID_PREFIX_GATED` (C-59.5) and allocates nothing — and the MEANING of a gated id changes: its four digits no longer order or count anything, so a consumer sorting or comparing by id to recover minting order becomes wrong without changing a line. The SHAPE (`<P>-<year>-\d{4}[-slug]`) is unchanged, so `BUNDLE_ID_RE`, C-19.1's TASK grammar and every reader of the shape still hold. One CSPRNG minter (`Store#mintOpaqueId`) now serves all five gated prefixes; REC-141's PROJ draw was lifted into it. **An AUTHORITY and DISCLOSURE closing** — a sequential id counted objects the caller cannot see (§7.9); DIST told. **`TASK`'s gating is RULED CORRECT by BOB #17 (`63926201`, 2026-09-19) and STAYS.** It was raised as a question at this resolution — Membership v2 §7 enumerated CASE/DRAFT/RVG/PROJ — and BOB answered that the rule is the PREDICATE preceding that list (*every prefix whose objects a read withholds from some caller*), not the list: the builder was instructed to enumerate every `allocId` caller and state per prefix which rule governs, and did. `TASK` satisfies the predicate — `taskList`'s `#bundleGate("tk.refers_to", viewer)` (REC-30) withholds a task whose bundle the viewer cannot see, so a counted `TASK` suffix says how many such tasks exist. §7 now carries the dated amendment naming the set `PROJ`, `CASE`, `DRAFT`, `RVG`, `TASK` with the read that withholds each. No correction owed to this landing. D-432 (an opaque id can be reissued across a whole-store purge, which the counter could not) is MINTED with its fix named, not closed here.
