...
**Status** · v0.1 DESIGN, written by session BOB #14 under Bob's standing delegation (mechanism is the architect's; the doctrine it serves was ruled by Bob on 2026-09-14 and is quoted, not restated). Amended the same day with five decisions from MK-1's build (§2, §3, §7). It closes the design act D-184 and D-194 owed — which BOB #14 had wrongly marked "verified: Program B" in a build order, until CONDUCT #4 found no such design at the artifact. **§5 (THE LEAD) IS BUILT by MK-4 (2026-09-18, IC-135 on I5, IC-136 on I3): the `leads` table, `op=lead`, `op=leadlook` writing `observation_log` under `authority_kind = 'lead'`, `op=leadread`, and §7's lead-as-leg refusal as C-54.1 at every leg grammar; its visibility is a provisional (the author's), reported as a design gap. The rest is not built by MK-4.** `node tools/status.mjs 2.firsthand`, `10.lead` and `13.attribution` are the status authority. as of 2026-09-18
