...
- §5 — the lead's surface (how a member writes one, where it is shown) was Program B's; **BUILT by D-194 (2026-09-25)** as above. STILL OPEN, and the plane's rather than the surface's: no lead op lists a member's leads, so the surface lists from `op=frontier&level=internet`, which keeps the latest look per SUBJECT (the lead's words) — a lead whose words repeat another's that was looked at later is not listed; and no lead op publishes the observation states' meanings, so the surface MIRRORS `OBSERVATION_STATES` under a guard. The first is D-681, the second D-682, both sent to SCHEDULER by D-194.
