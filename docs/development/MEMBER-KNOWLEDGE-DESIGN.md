...
- §4 — BUILD ONLY. The design is complete as of 2026-09-21 (§4.1–§4.6, BOB #19). It answers the five questions MK-3's first act raised when it found no contribution act in this plane. Its measurements are §4.0, moved there verbatim from this entry. **Replacement (i), §4.1, BUILT by MK-6 (2026-09-23):** `op=testify` names the author in every file of the bundle as `observer:<testimony id>` (`Store.observerRef`), and the register's `author` column alone resolves it; `test/mk6-bundle-names-no-author.test.mjs` proves it as a population over a fixture plane with the fence cut out in memory. `_history/` still names the promoting member, and it is never published (`op=ratify` skips it). **Replacement (ii), §4.2–§4.6, BUILT by MK-7 (2026-09-25, IC-319, C-92)** — see Status. Its acceptance is `test/mk7-attribution.test.mjs`: a round trip per level through the ops over the whole population of published parts. STILL OPEN: (a) the two PROVISIONALS, §4.4's narrow veto and §4.6's `name` = handle, built as written and carried to BOB #34; (b) HOW AN AUTHOR LEARNS that a prepared edition reaches their observation — no read tells them, so today the case id reaches them out of band (a surface question for Program B, and possibly a notification); (c) a draft of a NEW case has no identity to key a level to, so the act lands only once `op=publish` has prepared the case document — REC-194's same gap for acknowledgements; (d) D-598, routed to BOB: once an observation is published as a case's evidence, C-21.2 lets a new finding cite it only ungraded.
- §5 — the lead's surface (how a member writes one, where it is shown) was Program B's; **BUILT by D-194 (2026-09-25)** as above. The list's gap (D-681) is CLOSED: D-194 listed from `op=frontier&level=internet`, which keeps the latest look per SUBJECT (the lead's words), so a lead whose words repeated another's looked at later was not listed; `op=leadlist` (D-681, 2026-09-25) lists each lead the viewer may read once, and the surface reads it. The frontier was NOT regrouped — "latest look per subject" is its contract at every level (`OBSERVATION-LOG-DESIGN.md` §6). Until D-682, and the plane's rather than the surface's: no lead op published the observation states' meanings, so the surface mirrored `OBSERVATION_STATES` under a guard. **D-682 is BUILT (2026-09-25):** `op=leadread` and `op=frontier&level=internet` carry `vocabulary: { states, outcomes }` (`Store.LEAD_VOCABULARY`), the states in a member's words (`OBSERVATION_STATE_WORDS`, `partial` without its maintainer's note), and the surface renders from it and holds no copy. Since the c22-batch30 union (D-681 x D-682) `op=leadlist` carries the same `vocabulary`, and the surface's list renders from it.
...
- [4. Attribution: chosen by the attesting member, carried with the act](#4-attribution-chosen-by-the-attesting-member-carried-with-the-act)
  ...
  - [4.4 An unchosen observation blocks its own use, not the case: question 2 (PROVISIONAL — built as written by MK-7, C-92.10; carried to BOB #34, 2026-09-25)](#44-an-unchosen-observation-blocks-its-own-use-not-the-case-question-2-provisional-built-as-written-by-mk-7-c-9210-carried-to-bob-34-2026-09-25)
  - [4.5 A member who cannot act: question 3](#45-a-member-who-cannot-act-question-3)
  - [4.6 What `name` and `cover` publish (PROVISIONAL — `name` = handle built as written by MK-7, C-92.9; carried to BOB #34, 2026-09-25)](#46-what-name-and-cover-publish-provisional-name-handle-built-as-written-by-mk-7-c-929-carried-to-bob-34-2026-09-25)
...
### 4.4 An unchosen observation blocks its own use, not the case: question 2 (PROVISIONAL — built as written by MK-7, C-92.10; carried to BOB #34, 2026-09-25)
...
### 4.6 What `name` and `cover` publish (PROVISIONAL — `name` = handle built as written by MK-7, C-92.9; carried to BOB #34, 2026-09-25)
...
- **THE SCOPE of this rule is constructs whose EXISTENCE is ruled undisclosed — the lead and its looks, AND a HIDDEN
  project** (Membership v2 §7.9, *"not its existence"*; corrected 2026-09-24 by BOB #32 on D-464's finding. The 2026-09-18
  search missed §7.9). What a run over a hidden project produces is the project's thinking: counts and frontier tallies
  served outside its sight subtract those rows (D-464, D-486). Every other `op=stats`
...
| MK-3's replacement (ii) — **MK-7, BUILT 2026-09-25** (`op=attribute`, IC-319, C-92) | THE ACT AND THE LIFT (§4.2–§4.6): the attribution act, an op the builder names and, if a design names it before it is built, registers in `op-claims.mjs`' `PLANNED_OPS`; each edition's attribution statements in the case document; the ratification refusal naming every unchosen observation; `name` refused without a handle. **Then lifting MK-1's fence (C-53.10–.12) as this item's own act, with a control arm per level.** Off-the-record stays a structural absence. | I3 | replacement (i); the review copy (REC-126, built) |
