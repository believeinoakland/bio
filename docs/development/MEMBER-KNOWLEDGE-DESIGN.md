...
- §4 — BUILD ONLY. The design is complete as of 2026-09-21 (§4.1–§4.6, BOB #19). It answers the five questions MK-3's first act raised when it found no contribution act in this plane. Its measurements are §4.0, moved there verbatim from this entry. NOT BUILT: the two items that replace MK-3 in §8. `13.attribution` stays ABSENT, and MK-1's fence (C-53.10–.12) stands until the second of those items lifts it. Two rulings are carried to Bob as provisionals that are cheap to reverse until the act is built: §4.4's narrow veto and §4.6's reading of `name`.
...
- **THE SCOPE of this rule is constructs whose EXISTENCE is ruled undisclosed — the lead and its looks, AND a HIDDEN
  ...
  served outside its sight subtract those rows (D-464). Every other `op=stats`
...
| MK-3's replacement (i) | THE BUNDLE NAMES NO AUTHOR (§4.1): `observer:<testimony id>` in every file and manifest record an authored bundle can publish, resolved privately by the register; the population arm and its control | I3, I5 (the authored provenance document's shape; the builder states additive or breaking) | MK-1 |
