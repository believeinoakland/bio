...
- **ONE KEY NEVER CARRIES TWO MEANINGS (`kickoffs/BOB.md` rule 7 — added 2026-09-18 by BOB #15, resuming REC-131 before
  integration).** The bullet above re-meant `observations` on the wire while `op=purge`'s proof kept the whole-log meaning
  under the same name. So the key whose meaning CHANGED is renamed: the wire publishes **`observationsNonLead`**, a name that
  states its predicate (a later existence-private construct cannot join the exclusion without another rename, i.e. without
  an IC), and carries no `observations` key; `op=purge`'s `observations` keeps the whole-log meaning it always had.
- **`dbBytes` leaves `op=stats` (and `op=selftest`/`op=livefire`) for MEMBER and PROBE (BOB #15, same pass).** The
  database's size moves in WHOLE PAGES on every write, a lead's included: measured on REC-131's suite, one lead near the
  128 KiB cap (C-54.4) grew it by ~364 KiB while a short lead moved nothing, so a member diffing it across a colleague's
  authoring could detect a large lead. **The `admin` class keeps it, because capacity is an operator need. THE RESIDUE,
  STATED:** the operator can see that SOMETHING large was written; the figure mixes every write, it cannot tell a lead from
  any other, and no lead is readable to it. This is the one class distinction `op=stats` keeps, it governs `dbBytes` and
  nothing else, and it is set by the SERVER from the authenticated class, never by the caller (`capacity`).
