...
- §5 — the lead's surface (how a member writes one, where it is shown) was Program B's; **BUILT by D-194 (2026-09-25)** as above. The list's gap (D-681) is CLOSED: D-194 listed from `op=frontier&level=internet`, which keeps the latest look per SUBJECT (the lead's words), so a lead whose words repeated another's looked at later was not listed; `op=leadlist` (D-681, 2026-09-25) lists each lead the viewer may read once, and the surface reads it. The frontier was NOT regrouped — "latest look per subject" is its contract at every level (`OBSERVATION-LOG-DESIGN.md` §6). STILL OPEN, and the plane's rather than the surface's: no lead op publishes the observation states' meanings, so the surface MIRRORS `OBSERVATION_STATES` under a guard (D-682).
