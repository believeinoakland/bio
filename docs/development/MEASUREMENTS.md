...
## 2026-09-11 · CPDF-16 — the floor instrument gains D-305's fifth expression and D-314's noise gate, and NOTHING COMMITTED MOVED (worktree `agent-ac13e0cfc839cfcd7`)

**Instrument:** `bio-plane/test/ocr-measure-probe.mjs` (CPDF-9's floor, edited ADDITIVELY),
its run of record 2026-09-11 on this machine (calibration 40,000,000 ref-iter = 1,450 ms);
controls driven by `bio-plane/test/cpdf16-floor-controls.mjs` (8 arms) and by both landed
probes' own `--controls` suites. Nothing deployed, no service touched: the floor engine
runs locally (tesseract.js@7.0.0; models eng 4.0.0_best_int and tessdata_fast eng).

**The comparability constraint, held and asserted by the instruments that own it:** the four
metric expressions, `norm`, `levenshteinPairs` and `GT_PAGE2` are byte-unchanged — CPDF-14's
and CPDF-15's guards both exit 0 against the edited file (GT digest `17dff6b3…` unmoved), and
their own NC2 control suites came back all-as-declared against it (CPDF-14: 4/4; CPDF-15: 5/5
including the engine-byte arm). Each pinned literal occurs EXACTLY once in the edited file,
measured — a duplicate would have silently disarmed their first-occurrence mutation arms.

**Run of record, page 2 vs the human ground truth (2,687 normalised chars, 90 digits) — the
four committed columns beside the new fifth:**

| model | char acc | digit errors | MINTED | DISAGREE (new, D-305) |
| --- | --- | --- | --- | --- |
| eng 4.0.0_best_int | 99.93% (2 edits) | 0/90 | 1 | **0** |
| eng tessdata_fast | 99.96% (1 edit) | 0/90 | 0 | **0** |

Both rows are CPDF-9's 2026-08-03 committed figures EXACTLY (same edits: `$`->`5`,`l`->`{`;
`s`->`S`) — **nothing in any committed figure moved**, as the additive constraint predicted.
The fifth column is defined as: a position where the ground truth has a digit, the output has
a digit OPPOSITE it, and they differ — the class `minted` is blind to. The D-305 exhibit is
driven on EVERY run before any engine is installed: GT with `$50,000`->`$10,000` scores
disagree=1, minted=0, and the instrument refuses to proceed if either side fails.

**The noise gate (D-314), run of record:** blank control PASS on both models; noise control
(CPDF-11's own recipe, seed 11, the ground-truthed page's dimensions, generated by the floor
itself) **FAIL on BOTH models** — tessdata_fast invented **9,968 chars** (confidence 11),
reproducing CPDF-15's D-314 figure EXACTLY by a different harness, and **eng 4.0.0_best_int,
unmeasured until now, invented 7,558 chars** (confidence 5). So NEITHER local model may serve
as an agreement reference today: both FLOOR REFERENCE verdicts read `undetermined — noise
gate FAILED`, both directions of the driven agreement-with-the-floor column answered
`undetermined` naming the gate (never a number), and the instrument exits 1, which is the
honest exit for an engine failing its own control. The gate refuses FUTURE references; it
re-grades nothing already published.

**Negative controls (driver, 8 arms, each alone, FOOT: 8/0 not-as-declared on the final
run):** baseline all-zero + numeric agreement over a clean gate; substitution-only corruption
moved ONLY the new column (disagree=1, minted=0, both asserted); letter-for-letter error
moved neither (over-strictness); noise-failing gate answered the string `undetermined`
naming the gate; clean gate answered a number (over-strictness); one pinned expression
mutated in the REAL file -> BOTH guards exit 4 naming the expression; one GT digit mutated ->
BOTH guards exit 4 NAMING THE DIGEST; restored file -> both guards exit 0. Every restore
verified by sha256 AND cmp against per-arm uniquely-named pristine copies (20,630 B, floored).

**What the first arm found, recorded not smoothed (D-315):** mutating an expression to a
SUPERSTRING (`* 100` -> `* 100.0`) left BOTH guards at exit 0 — their pin is a
substring-presence check and cannot see extension. The arm was corrected to a
substring-removing mutation; the guards' narrowness is D-315, owned by the probes' files,
which this item's claim deliberately excludes.
...
## 2026-09-14 — SK-7 (RESPAWN): THE ENTRY ABOVE HAS A FALSE PREMISE, AND THE FALSIFIER LANDED WHILE ITS AUTHOR WAS DEAD

**What is corrected.** The mint-label entry above closes with *"anything outside those four ops —
including `civicos-ui/`, which **renders no content row at all today** (measured: `op=content`
landed with REC-83 and has zero consumers in `civicos-ui/app.html`), so there was no UI label to add
and none was invented."* That was measured on `origin/main` at `3f92e5c` and was correct there.
**It is false on `origin/main` today.** UI-61 landed `legReferentHtml` in `civicos-ui/app.html`,
which takes a content row's `standing` — the object `#contentStanding` composes, and the object this
item puts `mint` on — and renders its `ref` to a member, with the plane's `says` when the row is
stale. Driven by `civicos-ui/test/content-extent.test.mjs`, which was already in the tree when the
sentence above was written into it.

**So the walk's stated blind spot was real and the conclusion drawn from it was not.** The walk
itself is honest — it names what it cannot see, which is why this was findable at all — but the
sentence *"there was no UI label to add"* is not a limit of the instrument, it is a claim about the
world, and the world had moved. **A measurement's expiry is not the instrument's fault and is still
the measurement's problem.** The lesson for the next worker who cuts a branch and is gone for a
while: re-run the census, do not re-read it.

**Re-measured on the merged tree.** `op=content`'s consumers in `civicos-ui/app.html`: still 0 (the
op itself has no caller). **`op=earnedbasis`'s `standing` consumers: 1** — `legReferentHtml`, which
is the surface the rule *labelled everywhere it is shown* actually reaches. The label is rendered
there from this landing, through the plane's own `machine_work` predicate and the plane's own
`says`. `op=attesttext` in `civicos-ui/app.html`: still **0**. In `agent-worker/`: still **0**.
The second measurement in the entry above — that `op=attesttext` reads its attestor from the request
body — was RE-TAKEN against `origin/main` today and **still holds**; nothing between the two
measurements closed it.

## 2026-09-14 — SK-7 (RESPAWN): baseline and final, re-measured on a tree the first worker never saw

**Baseline**, on a pristine scratch `git worktree add` of `origin/main` at `f38af22` (never `git
stash`, which is repository-wide across every worktree) with `npm ci` run in `bio-plane/`,
`pdf-worker/` and `ocr-worker/` first: **195/195 suites green · 12,100 assertions · exit 0**,
324.8s. **The spawn brief's figure was 195/195 · 12,100 and it was RIGHT** — measured rather than
trusted, and reported either way.

**The first worker's own baseline is kept and is not wrong**: 190/190 · 11,713 on `3f92e5c`. The
delta is five suites and 387 assertions that landed between the two cuts (REC-84, REC-85, CAP-8,
CAP-9, FW-17, UI-61). **Two baselines taken on two trees are two facts, not a disagreement**, and
the reason this is written down is that a respawn quoting its predecessor's figure would have been
quoting a number true of neither tree — the shape `coverage.mjs`'s own `REGISTER_FLOOR` comment
records six separate times.

**NEGATIVE CONTROL, re-run in full on the merged tree**: six arms, each armed ALONE with the others
held open, every restore verified byte-identically by sha256 AND by content. **ALL SIX AS DECLARED,
none re-cut and none re-declared** — baseline 45/0 · attest 39/6 (5/5) · label 41/4 (4/4) · finding
...
*(Filed by FW-18 as `M-21` and RENUMBERED to `M-24` at REC-90's integration, 2026-09-15 by CONDUCT #11 — **and this one reached `main` before it was caught, which is the honest part of the record.** FW-18 merged at `a59ac7b` carrying a second `## M-21` heading beside M0-34's, and nothing failed: no gate reads this file for duplicate ids. It was found only when a THIRD collision on the same number turned up one merge later and the headings were counted. **FOUR workers of one wave took `M-21` on the same day** — M0-34, COFF-11 (→ `M-22`), REC-90 (→ `M-23`) and this one. **`M` is one of the two namespaces `mintid --audit` cannot grade** (CORRECTED 2026-09-15 at BOB #11's check: the allocator DOES allocate `M`; the AUDIT cannot, because `M` declares no allocation site — so these four were a BYPASS of a working allocator, not a gap in it), so nothing refused any of them. **Four collisions in one wave is a fact about the allocator, not about four workers**, and it is rowed as `M0-39` rather than left as four renumberings.)*
...
**THE COMMON FAILURE IS NOT ABSENCE.** `mintid` ALLOCATES `M` correctly and four workers of one wave still collided on `M-21`, because **calling it is OPTIONAL and the audit declares no allocation site for `M`**. The tool is right, the tool works, and nothing records who skipped it. So the shape hunted here is **EXISTS / OPTIONAL / UNAUDITABLE**, and **an all-green table is the result to distrust**.

**EVERY ANSWER BELOW WAS TAKEN FROM THE TOOL, NEVER FROM ITS DOCUMENTATION**, and that is not a style note. The premise that produced this row was a TRUE sentence about one half of a tool widened across a boundary INSIDE it: `plancheck`'s own note line reads `id allocations: 690 site(s) across 19 graded namespace(s); C, M not gradable`, which is **true of the AUDIT and false of the ALLOCATOR** — and re-reading it in any of the six places it had been copied would have confirmed it every time.

---

### 0 · THE GATE FIGURES THIS WAS TAKEN ON, AND THE ONE RED

Pristine scratch `git worktree add` of `origin/main` at `58b77ea`, `npm ci` in **`bio-plane/`, `pdf-worker/` AND `ocr-worker/`** (three packages; `agent-worker/` carries none):

| | measured |
| --- | --- |
| battery | **204/205 suites green · 12,685 assertions · exit 1 · 326.1 s** |
| the one red | `acquire.test.mjs`, **`assertions unknown`** |
| provenance | **208 of 208 discovered item(s) are in the commit at HEAD (`58b77ea`) · 205 suite(s) run · 3 fleet manifest(s)** |
| `coverage.mjs --strict`, run DIRECTLY, `$?` read UNPIPED | **exit 0** · REGISTER FLOOR **arms 1087/1087 · classified 196/196 · corpus 197/197** · FLEET **3 members / 6 ops / 8 suites / 76 arms**, floor met · **1 UNCLASSIFIED, named: `case-opened.test.mjs`** |
| `corpuscheck.mjs` | **50 governed document(s), 0 fail** |
| `mergecarry.mjs` | **0 merge(s) in `origin/main..HEAD`** |
| `decided.mjs --check` | **current** |
| `plancheck.mjs --local` | **0 fail, 0 warn** |

**THE RED IS THE MACHINE, ESTABLISHED RATHER THAN ASSUMED.** `acquire.test.mjs` re-run ALONE on the same pristine checkout: **84 pass, 0 fail, exit 0**. The battery's own failure text names the cause and it is not this repository's: `workerd/util/sqlite.c++:844 … disk I/O error: SQLITE_IOERR (extended: SQLITE_IOERR_SHMOPEN)` — SQLite failing to open shared memory inside workerd. `node tools/waitquiet.mjs --check` at the same moment returned **exit 1, BUSY, 8 concurrent `scripts/battery.mjs` processes**, on a machine sharing one temp root between eight worktrees.

**AND THE BRIEF'S FIGURE WAS RIGHT, WHICH IS WORTH SAYING BECAUSE THE PRACTICE IS TO TRUST THE MEASUREMENT AND NOT THE STREAK.** This worker's brief carried *205/205 · 12,769* and noted that eight workers in a row had corrected the figure theirs carried. **204 + the 1 contended suite = 205, and 12,685 + acquire's own 84 = 12,769 EXACTLY.** The brief was not stale; the machine was busy. A ninth consecutive correction was the expected result and it is not what the measurement said.

---

### 1 · THE STRUCTURAL ANSWER, MEASURED FIRST BECAUSE IT CONDITIONS EVERY ROW BELOW

| | measured, 2026-09-15 |
| --- | --- |
| git hooks installed | **NONE.** `.git/hooks/` holds 14 files and every one ends `.sample`; `git config --get core.hooksPath` is unset (exit 1) |
| CI | **NONE.** No `.github/`, no workflow file, no runner config anywhere in the tree |
| entry loops a session is told to run | **5** — `npm run test:battery`, `coverage.mjs --strict`, `civicos-ui/test/run.mjs`, `plancheck.mjs`, `gates.mjs` |
| how many of the five are composed by anything | **ZERO** |

**SO EVERY ANSWER IN THE TABLE BELOW IS CONDITIONAL ON SOMEBODY RUNNING THE LOOP, AND NOTHING MAKES THEM.** This is not a criticism of the design — a repository driven by agent sessions rather than by pull requests has no natural place to hang a hook, and `kickoffs/WORKER.md` §*Before you finish* is where the requirement actually lives. It is stated first because **a census that graded each instrument inside its loop and never asked what composes the loops would read far greener than the estate is**, which is the exact shape of the defect the row exists to catalogue, one level up.

---

### 2 · THE FOUR QUESTIONS, PER INSTRUMENT

**READ `USED` AS: does the loop invoke it as a step of its own.** `TESTED ONLY` means a discovered suite imports it — the loop EXECUTES the code, and there is no gate step. `BYPASS` answers: if somebody skips it or works around it, does anything record that.

| instrument | POSSIBLE | EXISTS | USED at the moment it matters | AUDIT sees a bypass | evidence, taken from the tool |
| --- | --- | --- | --- | --- | --- |
| **`mintid` — the ALLOCATOR** | **yes, and rarely: a SHARED ATOMIC allocator** | **YES** — 21 registered namespaces, ledger under `git rev-parse --git-common-dir`, allocation by `O_CREAT\|O_EXCL` | **NO — calling it is OPTIONAL.** Nothing requires a session to mint; the corpus accepts a hand-written id and no site refuses one | **PARTIAL, and namespace-dependent** | `--audit` prints `O_CREAT\|O_EXCL honoured on this filesystem: YES, probed just now`; `M ledger holds 15 (6..21)` — **`M` IS allocated, and the parent premise's correction is right here in the tool's own output** |
| **`mintid --audit` — DUPLICATE detection** | yes | **YES** — 690 allocation sites across 19 graded namespaces | **YES**, inside `plancheck` §2b, and it **FAILS** on a fresh duplicate or a stale register entry | **for 19 of 21 namespaces** | `plancheck` imports `{ collisions, unregisteredNamespaces }`. **`C` and `M` are NOT COVERED and the tool says so of itself**: `M — no allocation site is declared for this namespace, so a repeat cannot be told from a mention` |
| **`mintid --audit --base` — BYPASS detection (HELD / NOT HELD)** | yes | **YES** — §3 classifies every id a diff introduces | **NO — `plancheck` DOES NOT CALL IT.** It is placed in `kickoffs/CONDUCT.md` step 6 (*after the merge, before the push*), which is the loop CONDUCT reads, so the mechanism IS in a reader's loop — but nothing enforces it and **nothing records that it was skipped** | **NO for a SKIP; and NO AT ALL for `C`/`M`** | **DRIVEN (control arm 2):** a hand-picked `M0-9001` committed → `introduced 1 · NOT HELD 1`, named under the label `QUESTION`; **`plancheck --local` fired NO id arm.** Deliberately a question, not a gate — CONDUCT.md: *"an id the ledger does not hold is a QUESTION you ASK the worker, never a failure"*, because every id allocated before 2026-08-08 is honestly unknown |
| **`mintid` over `M` specifically — THE WORKED EXAMPLE** | yes | allocator YES · **allocation SITE no** | **NO** | **NO** | **DRIVEN (control arm 3):** `M-9002` committed into `MEASUREMENTS.md` → `introduced 0 · NOT HELD 0 · names M-9002: false`. **THE CENSUS CAME BACK WITH A NEGATIVE AND THAT IS THE FINDING, NOT THE PASS.** `mintid`'s own note says why and refuses to invent a site: `MEASUREMENTS.md` allocates nothing recognisable, and `M-4` already names BOTH a measurement entry and a QUEUE item — **two allocation spaces wearing one prefix** |
| **`plancheck` §0 merge markers · §1 publication · §2 plan coheres · §2b ids · §2c merge carry · §3 decisions · §5 inbox** | yes | **YES** | **YES** — one process, every arm runs, and each **FAILS** | **n/a — they ARE the audit.** The bypass is **not running `plancheck`**, and nothing records that | `plancheck --local` 0 fail / 0 warn at `58b77ea`; `--local` skips §1 by design and says so in its own footer |
| **`plancheck` §2b-decided · §6 corpuscheck · §7 rowdesign — THE THREE THAT DEGRADE TO A WARN** | yes | YES | **YES, while their module LOADS** | **NO — a load failure is INVISIBLE IN THE EXIT STATUS** | **DRIVEN (control arm 5):** `tools/corpuscheck.mjs` moved aside → **`plancheck --local` reads `0 fail, 2 warn` and EXITS 0**, saying `UNVERIFIED this run`. The design corpus AND the row-design rule both went unchecked and the gate read green. Restored identically (13,901 B, sha `bdddb927a2b1`) |
| **`corpuscheck` — front matter** | yes | **YES** | **YES** — `plancheck` §6, and it **FAILS** per document | **PARTIAL** | 50 governed, 0 fail. **`docs/architecture/*.md` is DISCOVERED and cannot fall behind; the 29 `docs/development/` entries are a HAND-KEPT TABLE in `CORPUS-STANDARD.md` §5** — so a NEW design document under `docs/development/` is ungoverned silently. Measured precisely, because the first draft of this cell subtracted two different populations: **39 top-level `.md` on disk, 26 of them governed, 13 ungoverned** — plus 3 more governed documents under `docs/development/research/`, giving the 29 `docs/development/` entries in the governed set of 50. **The 13 are ledgers and process documents excluded BY NAME in CORPUS-STANDARD §6, so they are a DECLARED exclusion and are NOT scored a gap here** — `VERIFICATION.md`, this row&apos;s own authority, is one of them, and `rowdesign.mjs` admits it explicitly as a process document that carries no front matter |
| **`rowdesign` — a row names its design** | yes | **YES** | **YES** — `plancheck` §7, and it **FAILS** | **PARTIAL, and it states its own limit** | 29 open rows judged of 313, 0 naming no design. Its header: *"it can prove a row names a governed document; it cannot prove the named section is the row's real authority, or that the section EXISTS"* — **a section pointer is never resolved, and a pointer invented to pass is worse than the gap it hides** |
| **`mergecarry` — did the merge carry what the branch changed** | **yes, and it is the answer only the merge can give** | **YES** | **YES** — `plancheck` §2c, and it **FAILS** on an undeclared drop | **YES, by construction** — a drop must be DECLARED per path in a `Dropped-from-branch:` trailer | **NOT RE-TAKEN HERE, and labelled as somebody else's measurement rather than restated as this row's:** `VERIFICATION.md` §*DID THE MERGE CARRY* records 182 merges over `origin/main`'s whole history, **3 findings, none a false positive**; benign classes DRIVEN one by one through real `git merge`. Honest limit at the site: a drop is also the shape of a correct hand-resolution, and **no archaeology separates them** — which is why the trailer exists |
| **`decided` — the index is current** | yes | **YES** | **YES** — `plancheck`, and it **FAILS** on drift | **YES** | **DRIVEN twice as a side effect (control arms 2 and 3):** editing any corpus file staled `docs/DECIDED.md` and `plancheck` failed by name — `STALE — docs/DECIDED.md does not match the corpus it indexes.` **A live receipt that this arm has teeth.** Its own stated limit: a ruling recorded without a marker word is invisible to it, so it is a FLOOR and never a ceiling |
| **`coverage --strict` — REGISTER_FLOOR / FLEET_FLOOR** | yes | **YES** | **YES** — the floors are compared on every `--strict` run and **FAIL** when they fall | **YES for the fall; NO for SLACK IN THE MOVE** | arms 1087/1087 · classified 196/196 · corpus 197/197 · fleet 3/6/8/76 — **every floor met EXACTLY, zero slack today.** But **the floor is MOVED BY HAND** to a figure a green run printed: possible-and-absent is a mechanical move, and *five consecutive items found a floor already stale by measuring it; one sat 19 codes low and had already flipped a control from RED to GREEN* |
| **the NEGATIVE-CONTROL REGISTER — DECLARATION** | yes | **YES** | **YES for the plane (197) and the fleet (8)** — floored, and an undeclared fleet suite fails BY NAME | **NO for the rest of the estate** | **`civicos-ui/test` 28/49 declaring · 145 arms · 7 unclassified — 21 suites named with NO CONTROL; `newgroup/test` 1/2.** REPORTED, NOT GATED, and `coverage.mjs` says so in its own words. 1 UNCLASSIFIED in the plane register is NAMED, never scored zero |
| **the NEGATIVE-CONTROL REGISTER — was the control ever RUN** | **yes in part** (a dated/ref'd run line is possible; proving execution is not) | **NO — this is the rare ABSENCE case** | — | **NO** | The instrument says it of itself: *"This walk reads `*.test.mjs` only, and says nothing about whether a declared control was ever RUN."* **A worker can declare a control, never run it, and every figure stays green.** The adjacent half IS built — `scripts/armdecay.mjs` + `m025-arm-anchor-witness.test.mjs` catch a declaration that has DECAYED (a dead anchor, a composed label) — so what is missing is execution, not decay |
| **FL-10's FRESHNESS GUARD (`fleetbundles.test.mjs`)** | yes | **YES** | **YES — and this is the strongest row in the table.** The battery DISCOVERS it from the directory, so it cannot fall out of the run by not being mentioned, and it **FAILS** the battery | **YES** — staleness is computed from an input-hash manifest, not from a list | 8 · THE PLANE ITSELF (FL-10, D-298) asserts a fresh build of `src/index.mjs` is byte-identical to the committed `dist/bio-plane.bundled.mjs`. Six arms run 2026-09-10, including the cross-tree arm: one appended export in `bio-plane/src/pdfstructure.mjs` stales **both** committed artifacts by name |
| **the PLANE-BUNDLE REBUILD (`scripts/build-plane.mjs`)** | yes | **YES** | **NO — reached by NO entry loop.** The guard FAILS on a stale artifact; nothing REBUILDS it | **the STALENESS is fully audited; the OBLIGATION was not recorded** | Census: `CONVENTION — reached by no entry loop`. **D-377, open:** the rule *whoever changes `docprofile/` rebuilds in the same commit* was written for `app.html`'s embed and NOWHERE for the plane's bundle, so FW-18 met a green-to-red battery on a suite it never touched and **paid for the discovery**. `DOCUMENT-PROFILES.md` now names it; `kickoffs/FRAMEWORK.md` — the file the worker actually reads — still does not |
| **`bundle-docprofile` — the flattened UI copy** | yes | **YES** | **YES** — `civicos-ui/test/run.mjs` SPAWNS `check-semantics.mjs`, which imports the bundler and **FAILS** on any drift | **for DRIFT yes; for an OMISSION from `ORDER`, no** | **`ORDER` is a HAND-KEPT LIST** and the file records the cost twice in its own comments: a doctype added to `doctypes/registry.mjs` and not to `ORDER` produces a flattened copy that REGISTERS a symbol it never DEFINED — *a ReferenceError in the runtime and in every UI harness at once*. **The bundler guards duplicate top-level names and surviving module syntax and NOT an undefined reference**, which is why it shipped green from the package's own side |
| **`waitquiet`** | yes | **YES** | **TESTED ONLY — and LEGITIMATELY OPTIONAL, recorded as such WITH ITS REASON rather than scored a gap.** It answers *is the machine quiet enough for an uncontended figure*; there is no moment at which it MUST be called, and a gate on it would fail honest runs on a busy machine | n/a | `--check` → **exit 1, BUSY, 8 processes**, read UNPIPED. **The contract is right and the first reading of it here was wrong: piped through `head`, `$?` reported HEAD's status and read `exit 0` beside the word BUSY** — `CLAUDE.md`'s own pipe trap, met by this census inside the census |
| **`gates.mjs` — the profile classifier** | yes | **YES** | **NO — convention-only, and it is one of the five loops nothing composes** | **NO — nothing records which profile a session ran, or whether it ran the gates at all** | `--explain` on this worktree read `change class FULL — non-docs path in the diff`. The class is MEASURED and unknown state is FULL, which is the fail-safe direction |
| **`armdecay` / the arm-anchor witness** | yes | **YES** | **TESTED ONLY** — executed through `m025-arm-anchor-witness.test.mjs`, which the battery discovers and which FAILS on a decayed anchor | **YES for a decayed anchor** | D-329/D-331/D-333. Catches an arm whose anchor went to zero matches and a `mustNotFail` fragment that went **SILENTLY VACUOUS** — *a fragment that can never match can never be violated, so that guard proved nothing and said nothing for a month* |
| **`provenance` — a directory is not a commit** | yes | **YES** | **YES** — imported by `battery.mjs`, `coverage.mjs` AND `civicos-ui/test/run.mjs`, one module with three callers rather than four copies | **REPORTS, DOES NOT FAIL — by design, and it says so** | 208 of 208 in the commit at `58b77ea`. Its own stated limits: it sees an ARRIVAL and not a MODIFICATION; it is relative to HEAD; and **anything discovery never admitted is invisible, so every caller prints its corpus size and floors it** |
| **`hygiene` — the `readdirSync` census** | yes | **YES** | **YES** — battery-discovered, FAILS on a new unguarded walk BY NAME | **YES within its two roots** | **RUN, and the figure it PRINTS is `34 file(s) … walk a directory with a discovery primitive in CODE · 24 GUARDED by scripts/provenance.mjs · 10 named and not guarded`**, plus a second census: `479 module(s) read · 23 walk module(s) · 18 floor(s) whose value crosses a module boundary (5 GUARDED, 13 named) · 7 ceiling-at-zero · 21 UNCLASSIFIED · provenance VERIFIED`. **THE FIRST DRAFT OF THIS ROW SAID `21 files walk, 3 GUARDED, 18 NAMED` — THE FIGURE IN `hygiene.test.mjs`'s OWN COMMENT, WHICH IS STALE AGAINST WHAT THE INSTRUMENT PRINTS.** See the finding below: this row's own rule caught this row. Stated blind spots include a glob library, a shell walk, an aliased binding, and **everything in `tools/`, `newgroup/`, `agent-worker/`, `pdf-worker/`** — **this census's own driver uses `git ls-files` and no directory walk at all, so it is outside that matcher in both directions, said plainly rather than left as an accident of spelling** |
...
3. **`mintid` over `C` and `M`** — no allocation site, so even the audit that IS run cannot grade them. **DRIVEN and returned a negative.**
4. **`gates.mjs`** — nothing records which profile ran, or whether any gate ran.
5. **`plancheck`'s three degrade-to-warn arms** — **DRIVEN: a module that will not load leaves the gate GREEN at `0 fail`.**
6. **the plane-bundle rebuild** — no loop performs it; the guard only refuses the stale result afterwards (D-377).

**THREE MORE are `exists / gated-in-part / blind-in-the-rest`, and the blind part is named in each:** `corpuscheck`'s hand-kept §5 table for `docs/development/`; the negative-control register outside the plane and the fleet (**21 named `civicos-ui` suites with no control, REPORTED and NOT GATED**); `bundle-docprofile`'s hand-kept `ORDER`.

**ONE is a genuine ABSENCE, and it is the rare case:** **nothing in this repository can tell a negative control that was RUN from one that was only DECLARED.** The register's own sentence is the citation. This is the only row where the answer to *does it exist* is NO.

**ONE is legitimately optional and is recorded as such rather than scored a gap:** **`waitquiet`**. There is no moment at which it must be called, and gating on it would fail honest runs on a busy machine — which is the same reasoning that keeps `coverage --strict`'s fleet arms honest and the reason `mergecarry --strict` is not the gate yet. **A census that treats every optional tool as a defect produces a list nobody can act on**, so this row is here with its reason and out of the count.

**AND THE ANSWER THAT CONDITIONS ALL OF THEM:** **no git hook, no CI, nothing composing any of the five entry loops.** Every instrument graded USED above is used *provided somebody runs the loop*, and **nothing anywhere records that they did**.

---

### 4 · WHAT THIS CENSUS CANNOT SEE, STATED BECAUSE A MATCHER THAT HIDES ITS BLIND SPOTS IS READ AS THOUGH IT HAD NONE

The driver follows two edge kinds out of a file's CODE: an `import` of a relative `.mjs` (read across line wraps, via a code mask), and a spawn whose argument text names a repository `.mjs`. **It does NOT see** an edge built by concatenation or held in a variable, a tool invoked from a `package.json` script or from prose in a kickoff, a shell wrapper, or an `eval`. **So `CONVENTION-ONLY` is a claim about these edges and never a claim that nobody runs the tool** — which is why every row above carries the kickoff placement, read by hand, beside the machine answer. Five spawn edges could not be resolved to a tracked path and are NAMED by the driver on every run rather than dropped. Corpus: **764 tracked files · 23 instruments walked · 5 entry loops**.

**FOUR FALSE GAPS WERE FOUND IN THIS CENSUS'S OWN READER BEFORE THE CONTROL COULD REACH THEM, AND ALL FOUR ARE THIS ESTATE'S OWN REPEATED SHAPES ARRIVING INSIDE THE INSTRUMENT BUILT TO CATALOGUE THEM.** They are recorded rather than smoothed: (1) a discovered suite marked `seen` without following its edges scored three scripts convention-only; (2) a line-by-line import reader could not cross a WRAPPED import and scored the same three again, in a blind spot the header had not declared; (3) the id matcher graded one spelling — `mintid` prints the TALLY as `NOT HELD` and LABELS the ids `QUESTION`, so the count was right and every id was invisible; (4) the `plancheck` fail-line reader matched lowercase `fail` against an uppercase `FAIL`, so an arm asserted *plancheck did not see the bypass* over an EMPTY LIST — **an equality that cost nothing to produce, inside the harness that exists to say so.** A `blind` guard now fires when the tally is non-zero and the reader extracts nothing.

**AND THE ROW'S OWN RULE CAUGHT THIS ROW, WHICH IS THE SINGLE MOST USEFUL THING THE CENSUS PRODUCED.** The `hygiene` line above was first written from the figure in **`hygiene.test.mjs`'s own comment** — *"21 files walk a directory, 3 are GUARDED, 18 are NAMED"* — because a comment inside the instrument reads like the instrument. **RUN, the instrument prints `34 · 24 GUARDED · 10 named`.** The comment is stale by 13 walks and 21 guards. **A tool's own prose is documentation about the tool, not the tool**, and `CLAUDE.md`'s agreement-is-not-evidence rule applies to it exactly as the row's `accepts-when` says it does. Every other figure in this table was taken from a run; this one was caught and re-taken, and it is recorded because **the near-miss is the evidence that the discipline is doing work** rather than being restated.

### 5 · THE NEGATIVE CONTROL — 5 arms, declared vs actual, all armed ALONE

| arm | declared | actual |
| --- | --- | --- |
| **(1) baseline** | the census reads clean | **used 14 · TESTED ONLY 3 · convention 6 · introduced 0 · NOT HELD 0 · UNAUDITABLE C, M · plancheck 0 fail** — AS DECLARED |
| **(2) planted bypass, GRADED** | census NAMES `M0-9001` NOT HELD; **no ID arm of `plancheck` fires** | `introduced 1 · NOT HELD 1 · names M0-9001: true` · `plancheck 1 fail [STALE — docs/DECIDED.md …]` · **plancheck SAW the id bypass: NO** · HEAD restored YES — AS DECLARED |
| **(3) planted bypass, UNGRADED (`M`)** | the census MUST NOT find it and MUST name `M` UNAUDITABLE — **a green here is the defect, not the pass** | `introduced 0 · NOT HELD 0 · names M-9002: false` · `UNAUDITABLE C, M` · HEAD restored YES — AS DECLARED |
| **(4) OVER-STRICTNESS** | with `spawnEdges` neutered, `check-semantics` and `check-refusal-codes` flip USED → CONVENTION (two FALSE gaps); the shipped reader must NOT produce them | both flipped, convention **6 → 9**; restore byte-identical **YES** (29,230 B, sha `98a1c65472e9`, `cmp` ok) — AS DECLARED |
| **(5) SILENT DEGRADE** | `corpuscheck.mjs` unloadable → `plancheck` WARNS and **STILL EXITS 0**; **it must NOT fail, because the not-failing IS the finding** | **`plancheck 0 fail, 2 warn`, said UNVERIFIED: true**; restored identically YES (13,901 B, sha `bdddb927a2b1`) — AS DECLARED |
