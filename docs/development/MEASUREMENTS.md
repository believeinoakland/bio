...
| **`corpuscheck` — front matter** | yes | **YES** | **YES** — `plancheck` §6, and it **FAILS** per document | **PARTIAL** | 50 governed, 0 fail. **`docs/architecture/*.md` is DISCOVERED and cannot fall behind; the 29 `docs/development/` entries are a HAND-KEPT TABLE in `CORPUS-STANDARD.md` §5** — so a NEW design document under `docs/development/` is ungoverned silently. Measured: 39 `.md` on disk, 29 governed; **the 13 ungoverned are ledgers and process docs excluded BY NAME in §6, so they are a DECLARED exclusion and are NOT scored a gap here** |
