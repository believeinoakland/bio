...
| **`hygiene` — the `readdirSync` census** | yes | **YES** | **YES** — battery-discovered, FAILS on a new unguarded walk BY NAME | **YES within its two roots** | 21 files walk, 3 GUARDED, 18 NAMED. Stated blind spots include a glob library, a shell walk, an aliased binding, and **everything in `tools/`, `newgroup/`, `agent-worker/`, `pdf-worker/`** — **this census's own driver uses `git ls-files` and no directory walk at all, so it is outside that matcher in both directions, said plainly rather than left as an accident of spelling** |
