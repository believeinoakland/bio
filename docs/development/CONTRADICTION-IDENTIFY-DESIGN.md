...
- §5 — the judgement's model, prompt and skill packaging are the build's, inside the investigative session's run (`INVESTIGATIVE-SESSION.md`); this document fixes its inputs, its output vocabulary and its fence, not its wording. **REC-147 built the prompt and its rendering in the plane's source (pinned by sha256) but NOT its packaging into the skill pack, and NOT a model caller** (the plane has none, D-260): the judgement runs in a session and enters by `op=contradictionpropose`. Packaging it into `skillpack.mjs`'s disclosed layers is SKILL's, and moves the pack version.
- §7 — (REC-147, `measurements/M-162.md`) **the machine judgement scored 0/17 and 9/9 on every run, so this 26-pair synthetic corpus NO LONGER DISCRIMINATES**: it cannot tell a good judgement from a better one, and it says nothing about text that contradicts across pages, hedges, or changes legitimately between dates. A corpus drawn from REAL captured documents, several times larger and with hard negatives, is now the binding gap before the threshold means anything beyond this corpus.
- §8 — **two points the build decided, reported as DESIGN GAPS by REC-147.** (1) *Which version* a claim side is at: a held version is FROZEN (C-25.11), so its name would do; the build versions a claim side by the sha256 of the claim text it compared as well, so a candidate cannot outlive a change in what it judged by any route, and versions a leg or extent side by its capture. (2) *Whether a run's context bounds what it may propose over*: §8 is silent. The build applies §6's viewer gate only (a proposal must name a pair the plane forms for that viewer now) and no context rule, because a K4 pair names no question a context could bound. Both belong in §8 when it is next edited.
...
3. **RECORD + the investigative session's skill — the judgement and the candidate table**, after 1 and 2: the machine's
   ...
   **BUILT 2026-09-25 by REC-147 (IC-318, C-93).** The judgement is a prompt (`bio-plane/src/contradiction.mjs`
   `JUDGEMENT_PROMPT`, pinned by sha256) over §5's input (`renderJudgementInput`: the two sides and their immediate
   context, nothing else), run BLIND three times by two model families over item 2's 26 formed pairs: **0/17 false
   conflicts and recall 9/9 on every run, gate PASS**, beating the lexical baseline's 2/9 (`measurements/M-162.md`).
   The runs are recorded verbatim (`test/contradiction-judge-recorded.mjs`) and re-scored through item 2's gate on
   every battery; an edited prompt fails its digest. `op=contradictionpropose` writes `contradiction_candidates` (I5)
   through ONE append site, only over pairs the plane itself forms for the viewer, and a re-run over unchanged
   referents writes nothing. **NOT BUILT:** any read of a candidate (PRESENT), the skill-pack packaging, a model caller
   in the plane, and a real-document corpus.
