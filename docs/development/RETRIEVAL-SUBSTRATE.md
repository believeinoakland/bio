...
- §Recommendation — item 3 (the default order: relevance, by bm25) was AMENDED 2026-09-23 by D-447 (IC-238): FTS5's `bm25()` reads the whole index, hidden projects included, so it disclosed them (Membership v2 §7.9). The shipped order is bm25's shape computed over the rows the VIEWER can see (`visibleBm25` in `query.mjs`: IDF over the gated rows, term frequency from `highlight()`, no length normalisation), published as an order and never a score. The 21 ms *ranked top 50 with bm25 and snippets* figure in §Actuals measured the index-wide function and is NOT a figure for the shipped rank: MEASUREMENTS M-122 measured 210 ms against 74 ms at 2,000 rows all matching one term (miniflare).
