...
- §Actuals — actuals of the probe harness on 20,000 synthetic rows, not of the shipped ops, and never re-measured since 2026-07-25. The facet figures in particular predate the constraint that shapes the shipped implementation: workerd refuses a compound SELECT of more than five terms, far below SQLite's documented 500, so a six-field facet sidebar measured 283 ms until it was folded into one statement (`query.mjs`, `MAX_COMPOUND = 4`). That ceiling is a property of the substrate, and it is one of TWO, STATED here as a substrate limitation (D-36; BOB #24, 2026-09-21). workerd binds about **100 variables** per statement (SQLite's documented default is 32,766) and allows **five terms** in a compound SELECT (default 500). Neither is Cloudflare's published behaviour; both were binary-searched through the real code path on 2026-07-25. The code guards both: id lists chunk at 64 (`Store.SELECTION_ID_CHUNK`), and compound chains nest through subqueries at four terms (`MAX_COMPOUND = 4`, asserted in `test/query.test.mjs`). `npm run probe:limits` (`test/limits-probe.mjs`) re-measures each against a real Durable Object; run it when a statement shape changes or workerd is upgraded. The CLASS stays open by its nature, because any new statement shape can meet another undocumented limit. D-390 and D-443 (2026-09-23) moved every id list in `store.mjs` known to bind past the first ceiling onto ONE `json_each(?)` value or a chunk; `test/frontier-chunk.test.mjs` drives them past it.
...
- §Recommendation — item 3 (the default order: relevance, by bm25) was AMENDED 2026-09-23 by D-447 (IC-238): FTS5's `bm25()` reads the whole index, hidden projects included, so it disclosed them (Membership v2 §7.9). The shipped order is bm25's shape computed over the rows the VIEWER can see (`visibleBm25` in `query.mjs`: IDF over the gated rows, term frequency from `highlight()`, no length normalisation), published as an order and never a score. The 21 ms *ranked top 50 with bm25 and snippets* figure in §Actuals measured the index-wide function and is NOT a figure for the shipped rank: MEASUREMENTS M-122 measured 210 ms against 74 ms at 2,000 rows all matching one term (miniflare). M-164 (D-465, 2026-09-25) took it again with a kept instrument (`tools/d465-search-bench.mjs`), changing only `query.mjs`: the ADDED cost reproduces (+121 ms at 2,000 rows of 60 words, +150 ms at 200 words), grows with matching rows × their text × positive terms, and is +3 to +7 ms at the live instance's 31 documents. No fix is owed at that size, and M-164 records why.
...
**Contents**
- [Why probe 2 exists](#why-probe-2-exists)
- [Finding 1: the engine has everything the query language needs](#finding-1-the-engine-has-everything-the-query-language-needs)
- [Finding 2: frontmatter is heterogeneous, and the current projection covers about half of it](#finding-2-frontmatter-is-heterogeneous-and-the-current-projection-covers-about-half-of-it)
- [Finding 3: typed columns beat a facet table, and JSON1 covers the rest](#finding-3-typed-columns-beat-a-facet-table-and-json1-covers-the-rest)
- [Finding 4: indexing the filter fields is the difference between a seek and a scan](#finding-4-indexing-the-filter-fields-is-the-difference-between-a-seek-and-a-scan)
- [Actuals at 20,000 documents](#actuals-at-20000-documents)
- [What the measurements constrain in the UX](#what-the-measurements-constrain-in-the-ux)
- [Recommendation](#recommendation)
- [Settled design (Bob, 2026-07-25)](#settled-design-bob-2026-07-25)
- [Why a bare string is AND](#why-a-bare-string-is-and)
- [The public-class fence hole, fixed in 0.14.1](#the-public-class-fence-hole-fixed-in-0141)
- [Serialization against the membership model](#serialization-against-the-membership-model)
- [A selection is a lease, and its expiry is a term rather than a fault](#a-selection-is-a-lease-and-its-expiry-is-a-term-rather-than-a-fault)
