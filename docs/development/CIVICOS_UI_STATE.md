...
v105, 2026-09-24 session, thread UI, UI-86 (a WORKER of CONDUCT #19, cloud session). Landed on `land/worker/UI-86` (base
`origin/main` @ `3f4b8f8c`), in the commit that carries this entry. (If a concurrent batch has taken `v105`, renumber this
entry at integration.) SURFACES: the QUEUE (`SURFACES["queue"]`) — the case group's mute, each item's own controls, and the
feed's mute report.
