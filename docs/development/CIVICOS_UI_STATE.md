...
v105, 2026-09-24 session, thread UI, UI-83 (a WORKER of CONDUCT #19, cloud session). Landed on `land/worker/UI-83` (base
`origin/main` @ `15b2a4c0`), in the commit that carries this entry. (If a concurrent batch has taken `v105`, renumber this
entry at integration.) SURFACE: the Progressions screen's "Declare how something is supposed to go" form.
