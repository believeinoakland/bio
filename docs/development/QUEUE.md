...
### D-309 · running — spawned 2026-09-10 by CONDUCT #9, Opus 5, worktree-isolated, into the slot CASE-6 freed. Falsify rather than believe: a live worker holds an `agent-*` worktree with commits toward the set-valued readers; if none does, this row reads `queued`.
...
### CPDF-15 · running — spawned 2026-09-10 by CONDUCT #9, Opus 5, worktree-isolated, measurement lane (holds no slot). Falsify rather than believe: a live worker holds an `agent-*` worktree with a probe report for this item; if none does, this row reads `queued`.
...
### CPDF-10 · blocked — **RE-ROUTED 2026-09-10 by CPDF-14's NO-GO verdict: DEC-35's in-account candidates are exhausted (Moondream default NO-GO at CPDF-12, Moondream composed NO-GO at CPDF-14), so the path is the EXTERNAL escalation tier or the unmeasured tesseract runtime — and the external tier is never funded without Bob's word. Waits on DEC-74 (open, provisional: blocked, nothing built or funded; the measured gap is 13 image-only pages of 1,458).**
milestone: M2
...
### UI-57 · running — spawned 2026-09-10 by CONDUCT #9, Opus 5, worktree-isolated, background lane (holds no slot). Falsify rather than believe: a live worker holds an `agent-*` worktree with commits touching `publicationEntryHtml`; if none does, this row reads `queued`.
milestone: M10
