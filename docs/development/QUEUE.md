...
### CASE-4 · running
spawned: 2026-09-10 · CONDUCT (worker live at spawn). **`running` is only true while a worker is alive** — verify with `git log --oneline main | grep -i 'CASE-4'`.
ic-preallocated: **IC-69, minted BY CONDUCT AT SPAWN** (the IC-64 lesson — see FL-9's row).
milestone: M10
...
### FL-9 · running
spawned: 2026-09-10 · CONDUCT (worker live at spawn). **`running` is only true while a worker is alive** — verify with `git log --oneline main | grep -i 'FL-9'`.
ic-preallocated: **IC-68, minted BY CONDUCT AT SPAWN.** The IC-64 lesson enacted rather than remembered — two parallel workers each minted `IC-64` on 2026-08-10 because `mintid` reads its floor from ids MENTIONED IN PROSE and neither branch could see the other's file.
milestone: M7
...
### CASE-5b · queued
milestone: M10
interface: I3 + I5 via the IC protocol — a new signing ceremony is a new act surface; file the IC before building
