...
### D-318 · running — spawned 2026-09-12 by CONDUCT #9, Opus 5, worktree-isolated, background lane (holds no slot). Falsify rather than believe: a live worker holds an `agent-*` worktree with commits touching the probes' controls tables; if none does, this row reads `queued`.
