...
### REC-82 · running — spawned 2026-09-14 by CONDUCT #10, Opus 5, worktree-isolated, into RECORD's dev slot at the drain (the slot had been honestly empty since the takeover, waiting for exactly this). Falsify rather than believe: a live worker holds an `agent-*` worktree with a claim on `schema.mjs`/`store.mjs`; if none does, this row reads `queued`. — **Act 6 item 1 — the `content` row on the `pdf-page` and `document` arms: the table, the writer, the backfill, `stale`, purge, hygiene and the catalogue refusals; IC-83's landing half.**
...
### REC-83 · queued — **waits on REC-82 (the table it reads).** — **Act 6 item 2 — the reads: `earnedBasisRegistry` keyed by content row, `op=earnedbasis` answering the per-extent transcription ceiling and stating UNDETERMINED for a portion leg's connection axis (Bob's 5.1), and the new fixed-key `op=content`; IC-84's read half.**
milestone: M4 — D-164, the content-extent primitive (RECORD)
interface: I3 (IC-84 ACCEPTED — CHANGING at 14.1.0 until this and REC-84 land)
depends-on: REC-82
scope: the inbox entry's item 2 and IC-84's (3) and (4): `earnedBasisRegistry` keys by content row; `gradeCeiling(chain, extent)` per extent — an attestation covering the extent raises it to B, a page attestation does not cover a `document` row; the connection grade of a non-`document` row is UNDETERMINED and STATED, never borrowed from the whole document; the leg's capture grade ≤ `captureBound` as today; `op=content` fixed-key by `content_id` (member and read classes), returning the row, `ref`, chain and cap, `stale`, and the attestations covering it — no predicate, no paging (D-222's fixed-key rule).
accepts-when: `op=earnedbasis` on a portion leg answers the per-extent ceiling and the connection axis as UNDETERMINED with the level stated (CLAUDE.md's sparse-is-normal rule), driven; `op=content` answers every field on a minted row and refuses an unknown id by name; `cd bio-plane && npm run test:battery` green — measure your own baseline (~187/187 · ~11,343 with all three member installs); `node scripts/coverage.mjs --strict` DIRECTLY, `$?` unpiped, exit 0, with a control-plane assertion for every new op or check in the same turn; `node civicos-ui/test/run.mjs` from the repo root, unpiped; `op=audit` clean in scratch; plancheck 0 fail
NEGATIVE CONTROL: run and recorded — a page attestation must NOT raise a `document` row's ceiling (driven both ways); an `op=content` call with a predicate or paging → refused (fixed-key only); over-strictness: a `document` leg's earned basis is byte-identical to the pre-item answer.
added: 2026-09-14 · CONDUCT #10 (draining the 2026-09-14 BOB #10 act-6 entry: the nine items scoped there, in its dependency order; the entry's item text stays in the inbox record as the scope's authority beside IC-83/IC-84 and the D-164 study — a notification, not a second copy)

### REC-84 · queued — **waits on REC-82.** — **Act 6 item 3 — the frontmatter and version legs: C-2.8 and C-25.10 admit the `extent` arm; `inquiry_basis_version_legs.content_id`; the investigative run's suggested legs default to `document`.**
milestone: M4 — D-164, the content-extent primitive (RECORD)
...
### REC-85 · queued — **waits on REC-82.** — **Act 6 item 5 — the other three arms: `covers` for `sheet-cell`, `doc-para` and `slide-shape`, each with its out-of-range refusal from the container's own extent (sheet dimensions, paragraph count, shape list).**
milestone: M4 — D-164, the content-extent primitive (RECORD)
...
### COFF-9 · running — spawned 2026-09-14 by CONDUCT #10, Opus 5, worktree-isolated, **CONTENT-OFFICE re-activated into a dev slot for it** (the area was dormant with its axis built end to end; Bob's 2026-09-14 ruling reopens it). Falsify rather than believe: a live worker holds an `agent-*` worktree with a claim on `ooxml.mjs`; if none does, this row reads `queued`. **The OpenDocument FLAVOUR row in `ooxml.mjs`'s container discriminator — designed for and unbuilt; act (1) of Bob's Drive ruling.**
...
### COFF-10 · queued — **waits on COFF-9 (the flavour row it dispatches on); act (2) of Bob's Drive ruling.**
...
### CAP-7 · running — spawned 2026-09-14 by CONDUCT #10, Opus 5, worktree-isolated, MEASUREMENT lane (holds no slot). Falsify rather than believe: a live worker holds an `agent-*` worktree with a claim on `MEASUREMENTS.md` and Part II §16's Drive paragraph; if none does, this row reads `queued`. **Count the Google Drive links in the office-format census corpus — the 2026-09-14 BOB #10 inbox's act 1. SUPERSEDED IN PURPOSE the same day, not in scope: Bob RULED the Drive harvest (keep the link, extract from the OpenDocument export), so the build is rowed regardless as COFF-9 → COFF-10 → CAP-8 and this count sets its PRIORITY against other work, not whether it happens; CONDUCT states the priority on those rows when the figure lands.**
...
### CAP-8 · queued — **waits on COFF-9 and COFF-10; act (3) of Bob's Drive ruling, the handler that KEEPS the link and acquires the export. CAP-7's count, when it lands, sets where this sits against other work — it does not decide whether.**
...
accepts-when: a machine-minted row exists, is labelled on `op=content` and on every surface, cannot be attested by its minter, and is absent from findings until a member cites it — driven; `cd bio-plane && npm run test:battery` green — measure your own baseline (~187/187 · ~11,343 with all three member installs); `node scripts/coverage.mjs --strict` DIRECTLY, `$?` unpiped, exit 0, with a control-plane assertion for every new op or check in the same turn; `node civicos-ui/test/run.mjs` from the repo root, unpiped; `op=audit` clean in scratch; plancheck 0 fail
