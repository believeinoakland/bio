...
### CASE-2 · running
spawned: 2026-08-10 · CONDUCT (worker live at spawn). **`running` is only true while a worker is alive** — verify with `git log --oneline main | grep -i 'CASE-2'` before believing it.
milestone: M10
...
### CASE-3 · running
spawned: 2026-08-10 · CONDUCT (worker live at spawn). **`running` is only true while a worker is alive** — verify with `git log --oneline main | grep -i 'CASE-3'` before believing it.
milestone: M10
