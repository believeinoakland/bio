...
M10, IC protocol against I3/I5) are `docs/development/CASE-AS-PRODUCTION.md` — one
...
### D-265 · running — spawned 2026-09-10 by CONDUCT #9, Opus 5, worktree-isolated, background lane (holds no slot). Falsify rather than believe: a live worker holds an `agent-*` worktree with commits touching the walk census; if none does, this row is stale and reads `queued`.
milestone: M0 (background lane, holds no slot)
interface: none
depends-on: none
...
## CASE — DEC-72's publication redesign, M10. **SEQUENCED BY CONDUCT; the design doc holds SCOPE and this section holds STATUS.**
...
### FL-10 · queued — **ASSIGNED TO FLEET, which runs it. CONDUCT minted and sequenced it and is NOT spawning it.**
milestone: M7
...
### CASE-5b · running — spawned 2026-09-10 by CONDUCT #9, Opus 5, worktree-isolated. **IC-71 pre-minted at spawn** for the ceremony's act surface. Falsify this status rather than believing it: a live worker holds an `agent-*` worktree with commits for this item (`git worktree list` + the process table); if none does and no branch carries the ceremony, this row is stale and reads `queued`.
milestone: M10
...
### UI-56 · running — spawned 2026-09-10 by CONDUCT #9, Opus 5, worktree-isolated. Falsify rather than believe: a live worker holds an `agent-*` worktree with commits touching `civicos-ui/app.html`; if none does, this row is stale and reads `queued`.
milestone: M8
