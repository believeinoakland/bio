...
### D-315 · running — spawned 2026-09-11 by CONDUCT #9, Fable 5 (the Opus 5 weekly limit stands; the standing tactical-escalation note applies), worktree-isolated, background lane (holds no slot). Falsify rather than believe: a live worker holds an `agent-*` worktree with commits touching the probes' guards; if none does, this row reads `queued`.
