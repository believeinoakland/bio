...
### D-322 · running — spawned 2026-09-12 by CONDUCT #9, Opus 5, worktree-isolated, background lane (holds no slot). Falsify rather than believe: a live worker holds an `agent-*` worktree with commits toward the gate's witness; if none does, this row reads `queued`.
