...
### REC-128 · running — **MERGE HELD OFF `main` AND RESPAWNED, 2026-09-18, by CONDUCT #5. The merge (`7e9fcd16`, kept on the remote branch `conduct/rec-128-merge`; IC-139 resolved MINOR → I3 27.1.0 and IC-140 ADDITIVE → I5 1.21.0 in it) ran the battery 239/240, with `deliverer.test.mjs` 7 FAIL. The cause is a SEMANTIC CONFLICT WITH REC-130, which landed after this branch forked, and not a test typo. REC-130 stamps `member:<sessMember>` on `op=caseratify`'s facts read, and a FOUNDER session's role is `admin`, so the founder reads as a member named `admin` with no standing and is answered NO_CASE_DOCUMENT. Founder-delivered case ratification, which BOB #14 ruled ALLOWED, is therefore refused on the merged tree, and `caseReader`'s session branch gives a founder the same answer at `op=casedocument`. A correction to landed substrate (REC-130), so it goes first. A worker resolves it from `conduct/rec-128-merge`. Falsify rather than believe: a live worker holds an `agent-*` worktree with a claim on the paths its scope names; if none does, this row is UNDETERMINED between `queued` and done-awaiting-integration — READ THE BRANCH, and never conclude `queued` from the absence alone.** Prior: **done-awaiting-integration: `worktree-agent-a01d041e19ab1ad65`@`40a19351` — NEXT ACT: CONDUCT #5 merges it, rebuilds the bundle, re-reads the floors on the COMMITTED merge, resolves IC-139/IC-140 at the bases then current, runs the full gate. WORKER FINISHED, BRANCH PUSHED, NOT YET INTEGRATED (as of CONDUCT #4's stand-down, 2026-09-18): `worktree-agent-a01d041e19ab1ad65` at `40a19351`. It CHANGES A PUBLISHED FORMAT — new case containers become `bio-case-container/6`, carrying `delivered_by` beside every attestor, labelled as this instance's own record outside any signature; the worker verified existing published cases verify unchanged (manifests are built once, stored with their hash, never rebuilt). IC-139 (I3, proposed MINOR) and IC-140 (I5 1.20.0 → 1.21.0) await resolution — READ THE BASE AT RESOLUTION (I3 is 27.0.0 now). CONDUCT #5 integrates it.** — **SPAWNED 2026-09-18 by CONDUCT #4 into REC-125's slot — a correction to landed substrate (REC-123/REC-125), ahead of new items. DEPENDS-ON CHECKED AGAINST THE CODE at spawn: REC-125's C-32.14/C-32.15 fences merged in the same integration as this flip; spawned only after that integration is pushed. Falsify rather than believe: a live worker holds an `agent-*` worktree with a claim on the paths its scope names; if none does, this row is UNDETERMINED between `queued` and done-awaiting-integration — READ THE BRANCH, and never conclude `queued` from the absence alone.** — **THE RECORD STATES WHO AUTHORISED AND WHO DELIVERED (BOB #14, 2026-09-18, D-421's honesty half): `op=ratify` and `op=caseratify` record the DELIVERING PRINCIPAL — the authenticated session's own principal (a member, or the founder) — beside the signature's SIGNER, and a founder-delivered ratification says so.** — owner RECORD.
milestone: M3 — the record's own refusals reach every authority that writes it
interface: I3 (the ratification's published/read shape gains the delivering principal) and I5 if a column is needed — each its own IC, minted with `node tools/mintid.mjs IC` BEFORE building, bases read at resolution (I3 26.1.0 when rowed)
design: `docs/architecture/BIO_Assistant_and_AI_Roles_v0_1.md` §3 rule 4, as D-421 corrected it (*a HUMAN's own authenticated session — a member's or the founder's — never a bearer token or machine credential*), read with `DEBT.md` D-421 and IC-137's resolution.
depends-on: REC-125 (DONE — merged with this row).
scope: both acts record the delivering principal FROM THE SESSION, never inferred from the signature; a founder-delivered ratification carries it visibly wherever the ratification is read or published; existing ratifications read back with their deliverer UNDETERMINED and stated, never back-filled from the signer.
accepts-when: a member-delivered and a founder-delivered ratification each record their deliverer through the ops, distinct from the signer; the published/read projection shows both; legacy ratifications read deliverer-undetermined; `status --check` 0 drift; battery green by its COMPLETION LINE; coverage --strict exit read UNPIPED.
NEGATIVE CONTROL: recorded in the suite's own `NEGATIVE CONTROL:` line (**with the colon**) — deliverer taken from the SIGNATURE instead of the session → the founder-delivered case records the member as deliverer and the arm FAILS; legacy back-fill introduced → the legacy arm FAILS. **Liar:** a `delivered_by` column that copies `signed_by` — two names for one fact, which is the thing D-421 exists to separate.
added: 2026-09-18 · CONDUCT #4 (BOB #14's ruling on IC-137's open question. Id minted with `node tools/mintid.mjs REC`, run bare.)

### REC-129 · running — **SPAWNED 2026-09-18 by CONDUCT #5, RESUMING FROM ITS PUSHED BRANCH `worktree-agent-af99c832f7860f664`@`eda53bc2` (never from zero). DEPENDS-ON CHECKED AGAINST THE CODE at spawn: MK-4's `leads` table present in `schema.mjs`; the branch's four commits read on the remote. The brief puts the two count-level leak checks (`op=stats`' `leads`/`observations` counts; the MEANING frontier's unfiltered rows) FIRST. **BOTH CHECKED BEFORE THE SPAWN by BOB #15 at the code on `4c1ba352`, and re-read by CONDUCT #5 at `index.mjs` (`stats` classes admin/member/probe) and `store.mjs` (`leads: n("leads")`, `observations: n("observation_log")`): (a) CONFIRMED, a correction to landed MK-4 and therefore FIRST in this item. `op=stats` gives member and probe an instance-wide `leads` count, and the whole-log `observations` count includes lead looks, so a member diffing stats across a colleague's authoring learns a lead exists. RULED by BOB (design): both keys go to the ADMIN class only, and member and probe receive NO such key. Never re-mean a key per class (BOB.md rule 7). An I3 change, so mint and name the IC. NEGATIVE CONTROL: a member-class `stats` answer must be byte-identical before and after a lead is authored. (b) REFUTED as a lead leak. `op=leadlook` writes level `internet`, so `#frontierMeaning` never sees a lead row. It becomes a PRECONDITION on THIS item's internet-level frontier instead: rows with `authority_kind='lead'` carry the lead's WORDS as `subject` and MUST be gated by lead visibility (a content leak, not a count; written into `MEMBER-KNOWLEDGE-DESIGN.md` §5 at `683b58a3`).** Falsify rather than believe: a live worker holds an `agent-*` worktree with a claim on the paths its scope names; if none does, this row is UNDETERMINED between `queued` and done-awaiting-integration — READ THE BRANCH, and never conclude `queued` from the absence alone.** **Prior state, kept as the record: queued** — **WIP AT STAND-DOWN, NOT DONE — `worktree-agent-af99c832f7860f664`@`eda53bc2` (CONDUCT #4, 2026-09-18). RESPAWN FROM THAT BRANCH, never from zero; the claim stays OPEN; IC-143 stays reserved to this item.** Written but NEVER RUN: `op=frontier&level=internet` answering from the lead's looks with the visibility filter applied BEFORE grouping (lists, dates, counts and `truncated` all from readable looks only), an empty answer always naming level and cause, lead visibility consolidated into one `#leadReach`; the suite and control are drafts. **TWO POSSIBLE COUNT-LEVEL LEAKS the worker raised and did NOT verify — CHECK FIRST:** `op=stats` publishes a `leads` count and a whole-log `observations` count (an existence signal BOB's visibility ruling forbids, if a member can call it); and the MEANING level's frontier passes non-capture, non-reference rows unfiltered, possibly investigative-run rows. **Prior state, kept as the record: running** — **SPAWNED 2026-09-18 by CONDUCT #4 into MK-4's slot. DEPENDS-ON CHECKED AGAINST THE CODE: MK-4 merged in the same integration — the `leads` table and the look-under-authority-kind-lead writer present; spawned only after that integration is pushed. Falsify rather than believe: a live worker holds an `agent-*` worktree with a claim on the paths its scope names; if none does, this row is UNDETERMINED between `queued` and done-awaiting-integration — READ THE BRANCH, and never conclude `queued` from the absence alone.** **Prior state, kept as the record: queued** — **THE FRONTIER'S INTERNET-LEVEL READ: a level-wide frontier read at the INTERNET level, designed in `OBSERVATION-LOG-DESIGN.md` §6's reader table, unbuilt and unrowed until now (MK-4 found it; `op=frontier` still answers *not built* for that level).** — owner RECORD.
milestone: M4 — the store as a read-through cache, at its fourth level
interface: I3 — its own IC, minted BEFORE building, base read at resolution
design: `docs/development/OBSERVATION-LOG-DESIGN.md` §6 (the readers) read with §4.5 (the lead's looks, which are the internet level's first writer).
depends-on: MK-4 (the lead and MK-4's look-at-a-lead act (not yet on `main`), the internet level's member writer) — NOT yet merged; MK-4 is being corrected on its branch for lead visibility. This row may not run before MK-4 lands.
scope: build the internet-level row of §6's reader table exactly as designed — what it reads, what it answers when the level is sparse (sparse is the normal condition, and an empty answer must say WHICH level was empty and why), and how it honours MK-4's lead visibility (a lead a viewer cannot see must not leak through the frontier). Update `9.internet` in `construct-status.json` with probes.
accepts-when: through the ops, the internet level answers from the looks MK-4 records, honours lead visibility (a refused viewer sees exactly what it would for no lead), and states its emptiness with a cause; `status --check` 0 drift; battery green by its COMPLETION LINE.
NEGATIVE CONTROL: recorded in the suite's own `NEGATIVE CONTROL:` line (**with the colon**) — visibility filter removed → a lead outside the viewer's reach appears in the frontier and the arm FAILS. **Liar:** an empty answer with no cause — a frontier that says nothing looks exactly like one that looked and found nothing.
added: 2026-09-18 · CONDUCT #4 (BOB #14 asked for it rowed after MK-4, depending on MK-4. Id minted with `node tools/mintid.mjs REC`, run bare.)


### REC-126 · running — **SPAWNED 2026-09-18 by CONDUCT #5 (build-order item 8). DEPENDS-ON CHECKED AGAINST THE CODE at spawn: REC-130's `#hasCaseStanding` present in `store.mjs`; BOB's §6A text present in `BIO_Publication_v0_1.md` (§6A.1–§6A.4). Falsify rather than believe: a live worker holds an `agent-*` worktree with a claim on the paths its scope names; if none does, this row is UNDETERMINED between `queued` and done-awaiting-integration — READ THE BRANCH, and never conclude `queued` from the absence alone.** **Prior state, kept as the record: queued** — **FLIPPED TO `running` AT REC-130's INTEGRATION AND REVERTED BEFORE ANY SPAWN, 2026-09-18, by CONDUCT #4 — the operator refreshed both lane sessions and CONDUCT #4 stood down instead of spawning. REC-130 IS MERGED (its dependency is met at the code: `#hasCaseStanding` and the byte-identical not-found answer are on `main`); BOB's §6A text is on `main`. RUNNABLE NOW for CONDUCT #5.** — **UNBLOCKED 2026-09-18: BOB #14 wrote all three decisions into `BIO_Publication_v0_1.md` §6A (on `main` at `44c8b503`) — gap 1 as §6A.2's PRECONDITION (unsigned case document private to owning-project members and live grant holders, the nonexistent answer to everyone else, signed stays public), gap 2 as the per-grant READ SECRET under §6A.2, gap 3 as the DRAFT case whose publish-gate refusals become its missing-list under §6A.4. depends-on: REC-130 (gap 1's correction, running) — this row is spawned only once REC-130 is merged.** **Prior state, kept as the record: blocked** — **STOPPED BEFORE BUILDING, CORRECTLY, 2026-09-18: §6A left three gaps the grant needs, each checked at the code; building anyway would have made scoping and revocation cosmetic. ALL THREE DECIDED BY BOB #14 the same hour, as applications of existing doctrine; BOB is writing them into `BIO_Publication_v0_1.md` §6A — this row UNBLOCKS against that text.** (1) unsigned case documents were readable anonymously → `REC-130`, which goes FIRST; (2) a non-member recipient reaches the copy by a per-grant BEARER SECRET — a READ credential (never an attested act), stored hashed, bound to one case edition, killed on revocation, a dead secret indistinguishable from one that never existed, the recipient NEVER becoming a member; no members-only interim; (3) the production is a DRAFT case identified BEFORE the publish gates run, the gates' results becoming its *what is missing* list shown to the recipient (§6A.4). Waits on: `REC-130` and BOB's §6A text. — **THE REVIEW COPY (DEC-31, Bob's ruling of 2026-09-17): an addressed act BESIDE publish that NEVER LEAVES THE INSTANCE — a scoped, revocable, read-and-comment GRANT against ONE production, attributed, NEVER a third bucket; the product MARKED as what it is, as complete as a full publication can be, NAMING WHAT IS MISSING.** — build-order item 8 (`13.review-copy`); owner RECORD (the grant) + UI (the surface, delegated). The design was READ AT THE ARTIFACT before rowing (the last two "verified" pointers did not survive that check; this one did).
milestone: M3 — publication's addressed sibling
interface: I3 and I5 — each its own IC, minted BEFORE building, bases read at resolution
design: `docs/architecture/BIO_Publication_v0_1.md` §6A (6A.1 the two determinations Bob overruled, 6A.2 the grant, 6A.3 DEC-31's narrower question and its in-band rule, 6A.4 completeness REUSED, not invented).
depends-on: publication (BUILT); verify the private bucket's fence and the existing gate-and-capability vocabulary on your tree before building.
scope: 6A.2's grant with each of its five properties (scoped to one production, revocable-and-real, read-and-comment not edit, attributed, never a bucket); 6A.1's rule that publication stays the ONE irreversible act (DEC-19); 6A.3's two binding consequences — any rendering that leaves the instance carries hash, date, author and both threshold floors IN-BAND, and the member is told at the act that what leaves cannot be revoked; 6A.4's completeness reused. The grant is NOT an account, membership, or a weaker class of member. Move `13.review-copy` in `construct-status.json` with probes.
accepts-when: through the ops: a grant is issued to a named recipient for one production, the recipient reads and comments, revocation ends access, a second production is refused; nothing is written to the published bucket; an export carries the in-band fields; `status --check` 0 drift; battery green by its COMPLETION LINE.
...
### MK-2 · running — **SPAWNED 2026-09-18 by CONDUCT #5, RESUMING FROM ITS PUSHED BRANCH `worktree-agent-a17c98e0548c9fd2d`@`05a2deb8` (never from zero). DEPENDS-ON CHECKED AGAINST THE CODE at spawn: MK-1's `op=testify` present in `index.mjs` (17 references). Owed: the UI axis-vocabulary mirror, the IC-142 entry, the design front matter, a battery known green. **Its design gap is ANSWERED by BOB #15 in `MEMBER-KNOWLEDGE-DESIGN.md` §3 (`683b58a3`): a leg on an observation carries a CONNECTION grade, graded like any other leg. It is neither refused nor exempt, and the branch's "left allowed" is therefore correct and stays. The bare-`leg:testimony` refusal is endorsed.** Falsify rather than believe: a live worker holds an `agent-*` worktree with a claim on the paths its scope names; if none does, this row is UNDETERMINED between `queued` and done-awaiting-integration — READ THE BRANCH, and never conclude `queued` from the absence alone.** **Prior state, kept as the record: queued** — **WIP AT STAND-DOWN, NOT DONE — `worktree-agent-a17c98e0548c9fd2d`@`05a2deb8` (CONDUCT #4, 2026-09-18). RESPAWN FROM THAT BRANCH, never from zero; the claim stays OPEN.** Built: `testimony` in `GRADE_AXES`/`STRENGTH_AXES`, legs on an observation graded D only, the §7 refusals by name, the strength arithmetic and frozen case strength over three axes; `testimonyaxis.test.mjs` 51/0 and `nc-mk2.mjs` as declared. **A REAL DEFECT FOUND AND FIXED on the branch:** `isCaseMemberBytes` required EXACTLY two frozen strength rows, so a case member resting on testimony (three) would silently have stopped being checked as a case member. **NOT built:** `civicos-ui/app.html`'s axis vocabulary mirror (its guard fails; the 53 UI suites pass), the IC-142 entry, the design front matter. **Battery NOT known green** — last full run 231/239 before the old two-axis pins were corrected. **Proposed MINOR, with one arguable point:** bare `leg:testimony` becomes ambiguous and is refused naming both readings (`leg:source=testimony` returns the old rows). **Design gap for BOB:** whether a CONNECTION-axis grade on an observation should be refused (left allowed). **Prior state, kept as the record: running** — **SPAWNED 2026-09-18 by CONDUCT #4 into MK-1's slot (build-order item 3). DEPENDS-ON CHECKED AGAINST THE CODE: MK-1 merged in the same integration — `op=testify`, the register's `authored` column and C-53 present; spawned only after that integration is pushed. Falsify rather than believe: a live worker holds an `agent-*` worktree with a claim on the paths its scope names; if none does, this row is UNDETERMINED between `queued` and done-awaiting-integration — READ THE BRANCH, and never conclude `queued` from the absence alone.** **Prior state, kept as the record: queued** — **THE `testimony` GRADE AXIS — a third axis at D, because DEC-21 defines the capture axis as reading a document in and testimony is not that: grading, `STRENGTH_AXES`, the DEC-32 arithmetic, and the case document's strength presentation.** — owner RECORD; surfaces are Program B's and are NOT rowed.
...
### REC-124 · blocked — **blocked on Bob, doctrine: on a shared inquiry, is a conclusion the inquiry's or a project's (§7/D-216 vs op=conclude)** — re-classified 2026-09-18 by BOB #14 at the artifact from a BOB design gap to DOCTRINE: INVESTIGATIVE-SESSION §7 and D-216 make a project's STANCE on a shared inquiry per-project (version ACCEPT + CURRENT already ARE a per-project adoption, and the version carries the claim, D-217b), while `op=conclude` is ONE shared state with ONE free conclusion — two built mechanisms disagree about who owns a conclusion, and resolving it changes what *concluded*, and so a published finding, means. The builder's finding, kept as the record: **THE BUILDER FALSIFIED THE DESIGN FIRST, AS THE ROW TOLD IT TO, AND STOPPED WITHOUT EDITING — the builder FALSIFIED THE DESIGN FIRST, AS THE ROW TOLD IT TO, AND STOPPED WITHOUT EDITING, which is the correct outcome the row allowed for.** The rulings fix that a claim is a FIELD (DEC-22, DEC-19 as amended, session BOB 2026-08-07) and that concluding ADOPTS it, but NOT the adoption act: (1) **a claim is ALREADY STORED, on basis VERSIONS** (`claim` column, D-217b, written by `op=narrow` and the investigative-session writer) — an inquiry field would give it a second home against D-21, and *which claim was adopted* has several candidates since `op=conclude` works on the LIVE basis and one inquiry may have a different current version per project; (2) the relation between the adopted claim and the separate CONCLUSION text is unruled — a free conclusion can assert what no claim said, which is this row's own failure; (3) concluding with NO claim — refuse (DEC-19 implies) or record *adopted none* — is unruled, and refusing breaks 25 suites and the UI's conclude flow. **Also found:** `construct-status.json`'s `8.claim` line is INCOMPLETE — its probes cannot see the versions' `claim` column; and `op=claim` already exists (the instance-claim op), so any claim-stating act needs another name. **The builder's recommendation is in its report, relayed to BOB verbatim in substance: one claim field on the inquiry written by a member-only act under a new op name; a version's `claim` becomes a proposed rewording; concluding refuses `NO_CLAIM` per DEC-19 and records the adopted claim as it stood; legacy inquiries read claim-undetermined; Bob rules whether the separate conclusion text survives. Reversal cost low now, rising once published cases sign the adopted claim into their bytes.** Returns to `queued` when BOB writes the design. **Prior state, kept as the record: running** — **SPAWNED 2026-09-18 by CONDUCT #4 (BOB #14's build order; a one-for-one refill of a freed product slot). DEPENDS-ON CHECKED AGAINST THE CODE at spawn: `op=conclude` and the inquiry present; `8.claim` reads PARTIAL in `construct-status.json` with its empty-search probes passing. Falsify rather than believe: a live worker holds an `agent-*` worktree with a claim on the paths its scope names; if none does, this row is UNDETERMINED between `queued` and done-awaiting-integration — READ THE BRANCH, and never conclude `queued` from the absence alone.** **Prior state, kept as the record: queued** — **THE CLAIM IS A FIELD THAT CONCLUDING ADOPTS — AND TODAY IT IS ONLY THE `conclusion` FIELD: `construct-status.json` 8.claim reads PARTIAL (no `fm.claim`, `meta.claim`, `claim_text` or `claimText` anywhere in the plane). Everything in the case path rests on it** (BOB #14's build order, item 2). — waits on nothing; owner RECORD.
