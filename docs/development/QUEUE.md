...
### D-310 · running — spawned 2026-09-10 by CONDUCT #9, Opus 5, worktree-isolated, into the second slot. Falsify rather than believe: a live worker holds an `agent-*` worktree with commits touching `affordances.mjs`; if none does, this row reads `queued`.
