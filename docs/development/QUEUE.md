...
### REC-80 · running — spawned 2026-09-14 by CONDUCT #10, Opus 5, worktree-isolated, background lane (holds no slot). Falsify rather than believe: a live worker holds an `agent-*` worktree with a claim on these files; if none does, this row reads `queued`. — **RECORD's two designs get front matter — `RETRIEVAL-SUBSTRATE.md` (the FTS5-in-the-DO specification) and `SCHEDULER.md` (the alarm model); prose only.**
milestone: M0 (background lane, holds no slot) — the design corpus standard, Bob's 2026-09-14 ruling: `docs/architecture/CORPUS-STANDARD.md` §5 "Not yet governed"
interface: none — front matter and one row in `CORPUS-STANDARD.md` §5's governed table; no shape, no op, no behaviour moves
depends-on: none
...
### REC-81 · queued — **waits on CPDF-17 (running), which edits comment blocks in `schema.mjs` and `index.mjs` that carry some of these citations; sequenced after it so two comment-only workers never hold one region.** — **The 2026-09-14 inbox's act 3: the ten `framework:LINE` citations in `bio-plane/src` and the one in `INTERFACES.md` point 84 lines early since the framework gained front matter — convert every one to a SECTION citation per `CORPUS-STANDARD.md` §4.6; comments and one doc line only.**
milestone: M0 (background lane, holds no slot) — the design corpus standard, Bob's 2026-09-14 ruling: `docs/architecture/CORPUS-STANDARD.md` §5 "Not yet governed"
interface: none — front matter and one row in `CORPUS-STANDARD.md` §5's governed table; no shape, no op, no behaviour moves
...
### D-329+D-331+D-333 · running — spawned 2026-09-14 by CONDUCT #10, Opus 5, worktree-isolated, background lane (holds no slot). **The lull the row named arrived and was taken rather than admired: both dev slots empty, CPDF-17 the only other runnable item, act 6 awaited from BOB, VF-7 gated on a deploy — an idle slot beside a queued item is the refill defect, not prudence. Prior state, kept as the record: DELIBERATELY NOT SPAWNED, and the tactical call is stated rather than implied: the instrument-hardening chain (D-305 → D-314 → D-315 → D-318 → D-322 → M0-25 → D-330) has reached diminishing returns — each residue is now narrower than the item that raised it, every one is filed with a disposition and a stated interim law, and none is a live exposure. This row exists so the three remaining driver-estate shapes are ONE schedulable item when a lull or a re-prioritisation warrants it, rather than three notes.**
milestone: M0 (background lane, holds no slot)
...
### COFF-8 · running — spawned 2026-09-14 by CONDUCT #10, Opus 5, worktree-isolated, background lane (holds no slot). Falsify rather than believe: a live worker holds an `agent-*` worktree with a claim on these files; if none does, this row reads `queued`. — **CONTENT-OFFICE's `OFFICE-FORMATS.md` (the format axis) gets front matter; the axis is built end to end (COFF-1..7), so the Status is mostly [BUILT] and the honesty is in what the axis still does not extract; prose only.**
milestone: M0 (background lane, holds no slot) — the design corpus standard, Bob's 2026-09-14 ruling: `docs/architecture/CORPUS-STANDARD.md` §5 "Not yet governed"
interface: none — front matter and one row in `CORPUS-STANDARD.md` §5's governed table; no shape, no op, no behaviour moves
depends-on: none
...
### CAP-6 · running — spawned 2026-09-14 by CONDUCT #10, Opus 5, worktree-isolated, background lane (holds no slot). Falsify rather than believe: a live worker holds an `agent-*` worktree with a claim on these files; if none does, this row reads `queued`. — **CAPTURE's seven designs get front matter — the largest owner group, all prose, the area DORMANT so CONDUCT answers-for the Status/Place/Incomplete judgment and the worker writes it against the tree.**
milestone: M0 (background lane, holds no slot) — the design corpus standard, Bob's 2026-09-14 ruling: `docs/architecture/CORPUS-STANDARD.md` §5 "Not yet governed"
interface: none — front matter and one row in `CORPUS-STANDARD.md` §5's governed table; no shape, no op, no behaviour moves
depends-on: none
...
### FW-16 · running — spawned 2026-09-14 by CONDUCT #10, Opus 5, worktree-isolated, background lane (holds no slot). Falsify rather than believe: a live worker holds an `agent-*` worktree with a claim on these files; if none does, this row reads `queued`. — **FRAMEWORK's `DOCUMENT-PROFILES.md` (docprofile's own design) gets front matter; the area is DORMANT and CONDUCT answers-for; prose only.**
milestone: M0 (background lane, holds no slot) — the design corpus standard, Bob's 2026-09-14 ruling: `docs/architecture/CORPUS-STANDARD.md` §5 "Not yet governed"
interface: none — front matter and one row in `CORPUS-STANDARD.md` §5's governed table; no shape, no op, no behaviour moves
depends-on: none
...
so opening a `### SK-6 · queued — **waits on CPDF-17 (running), which edits the same file's lines 67–72; two claims on one small document is the collision the claim mechanism cannot protect against, so this runs AFTER it lands.** — **SKILL's `ASSISTANT-PILOT.md` (the assistant pilot design) gets front matter; prose only.**
milestone: M0 (background lane, holds no slot) — the design corpus standard, Bob's 2026-09-14 ruling: `docs/architecture/CORPUS-STANDARD.md` §5 "Not yet governed"
interface: none — front matter and one row in `CORPUS-STANDARD.md` §5's governed table; no shape, no op, no behaviour moves
...
### UI-58 · running — spawned 2026-09-14 by CONDUCT #10, Opus 5, worktree-isolated, background lane (holds no slot). Falsify rather than believe: a live worker holds an `agent-*` worktree with a claim on these files; if none does, this row reads `queued`. — **UI's three designs get front matter — `UI-PLAN.md` (the plan of record), `UI-KICKOFF.md` (Bob's UX principles) and `NOTIFICATIONS.md` (the queue content, shared with RECORD's alarm model); prose only.**
milestone: M0 (background lane, holds no slot) — the design corpus standard, Bob's 2026-09-14 ruling: `docs/architecture/CORPUS-STANDARD.md` §5 "Not yet governed"
interface: none — front matter and one row in `CORPUS-STANDARD.md` §5's governed table; no shape, no op, no behaviour moves
depends-on: none
