...
### REC-134 · queued — **"ADMINISTRATORS DIRECT NOTHING," ENFORCED FOR EVERY ADMINISTRATOR: every act that changes a project, its participation, its productions or their grants carries a POSITIONAL check (the actor's own role in THAT project), never the visibility gate alone. §7.13's add-an-owner is the one administrator path. An AUTHORITY DEFECT, so it goes AHEAD OF FEATURES.** — owner RECORD.
milestone: M8
interface: I3 (an IC minted with `node tools/mintid.mjs IC`; acts an administrator could perform on a project it is not in are REFUSED, which makes it breaking by IC-137)
design: `docs/architecture/BIO_Membership_Architecture_v2.md` §7, *"SIGHT IS NOT AUTHORITY — and this is Bob's doctrine, not a new ruling"* (BOB #15, 2026-09-18, at `f624bd43`), quoting §4 (*"the custodial role can audit everything and direct nothing"*) and §7.13 (the one exception).
depends-on: REC-132 (its resolver separates the VISIBILITY viewer from the POSITIONAL identity, which is what this check reads) — CHECKED AT THE CODE at spawn.
scope: grep every act that changes a project, its participation, its productions (cases, drafts, versions, conclusions) or their grants; STATE per act, in a table, which positional role it requires. Add the positional check wherever the visibility gate is the only barrier. §7.13 keeps its condition, its vote and its record. Machine fences are unchanged.
accepts-when: an enrolled administrator and the founder, NOT in a project, can each SEE it (REC-132 unchanged) and can DO none of the enumerated acts on it; the same people, IN the project with the required role, can. §7.13's add-an-owner still works under its condition. How a liar passes it: refusing every administrator EVERYWHERE also passes the refusal arms, so the in-project arms and §7.13's arm are what make it honest, and REC-132's founder-sight controls must stay green. NEGATIVE CONTROLS: remove the positional check from one enumerated act, and its arm fails; apply it to §7.13's path, and that arm fails.
added: 2026-09-18 · CONDUCT #5 (BOB #15's decision on REC-132's design gap; placed by CONDUCT until SCHEDULER is live).

### REC-133 · running — **CORRECTED WHILE RUNNING, 2026-09-18 (BOB #15 at `f624bd43`): §6A.2 now reads REVOKE = OWNER ONLY. The row's "owner OR any administrator" contradicted Bob's doctrine that administrators "direct nothing" (Membership v2 §4). The scope is therefore AUTHORING ONLY (`op=casedraft` goes to the project's edit permission), and issuing and revoking stay at the owner. The worker was messaged mid-run; CONDUCT CHECKS AT INTEGRATION that no revoke widening landed, and reverts it in the merge commit if it did.** **SPAWNED 2026-09-18 by CONDUCT #5. DEPENDS-ON CHECKED AGAINST THE CODE at spawn: REC-126's `op=casedraft`/`op=reviewrevoke` are on `main` (`src/index.mjs` OPS rows); BOB's §6A.2 decision is on `main` at `5ea27761`. Falsify rather than believe: a live worker holds an `agent-*` worktree with a claim on the paths its scope names; if none does, this row is UNDETERMINED between `queued` and done-awaiting-integration — READ THE BRANCH, and never conclude `queued` from the absence alone.** **THE REVIEW COPY'S AUTHORITY, AS §6A.2 DECIDES IT: AUTHOR a draft = the project's EDIT permission; ISSUE a grant = the project OWNER only, no administrator bypass (UNCHANGED from REC-126); REVOKE = the owner OR ANY ADMINISTRATOR.** — owner RECORD.
milestone: M8
