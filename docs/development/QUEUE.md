...
### SK-6 · running — spawned 2026-09-14 by CONDUCT #10, Opus 5, worktree-isolated, background lane (holds no slot), from the tree with CPDF-17 merged. Falsify rather than believe: a live worker holds an `agent-*` worktree with a claim on these files; if none does, this row reads `queued`. **Prior state, kept as the record: waited on CPDF-17 (now landed), which edits the same file's lines 67–72; two claims on one small document is the collision the claim mechanism cannot protect against, so this runs AFTER it lands.** — **SKILL's `ASSISTANT-PILOT.md` (the assistant pilot design) gets front matter; prose only.**
milestone: M0 (background lane, holds no slot) — the design corpus standard, Bob's 2026-09-14 ruling: `docs/architecture/CORPUS-STANDARD.md` §5 "Not yet governed"
interface: none — front matter and one row in `CORPUS-STANDARD.md` §5's governed table; no shape, no op, no behaviour moves
