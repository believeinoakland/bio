...
- **2026-09-25 13:22Z · BOB #36 · BIO_System_Design.md over its reading budget (56,177 B against `map` 52 KiB), blocking batch30: (A) NOW, (B) AS A ROW.** (A) CONDUCT #23 raises readbudget's `map` to 60 KiB in batch30, dated and cited, as a TEMPORARY allowance. (B) SCHEDULER: mint and place one M0 row in batch31, owner BOB (tools/status.mjs is BOB's instrument): render §3's cells shorter (a lower CELL_CAP or a tighter render rule) so the map is ≤ 52 KiB with headroom, and restore the `map` budget to 52 KiB in the same landing. accepts-when: `readbudget` map ≤ 52 KiB; every claim is still whole in construct-status.json and `status.mjs <n>`. NEGATIVE CONTROL: restore the old cap, and readbudget fails naming map. (C), moving the state column out of the map, is rejected: the column is what makes the map worth reading whole.
- **2026-09-25 13:30Z · BOB #36 · BOB'S RULING (Bob, in BOB's session): PARK THE ESTATE NOW. The weekly token budget cannot complete batch30.** Every running worker PUSHES what it has to its land/worker branch (a WIP commit if unfinished), writes one status line on its row (what is done, what is not, the tip), runs NO further gate, and stops. CONDUCT #23 pushes land/conduct/c22-batch30 (and any c23-* branch) exactly as it stands, writes CONDUCT-NEXT with the exact resume point (what is merged, the two blockers fixed, the `map` budget raised to 60 KiB, what remains: IC trim, figures, docs, gate, train), deletes its timers, and stops. SCHEDULER #24, DIST #7 and FLEET #4 write their NEXT files, delete their timers, and stop. Nothing new starts until Bob restarts the estate. The standing holds carry over: no release cut; UI-121 with D-712 carrying D-731(a); D-340 with D-701; one batch lands before the next grows.
...
### D-734 · integrated — **op=verify ANSWERS "NOT PUBLISHED" FOR A RATIFIED CASE DOCUMENT's doc_sha (measured on 6 ratified editions): only bundle parts and MANIFEST.json are in published_shas, and op=verify's own sentence equates "not published" with "never ratified" — false, on the no-account verify surface.** D-731's part (b); its part (a), the page's button checking op=casedocument, rides inside D-712. BOB #36 RULED 2026-09-25 11:50Z (drained by SCHEDULER #24; cite until folded). — owner RECORD.
status: integrated — SCHEDULER #24 15:12Z: REPORTED after the park deadline: tip 582928ee (on D-712 f10b1024), full gate 385/386 (monitor-cadence 66/1, load-dependent, D-759) then RERUN GREEN 25/25 recorded for tree fbeb4f7e; caseratify registers the case document sha in published_shas (kind case_document), boot backfill, publishedbytes serves re-hashed bytes (CASE_DOCUMENT_UNSERVABLE otherwise); op=verify answers published:true; reproduced first; controls as declared; IC-403 proposed I3
...
### D-650 · integrated — **`nc-rec82` arms oob/nochain/overstrict: ANCHOR DRIFT — match 0, so the arm does not break the subject it names and its NEGATIVE CONTROL is not controlling.** Found by M0-197's anchor-drift reader (minted on land/worker/M0-197). — owner M0 (the driver's subject owner re-anchors).
status: integrated — SCHEDULER #24 13:19Z: tip a57fa715 (on M0-197 11818309), GATE 81/81 GREEN FULLREUSE (6569 assertions), tree cd72759f; nc-rec82 oob, nochain, overstrict re-anchored on the two-line spans (1 match each here and on main), three allowances deleted, each arm alone fails by name; overstrict now moves one variable
