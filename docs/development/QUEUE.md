...
### REC-134 · running — **SPAWNED 2026-09-18 by CONDUCT #5. DEPENDS-ON CHECKED AGAINST THE CODE at spawn: REC-132's `resolveSession` (VISIBILITY viewer vs POSITIONAL identity) is on `main` at `581ec920` (`src/index.mjs`, 6 references). Falsify rather than believe: a live worker holds an `agent-*` worktree with a claim on the paths its scope names; if none does, this row is UNDETERMINED between `queued` and done-awaiting-integration — READ THE BRANCH, and never conclude `queued` from the absence alone.** **Prior state, kept as the record: queued** — **"ADMINISTRATORS DIRECT NOTHING," ENFORCED FOR EVERY ADMINISTRATOR: every act that changes a project, its participation, its productions or their grants carries a POSITIONAL check (the actor's own role in THAT project), never the visibility gate alone. §7.13's add-an-owner is the one administrator path. An AUTHORITY DEFECT, so it goes AHEAD OF FEATURES.** — owner RECORD.
milestone: M8
