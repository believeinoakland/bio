...
### REC-133 · running — **SPAWNED 2026-09-18 by CONDUCT #5. DEPENDS-ON CHECKED AGAINST THE CODE at spawn: REC-126's `op=casedraft`/`op=reviewrevoke` are on `main` (`src/index.mjs` OPS rows); BOB's §6A.2 decision is on `main` at `5ea27761`. Falsify rather than believe: a live worker holds an `agent-*` worktree with a claim on the paths its scope names; if none does, this row is UNDETERMINED between `queued` and done-awaiting-integration — READ THE BRANCH, and never conclude `queued` from the absence alone.** **THE REVIEW COPY'S AUTHORITY, AS §6A.2 DECIDES IT: AUTHOR a draft = the project's EDIT permission; ISSUE a grant = the project OWNER only, no administrator bypass (UNCHANGED from REC-126); REVOKE = the owner OR ANY ADMINISTRATOR.** — owner RECORD.
milestone: M8
interface: I3 (an IC minted with `node tools/mintid.mjs IC`; authority widens for two acts, and nothing that answered before is refused)
design: `docs/architecture/BIO_Publication_v0_1.md` §6A.2 (BOB #15, 2026-09-18) — authoring is *"the editor's act"* (Bob, quoted in §6A); sending unratified material outside is an act of the kind DEC-72 makes the owner's; revoking only narrows exposure, and means a grant never outlives an owner who has left.
depends-on: REC-126 (landed `b9789f2f`).
scope: REC-126 runs all three acts at the project owner (the same check `publishCase` uses). Widen `op=casedraft` to the project's edit permission (find the existing one, never invent a new one) and `op=reviewrevoke` to the owner OR any active administrator; leave `op=reviewgrant` exactly as it is. The machine fence C-32.16 is unchanged.
accepts-when: a non-owner editor authors a draft; a non-owner editor CANNOT issue a grant; an administrator who is not the owner revokes a grant, and the revoked secret then answers byte-identically to a never-issued one; a plain member with neither right can do none of the three. How a liar passes it: widening all three to "any member" passes every positive arm, so each REFUSAL is asserted too. NEGATIVE CONTROLS: re-gate revoke at owner-only → the administrator arm fails; widen issue to editors → the issue-refusal arm fails.
added: 2026-09-18 · CONDUCT #5 (BOB #15's §6A.2 decision, draining the inbox entry of that date).
...
### REC-131 · running — **RESUMED 2026-09-18 by CONDUCT #5 BEFORE INTEGRATION, WITH TWO MORE RULINGS FOLDED IN (BOB #15, `MEMBER-KNOWLEDGE-DESIGN.md` §5, landed at `5ea27761`):** (a) `dbBytes` leaves `op=stats` for member and probe. The admin class keeps it, because capacity is an operator need and its reading mixes every write and moves in whole pages; that residue is stated. NEGATIVE CONTROL: a lead large enough to allocate pages leaves a member's `op=stats` unchanged. (b) One key never carries two meanings (BOB.md rule 7): the key whose meaning CHANGED is renamed, so `op=stats`' narrower count gets its own key and `purge`'s `observations` keeps the whole-log meaning it always had, inside IC-148 (already MAJOR). The worker's first build (`77efc750`, gate green) was merged by CONDUCT and HELD, never pushed, so the whole change lands under one MAJOR bump. **SPAWNED 2026-09-18 by CONDUCT #5. DEPENDS-ON CHECKED AGAINST THE CODE at spawn: REC-129's `stats-disclosure.test.mjs` and the operator stamp are on `main`; BOB #15's corrected §5 is on `main` at `852ccc7d`. Falsify rather than believe: a live worker holds an `agent-*` worktree with a claim on the paths its scope names; if none does, this row is UNDETERMINED between `queued` and done-awaiting-integration — READ THE BRANCH, and never conclude `queued` from the absence alone.** **Prior state, kept as the record: queued** — **A CORRECTION TO JUST-LANDED REC-129 (IC-144), ON BOB #15's CORRECTED RULING — FIRST AMONG EQUALS: `leads` LEAVES `op=stats` FOR EVERY CLASS, THE ADMIN TOKEN INCLUDED; `observations` STAYS ON `op=stats` FOR EVERY CLASS BUT COUNTS ONLY ROWS WITHOUT `authority_kind='lead'`, THE SAME MEANING FOR EVERY CALLER.** — owner RECORD.
milestone: M8
