...
### REC-132 · running — **SPAWNED 2026-09-18 by CONDUCT #5. DEPENDS-ON CHECKED AGAINST THE CODE at spawn: REC-128's `sessionCaseViewer` is on `main` (`src/index.mjs`, the seed), and BOB #15's design is on `main` at `852ccc7d`. Falsify rather than believe: a live worker holds an `agent-*` worktree with a claim on the paths its scope names; if none does, this row is UNDETERMINED between `queued` and done-awaiting-integration — READ THE BRANCH, and never conclude `queued` from the absence alone.** **D-422: ONE SESSION RESOLVER — THE FOUNDER'S SESSION SEES WHAT AN ADMINISTRATOR SEES (EXCEPT WHERE A RULING NAMES AUTHORS OR PARTICIPANTS), AND THE MEMBER ID `admin` IS RESERVED.** — owner RECORD.
milestone: M8
interface: I3 (an IC minted with `node tools/mintid.mjs IC`; the founder gains sight; classification is CONDUCT's)
design: `docs/architecture/BIO_Membership_Architecture_v2.md` §7, *"THE FOUNDER IS AN ADMINISTRATOR HERE TOO"* (BOB #15, 2026-09-18), with `MEMBER-KNOWLEDGE-DESIGN.md` §5 (a lead is never widened to an administrator) and DEBT D-422.
depends-on: REC-128 (landed `481dbbbe`, which carries `sessionCaseViewer`).
scope: `sessionCaseViewer` becomes the ONE resolver every session-stamped read uses, returning a VISIBILITY viewer and a separate POSITIONAL identity. Every `viewerPredicate` caller is grepped and the governing arm is STATED per site: an administrator's, or a ruling that names authors or participants and keeps its own predicate. `memberAdd` refuses the id `admin`, and `op=audit` REPORTS an existing member with that id (never a silent rename). Closes D-422.
accepts-when: the §7 negative controls, each driven: the founder's session lists a project it was never invited to; it still cannot read another member's unshared lead; `memberAdd` with id `admin` is refused; the admin token's answers are byte-identical before and after. How a liar passes it: widening EVERY read to the administrator arm passes the first control, so the lead control and a per-site table of the arm that governs are what make it honest. A positional question (authorship, votes, D-310's owner/participant) must still answer by the founder's identity, never by the visibility viewer.
added: 2026-09-18 · CONDUCT #5 (from D-422, designed by BOB #15).

### REC-131 · running — **SPAWNED 2026-09-18 by CONDUCT #5. DEPENDS-ON CHECKED AGAINST THE CODE at spawn: REC-129's `stats-disclosure.test.mjs` and the operator stamp are on `main`; BOB #15's corrected §5 is on `main` at `852ccc7d`. Falsify rather than believe: a live worker holds an `agent-*` worktree with a claim on the paths its scope names; if none does, this row is UNDETERMINED between `queued` and done-awaiting-integration — READ THE BRANCH, and never conclude `queued` from the absence alone.** **Prior state, kept as the record: queued** — **A CORRECTION TO JUST-LANDED REC-129 (IC-144), ON BOB #15's CORRECTED RULING — FIRST AMONG EQUALS: `leads` LEAVES `op=stats` FOR EVERY CLASS, THE ADMIN TOKEN INCLUDED; `observations` STAYS ON `op=stats` FOR EVERY CLASS BUT COUNTS ONLY ROWS WITHOUT `authority_kind='lead'`, THE SAME MEANING FOR EVERY CALLER.** — owner RECORD.
milestone: M8
interface: I3 (an IC minted with `node tools/mintid.mjs IC`; it SUPERSEDES IC-144's provisional class stamp)
design: `docs/development/MEMBER-KNOWLEDGE-DESIGN.md` §5 AS CORRECTED BY BOB #15 on 2026-09-18 (the text is being pushed by BOB; the brief pastes the ruling if it is not yet on `main`). The reason, in the ruling's words, summarised: `#leadVisibleTo` reaches no `class:*` credential and skips the administrator arm on purpose, so NO caller can read every lead, the admin token included; a counter over leads therefore has no class it may be shown to. `purge`'s D-113 proof reads the store's own `stats()`, not the wire op, so it loses nothing.
depends-on: REC-129 (landed; `stats-disclosure.test.mjs` and the server-set operator stamp are on `main`) — CHECKED AT THE CODE at spawn.
scope: (1) `op=stats`, `op=selftest`, `op=livefire` never publish `leads`; (2) `observations` is published to every class and excludes lead rows; (3) the server-set operator stamp REC-129 added is removed if nothing else needs it (say so either way); (4) `stats-disclosure.test.mjs` and `nc-rec129.mjs`'s stats arms corrected at their sites with the reason, never exempted; (5) `OBSERVATION-LOG-DESIGN.md`'s Incomplete entry that REC-129 added about REC-110's premise is STALE on BOB's ruling (REC-110 is NOT reopened, because `observations` stays published to member and probe and the three built levels' tallies never count lead rows): replace it with a pointer to MEMBER-KNOWLEDGE §5; (6) IC-144's text is marked SUPERSEDED IN PART by the new IC.
accepts-when: an ADMIN-token, a member-token, a member-session and a probe `op=stats` answer are each BYTE-IDENTICAL before and after another member authors a lead and follows it. How a liar passes it: dropping `observations` for everyone also makes every answer identical, so the suite ALSO asserts that `observations` is present for every class and MOVES when a non-lead observation is written. NEGATIVE CONTROL: put the lead rows back into `observations` → the byte-identical arms fail for every class; put `leads` back for admin → the admin arm fails.
added: 2026-09-18 · CONDUCT #5 (BOB #15's correction of its own §5 ruling, arriving while REC-129 was being integrated. REC-129 was landed as built, because it discloses strictly less than `main` did and nothing is deployed, and this row is the correction.)
