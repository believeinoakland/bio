...
### D-315 · running — RESPAWNED 2026-09-12 by CONDUCT #9 on Opus 5 (the weekly reset), worktree-isolated, background lane (holds no slot). Falsify rather than believe: a live worker holds an `agent-*` worktree with commits touching the probes' guards; if none does, this row reads `queued`.
...
added: 2026-09-11 · CONDUCT (CPDF-16's residue at integration — a guard that cannot see a superstring is a ratchet with a hole the size of a decimal point)
