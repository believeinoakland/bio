...
### REC-139 · running — **SPAWNED 2026-09-18 by CONDUCT #5. DEPENDS-ON CHECKED AGAINST THE CODE at spawn: REC-138's `Store#inSight` and `#noSuchProject` are on `main` at `ad4d1044`. Falsify rather than believe: a live worker holds an `agent-*` worktree with a claim on the paths its scope names; if none does, this row is UNDETERMINED between `queued` and done-awaiting-integration — READ THE BRANCH, and never conclude `queued` from the absence alone.** **Prior state, kept as the record: queued** — **A REFUSAL NEVER NAMES OR DESCRIBES A PROJECT THE CALLER CANNOT SEE (BOB #15, Membership v2 §7, at `7b733d07`): `NAME_TAKEN` drops the other project's id AND title; the PLANE MINTS project ids, which closes `EXISTS` outright; an inquiry RUN's report lists ONLY the citing projects its member can see and counts none of the others. A §7.9 DISCLOSURE DEFECT, ahead of features, in D-426's class.** — owner RECORD.
milestone: M8
interface: I3 (an IC minted with `node tools/mintid.mjs IC`; project ids become plane-minted, which changes `op=promote`'s create contract and so is breaking), and I5 if id minting needs a sequence.
design: `docs/architecture/BIO_Membership_Architecture_v2.md` §7 (BOB #15, 2026-09-18, at `7b733d07`), §7.9 (*"not its existence, not its name"*), and DEC-63 (`node tools/decided.mjs "DEC-63"`), which governs who may START a run and does not require the disclosure.
depends-on: REC-138 (landed; `Store#inSight` and `#noSuchProject`) — CHECK AT THE CODE at spawn.
scope: (1) `NAME_TAKEN` carries neither the other project's id nor its title. Provisionally (OPEN for Bob) uniqueness still holds, and the refusal reveals only the name the caller typed. (2) Project ids are minted by the plane (`allocId`'s pattern); a caller-supplied id for a NEW project is refused or ignored, whichever the design says; STOP if it does not say. (3) An inquiry run's report counts and lists ONLY citing projects the member can see. Every caller that chose a project id is corrected at its site; any UI caller gets a DELEGATION.
accepts-when: an uninvited member's `NAME_TAKEN` names no id or title; a create never answers `EXISTS` about a hidden project; a run report's citing-project list and count are byte-identical whether or not a hidden project cites the inquiry. How a liar passes it: dropping all project names from every report, so a VISIBLE citing project must still be listed. NEGATIVE CONTROLS: restore the title in `NAME_TAKEN`; count hidden projects in the run report.
added: 2026-09-18 · CONDUCT #5 (D-428, decided by BOB #15; D-428 stays open for the name-uniqueness question with Bob).

### REC-137 · running — **SPAWNED 2026-09-18 by CONDUCT #5. DEPENDS-ON CHECKED AGAINST THE CODE at spawn: REC-134's `projectAuthority` is on `main` at `e7a3598f` (9 references in `store.mjs`). Falsify rather than believe: a live worker holds an `agent-*` worktree with a claim on the paths its scope names; if none does, this row is UNDETERMINED between `queued` and done-awaiting-integration — READ THE BRANCH, and never conclude `queued` from the absence alone.** **Prior state, kept as the record: queued** — **A CASE RATIFICATION'S AUTHORITY AND ITS DELIVERY, AS MEMBERSHIP v2 §7 DECIDES THEM: the SIGNATURES must include an OWNER of the publishing project (DEC-72 cl. 5); DELIVERY is carriage (AI Roles rule 4), so a member with a role in the project may deliver, and so may the FOUNDER (DEC-33's interim route), while an ENROLLED ADMINISTRATOR WITH NO ROLE in the project may NOT.** A follow-up to REC-134, ahead of features (an authority defect). — owner RECORD.
milestone: M8
interface: I3 (an IC minted with `node tools/mintid.mjs IC`; a refusal where none stood, so breaking by IC-137)
design: `docs/architecture/BIO_Membership_Architecture_v2.md` §7, *"A CASE RATIFICATION: who AUTHORISES it and who may DELIVER it"* (BOB #15, 2026-09-18, at `7a7cef2d`), with DEC-72 cl. 5, DEC-33, and `BIO_Assistant_and_AI_Roles_v0_1.md` §3 rule 4.
depends-on: REC-134 (landed; `Store#projectAuthority` and the positional identity) and REC-128 (the recorded deliverer) — CHECKED AT THE CODE at spawn.
scope: VERIFY AT THE CODE FIRST whether `ratifyCaseDocument`/`op=caseratify` requires an owner among the signers (BOB found only the instance-wide `publish` capability); add it if absent. Refuse delivery by an enrolled administrator with no role in the project, through REC-134's one check, keeping the founder's route.
accepts-when: a case signed by non-owners only is refused; with an owner's signature it commits; a project member delivers; the founder delivers; an enrolled administrator outside the project is refused, even carrying an owner's valid signature. How a liar passes it: refusing every administrator also refuses the FOUNDER, so the founder arm must commit. NEGATIVE CONTROLS: drop the owner-signer check, and the non-owners arm commits; drop the delivery check, and the outside administrator commits.
added: 2026-09-18 · CONDUCT #5 (BOB #15's §7 decision on REC-134's gap).
