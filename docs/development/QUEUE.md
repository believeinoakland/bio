...
### REC-33 · blocked
milestone: M1
scope: **The `monitor` token class — the DIST→RECORD delegation, blocked on DEC-37.** Today every monitor tick and archive fallback on every installed instance runs under ADMIN_TOKEN, the root-of-trust credential, to do two narrow things. The one-change recipe (from DIST-1's delegation in CLAIMS): a `monitor` class in classify(); acceptance in OPS.monitor.classes and op=acquire's archive arm (whose comment already says "operator or daemon credential"); NO scratch confinement in scopeFor() (why PROBE_TOKEN is not the answer); the op=selftest/livefire binding lists; the installer then generates and binds MONITOR_TOKEN (DIST's half, one more DIST item when this lands). tokens.mjs needs no edit (per-value denylist). Blocked on DEC-37 (whether a fourth token class is worth opening — CAPABILITIES §4/§5-adjacent doctrine); provisional: the ADMIN_TOKEN fallback continues, nothing blocked.
behind-interface: I3
depends-on: none — blocked on DEC-37, not an item
accepts-when: (on unblocking) `cd bio-plane && npm run test:battery` green with a suite where a MONITOR_TOKEN credential reaches op=monitor and the acquire archive arm and NOTHING else (memberlist, tasks, purge all refused), while ADMIN_TOKEN still works on instances not yet updated; `npm run test:coverage` --strict exit 0; negative control — grant the monitor class one op beyond its two and the suite names it.
