...
### M0-67 · running — **SPAWNED 2026-09-18 by CONDUCT #5. SHARPER HYPOTHESIS (BOB #15): the battery counts EXIT STATUSES, so the question is whether a suite can print `N FAIL` and still EXIT 0. BOB's five complete full-gate logs today show no FAIL under a green headline; the one run with real failures read 242/244 correctly; `machinefences-dec49` passed every run at 66–67 assertions, and the worker's 68+2=70 came from its own branch's tree. The durable fix, if the exit path is the defect: the runner cross-checks each suite's printed tally against its exit status (FAIL > 0 with exit 0 is a RED run, named). Falsify rather than believe: a live worker holds an `agent-*` worktree with a claim on the paths its scope names; if none does, this row is UNDETERMINED between `queued` and done-awaiting-integration — READ THE BRANCH, and never conclude `queued` from the absence alone.** **Prior state, kept as the record: queued** — **D-425: CAN THE BATTERY REPORT GREEN OVER A SUITE THAT PRINTED FAIL? A worker's baseline log showed `machinefences-dec49 … FAIL 68 pass, 2 FAIL` under a `245/245 suites green` headline and exit 0; unreproduced. Establish which: two runs interleaved into one log, or a tally that can count a printed FAIL as green.** — **EXEMPT FROM THE M0 HOLD BY NAME, FIRST AMONG M0 ROWS (BOB #15, 2026-09-18): a gate that can count a printed FAIL as green is substrate under every other verification.**
milestone: M0
interface: none
...
### REC-138 · running — **SPAWNED 2026-09-18 by CONDUCT #5. DEPENDS-ON CHECKED AGAINST THE CODE at spawn: REC-134's `projectAuthority` is on `main` at `e7a3598f` (9 references in `store.mjs`). Falsify rather than believe: a live worker holds an `agent-*` worktree with a claim on the paths its scope names; if none does, this row is UNDETERMINED between `queued` and done-awaiting-integration — READ THE BRANCH, and never conclude `queued` from the absence alone.** **Prior state, kept as the record: queued** — **D-426: A HIDDEN PROJECT IS DISTINGUISHABLE FROM A NONEXISTENT ONE at `cite`/`sever`/`reinstate` (they check no visibility on their project), and `promote`'s version of the same leak is UNMEASURED. A DISCLOSURE DEFECT, ahead of features (BOB #15).** — owner RECORD.
milestone: M8
interface: I3 (an IC minted with `node tools/mintid.mjs IC`; an answer that disclosed existence now reads as not-found)
design: `docs/architecture/BIO_Membership_Architecture_v2.md` §7 (what a member who cannot see a project receives), with REC-130's rule that a caller without standing receives the byte-identical answer a nonexistent id receives, and DEBT D-426.
depends-on: REC-134 (landed; it touched these acts' authority) — CHECKED AT THE CODE at spawn.
scope: measure `cite`, `sever`, `reinstate` AND `promote` against a project the caller cannot see versus one that does not exist, comparing status, content type and body RAW. Make every answer to a caller who cannot see the project byte-identical to the nonexistent answer. Sweep the other project-targeted acts for the same shape and state your reach.
accepts-when: for each act, an uninvited member's answer on a hidden project is byte-identical to its answer on a never-minted id; a caller who CAN see it still gets the real answer. How a liar passes it: answering not-found to everyone, so the visible-caller arm must succeed. NEGATIVE CONTROL: restore one act's distinguishing answer, and its byte-identity arm fails.
added: 2026-09-18 · CONDUCT #5 (D-426, placed by BOB #15).

### REC-136 · running — **SPAWNED 2026-09-18 by CONDUCT #5. DEPENDS-ON CHECKED AGAINST THE CODE at spawn: REC-124's `conclusions[]` and `op=conclude&project=` are on `main` at `7fd7adca` (`conclude-project.test.mjs` present); BOB's §7.1 items 6–8 are on `main` at `46ee55a3`. Falsify rather than believe: a live worker holds an `agent-*` worktree with a claim on the paths its scope names; if none does, this row is UNDETERMINED between `queued` and done-awaiting-integration — READ THE BRANCH, and never conclude `queued` from the absence alone.** **Prior state, kept as the record: queued** — **A CORRECTION TO JUST-LANDED REC-124, PLACED IMMEDIATELY AFTER IT (BOB #15, `INVESTIGATIVE-SESSION.md` §7.1 items 6 and 7): (6) a NO-PROJECT conclusion NAMES the version whose claim it adopts, and an unnamed version or one with no claim is refused `NO_CLAIM`; (7) WITHDRAWAL is a dated, authored act that APPENDS to the relationship's history, where the latest entry is the stance and every conclusion and withdrawal stays readable (DEC-19). As built, a second conclusion REPLACES the row, which ERASES THE RECORD: a defect.** — owner RECORD.
milestone: M8
