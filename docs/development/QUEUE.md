...
### M0-25 · running — spawned 2026-09-13 by CONDUCT #9, Opus 5, worktree-isolated, background lane (holds no slot). Falsify rather than believe: a live worker holds an `agent-*` worktree running the drivers; if none does, this row reads `queued`.
milestone: M0 (background lane, holds no slot)
