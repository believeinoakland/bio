...
### CPDF-14 · running — spawned 2026-09-10 by CONDUCT #9, Opus 5, worktree-isolated, measurement lane (holds no slot). Falsify rather than believe: a live worker holds an `agent-*` worktree with a probe report for this item; if none does, this row reads `queued`.
