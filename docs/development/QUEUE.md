...
### REC-138 · queued — **D-426: A HIDDEN PROJECT IS DISTINGUISHABLE FROM A NONEXISTENT ONE at `cite`/`sever`/`reinstate` (they check no visibility on their project), and `promote`'s version of the same leak is UNMEASURED. A DISCLOSURE DEFECT, ahead of features (BOB #15).** — owner RECORD.
milestone: M8
