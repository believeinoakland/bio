...
### D-323 · running — spawned 2026-09-13 by CONDUCT #9, Opus 5, worktree-isolated, background lane (holds no slot); covers D-324 in the same item (one vocabulary, one ground). Falsify rather than believe: a live worker holds an `agent-*` worktree with commits toward the suggest vocabulary; if none does, this row reads `queued`.
