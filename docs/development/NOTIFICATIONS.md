...
- §MARKED AS HANDLED — a member's PERSONAL mute of a FINDING (DEC-10's (b) per item, keyed on the finding's stable identity, and (c) per case over the kinds named) and the item form's reach to an UNGROUPED CONDITION (D-170) are BUILT on the plane (D-125, 2026-09-23); the member SURFACE offering them is NOT BUILT.
...
- **FINDING adopted / deferred / dismissed** → an authored record act, carrying its
  ...
  the plane by D-125 on 2026-09-23 — `op=queuemute`'s case and item forms): that leaves only that member's own feed, which states the suppression; it writes no
