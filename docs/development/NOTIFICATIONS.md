...
- §The catalogue — the ids it says will be allocated *"when a generator is built"* have not been: ten generators exist and not one `N-<n>` has been minted, so the catalogue's stable-id promise is unexercised and `kind` carries a slug.
...
- a re-run owed after a lens change `[OBLIGATION]` (D-86 — **DISCLOSED, never
  ...
  run's read gate (`bio-plane/src/store.mjs` `#biasDebtSweep`, IC-231)
