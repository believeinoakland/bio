...
- §Applying a handler to a selection — BUILT 2026-09-23 by D-126 (IC-235); the section's body and its weight table still read `per-item` as [DESIGNED-not-built] and are not rewritten here. What stays open: `op=taskforward` takes a set on the plane but the queue has no bulk forward (its member picker is per item), and a project-scoped finding is not selectable ON THE SURFACE — CORRECTED 2026-09-24 by REC-205, which measured the reason given here (*its act names a project per item*) to be FALSE OF THE PLANE: the act takes a per-item project and a mixed selection is one act. What is left is `civicos-ui/app.html`'s `queueSetOpFor`, which returns null for an item whose `disposition.scope` is `project`, and that is a UI row, not a plane one.
