...
**Incomplete sections** ·
- §The catalogue — 26 of the 35 kinds `queuestate.mjs` now carries have NO PRODUCER, and this list does not say which. The live nine are named in the Status above; the file that knows is `bio-plane/src/queuestate.mjs`, whose header states the split.
- §The catalogue — the ids it says will be allocated *"when a generator is built"* have not been: nine generators exist and not one `N-<n>` has been minted, so the catalogue's stable-id promise is unexercised and `kind` carries a slug.
- §Applying a handler to a selection — BUILT 2026-09-23 by D-126 (IC-224, PROPOSED); the section's body and its weight table still read `per-item` as [DESIGNED-not-built] and are not rewritten here. What stays open: `op=taskforward` takes a set on the plane but the queue has no bulk forward (its member picker is per item), and a project-scoped finding is not selectable (its act names a project per item).
...
- §MARKED AS HANDLED — a member's PERSONAL mute of a FINDING (DEC-10's (b) per item, keyed on the finding's stable identity, and (c) per case over the kinds present) is RULED 2026-09-22 (D-125) and NOT BUILT; `queuestate.mjs` refuses a FINDING by class until it lands; its item form also reaches an UNGROUPED CONDITION (D-170, RULED 2026-09-23), equally NOT BUILT.
