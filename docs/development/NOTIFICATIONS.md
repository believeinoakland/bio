...
- §The catalogue — `bias-debt` (D-86) is LIVE and raises its OBLIGATION, and NOTHING DISCHARGES IT but the lens moving back: an obligation "leaves everyone's list when it is resolved", and no resolve act exists for this kind (no re-run is recognised as settling it, and `op=taskresolve` addresses tasks, not runs). Designed nowhere; reported as a DESIGN GAP by D-86.
...
- a re-run owed after a lens change `[OBLIGATION]` (D-86 — **DISCLOSED, never
  ...
  run's read gate (`bio-plane/src/store.mjs` `#biasDebtSweep`, IC-234)
