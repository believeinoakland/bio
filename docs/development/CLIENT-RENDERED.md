...
- §There is no collision — the DAILY ALLOWANCE this section's budget stands behind is BUILT and, since 2026-09-24 (D-492), holds a bound rather than claiming one: a render reserves its maximum cost at admission, so the renders in flight are counted and `spent_ms + reserved_ms <= allowance` at every moment. **TWO RESIDUES, stated at `store.mjs` `renderAdmit`, and D-520 (2026-09-25) NARROWED BOTH without closing either:** the reservation binds only a renderer that HONOURS the navigation and wait timeouts it was asked for — one that overruns reports the longer time and the allowance is passed by exactly that excess, which the plane cannot check because the elapsed time is the RENDERER'S CLAIM; D-520 made the in-plane driver hold both bounds (it never read the navigation bound before), which binds THIS renderer and says nothing of a `RENDERER` service. And the allowance is an ACCOUNT, not a throttle; D-520 added the throttle (`render_slots`, C-83.8), whose figure is the VENDOR'S CLAIM and NOT MEASURED, and which is per instance while the vendor's limit is per account. The navigation bound is MEASURED since D-520 (M-151: 10,000 ms from a 1,300 ms tail, with a 5,000 ms term for session acquisition that is CHOSEN and replaced by the first live render); the daily figure is still chosen. **DESIGN GAP (D-570):** the asked wait, `networkidle`, never fired on `oaklandca.opengov.com` in 16 of 16 measured runs, so every render of the corpus's founding shell will end on its timeout and read *render may be incomplete* — which wait condition counts a page that keeps a request open as settled is not designed.
...
- [What this changes in what is already built](#what-this-changes-in-what-is-already-built)
  ...
  - [RULED 2026-09-25 by BOB #35 (09:15Z, confirming CONDUCT #22's batch29 composition): a rendered capture's tick captures nothing, and D-338's withdrawal does not apply to it](#ruled-2026-09-25-by-bob-35-0915z-confirming-conduct-22s-batch29-composition-a-rendered-captures-tick-captures-nothing-and-d-338s-withdrawal-does-not-apply-to-it)
  ...
  - [RULED 2026-09-24 by BOB #33: a render HELD for a C-83 reason is shown, and at expiry is recorded UNDETERMINED (D-491, D-523)](#ruled-2026-09-24-by-bob-33-a-render-held-for-a-c-83-reason-is-shown-and-at-expiry-is-recorded-undetermined-d-491-d-523)
  - [RULED 2026-09-24 by BOB #33: the render throttle (D-520; folded by D-520 from BOB's message of 19:10Z, recorded in the drained BOB INBOX on `coord` — QUEUE.md's D-520 row cites it as 19:11Z)](#ruled-2026-09-24-by-bob-33-the-render-throttle-d-520-folded-by-d-520-from-bobs-message-of-1910z-recorded-in-the-drained-bob-inbox-on-coord-queuemds-d-520-row-cites-it-as-1911z)
...
| Browser Run | 10 min/day | 10 hr/month, 10 concurrent included (re-read 2026-09-25, D-520: the limit is 200 per account, 3 new browsers a second; the 10 is what the price includes) |
...
### RULED 2026-09-25 by BOB #35 (09:15Z, confirming CONDUCT #22's batch29 composition): a rendered capture's tick captures nothing, and D-338's withdrawal does not apply to it

Two consequences of the BOB #34 ruling above, surfaced when CONDUCT #22 composed batch29 and CONFIRMED by BOB #35,
2026-09-25 09:15Z (the drained BOB INBOX on `coord`):

1. **A monitor tick on a RENDERED capture captures nothing.** The tick compares shell with shell and states the content
   UNDETERMINED, so D-455's `changed`-tick capture (`OBSERVATION-LOG-DESIGN.md` §4.1) would otherwise file the served
   shell as new content bytes, which the record cannot support. The tick is logged as a look, and its content reading
   stays UNDETERMINED.
2. **D-338's `unmonitorable` withdrawal does not apply to a rendered capture.** It is monitorable at the shell (the
   frame), and its content is already stated undetermined; D-338 withdraws the status of a shell captured as the
   document itself.

BUILT at the c22-batch29 composition, in `op=monitor` (`bio-plane/src/index.mjs`): both the `unmonitorable` test and the
D-455 capture are guarded by `!renderTick`, with CONDUCT #22's comments at each. No suite names either exclusion as its
subject; `monitor-rendered.test.mjs` drives the frame comparison, not these two guards.
...
### RULED 2026-09-24 by BOB #33: a render HELD for a C-83 reason is shown, and at expiry is recorded UNDETERMINED (D-491, D-523)

BOB #33 ruled at 19:54Z, recorded in `docs/archive/ledgers/BOB-INBOX-drained.md` on `coord` (*"a held render expires to
UNDETERMINED with its reason; op=queue shows a waiting render"*) and quoted on QUEUE.md's D-523 row, folded here by D-523:
**KEEP the hold, bounded by the request's `expires`.** A render the unattended sweep asks for and this instance cannot do
(item 3 of the ruling above, built by D-491) stays queued under the C-83 code the plane gave, because every such reason can
change without the ask changing. **At `expires` the render is RECORDED UNDETERMINED with its C-83 reason and RELEASED**: it
is never dropped silently, never asked for again past its expiry, and the served shell is never filed in its place. **An
op=queue condition kind shows a deferred render and its reason in DEC-49 words**, while it waits and after it ends.

D-523 builds it. The drain releases every render request past its own `expires` to the request state `expired`, keeping the
code it was held under, and the run's log records the release as a governed indeterminate under the new CONDITION kind
`render-deferred` (NOTIFICATIONS.md, the catalogue). `op=capturerequests` publishes `render_deferral` (`deferred` or
`expired`, content `undetermined`, with the code, its C-number and its canned translation), and `op=queue` emits one
`render-deferred` item per held or expired render request. **Measured while building it:** before D-523 nothing in the drain
read `expires` at all, so the hold D-491 described as bounded was not; and a render held under C-83 on one tick is held by
the drain's own rate rule (C-28, `CAPTURE_CONDUCT_TICK_SPENT`) on the next when a plain request for the same host takes the
tick's one slot, which overwrites the row's code. So the release applies to EVERY render request past its `expires`, whatever
it last carried, and the reason recorded is the code the row carries, read from the family that minted it. **Not reached by
this ruling:** a PLAIN request past its `expires` is still drained, and a run waiting on a render that expires is told in its
own log but not WOKEN by FL-4's wake, which reads `captured` and `refused` only (both named in D-523's report).

### RULED 2026-09-24 by BOB #33: the render throttle (D-520; folded by D-520 from BOB's message of 19:10Z, recorded in the drained BOB INBOX on `coord` — QUEUE.md's D-520 row cites it as 19:11Z)

1. **The navigation bound is MEASURED, not chosen.** The render's reservation (D-492) rests on the time a navigation may
   take, and that figure comes from a measurement with its date and instrument. D-520 took it: M-151, 24 navigations over
   the client-rendered sources the corpus names, tail 1,300 ms to commit, bound 10,000 ms by the rule M-151 states. The
   in-plane driver now HOLDS the bound it is asked for (before D-520 it never read it).
2. **A concurrency cap, from the vendor's stated limit, LABELLED as the vendor's.** At most `RENDER_CONCURRENCY_CAP` renders
   run at once on an instance. The default is Cloudflare's stated INCLUDED concurrency on Workers Paid, 10 (its pricing
   page, read 2026-09-25); its stated hard limit is 200 per account. It is not measured, because no instance here has run a
   browser.
3. **A render over the cap WAITS; it is never dropped.** In the unattended sweep the tick HOLDS the request under C-83.8 with
   the word `waiting` and asks again on its next tick. A member asking directly is told by name that the render is waiting,
   with nothing fetched and nothing filed, and asks again. A render that cannot run is recorded undetermined with its reason,
   never as a capture that found nothing.
