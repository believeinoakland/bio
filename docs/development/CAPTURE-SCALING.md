...
**Status** · Written 2026-07-28 from figures measured against live pages at 0.37.0, and already corrected once: on 2026-07-31 its header still read *"Nothing here is built yet"* when five of six order-of-work items had shipped, which is why this document carries per-item status at all. Measured again against `bio-plane/src` at plane 0.58.0, **all six items are now [BUILT]** — item 6 (CAP-4) landed as the `reuse_verdicts` table (`schema.mjs`) with the free POSTHOC detection phase, the RATIFY phase's plain GET, and all four outcomes including `not_attempted` — so item 6's own status line is now the stale one. The measurements stand. **Partially complete.** No order-of-work item is unbuilt; what is unbuilt is MEASUREMENT: the freshness window and the recurrence thresholds are constants CHOSEN in code (24 h and `minDocuments: 2` in `reuseDecision`, `subresources.mjs`; a 0.6 share over at least three pages in `siteChrome`, `store.mjs`), which is the `SUBRESOURCE_CAP = 45` mistake this document names by name — and the sections listed below. **§Job one was corrected on 2026-09-21 (D-339)** to state the reuse rule the plane runs — recency of fetch, not stability — and where the measurement that refused the stability gate is recorded; the superseded rule stays in the section, unedited, beside its dated reason. **Reuse condition 3 was corrected in code on 2026-09-23 (CAP-13):** the two-document floor and chrome recurrence count distinct PAGES (document addresses), not primary captures, and a capture whose page the record does not name is stated as undetermined rather than counted. **A reused part names its source capture since 2026-09-23 (CAP-14):** `site_assets.last_fetched_by`, the manifest's `reused_from` and the reusing capture's `site_asset_refs.reused_from`, as BOB #21 ruled; a reuse recorded before then reads undetermined as to its source. **A composite states its temporal spread since 2026-09-25 (D-191):** the manifest's `part_fetch_spread` (also on op=acquire's `snapshot`) gives the earliest and latest instant its held parts were fetched, PER CLOCK — `capture` for parts this capture fetched (`fetched_at`, the capturing Worker's clock) and `record` for reused parts (`reused_from_fetched_at`, `site_assets.last_fetched` on the Store's clock, stamped when the earlier fetch was filed) — and never orders one clock against the other: a composite holding both reads `two_clocks_not_compared` with its composite-wide instants null. as of 2026-09-25.

**Place in the system** · A level-2 design serving construct 2, **intake, capture and provenance**, whose level-1 home is `BIO_Intake_Doctrine_v1_1.md` (`BIO_System_Design.md` §3 names it there). It owns the three things that make capture survive a real corpus — the per-site asset record, resumable sessions, and the empirically calibrated ceiling — and it is the substrate `CAPTURE-FIDELITY.md`'s subresource capture now runs on. Its recurrence signal is what `LINK-FIDELITY.md`'s chrome section reaches for; its composite-capture consequence is D-191 (a capture assembled from reused parts has a temporal spread, which the manifest states per clock since 2026-09-25); and DEC-42 supersedes its free-tier premise while D-185 keeps the frugality.
...
- §Job one: stop re-fetching — CORRECTED 2026-09-21 (D-339): the body now states the rule `reuseDecision` (`subresources.mjs`) runs — furniture kinds only, at least two distinct pages on the host (primary captures until CAP-13), and the source seen serving the bytes within 24 h, with stability demoted to a secondary signal — and where the live measurement that refused the `stable_since` rule is recorded; that rule stays in the section unedited, marked superseded with its reason. Two things in it remain open: both constants are CHOSEN, not measured (§Open questions carries them); which capture fetched a reused part (RULED OWED 2026-09-21 by BOB #21) is **CLOSED 2026-09-23 (CAP-14)**: the build the section names is in the plane — `last_fetched_by`, `reused_from` in the manifest and on the reusing capture's ref row, and `reusedParts` reading that row — and a reuse recorded before it reads `reused_from: null`, `reused_from_state: "undetermined"` (`cap14-reused-from.test.mjs`). **CLOSED 2026-09-23 (CAP-13): the two-document floor counts distinct PAGES.** Until then it counted primary captures, so one page whose bytes changed between two captures met it alone and every re-capture of a changed page wrote an inflated *"across N documents on this host"* into the manifest; `siteAssets` and `siteChrome` (`store.mjs`) now count the primary's document address (`captured_locators.address_norm`), and a primary with no locator row counts as `documents_undetermined`, never as a page (condition 3 below). Until 2026-09-21 this bullet said the sketched `reused_from` / `reused_fetched_at` / `fetched_this_capture` shape "is built as written", and it was not: `fetched_this_capture` was, the time is written as `reused_from_fetched_at`, and `reused_from` existed neither in the manifest nor in the store's `reusedParts` read until CAP-14 (2026-09-23).
- §The per-site asset record — the column list drawn here is not the table that exists. `site_assets` (`schema.mjs`) carries `last_fetched` (the clock the reuse gate reads), `kind` and `changes`, and it has no `capture_count` or `distinct_documents`: the distinct-document count is DERIVED from `site_asset_refs`, one row per (asset, primary capture), joined to `captured_locators` so that it counts document ADDRESSES (CAP-13, 2026-09-23). CORRECTED 2026-09-21 (D-339): until then this bullet said a re-capture cannot double-count it at all, and one whose bytes changed did — it was a new primary sha (§Job one, condition 3); CAP-13 closed that by counting pages.
...
**BUILT 2026-09-23 (CAP-14), as ruled.** `recordSiteAssets` (`store.mjs`) writes
`last_fetched_by` with `last_fetched` on every fetched observation and leaves both
alone on a reuse; a reused part carries `reused_from` in the manifest and its detail
sentence names the capture; the reusing capture's `site_asset_refs.reused_from` is
taken from the capture's own observation, never re-read from `site_assets`; and
`reusedParts` returns it with `reused_from_state` (`recorded` or `undetermined`).
Both columns are added by the additive migration, nullable and never back-filled, so
every reuse recorded before the build reads `undetermined`. The paragraph above
this one, *"What the plane writes"*, is the state before the build.
...
3. at least **2** distinct PAGES on the host have referenced it (`minDocuments`;
   `not_yet_shared_across_documents`), because an asset only one page references
   is that page's own wherever it sits. **A page is the primary's document
   address** — `captured_locators.address_norm` for the `primary_sha` of each
   `site_asset_refs` row (CAP-13, 2026-09-23; `siteAssets` in `store.mjs`), the
   identity the record already keys a document on, so an archive capture and a
   direct one of the same document are one page, and one page captured twice with
   changed bytes is one page, not two. Until CAP-13 the count was of primary
   capture SHAs, and a page whose bytes changed met this floor alone. A primary
   with NO locator row (older than D-58's unconditional write, or one whose write
   failed) cannot say which page it was: it is counted apart as
   `documents_undetermined`, never toward the floor, and an asset only that
   remainder could lift over it is refused as
   `shared_across_documents_undetermined`; a reused part's `detail` names the
   undetermined captures beside N;
...
**THE PART-FETCH SPREAD, RULED 2026-09-25 by BOB #35 (confirming D-191 as built, `9351b715`).** A composite capture's
`part_fetch_spread` spans its PARTS' retrieval instants only. The primary document's own retrieval is a different event
— the fetch that named the parts, not one of them — and is excluded, which the field's note states. The spread orders
instants on ONE clock and never across two: the Worker's clock and the Store's clock have an offset nobody has measured
in the live record, so where a spread would compare them it reads `two_clocks_not_compared` rather than a figure. A
measurement of that offset, if one is ever taken, is what would let a later ruling compare them; until then the field
says which comparison it did not make.
...
- The recurrence threshold, which should come from measurement across fifteen
  ...
  (D-339, 2026-09-21), and in two places.** Reuse requires 2 distinct pages
  on the host (`minDocuments` in `reuseDecision`; distinct primary captures until
  CAP-13, 2026-09-23), and recurrence chrome classification calls an address
  chrome at a share of 0.6 of the host's distinct pages once the host has at
  least three (`siteChrome` in `store.mjs`, whose
