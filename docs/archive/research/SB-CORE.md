...
| **GAP-I10** | **`data/citations.json` is a second claim structure.** C-8.1 validates `{claims:[{claim_id, claim, cites[], snapshot, as_of, hash}]}` — a proto-finding, already specified, already gated, and written by nothing. | **CLOSED 2026-08-08 by FW-13: RETIRED.** Measured first — nothing in the estate writes or reads the file (every sibling register has a real producer; this one had none), and C-8.1 fired correctly and ALONE, so it was a check with no producer rather than a redundant one. `inquiry_basis` holds the claim layer as rows, and a claim inside another object's file cannot be cited, contradicted, graded or composed. The check is deleted, its id and reasoning survive in `CHECK_RETIREMENTS` in `bio-checks.mjs`, and `check-firing.test.mjs` asserts the shape no longer fires AND that the estate grew no producer — so the record keeps ONE claim structure. |
