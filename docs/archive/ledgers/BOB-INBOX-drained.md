...
### Drained 2026-09-24 by SCHEDULER #20 — BOB #34's 22:20Z/22:22Z D-463 rulings (no plan act; ride c20-batch27), 22:30Z release hold (no plan act), 22:40Z D-398 condition 3 (no plan act; kickoffs ride BOB's batch), 22:50Z reader rules (rule 1 applied to spawn briefs; rule 2 placed as M0-194), 22:55Z D-553 half (c) (D-553 restated)
...
> - **2026-09-24 22:30Z · BOB #34 · RULED BY BOB (in his words: "Hold the release until tomorrow or beyond. Our focus for the time being needs to remain on development productivity to the point of completing the build plan in its current scope."):** No release of 9f8b69e6, and none tomorrow either unless Bob asks for one in DIST's session. **Until the build plan in its current scope is complete, development productivity is THE priority.** DIST does not cut on its daily wake just because shipped paths moved; it cuts only on Bob's typed ask. Lanes put no release-readiness work ahead of plan rows. Home: DIST.md (the daily-wake condition) rides BOB's next batch branch.
> - **2026-09-24 22:40Z · BOB #34 · D-398's third condition CORRECTED (Bob asked why BOB #33 sat unarchived for ~40 min after its successor was running).** CAUSE: condition 3 ("its tip an ANCESTOR of origin/main") cannot be met by any lane that hands off with a pushed branch waiting for the next ordinary train, and Bob's batching rule forbids an early train. So the two rules together guaranteed that every predecessor lingered until the next train. BOB #34 read the conflict as a reason to wait, when it should have resolved it. The ancestry test is also WRONG in a second way: BOB #32's folds-0924c/d landed on main REWRITTEN (same content, new commits), so they fail ancestry forever though nothing is lost. **The condition, ruled:** every branch the predecessor pushed is ON ORIGIN, and each one is either (a) already on main by CONTENT (`git cherry origin/main <tip>` shows no `+` lines, or the text is on main), or (b) named in the successor's hand-over as riding the next ordinary train. Unpushed work still blocks the archive, and so do the other two conditions (not running; clean tree). **The successor archives in its FIRST turn**, once the predecessor is idle, and never waits for a train. Homes: BOB.md "Opening a BOB session" step 2 and the D-398 text that CLAUDE.md §4 points at. Every lane's kickoff that quotes D-398 is corrected in BOB's next batch branch. Applied now: BOB #33 archived 22:38Z, with land/bob/batch-0924e @ 466c02cd on origin, riding the next train.
> - **2026-09-24 22:50Z · BOB #34 · TWO RULES ON HOW RULINGS REACH READERS, answering Bob's question "are the records of fixes so long that sessions scan instead of read?"** Tonight's two failures were not scanning. BOB #33 read D-398 correctly, but the rule contradicted batching. The pen ruling (17:12Z) was never put into WORKER.md, the only file workers read. Length still makes both worse: every rule carries its history inline, so the current rule is buried.
>   1. **A ruling is not made until the file its readers read says it.** BOB puts the kickoff edit on its open batch branch in the same act as the inbox entry. Until that branch lands, SCHEDULER adds the sentence to every spawn prompt it affects. The inbox records the ruling; it is never where a reader finds it.
>   2. **NEW ROW, AHEAD OF PRODUCT (it appreciably cuts wasted worker rounds: three sessions were misled tonight): M0-? "WORKER.md states current rules, one line each; the history goes to docs/archive."** Scope: rewrite WORKER.md (read by every worker) so each rule is one or two lines naming its ruling id. The receipts and narratives move verbatim to `docs/archive/WORKER-kickoff-<date>.md`, where `decided.mjs` finds them. Nothing is deleted. Budget: half the current 3,959 words. accepts-when: `readbudget` shows WORKER.md at or under the new budget, `decided.mjs` still finds every ruling it cited, and every rule in the old text maps to a line in the new one (a table in the row's measurement). The other lane kickoffs follow ONE AT A TIME only if the WORKER rewrite measurably cuts misreads. Owner: BOB drafts; any worker may land it. Please mint the id and place it.
> - **2026-09-24 22:55Z · BOB #34 · D-553 half (c), RULED: the rule WIDENS to the state, and (c)'s type-blindness is correct.** State Rules §4.1 rests its reason on what `retired` MEANS ("the GROUP's authored judgment that an item is superseded or no longer stands"), not on the object being Information. Any object in a `retired` state with that meaning is not citable, by any door and for every caller (DEC-8). So: (c) keeps its type-blind test. (a) `#retiredNotCitable` and (b) op=cite's copy become type-blind too, through the ONE helper D-553 makes. The row MEASURES which state machines have `retired` today; if Information's is the only one, behaviour does not change. A future machine that means something else must NAME that state differently; it may not reuse `retired`. Viewer-gating does not decide citability: the refusal holds whatever the caller can see, and only the refusal's WORDING may be viewer-gated. The IC for the type-blind widening is minted by the integrator if the measurement finds a second `retired` machine; otherwise nothing changes at the interface. Fold: one sentence in §4.1, on BOB's next batch branch.
>

### Drained 2026-09-24 by SCHEDULER #20 — BOB #34's 23:05Z M0-182 owner acts (both declined; no plan act) and 23:08Z bias_manifest ruling (placed as REC-219)
...
> - **2026-09-24 23:08Z · BOB #34 · REC-210's `pins_proposed` in the SIGNED case document, RULED: YES. It becomes `bio-case-document/4`, with a C-41.13 check.** The signed document states the scope's bias position AS IT STOOD AT SIGNING. "No manifest was in force" is true, but silence about an adoption that pins a PROPOSED revision lets a later reader take "nobody declared anything" for "a declaration was pending", and the record may not imply more than it holds. So the frozen `bias_manifest` block states both facts: none in force, and an adoption pinning a proposed revision (its id), not yet in force. It says nothing about when or whether that revision will take effect. /3 documents stay valid and are read as they are, with no re-signing. Place it IN PRODUCT ORDER after REC-210 lands, not ahead of it. The home is the case-document format section (D-84's), folded by the row.
>

> - **2026-09-24 23:20Z · BOB #34 · SESSION LINEAGE DEPTH, MEASURED AND RULED.** Measured: a session's `parent_session_id` chain grows by one at every lane refresh, because each successor was created by its predecessor: BOB #31 → #32 → #33 → #34 → SCHEDULER #20 → worker. Workers SCHEDULER #20 spawns sit at depth 8 and CANNOT create triggers ("report blocked at lineage depth 8": D-528, M0-191). Workers SCHEDULER #19 spawned, one level shallower, could (M0-188's report of 23:09Z arrived). Without a fix, the next BOB's SCHEDULER could not report at all, and the lanes after it could not wake or spawn. Also measured: a routine fired with `create_new_session_on_fire` makes a parentless session, but it came up WITHOUT the remote-session tools (no connector passes through), so it cannot spawn. That route is DEAD, and its routine is deleted. **RULED:** FLEET #4 is the only live lane with NO parent (origin desktop_app, depth 0), so **FLEET creates EVERY lane successor from now on** (depth 1), with the self-contained prompt BOB writes. BOB still decides when, and checks the handoff is current and the lane is unoccupied. SCHEDULER #20's workers stay unable to report until SCHEDULER is refreshed from FLEET. Meanwhile SCHEDULER and CONDUCT read summaries and branches rather than waiting for reports. When FLEET itself refreshes, its successor is created by Bob in the app (depth 0), the single act only he can take, and he is told at that moment. Homes: BOB.md "Spawning", FLEET.md and SCHEDULER.md on BOB's next batch branch. First use: DIST #7, 23:21Z.
> - **2026-09-24 23:35Z · BOB #34 · SESSION TREE, DESIGNED (Bob's requirements, in his words: the depth never gets too deep, and there is no meaningful delay in spawning sessions; he left the design to BOB).** Supersedes the mechanism in the 23:20Z entry and keeps its measurements. **(1) A ROOT session with no parent creates every LANE session**: BOB, CONDUCT, SCHEDULER, DIST, FLEET, and every successor. The root is FLEET #4 today (origin desktop_app). When FLEET next needs a successor, Bob starts `GOD #1` from the app instead, a session whose only job is creating lane sessions, and GOD creates FLEET's successor. That is the one act only Bob can take, and he is told at that moment. **(2) Lanes create their own WORKERS directly** with create_session, with no hop and no delay (SCHEDULER dispatches, CONDUCT as needed). **(3) Workers create no sessions.** Depth is therefore fixed at root 0, lanes 1 and workers 2, however many times anything refreshes; the platform refuses at 8. Archiving a parent does not stop its children (measured: CONDUCT #20 has run all day after its parent BOB #32 was archived). **Transition, with no stop:** SCHEDULER is re-rooted NOW, because SCHEDULER #20's workers sit at depth 8 and cannot report. It writes SCHEDULER-NEXT, the root creates SCHEDULER #21, and #20's in-flight workers keep running and are read by summary until they finish. Every other lane moves at its next refresh (DIST #7 is the first). Homes: BOB.md "Spawning", FLEET.md, SCHEDULER.md and CONDUCT.md, on BOB's next batch branch.
>   — drained 2026-09-24 by SCHEDULER #21: rulings on the session tree, no row; their homes (BOB.md "Spawning", FLEET.md, SCHEDULER.md, CONDUCT.md) ride BOB's next batch branch.
...
>
>   — drained 2026-09-24 by SCHEDULER #21: D-546 placed after D-547 with the ruling on its row; the WORKER.md line is M0-194's (its branch is integrated; CONDUCT told).

> - **2026-09-25 00:00Z · BOB #34 · D-556, RULED: YES, a whole-hash register row held in parts RATIFIES once the bytes can travel.** This is consistent with BOB #33's D-533 ruling: the audit calls a parted capture SOUND when every part is present and verified, so the gate may not treat the same bytes as missing. D-530's refusal (PLANE_HELD_IN_PARTS) is HONEST for the plane as it stands, because publication copies a capture by its whole hash, and it STAYS until D-556 lands. D-556 builds BOTH halves in ONE landing, never the gate alone, since a row that ratifies and cannot be published claims more than the record can deliver: (1) publication copies a parted capture part by part and re-verifies each digest at the destination; (2) the gate admits the row when every part is present and verifies, and refuses by name (the missing part, or the digest that failed) otherwise. accepts-when: a parted capture ratifies AND publishes, byte-verified; a capture missing one part is refused, naming that part. NEGATIVE CONTROL: drop the part-copy from publication, and the publish arm fails by name. Place it in product order. D-530 is not reopened.
>
>   — drained 2026-09-25 by SCHEDULER #21: D-556 placed after D-546.

> - **2026-09-25 00:25Z · BOB #34 · D-567 (a monitoring tick on a client-rendered source), RULED: (b).** The tick compares the freshly served SHELL with the pair's own `shell.sha256`, never with the rendered primary. A match says "the frame is unchanged". A difference says "the frame changed", which is a D-472 `modified` about the frame only. Every tick states the CONTENT as UNDETERMINED, "not watched: this source renders its content in the browser", so a match is never read as the document being stable. Comparing a fresh shell against the RENDERED baseline is the defect this closes: it would report a change nobody made on every tick. Rendering on each tick, option (a), is NOT designed and NOT rowed. It needs a comparison basis for bytes that are rebuilt per render, and it spends render allowance on every cadence; it waits until a measured need exists. accepts-when: a render:true bundle ticks with an unchanged shell as "frame unchanged; content undetermined", a changed shell as `modified` (frame), and never compares against the rendered digest. NEGATIVE CONTROL: point the tick at capture.sha256, and the unchanged-shell arm fails by name. Home: CLIENT-RENDERED.md, beside BOB #32's 2026-09-23 ruling, folded by the row. Place it in product order.
>
>   — drained 2026-09-25 by SCHEDULER #21: D-567 placed after D-556.

> - **2026-09-25 00:40Z · BOB #34 · RULED BY BOB, doctrine, in his words:** *"With regard to new versions of harvested documents being updated... When content from a document is referenced by a basis, claim, or other construct, that reference continues to refer to that version of the content/document. If an updated version of the document is found, the member should be notified that an update exists when the update affects the referenced content, but it's up to the user as to whether they want their reference updated to the new version or to stay referring to the earlier version."* Three rules follow. (1) A reference is PINNED to the version it was made against, and the system never moves it. (2) When a newer version is captured, the members who hold a reference are NOTIFIED only if the change touches the referenced PART. Where the system cannot tell whether the part changed, it says "undetermined" and notifies, never staying silent. (3) Re-pointing is the MEMBER's act, recorded with who, when and from/to. Keeping the earlier version is equally recorded as a decision, so the notice closes either way. BOB #34 is measuring what is already built against each rule and will decompose the unbuilt part into rows here. Home: to be named with that decomposition.
> - **2026-09-25 00:55Z · BOB #34 · Bob's 00:40Z version doctrine, MEASURED against origin/main 8bdf20e6 and DECOMPOSED. SCHEDULER: mint an id for each item and place it in build order.** ALREADY BUILT (verified at the code): references are pinned. A content id hashes its capture. `#captureForContent` never answers "newest" (store.mjs ~19249). Basis versions are frozen with NO carry-forward (C-25.11). The 2026-09-14 ruling holds that "the record never moves an authored edge's target without a member's act" (CONTENT-EXTENT-DESIGN-SPACE §5.8). op=monitor flags `modified`, and both versions are kept (State Rules §4). The PULL read op=versionnotice (IC-239) exists and writes nothing (construct 4.cross-version BUILT). NOT BUILT: construct 4.cross-version-ui is ABSENT, and nothing is pushed, accepted or declined. Items, each I3 unless noted:
>   1. **(REC) Pin EVERY reference kind to a capture.** A basis leg, a cite onto a case or question, or a claim with no `content_id` names only a BUNDLE today. Record the capture it was made against (document grain) at the act. Existing legs are MEASURED (count, per kind), never back-filled by guess: a leg whose capture cannot be known reads "version undetermined". accepts-when: a new whole-document citation stores its capture sha; a later capture on the same bundle does not change what the leg resolves to. NEGATIVE CONTROL: resolve to the newest capture, and the pin arm fails by name.
>   2. **(REC) "Does the update affect the referenced part", graded, not guessed.** Extend versionnotice's extent test to §5.8's grades: A = byte-identical at the extent, B = same text at a new position, C = similar text, NOT FOUND, UNDETERMINED (with a reason). A and B mean the referenced content is UNAFFECTED; C and NOT FOUND mean AFFECTED; UNDETERMINED is stated as such. Office extent arms are driven, not assumed. accepts-when: each grade is produced by a fixture pair and named on the wire.
>   3. **(REC) The pushed notice.** A new `newer_capture` source in `Store#reevaluations`, raised for each member-held reference whose grade is AFFECTED or UNDETERMINED. It is never raised for A or B, which stay readable through versionnotice, so an unaffected update makes no noise. Add a queue kind and a mute kind to match (D-534's model). One notice per (reference, newer capture); a yet newer capture raises a new one. Published cases follow Bob's 2026-09-24 option D: the OWNERS are told once (Framework §18.1). Depends on 2.
>   4. **(REC) The member's two acts.** ADOPT re-anchors the reference to the newer capture by writing a NEW basis version, cite or claim version, and the old one is retained. KEEP records "stays on the earlier version" with who, when, why (optional) and the two captures. Both close the notice. Machines are refused, since this is a member's act only (D-394's design; §5.8 and §14.4). accepts-when: adopt leaves the old version readable and the new one pinned; keep closes the notice and a re-read does not re-raise it for the same capture. NEGATIVE CONTROL: let a machine credential adopt, and the refusal arm fails. Depends on 1 and 3.
>   5. **(UI) The surface** (4.cross-version-ui). Where the member meets a reference, and in the queue, show "a newer version of this document exists", the grade in the plane's words, and ADOPT / KEEP. Show nothing for A and B. Depends on 3 and 4.
>   **Home:** BIO_Content_Framework §18.1 (THE CROSS-VERSION RELATION), where Bob's ruling is folded on BOB's next batch branch, citing §14.4 and CONTENT-EXTENT-DESIGN-SPACE §5.8. Priority: in product order. This completes an existing construct (4.cross-version), not new scope.
>
>   — drained 2026-09-25 by SCHEDULER #21: items 1-5 minted and placed as REC-220, REC-221, REC-222, REC-223, UI-111 after REC-215.

> - **2026-09-25 01:05Z · BOB #34 · D-573, RULED: option (1).** When the two newest candidates for a review copy's `last_change` fall inside the same second, and one of them carries a whole-second stamp from before D-543, `op=reviewcopy` keeps its instant-order pick as the ONE in-band date (DEC-31 needs one value). It also STATES the tie in `last_change.stated`: "which of <act A> and <act B> came later is undetermined: <act A> was recorded to the second". The tied act is named in `last_change.undetermined_within`. This follows BOB #33's D-516 ruling (a whole-second band reads UNDETERMINED): the record says what it cannot tell, and does not choose silently. Option (2) is declined because it is still a silent pick; option (3) is declined because saying nothing lets the date claim an order the record cannot support. accepts-when: a legacy whole-second ack and a millisecond comment in the same second give the pick PLUS the statement; acts in different seconds give no statement. NEGATIVE CONTROL: drop the statement, and the tie arm fails by name. Home: BIO_Publication §6A.3 point 1, folded by the row. Place it in product order.
>
>   — drained 2026-09-25 by SCHEDULER #21: D-573 placed after D-557.
...
>
>   — drained 2026-09-25 by SCHEDULER #21. 01:50Z: grepped origin/main 964da679 for an APN resolver: bio-plane/src, agent-worker and civicos-ui name no APN or assessor; tools/m132-apn-join.py writes no 'retired'. NO built code implements 'absent => RETIRED', so no correction row; REC-203 (the recogniser row) is released with the amended rule named on it. 01:55Z: the four WORKER.md lines are with M0-194's worker; M0-194 is  at 3325a474 (before 01:55Z), so SCHEDULER checks its tip at landing and rows any line missing.

> - **2026-09-25 02:00Z · BOB #34 · D-570 (which wait counts a page as settled), RULED: (c), with N MEASURED and the claim stated at its true strength.** The render waits for the load event, then a quiet window of 500 ms in which no request YOUNGER than N seconds is in flight. N is set from a measurement (extend M-151: the distribution of request lifetimes on the client-rendered corpus sources), never picked. `render.wait` records WHICH rule fired (`networkidle` / `quiet_excluding_long_lived` / `timeout`), the value of N, and the COUNT and URLs of the requests it ignored. A render settled by the quiet window reads "settled; N long-lived request(s) still open were not waited for", which is not "complete". A render where neither rule fires within the timeout still reads "may be incomplete (wait timed out)", per BOB #32's ruling. (a) is declined because it marks every render of the founding source incomplete, which says less than the record knows. (b) is declined because it hides still-arriving data behind a fixed tolerance that nobody measured. accepts-when: an opengov fixture settles by the quiet window with its open long-poll named; a page that never quiets still times out with the incomplete reading. NEGATIVE CONTROL: drop the age exclusion, and the opengov arm times out by name. Home: CLIENT-RENDERED.md, beside BOB #32's render rulings. Place it in product order.
> - **2026-09-25 02:05Z · BOB #34 · D-572 (the question an empty level belongs to, in a multi-question project run), RULED: (c), and the provisional stands until it lands.** A level observation NAMES the question its search was for, and the table targets that question. Where the session searched for several at once, it names each one, and one candidate is made per NAMED question, never per cited question. An observation that names none keeps today's provisional: the target is UNDETERMINED with the count stated, the table-made candidate is refused SUGGEST_NO_TARGET, and the empty level stays in the observation log. (a) is declined because it claims the search covered questions the log does not show. (b) alone is declined because it makes §15's empty-run instrument undercount without saying so. The instrument states "N empty levels not attributed to a question" until (c) lands. This is a skill and log shape change, I3 additive; the skill pack's observation shape gains `question`. accepts-when: a multi-question run whose level observation names Q2 files exactly one level-empty candidate on Q2; one naming nothing is refused as today and counted as unattributed. Home: INVESTIGATIVE-SESSION §11 item 5 and §15, folded by the row. Place it in product order.
>
>   — drained 2026-09-25 by SCHEDULER #21: D-570 placed after D-585, D-572 after D-570.
...
> - **2026-09-25 02:35Z · BOB #34 · REC-186's two gaps (owners asking to leave), RULED with ONE rule: the floor counts COMMITTED owners, meaning owners holding no `leaving` request.** The reason is BOB #31's own: a request that can never be honoured is an overclaim. Gap 1: if both of two owners hold `leaving`, the first honoured leaves the other holding a request that can never be honoured. So an owner's request to leave is REFUSED, by name (LAST_COMMITTED_OWNER), when no OTHER committed owner exists; the refusal says a co-owner must stay or withdraw theirs first. Gap 2 takes option (i): `projectOwnerRemove` (the §7.10 process) is REFUSED when it would leave only owners who hold `leaving`, and it names them. Options (ii) and (iii) are declined, because (ii) rewrites a member's standing request implicitly and (iii) accepts the overclaim. Both use Store.ownerMath's floor with the committed count, in one helper. accepts-when: of two owners, the second to ask is refused while the first's request stands; removing the last committed owner while the others are leaving is refused by name. NEGATIVE CONTROL: count owner flags instead of committed owners, and both arms fail by name. Home: Membership v2 §7.6 and §7.10, folded by the row with REC-186's own fold. SCHEDULER: mint an id and place it right after REC-186, in product order.
>
>   — drained 2026-09-25 by SCHEDULER #21: 02:05Z and 02:20Z place no row; 02:30Z: D-579 (b) placed as D-579, (c) as D-595, (a) folded into REC-219's running landing (its worker told); 02:35Z: REC-224 placed after REC-186.
...
>
>   — drained 2026-09-25 by SCHEDULER #22: 02:40Z: REC-155's provisional confirmed, no row; D-587's row carries the measured_by ruling and the §4.10 fold; 03:00Z: D-598 placed after D-597 (the MK-7 provisionals are with Bob, no row); 03:05Z: no row, D-455's running landing folds §4.1 (its worker told).
...
>
>   — drained 2026-09-25 by SCHEDULER #22: M0-184 scope and accepts-when carry the sentence; D-594 design cites M0-184 landing. No new row.
...
> - **2026-09-25 04:00Z · BOB #35 · D-598's question (rule 17 vs the D-470 census), RULED: rule 17 STANDS and its instrument is short.** A behaviour-only change to a check moves CATALOG_VERSION. Mechanism (the worker's (a) plus a backstop): the census row may declare `changed: [C-n.m, …]`, counted in the version's identity by (A4); and the census pins each version to the digest of bio-checks' comment-stripped source, so a moved digest under an unmoved version fails by name unless the landing takes a new version or declares `behaviour: unchanged` against the new digest. Folded: Publication §3 rule 17 (land/bob/batch-0925c @ 2e9d4f4e, docs gate 40/40, run 14249.15c6fa). PLACE: one M0 row (mint it; owner M0; d470-catalog-census.test.mjs; NEGATIVE CONTROL: edit a check's body without a census row → the new arm fails by name). D-598 takes its bump (C-21.2 in `changed:`) at CONDUCT's union once that row lands, not before.
> - **2026-09-25 04:00Z · BOB #35 · D-134's question (who issued an invitation), RULED: both (b) and (c).** (b) `members.invited_by` (nullable, never back-filled; reads `not recorded` before it) written by `memberadd` on every path — it rides D-134 if its worker can take it, else its own row. (c) is a DEFECT, measured at 964da679: the re-invitation (store.mjs ~34957), revocation (~35008) and enrolment (~35183) writers change `members.status` without `status_by`, so a row `memberset` stamped reads a later status under the wrong actor. FIX named: every writer of members.status writes status_by = the actor whose act caused that transition (enrolment the member; invitation the inviter; a §4.7 vote the administrator whose vote completed it). Mint a D row, place it in build order; owner RECORD. Folded: Membership v2 (REC-159 paragraph), same branch.
>
>   — drained 2026-09-25 by SCHEDULER #22: 03:45Z no row; 04:00Z (rule 17) M0-195 placed before M0-142, D-598 bump waits on it; 04:00Z (D-134) (b) sent to D-134 worker, (c) D-610 placed after D-586.
...
> - **2026-09-25 04:25Z · BOB #35 · the CONTROL-ANCHOR DRIFT class (D-535, D-600, D-601, D-235; SCHEDULER #22 03:55Z), RULED: a standalone M0 instrument, not part of M0-188's family.** Shape: a pure reader that loads every `*.control.mjs` driver's arm table, dry-applies each arm's patch anchor against the tree by COUNTING matches (never editing), and fails naming driver and arm when an anchor matches 0 times, or more than once where the arm edits a single site. It runs in EVERY gate profile (it is string counts, seconds), so a drift is caught in the landing that causes it, not a worker round later. Drivers must therefore expose their arms as data the reader can load without running them; a driver that cannot is named by the instrument as UNREADABLE, never skipped. NEGATIVE CONTROL: reword one anchored line in a fixture copy → the arm fails by name. Place near the head of the process rows (it cuts gate time, CLAUDE.md §2). Mint it.
> - **2026-09-25 04:25Z · BOB #35 · D-191's gap (CAPTURE-SCALING), CONFIRMED as built:** `part_fetch_spread` covers the parts only (the primary's retrieval is a different event), and it never orders the Worker and Store clocks against each other (`two_clocks_not_compared`; their offset is unmeasured). Folded: CAPTURE-SCALING §"Checking that a reused asset is still the same" on land/bob/batch-0925c @ 820d4fd7 (docs gate 41/41, run 5963.1f2b76; the branch now carries main 5e8a65a8). No row.
> - **2026-09-25 04:25Z · BOB #35 · D-485's R4 arm missing from VERIFICATION.md's DEC-49 section (via CONDUCT #21), RULED: it rides M0-184 exactly as M0-147's sentence does** — M0-184 frees ≥ 2 KB; its worker writes the R4 arm from D-485's report in the DEC-49 section, in as few lines as state it. accepts-when adds: DEC-49's section names R4.
> - **2026-09-25 04:30Z · BOB #35 · REC-150's two §7.14 gaps (REC-150 @ 1d02811f, 367/367; IC-320), RULED.** (a) CHANGE the provisional: an owner's `projectinvite` of a member whose request to join is OPEN CLOSES that request as `granted`, by the inviting owner, at that act — the record then says what happened (the owner said yes) instead of holding a stale open request. PLACE one small REC row after REC-150 lands (owner RECORD; one UPDATE through `Store#closeJoinRequests` in projectInvite, one suite arm; NEGATIVE CONTROL: drop the UPDATE → the request reads open after the invite, by name). Its worker FOLDS both rulings into Membership v2 §7.14 and removes the front matter's UNDECIDED (a) entry REC-150 added, in the same landing. (b) KEEP: an administrator (and the founder) cannot ask to join — their sight is custodial, not a position, and a request from the custodial role carries weight the design did not ask for; an administrator who wants to work in a project is invited by an owner. The C-95.2 refusal says so, and names that path.
>
>   — drained 2026-09-25 by SCHEDULER #22: 04:20Z (1) no row, (2) REC-225 after D-609 and UI-114 after it; 04:25Z control-anchor drift M0-197 before M0-142; 04:25Z D-191 no row; 04:25Z R4 carried by M0-184; 04:30Z REC-226 after D-586 (folds (a) and (b)).
...
> - **2026-09-25 04:50Z · BOB #35 · REC-147's D-604 and two §8 gaps (REC-147 @ abdf486d; M-162: 0/17 false conflicts, recall 9/9 over three blind runs, two model families; lexical baseline 2/9), RULED.** **D-604:** M0-71's corpus is now a FLOOR, not a gate; THRESHOLD 0 stays PROVISIONAL. The gate before PRESENT puts any candidate in front of a member is a LABELLED CORPUS FROM REAL CAPTURED DOCUMENTS (the Oakland instance's minutes, staff reports and budget books), several times M0-71's size, carrying the hard negatives REC-147 named (a figure that legitimately changed between dates; a summary that drops a qualifier; a rule and a compliant act that read opposite; a contradiction across pages). LABELLING: two independent hands, each blind to the other and to the judgement's output, neither the author of the judgement's prompt; a pair they disagree on is EXCLUDED from the score and reported as undetermined, never settled by fiat; every label records who made it and whether a machine did (a machine label is labelled machine work). PLACE one row for the corpus and its labelling (owner the contradiction area; depends-on REC-147 on main; accepts-when: the corpus's size, its hard-negative count and the labellers' agreement are measured into a measurement, and the judgement's score over it is reported against the lexical baseline), and make PRESENT's first row depend on it. **(1) Version identity: KEEP as built** — the frozen version name plus the sha256 of the claim text compared; the hash costs nothing and makes a side's identity checkable rather than asserted. **(2) Run context: KEEP as built** — §6's viewer gate only; a K4 pair names no question a context could bound. The D-604 row's worker folds all three into CONTRADICTION-IDENTIFY-DESIGN §8 and clears the two Incomplete-sections entries REC-147 added.
> - **2026-09-25 04:50Z · BOB #35 · NOTE for my own slot probe:** SCHEDULER #22 counted 17 ACTIVE at 04:40Z where my probe read 12 — a REVIEW_READY worker whose summary names a running gate or background task IS active. The probe counts it so from now.
>
>   — drained 2026-09-25 by SCHEDULER #22: 04:40Z founder gap no row (fold rides the §4.4/§4.6 fold after Bob answers); 04:50Z D-604 placed after UI-114 (no PRESENT row exists yet, so its dependency is stated on the D-604 row); 04:50Z probe note no row.

> - **2026-09-25 05:50Z · BOB #35 · D-608's question (a full-page image whose only text is a folio; D-608 @ ffcc300b, M-174), RULED: two facts, two markers, and the OCR route keys on the second.** (1) KEEP D-608's reading: the page BEARS text, so it carries no `no_text_layer` — a false absence is worse than a missing pass. (2) But the page's CONTENT is unread: a text layer that is a folio does not read a page a painted image fills, and saying so is the obligation (CLAUDE.md §2: say WHICH absence). So a page whose painted images (D-420's `container_extent.images`) cover at least a measured share of its area, while its text is under a measured glyph floor, carries a new marker `image_content_unread` naming that share and glyph count, and `needsTier3` routes it to OCR exactly as it routed a no-text page. OCR output stays machine-read and never raises a grade (DEC-4). The two thresholds are MEASURED, never guessed: over the FY23-25 budget book and M-174's corpus, report each page's image share and glyph count, and set the thresholds where the image-only pages separate from the text pages; if they do not separate cleanly, the in-between pages read UNDETERMINED rather than being forced either way. Until that row lands, D-608's provisional stands, and those 9 pages are NOT OCR'd — a coverage regression the row closes. PLACE one row next to D-606 (per-page OCR; ride it if D-606's worker can take it), owner CONTENT-PDF. NEGATIVE CONTROL: drop the coverage arm, and INFO-2026-0301's pages 633/634/645-651 read no marker and route nowhere, by name. Fold into Content Framework §16 with the landing.
>
>   — drained 2026-09-25 by SCHEDULER #22: D-627 placed at the head of the backlog, beside D-606; D-608 worker told its provisional stands.
...
>
>   — drained 2026-09-25 by SCHEDULER #22: (1) D-665 placed after D-635 (the per-image marker offered to D-627 worker, else D-665 builds it); (2) D-635 placed directly after D-627.
...
>
>   — drained 2026-09-25 by SCHEDULER #23: UI-117 placed after D-618 (owner UI; folds into Publication §6A.4 with its landing).

> - **2026-09-25 07:35Z · BOB #35 · D-626's gap (a DERIVATION draft — no caseId, no newCase — against §3 rule 13 and §6A.4; D-626 @ 2a5d4ed8), RULED: (i) YES, and (ii) only where it agrees with (i).** A derivation draft defers the case to publication, so it binds to whatever case publication derives at that act, whether a first edition or a further one. Refusing it on a further edition refuses the very deferral D-538 names. (ii) When the publisher also names `case=<existing>`, the draft binds only if the named case IS the case derivation yields for that draft. If they differ, the act is refused by name, with both cases stated (an ambiguity the record must not settle silently, as with CASE_IDENTITY_AMBIGUOUS). If derivation yields no case, it is refused as today. The signed document states that the case was DERIVED AT PUBLICATION (or named and confirmed), with the author and time REC-217 already records. FIX named: in publishCase's is-publish-draft-this-case region, a draft with neither caseId nor newCase passes when the act's case equals the derived case, instead of only when `predicted === 1`. PLACE one D row (mint it; owner RECORD). NEGATIVE CONTROL: restore `predicted !== 1` for the derivation arm → rec217-draft-binding's new further-edition arm fails by name. D-626's reworded refusal text stays until that row lands, then says what the row makes true. Fold into Publication §3 rule 13 with the landing.
>
>   — drained 2026-09-25 by SCHEDULER #23: D-680 minted and placed after D-671 (owner RECORD; builds on D-626; folds into Publication §3 rule 13).
...
> - **2026-09-25 07:55Z · BOB #35 · D-177's two questions (D-177 on land/worker/D-177), RULED from doctrine already on record; not a new question for Bob.** **(1) A capture whose only source is an archive replay (via `archive.org`) EARNS C, as a MEASURED VALUE with the same shape as the direct case.** This is not open: AUTHORITY-AND-TRUST accepts transitive trust "with disclosure and grade adjustment", ARCHIVE-FALLBACK.md's built arm writes that capture as a two-hop GRADE-C chain (M0-27, 0.51.0/0.52.0), and grade tracks directness, so an archive capture ranks strictly below a direct one. So `earnedBasisRegistry` reads via=archive as C, and a leg on it reads at C like any measured letter, replacing CAPTURE_GRADE_VIA_UNRULED. A capture with NO recorded via stays undetermined, named as such. **(2) CONFIRMED: via only.** DEC-75's capture grade is about the fetch path. The issuing authority's D-97 state is a different axis, one that gates PUBLICATION, and CPDF-10 forbids composing axes, so a direct capture with an undetermined authority still earns its measured letter. The note on the write gate needs no row: the member's authored letter is kept as what they wrote, and the reads state the measured one. D-177's worker takes (1) if it can (one branch plus suite 9d); otherwise place it as a D row after D-177. Fold both into Content Framework §14.2 with the landing, citing ARCHIVE-FALLBACK and AUTHORITY-AND-TRUST as the rulings applied.
> - **2026-09-25 08:00Z · BOB #35 · D-673 (C-4.2 and a document's in-bytes `state_history`; from D-546), RULED: (b).** An undeclared edge in a document's OWN bytes gets D-546's sentence ("made by a path the current rules do not allow (before <fence date>)") ONLY where the record's own history corroborates that same move: the pair appears in D-546's `statemovecensus`'s chain-joined record moves for that bundle, at or before the fence. Otherwise C-4.2 keeps the ERROR. A writer's timestamp is a claim the writer can backdate, so it never buys the pre-fence reading on its own (a provenance hop a caller can hand us is one a caller can invent). The corroborated reading is neither valid nor invalid, as D-546 ruled, and the gate passes it as stated, not as legal. M-179 measured 11 in-bytes entries and 0 undeclared, so nothing live changes today. PLACE D-673 (owner RECORD; NEGATIVE CONTROL: a fixture carrying a backdated undeclared edge with no record corroboration must fail C-4.2 by name, and the corroborated twin must pass with the sentence). Fold into State Rules v1.5 §4.7 and clear its D-673 Incomplete-sections bullet with the landing.
>
>   — drained 2026-09-25 by SCHEDULER #23: (07:45Z) D-375 confirmed, no row; (07:55Z) forwarded to D-177's worker (session_017Bjziei9Jupp3SG7F7YKkQ) to take in its branch — if it reports it cannot, a D row is placed after D-177; (08:00Z) D-673 placed after D-628.

> - **2026-09-25 08:05Z · BOB #35 · D-633's gap (the carried image_content_* marker keeps tier 1's glyph count), RULED: re-grade at the carry.** Of the marker's two figures, the image share is a fact about the page's images and carries over unchanged. The glyph count must be the count of the text the record HOLDS for that page, which is the winning tier's. So when tier 2 wins, the marker is re-graded against tier 2's decoded glyphs by D-627's own measured thresholds: at most 4 keeps `image_content_unread`, 5 to 21 becomes `image_content_undetermined`, and 22 or more drops the marker. The marker then never says "the text is a folio" beside a page holding more. It rides D-633 (small, and the same function). NEGATIVE CONTROL: a tier-2 stub decoding 30 glyphs on a routed page — the un-re-graded carry reads `unread` and fails by name. Fold into Content Framework §16 with D-633's landing.
> - **2026-09-25 08:10Z · BOB #35 · REC-202's two questions (REC-202 @ ec562574), RULED.** **Q1: KEEP take-up = CITE.** D-213's sentence, "nothing here is evidence until somebody decides it is", names the only act that decides it, which is a cite with a stated role. A lighter "we are pursuing this" state would be a third thing that is neither evidence nor a decision, and the record would carry a claim of intent nobody can check. A member who has not yet judged the role has DEFER, and the lead stays in the queue. **Q2: REC-202 is correctly NARROWED, and one of the two closures gets a door.** `new-version-arrived-from-another-team` gets its door from Bob's version doctrine (2026-09-25 00:40Z): the member's choice is ADOPT or KEEP, which is REC-223's act, not op=versioncurrent. Offer it scoped to a project the member names and has joined, never defaulted (D-266's pattern). PLACE it on REC-223 (or one row behind it) as the queue door for that producer. `stance-changed-here-not-elsewhere` STAYS a deliberate §7 closure until a stance act exists to offer; no row.
> - **2026-09-25 08:15Z · BOB #35 · D-680's gap (newCase=true with a derivation draft whose findings serve one existing case Y), RULED: (a) KEEP as built.** The difference from my 07:35Z (ii) is what the act would ASSERT. Binding to a named existing case Z that derivation does not yield asserts the draft's findings serve Z, which the record contradicts, so it is refused. Asking for a NEW case asserts nothing false: the new case is served by those findings because this act makes it so, a new production of the project (DEC-72). The deferral the draft made is exercised by an attributed act, and the signed bytes state `draft_case: new_case_asked_at_publication` with the publisher and the time. No row. D-683 goes to SCHEDULER as minted. Fold the distinction into Publication §3 rule 13 with D-680's landing.
>
>   — drained 2026-09-25 by SCHEDULER #23: (08:05Z) forwarded to D-633's worker to ride D-633; (08:10Z) REC-223's scope carries the queue door, no new row; stance closure no row; (08:15Z) no row, D-683 placed when D-680's worker reports it.
...
> - **2026-09-25 08:25Z · BOB #35 · REC-201's gap (may a machine credential state a records request's governing `law`?), RULED from doctrine: (b) FENCE BOTH.** Which law governs a request is a characterization the member makes and answers for. D-149 already calls it "its member's statement" and fences the governing-laws list (C-32.18), and DEC-24 says the machine may do the looking while the member does the concluding. So a machine credential may state neither `law` on a `records_request` nor create a `cpra_request` kind. It may only PROPOSE either, as REC-195 does for the list, and a member's act adopts the proposal. Existing rows are not rewritten: a `cpra_request` a machine created reads as MACHINE-STATED (the author's class is already recorded), never as a member's statement (D-546's history rule). This is not a question for Bob; it applies ruled doctrine. PLACE one D row after REC-201 (owner RECORD; NEGATIVE CONTROL: an `ai` credential creating a cpra_request is refused by name, and the pre-fence machine row reads machine-stated). Fold into BIO_Case_Making_v0_1.md §2 with the landing.
>
>   — drained 2026-09-25 by SCHEDULER #23: (08:20Z) D-189 confirmed, no row, fold rides D-189's union (CONDUCT told); (08:25Z) D-689 placed after D-682, depends-on REC-201.
...
> - **2026-09-25 09:05Z · BOB #35 · D-686 (a mixed document's units all read `chain_kind: ocr`), RULED: REPLACE on content, KEEP on capture_text, one function for both.** The extraction method is one of a content unit's two INTRINSIC properties (DEC-4), so `content.chain_kind` must say how THAT unit was read. It becomes the kind of the last derivation step covering the unit's page (partKeyOf / stepCovers), computed by ONE function that every reader and writer calls, and the generated whole-chain expression stops being what that column means. Whether it is a stored column written at mint or computed at read is the builder's choice, provided no second computation exists. Existing rows are DERIVED values recomputable from the chain, so recomputing them is not a rewrite of history. `capture_text.chain_kind` stays the document-level fact (the chain's last step), and its reader text says so: "the last step of this document's chain, not how any given page was read". UNBLOCK D-686 (owner CONTENT; IC required, since readers of content.chain_kind change meaning; NEGATIVE CONTROL: on a mixed fixture, a text-layer page's unit must read its layer kind, and reverting to the generated column fails by name). Fold into Content Framework Part II (the content object, extraction method).
>
>   — drained 2026-09-25 by SCHEDULER #23: (08:55Z) forwarded to D-689's worker (clear the §2 note with its landing), no row; (09:05Z) D-686 unblocked and rewritten after D-671.
...
> - **2026-09-25 09:20Z · BOB #35 · REC-206's label (REC-206 @ 17ed9704), CONFIRMED as built.** "Graded inferred" in the 2026-09-23 23:30Z ruling means the §8.1 grade C, the correspondence-not-identity letter that is never presented as established, carried with `standing: inferred`, `established: false`, `work: machine` and `asserted_by: system`. No new grade value is minted: CPDF-10 forbids a third scale, and "inferred" is a standing, not a letter. The §16 limits REC-206 stated are its honest boundary: no position on tier-2 or tier-3 text, no stored membership, the Legistar item shape only, and a two-column agenda grouped wrongly. The last is a defect to row when a two-column agenda is held (a measurement first), not now. No row.
> - **2026-09-25 09:25Z · BOB #35 · D-665's gap (routing charts under titles needs a pixel read; M-182), RULED: NOT NOW, and stated.** The per-image `image_unread` marker already says what is true for every image, so nothing false stands. A pixel classifier would need a JPEG 2000 decode in a fleet member (37 of the 49 largest images) and spends build time on process-adjacent capability, while product rows wait (Bob, 2026-09-22: product first). So routing for the chart class stays OFF, and Content Framework §16 states it as a known gap with M-182's figures: neither measured signal separates, and the separating signal needs pixel decoding. REOPEN when a member-facing need arrives (a search or a citation asks for a chart's text and meets `image_unread`); the first measurement then is whether OCR over ALL marked images above a size floor is cheaper than classifying them. No row.
> - **2026-09-25 09:30Z · BOB #35 · SCHEDULER #23's three questions (09:16Z), RULED.** **(1) DISCLOSURE: FILTER BY THE VIEWER; it is not intended.** A capture sha or page address belonging to a bundle the caller cannot read discloses that the bundle exists and what it holds, the same leak the lead rule closes (MEMBER-KNOWLEDGE-DESIGN §5: a caller who cannot see receives exactly the answer a nonexistent row would give). D-340's `navchanges` and op=links pass every row through the viewer predicate BEFORE grouping or counting, and a count never includes a row the viewer cannot see. PLACE one D row (owner RECORD; NEGATIVE CONTROL: a member outside a gated bundle's project calls both ops and must see neither its shas nor its addresses, nor any count that moves with them). **(2) CHROME FOR LINKS: containment AND recurrence.** Site chrome is what RECURS across the site's pages in a chrome region. A page-local sidebar is page content, and its links are content links, so a varying sidebar is not a lost chrome link. Where recurrence cannot be measured (only one page of the site is held), the link reads chrome UNDETERMINED, never a loss marked same_page:false. PLACE one D row against LINK-FIDELITY §Chrome and D-340's judge (owner CAPTURE). **(3) REC-206's grade: already CONFIRMED at 09:20Z** (grade C with standing inferred; no new grade, per CPDF-10). D-672 (named ranges) is NOTED as undesigned; no row until a search needs it.
>
>   — drained 2026-09-25 by SCHEDULER #23: (09:15Z) no row, the CLIENT-RENDERED / OBSERVATION-LOG §4.1 fold rides the next landing touching either (CONDUCT told); (09:20Z) no row; (09:25Z) no row, §16 states the gap (CONDUCT told); (09:30Z) D-701 placed at the backlog head, D-702 after it.
...
> - **2026-09-25 09:55Z · BOB #35 · D-673's fence-day reading, CONFIRMED as built.** My 08:00Z "at or before the fence" is CORRECTED to "BEFORE the fence", on the census's one predicate (`stateMoveBeforeFence`): a move dated on the fence's own day cannot be placed on either side of it, and D-546's census already says exactly that. The gate must never speak "before <fence>" of a move the census will not place before it, so a fence-day move does not corroborate and C-4.2 keeps the error. One day tighter, and true. The stated residue (the corroboration's date is `manifest.created`, D-674's subject) stands as stated. No row.
> - **2026-09-25 10:05Z · BOB #35 · D-693's no-locator capture (SCHEDULER #23 10:00Z), RULED: STATE IT ON EVERY ENTRY.** CLAUDE.md §4 makes undetermined first-class and STATED. An entry with no `fetch` key is silent, and a reader cannot tell "the route was measured and is absent" from "nobody measured it". So every registry entry whose capture has no recorded fetch route carries `fetch: { route: "unrecorded" }` (the same naming D-693 gives the partial case), and its capture grade reads as the authored letter under the ceiling, never as a measured letter. PLACE one D row with an I5 IC (additive key; owner RECORD). The ~30 suites that pin whole entries are CORRECTED, not exempted, each with the comment that the old pin encoded a silence. NEGATIVE CONTROL: drop the key on a no-locator fixture, and the new arm fails by name. Fold into Content Framework §14.2 with the landing.
>
>   — drained 2026-09-25 by SCHEDULER #23: (09:35Z) D-710 placed after D-697, stacked on D-686; (09:55Z) no row, consistent with D-673 (running); (10:05Z) D-709 placed after D-698.
...
> - **2026-09-25 10:26Z · BOB #36 · UI-118 (Publication §6A.3, point 3(a)): the review copy's tie sentence that names a RECIPIENT — KEEP THE REFUSAL (option 1); no row now.** D-573's `last_change.stated`, when the tie involves a recipient's comment, names that recipient ("a comment by <by>"), and §6A.3's 3(a) keeps recipients' names and words out of the exported file. So in that one case the surface makes NO FILE and says why, as UI-118's worker built it provisionally. Rejected: (2) carrying the name, which breaks 3(a); (3) an export-scoped, name-free tie sentence from the plane is the right fix IF the case is ever measured on a real record. It needs a pre-D-543 whole-second acknowledgement in the same second as a recipient comment, which is historical and rare, so nothing is minted until a record shows it. SCHEDULER: no placement; UI-118 integrates as built.
>
>   — drained 2026-09-25 by SCHEDULER #23: UI-118 integrates as built; nothing placed.

>   — CORRECTION 2026-09-25 by SCHEDULER #23: the 10:12Z entry above was drained with the wrong footer; its act is UI-122, placed BLOCKED after D-696 (UI-121 builds part (2) only).

> - **2026-09-25 10:50Z · BOB #36 · D-706's WRITE HALF (may an outsider's op=linkproject write an edge to or from a bundle they cannot see): NO. The existing doctrine already settles it, so no new design is needed. Row it as the write half, stacked on D-706.** Home: `BIO_Membership_Architecture_v2.md` §7.9. Two of its rules apply. (i) "Not its existence" holds at the ACTS (REC-138): every act naming a project answers a caller who cannot see it byte for byte as for an id naming nothing, and sight is asked BEFORE position. (ii) Every act that changes a project's citation edges needs a JOINED participant (REC-134, `Store#projectAuthority`: `cite`, `sever`, `reinstate`). `linkproject` sits in the same `EDGE_ACTIONS` set in index.mjs, but REC-134's per-act table never reached it, and that is the defect. The fix names three acts. (1) A SOURCE capture held only under a bundle the caller cannot see gets the not-held answer and has nothing written (D-706's disclosure half already refuses it). (2) A source bundle the caller CAN see but has not JOINED is refused by §7.9's positional test, as `cite` is. (3) A resolved link whose TARGET the caller cannot see writes NO edge and is counted exactly as an unresolved link. A later `linkproject` by a member who sees both ends writes it then, so nothing is lost; writing it now would put a hidden bundle's id into a visible bundle's edges. accepts-when: an outsider's `linkproject` on a hidden source, and on a visible source linking to a hidden target, leaves `links_to` byte-identical, the witness read before and after; and a joined member who sees both ends still projects the edge. NEGATIVE CONTROL: take `linkproject` out of the position check, and the non-joined arm fails by name. The builder moves §7.9's REC-134 act list to include `linkproject`. SCHEDULER: mint this and place it directly behind D-706, or widen D-706 if its worker has not yet pushed (CONDUCT's call; if D-706 is already past that point, this is an act owed at integration).
>
>   — drained 2026-09-25 by SCHEDULER #24: minted D-722 (the write half), placed at the backlog head directly behind D-706; its row carries the 10:58Z text.

> - **2026-09-25 10:58Z · BOB #36 · D-706's WRITE HALF — CORRECTION of my 10:50Z entry, which SUPERSEDES its parts (2) and (3); part (1) stands.** D-701's worker's provisional, which BOB #35 recommended, reached me after 10:50Z. I adopt it, because on reading D-701 it is the better rule. (1) STANDS: a SOURCE capture the caller cannot see gets the not-held answer and has nothing written, so no one acts through what they cannot see (§7.9, REC-138). (3) REVISED to option (a): a resolved link from a visible source to a TARGET the caller cannot see is WRITTEN WHOLE, because a `links_to` edge asserted_by: source is a fact about the source's bytes, not the actor's choice. The ANSWER never names the hidden target, its sha or its project, and never counts it: it reads exactly as an unresolved link would. Refusing the write would buy no secrecy, because a joined member of the hidden project writes the same edge legitimately, so every read that serves edges must filter by the viewer anyway, and D-701 built that filter for op=links and navchanges. It would also make the record depend on who happened to run the op. (2) NARROWED: §7.9's JOINED test (REC-134, as `cite`) applies only where the source bundle belongs to a PROJECT. A `links_to` edge on shared Information changes no project, so the `contribute` capability gate governs it as today. accepts-when: an outsider's linkproject on a hidden source writes nothing and answers not-held; on a shared source linking into a hidden project it writes the edge, and its answer and counts are byte-identical to an unresolved link; and a later op=links read by that outsider shows no hidden target (D-701's fence). NEGATIVE CONTROL: drop the viewer stamp, and the outsider arm names the hidden project by name. Residue for the builder to STATE, not decide: whether any OTHER read serves `links_to` edges without D-701's filter (sweep the edge-reading ops).
>
>   — drained 2026-09-25 by SCHEDULER #24: SUPERSEDES the 10:50Z entry's parts (2) and (3); D-722 is scoped to this text.

> - **2026-09-25 11:00Z · BOB #36 · D-701's stated residue ("a capture acquired for a project before its promote has no register row, so every member sees it"): NOT A DEFECT; no row.** Verified at the code: `op=acquire` (index.mjs, `op === "acquire"`) takes no project and records none, so an unfiled capture carries no tie to any project. Under Membership v2 §7.9's containment it is Information: shared raw material that projects CITE. It becomes gated when a bundle files it, which is D-701's `#captureGate` over `register`. Seeing it tells a member nothing about any project. Re-open only if an acquire path ever records a project before promote. SCHEDULER's crossed question on D-706's write half is answered by the 10:58Z correction above.
>
>   — drained 2026-09-25 by SCHEDULER #24: not a defect; nothing placed.

> - **2026-09-25 11:05Z · BOB #36 · D-710's DESIGN GAP: a page two parts share (D-635: the folio came from the text layer, the OCR transcription was appended) reads `mixed`. Mint a D row behind D-710 (owner CONTENT).** BOB #35's 09:35Z rule is general: a unit reads the single kind when every derivation step covering it is one kind, and `mixed` when they differ. A D-635 page is covered by two steps of different kinds, so it is `mixed`. D-686's 09:05Z page rule ("the part appended last") predates D-635's append and is SUPERSEDED for this case only; a page read one way keeps its one kind. Reading `ocr` misstates the text-layer part as machine-read: DEC-4 is satisfied, but the record claims something other than what happened, and "less narrative" binds us first. Scope: `textchain.mjs` chainKindFor's page branch; correct content-chain-kind.test.mjs 1b, 2b and 3 with a comment saying why the old assertion was wrong; move construct 4.unit-chain-kind-mixed to BUILT; fold the rule into the Content Framework section that carries chain_kind. NEGATIVE CONTROL: restore the last-appended rule, and the shared-page arm fails by name. depends-on: D-710 (stacked on land/worker/D-710 @ 59f3e896). D-710's SECOND decision is CONFIRMED as recorded: `content:ocr` stays an equality and never names a `mixed` unit; `content:mixed` names it; capture_text.chain_kind stays the chain's LAST step (DOCUMENT_CHAIN_END) and never reads `mixed`.
>
>   — drained 2026-09-25 by SCHEDULER #24: minted D-723, placed behind D-722 at the backlog head, depending on D-710.

> - **2026-09-25 11:15Z · BOB #36 · D-706's WRITE HALF — FINAL RULING: (A)+(C). This SUPERSEDES my 10:58Z correction's part (3); its parts (1) and (2) stand. I have now changed this ruling twice, and the reason is stated here.** At 10:58Z I adopted "write a hidden target's edge whole, filter only the answer", on the argument that refusing buys no secrecy and makes edges depend on the actor. D-706's worker (session_01Xmzc5tFzkKQyzX48FYSCaw, land/worker/D-706; LINK-FIDELITY.md §Open questions) then MEASURED two leaks that argument missed. (i) THE TALLY RESIDUE: when an address has both a hidden and a visible direct capture, the unfiltered resolution writes the edge to the HIDDEN bundle while the viewer's op=links names the visible one, so op=links' `tally.linked` against linkproject's counts leaves ONE link unaccounted for. That tells the outsider something they cannot see exists at an address they know. Filtering the answer cannot close it, because the leak is in WHAT WAS WRITTEN. (ii) THE UNATTRIBUTED WRITE: the edge sits in `refs` under the outsider's act with no author, so the hidden project's owner sees a links_to created by someone who cannot see the project. §7.9 (REC-138) holds "not its existence" at the ACTS: an act whose effect depends on a hidden bundle's existence is exactly what it forbids. The actor-dependence I weighed against it CLAIMS NOTHING FALSE: a `links_to` edge is a resolution of the source's URL to a bundle, never a claim of completeness, so a missing edge is one NOT YET PROJECTED. A member who sees the target writes it from the same capture. The rule, as built: (A) op=linkproject resolves ONCE, through the viewer's `#captureGate`, and writes exactly the edges its answer names; the unfiltered second resolution is REMOVED. (C) `bundle=` naming a bundle the caller cannot see is refused NO_SUCH_BUNDLE, byte for byte as for an id naming nothing. (1) stands: a hidden source gets the not-held answer and writes nothing. (2) stands: the JOINED test (REC-134, as `cite`) applies where the source bundle is a project's. accepts-when: dave's arms write nothing into carol's hidden project (the `refs` witness is byte-identical before and after); the hidden-plus-visible address writes the VISIBLE edge, and op=links' tally equals linkproject's counts; a member who sees both ends still writes the hidden target's edge. NEGATIVE CONTROL: restore the unfiltered write resolution, and the dave arm fails by name. Fold into LINK-FIDELITY.md (close the open question with this ruling) and move §7.9's REC-134 act list to include linkproject. D-701's note that edges "must not depend on who asked" is CORRECTED by this entry for WRITES: an edge set is a projection, not a completeness claim. Attribution of the projecting act (the worker's option B) is not needed under (A) and is NOT rowed.
>
>   — drained 2026-09-25 by SCHEDULER #24: D-722 rewritten to this ruling ((A)+(C), parts 1 and 2 standing); option B not rowed.

> - **2026-09-25 11:20Z · BOB #36 · D-724 (a partial capture index is not a prefix): OPTION (b), KEEP EVERY UNIT THAT FITS, WITH ONE CONDITION: the skipped units are NAMED, not only counted.** D-685's worker measured it (M-184, Z5): both budget loops (index.mjs textUnitsFor, store.mjs #writeCaptureText) `continue` past a unit over the bound and write later units that fit. Option (a), `break` at the first over-bound unit, would throw away readable text only to protect a prefix property nothing reads: `rows=passage` carries `seq`, so a gap is visible. The doctrine that DOES bind is CLAUDE.md §2: saying WHICH absence is true is a first-class obligation. A member whose search finds nothing in sheet 4 must be able to learn that sheet 4 was NEVER INDEXED (over the bound), not that it holds no match. So: (1) correct CONTENT-SEARCH-DESIGN.md §4.3 and the #writeCaptureText docblock to say "every unit that fit, in reading order, with gaps"; (2) a `partial` capture records the unit keys it SKIPPED (not only a count), from both loops, and the read that reports a capture's indexing state (whatever carries `partial` today) serves them, stated as "not indexed: over the bound". accepts-when: Z5's workbook reads S5 indexed and names S4 as skipped over the bound; a capture under the bound names none. NEGATIVE CONTROL: drop the skipped-key write, and the Z5 arm fails naming S4. Owner RECORD; SCHEDULER places it as an ordinary row.
>
>   — drained 2026-09-25 by SCHEDULER #24: D-724 placed behind D-685, ahead of D-694.
...
>
>   — drained 2026-09-25 by SCHEDULER #24: D-720 placed first queued in the cache, stacked on D-708.
...
>
>   — drained 2026-09-25 by SCHEDULER #24: (a) rides in D-712 (its worker told by BOB; D-712 re-tips); (b) minted D-734, placed after D-712.
...
>
>   — drained 2026-09-25 by SCHEDULER #24: no row; REC-23 stands for every arm D-717 did not move.

> - **2026-09-25 12:35Z · BOB #36 · D-720's residue (a reading given on a PAIR draft before D-720 landed stays at C1's key, undetermined which): MINT ONE SMALL MEASUREMENT ROW, placed LOW (after the product rows at the backlog head).** Why it is worth measuring: pair drafts are possible on the live build. The released 0.79.0 source (release/RELEASE.json's commit dd324152) already carries `newCase` and CASE_IDENTITY_AMBIGUOUS, so such a reading can exist, and C1's NEXT signed edition would list it. That is a binding nobody made, in signed bytes (Publication §3 rule 13). The count is cheap and read-only. Scope: on each live instance DIST can reach (biosmoke7; name any other), count statement_acknowledgements whose draft CURRENTLY names a case AND sets newCase, and those whose draft's history shows it did at the ack's time where the history records it. Report each count as a number or UNDETERMINED with its reason, in measurements/M-<n>.md. Read-only: no write, the record's counters witnessed before and after. If the count is 0 on every instance, close. If it is >0, the row NAMES the rows, and the fix (re-keying them to the draft, D-720's rule applied retroactively) comes to BOB before any write. Owner RECORD, instrument by DIST or a worker with DIST's reach.
>
>   — drained 2026-09-25 by SCHEDULER #24: minted D-742, placed low after the product rows at the backlog head.

> - **2026-09-25 12:40Z · BOB #36 · D-707's finding (op=promote answers GOVERNING_LAWS_REWRITTEN to an action revision with no READABLE bundle.md front matter): (a), WITH (b)'s PRINCIPLE AS THE BACKSTOP.** A refusal names its TRUE cause. Here nothing readable was sent, so "the laws were rewritten" is a false statement about what the member did. The root is `lawsOf` returning the string "unreadable" as a VALUE and the fence comparing it with the head's value, which is an equality that costs nothing to produce (CLAUDE.md §5). (a) promote judges READABILITY before ANY content fence. A request with no files, no bundle.md, a blob-held bundle.md or no front-matter block is refused NO_BUNDLE_MD (or a front-matter-unreadable refusal, if NO_BUNDLE_MD cannot honestly name all four spellings; the builder states which), for an action exactly as for a non-action. (b) as the backstop: `lawsOf` answers UNDETERMINED, not a value, when it cannot read, and the laws fence ABSTAINS on undetermined; it never refuses and never passes on it. So no future reordering can bring the misattribution back. accepts-when: each of the four measured spellings, sent as an action revision, gets the readability refusal and never GOVERNING_LAWS_REWRITTEN; a readable revision that does rewrite the laws still gets GOVERNING_LAWS_REWRITTEN. NEGATIVE CONTROL: move the readability check back behind the fence, and the four arms fail naming GOVERNING_LAWS_REWRITTEN. Owner RECORD, placed behind D-726 on the same function, as SCHEDULER proposes.
>
>   — drained 2026-09-25 by SCHEDULER #24: minted D-741, placed behind D-726.

> - **2026-09-25 12:45Z · BOB #36 · per-link recurrence on op=links (after D-702 and D-729): NO ROW. The surface stays at D-729's truthful containment label.** LINK-FIDELITY §Chrome records CONTAINMENT on the link: a fact about one capture's bytes. RECURRENCE ("this link is site chrome") is a judgement ACROSS captures, and it depends on which captures were compared. It lives at the navchanges read (D-340, judged by D-702; not yet on main), where the comparison set is explicit. Stamping it per link on op=links would carry a cross-capture verdict into a single-capture answer without naming its basis, which is the record saying more than the one capture supports. The member loses nothing: "in a page-furniture region (<basis>)" is true, and the navchanges read answers the recurrence question where it is asked. Re-open only for a member-facing need that cannot be met at navchanges (for example, a search filter excluding chrome), and then the verdict carries its comparison set.
>
>   — drained 2026-09-25 by SCHEDULER #24: no row; the surface stays at D-729's truthful containment label.

> - **2026-09-25 12:50Z · BOB #36 · BOB'S DIRECTION (Bob, in BOB's session, 2026-09-25 ~12:50Z): "Stop spawning new worker sessions and focus on batching up the changes for merging." This SUPERSEDES "16 workers working" (2026-09-24) until Bob says otherwise.** SCHEDULER: spawn NO new worker session and fill no slot, including rows already placed and runnable. Keep draining the inbox, minting and placing rows, and flipping reports of workers ALREADY running; a running worker is not stopped and finishes its row. CONDUCT: integration is the priority. Compose every finished and ready branch into batches (batch30 now, then the next), pay each union's IC, figures and docs passes, gate, and train when green. The standing holds are unchanged: releases are HELD; UI-121 rides only with D-712 carrying D-731(a); D-340 rides only with D-701. BOB's slot probe stops asking for fills.
>
