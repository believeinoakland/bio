...
> ### M0-118 · queued — **SIX LIVE-VERIFICATION HELPERS READ THE KEYS ONLY FROM A `.env` FILE, SO IN THE CLOUD, WHERE THE KEYS ARE ENVIRONMENT VARIABLES AND NO `.env` EXISTS, EACH FAILS BEFORE ITS FIRST CALL.** `vf4-call.mjs` (and through its `loadEnv` `vf4-live-scratch.mjs`, `vf4-suggestprobe.mjs`, `rec88-instance-census.mjs`), `vf4-bindings.mjs`, `vf4-secretnames.mjs`, `ocr-composed-probe.mjs`, `ocr-moondream-probe.mjs`, `cpdf15-tesseract-runtime.probe.mjs` (`bio-plane/test/`; none reads `process.env`, re-read on `17c0cab1`). — owner M0.
...
> milestone: M0
> interface: none
> design: `docs/development/VERIFICATION.md` (admitted for M0 by name), with `CLAUDE.md` §8: secrets are the environment's variables in the cloud (Bob's option C, 2026-09-22).
> depends-on: none — `loadEnv()` in `fl1-billing-surface-check.mjs` reads `process.env` first and a `.env` found upward second.
> scope: each helper imports that one loader and drops its own reader; no value is ever printed.
...
> ### D-147 · queued — **A RECORDS REQUEST IS ONE ROUND TRIP: `awaiting_response` HIDES THE FEE ESTIMATE, THE WAIVER DECISION, A PARTIAL PRODUCTION AND THE APPEAL, EACH A FACT ABOUT THE BODY.** `action` runs planned, active, awaiting_response, resolved or abandoned. — owner RECORD.
...
> milestone: M10
> interface: I3 and I5 — correspondence entry kinds, a closed outcome vocabulary and a stated due date; the integrator mints and classifies the ICs.
...
> scope: each stage after the request is its own correspondence entry naming the entry it answers or follows, in §2's `sent` and `received` kinds; a decision carries its outcome in the closed vocabulary (granted, denied, partial, reversed, affirmed, none stated); a member may state a next-stage due date with its citation; the plane derives only elapsed time and a stated date passed with no answering entry. No law's clock is encoded.
> accepts-when: a request, a fee estimate, a waiver decision, a partial production and an appeal read back as one dated chain; an entry with no stated due date reads UNDETERMINED; a stated date passed with no answer is derived and no law is encoded. NEGATIVE CONTROL: default a due date from the action's kind, and the undetermined arm fails by name.
...
> ### D-128 · queued — **`op=progressiondefine` UPSERTS, SO A GROUP'S EARLIER DECLARED FLOW DISAPPEARS SILENTLY AND A FINDING READ AGAINST IT LOSES ITS BASIS.** `progression_defs` is written `ON CONFLICT … DO UPDATE` and the stages are deleted and rewritten (`store.mjs`, re-read on `17c0cab1`). — owner FRAMEWORK.
...
> milestone: M4
> interface: I3 and I5 — a definition's versions, and the version an instance read and a finding name; the integrator mints and classifies the ICs.
...
> scope: a definition is APPEND-ONLY: a revision writes a new version carrying its author, date and basis (the member's statement and a citation); the prior version stands and reads back; an instance read and each finding name the version they were read against. The out-of-order shape and institution scoping stay deferred with §8.2's trigger.
> accepts-when: a revised definition leaves the prior version readable with its basis, and an instance read or a finding names the version it was read against. How a liar passes it: a history table no read consults, so the finding arm reads the version through the op. NEGATIVE CONTROL: restore the UPSERT, and the prior-version arm fails by name.
...
> ### D-420 · queued — **AN `image {page, rect}` ROW IS BOUNDED BY THE PAGE SET AND NOT BY WHAT THE PAGE PAINTS.** A rectangle where a PDF page paints no image mints as an `image` row: acquire persists no image list for a PDF (its `container_extent` is null, `index.mjs`), and `coversImage` has no `{page, rect}` branch; only the crop refuses it, `NO_IMAGE_AT_RECT` (`pdf-worker/src/imagecrop.mjs`) (re-read at the code 2026-09-21). — owner CAPTURE, with RECORD.
...
> ### D-85 · queued — **AN ASSISTANT CAN OPEN A QUESTION OUTSIDE ANY RUN IT HOLDS, AND A RUN KEEPS ONLY THE LENS IT WAS HANDED.** NARROWED by BOB #25 to §11 item 5's rules 2 and 3: no row links an inquiry an `ai` credential creates to a run, and `aiRunOpen` stores the manifest it is handed, so `moved` cannot tell a changed lens from a stale one (re-read 2026-09-21). — owner RECORD.
...
> scope: rule 2: an `ai` credential's inquiry names a running run whose principal it is; the link is an instance row keyed by the inquiry, cleared by both `purge` arms and never in signed bytes, counted against a declared `surfaces` bound; `op=projection` states its run and lens, `not recorded` for older ones. Rule 3: `aiRunOpen` also stores the plane-computed `statements_sha`, and the run's read tells `moved` from a stale hand.
> accepts-when: a creation outside a run, in another principal's run or past the bound is refused by name; one inside lands with its row and reads the lens and `moved`; a member's is untouched; a stale-handed run reads stale, not `moved`. NEGATIVE CONTROL: drop the run check, and the outside-a-run arm fails by name.
