# DECISIONS — answered and enacted entries, rolled 2026-08-10

Rolled out of `docs/development/DECISIONS.md` by session BOB on the pattern the
2026-08-10 consolidation proved on CLAIMS, QUEUE and DEBT: the live file keeps its
rules, its open and deferred entries, every entry whose enactment CONDUCT still owes,
and the byte-read entries — DEC-39 (`affordances.test.mjs`), DEC-32/DEC-33
(`civicos-ui/test/analyst-vocabulary.mjs`); everything settled
moves here UNEDITED. `tools/decided.mjs` and `tools/mintid.mjs` both scan
`docs/archive/**`, so every ruling stays indexed and no id floor falls.
...
response: **SWEEP IT.** Bob, 2026-08-10, same day, reaffirming the original instruction
  after the recommendation to keep the advice: "Again any references to Google Drive and
  App Scripts are old and outdated. Neither Google Drive nor App Scripts are a part of the
  current architecture. Period."
decided: 2026-08-10 · Bob
enacted: 2026-08-10 · session BOB, same turn — `BIO_Communications_Platforms.md`'s
  Function 2 recommendation, comparison row, resilience path and onramps;
  `BIO_Complete_Roadmap_v5.md` §Function 2; `BIO_Design_Requirements_v2.md` R9 — all
  restated vendor-neutrally (an accessible mainstream shared-folder service), with R9's
  multiple-platforms doctrine and every limitation warning intact. The ledgers' dated
  rows, measurement literals and the archive remain the licensed exceptions, as records
