...
| FL-4 | **Scheduler consumer.** ONE appended entry to `#schedConsumers` (`{name, due, wake, tick}`) waking suspended runs on daemon completion and on resumption ticks. **No second alarm and no cron** — `SCHEDULER.md`'s recorded decision. **TWO FIGURES IN THIS ROW WERE STALE AND ARE CORRECTED FROM MEASUREMENT (2026-08-09, FL-4):** the registry is at `store.mjs:1778` and not `:1452` (`store.mjs` is 23,971 lines, not the ~16,300 this plan was written against), and the entry is the **TENTH** consumer and not the eighth — PL-5's `ai-run-reap` was the eighth and PL-4's `capture-request-drain` the ninth, both landed after this row was written. | I5 | PL-5 | `node scripts/battery.mjs scheduler` green: the **tenth** consumer registered; a run suspended on a capture request wakes when the drain completes; idle instance carries no timer | add the run's own alarm or cron → the one-alarm assertion in the scheduler suite fails; starve a slow consumer → the reconcile-keeps-every-wake arm fails |
...
| FL-6 | **The Claude-account cascade at runtime.** Resolve member → project → instance; the run records WHICH level paid beside the plane-credential principal (two principals, both named, PL-4's rule); when no token resolves the capability is UNAVAILABLE and says so — never a silent no-op. DEC-43 read before any fallback-instance deployment (§14a). **LANDED 2026-09-12 (FLEET area session): `agent-worker/src/cascade.mjs` is the one expression of the order and the per-level judgement, resolved IN THE FLEET MEMBER because the plane's airunopen stamp says the plane learns the level by being told; material arrives per call beside the `ai` credential and is retained the same way (not at all); a revoked-by-publication value falls through BY NAME (the plane's one denylist, imported as the member's first cross-tree build input — the FL-9 guard hashes it); the endpoint refuses when supplied material resolves nothing (`NO_ACCOUNT_RESOLVED`, per-level absences stated) AND when the record's payer disagrees with the runtime's resolution (`RUN_NAMES_A_DIFFERENT_PAYER`, both facts named); a caller offering no material is a THIRD stated fact, not conflated with an exhausted cascade. `battery.mjs cascade` green 29/0; NC four arms measured (26/3, 20/9, 26/3, baseline 29/0). DEC-43 read and not triggered — nothing deployed. Model TURNS still do not run (turns_run: 0, honest on the wire) — that is D-218's sizing, deliberately not this row's.** | none | FL-2, DS-3 | `node scripts/battery.mjs cascade` green: three fixtures resolve to the right level and the run object names it; the no-token fixture answers a stated absence | remove every token → assert the response is the named UNAVAILABLE, not an empty success (an honest absence, stated — the silent no-op is indistinguishable from a run that found nothing) |
