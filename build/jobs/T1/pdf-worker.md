...
- **N9 · applied.** Only `pdf-reader`'s named services: `loadPdf` is `openPdf` (the `__objnum` stamping it did on every dictionary is gone; nothing read it); pages come from `pageCount` and `pageDict`; a placement's stream and CTM from `imagePlacementSource`, in `imageOf` and in `cropImage`'s `upright` rule. No `.objects`, `._pageOrder`, `_stream` or `_ctm` is read anywhere in the module. This also restores `cropImage`, which after `pdf-reader`'s change refused every crop `INLINE_IMAGE` (`pdf-reader`'s report).
...
- `bio-plane/test/fleetbundles.test.mjs` — 84 pass, 12 fail: all staleness of the `pdf-worker`, `ocr-worker` and `bio-plane` bundles against `pdf-reader`'s and this job's source; expected, BOB regenerates them at the layer close
- `checks/format.mjs` — 61 modules, 19 requirements files; 0 failures
- `checks/architecture.mjs … pdf-worker` — 26 product files, 25 relative imports (0 naming no tracked file, not judged); 0 failures
- `checks/coverage.mjs … pdf-worker` — 1 modules, 40 of 40 live requirement ids named by a test; 0 failures
- `checks/ownership.mjs … pdf-worker tranche/T1` — 13 files changed by pdf-worker between tranche/T1 and HEAD; 0 failures
