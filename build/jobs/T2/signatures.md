...
**Status** · COMPLETE, 2026-09-26, after Q1's answer (K37): `origin/tranche/T2` @ 1626717669 merged, R30–R32 applied, steps 5–7 re-run. Job for module `signatures`, tranche T2, branch `job/T2/signatures`. Entries: T2-5, N7 (K33). No open question; waits on nothing.

## Questions to BOB

**Q1 (2026-09-26), two parts. ANSWERED by K37: R30, R31 and R32 added, as recommended; applied below.**
...
- **T2-5** · Requirement-named tests for every live id. `bio-plane/test/m/signatures/signatures.test.mjs` (42 tests, `node:test`) names R1–R32 in test titles and checks each at the interface. Signatures come from three independent sources: stock `ssh-keygen` (fresh keys, sha512 and `-O hashalg=sha256`, plus the pinned OpenSSH 9.6p1 fixture), a signer written in the test from PROTOCOL.sshsig (which can set any blob field wrong, for every `MALFORMED` case), and the served page's own script. R15 compares `timestampRequest` byte for byte with 40 `openssl ts -query -sha256 -cert` queries (nonce taken from each) and with hand-built DER for leading-zero and high-bit nonces. R17 parses a real reply from a throwaway `openssl ts -reply` TSA. R26 runs every service in a child process whose `fetch`, `Date`, `performance.now` and timers are traps. Cases needing `ssh-keygen` or `openssl` skip by name when the tool is absent.
- **N7** (K33) · The page's source is now `bio-plane/src/sign-release.html` (a byte copy of `tools/sign-release.html`, which is left in place for BOB to remove at the layer close). `embed-signpage.mjs` renders from it (`SIGNPAGE_SRC`), its generated header names the new source, and `src/signpage.mjs` is re-rendered (content unchanged, header line only). The module's tests now check that the served `SIGN_HTML` is the current render of the source and that the served page's signatures verify under `verifySshsig` and `ssh-keygen -Y verify` (R30, R31 after K37).
- **K37** · R30 and R31: the render and conformance tests are retitled under them. R32: the page's visible text now names CivicOS (`<title>`, `<h1>`, the development-key note, the not-a-key error, and the header comment's "no CivicOS code"); the `BIOKEY-RAW1.`/`BIOKEY1.` prefixes, the `bio-release`/`bio-ratify` namespaces and `bio-signing-keys.txt` are unchanged. `src/signpage.mjs` re-rendered. A new R32 test runs the served page and checks both halves.
...
## Tests and checks run (job/T2/signatures after the K37 merge, at the commit before this record)

- `node --test bio-plane/test/m/signatures/` — `tests 42, pass 42, fail 0, skipped 0` (5 runs in the job: the first found one test bug, a stub-DOM lookup; one run was a negative control against the unchanged module code, which failed 8 tests exactly on the flaws fixed above: R4, R18, R19, R20, R23, R24, R26, R29).
- Tests of the modules that use this one (legacy battery): `sshsig 18/0`, `signpage 35/0` (re-run after R32: it still reads `tools/sign-release.html`), `attest 48/0`, `machine-attest 36/0`, `operator-attest 18/0`, `ratify-authority 52/0`, `casesign 77/0`, `case-authority 26/0`, `signer-enrolment 32/0`, `d530-parted-attest 28/0`, `group-public 26/0`, `deliverer 20/0`, `d84-case-manifest 44/0`, `rec219-case-document-v4 39/0`, `casesearched 26/0`, `machine-fences 88/0`; `fleetbundles 92/4` (re-run after R32: the same four plane arms; its render arm passes) and `newgroup-bundle-fresh 3/1`, both the stale bundles reported above (baseline 96/0 and 4/0).
...
- `node checks/coverage.mjs … signatures` — `coverage: 1 modules, 32 of 32 live requirement ids named by a test; 0 failures`
...
session_01GJC6ytwQrBRdr2Y8KdcybF,job,signatures,7008606,147110,100,55186,50,5,1023
```
