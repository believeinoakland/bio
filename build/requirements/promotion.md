...
- **R18** Every refusal the catalogue assigns to the promote act (its row names this write path as its site) is enforced at the write, not only by the audit. Tested against the catalogue as it stands; the records-law arm C-2.10 (D-695) and the five action-catalogue arms (D-717) arrive with `actions`' extraction (REC-201, D-689) and are enforced then through R39 (K64).
