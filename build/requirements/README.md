...
5. **Ids are permanent once Bob approves the file.** A retired id is marked retired and never reused. Before approval, a draft may renumber.
6. **No jurisdiction (`layers.md`, "No jurisdiction in the product").** Local facts come from a jurisdiction profile. Only the `jurisdictions` module's profile data names a place.
7. **Satisfies cites the canon by section** (`requirements/README.md`), never by line.
