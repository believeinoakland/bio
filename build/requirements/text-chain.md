...
Not yet met: R42 (D-633, `mergeTier2Text` does not carry a base page's `image_content_*` markers
onto a page tier 2 wins), R52 (D-723, no service yet answers a shared page `mixed`), R32
(D-416, `readingPositionInExtent` has no `sheet-range` extent arm). No local fact: this module names
no place and holds no jurisdiction data, so `layers.md`'s rule needs no plan entry against it.
...
- **R1** Each entry has `role` (`"derivation"` or `"verification"`) and `label` (a human phrase). A
  `"derivation"` step produced or changed the text; a `"verification"` step (`attested`) checked
  text already there and never lowers or raises a derivation cap by itself.
...
- **R10** A step of a kind whose `letter` is `"calibrated"` (R3) carrying `.cap` with no
  `.calibration` refuses `TEXT_CHAIN_LETTER_UNCALIBRATED` (C-35.13).
...
- **R13** For a `step` whose kind's `role` is `"derivation"`: when `step.cap` and the chain's current
  `derivationCap(chain)` are both known grade letters and `step.cap` is STRONGER (a lower `BASIS_GRADES`
  index) than the chain's, refuses `TEXT_CHAIN_STRENGTHENS` (C-35.6). Otherwise appends.
...
- **R27** With `target.page` given, only steps whose extent (R22's shape, or absent = "all") covers
  that page bound the answer; the weakest of their measured caps, `null` when none of them measures one.
- **R28** A MIXED document (derivation steps carry different page-list extents): with no `target`, the
  ...
  OCR pass's letter).
- **R29** A step of a kind whose `unmeasured` is `"undetermined"` (R3: `convert`) that carries no `.cap`
  and covers the queried target (or the whole document with no `target`) makes the answer `null` — an
  unmeasured conversion is not "sequence-neutral" the way an ordinary unmeasured step is, because every
  step after it measured the CONVERTED bytes.
...
- **R32.1** The `.step` of the chain's LAST entry; `null` for a malformed chain. Always document-level:
  it never distinguishes a page covered by two different step kinds from one covered by a single kind
  (see R52, not yet met, for that question).
- Errors: never throws.
...
- **R36** Tier entries are returned in the ORDER the steps that created them first appeared in `chain`,
  never sorted by tier number.
...
- **R42.1** An object whose `.basis` is not `"engine"` or `"none"` refuses `TEXT_CONFIDENCE_PSEUDO`
...
- **R44** A region whose `checkConfidence` refuses, or whose `.confidence` is `{value, basis:"engine"}`
  with `value < floor` (only when `floor` is a number), is replaced: `text: null`, `undetermined: true`,
  `why` (the refusal's detail, or the shortfall sentence), `confidence: "none"`, every OTHER field kept.
...
**`extentCovers(extent, target) → boolean`** Does an attestation's extent cover `target = {page, rect?}`?
- **R52.1** `extent.kind === "document"` → `true` always.
- **R52.2** `extent.kind === "page"` → `true` only when `extent.page === target.page`.
- **R52.3** `extent.kind === "region"` → `true` only when `extent.source.page === target.page` and
...
- **R53** Every other case — a malformed `extent` or `target`, an unrecognised `.kind`, a missing
...
- **R54** Filters `attestations` to those `checkAttestation` accepts AND whose `.extent` `extentCovers`
  ...
  `target` may claim.
- **R55** Otherwise: `{ceiling: derivationCap(chain, target), determinant:"derivation", by: [], why:
...
- **R56** `!isTranscribed(chain)` (including a malformed or `null`/absent chain) → returns `byteGrade`
  unchanged.
- **R57** Transcribed with `derivationCap(chain) === null` → returns `null` (an unmeasured engine's
  output never rides the byte grade through).
- **R58** Transcribed with a measured cap → returns `weaker(byteGrade, cap)` (never stronger than either).
- Errors: never throws.

**`readingSource(source) → {kind, ref, ...} | null`** Normalises a reader's element reference to IC-1's
canonical, key-ordered shape. TOTAL: every malformed or unrecognised input answers `null`, never a
refusal — a reading still writes with the position simply absent.
- **R59** `null` unless `source` is a plain object with `.kind` one of `pdf-page`, `sheet-cell`,
  `slide-shape`, `doc-para`, and a non-empty string `.ref` (truncated to 200 characters). `dom` (IC-1's
  fifth arm) and any other kind answer `null` — there is no producer for it yet.
- **R60** `pdf-page` needs `.page` a non-negative integer; `.rect` (4 finite numbers) carries through
  when present and valid, else `null` (the page alone still answers).
- **R61** `doc-para` needs `.para` a non-negative integer; `.run`, when a non-negative integer, carries
  through, else `null`.
- **R62** `sheet-cell` needs non-empty string `.sheet` and `.cell` (each truncated to 200/64 characters).
- **R63** `slide-shape` needs `.slide` and `.shape`, both non-negative integers.
- Errors: never throws; never refuses — malformed input is absence, not an error a caller must handle.

**`readingSourceJson(source) → string | null`** The canonical JSON of everything `readingSource` returns
except `kind` and `ref` (which are stored in their own columns).
- **R64** `null` when `readingSource(source)` is `null`; otherwise a `JSON.stringify` of the per-arm
...
- **R65** `` `${kind}:${readingSourceJson(source)}` `` when `readingSource(source)` normalises;
...
- **R66** `null` unless `posKind` and `posRef` are non-empty strings, `pos` is a string that
...
- **R67** `false` unless `readingSource(position)` normalises and `extentKind` is a non-empty string.
- **R68** `extentKind === "document"` → `true` always (Bob's 5.3: a document extent's portion is the
  whole document).
- **R69** Otherwise `extentKind` must equal the position's own `kind` (a coarser reading position is
...
- **R70** `0` for a non-string. Otherwise counts code points (a surrogate pair counts once) that do not
  match `\s` (Unicode-aware).
- Errors: never throws.
...
- **R71** When `base.pages` has no usable entries (no integer `.page`): if `base.document` is a string,
  refuses (`ok:false`) whenever it holds at least one glyph (`glyphCount`); if `base.document` is not a
  string, refuses whenever `base.counts.chars > 0`. The refusal's `why` names what was found and that a
  page-wise comparison could not be made.
- **R72** In that same no-usable-pages case, when nothing is held (R71's condition false), `t2` is
  adopted WHOLE: `{ok:true, text:t2, wholesale:true, perPageTier:null, replaced:<t2's page numbers>,
  kept:[]}`.
- **R73** Otherwise, for each of `base`'s usable pages, tier 2 wins that page — `{ok:true}`'s merged
  `pages[]` entry becomes `{page, text:<t2's text>, undetermined:<t2's markers>, tier:2}` — only when
  BOTH: tier 2's page has strictly FEWER undetermined characters (summed `.undetermined[].count`) than
  tier 1's AND strictly MORE glyphs (`glyphCount` of `.text`, R70) than tier 1's. Otherwise tier 1's
  page is kept, spread unchanged plus `tier:1`.
- **R74** [NOT MET — D-633] When tier 2 wins a page (R73), every field on the base page whose name
...
- **R75** On the page-wise path (R73), the result also carries `{ok:true, wholesale:false, text:{
...
**`tier2Note(m) → string | null`** A human sentence for `mergeTier2Text`'s result, for a document's notes.
- **R76** `m.why` (or `null`) when `!m.ok`. `null` when `m.wholesale`. `null` when nothing replaced.
- **R77** Otherwise a sentence naming how many pages tier 2 recovered and how many kept tier 1, plus a
  second clause noting the chain records the tier per page, when both `replaced` and `kept` are non-empty.
- Errors: never throws.

**[NOT MET — D-723] A page's chain kind, reading `mixed` when it should.** No exported service answers
this today; `terminalStep` (R32.1) is document-level and a caller (`store.mjs`) reads a page's kind as
...
`mixed`, and BOB #36 superseded D-686's page rule for this case (11:05Z, 2026-09-25). This module must
add a service that:
- **R52** Given a chain and a page, answers `mixed` when that page is covered by derivation steps of
  more than one kind (per R22's/`stepCovers`'s extent test), and answers the one kind covering it
  otherwise — `null`/undetermined when no derivation step covers the page or its extent is unreadable
  (R30's rule). `terminalStep` itself is unchanged: it stays document-level and never answers `mixed`.
...
- **R78** Pure: no store, no network, no clock. The same inputs always give the same answer.
- **R79** No function in this module throws. Every malformed input answers `null`, `false`, `[]`, or a
  refusal object — never an exception, and a bad extent or position is never read as "covers
  everything" or "the whole document" (R30, R52.3's `false` default, R53).
- **R80** No place is named in this module (`layers.md`, "No jurisdiction in the product"). It takes no
  jurisdiction profile and needs none: every space, grade, engine and extent it reasons about is a
  shape a caller supplies, not a fact this module holds.
- **R81** Rule 2 (derivation only weakens) is enforced by `appendStep` and `convertedChain` alone, by
  computing `derivationCap` and comparing ranks — never by trusting a caller's claim.
- **R82** A "no" always says which kind of no (`legacy-checks`' translations): outside a floor,
...
- **For R52's new service**: a plausible name is `chainKindFor(chain, page)`, read by `store.mjs` in
  place of its own `terminalStep(chain) || "layer"` line — not binding; the module is free to shape it
  differently as long as R52's outcome holds and `store.mjs`'s caller-side obligation (asking per page,
  not once per document) is met.
- The D-501/D-514 sweep found three OTHER readers outside this module comparing the same
  producer-reported character counts instead of glyphs (`needsTier2` and the tier-3 layer attribution in
  `index.mjs`); they are that module's requirements, not this one's, once it is written.
