...
**Status** · DRAFT by a drafting worker for BOB #42, 2026-09-26 (P18), from a reading of the code; for Bob's approval (a product module, P17). Layer 6. Code today (measured on `tranche/T3` @ `35ea098`, after promotion's merge; `build/extraction/inquiry.md` has the table): `bio-plane/src/store.mjs` 1872–1907 (the superseded-by index), 4288–4790 (sever, reinstate, dispose), 5198–5301 (`#restsOnLive`, `#retiredNotCitable`), 12199–13169 (`divide`, `groundInquiry`), 13389–14300 (splice helpers, `cite`), 15566–15636 and 15753–15902 (splice helpers), inside `#promoteChecks` 17551–17671, 17844–17899, 18037–18063 and `#promoteProjections` 18154–18351, 18618–18645, 18745–18760, 22686–22745 and 23166–23214 (leg content backfill), 26364–26744 (the earned registry), 31123–31191 (cycle guard, `basisFor`, `restingOn`), 31922–31995 (`#capturedAt`), 35552–35759 (`earnedBasis`), and the dispatch entries `basis`, `restson`, `earnedbasis`, `cite`, `sever`, `reinstate`, `dispose`, `inquirydivide`, `inquiryground`. `bio-plane/checks/bio-checks.mjs` 55–75, 409–434 (`STATES.inquiry`), 2330–2364, 2406–2448, 2615–2931, 3276–4177 (the leg grammar), 14177–14222 (`leadLegFindings`), and rows C-45.7–C-45.10, C-54.1, C-33.13, C-33.15–C-33.19, C-33.22, C-33.23, C-33.39, C-32.7, C-32.8. `schema.mjs`: `inquiry_basis`, `inquiry_exclusions`, `inquiry_migration_replays`. `from`: `legacy-store` and `legacy-checks` (K64's pattern); `index.mjs` holds only these ops' routing, gates and stamps, which stay with `control-plane` (K3). Not yet met: R19 (D-592), R22 (found by this reading), R26 (found by this reading), R34 (MK-5). Old-plan rows carried to `inquiry`: REC-202, D-572, D-592, MK-5; REC-202 and D-572 are proposed to other modules (map §5.4).

**Size (P6).** About 5,690 lines move (about 2,540 without comment-only and blank lines): `store.mjs` 4,115, `bio-checks.mjs` 1,390, `schema.mjs` 185. Past the 4,000 mark. Proposed split (map §5.1): a `citation` module, next after `inquiry` in layer 6, takes `cite`, `sever`, `reinstate` and their rows (R29–R31 here, about 1,300 lines), leaving `inquiry` about 4,390 (about 1,900 of code), which one session reads with its uses' public parts, as K74 accepted for `extraction`. This counts the earned registry here (map §5.2); in `strength` instead, `inquiry` is about 670 lines smaller.
...
**Citation** (`op=cite`, `op=sever`, `op=reinstate`; to `citation` if the split is taken)
- **R29** `cite`: the selection at weight `report`; an absent or invisible citing object is `NO_SUCH_PROJECT` (after `membership`'s existence answer); `NOT_A_PROJECT` for one neither a project nor an inquiry; a project needs the actor joined (`membership.projectAuthority`); `BAD_NOTE` (C-33.15); on an inquiry `NO_ROLE` (C-33.16) and `BAD_ROLE` (C-33.17), on a project `ROLE_NOT_APPLICABLE` (C-33.18); `NOT_CITABLE` (inquiry) or `NOT_INFORMATION` (project) when any member is neither information nor an inquiry; `RETIRED_NOT_CITABLE` (C-33.39); `SEVERED_EDGE` (C-33.19); the part: an unknown field `UNKNOWN_EXTENT_FIELD` (C-45.7), a part on a project `EXTENT_NOT_APPLICABLE` (C-45.8), a part with more than one new member `EXTENT_ON_MANY` (C-45.9), an unwritable value `BAD_EXTENT_VALUE` (C-45.10), then R5 (`BASIS_REFUSED`); `EMPTY_SELECTION`; past 1 MiB `CITATION_TOO_LARGE` with how many would fit.
- **R30** On an inquiry each new member becomes a leg with the member's role; its connection grade is filled from R13 when earned (A–C, axis `connection`, source `resolution`), else it carries none; a document leg naming no content row is pinned to `content.captureFor`'s capture (REC-220); its target joins `references[]`. On a project each becomes a `confirmed` `cites` edge, a document's pinned to its capture (REC-219). Members already cited are reported, not rewritten; nothing new is `ok` with an empty `cited` and says a named part was written nowhere.
- **R31** `sever` and `reinstate` act on a project's `cites` edges at weight `refuse`: `NOT_A_PROJECT`; the actor joined; `NO_REASON`, `BAD_REASON`; `EMPTY_SELECTION`; `NOT_INFORMATION`; reinstating a retired member `RETIRED_NOT_CITABLE`; every member's edge must be in the source state (`NOT_CITED`, `NOT_SEVERED`), whole call. The edge's target and relation stay; only its status moves, and the reason is appended to its note (at most 480 characters, oldest dropped).

## Private

### Uses

- `legacy-checks`: the C-2, C-6.1, C-21.2, C-33 and C-45 rows until they move (R41), `normalizeType`, `OBJECT_TYPES`, `parseFrontmatter`, `isMachineIdentity`, `BASIS_GRADES`, `EARNED_CAPTURE_CEILING`, `TESTIMONY_GRADE`, `createSha256`.
- `record-core`: `recordOf(ctx)`, `declarePurge`, the `bundles` read contract (R37).
- `membership`: `viewerPredicate`, `inSight`, `existenceAct`, `projectAuthority`, `bundleGate` (R43, R55, R61).
- `promotion`: `promote`, `registerStep`; the facts `caseMember`, `publishedRegistry` and `producingGroup` (map §5.3). *(not declared in `modules.json` today)*
...
- `entities`: `has` (R7), `strongestByCapture` (R16). *(not declared)*
- `provenance`: the `register` read contract (R48). *(not declared)*
- `extraction`: the `readings` and `reading_text_source` read contract. *(not declared)*
- `text-chain`: `captureBound`, `isTranscribed`. *(not declared)*
- `observation-log`: `LEAD_ID_RE` for C-54.1. *(not declared)*
- `bias`: nothing here calls it (map §5.5).

### Invariants

- **R32** The basis is a directed acyclic graph at every write (Case Making R3); `inquiry_basis` is a projection of the document and never written otherwise (D-21).
- **R33** A machine credential authors no division, no grouping and no leg role; it may surface a question (`ai-runs`).
- **R34** A leg naming an opinion case element is refused by name: an opinion is a view about evidence, never a leg (MEMBER-KNOWLEDGE §6). *(not yet met: MK-5)*
- **R35** An ungraded leg is inert and always named; nothing here gives a leg a grade the record did not earn or a member did not author (DEC-18, DEC-24).
- **R36** Every act and read naming an inquiry or project the viewer may not see answers exactly as an absent one.
- **R37** A leg that cuts against travels every path a supporting leg does: apportionment, projection, grounds (Invariant 7).
- **R38** A published case member cannot be divided, re-grouped or set down; the route is reopen (DEC-12, DEC-72).
- **R39** `inquiry_basis`, `inquiry_exclusions` and `inquiry_migration_replays` carry `bundle_id` and are declared to record-core's purge (K23); the columns this module writes on `bundles` today (`inquiry_basis_count`, `inquiry_subject_entity`, `inquiry_superseded_by`) move to a table of its own keyed by `bundle_id` (K75 (3)).
- **R40** No place is named in this module's behaviour or outward text.
- **R41** Each check moves here as an invariant with its test (K6): C-2.8 and C-21.2 as the grammar uses them, C-6.1's supersession and division arms, C-6.3, C-45.7–C-45.10, C-54.1, C-33.13, C-33.15–C-33.19, C-33.22, C-33.23, C-33.39, C-32.7, C-32.8.
