...
- **Checks.** No entry in `bio-plane/checks/bio-checks.mjs` calls into this module: the gate's own release-signature check (C-18.8, "Release-signature primitives", from line 6133) is a second, independent SSHSIG/Ed25519 implementation, deliberately hand-written rather than importing this module, because the gate runs in three environments (node, browser, Apps Script) and Apps Script has no WebCrypto Ed25519, which `verifySshsig` depends on. The two must keep agreeing with stock OpenSSH independently; neither should be made to call the other. This module therefore takes on no check-derived invariant today.
