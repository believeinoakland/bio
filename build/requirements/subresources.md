...
- **R17** *(not yet met: D-603)* Only a `subresources[]` record whose fetch was actually issued this run carries `fetched_at`. A reused, policy-skipped, `DEFERRED`, `CAP_REACHED`, `BUDGET_EXHAUSTED`, or refused/unresolvable record never claims a fetch instant it does not have.
