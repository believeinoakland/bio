...
**Status** · DRAFT by BOB #37, 2026-09-25 (T6). Layer 1. Code today: `bio-plane/src/subresources.mjs`. R17 met in T1 (D-603); the old battery's assertions of the defect are entry N15.
...
- **R17** Only a `subresources[]` record whose fetch was actually issued this run carries `fetched_at`. A reused, policy-skipped, `DEFERRED`, `CAP_REACHED`, `BUDGET_EXHAUSTED`, or refused/unresolvable record never claims a fetch instant it does not have.
