...
**Status** · CLOSED by BOB #41, 2026-09-26 ~18:00 UTC; outcomes and usage at the end. Opened by BOB #41, 2026-09-26 (PROCESS-MECHANICS §5), from the plan BOB #40 prepared. Branch `tranche/T2` starts at the commit that opened this plan. Bob's weekly meter at the opening: not yet given. BOB's session: `session_01JwTyEmUPSgvwiHEwzCzoxK` (BOB #41, since 2026-09-26).
...
Every entry was applied and every job merged into `tranche/T2`; `jurisdictions` merged early (17:12) for `id-spaces` and `docprofile`. Every layer-1 module's tests pass on the merged tranche (17 modules, T1's four included), each with every live id named by a test. Met in T2: N1, N11, N2, N3, N7, D-415, D-612, D-346, D-633, D-723, D-416, D-622, and T2-1 to T2-12. Deferred, with why, to `next.md`: DIST-14 (needs a deployed plane), N22 (a root-only container), N26 (needs a reference migration), N30 and N34 (need a requirement first), and PPM/PPT JBIG2 (no encoder for a checkable fixture). Reported into `next.md`: N21, N23–N25, N27–N29, N31–N33. Rulings K33–K55.

**Known red on `main` after T2, each owned by `legacy-tests` (layer 11) and carried to T3 by K53:** 39 harnesses without `jurisdictions/` (N32), `formats-odf.test.mjs` on its 13 D-346 assertions (N29), the pinned ocr-worker input list in `fleetbundles.test.mjs` (N33; every verifying arm passes, 95/1, K55), plus T1's N15. The full regression runs only at a release (K19).

**Checks at the close:** format 0 failures; architecture 66 failures, the same count as `main` before T2 (N12–N14); coverage 7 failures, the seven layer-2 and layer-3 modules not yet built. Generated artifacts regenerated: the plane, pdf-worker, ocr-worker and newgroup bundles (`newgroup-bundle-fresh` 1/0).

**Usage (P14):** the 13 job sessions processed 221.6 million tokens (`build/metrics/T2.csv`, from `archive_session`); the largest was pdf-worker (81.9M, D-622's two decoders). BOB #41's own row is recorded at its handoff. Bob's weekly meter: not given at the opening or the close. **Bob's first reading, ~18:08 UTC 2026-09-26 (after the close):** a $250 credit redeemed at the end of 2026-09-25 is fully drained by 2026-09-26's work (the transition, T1, T2 and the BOB sessions). At ~18:13 UTC Bob gave the weekly quota as 0% used and said to continue; he set no spending bound.
