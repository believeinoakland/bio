...
## Job session (layer 11, started 2026-09-26 19:46 UTC)

| module | session | title |
| --- | --- | --- |
| legacy-tests | `session_016rDfwRdhPxZqNpBWWNS5Xa` | LEGACY-TESTS #1 |
...
- **membership** · COMPLETE at `39e4677ec3`, verified 2026-09-26 19:19 UTC: ownership 0 failures (legacy-store 118 added, 3,013 removed; every `ADDED` line read: the import, `Store`'s static names re-pointed at `Membership`'s, and callers rewired to `membershipOf`), coverage with record-core 120 of 120, architecture 0, module tests with record-core 106/106. Merged early at `221d4816d7`; CHANGE sent to promotion. Its REPORTs: 18 old-battery suites (itemised in its record) are `legacy-tests`' under T3-4; `promote()` calling `projectCreated` (R71) forwarded to promotion; the rest routed to `next.md` (N43–N45); `bio-plane/dist` regenerated at the close.

- **promotion** · COMPLETE at `9970721d`, verified 2026-09-26 19:46 UTC: ownership 0 failures (legacy-checks 0 added, 909 removed; legacy-store 37 added, 1,226 removed; every `ADDED` line read: the import, the K31 registrations of legacy-store's share, its checks and projections moved out of the old inline `promote` into registered steps, and callers rewired to `promotionOf`), coverage 44 of 44, tests 50/50. Its REPORTs: 36 old-battery suites newly red, each diagnosed in its record (19 fixtures R12 now refuses, 4 R13/R14, 2 refusal-order changes per R39/K62, 4 reading the moved checks or the census, the rest source-anchored), all `legacy-tests`' under T3-4; `record-core.auditPass` has no hook for extra checks (N51); `affordances` keeps its own `REOPENABLE_FROM` (N52); one C-4.2 arm stays in `checkStateLegality` (C-2.6 needs it).

**Layer 2 CLOSED** by BOB #42, 2026-09-26 19:48 UTC: promotion merged; format 0, coverage 164 of 164 over the three modules, architecture 0 over them (66 over the whole product, the count before T3), module tests 156/156; `bio-plane` bundle regenerated and verified (only N33's pinned list fails, as K55; the installer's bundle fresh); no routine into a job session; the three sessions archived and their rows written to `build/metrics/T3.csv` (236,617,804 tokens processed: record-core 37.8M, membership 67.2M, promotion 131.7M). Layer 11 (`legacy-tests`) starts.
