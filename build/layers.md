...
| consequences | What the breach did, and to whom: who or what is affected (a class of people, a fund, a program, a service), the measure (money, benefits, services, time, counts) and the period. Computed from the record where the record holds the figures, each with its basis and grade; stated as undetermined where it does not. That a harm follows from the act is itself a finding that needs evidence, never assumed. People are counted as a class or named in their official role, never singled out as individuals. Escalation's exit condition ("consequences addressed") is checked against it. | Bob, 2026-09-26; Roadmap OP6; Functional Architecture Function 4 ("informed by context, scale, pattern, and consequences") |
...
**Bob's three points on the draft (2026-09-26), each with BOB #38's recommendation, awaiting his ruling:**
1. *Significance to whom?* Split it. **Consequences** (what the breach did and to whom) are computed from the record where it holds the figures, in the new `consequences` module. **Significance** (whether the breach warrants action, and how urgently) stays the member's judgment, made with the consequences in front of them.
2. *Tier 3 support for counsel.* Yes: the counsel packet above. It changes the meaning of Design Requirement 8, which today withholds all Tier 3 filing material, so it needs Bob's ruling. The requirement's purpose (no adverse precedent from an unsophisticated filing) is kept: the packet goes only to named counsel, is never published, and cannot be filed as it stands.
3. *A seventh, political stage.* It meets Operational Principle 1 ("We take no position on what the policies should be"). Recommendation: add stage 7, **political accountability**, entered from response evaluation or legal tools and still aimed at compliance restored and consequences addressed. It covers asking elected officials to act on the breach, oversight and audit requests, testimony, and legislation that restores or enforces an existing requirement. **Advocating new policy and supporting candidates stay out** unless Bob amends Principle 1. Both would reach people outside the project: campaign-finance and nonprofit-status rules depend on each group's legal form. If Bob wants them, they become a module beyond MVP, like Discovery.

**What changes if approved.** `modules.json` gains five modules and moves `actions`; the layers renumber to eleven (58 modules). Layer 1 is unaffected except `jurisdictions`, whose profile gains the laws, bodies, filing kinds and venues these modules read: an entry in `plan/next.md`, not a change to its approved requirements now.
