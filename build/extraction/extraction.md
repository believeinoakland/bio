...
**Status** · Measured 2026-09-26 on `tranche/T3` @ `91933d75` (after record-core's early merge) by a drafting worker for BOB #42 (P18). The line numbers are from `grep -n`/`awk` over `bio-plane/src/store.mjs` (53,685 lines), `index.mjs` (13,438), `schema.mjs` (4,180) and `bio-plane/checks/bio-checks.mjs` (16,591); the extraction job confirms them. A range runs from the comment block above a method to its closing brace. The contract is `build/requirements/extraction.md` (R1–R57). K31, K49 and K61 apply. **`from`: `["legacy-checks", "legacy-store", "legacy-index"]`**: `legacy-index` because the reading pipeline and `op=pdfstructure` are in `index.mjs` (K49), and `legacy-checks` for C-42 and C-51 on K64's precedent (§5).

## 1. What moves to `extraction`

**Already on its paths** (move to `bio-plane/src/extraction/` or stay where they are, the job's call): `readingprov.mjs` (218), `calibration.mjs` (429) and `extractrun.mjs` (347).
...
| the calibration region: `calibrationSubjects` … `#calibrationTick` | 24646–25154 | R42–R47 |
...
| dispatch arms `reading`, `reextractbasis`, `reextract`, `readingref`, `textprovenance`, `calibrations`, `calibrationdrift`, `calibrate`, `calibrationsubject`, `calibrationsignal`, `readingtermsclear`, `readinghistoryclear`, `reindexnames` | 52392–52479, in part | |
...
**Tables it owns** (`schema.mjs`, each with its indexes and the comment above): `readings` (668–687), `reading_refs` (718–733), `reading_ref_terms` (798–807), `reading_text_source` (2922–2950), `calibrations` (3101–3138), `calibration_subjects` (3151–3158), `calibration_signals` (3172–3182), `capture_text` (3526–3539) with `capture_text_fts` (store.mjs 1661), `reading_history` (4081–4092). About 170 lines of DDL, about 420 with their comments. Purge: `legacy-store` declares the reading tables, `text_attestations` (content's) and `capture_text_fts` today (store.mjs 866–872, record-core R21/R46). Extraction declares them itself, and declares the three calibration tables exempt. No module declares those today, so they are never touched, which is the same outcome (R56).

**Checks** (`bio-checks.mjs`): `CALIBRATION_CHECKS` (C-42.1–C-42.7, 11250–11327, `where` naming `calibration.mjs` and `store.mjs calibrationRecord`) and `REEXTRACT_CHECKS` (C-51.1–C-51.5, 12205–12272). D-668 adds `CAL_UNATTRIBUTED`, `CAL_SUBJECT_UNNAMED` and `CAL_SUBJECT_NO_PROBE`. `text-chain`'s C-35 family (`checkAttestation`) is used, not moved.

**Size.** The ranges above total about 5,450 lines, about 2,250 without comment-only lines: `index.mjs` 2,297 (765 code), `store.mjs` about 1,570 (about 740 code), `schema.mjs` about 400, checks 146, the three files 994 (431 code). Calibration's share is about 1,140 (requirements, Open for Bob 2).

## 2. What stays in the legacy modules or goes elsewhere, and why

| what | where | owner |
| --- | --- | --- |
| `extractPropose`, `extractProposals`, `#mintsBound`, `#posFields`, `proposed_readings` (schema 3449–3473), the `extractpropose`/`extractproposals` arms | store.mjs 23473–23847 | **`ai-runs`** (layer 6). They read `ai_runs` and `ai_run_bounds`, gate on the run's principal, and mint through `content.mintContent`, so they need layers 4–6. They use extraction's vocabulary (R48–R50), so `ai-runs` gains a use of `extraction` (§3) |
...
| the `calibration-reprobe` scheduler consumer | store.mjs 3818–3872, 3975 | `scheduler`, which calls R47 |
...
**Callers in `index.mjs`**: `op=acquire` calls `extraction.read` after capture and answers both (Open for Bob 1). `tier3Extend`'s `op=calibrations` fetch to the store (5210–5222) becomes a named read of the live calibration.
...
- **`nc-*` drivers that edit `index.mjs` or `store.mjs`:** `nc-cpdf10.mjs`, `nc-cpdf19.mjs`, `nc-d536.mjs` (also `readingprov.mjs`), `nc-rec102.mjs`, `nc-rec111.mjs`, `nc-cap9.mjs`, `nc-cap12.mjs`, `nc-sk8.mjs` (`extractrun.mjs` and the store's SK-8 region, which goes to ai-runs), `nc-sk7.mjs`, `calibration.control.mjs`.
- **Suites that scan or regex source:** `capture-text-index.test.mjs` (lifts the DDL out of `store.mjs`), `bounds.test.mjs` and `derivation-bounds.test.mjs` (scan `store.mjs` for bounds), `tier3-layer-parts.test.mjs` (anchored at an indentation in `index.mjs`), `calibration.test.mjs` (reads `index.mjs` at 392), `textchain.test.mjs`, `extractrun.test.mjs`.
- **Suites that drive these ops through the worker** (behaviour, no anchor; the job sorts them): `reading`, `reading-wire`, `reading-position`, `reading-dialect`, `tier2-wire`, `d606-perpage-ocr`, `reextract`, `pdfstructure-op`, `d536-reading-provenance`, `capture-pagecount`, `capture-container-extent`, `pdf-worker-binding`, `ocr-member-e2e` (and `.control`), `producer-provenance`, `provenance-marker`, `textshown`, `rec169-consume`, `rec165-production-principal`, `pipeline-e2e`. `civicos-ui/check-refusal-codes.mjs` reads C-42/C-51 from the catalogue.

## 5. Undetermined (stated, not guessed)

- **Where `read` runs, and its input.** The pipeline runs in the Worker, with `env.CAPTURES`, `PDF_WORKER` and `OCR_WORKER`; the writer runs in the Durable Object (`extractionOf(ctx)`, K61). Whether `read` takes capture's answer document or a capture digest plus a context read back from the record, and whether it re-derives `docType` or takes capture's, is for the job, after Open for Bob 1.
- **`from` including `legacy-checks`.** Moving C-42 and C-51 now follows K64. Leaving them for a `legacy-checks` entry follows the earlier N36 pattern. The refusal-code scripts (legacy-tests) read the catalogue either way.
...
- **Calibration as its own module** (Open for Bob 2). If Bob agrees, this map's calibration rows (`calibration.mjs`, 24646–25154, C-42, the three tables, the five ops) go to that module's map.
