...
**Schema (K4).** `refs` (schema.mjs 8–15), `connections` with five indexes (902–992), `connection_pair_choices` (993–1027), `connection_dirty` (1188–1215). Today `legacy-store` declares them to purge (store.mjs 869, 877, 879, 891). The job removes them there when `connections` declares its own (R36, K23).
...
**Checks it needs and does not take:** C-6.2's row (the catalogue's reference checks), `isMachineIdentity`, `describeExtent` (content's, through `legacy-checks` until content moves it).

**Measured size:** store.mjs 1,210 (610 code), bio-checks.mjs 261 (127), schema.mjs 162 (56): about 1,635 lines, about 790 of code.
