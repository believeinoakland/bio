...
**Status** · Measured 2026-09-26 on `tranche/T3` @ `35ea098` (promotion merged; `store.mjs` 49,817 lines, `schema.mjs` 3,964, `checks/bio-checks.mjs` 15,682, `index.mjs` 13,438) by a drafting worker for BOB #42 (P18). A method's range runs from the line after the previous method's close to its own close, so the comment above it goes with it; the extraction job confirms each. The contract is `build/requirements/inquiry.md` (R1–R41); K23, K31, K57, K61, K63, K73 (5), K75 (2)–(3), K78 (3) and K79 apply. The module exports `inquiryOf(ctx)` (K61); `legacy-store` delegates to it and registers what later modules still owe it (K31). `from` should read `["legacy-store", "legacy-checks"]` (K64's pattern). Nothing moves from `index.mjs`. Written beside the parallel `basis-versions` and `strength` drafts (same worker); §5 reconciles them.
...
| --- | --- | --- | --- |
| `#writeSupersededBy`, `supersededByOf` | store | 1872–1907 | R12, R16; the `inquiry_superseded_by` column leaves `bundles` (R39) |
| `EDGE_*` constants, `#edgeTransition`, `sever`, `reinstate` | | 4288–4551 | R31 (to `citation` if split, §5.1); `CORRESPOND_LEASE_MS` in the header block is `actions'` |
| `dispose` | | 4552–4790 | R20–R22; `DISPOSITIONS` becomes this module's copy (K78 (3)) |
| `#restsOnLive`, `#retiredNotCitable` | | 5198–5252, 5268–5301 | R17, R29; `#retirementCitedBy` between them is connections' fact (K76) |
...
| `#setSection`, `#removeBlock`, `#setOrAddBlock`, `#spliceEdgeStatus`, `cite` | | 13389–14300 | R29–R30; the four helpers are copied by each module that splices (K57) |
| `#appendStateHistory`, `#setScalar`, `#setOrAddScalar` | | 15566–15636 | helpers, copied |
| `#spliceReferences`, `#legExtentLines`, `#spliceBasis` | | 15753–15902 | cite's splices |
...
| dispatch `basis`, `restson`, `earnedbasis`, `cite`, `sever`, `reinstate`, `dispose`, `inquirydivide`, `inquiryground` | | 48584–48585, 48619–48626, 49473–49530, 49598–49601, 49681–49704 | K3 |
| migrations: `inquiry_basis.ground`; `bundles.inquiry_basis_count`, `inquiry_subject_entity`, `inquiry_superseded_by` | | 1072, 1092, 1107, 1115 (with their comments) | the three `bundles` columns move to an own table (R39, K75 (3)) |
...
| rows C-45.7–C-45.10 (in `CONTENT_EXTENT_CHECKS`), C-33.13, .15–.19, .22, .23, .39 (in `ACT_SHAPE_CHECKS`), C-32.7, C-32.8 (in `MACHINE_FENCE_CHECKS`) | bio-checks | in 12189–12401, 9490–10138, 9191–9421 | R41; each family is one object, split by the first job to move, numbers unchanged |

**Schema (K4).** `inquiry_basis` with its four indexes (schema.mjs 1267–1402), `inquiry_exclusions` (1462–1494), `inquiry_migration_replays` (2112–2127). All carry `bundle_id` and sit in `legacy-store`'s purge list (store.mjs 872–876), from which the job removes them when this module declares its own (R39).

**Measured size:** store.mjs 4,115 (1,943 code), bio-checks.mjs 1,390 (560), schema.mjs 185 (37): about 5,690 lines, about 2,540 of code.
...
- `earnedBasisRegistry`: `#legEarnedCapture` (2486; `retrieval`, through its registration, K75 (2)), `auditPass`'s context (15936), `gateFacts` (33884), `#reevalLegsEarned` (31436), `#captureBoundsFor` (32028), `#versionLegsEarned` (37454), `#versionLegsAsMembers` (37934).
...
- `#restsOnLive`: `affordanceFacts` (3046), `#reevalRaisedBy` (4800). `#retiredNotCitable`: `affordanceFacts` (3022), `suggestVersion` (39445). `supersededByOf`: `#reevalMoved` (31468). `#writeSupersededBy`: `#migrate`'s backfill (1729).
...
Negative controls and pins that read or patch the text of moved code, so they move or re-anchor (a `legacy-tests` entry, K53): `nc-mk4.mjs`, `nc-rec84.mjs`, `nc-rec97.mjs` (the leg grammar, `leadLegFindings`, `cite`'s extent bag), `nc-rec72.mjs`, `nc-rec83.mjs`, `nc-rec114.mjs`, `nc-rec116.mjs` (cite, `ensureLegContent`, `#capturedAt`), `nc-m040.mjs`, `nc-rec88.mjs`, `nc-sk7.mjs` (the earned registry), `rec-183-reinstate-retired.control.mjs`, `d280-strengthbar.control.mjs`, `founder-sight.control.mjs`, `project-sight.control.mjs` (`#edgeTransition`, `restingOn`), `machinefences-dec49.test.mjs` (the DEC-49 regions of `divide`, `groundInquiry`, `dispose`, `cite`), `hygiene.test.mjs` (purge tables), `identity-claims.test.mjs` (stamped authorship of cite, dispose, divide, ground). The setup page embeds `deriveInquiryTitle`'s source verbatim (its comment), so a pin reads it where it lands. Suites that drive the behaviour through the plane and follow the module: `basis`, `citeinquiry`, `cite`, `edges`, `disposition`, `inquiry`, `grounds`, `caselifecycle` (divide), `severedhomes`, `rec114-leg-earned`, `strength` (the earned arms), `derivation-bounds`, `selection`.

## 5. Undetermined, conflicts, and code others could claim

1. **Size, and the proposed split (P6).** 5,690 lines, past 4,000. Proposed: `citation`, next after `inquiry` in layer 6, takes `cite`, `sever`, `reinstate`, `#edgeTransition`, their splices and rows (C-33.15–C-33.19, C-33.39, C-45.7–C-45.10): about 1,300 lines (650 code), leaving about 4,390 (1,900 code), K74's case. `citation` uses `inquiry` (the leg grammar, `earned`), `retrieval` (selections) and `membership`. Read as BOB's under P17 (K70, K73 (7): a split for P6, no capability added or removed).
2. **The earned registry is placed here, not in `strength`** (where the progressions map and this draft's brief placed it). It is needed by the leg grammar at every write (R6, `checkEarnedLeg`), by `cite`'s fill (R30), by `divide` and `ground`'s pre-flight, by `basis-versions` (R10) and by `strength`; here it needs no registration. In `strength` it needs one `inquiry` offers and `strength` fills, and `inquiry` refuses an earned leg while it is unfilled (never reads it as not earned). Its reads are `entities.strongestByCapture`, the `register` contract (provenance R48), `readings` and `reading_text_source` (extraction's read contract), `text-chain.captureBound`, `content.standings`, `connections.portionGrades`. BOB settles which.
3. **Facts from later modules.** `dispose`, `divide`, `ground` and `#restsOnLive` ask `caseMember`; the leg grammar asks `publishedRegistry`; `divide` asks `producingGroup`. `promotion` holds all three as registered facts (store.mjs 900–902) but reads them privately. Proposed: `promotion` offers `fact(name, …args)`, answering `FACT_UNAVAILABLE` when unprovided (its R40's rule), so a fact is registered once for every earlier reader; otherwise this module offers its own registrations for each.
4. **Old-plan rows.** REC-202 (a member takes up or sets aside at the inquiry's grain) is built on `land/worker/REC-202` @ 1716321d in `queue`'s findings producer (`#findingsOutOfInquiryLead`, `options_grain`): the acts exist (`op=cite`'s inquiry arm, `op=proposedispose`); proposed target `queue`. D-572 (a level-empty candidate per named question) is `ai-runs`' `op=suggest` and `observation-log`'s look; proposed target `ai-runs`. D-592 (who reopened) is R19 here, read from the state history `promotion`'s `reopen` writes. MK-5 is R34, not yet met: it waits on `publication` defining the opinion element's id.
5. **Uses.** `modules.json` gives `inquiry` `bias`, which nothing here calls (HUNCH debt is refused at publication, `publication`'s). The moved code calls `promotion`, `entities`, `provenance`, `extraction`, `text-chain` and `observation-log` (Private/Uses). Proposed: add those six, drop `bias`.
