 ...
 * **RE-MEASURED 2026-09-24 BY D-478 (cloud worker, base `e9b21be6`), because D-478 changed this suite and every figure above went stale the moment it did — corrected, never left standing. THE PEN RAN OUTSIDE THE WORKTREE** (`BIO_CONTROL_PEN`, added by this item on BOB #32's ruling of the same day; the harness's own header still argues for the in-tree default and that reasoning is kept, with the resolution recorded at the `PEN` line). New BASELINE **87 pass / 0 fail / exit 0 / foot reached** (was 70/0; +17 assertions, all D-478's §8 and §8b). Re-measured: (a) 86/1 · (b) 84/3 · (c) 83/4 · (d) 85/2 · (e) 84/3 · (f) 87/0, the baseline exactly. **8 arms run, 0 NOT as declared**, every restore byte-identical by sha256 AND `cmp`, tree re-green at 87/0.
...
console.log("\n--- 8b · D-478: *not found* is not *absent* — the namespace this member reads from is exactly `bio` or `scratch` ---");
{
  ...
  for (const name of ["biosmoke-fleet", "bio_smoke", "Scratch", "BIO", "ocrsuite", ""]) {
    ...
    t(`a namespace no instance holds (${JSON.stringify(name)}) -> 400 NAMESPACE_UNKNOWN`,
      [r.status, r.body.reason], [400, "NAMESPACE_UNKNOWN"]);
  }
