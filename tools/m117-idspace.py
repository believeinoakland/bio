#!/usr/bin/env python3
"""M-117 (D-74): which identifier values does Oakland reuse across its systems?

Framework §8.3's five identifier classes, matched over a corpus of texts already
extracted from captures held in the record (see measurements/M-117.md for how the
...
for PDF text). This script only MATCHES; it fetches nothing.
  ...
  python3 tools/m117-idspace.py <corpus-dir>            the measurement
  python3 tools/m117-idspace.py --selftest              the negative control (synthetic)
  python3 tools/m117-idspace.py <corpus-dir> --plant    the control arm on the real corpus
...
def selftest():
    ...
    checks = [
        ...
        # Corrected 2026-09-23 (M-117 arm B): this check first read `"03100" not in got["fund"]`,
