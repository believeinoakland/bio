...
STEPS.forEach((s, i) => {
  ...
  /* CONDUCT #16, at the merge with M0-127: a step M0-126 turns RED on a clean exit (an under-inclusion or a history
     read) has no cause `stepCauses` can see, since it reads only the exit and the verdict file — so each such unit is
     named here, and a RED still names every cause (M0-127). */
  const keyCauses = under.flatMap((id) => [...(historyRead.has(id) ? [`history:${tokenSafe(id)}`] : []),
    ...(underIncluded.get(id).length ? [`underinclusion:${tokenSafe(id)}`] : [])]);
  results.push({ label: s.label, units: s.units, ok, ...(timedOut ? { timedOut: true, unmeasured } : {}),
    ...(fu.length ? { failedUnits: fu } : {}), causes: [...stepCauses(s, r, v, { timedOut, unmeasured }), ...keyCauses] });
});
if (REUSED.size)
