 ...
 * directory and removes that synchronously on exit. CORRECTED 2026-09-23 (M0-127,
 * from M-111): "removes that synchronously" was not enough — the synchronous
 * `rmSync` raced dispose()'s already-started async `rm` and could return with the
 * tree still there (21 of 300 endings leaked). It now RENAMES the directory aside
 * first and removes the moved tree until it is verified gone (`sweepSandbox`,
 * cdfaea39). The import below is still the whole contract. A suite gets the guarantee
...
console.log("\n--- every table is purged or explicitly exempt (D-113 / D-137) ---");
{
  ...
  const EXEMPT = {
    ...
    /* D-520: the renders RUNNING now, one row each, for the concurrency cap. The same family
       as render_allowance: an operational fact about this instance, nothing corpus-derived. */
    render_slots:         "the renders running now, for the concurrency cap (D-520); operational state that expires with each render, not corpus-derived",
