...
{
  /* D-596 CORRECTED this arm: the page now reads op=groupidentity's public projection (`groupidentitypublic`) while
     the op keeps `instancegrouppublic`, so a silence injected on the op's path ALONE left the page reading fine and P4
     failing over a correct plane. Both public paths are silenced, so each surface meets the silence it reads. */
  const armed = await (await D.dispatchFetch("http://x/__failpaths?paths=instancegrouppublic,groupidentitypublic")).json();
  ...
  t("P4: when the record does NOT ANSWER, the page is still served, and its group line says it could not read the group "
    ...
    [armed?.result?.failing, p.status, g.state, /recorded/i.test(g.text), g.text.length > 10, p.body.includes(FIRST),
     LITERAL.test(p.body)], [["instancegrouppublic", "groupidentitypublic"], 200, "unread", false, true, false, false]);
