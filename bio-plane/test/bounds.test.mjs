...
/* MOVED TO 32, 2026-09-14 by SK-8 — MEASURED by running the walk on this tree and
   taking what it PRINTED (it reported 32 against a pin of 31), never by adding one
   to the figure above, which is this pin's own recorded hazard and has already
   collided two items by arithmetic. The arrival is `op=extractproposals`, SK-8's
   read of what an EXTRACT run proposed. It is the ORDINARY kind of arrival: a
   `LIMIT ?` against a cap clamped in the method, so nothing about the detector had
   to move to admit it. The table it reads grows by one per reference per proposal
   per run — which is exactly the shape that grows FAST rather than slowly, and is
   the reason this op would have been worth bounding even if this roster had not
   demanded it. It is DRIVEN in `test/extractrun.test.mjs` rather than in this
   loop; see `DRIVEN_ELSEWHERE` below for the corpus argument. `op=extractpropose`
   is NOT on this roster and that is correct rather than a gap: it is a WRITE, its
   batch is refused WHOLE at the run's `mints` bound rather than truncated, and a
   write that refuses publishes its bound in the refusal — which is the posture
   `op=suggest`'s two arms below already pin. */
  OPS.size, 32);
...
const generatedReason = (src) =>
  ...
  /* ADDED 2026-09-13 (DS-4). The clause above was STALE and passing on a month-old
     artifact: the current plane bundle opens `var __defProp = Object.defineProperty;`,
     esbuild's interop preamble, not a module comment. It kept matching only because
     `release/` still held the 0.56.0 asset; a freshly built plane published there is
     NOT recognised and is counted as a consumer of its own code. Structural at byte 0,
     never by name. Member bundles match the same way and are excluded for the same
     reason — they inline plane source. */
...
+ "than the reproducible one quietly reading zero",
  ...
  [true, true, true]);
t("REC-59 REACH: ALL THREE string embeds of the plane are excluded, each recognised STRUCTURALLY at byte 0 "
+ "and neither by name, and each is over a megabyte — so none counts the plane as its own consumer. WAS 2 "
+ "UNTIL 2026-09-13 (DS-4): release 0.57.0 publishes the fleet, so `release/pdf-worker.bundled.mjs` joins — "
+ "it INLINES bio-plane/src (three of its six build inputs live there, FL-9), so counting it would make the "
+ "plane its own consumer a third time. The figure moved because the corpus did, and the recogniser above "
+ "was stale in the same turn",
  [consumerExcluded.length, consumerExcluded.every((x) => x.chars > 1_000_000)], [3, true]);
