...
/* M0-197: each arm's anchor as data for tools/anchordrift.mjs (a no-op otherwise). Only 9, 10 and 10b quote a line.
   c22-batch30: D-560's 11, 11b and 11c (appended on main) quote the assembler's lines, so they are rows too. */
anchorTable([...Object.keys(ARMS).filter((n) => !["9", "10", "10b", "11", "11b", "11c"].includes(n)).map((arm) => ({ arm,
  ...
  { arm: "11", file: ASSEMBLER, find: NOART_NOW, put: NOART_OLD }, { arm: "11b", file: ASSEMBLER, find: NOART_SET_NOW },
  { arm: "11c", file: ASSEMBLER, find: NOART_NOW }]);
