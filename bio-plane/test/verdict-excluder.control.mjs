...
const PLANT_REFUSAL = [
  ...
  "    const entries = this.#rows(`SELECT seq FROM observations WHERE authority_kind = 'run' AND authority = ?`, run);",
...
const PLANT_BOUNDED = [
  "  ncD240Read(input = {}) {",
  "    const cap = 50;",
  "    const rows = this.#rows(`SELECT seq FROM observations ORDER BY seq LIMIT ?`, cap + 1);",
