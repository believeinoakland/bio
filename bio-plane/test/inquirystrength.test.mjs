...
console.log("\n--- 9. D-177: a DIRECT capture's grade is MEASURED from its fetch path, not authored ---");
{
  ...
  const promote9 = async (id, chain) => {
    ...
    const r = await POST(`op=promote&token=${carol}`, {
      ...
      /* CORRECTED 2026-09-25 at the c22-batch30 union (D-563, C-86.3), never exempted: D-177's section was written
         before D-563 and labelled the bundle `Bundle ${id}` over a document whose own title is `Info ${id}`; the
         record goes by the document, so the request now names the document's own title (section 8's provPromote
         names none). The dates were already the document's own (D-615, C-86.7). */
