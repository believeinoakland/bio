...
/* NEGATIVE CONTROL: section 9g (D-709) — RUN 2026-09-25 by the D-709 worker through `node test/nc-d177.mjs <arm>`, EVERY arm re-run after the subject changed (none, a-k), each ALONE, declared before running, restored from its own per-arm pristine copy in `controlPen` and verified by sha256 AND `cmp` AND size (store.mjs 3,484,029 bytes, sha256 ef1852b072790c92, re-run after the reasons were reworded in the member lexicon analystvocab §4a enforces, every figure unchanged; bio-checks.mjs 1,007,326 bytes, sha256 39b3f9809967fd38). baseline 86/0. (a) 82/4, (b) 81/5, (c) 75/11, (d) 86/0, (e) 85/1, (f) 85/1, (g) 85/1, (h) 83/3 with acquire in the same run — all AS DECLARED, D-693's and D-698's arms unmoved in verdict by this row. (i) THE ROW'S OWN — the key OMITTED again on a no-locator entry (`continue` before the stated route is written) -> 81/5, AS DECLARED, 9g failing BY NAME ("A NO-LOCATOR CAPTURE'S ENTRY STATES ITS ROUTE UNRECORDED", "A LEG ON IT READS AUTHORED-UNDER-CEILING AND SAYS SO") with 9a's corrected BEFORE and 8c's two corrected pins, while every measured-route headline held. (j) THE READ SILENT AGAIN (the entry still states the route; `#capturedAt` returns nothing at or under the ceiling) -> 83/3, AS DECLARED, the entry assertion held and the three leg reasons failed. (k) OVER-STRICTNESS, the same test with optional chaining -> 86/0. */
...
console.log("\n--- 8. REC-105 / D-373: the capture axis is resolved through `earnedBasisRegistry` ---");
{
  ...
  t("and it carries no BOUND, and states its letter is the author's under the ceiling (no route recorded)",
    [pubWalk.grade, "bounded_by" in pubWalk, /at its author's own B, not over /.test(pubWalk.weakest?.why ?? "")],
    ["B", false, true]);
  ...
  t("a leg stating a WEAKER letter than the ceiling keeps its own, and is not raised to the maximum",
    [weakWalk.state, weakWalk.grade, /at its author's own C, not over /.test(weakWalk.weakest?.why ?? "")],
    ["graded", "C", true]);
...
console.log("\n--- 9. D-177: a DIRECT capture's grade is MEASURED from its fetch path, not authored ---");
{
  ...
  t("A LEG ON IT READS AUTHORED-UNDER-CEILING AND SAYS SO — the author's letter, never a measured one",
    { grade: nolWalk.grade, authored: /at its author's own D, not over /.test(nolWalk.weakest?.why ?? ""),
      ceiling: new RegExp(`not over ${nolReg?.grade}: the record does not say where`).test(nolWalk.weakest?.why ?? "") },
