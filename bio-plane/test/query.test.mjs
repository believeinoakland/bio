...
{
  /* D-258, D-255's class one field along. The descriptor used to carry
     `columns: MEANING[arm].row` and `refs: MEANING[arm].refs` and NOTHING read
     either — established by making each throw on any read and running the whole
     battery, not by grep (`test/fieldread.control.mjs --tripwire-sweep`), which
     is what reaches `store.mjs` inside workerd. The decision recorded at the
     site in `compile()` was DELETE rather than publish, because `op=searchfields`
     ALREADY publishes both, per arm, from the same registry.

     STRUCTURAL FOR THE SAME REASON AS D-255's PINS ABOVE, and it is worth saying
  ...
  for (const a of arms)
    t(`\`rows=${a}\`: the descriptor carries exactly the six the op reads, and no others`,
      Object.keys(compile({ q: "", viewer: M, rows: a }).meaning), SIX);
  t("a compilation that asked for no rows carries no descriptor at all",
    compile({ q: "", viewer: M }).meaning, null);

  /* THE TWO NAMES ARE STILL LIVE ONE LAYER DOWN, and that is the distinction
     that made this a finding rather than a grep result: `MEANING[arm].row` and
     `.refs` ARE read — inside `compile()` itself and by `meaningVocabulary()`.
     It was only the COPIES on the plan that nothing asked for. A future sweep
     that greps the names will see them read and conclude wrongly; these two pins
     are here so that the difference is asserted rather than explained. */
  t("`MEANING[arm].refs` is LIVE — the ref-visibility clauses are built from it",
    compile({ q: "", viewer: M, rows: "leg" }).statements.meaning().sql.includes("m.target_id"), true);
  t("and `MEANING[arm].row` is LIVE — the projected columns come from it",
    MEANING.leg.row.every((c) => compile({ q: "", viewer: M, rows: "leg" })
      .statements.meaning().sql.includes(`m.${c}`)), true);
}
