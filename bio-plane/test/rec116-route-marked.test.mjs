/* NEGATIVE CONTROL: ALL SIX ARMS RUN 2026-09-17 by `test/nc-rec116.mjs` against the FINAL files — 40 assertions in the driver, 0 failing, exit 0, read UNPIPED. Each arm armed ALONE with the others held open; each asserted it ARMED by matching its patch anchor EXACTLY ONCE and by the source really changing; each restored from a PRISTINE copy named uniquely for that arm and verified by sha256 AND by `cmp`, floored at 100,000 bytes (store.mjs restored byte-identically at sha `94693822ae99` / 2,323,431 bytes after every one of the five that patch it); the harness lives INSIDE this worktree and never in a shared scratchpad. BASELINE ROW FIRST, because without one six-arms-broken and six-arms-working are indistinguishable: this suite 65 passing / 0 failing with its FOOT line printed. CLOSING BASELINE re-measured EQUAL to the opening one, and store.mjs pinned byte-identical to the driver's opening sha, so no arm leaked into another.
   ...
   AND A FIFTH INSTRUMENT FINDING, THIS ONE ABOUT THIS FILE'S OWN FOOT LINE: it first printed `65 passing, 0 failing`, and `scripts/battery.mjs` reported the suite as **assertions unknown**. Its `tally()` matches `pass`/`passed` and `fail`/`failed`, never `passing`. Sixty-five real assertions ran, all green, and NOT ONE of them was in the battery's headline total — the quiet-skip class one step over, where the run reads healthy because a whole suite is missing from the count rather than because it was named as skipped. The foot is now spelled the way the reader READS.
...
/* THE FOOT LINE IS `N pass, M fail` AND THE SPELLING IS LOAD-BEARING.
   `scripts/battery.mjs`'s `tally()` matches /(\d+)\s+pass(?:ed)?,\s+(\d+)\s+fail(?:ed)?/,
   which accepts "pass" and "passed" and NOT "passing". This suite first printed
   `65 passing, 0 failing`, and the battery reported it as **assertions unknown**:
   65 real assertions ran, all green, and NONE of them were in the headline total.
   That is the quiet-skip class one step over — the run reads healthy and a whole
   suite is missing from the count — so the foot is spelled the way the reader
   reads rather than the way it scans. */
console.log(`\nREC-116 route-marked roster: ${pass} pass, ${fail} fail`);
