...
const promoteRead = async (id, captureSha, chain) => {
  ...
  const r = await post("promote", {
    ...
    /* CORRECTED 2026-09-25 at the c22-batch30 union (D-563, C-86.3), never exempted: this section was written
       before D-563 and labelled the bundle `Bundle ${id}` over a document whose own title is `Info ${id}`; the
       record goes by the document, so the request now names the document's own title. */
