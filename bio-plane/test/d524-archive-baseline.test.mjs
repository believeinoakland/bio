/* NEGATIVE CONTROL: (run 2026-09-24, D-524, on land/worker/D-524 over 9f8b69e6, FOUR ARMS PLUS A BASELINE, each armed ALONE by an exactly-once patch, each restored from a uniquely-named per-arm pristine copy in the session scratchpad and verified by sha256 AND cmp: 4 of 4 MATCH/IDENTICAL, cdx.mjs 9433 bytes 4714ec20c2f95d45…, index.mjs 834701 bytes 91cf3a0d03bffda6…). Declared before arming. BASELINE, nothing armed: 19 pass 0 fail. (A) THE ROW'S ARM — drop `document_address` from `archiveHop`: DECLARED red at the archive-baseline arm; ACTUAL 12/7, "ARCHIVE-BASELINE: op=monitor finds the archive-sourced baseline on both ticks" got [null,null] and ACCEPTS-WHEN got [null,null] — the measured failure itself, "no captured baseline" on every tick. (B) the monitor's hop fallback removed, the key held: DECLARED red at the same arm; ACTUAL 14/5, the same five tick assertions. (C) the hop match made EXACT rather than in `normalizeAddress` form: DECLARED only the over-strictness arm fails; ACTUAL 18/1, exactly "OVER-STRICTNESS: an equivalent spelling finds the archive baseline and reads unchanged". (D) the match taking ANY archive row: DECLARED only the wrong-document arm fails; ACTUAL 18/1, exactly "a hop naming another document is not this bundle's baseline". Restored: 19/0. A FINDING ABOUT THE INSTRUMENT, recorded: the first draft read the Archive's hop by `via` and got OUR hop, which also reads `via: "archive.org"`. */
