 ...
 *
 * ELEVEN ARMS (seven D-262's, h and i REC-185's, j and k D-495's), each armed ALONE with every other defence held OPEN, DECLARED
...
arm("j", "an ADMIN-ONLY refusal stripped of its code \u2014 src/store.mjs (THE SUBJECT, D-495)", STORE,
  (s) => s.replace(SITE_J,
                   `      return { ok: false, error: "that is not a slug in the installer's grammar. Nothing was recorded." };`),
  "RED");

/* ---------------------------------------------------------------- ARM k
   OVER-STRICTNESS FOR D-495's ARM, on the REAL site. The same refusal is rebuilt
   ...
   to route around it. */
arm("k", "the same ADMIN-ONLY refusal in an UNANTICIPATED spelling \u2014 src/store.mjs (OVER-STRICTNESS)", STORE,
  (s) => s.replace(SITE_J,
