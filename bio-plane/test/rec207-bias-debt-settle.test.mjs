/* NEGATIVE CONTROL: DECLARED HERE, RUN BY `test/rec207-bias-debt-settle.control.mjs` — deliberately NOT a `.test.mjs`, because it patches COPIES of `src/` while it runs and the battery must not discover it. Re-run in one step from `bio-plane/`: `node test/rec207-bias-debt-settle.control.mjs [arm]`. RESULTS, RUN 2026-09-24 by the REC-207 worker (cloud session WORKER REC-207 (SCHEDULER #19)) on origin/main 1a7f0bcc0 + this item (real src/index.mjs 813,575 B sha256 719552485cb3…, src/store.mjs 3,316,859 B c9ca9654fd4c…, src/schema.mjs 251,439 B c78cf8b80a89…, src/queuestate.mjs 23,647 B 402dc615e667…, checks/bio-checks.mjs 958,311 B 5f16a92dac4a…, untouched: YES), ELEVEN ARMS, each patching a COPY, each anchor occurring exactly its declared number of times: (1) baseline -> 34/0. (2) **discharge-any-rerun — THE ROW'S CONTROL, the lens comparison alone removed so any re-run discharges -> 33/1: ARM O1, THE OTHER-LENS ARM, BY NAME**, with ARM O2 (the undetermined hand), R3 and R4 GREEN — the asymmetry is what makes it evidence rather than an outcome that cost nothing. (3) undetermined-discharges — two absent hashes read as agreement -> 33/1: ARM O2 by name, O1 green. (4) no-reason-required -> 29/5: S2, S2b AND S3, S4, S5 — **NOT AS FIRST DECLARED (S3/S4 were declared GREEN), and the cause is the SUITE'S SHAPE: ARM S runs against one store in sequence, so the reasonless resolve this arm lets through settles the obligation S3 and S4 were going to act on and both meet ALREADY_SETTLED; everything after ARM S is untouched.** (5) settlement-not-appended — the append-only row not written while `bias_debts` is still stamped -> 28/6: S5, S7, R4, L1, L2, N1, **with ARM S4 GREEN — the ACT still claims the settlement and only the RECORD is missing, which is why this suite reads op=biasdebt rather than the act's own answer**. (6) held-dropped -> 31/3: H1 by name plus L2 and N1 — **NOT AS FIRST DECLARED (H1 alone), and the cause is the mechanism: restating clears `settled_kind`, so the resolve stops naming itself later.** (7) machine-allowed -> 31/3: S3 by name plus S4 and S5 — **NOT AS FIRST DECLARED, arm (4)'s cascade one act later; S2 stays GREEN, the pair that says only the machine fence moved.** (8) sight-dropped -> 33/1: G5 by name, with G2 GREEN (that run genuinely does not exist), which is what makes G5 a measurement of SIGHT and not of spelling. (9) context-check-dropped -> 32/2: G3 and G5. (10) door-taskresolve — `instead` put back as it was -> 33/1: ARM Q1, the row's headline, by name. (11) spelling (OVER-STRICTNESS) — the lens comparison written `!(formed === inForce)` -> 34/0, ALL GREEN as declared. BEFORE THIS ITEM: no re-run link, no resolve act and no settlement table — the suite cannot load a store without `bias_debt_settlements`.
 * =========================================================================
