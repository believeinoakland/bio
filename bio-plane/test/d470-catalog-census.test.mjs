...
const CATALOG_CENSUS = {
  ...
  /* 1.30.0 (REC-203, 2026-09-25, branch land/worker/REC-203): THREE ARRIVALS, NO DEPARTURES — C-91.1, C-91.2 and
     C-91.3, `op=idmatch`'s IDSPACE_CHECKS. 466 -> 469, count AND digest from THIS SUITE'S OWN PRINT on the item's
     tree over origin/main 964da679, never arithmetic. **IF ANOTHER BRANCH IN THE SAME BATCH ALSO ADDS ROWS, THIS
     ROW IS NOT THE UNION'S: CONDUCT takes the next number and re-reads the census from this suite's print.** */
...
/* CORRECTED by REC-203 (2026-09-25), not exempted: the stamp moved 1.29.0 -> 1.30.0 with C-91's three rows, and
   the stamp this arm pins is the one the catalogue now carries — the old literal named a catalogue that is gone. */
