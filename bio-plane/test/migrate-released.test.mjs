/* GATE: never-cache (history) — M0-126, BOB #30 (TREE-SHARING §3a condition 1): its verdict reads git show of released commits' RELEASE.json, which no
   ...
   READS NO LIVE REF — M0-136, 2026-09-23. What it reads: `git show <commit>:release/…` for the commit ids NAMED in
   `RELEASES` below (36 calls a run, 18 full 40-hex ids — content-addressed, so the same bytes in every clone that has
   them), and nothing of `origin/*`, `coord`, `FETCH_HEAD` or `ls-remote`; no fetch. HOW CHECKED: this file and the two
   modules it imports (`./stdio.mjs`, `./sandbox.mjs`; the plane bundle it boots is read from git, not imported) grepped
   for `execFileSync`/`spawnSync`/`git` and for those ref tokens — one site, `gitShow`; and the suite run with a logging
   `git` first on PATH: every call is `show <named id>:release/…`. The CLOCK: the plane it boots under miniflare reads the
   wall clock as every plane suite does; no arm here compares a date. */
...
const t = (label, got, want) => {
  ...
  console.log(`  ${ok ? "PASS" : "FAIL"}  ${label}${ok ? "" : `\n         want ${statedJSON(want)}\n         got  ${String(statedJSON(got)).slice(0, 1500)}`}`);
  ok ? pass++ : fail++;
};
