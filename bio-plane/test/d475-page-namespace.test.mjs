/* NEGATIVE CONTROL: DECLARED HERE, RUN BY `test/nc-d475.mjs` — `node test/nc-d475.mjs [arm]` from `bio-plane/`. Four arms, each ALONE against a uniquely-named pristine copy of `src/index.mjs`, restore verified by sha256 AND by byte comparison with the byte count floored. (A) THE ROW'S CONTROL — the parameter ignored again, `publicInstanceGroup(env, "bio")` as it stood: MUST FAIL at `A1: /?store=scratch serves the SCRATCH record's slug` BY NAME and at `A3: the two surfaces agree`, while A2 (no `store=` reads bio) stays GREEN, which is the whole point of pairing them. (B) a HALF fix — the store passed through but the unknown-namespace gate not run, so `/?store=nonsense` serves `bio` again: MUST FAIL at C1, C2 and C3 and at nothing else. (C) OVER-STRICTNESS — the gate refuses any named store, `bio` included: MUST FAIL at B1. (D) OVER-STRICTNESS — the page reads scratch by DEFAULT (the ternary inverted): MUST FAIL at A2 and A3. RUN 2026-09-24 in worktree /home/user/bio on `e9b21be6` + this row (pristine src/index.mjs 807,869 B sha256 d3dcaee8fa37, restored sha256+cmp identical after every arm): 4/4 AS DECLARED, each arm's failure set COMPLETE (the driver reports undeclared failures as well as missing ones, and its first draft's arm C was silently also breaking B2 — now declared) — (A) the parameter ignored again: 16/18, A1 and A3 alone, A2 GREEN · (B) the gate not run at the route: 14/18, R4, C1, C2 and C3 alone, every arm in sections 1, 2 and 4 green · (C) any named store refused, `bio` included: 16/18, B1 and B2 alone · (D) the default inverted to scratch: 16/18, A2 and A3 alone, A1 GREEN.
 * =========================================================================
