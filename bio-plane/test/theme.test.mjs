 ...
 *      reading; a declaration WITHOUT A TEST is refused (C-74.3), and so is a machine's (C-74.2);
 ...
 *      the members), a machine cannot PLACE (C-74.7), and a member's confirmation turns the hunch
 *      into membership while keeping who proposed it;
 *   4. A LEG CITING THE THEME IS REFUSED BY NAME (C-74.1, THEME_NOT_EVIDENCE) — through op=promote
...
t("A DECLARATION WITHOUT A TEST IS REFUSED — by name (C-74.3, THEME_NO_TEST), carrying its canned translation",
  [codeOf(nT), nT && nT.check, nT && nT.translation === THEME_CHECKS.THEME_NO_TEST.translation],
  ["THEME_NO_TEST", "C-74.3", true]);
t("a test of whitespace is no test (C-74.3)",
  codeOf(await post("themedeclare", { name: "blank", test: "   \n " }, RUTH)), "THEME_NO_TEST");
t("a theme with a test but no idea named is refused (C-74.4)",
  codeOf(await post("themedeclare", { name: " ", test: TEST }, RUTH)), "THEME_NO_NAME");
t("a test over one passage is refused rather than cut (C-74.5)",
...
t("fence 1: the MEMBER token is a machine credential — refused BY NAME (C-74.2); no machine declares a theme",
  [codeOf(mTok), mTok && mTok.check], ["THEME_NOT_A_MEMBER", "C-74.2"]);
t("and the ADMIN token is a machine credential too (C-74.2)",
...
t("fence 3: a MACHINE credential cannot PLACE — refused BY NAME (C-74.7), pointing it at a proposal",
  [codeOf(mPlace), mPlace && mPlace.check], ["THEME_PLACEMENT_NOT_A_MEMBER", "C-74.7"]);
const ghostT = await post("themeplace", { theme: "THEME-2026-0923-nosuch", target: DOC_C }, RUTH);
t("a theme that does not exist is refused (C-74.6)", [codeOf(ghostT), ghostT && ghostT.check],
  ["THEME_NOT_FOUND", "C-74.6"]);
...
const f1 = findingsOf(p1).filter((x) => x.check === "C-74.1");
t("ACCEPTS-WHEN: op=promote REFUSES an inquiry whose basis[].target is a THEME — BY NAME (C-74.1, THEME_NOT_EVIDENCE)",
...
t("and the theme leg is answered ONCE, by its name — not also as 'not a canonical bundle id'",
  findingsOf(p1).filter((x) => /basis\[0\]\.target/.test(x.message) && x.check !== "C-74.1").length, 0);
...
t("citing the theme through basis[].content_id is refused BY NAME too (C-74.1)",
  [p2 && p2.ok === false, findingsOf(p2).some((x) => x.check === "C-74.1" && x.code === "THEME_NOT_EVIDENCE")],
  [true, true]);
const p3 = await promote(Q1, inquiryMd(Q1, [{ target: DOC_A, theme: TID }]), "inquiry");
t("a leg on a real document that claims it THROUGH the theme (`theme:`) rests on membership — refused BY NAME (C-74.1)",
  [p3 && p3.ok === false, findingsOf(p3).some((x) => x.check === "C-74.1" && x.code === "THEME_NOT_EVIDENCE")],
  [true, true]);
const p4 = await promote(Q1, inquiryMd(Q1, [{ target: `${TID}#${DOC_A}` }]), "inquiry");
t("a leg naming a MEMBERSHIP by address (`THEME-…#INFO-…`) is refused BY NAME (C-74.1)",
  [p4 && p4.ok === false, findingsOf(p4).some((x) => x.check === "C-74.1" && x.code === "THEME_NOT_EVIDENCE")],
  [true, true]);
...
t("the VERSION-LEG grammar refuses a theme BY NAME (C-74.1)",
  vf.some((x) => x.check === "C-74.1" && x.code === "THEME_NOT_EVIDENCE"), true);
const af = []; actionBasisFindings({ action_basis: [{ target: TID, kind: "rests_on" }] }, af);
t("the ACTION-BASIS grammar refuses a theme BY NAME (C-74.1)",
  [af.length, af[0] && af[0].check, af[0] && af[0].code], [1, "C-74.1", "THEME_NOT_EVIDENCE"]);
const af2 = []; actionBasisFindings({ action_basis: [{ target: DOC_A, kind: "rests_on", themes: [TID] }] }, af2);
t("and an action leg claiming its target through `themes:` is refused BY NAME (C-74.1)",
  af2.some((x) => x.check === "C-74.1"), true);
...
t("otto placing it answers EXACTLY as a project that does not exist (C-74.8) — the act is no oracle",
