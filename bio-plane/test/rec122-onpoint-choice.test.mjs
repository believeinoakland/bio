...
/* CORRECTED by D-575: this compared the WHOLE answer byte for byte, and that was wrong in one field —
   `determining_pair.selection.says` said "which nobody has chosen" beside the member's choice, the record
   contradicting itself. The sentence now names the choice, so the golden is compared with `says` set aside
   (`sansSays`), and the next assertion pins that the sentence moved to name the chooser. */
const sansSays = (g) => JSON.stringify(g, (k, v) => (k === "says" ? undefined : v));
t("the WHOLE of A answers exactly as before the choice (no part named = every mention is inside), the pair's sentence aside",
  sansSays(gWhole), sansSays(gWholePre));
t("…and the pair's sentence on the chosen connection names the member's choice instead of 'nobody has chosen' (D-575)",
  (gWhole.reaching || []).map((e) => /a member \(ruth\) chose ordinance:13579-amended \(read at p\.9\)/.test(e.determining_pair?.selection?.says ?? "")).includes(true), true);
...
/* CORRECTED by D-575 (see section 3's whole-document assertion): B's portion answers stay byte-identical
   in everything but the pair's sentence, which now states the choices made on A and C's ends. */
t("page 5 of B: byte-identical to its answer before any choice, the pair's sentence aside", sansSays(await grade(B, 5)), sansSays(gB5pre));
t("page 3 of B: byte-identical to its answer before any choice, the pair's sentence aside", sansSays(await grade(B, 3)), sansSays(gB3pre));
