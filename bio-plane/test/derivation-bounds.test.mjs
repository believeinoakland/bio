/* NEGATIVE CONTROL: RE-RUN 2026-09-24 by D-559's worker, `cd bio-plane && node test/nc-rec66.mjs`, after re-anchoring arms 1
   and 2 (FW-17/REC-120 had added `ref` to the scan, so the old anchor matched 0 times and the driver threw before arm 1).
   Every arm ARMED, each ALONE, every restore sha256 + cmp MATCHES. BASELINE derivation 72/0, meaning 96/0, bounds 207/0.
   (1) unbounded derivation: derivation 54/18, meaning and bounds green. (2) the naive fix: derivation 58/14, siblings
   green. (3) taint propagation broken: 64/8. (4) walk neutered: 59/13. (6) fixture unarmed: 62/10. The siblings staying
   green under (1) and (2) is REC-66's recorded finding, reproduced. */
...
/* 24 -> 25, 2026-09-23 by REC-170 — an ARRIVAL, and a real one: `#frozenPairsByCase` (store.mjs, beside
   `#frozenFromPinningDocuments`) reads every pinned roster row UNBOUNDED and then opens each pinning case
   document (`#caseDocMemberFrozen`, one read per case edition, cached) for every sha SEVERAL editions pin.
