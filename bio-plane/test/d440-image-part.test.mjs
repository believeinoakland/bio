/* NEGATIVE CONTROL: RUN 2026-09-23 by the D-440 worker, two arms, each editing ONE line of `checks/bio-checks.mjs` ALONE, declared before running, restored from a uniquely-named pristine copy verified by sha256 AND cmp (836,175 B, sha256 01a45ac60c58…, byte-identical after each). BASELINE 19 pass / 0 fail. (a) `nocontainer` — the row's arm — delete `if (notContainer) return refusal("CONTENT_EXTENT_NOT_A_CONTAINER", notContainer);`: DECLARED 6 must fail (section 1's four, section 5's op=cite refusal, section 6's first PURE row), 13 must hold — RAN 13 pass / 6 fail, exactly those six, the HTML arm failing BY NAME ("the image served BESIDE the page ... is REFUSED C-45.11"). (b) `liar` — the liar's arm, refusing EVERY `{part}` — `partOutsideAnyContainer`'s guard reduced to `if (!container) return null;`: DECLARED 8 must fail (section 2's two, section 3's two, section 4's two, section 5's DOCX cite, section 6's first PURE row), 11 must hold — RAN 11 pass / 8 fail, exactly those eight, so a fence that refuses every part cannot pass this suite. */
