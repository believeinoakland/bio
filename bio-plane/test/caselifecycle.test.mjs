...
/* NEGATIVE CONTROL — RUN 2026-09-10 (case4-lifecycle-flag). SEVEN ARMS PLUS A BASELINE, each armed ALONE with every other defence held open, each RUN, and each restore verified by CONTENT and by sha256 against a uniquely-named per-arm pristine copy taken INSIDE THIS WORKTREE (store.mjs 1,764,229 bytes, sha256 496745342a0c8726…; this file's own sha is not quoted, because a file cannot state its own). The driver is `test/caselifecycle.control.mjs` — COMMITTED, so every arm re-runs in one step with `node test/caselifecycle.control.mjs [arm]`. WHOLE (baseline, nothing armed) = 60 pass, 0 fail.
   ...
   (d2) OVER-STRICTNESS, THE REALISTIC SHAPE — ONE guard translated from the state word to `concluded` instead of to the case relation -> **59 pass, 1 FAIL**, and it is EXACTLY the over-strictness arm: `and the op agrees: restructuring a finding in no case is NOT refused by the published fence`. **(d) and (d2) are both kept.** (d) proves the widening is caught catastrophically; (d2) proves block 4's over-strictness arm is what catches the shape a worker would actually produce — every refusal in block 4 still fires under it, so a suite asserting only that the guards bite would have read a fence around the whole corpus as a stronger plane.
   (e) THE SUITE'S OWN ANCHOR — break block 9's design-document parse so the CASE-4 bullet is never found -> **56 pass, 4 FAIL**, and the FIRST is the anchor itself: `the design document is readable and its CASE-4 bullet was found`. The three clause arms fail behind it rather than passing over an empty string, which is what that arm is for. */
