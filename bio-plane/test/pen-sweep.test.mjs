...
/* MOVED 20 -> 21 by D-546 (2026-09-25), BY NAME, never as slack: `d546-state-edge.control.mjs` is D-578's driver derived
   verbatim (read here: the same plane-root READ and `dirname` of it; every write goes to a `mkdtempSync(join(tmpdir(), …))`
   mirror, removed after the arm; the real sources are hashed before and after). The walk resolving that READ is the row
   that lets this fall again. */
t("the estate's UNCLASSIFIED drivers have not grown past what M0-182 left, plus the copy sources of D-510, D-526, D-547, D-548, D-563, D-578 and D-546 (<= 21)", g("UNCLASSIFIED").length <= 21, true);
