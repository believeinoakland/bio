 ...
 *
 * WHICH REFUSAL A CALLER MEETS when an earlier fence also applies is D-526's, and `d526-refusal-order.test.mjs`
 * drives it. This note read, until D-526, that the fences above the refusal still read the envelope and that
...
/* CORRECTED BY D-526, not exempted: this arm ran under the MEMBER_TOKEN bearer, a MACHINE credential, and passed
   only because D-85's surfacing gate asked the ENVELOPE's type and so skipped a document that IS a question. Since
   D-526 every fence asks the document's type, so that machine meets SURFACE_NO_RUN — the refusal the same question
   correctly labelled meets, which is D-526's rule. The arm's subject is D-510's refusal for an inquiry, so it now
   runs under a member SESSION, which no surfacing gate asks. */
const idiv = await promote(IDIV, inquiryMd(IDIV), { metaType: "information", state: "open", token: MEMBER });
