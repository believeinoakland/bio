...
const pkg = (state, n, extra = []) => {
  ...
  return {
    /* CORRECTED 2026-09-23 (REC-176, IC-193), never exempted: ONE literal key served every promote here, so the
       revision (rev 3) REPLACED the creation's manifest row and the suite asserted the survivor. A key is now one per
       promotion, `…10000<n>Z`; the revision's snapshot of rev 1 sits under rev 3's key. */
    bundleId: "INFO-2026-0001-x", snapKey: `20260723T10000${n}Z_aaaa1111`, author: "claude",
...
t("history snapshot present for the superseded rev", "_history/bundle_20260723T100003Z_aaaa1111.md" in img, true);
...
t("the verbatim promotion record is projected", "_history/promotion_20260723T100003Z_aaaa1111.json" in img, true);
...
t("history holds the prior revision, not the stale one", /rev 1/.test(img["_history/bundle_20260723T100003Z_aaaa1111.md"]), true);
{
  const rec = JSON.parse(img["_history/promotion_20260723T100003Z_aaaa1111.json"]);
  ...
  t("manifest entries use the catalog's kind vocabulary", man.entries.map((e) => e.kind), ["promotion", "promotion"]);
  t("and record what was snapshotted", man.entries.find((e) => e.key === "20260723T100003Z_aaaa1111")?.snapshotted.includes("bundle.md"), true);
}
