/* NEGATIVE CONTROL: (run 2026-07-31) overclaim the capture grade in the acquire path (index.mjs: a direct fetch stamps "A" instead of "B") -> 1 assertion fails (the load-bearing "acquire says B, a Worker cannot produce a grade-A capture"); restored, 72 pass. (run 2026-08-04, REC-48) publish op=acquire's `note:` as a hand-typed literal copy instead of the composed `ACQUIRE_GRADE_NOTE` -> THIS SUITE STAYS GREEN and only hygiene.test.mjs's structural sweep fails; then move the enforced ceiling to C and 5 assertions here fail naming the note that still says B. The three arms are below IN THIS SAME DECLARATION, each RUN.
   ...
   D-698's TWO ARMS (run 2026-09-25, worker D-698), each broken ALONE and restored by cp from a per-arm pristine copy in the session scratchpad, verified by sha256 AND cmp AND size. Whole = 97 pass. (i) THE ITEM'S OWN — hand-type the archive letter back: `? ARCHIVE_CAPTURE_GRADE :` -> `? "C" :` in src/index.mjs -> 96 pass, 1 FAIL, "the ARCHIVE-SOURCED arm names the RULED constant ARCHIVE_CAPTURE_GRADE and spells no letter of its own"; inquirystrength.test.mjs STAYS 84/0, because a copy that agrees today satisfies every value pin at zero cost — which is why the structural pin is here (restored 887,852 bytes, sha256 de0555c652536696, cmp identical). (h) THE ROW'S DECLARED CONTROL — the EXPORTED letter moved: `ARCHIVE_CAPTURE_GRADE` typed "D" in checks/bio-checks.mjs, through `node test/nc-d177.mjs h` -> THIS suite 96 pass, 1 FAIL, "the archive letter op=acquire stamps is the RULED one, C … got \"D\"", AND inquirystrength 81/3 in the same run, 9d reading `earned:"D", stamped:"D"` — the stamp and the measurement moved TOGETHER, AS DECLARED (restored 1,007,326 bytes, sha256 39b3f9809967fd38, cmp identical). */
...
import { EARNED_CAPTURE_CEILING, UNREACHABLE_CAPTURE_GRADE,
         BASIS_GRADES, ARCHIVE_CAPTURE_GRADE } from "../checks/bio-checks.mjs";
 ...
 *
 * D-698, 2026-09-25: THE NEXT TWO PARAGRAPHS ARE REC-50'S, KEPT AS ITS RECORD,
 * AND SUPERSEDED — BOB #35 has since RULED the archive letter and it is now the
 * exported ARCHIVE_CAPTURE_GRADE; the assertions below say so, and why the old
 * one was wrong is at the assertion.
 *
 ...
 * in place of a constant it must not write. */
{
  ...
  /* D-698 · CORRECTED, NOT EXEMPTED. REC-50 asserted here that the archive arm
     "is still a TYPED letter, which is OPEN BY DECISION" — true on 2026-08-04,
     when no ruling named what an archive-sourced capture earns. It became FALSE
     on 2026-09-25: BOB #35 (07:55Z) ruled it — one rank below the direct
     ceiling, measured — and D-693 derived that letter as ARCHIVE_CAPTURE_GRADE.
     From then the old assertion was REQUIRING a second, typed copy of a ruled
     value: it would have failed the very change that closes the duplicate. So it
     now pins the opposite, the same way the direct arm is pinned: the stamp
     spells no letter, it names the ruled constant, and that constant comes from
     the catalogue the store's measurement also reads. */
  t("the ARCHIVE-SOURCED arm names the RULED constant ARCHIVE_CAPTURE_GRADE and spells no letter of its own",
    [archiveArm, /^"[A-Z]"$/.test(archiveArm)], ["ARCHIVE_CAPTURE_GRADE", false]);
  t("and index.mjs takes ARCHIVE_CAPTURE_GRADE from the catalogue, where the store's measurement reads it too",
    /import \{[\s\S]*?\bARCHIVE_CAPTURE_GRADE\b[\s\S]*?\} from "\.\.\/checks\/bio-checks\.mjs"/.test(indexSrc),
    true);
  /* THE TODAY-VALUE WITNESS for the ruled letter: BOB #35 named C. If the
     constant moves (or the ceiling it is derived from moves) this fails BY NAME,
     so the move is made deliberately against the ruling rather than silently. */
  t(`the archive letter op=acquire stamps is the RULED one, C (BOB #35, 2026-09-25) — got ${JSON.stringify(ARCHIVE_CAPTURE_GRADE)}`,
    ARCHIVE_CAPTURE_GRADE, "C");
  t(`and it ranks strictly BELOW the enforced ceiling ${JSON.stringify(EARNED_CAPTURE_CEILING)} — grade tracks directness (RULED)`,
