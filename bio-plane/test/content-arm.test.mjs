...
console.log("\n--- 6. rows=content: the grain, and `cited`/`chain_last` ---");
{
  ...
  /* `chain_last` IS THE WORKERD PIN. CORRECTED BY REC-104: it no longer parses at
     read — it and the filter both read `content.chain_kind`. CORRECTED AGAIN BY
     D-686: that column is no longer GENERATED from the chain's last step; it is
     the kind of the last derivation step covering the UNIT's page, written at
     mint by `textchain.mjs` `chainKindFor`. It is still taken THROUGH the op,
     inside workerd, which is the only place it counts. */
  ...
  /* REC-104 — THE COLUMN IS STILL TRUE, CROSS-READ RATHER THAN SELF-CONFIRMED.
     ...
     chain that read returns. It is what shows the column is not merely faster but
     still describes the chain it claims to. D-686: the column is now the last step
     covering the unit's PAGE, which equals the chain's last step on every row of
     THIS fixture — no document here is mixed — so the witness is unchanged; the
     mixed case, where the two differ by design, is `content-chain-kind.test.mjs`
     section 3, and its negative control is `nc-d686.mjs` (REC-104's `firststep`
     arm broke a generated column that no longer exists). */
