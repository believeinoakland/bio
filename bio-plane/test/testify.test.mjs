/* NEGATIVE CONTROL: RUN 2026-09-18 with `node test/nc-mk1.mjs [arm]` from `bio-plane/`, every arm ALONE with the others held open, each EDITING A REAL SOURCE and restored from a uniquely-named per-arm pristine copy verified by sha256 AND by content (store.mjs 2,455,143 B, index.mjs 614,575 B, bio-checks.mjs 747,630 B; 10 of 10 restores byte-identical; never `git checkout --`). Declared BEFORE arming, and the result: (a) `baseline` — nothing armed, MUST be green: 45/0. (b) `unearned` — THE LIAR, the fence stops refusing a document that CLAIMS authored without the testimony path: "THE LIAR" and "ANY truthy spelling" MUST FAIL — 42/3, the forged bundle LANDS (the third failure is "nothing landed"). (c) `hijack` — a register entry re-filing an authored observation's bytes under another bundle: MUST FAIL — 40/5; the re-filing then moves the register row, so the observation's own later revision and its capture-axis entry fail too, which is the harm the refusal prevents, measured. (d) `origin` — §7's first refusal removed: both C-53.7 arms MUST FAIL — 43/2. (e) `dropped` — both C-53.9 sites removed: both MUST FAIL — 43/2. (f) `stamp` — THE AUTHOR STAMP REMOVED at the control plane: "THE STAMP" MUST FAIL — 8/19, and the failure is the one declared rather than a cascade standing in for it: re-driven by hand with a cp-aside/cp-back of index.mjs (sha 017d8d3e… before and after, `cmp` same), the stamp assertion read got [true,"mallory"] — a caller-supplied author LANDED; the other 18 are every unstamped call arriving with no author and being refused C-53.1. (g) `supplied` — C-53.2 removed: both body-author arms MUST FAIL — 43/2, while THE STAMP stays green (the store reads the stamp). (h) `machine` — C-53.1 removed: the machine-credential arm MUST FAIL — 44/1. (i) `axis` — an authored capture counted as a capture again: the capture-axis arm and the B-leg refusal MUST FAIL — 41/4, the leg claiming B on a member's own words is ACCEPTED under the arm. (j) `c181` — C-18.1's authored arm removed: the whole-catalogue arm and the graded arm MUST FAIL — 42/3. (k) `overstrict` — THE OVER-STRICTNESS DIRECTION, every member-origin document treated as a claim: the member-UPLOADED and `authored: false` arms MUST FAIL while THE LIAR stays green — 41/4, as declared. EVERY ARM AS DECLARED.
 *
