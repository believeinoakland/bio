...
/* NEGATIVE CONTROL: RUN 2026-09-25 (WORKER D-679), ONE arm for this file's D-679 change (detector C follows a Response bound to a name). Baseline 62/0. ARM: `src/index.mjs` replaced by its text at land/worker/D-629 @ 5e202b33 (the four public doors' bare `json(await r.json(), 200)` reads restored) -> **61/1**, the ONE failure DETECTOR C's set arm, got set = the stated ten + claim, enroll, invitelook, login. Before this change the same source passed this file green, which is the blindness D-679 found. Restored from a pristine copy verified by sha256 AND cmp (891382 B, 9c31248a…). The follow's own over-strictness and reach are the synthetic D-679 arm (bound-then-read seen; fire-and-forget, a status-only read and a sibling name not). */
...
function doFetchSites(src) {
  ...
  for (const f of callSpans(src, "\\.fetch\\(")) {
    ...
    /* D-679, AND IT IS WHY FOUR PUBLIC DOORS WERE NEVER SEEN: "consumed" meant only `).json()` CHAINED onto the call,
       so `const r = await stub.fetch(...); return json(await r.json(), 200);` — the Response BOUND TO A NAME and read a
       statement later — counted as fire-and-forget and left this sweep. That shape was op=claim, login, invitelook and
       enroll, each relaying a store failure at HTTP 200. A binding is now followed: `const|let|var <n> = await
       <receiver>.fetch(...)` is consumed when `<n>.json(` follows within the next 600 characters.
       ITS LIMIT, STATED: a binding read further away than that, or passed to another function that reads it, is not
       followed; the D-679 sweep over this file (worker report) found no DO fetch bound to a name outside these four. */
    const bound = /(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*await\s+$/
      .exec(src.slice(Math.max(0, f.start - 240), f.start).replace(/[A-Za-z_$][\w$.]*$/, ""));
    const rawConsumed = /^\)?\s*\.json\(\)/.test(src.slice(f.end, f.end + 24))
      || (!!bound && new RegExp(`(?<![\\w$.])${bound[1].replace(/\$/g, "\\$")}\\.json\\(`).test(src.slice(f.end, f.end + 600)));
...
/* D-679 · DETECTOR C's BINDING-FOLLOW, DRIVEN BOTH WAYS over a synthetic set, because a follow that sees nothing
   passes the set assertion above exactly as one that works: the bound-then-read shape IS seen (the four public doors'
   old spelling, and a `let` with a `$` name), fire-and-forget and a status-only read are NOT, and a SIBLING name that
   merely ends in the bound one is not taken for it. */
t("D-679 — detector C follows a Response BOUND TO A NAME to its later `.json()`, and nothing else",
  [["const r = await stub.fetch(new Request(\"http://do/zza\", { method: \"POST\" }));\n        return json(await r.json(), 200);", 1],
   ["let $r = await invStub.fetch(\"http://do/zzb\");\n  const b = await $r.json();", 1],
   ["await stub.fetch(\"http://do/zzc\");\n  return json({ ok: true }, 200);", 0],
   ["const r = await stub.fetch(\"http://do/zzd\");\n  return json({ ok: r.status === 200 }, 200);", 0],
   ["const r = await stub.fetch(\"http://do/zze\");\n  const s = await hr.json();", 0]]
    .map(([code]) => doFetchSites(code).filter((x) => !x.viaDoAnswer).length),
  [1, 1, 0, 0, 0]);
