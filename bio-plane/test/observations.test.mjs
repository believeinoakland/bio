...
t("A1: `observations` is declared in schema.mjs",
  /CREATE TABLE IF NOT EXISTS observations\s*\(/.test(SRC_SCHEMA), true);
...
t("A2: and it is declared BEFORE the host_governor block (the standing schema rule)",
  SRC_SCHEMA.indexOf("CREATE TABLE IF NOT EXISTS observations")
   ...
   wrote. PL-1 cost fifteen minutes and three comments to the semicolon. */
{
  ...
  const j = SRC_SCHEMA.indexOf("CREATE INDEX IF NOT EXISTS observations_tally");
...
for (const [name, cols] of [["observations_frontier", "level, subject_kind, subject, seq"],
                            ["observations_authority", "authority_kind, authority, seq"],
                            ["observations_tally", "level, state, seq"]])
  t(`A6: index ${name} exists on exactly (${cols})`,
    SRC_SCHEMA.includes(`CREATE INDEX IF NOT EXISTS ${name} ON observations(${cols})`), true);
...
{
  const inserts = [...SRC_STORE.matchAll(/INSERT INTO observations\b/g)].length;
  t("B1: there is EXACTLY ONE `INSERT INTO observations` in store.mjs outside the migration",
  ...
  t("B2: and the second is the FOLD's one-time copy inside #migrate, not a second writer",
    /PRAGMA table_info\(ai_run_log\)[\s\S]{0,1200}INSERT INTO observations/.test(SRC_STORE), true);
  ...
  t("B3: nothing UPDATEs the table — append-only is a property of the code, not a promise",
    [...SRC_STORE.matchAll(/UPDATE observations\b/g)].length, 0);
  t("B4: and exactly one DELETE, which is the whole-store purge arm",
    [...SRC_STORE.matchAll(/DELETE FROM observations\b/g)].length, 1);
}
...
console.log(`\nobservations: ${pass} pass, ${fail} fail`);
