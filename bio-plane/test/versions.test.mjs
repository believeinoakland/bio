/* NEGATIVE CONTROL: (run 2026-08-07, pl1-agent, IS-BUILD-PLAN PL-1 / IS-1. SIX arms, each broken ALONE and restored BYTE-IDENTICALLY — every file's sha256 compared before and after every arm; the whole suite passes when the tree is whole. THE HARNESS ITSELF LIVES IN THIS WORKTREE and never in a shared scratchpad, because a worker's harness was overwritten mid-turn by a concurrent worker on 2026-08-07 and a harness silently replaced between ARM and RESTORE reports a restore it never performed; each restore below was verified BY CONTENT as well as by hash.)
   (1) THE ITEM'S FIRST NAMED CONTROL — EDIT A FROZEN VERSION IN PLACE. In src/store.mjs promote's basis-version arm, replace `if (!prior || prior.composition === v.composition) continue;` with `continue;` -> the freeze stops existing. RESULT: 9 FAIL. The headline is block 3's, measured: re-promoting `opening account` with one word of its description changed is ACCEPTED (`[false,"VERSION_FROZEN"]` -> `[true,null]`), the stored composition MOVES underneath a reader who already has it, and block 2's byte-freeze arm — which compares the version's every published field before and after an unrelated later promotion — fails naming `description` as the field that moved. The C-number, the DEC-49 code and the canned translation arms fail with it, and the `changed` field that names WHICH field moved is absent.
   (2) THE ITEM'S SECOND NAMED CONTROL — STRIP THE RELATIONSHIP FIELD. In checks/bio-checks.mjs basisVersionFindings, replace `if (!VERSION_RELATIONSHIPS.includes(rel)) {` with `if (false && !VERSION_RELATIONSHIPS.includes(rel)) {` -> 5 FAIL at BOTH gates: op=promote ACCEPTS a version that states no relationship (`[false,"BASIS_VERSION_REFUSED"]` -> `[true,null]`), the catalog finds nothing wrong with the same bytes, the version lands with `relationship: ""` and the composed answer is the flat implicit-AND basis REC-42 corrected — re-shipped inside the object built to end it. The DEC-49 completeness arm fails too, because VERSION_NO_RELATIONSHIP becomes unreachable and the driven code set no longer equals the registry's.
   (3) THE TRAP, PINNED RATHER THAN TRUSTED — ADD A SECOND VERSION TABLE AND A SECOND WRITE SITE. Add `CREATE TABLE IF NOT EXISTS inquiry_basis_versions_shadow (bundle_id TEXT, name TEXT);` before the host_governor block in src/schema.mjs and an `INSERT INTO inquiry_basis_versions (bundle_id,name,ord,description,relationship,state,derived_from,hidden,claim,run,author,at,regroup_by,regroup_at,regroup_note,composition,leg_count) VALUES ('x','x',0,'x','and','suggested',NULL,0,NULL,NULL,NULL,NULL,NULL,NULL,NULL,'x',0)` anywhere else in src/store.mjs -> 3 FAIL, naming both: "ONE WRITE SITE" reports 2 for inquiry_basis_versions, and the table-set arm reports the shadow table. NOTE the shape of the fix this control forces: the walk counts over COMMENT-STRIPPED source and is guarded BOTH WAYS (block 12 asserts the stripper removed something AND that a known statement survived), because a walk that silently stripped everything would report ONE write site over an empty corpus, which is the ceiling-without-a-floor failure REC-70 measured.
   (4) D-113 — A NEW DERIVED TABLE ABSENT FROM `purge`. In src/store.mjs, remove `"inquiry_basis_versions"` from purge's TABLES list -> 2 FAIL here (the per-bundle purge leaves the versions standing while reporting it took the bundle, and `op=stats` still counts them) AND hygiene.test.mjs's D-113 sweep fails naming the table, which is the instrument that would catch it on a clean tree. Both are kept: this suite proves the CONSEQUENCE, hygiene proves the OMISSION.
   (5) THE PRUNE OFFER MADE TO DELETE RATHER THAN HIDE. In src/store.mjs basisVersions, add ` AND hidden = 0` to both the `total` count and the rows SELECT -> 4 FAIL in block 8: a hidden version stops being reachable through the op, `total` drops, and the arm asserting the acts persist fails. D-214's rule is that the rejection PATTERN is queryable only if the acts persist, and DEC-29(b)'s wording promise is worthless if the query stops answering.
   (6) §14b.7 — KILL THE PROPOSING RUN. This one is run the other way round and is the acceptance rather than a break: the run that proposed a version is DELETED out of ai_runs, ai_run_bounds and ai_run_log through the probe door, op=airun then answers that no such run exists, and the version must read back BYTE-IDENTICALLY with its run still named. To make it a real control the arm ALSO breaks it: adding `JOIN ai_runs ar ON ar.run = v.run` to basisVersions' row read makes the version vanish with its run -> 5 FAIL, every one of them naming a version that outlived its run and then did not. */
