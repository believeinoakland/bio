...
async function seed(mf) {
  ...
  const promote = (id, text, type, reading) => {
    ...
    return post("promote", { bundleId: id, base: null,
      ...
      /* CORRECTED at c22-batch30 (CONDUCT #22), never exempted: this item was cut before D-563, whose C-86.3 refuses
         an envelope title the held document contradicts (`Bundle <id>` against the document's own `title:`); the
         envelope title is dropped as D-563 dropped it in its own fixtures, and promote derives it from the document.
         The envelope dates already equal the documents' own, so D-615's C-86.7 is not met. */
