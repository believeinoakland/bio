...
console.log("\n--- I. the class: reads whose failure is swallowed, pinned as a ratchet with its reach ---");
{
  ...
  /* MOVED 39 -> 40 by D-533 (2026-09-24, land/worker/D-533), FROM THE FIGURE THIS WALK PRINTED on this item's own
     tree: `origin/main` @ 9f8b69e6 prints 39, this branch prints 40. THE ONE NEW SITE IS THIS ITEM'S OWN AND WAS
     LOOKED AT:
     - D-533, `#partsNamedFor`: the register audit reads the holding bundle's `data/provenance.json` to learn which
       PARTS the record names for a capture held in parts. A register document that does not parse is returned as
       the stated state `unreadable` with its reason, and the control plane counts that row UNDETERMINED, outside
       `sound` (BOB #33's ruling of 21:17Z) — never as held, and never as unbacked. The class's remedy shape: a
       stated undetermined, never a normal-looking answer. A throw would take the whole audit down over one bundle's
       file, which is the blast radius REC-212's site narrowed for the same reason. */
  /* MOVED 39 -> 40 by D-525 (2026-09-24, land/worker/D-525), FROM THE FIGURE THIS WALK PRINTED on this item's own
     tree: `origin/main` @ 9f8b69e6 prints 39, this branch prints 40. THE ONE NEW SITE IS THIS ITEM'S OWN AND WAS
     LOOKED AT:
     - D-525, `driveShells`: the Drive shell sweep parses each Drive-linked bundle's `data/provenance.json`. A
       register that will not parse is NAMED in the answer's `unreadable` list with the reason "the register is not
       parsable JSON" and is not judged — never scored `no_baseline`, which would read as "nothing was captured"
       when the truth is "the record could not be read". The class's remedy shape: stated, not smoothed.
       WHY A SWALLOW AND NOT A THROW: the sweep walks a page of bundles, and one unreadable register must not take
       down the answer for every other bundle on the page. */
