...
/* Pristine copies live INSIDE this worktree, in a DOT-directory so neither the
   battery's discovery nor the fleet walk enrols what it holds — and never in
   the shared scratchpad, which is NOT isolated between sessions. */
const SAFE = join(REPO, ".coff11-control-pristine");
