/* NEGATIVE CONTROL: SIX arms, declared here and run in one step by `node test/skillsequencing.control.mjs [arm]` from `bio-plane/` — deliberately NOT a `.test.mjs`, because it EDITS REAL SOURCES while it runs and the battery must not discover it (`suggest.control.mjs`'s precedent, SK-2's and SK-3's shape). Each arm is armed ALONE with the others held open, each DECLARES BEFORE IT RUNS what must fail AND what must stay green, and every restore is verified BY CONTENT as well as by sha256 against a uniquely-named per-arm pristine copy. The arms and their measured results are recorded here after the run:
   ...
   (4) A MODE ADDED TO THE HARNESS AND NOT RECORDED — a third `MODES` entry lands in FL-3's table -> 24 pass, 2 FAIL: ARM B3 (the set, held in BOTH directions — the one-directional form "every recorded mode exists in the table" is blind to this by construction) and ARM B4 (the partition moves with it). **SINCE 2026-09-14 THE TABLE HOLDS THREE ROWS AND THE ARM INJECTS A FOURTH** — FLEET landed `extract` (not deployed) on SK-8's delegation and RECORDED it in `order` in the same commit, which is exactly the move this arm exists to make impossible to do silently; the arm's own edit is unchanged and still measures an UNRECORDED addition.
