...
console.log("\n--- 1. an anonymous caller reads the case, and NOTHING of the working record ---");
{
  ...
  /* IC-22 (UI-40), 2026-08-05 — `opened` IS NO LONGER PUBLISHED BY THIS OP.
     It was the instant the case edition was opened and nothing anywhere read
     it: re-measured across the whole repository at the proposal, zero consumers
     in `civicos-ui`, `newgroup`, `docprofile`, `pdf-worker` and `tools`, and not
     one assertion in this battery. `ratified_at` is what the surface renders and
     what the record can stand behind.
     ASSERTED AS ABSENT FROM THE KEY SET, not as undefined-valued: `c.opened ===
     undefined` is ALSO true of an answer that never had the key AND of one that
     has it set to undefined, so `in` is the test that distinguishes removed from
     blanked. That distinction is the whole of REC-41's precedent — the field is
     GONE, so nothing can re-expose it and no caller can tell one was computed.
     THE ASSERTION IS THROUGH THE OP, over the real control plane, because a
     source-level check would pass on a field the wire still carried by spread:
     `index.mjs` answers `{ ok: true, ...c, findings, verification }` and NEVER
     names this key, so the wire is the only place the removal is visible. */
  t("IC-22: `opened` is NOT PUBLISHED — removed from the answer, not blanked (the key is absent)",
    ["opened" in c, "ratified_at" in c], [false, true]);
...
console.log("\n--- D-549: C-68.5, the published store absent, at both public ops ---");
{
  ...
  /* ONE SITE, graded on the SOURCE, because the wire alone cannot show it: `json()` decorates every top-level
     ok:false with its catalogue row by code (dec49Attach, D-262), so a second literal minting this code at
     publishedbytes would STILL answer translated — measured by this item's control arm 1. What that second site
  ...
  console.log(`  (D-549 one-site pin: ${at.length} quoted occurrence(s); region at ${open}..${close})`);
  t("D-549: the code is minted at ONE governed site — every quoted occurrence inside is-published-store-absent",
    [open > 0 && close > open, at.length > 0, at.filter((i) => i < open || i > close).length], [true, true, 0]);
