...
let snapKeySeq = 0;
/* CORRECTED 2026-09-25 at the c23-batch30 union (CONDUCT #23), never exempted: this item was cut over REC-201, before
   D-563, whose C-86.3 refuses an envelope title the held document contradicts (`Bundle <id>` against the document's
   own `title: "Records request <id>"`) — every promote here answered ENVELOPE_TITLE_DISAGREES before reaching the
   fence. The envelope now carries the document's OWN title, read from its front matter; its dates were already the
   document's own (NOW / LATER). */
...
const promote = async (tok, id, text, base = null) =>
  rP(await POST(`op=promote&token=${tok}`, {
    ...
    register: [],
    meta: { object_type: "action", group: "believe-in-oakland", title: ownTitle(text) ?? `Bundle ${id}`,
...
console.log("\n--- 3. the fence, by name: a machine states no law ---");
{
  ...
  const m = await POST(`op=promote&token=${AI_TOKEN}`, {
    ...
    register: [], meta: { object_type: "action", group: "believe-in-oakland", title: ownTitle(actionMd(CPRA, { kind: "cpra_request" })),
                          current_state: "planned", created: NOW, last_updated: LATER } });
