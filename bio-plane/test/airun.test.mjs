...
console.log("\n--- ARM D · DEC-49: a code with a canned translation, read from ONE place ---");
{
  ...
  /* CORRECTED A FIFTH TIME 2026-09-19 BY REC-152, on the same terms: C-22.12 (`AI_RUN_NOT_PRINCIPAL` — tick
     and close are the run's PRINCIPAL's acts, Membership v2 §7, BOB #16), a fact about the run (whose it is).
     REC-152 was built beside REC-153 with the gap for C-22.11 left deliberately; the two were combined at their
     integration (CONDUCT #6), so the family holds TWELVE. */
  /* CORRECTED A SIXTH TIME 2026-09-23 BY REC-169, on the same terms: the old list was right for its day and is wrong
     now because the family grew, not because it was mis-stated. C-22.13 (`AI_RUN_CONSUME_INVALID` — a figure written
  ...
  /* CORRECTED AN EIGHTH TIME 2026-09-23 BY REC-177, on the same terms: C-22.16 (`AI_RUN_BOUND_NO_ALLOWANCE` — a bound
...
console.log("\n--- ARM PQ · D-451: a project run publishes the questions it confirmed-cites ---");
{
  ...
  const openRun = async (contextId, contextType) => {
    ...
    return r?.started === true ? run : (console.log("open:", JSON.stringify(r).slice(0, 400)), null);
  };
