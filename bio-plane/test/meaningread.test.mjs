...
console.log("\n--- 13. an equally correct phrasing must PASS, and the grain is published ---");
{
  ...
  /* ------------------------------------------------------------------ D-258
   * THE DECISION, DRIVEN THROUGH THE TWO OPS IT RESTS ON RATHER THAN ASSERTED
   * ABOUT THEM. `plan.meaning` carried `columns` and `refs` that nothing read;
   * they are deleted, and the reason deleting them loses a surface NOTHING is
   * that `op=searchfields` already answers the same question, per arm, from the
   * same registry. If that stopped being true, deleting them would have been
   * wrong — so it is pinned HERE, at the op, and not left to the comment. */
  t("D-258: `op=searchfields` publishes the row COLUMNS per arm — the fact the deletion rests on",
    Object.fromEntries(Object.entries(sf?.meaning ?? {}).map(([a, v]) => [a, v.rows.columns])),
    Object.fromEntries(Object.entries(meaningVocabulary()).map(([a, v]) => [a, v.rows.columns])));
  t("D-258: and the REFS — which columns of a row name another bundle — for every arm, including the empty ones",
    Object.fromEntries(Object.entries(sf?.meaning ?? {}).map(([a, v]) => [a, v.rows.refs])),
    Object.fromEntries(Object.entries(meaningVocabulary()).map(([a, v]) => [a, v.rows.refs])));
  t("and it is not vacuous: at least one arm publishes a NON-EMPTY refs list",
    Object.values(sf?.meaning ?? {}).some((v) => (v.rows.refs ?? []).length > 0), true);

  /* THE ENVELOPE ITSELF, KEY BY KEY. A deletion from a plan is only safe if the
     published answer did not change, and the only way to know that is to read
     the answer. This is also the pin a future author adding a field to this op
  ...
  /* The rows already carry their own column names as keys, which is the second
     half of why `columns` on the envelope would have restated the answer. */
  t("a returned row's own keys ARE the arm's published columns, so the envelope would only restate them",
    (env?.rows ?? []).length > 0
      && meaningVocabulary().leg.rows.columns.every((c) => c in env.rows[0]), true);
}
