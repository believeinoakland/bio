...
/* RE-ANCHORED 2026-09-25 at the c23-batch30 union (CONDUCT #23), never exempted: D-615 (merged beside D-674) derives
   manifest.created from the DOCUMENT's last_updated, so both promote INSERTs now read `promotedLastUpdated || new
   Date().toISOString(),` (the revision's with a trailing D-615 comment, and `files.map(` spelled two ways), and the old
   anchor `meta.last_updated || …\n          JSON.stringify(files.map(` matched 0 times — the arm did not arm. The
   anchor is the value expression alone, still exactly the two INSERTs (n: 2); the break is unchanged: the plane's
   clock in place of the writer's date. */
const CLOCK_SITE = "promotedLastUpdated || new Date().toISOString(),";
