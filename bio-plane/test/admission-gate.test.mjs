/* NEGATIVE CONTROL: (run 2026-08-09, REC-79) `civicos-ui/test/refusal-partition.control.mjs` (CORRECTED 2026-09-21 by D-355: this line named `bio-plane/test/admission-gate.control.mjs` from REC-79's landing on, a path that exists in no commit) — five arms, each armed ALONE against a uniquely-named pristine copy, restore verified by sha256 AND by `cmp`. (1) blank ONE row's translation in ADMISSION_CHECKS -> `admissionRow` throws and the arm for that code fails; (2) return the row WITHOUT its translation (`{ code, check }`) -> every translation assertion fails while every `reason` assertion still passes, which is the exact shape DEC-49 exists to prevent and the reason `translation` is asserted separately from `reason`; (3) make the two NOT_CAPABLE sites disagree by hand-writing a second wording at the project-creation site -> the same-sentence arm fails; (4) revert app.html's `a.translation` line so the surface re-invents wording -> the UI arm fails; (5) OVER-STRICTNESS — a correct refusal whose translation is written in an unanticipated voice must still PASS. CORRECTED 2026-09-21 BY D-355, NEVER EXEMPTED: arm (2) as written was true in REC-79's own tree and stopped being true at the merge `108020ce` (2026-08-09), where D-262's chokepoint in `json()` began backfilling a missing translation from its catalogue row — so a row returned WITHOUT its translation no longer drops it from the wire, this suite stays green, correctly, and the driver's arm read declared 1, actual 0 from that merge on (M0-29 measured it 2026-09-14). The arm now ALSO neuters that backfill and must fail at the CANNED TRANSLATION assertion by name; the driver's appended arm 10 drives the old single-route break alone and must stay green. RE-RUN 2026-09-21 by D-355: 19 of 19 sub-checks as declared over 11 announced arms, all 11 anchors live, exit 0, the pen absent after. */
 ...
 * here still passes while every sentence assertion fails. (Since 2026-08-09 the
 * helper is not the only route: D-262's chokepoint backfills the sentence on the
 * way out, so the arm strips BOTH — D-355, 2026-09-21.)
 *
