...
import { parseFrontmatter, checkGatheringGrammar, checkInboxGrammar, MECHANICAL_FIELD_SETS,
         ...
         /* REC-20: the MAP RULE applies to op=queue's ancestor walk too — a
            case's type and its state are read through the catalog's own
            vocabulary machinery, so a legacy spelling groups identically. */
         vocabFor,
