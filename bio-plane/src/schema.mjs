...
CREATE TABLE IF NOT EXISTS links (
  ...
  -- D-340: WHY the link reads as chrome (the furniture region it sat in, e.g.
  -- <nav> or role=navigation). NULL on a link that is not chrome, and on one
  -- filed before the column existed, which recorded no basis.
  chrome_basis   TEXT,
...
-- problems): CHROME IS A PROPERTY OF THE SITE, NOT OF THE PAGE. A nav link asserts
-- "this host has a Public Works section", so the site's navigation is recorded
-- once per HOST: one site_chrome row per distinct navigation (its fingerprint is
-- the digest of the sorted chrome addresses), and one site_chrome_refs row per
-- capture of the host that carried chrome, naming the fingerprint it carried.
-- Fifty captures of one site become one chrome record plus fifty references.
-- RATIFIED as a DERIVED TABLE: regenerated by scan of links (chrome = 1) joined
-- to captured_locators for the host, never authoritative, and cleared by a
-- whole-store purge (D-113). What it earns its place with is the per-host read
-- (op=navchanges): a link the host's navigation carried in one capture and not
-- in the next is a dated fact somebody may care about.
