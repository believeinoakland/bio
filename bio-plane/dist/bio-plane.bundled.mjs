...
`;
  ...
  *eachImage() {
  ...
  projectOwnerRemove({ projectId, handle, by, reason } = {}) {
    ...
    const owners = this.#owners(projectId);
    const math = _Store.ownerMath(owners.length);
    if (!math.possible)
      return {
        ok: false,
        reason: "LAST_OWNER",
    ...
    this.sql.exec(
      `UPDATE project_participants SET owner=0, updated=? WHERE project_id=? AND member_id=?`,
      now,
      projectId,
      target.member_id
    );
...
`;
  ...
  #owners(projectId) {
    return this.#rows(`SELECT member_id FROM project_participants WHERE project_id=? AND owner=1`, projectId).map((r) => r.member_id).sort();
  }
  ...
  #routeTask(bundleId) {
    ...
    const cite = this.#one(
      `SELECT r.bundle_id AS project_id FROM refs r
        ...
        WHERE r.target_id = ? ORDER BY r.bundle_id`,
      bundleId
    );
    if (cite) {
  ...
  #taskOf(row) {
    ...
    try {
      locators = row.locators ? JSON.parse(row.locators) : null;
    } catch {
      locators = null;
    }
    try {
  ...
  taskForward({ id = null, to = null, actor = null, now = null } = {}) {
    ...
    if (bad) return bad;
    this.sql.exec(
      `UPDATE tasks SET assignee=?, assignee_role=?, status=?, history=? WHERE id=?`,
      task.assignee,
      task.assignee_role,
      task.status,
      JSON.stringify(task.history),
  ...
  async fetch(req) {
    ...
    try {
      const map = {
        ...
        projectowneradd: () => this.projectOwnerAdd({
          projectId: url.searchParams.get("projectId"),
          handle: url.searchParams.get("handle"),
          by: url.searchParams.get("by")
        }),
        projectownerremove: () => this.projectOwnerRemove({
          projectId: url.searchParams.get("projectId"),
          handle: url.searchParams.get("handle"),
          by: url.searchParams.get("by"),
          reason: url.searchParams.get("reason")
