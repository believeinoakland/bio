...
export function checkInquiryBasis(fm, findings, publishedRegistry, earnedRegistry) {
  ...
  for (let i = 0; i < legs.length; i++) {
    ...
    /* D-162 / C-74.1: a THEME, or membership in one, is refused BY NAME at the same door (§8.4 fence 4). */
...
export function actionBasisFindings(fm, findings) {
  ...
  legs.forEach((l, i) => {
    ...
    /* D-162 / C-74.1: an action resting on a THEME rests on a member's lens, not on anything found. */
...
export function basisVersionFindings(fm, findings) {
  ...
  for (const [name, i] of byName) {
    ...
    for (const [li, leg] of legs) {
      ...
      /* D-162 / C-74.1: the theme refusal at the version's grain. */
 ...
 * C-74, minted with `node tools/mintid.mjs C`.
 *
 ...
 * does not contain it. The second is C-74.1, which names the theme instead of
...
/** C-74.1 — ONE LEG, ASKED WHETHER IT RESTS ON A THEME OR ON MEMBERSHIP IN ONE.
