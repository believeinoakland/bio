## THE LIVE `DEBT.md` AS IT STOOD AT RETIREMENT — moved here WHOLE, 2026-09-24 (M0-140)

**The DEBT construct is retired** (Bob, 2026-09-24: *"once that is done, we can remove all reference to the debt
construct"*; `docs/development/WORK-PIPELINE.md` §3, LED-7's end state). `docs/development/DEBT.md` no longer exists
on `coord` or on `main`. Its last three open rows — D-313, D-391 and D-388 — were closed by SCHEDULER #18 through the
third door (a stated limitation in the construct's home design document) and are the last three rows of the block
below; every other line of the file, including its own preamble and its 2026-07-31 dispositions note, is here VERBATIM
because the reasoning is the record.

**The rule that replaces it** (`CLAUDE.md` §4): a defect found anywhere is diagnosed until its FIX can be named, then
minted `D-` and placed by SCHEDULER in the build plan in build order — or routed to BOB first when the fix needs
design. There is no side list. Every `D-` id ever allocated is still resolved by
`node tools/ledger.mjs find <ID>`, which reads this file and `DEBT-closed-2026-08.md`; `tools/mintid.mjs` reads them
too, so no retired `D-` can be minted twice.

The file moved WHOLE rather than row by row because it could not be emptied row by row: `ledger.mjs`'
`DEBT_FLOOR_BYTES` (10,000 B) refused any archive that would leave it below the room `nc-m039.mjs` needed to plant
into, and an empty `DEBT.md` was 3,174 B. The floor is retired with the construct, and `nc-m039.mjs` arm 2 now plants
into `docs/development/DECISIONS.md`.

<!-- Nothing below this line is edited in place. It is the file as it stood, with the three closing dispositions
     written onto their own rows, which is how a row goes to the archive (WORK-PIPELINE §3, door 1). -->
