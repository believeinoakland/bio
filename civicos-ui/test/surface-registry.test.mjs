...
const ACTS_AWAITING_SURFACE = [
  ...
  /* `actionlaws` STOOD HERE AND WAS STRUCK 2026-09-24 BY UI-90, the item the row named as
     owing it — the fifth row this drain has collected, and ARM A4c fired on the item's first
     run naming the row and its owed_by before anything was touched. The act is now hosted by
     `SURFACES["action"]`, and `action-page.test.mjs` §8 drives it: the undetermined state
     reads the plane's own `action.governing_laws.stated` (asserted verbatim against
     `governingLawsOf`'s output, with the surface proved to hold no copy of the sentence), the
     three levels come from `vocabularies.law_levels` with `NO_LAWS`' `legal_levels` as the
     plane's other publication of the same array, and NO level is preselected — which the row
...
await section("ARM D · declared reads are real and reached", () => {
  ...
  /* UI-120 side, kept as history: ARM D1 floor 80 on its own branch (MOVED 2026-09-25 by UI-120 from the figure this arm PRINTED, forced by raising the floor and restored by sha256 and cmp: this item added exactly one, op=contentcrop, and found the floor ONE low at 79 before it; was 78, MOVED 2026-09-24 by UI-92 from the figure this arm PRINTED, the arm forced to print it by raising the floor and restored by sha256 — this item added exactly one, op=casedrafts, and found the floor TWELVE low at 65, which is the fifth consecutive item to find one stale: a floor with slack is not a ratchet. Was 65 under UI-45 when it sat one low, 64 under UI-42, and 50 before that when it sat thirteen low) — an empty call set would make D3 vacuous; the c22-batch30 union's figure is re-read from this arm's print. */
  /* CONDUCT #23 at the c23-batch30 union (2026-09-25): ARM D1 floor MOVED 79 -> 95 from the figure THIS ARM PRINTED
     on the merged tree with the floor raised out of reach on a throwaway copy of this file (a git-archive of HEAD
     50436ded in the session scratchpad; the committed file never edited to arm it): `95 ops called statically from app.html`. origin/main
     95fe7bc7 printed 87 by the same forced read, so 8 of the move is slack main already carried and 8 is this
     batch's (UI-120's contentcrop among them). Never a branch's figure. */
  ok(called.size >= 95, `ARM D1: ${called.size} ops are called statically from app.html, floor 95 (c23-batch30 union's print; MOVED 2026-09-25 by UI-88 from the figure this arm PRINTED, forced by raising the floor and restored by sha256 — this item added exactly one, op=partitionindependence, the accept ceremony's origins read; was 78, MOVED 2026-09-24 by UI-92 from the figure this arm PRINTED, the arm forced to print it by raising the floor and restored by sha256 — this item added exactly one, op=casedrafts, and found the floor TWELVE low at 65, which is the fifth consecutive item to find one stale: a floor with slack is not a ratchet. Was 65 under UI-45 when it sat one low, 64 under UI-42, and 50 before that when it sat thirteen low) — an empty call set would make D3 vacuous`);
  ...
  /* UI-120 side, kept as history: ARM D5 floor 55 on its own branch (MOVED 2026-09-25 by UI-120 from the figure this arm PRINTED, in the same forced print as D1 — this item declared exactly one more, op=contentcrop on the inquiry surface; was 54, MOVED 2026-09-24 by UI-92 from the figure this arm PRINTED, alongside D1 and in the same forced print — this item declared exactly one more, op=casedrafts on the project surface, and found the floor TWELVE low at 41; was 41 under UI-45, 39 under UI-42, and 30 before that when it sat seven low); the c22-batch30 union's figure is re-read from this arm's print. */
  /* CONDUCT #23 at the c23-batch30 union (2026-09-25): ARM D5 floor MOVED 55 -> 68 from the figure THIS ARM PRINTED
     on the merged tree with the floor raised out of reach on a throwaway copy of this file (a git-archive of HEAD
     50436ded in the session scratchpad; the committed file never edited to arm it): `68 reads described`. origin/main
     95fe7bc7 printed 61 by the same forced read, so 6 of the move is slack main already carried and 7 is this
     batch's (UI-120's contentcrop among them). Never a branch's figure. */
  ok(declared >= 68, `ARM D5: ${declared} reads are described, floor 68 (c23-batch30 union's print; MOVED 2026-09-25 by UI-88 from the figure this arm PRINTED, in the same forced print as D1 — this item declared exactly one more, partitionindependence on the accept ceremony; was 54, MOVED 2026-09-24 by UI-92 from the figure this arm PRINTED, alongside D1 and in the same forced print — this item declared exactly one more, op=casedrafts on the project surface, and found the floor TWELVE low at 41; was 41 under UI-45, 39 under UI-42, and 30 before that when it sat seven low)`);
});
