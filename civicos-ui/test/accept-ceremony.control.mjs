 ...
 * G the over-strictness arm, which MUST PASS.
 *
...
const ARMS = [
  ...
  },
  {
    ...
    what: "THE ORDERING RULE'S STRUCTURAL PIN: the origins read keeps the whole answer, pair included, in the page's state — drawn nowhere yet, and reachable by any code path that would.",
    mustFail: ["ORDERING: the pair that read also fetched is NOT HELD", "ORDERING: on arrival at the shared reading, the pair fetched with the origins is not held here either"],
  ...
  },
  {
    ...
    mustFail: [],
    mustPass: ["THE KEYSTONE", "THE ACCEPT LANDS", "D-195", "ORDERING", "OVER-STRICTNESS"],
...
const results = [];
console.log("UI-74 NEGATIVE CONTROL — each arm ALONE, restored and verified by sha256 AND cmp\n");
