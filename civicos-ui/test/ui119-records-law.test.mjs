...
import { ACTION_KINDS, CITATION_MAX } from "../../bio-plane/checks/bio-checks.mjs";
...
/* D-695: MEASURED, and pinned so the day it changes this suite says so. The catalogue refuses this law
   (C-2.10) only when the audit reads the bytes; op=promote does not run recordsLawFindings. Correct this arm
   — never exempt it — when D-695 lands: the act then refuses, and the page must show the plane's words. */
ok("D-695 PINNED: op=promote LANDS an over-long law today (C-2.10's law arm is audit-only) — the page sent it unjudged",
   !!s5.id);
