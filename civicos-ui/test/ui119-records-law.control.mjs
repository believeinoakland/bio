 ...
 *   baseline   nothing armed. MUST be GREEN, 37/0 (40/0 at c23-batch30: the D-695 arms, below).
 ...
 *   RE-DECLARED 2026-09-25 at the c23-batch30 union (CONDUCT #23), never exempted: D-695 is merged beside this
 *   suite, so `op=promote` now REFUSES C-2.10's `law` arm at the act (RECORDS_LAW_REFUSED, C-73.6), and the suite's
 *   §5 arm that pinned the landing was corrected into four D-695 arms. The declarations below were written when the
 *   act landed any law; each change is a consequence of that, read from this control's own print on the union:
 *   dropvalue and setupdrop ALSO fail the D-695 arms their surface feeds (a dropped law sends nothing over-long, so
 *   nothing is refused); setupwords ALSO fails the D-695 setup-words arm; and otherkind is now caught by THE PLANE —
 *   the law written on a cpra_request is refused at the act, so "the cpra_request is written" and its read-back fail,
 *   and the "NOT written — no law line" arm passes because nothing was written (the break still fails the suite).
 *   baseline is 40/0.
...
/* c23-batch30 (2026-09-25): the D-695 arms' names, and a RegExp expectation for an arm whose name carries the plane's
   answer (matched by test, every other expectation by equality). */
const D695_ACT = "D-695 REFUSES AT THE ACT: op=promote refuses the over-long law RECORDS_LAW_REFUSED with C-2.10's own sentence, and nothing landed";
const D695_APP = "D-695 IN THE PLANE'S WORDS (app): the app's send was REFUSED, nothing opened, and the page shows the plane's own sentence";
const D695_SETUP = "D-695 IN THE PLANE'S WORDS (setup): the setup page renders the plane's own canned translation for C-73.6, verbatim, not its code";
const hit = (x, f) => (x instanceof RegExp ? x.test(f) : x === f);
const ARMS = {
  baseline:   { expect: [] },
  ...
  otherkind:  { app: ['const lawText = a.kind === "records_request" ? String(a.law || "").trim() : "";', 'const lawText = String(a.law || "").trim();'],
                expect: [/^the cpra_request is written \( The record refused this and nothing was written\. law is carried by a records_request only/,
                         "…and the plane reads the cpra_request AS WRITTEN: law answers `kind`, no citation"] },
  setupwords: { setup: ['if (typeof x.translation === "string" && x.translation) return x.translation;\n  if (typeof x.detail === "string" && x.detail) return x.detail;',
                ...
                expect: [D695_SETUP,
                         "UI-119 SETUP WORDS: the setup page renders that refusal in the PLANE's words, verbatim, and not its code",
...
for (const [arm, a] of Object.entries(ARMS)) {
  ...
  const missing = a.expect.filter((x) => !failed.some((f) => hit(x, f)));
  const extra = failed.filter((f) => !a.expect.some((x) => hit(x, f)));
