...
/* CORRECTED A THIRD TIME 2026-08-09 BY UI-45, six -> SEVEN, and for the third
   time the old assertion was RIGHT to fail rather than wrong to exist:
   `stanceRouteFromHash` arrived with the stance surface (§7 — what a project
   stands on) and this arm stopped it arriving UNCLASSIFIED. The classification,
   made against this walk's own rule ("`boot()` cannot run without a credential,
   so the routers it asks are not pre-authentication surfaces"):

     `stanceRouteFromHash` is POST-AUTHENTICATION. It is asked inside `boot()`'s
     router chain and NOWHERE at the top level, so `#stands/<PROJ-…>/<INQ-…>`
     resolves for nobody holding nothing. It reads `op=basisversions`, whose
     classes are admin/member/probe — there is no uncredentialed arm to reach —
     and the one act it hosts, `op=versioncurrent`, is session-mode. So it adds
     no member-facing pre-auth vocabulary.
       ...
       "versionReviewRouteFromHash", "draftRouteFromHash"].every(f => ROUTE_FNS.includes(f)));
{
  ...
  /* UI-45's router, the same two halves, and the slice starts at the END of the
     notifications block for the reason the two above give: a router's own
     `hashchange` registration sits inside its own block and is not a pre-gate
     invocation, and everything after the block IS the pre-gate tail. */
  ok("WALK 2 CLASSIFICATION: stanceRouteFromHash is asked INSIDE boot(), which is what makes it post-authentication",
     !!BOOTCHAIN && BOOTCHAIN[0].includes("stanceRouteFromHash()"));
  const NTAIL = SCRIPT.slice(SCRIPT.indexOf("/*__NOTIFICATIONS_END__*/"));
  ok("WALK 2 REACH: the notifications block's END marker was found — a slice that missed it would make the pin below pass over nothing",
...
/* M0-23 (UI-56's delegation), 2026-09-10 — THE DIVERGED MEMBER, and it is here so
   ...
   the state that happens to render the same either way. CASE-5 unslaved a
   member's edition from its case's: a case at edition 2 may hold a finding that
   is at ITS OWN edition 1, and the roster-to-ratified join is the member's PINNED
   version hash, never edition to edition (`publishedManifest().production` says
   so in the plane's own words). `2` and `1` are two different numbers here, and
   that is the whole point of the pair. */
const CASE_DIV = "CASE-2026-0002", FIND_DIV = "FIND-2026-0002";
const DIV_SHA = "9".repeat(64), DIV_MAN = "8".repeat(64);
...
/* CORRECTED 2026-09-10 (M0-23, UI-56's delegation), never exempted, and the mock was
   as wrong as the one UI-56 corrected in `publishedcase.test.mjs` — D-173's rule, that
   a UI mock answers the WIRE SHAPE and never a shape convenient to the suite.
   `publishedManifest()` selects `case_id, edition, ord, bundle_id, version_sha, role`
   from `published_case_members`; this fixture carried the first FOUR and dropped the
   last two, so the join the surface actually performs could not have been WRITTEN
   against it and no assertion here could have noticed.

   IT WAS NOT A LIVE DEFECT AND THAT IS WORTH STATING RATHER THAN GLOSSING: the single
   member was undiverged, so `pubMemberKey`'s `bundle_id@edition` fallback answered the
   same row the pin does, and the surface rendered identically. MEASURED, not argued —
   with `version_sha`/`role` added and nothing else, this suite ran 68/68 with a
   CHARACTER-IDENTICAL vocabulary report. A fixture that cannot represent the wire
   shape still cannot ASSERT against it, which is why it is corrected anyway.

   `role` IS ADDED AND NO SURFACE READS IT. Measured: `caseMembers[].role` is read by
   nothing in `app.html` (`memberRole()` is the PROJECT roster's, a different table).
   It is carried here because the fixture's job is to answer the wire, not to answer
   the reads that happen to exist today — and the census arm in `check-mock-envelope.mjs`
   NAMES that asymmetry rather than failing anybody for it. */
const MANIFEST_ANSWER = {
  ...
  /* M0-23: THE DIVERGED MEMBER'S OWN RATIFIED ROW, AT EDITION 1 while its case is at
     edition 2. Note what is NOT here: no row at edition 2. The finding never moved;
     only the case did, which is exactly the state CASE-5 made reachable. */
  { bundle_id:FIND_DIV, edition:1, title:"Did the vendor disclose the sub-award?", bundle_sha:DIV_SHA,
    ratified_at:"2026-08-02T09:00:00Z", attestor_key:"SHA256:zzz", gate_version:"1.20.0",
    strength:{ capture:"B", connection:"C" }, required:{ capture:"B", connection:"C" } }],
  ...
  /* M0-23: A COMPLETE, ASSEMBLED CASE AT EDITION 2 whose only member is at its own
     edition 1. Assembled on purpose (`manifest_sha` set): there is nothing about this
     case that is awaiting anything, so a join on the case's edition would print
     "declared and not yet ratified" inside a row that is finished. */
  { case_id:CASE_DIV, edition:2, scope:"Whether the sub-award was disclosed to the board.",
    ...
    manifest:JSON.stringify({ format:"bio-case-container/2", case:CASE_DIV, edition:2,
      findings:[{ bundle_id:FIND_DIV, strength:{ capture:"B", connection:"C" },
                  required_strength:{ capture:"B", connection:"C" } }] }) }],
  caseMembers:[{ case_id:CASE_ID, edition:1, ord:0, bundle_id:FIND_ID, version_sha:SHA, role:null },
  /* THE DIVERGED ROSTER ROW: the case's edition is 2 and the pin names the finding's
     edition-1 bytes. Reachable ONLY through `version_sha` — which is the column this
     fixture did not have until now. */
  { case_id:CASE_DIV, edition:2, ord:0, bundle_id:FIND_DIV, version_sha:DIV_SHA, role:null }],
  shas:[],
...
if(S("public-record")){
  ...
  /* ===== M0-23 · THE DIVERGED MEMBER ON THE UNCREDENTIALED INDEX (UI-56's delegation) =====
     ADDED 2026-09-10, AND THE REASON IT EXISTS IS THIS ITEM'S OWN NEGATIVE CONTROL COMING
     BACK WRONG. M0-23 began as a fixture correction: `caseMembers[]` here carried four of
     the six columns `publishedManifest()` selects, and `version_sha` — THE PIN, the column
     the roster-to-ratified join is actually made on since CASE-5 — was one of the two
     missing. The correction was made and the declared arm was "delete the pin again and
     something must FAIL naming it". IT DID NOT. 68/68 green, because this file drove the
     published index and then asserted nothing about what the index SAID about a member.
     A fixture that cannot represent the wire shape cannot assert against it — and a fixture
     that CAN represent it still asserts nothing until somebody writes the assertion. The
     first half was the item; this is the second half, and without it the correction is
     decoration by the item's own test.

     WHY IT BELONGS IN THIS FILE RATHER THAN ONLY IN `publishedcase.test.mjs`. The published
     index is a PRE-AUTHENTICATION surface: `public-record #pl` is one of the surfaces this
     walk already covers, drawn for somebody holding no credential at all. What the old join
     did there was not a blank space but three false statements to a stranger — a finished
     case edition reported as awaiting ratification, a member's pair and bar blanked, and the
     same finding listed a second time as belonging to no case. "A defect that makes the
     record claim more than it can support" is this project's worst class, and this is that
     defect on the one page a stranger arrives at. The sibling suite owns the join's detail;
     this file owns the fact that the stranger is not lied to.

     THE FIXTURE IS THE ARM, and that is why the rows above were added rather than an
     assertion being written over the rows that were already here: with a single member whose
     own edition AGREES with its case's, deleting the pin changes NOTHING — the
     `bundle_id@edition` fallback answers the same row — and this assertion passes over a
     defect it cannot see. Measured: against the pre-M0-23 one-member fixture the same
     deletion is invisible, 68/68 green. */
  const pl = textOf("public-record", "#pl");
  ok("REACH: the DIVERGED member is joined to its own ratified row through its PIN, and the index "
     + "says three true things about it — its case edition 2 is complete rather than awaiting, the "
     + "member is ratified rather than declared-and-not-yet, and it is NOT also listed as belonging "
     + "to no case (the join is version_sha to bundle_sha, never edition to edition)",
     pl.includes('data-caserow="' + CASE_DIV + '" data-caseedition="complete"')
     && pl.includes('data-member="ratified" data-findingsec="' + FIND_DIV + '"')
     && !pl.includes('data-member="awaiting" data-findingsec="' + FIND_DIV + '"')
     && (pl.match(new RegExp('data-notacase="' + FIND_DIV + '"', "g")) || []).length === 0
     /* ONCE, and counted on the SECTION mark rather than on a badge attribute: UI-56's own
        arm counted `data-finding="…"` and read 3 for one member, because a pair emits that
        attribute on its axisrow AND on each grade badge. The subject was right and the
        instrument was wrong, in the direction that fails loudly. `data-findingsec` is emitted
        exactly once per rendered member block, which is the thing being counted. */
     && (pl.match(new RegExp('data-findingsec="' + FIND_DIV + '"', "g")) || []).length === 1);
  /* THE OVER-STRICTNESS HALF, AT THE SITE. The undiverged member must go on rendering
     EXACTLY as it always did — it is joined by a pin that agrees with the fallback, so
     nothing about it may move when the pin is present, absent, or deleted by an arm. This
     is what makes a failure of the assertion above SPECIFIC: one member moves, the other
     cannot. */
  ok("OVER-STRICTNESS: the UNDIVERGED member is untouched by any of this — ratified, listed once, "
     + "and never as a bundle in no case",
     pl.includes('data-member="ratified" data-findingsec="' + FIND_ID + '"')
     && (pl.match(new RegExp('data-findingsec="' + FIND_ID + '"', "g")) || []).length === 1
     && (pl.match(new RegExp('data-notacase="' + FIND_ID + '"', "g")) || []).length === 0);
}
if(S("design-preview"))
