 ...
 *   (l) LEAK THE ANALYST'S VOCABULARY ONTO THE SURFACE — in `elicPaint`'s read-back branch, change the set heading `Carries it on its own` to `Ground (OR-related branch)`. RUN: 1 of 46 fails, and it NAMES every hit: four banned patterns on each of three phases (the read-back, the read-back of a revision, and the refusal), 12 in all, with the matched text printed. The sweep is over everything this flow RENDERS in every phase, because DEC-32 clause 1 says *"not even as tooltips"* and a rule that only reads the headings is a rule about headings.
 *   (m) PREFILL THE MEMBER'S EARLIER ANSWERS — in `openElicit`, after `ELIC.standing = elicStanding(fmj);` add `for(const st of ELIC.standing.sets) for(const i of st.ords) ELIC.ans[i] = { holds:true, falls:st.ords.filter(x=>x!==i) };`. RUN: 1 of 46 fails, naming the revision's controls. It looks like a courtesy and it is the one way round the anti-gaming keystone: independent sufficiency would then survive a restructuring BY OMISSION, which is exactly what clause 4 says it must never do — the member would be able to keep a sufficiency claim by clicking past it.
 *   (n) SEND AN ATTRIBUTION — in `doElicit`, add the member's own name and a time to every set: in the `d.partition.map(...)` return, spread `{ asserted_by: (PLANE.me && PLANE.me.member) || "member", at: new Date().toISOString() }` into both branches. RUN: 2 of 46 fail, both on the WIRE. The record stamps both fields and DISCARDS a caller's (REC-45), so this changes nothing the record stores today — which is precisely why it is asserted on the wire: a surface that sends an attribution is a surface that would be believed the day anything downstream stopped discarding one, and an attribution a caller can hand us is one a caller can invent (CLAUDE.md).
 * Restore after each. */
