/* NEGATIVE CONTROL: (THIRTEEN ARMS. (m) and (m2) are UI-29's and (m) is BOB'S OWN, carried verbatim off DEC-44 onto this surface; (n) and (o) are UI-29's other two; UI-18's eight and UI-27's two are kept, RE-POINTED where UI-29 moved their subject, and ALL RE-RUN against the final file so every count below agrees with the file it names. Each arm is broken ALONE and `civicos-ui/app.html` is restored BYTE-IDENTICALLY — sha256 compared before and after every arm and equal to 7adbabb7d71e0cdc5e7fd62f23cfeba3e448b8c4082e8065f1d5e616f3c789c3 each time. The suite is 190 assertions whole; the counts are what each arm MEASURED, RUN 2026-08-04 by ui29-agent.)
 *   (m) A SINGLE CASE-LEVEL STRENGTH, ON THE SCREEN, IN PRINT AND IN THE EXPORT — UI-29 / DEC-44, and it is BOB'S OWN CONTROL. In `pubCaseHtml`, add one composed pair at case altitude: `${findings.length ? pubPairBadges(pubPair(findings[0]), pubCaseId(c)) : ""}` directly after the `<h1>` on page 1. RUN: 182 pass, 8 FAIL. The complement sweep NAMES what it found — "a pub-grade strength mark | a pub-grade strength mark | a data-axis attribute | a data-axis attribute | axis-and-letter text: Documents D | axis-and-letter text: Links C" — on the default rendering, on a filtered one, on the awaiting case, on the one-finding case and on the loose bundle; the OWNERSHIP assertion names `CASE-2026-0001` as an owner no finding of this case answers to; the mark/section tally disagrees; and the poisoned-answer block fires because the same code path draws it.
 ...
 *   (d) ONE FLOOR APPLIED TO BOTH AXES — in `readerFloors`, read every axis off the same value (`const v = f.capture;`). RUN: 185 pass, 5 FAIL — a reader's pair no longer needs both axes, a half pair draws a case instead of refusing, the `none`/real split stops naming `none` where the reader said `none`, and DEC-40's filter line names one floor twice. R2's forbidden composition performed by arithmetic.
 *   (e) ONE AXIS LEFT UNCONSTRAINED — in `readerFloors`, default a missing or unknown value instead of refusing (`if(!FLOOR_VALUES.includes(v)){ out[ax] = "none"; continue; }`). RUN: 188 pass, 2 FAIL — the resolver accepts half a pair, and the half-pair rendering that must not be drawn is drawn. The surface refusing to draw is the shipped answer; the harness failing is the statement that such a rendering may not be shipped, because an unstated floor reads as a satisfied one.
 *   (f) A `none` FLOOR OMITTED FROM THE RENDERING — in `pubInbandHtml`, wrap the capture floor line as `${floors && floors.capture === "none" ? "" : `<div>…</div>`}`. RUN: 188 pass, 2 FAIL — the in-band block stops naming the floor it applied to the documents axis, in both the unfiltered and the mixed rendering. TWO, and every other assertion including the DEC-34 header's floor line stays green, because the header names the floors from a different function: an omission in ONE of the two places both floors are printed is invisible to any check that reads only the other.
 *   (g) A PAGE-SHAPED ARTIFACT WITHOUT THE DEC-34 HEADER — in `pubPage`, return `<section class="pub-page"${at}>${inner}</section>`. RUN: 173 pass, 17 FAIL: page count and header count disagree, the per-page facts sweep finds none of the facts on any page, the finding/case altitude hashes go, the filter line count falls in both directions, the awaiting window leaves every header, and the edition a reader is on stops being stated at all. This is DEC-34's negative control seam, which `bio-plane/test/publishedcase.test.mjs` deliberately left unplaced because the plane produces no pages to put a header on. It is placed here.
 *   (h) THE IN-BAND BLOCK HIDDEN IN PRINT — add `.pub-inband{display:none}` inside app.html's `@media print{…}` block. RUN: 189 pass, 1 FAIL, naming the hiding rule. The print arm refuses ANY hiding rule rather than protecting a list of selectors, because the moment a stylesheet is allowed to hide one thing the argument for hiding the next is already written — and a qualifier that survives on screen and vanishes on paper is the forbidden compression performed by CSS.
 *   (i) STRIP THE FILTER LINE FROM A FILTERED RENDERING — in `dec34Header`, delete the whole `<span class="f"><b>What this is</b> ${pubFilterHtml(floors, "filterline")}</span>` line. RUN: 186 pass, 4 FAIL — the per-page count of filter lines falls short in BOTH directions, and neither rendering's DEC-34 header carries the line any more. THIS IS THE ARM DEC-40 EXISTS FOR: a filtered page indistinguishable from the case is the misrepresentation vector, and it is the one defect that leaves the page looking perfect — every leg it kept is real, every strength on it is the plane's own, and the only thing wrong with it is what it does not say.
 *   (j) PRESENT A FILTERED RENDERING AS THE CASE — in `pubRenderingName`, return `"THE WHOLE CASE, UNFILTERED"` unconditionally. RUN: 187 pass, 3 FAIL — the filtered rendering is no longer named as a view the reader constructed, the footer's rendering hash is described as the case, and the stripped-text sweep loses the statement.
