 ...
 *   (R5) the surface CALLS an op that mints it (D-542, 2026-09-25) — whether or
 *        not anything spells the code. R2 alone scored TEN of D-448's eleven
 *        review-copy codes out of reach for a day, because UI-68's surface renders
 *        the plane's `detail` and keys on no literal. `reach-by-op.mjs` says how
 *        "minted on an op" is read and what it cannot see.
 *   (R6) a STRANGER can call an op that mints it (D-562): an `OPS` row with
 *        `classes: null`. The member who meets it is whoever asked.
 *   (R4 is D-485's arm H pane rule, SEEN IN A PANE; it joins reach after arm H runs.)
 *
...
/* D-485 — arm H reads suite text through the estate's ONE lexer, so a regex quoted in a comment is not
...
/* D-542 / D-562 — R5 (by op) and R6 (public op): the walk that reads what a surface CALLS. */
import { reachByOp } from "./reach-by-op.mjs";
...
const FLOOR = {
  ...
  /* D-510 side, kept as history: families 53 on its own branch; the c20-batch25 union's figure is re-read from the print. */
  /* D-147 side, kept as history (c22-batch29: ours' families: 57 kept; NUMBER TO RE-READ from --strict's print on the union): families: 55, — D-147 (C-94 LIFECYCLE_CHECKS, 2026-09-25, branch land/worker/D-147): 54 -> 55, READ from this guard's own print on the item's tree over origin/main 964da679 — the new family, its eleven rows and codes, and its three regions (is-lifecycle-writable, is-lifecycle-grammar, is-lifecycle-due-cite). Every other key gated at slack 0 on the same run. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads regionLines at integration. */
  /* REC-203 side, kept as history (c22-batch29: ours' families: 57 kept; NUMBER TO RE-READ from --strict's print on the union): families: 55, — REC-203 (C-91 IDSPACE_CHECKS, 2026-09-25, branch land/worker/REC-203): MOVED 54 -> 55 from this guard's own --strict print on the item's tree over origin/main 964da679. Whose growth it absorbs: op=idmatch's ONE new family and its three refusals, each at its own DEC-49 REGION in store.mjs idMatch. regionLines is a property of the MERGED source, so CONDUCT re-reads it at integration. */
  /* REC-214 side, kept as history: families 55 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-530 side, kept as history: families 55 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-150 side, kept as history: families 55 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-134 side, kept as history: families 55 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* MK-7 side, kept as history: families 55 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* REC-147 side, kept as history: families 55 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* REC-197 side, kept as history: families 55 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-561 side, kept as history: families 58 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-419 side, kept as history: families 58 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  families: 66, /* CONDUCT #23 at the c23-batch30 union: 65 -> 66, READ from `node civicos-ui/check-refusal-codes.mjs --strict`'s print on the merged tree (HEAD 645f8c41); never a branch's figure, never added by hand. */ /* CONDUCT #22 at the c22-batch29 union: 57 -> 65, the union's figure READ from `node civicos-ui/check-refusal-codes.mjs --strict`'s print on the merged tree (`ratchet: families ... measured 65`). */ /* CONDUCT #21 at c21-batch28: 55 -> 57, the union's figure READ from --strict's print on the merged tree. */ /* D-448 (C-87 REVIEW_COPY_CHECKS, 2026-09-24, cloud session WORKER D-448 (SCHEDULER #19), branch land/worker/D-448): MOVED 54 -> 55 from this guard's own --strict print on the item's tree over origin/main 9f8b69e6. Whose growth it absorbs: ONE new family, REVIEW_COPY_CHECKS (C-87), the review copy's eleven refusals. ZERO of the move is pre-existing slack — the PRISTINE origin/main 9f8b69e6, run in this worktree BEFORE any edit, printed exit 0 with EVERY ratchet key at slack 0 (log kept as the item's baseline). A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads regionLines at integration. */ /* CONDUCT #20 at c20-batch25: 53 -> 54, D-510's C-86.1 (the PROMOTED_TYPE_CHECKS family and its region) over the union, READ from --strict's print on the merged tree. */ /* D-508 (C-85 KNOCK_CHECKS, 2026-09-24, cloud session WORKER D-508 (CONDUCT #20), branch land/worker/D-508): MOVED 52 -> 53 from this guard's own --strict print on the item's tree over origin/main 68fecb8d0. Whose growth it absorbs: ONE new family, KNOCK_CHECKS (C-85), the doorbell's two rate refusals. ZERO of the move is pre-existing slack: the PRISTINE origin/main 68fecb8d0, run in this worktree before any edit, printed exit 0 with EVERY ratchet key at slack 0. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads regionLines at integration — and D-507 moves the same catalogue in parallel. */ /* CONDUCT #20 (c20-integ1, 2026-09-24): 51 -> 52, RE-READ from this guard's own --strict print on the committed union (D-64 renumbered C-82 -> C-83, REC-184, and origin/main @ 548eb2c5 merged over c19-batch10) — never added by hand, and never either side's figure. Whose growth it absorbs: ONE new family, D-64's RENDER_CAPTURE_CHECKS (C-83). */ /* D-64 (C-83 RENDER_CAPTURE_CHECKS, 2026-09-24, cloud session WORKER D-64 (CONDUCT #19), branch land/worker/D-64): MOVED 42 -> 43 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: one new family, RENDER_CAPTURE_CHECKS. ZERO of the move is pre-existing slack: every other key gated at slack 0 on the same run, and the move is exactly this item's own. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration. */ /* c19-unionfix (c19-batch9, 2026-09-24): 50 -> 51, re-read from this guard's --strict print on the union with IC-246: its ONE new family STATEMENT_ACK_CHECKS (C-82), ONE row and code STATEMENT_ACK_DOCUMENTS_OVER_BOUND, ONE region `is-statement-ack-documents-bound` in acknowledgeStatement. */ /* CONDUCT #18 (c18-batch8, 2026-09-23): D-394 merged over the batch; 47 -> 48 RE-READ from this guard's --strict print on the committed merge 8477cdb5. The new members are D-394's; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-126 (C-75 and C-76, the per-item weight) merged over the batch; 45 -> 47 RE-READ from this guard's --strict print on the committed merge 6ab139df. The new members are D-126 (C-75 and C-76, the per-item weight)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-149 (C-70, discoverable or hidden) merged over the batch; 44 -> 45 RE-READ from this guard's --strict print on the committed merge f4be4c07. The new members are REC-149 (C-70, discoverable or hidden)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-122 (C-74, the on-point choice) merged over the batch; 43 -> 44 RE-READ from this guard's --strict print on the committed merge e1d34ee0. The new members are REC-122 (C-74, the on-point choice)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-149 (C-73 and C-32.18, the governing laws) merged over the batch; 42 -> 43 RE-READ from this guard's --strict print on the committed merge 8086d64f. The new members are D-149 (C-73 and C-32.18, the governing laws)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-148 (C-72, the fee quote) merged over the batch; 41 -> 42 RE-READ from this guard's --strict print on the committed merge c81aec68. The new members are D-148 (C-72, the fee quote)'s; neither side's figure is the sum. */ /* CONDUCT #17 (c17-batch5, 2026-09-23): REC-161 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-161 (C-71 PARTITION_INDEPENDENCE_CHECKS, 2026-09-23, cloud session WORKER REC-161 (CONDUCT #17)): MOVED 38 -> 39 from this guard's own print on the item's tree over origin/main 02603e88: one new family, PARTITION_INDEPENDENCE_CHECKS. ZERO of the move is pre-existing slack: the same guard over a pristine 02603e88 worktree printed 38, every key gated at slack 0. */ /* D-278 (C-68 + C-69, 2026-09-23, branch land/worker/D-278): MOVED 38 -> 40 from this guard's own print on the item's tree, every other ratchet key gated. two new families, INSTALLATION_CHECKS (C-68) and DISPATCH_CHECKS (C-69). ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own. */ /* D-85 (C-66 SURFACE_CHECKS, 2026-09-23, worktree agent-a09e49cc337fc2200): MOVED 37 -> 38 from this guard's own print on the item's tree, every other ratchet key gated. One new family, SURFACE_CHECKS. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* REC-167 (C-65 CASE_CONCLUSION_CHECKS, 2026-09-22, worktree agent-a33d4bac9a4ff9d3a): MOVED 36 -> 37 from this guard's own print on the item's tree, every other ratchet key gated. One new family, CASE_CONCLUSION_CHECKS. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* D-436 (C-64 INSTANCE_GROUP_CHECKS, 2026-09-21, worktree agent-a6dd0a0a3a3a6cc10): MOVED 35 -> 36 from this guard's own GREEN print on the item's committed tree 2f09b7b6. One new family, INSTANCE_GROUP_CHECKS. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* D-158 (C-63 SIGNER_ENROLMENT_CHECKS, 2026-09-20, worktree d158-conduct8): MOVED 34 -> 35 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE origin/main sources (e1aa2eee) with this item's three plane files (store.mjs, setup.mjs, bio-checks.mjs) swapped out and restored by sha256 AND cmp: pristine printed 34, so 0 of the move is PRE-EXISTING SLACK and +1 is this item's (one new family, SIGNER_ENROLMENT_CHECKS). */    /* D-270 (C-38.7/C-38.8 + C-61 REQUIRED_ARGUMENT_CHECKS, 2026-09-19, worktree d270-record): MOVED 23 -> 34 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE origin/main sources (98209ce2) with this item's two plane files swapped out and restored by sha256 AND cmp: pristine printed 33, so 10 of the move is PRE-EXISTING SLACK and +1 is this item's (one new family, REQUIRED_ARGUMENT_CHECKS). */    /* REC-86 (IC-123, C-50 NARROW_CHECKS, 2026-09-18, worktree agent-a3fbd59a3fef1a961): MOVED 22 -> 23 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE HEAD sources (92f4c64e) with the item's four plane files swapped out and restored by sha256 + cmp: pristine printed 22, so 0 of the move is PRE-EXISTING slack and +1 (NARROW_CHECKS) is this item's. */ // + AI_RUNS_CONTEXT_CHECKS (REC-69, C-36 — the context-keyed run list).
  /* ================================================================== *
  ...
  /* CONDUCT #19 (c19-batch9, 2026-09-24): D-162 (the THEME family, renumbered C-74 -> C-81) merged over the batch; RE-READ from this guard's --strict print on the merged tree. */
  /* D-510 side, kept as history: rows 382 on its own branch; the c20-batch25 union's figure is re-read from the print. */
  /* D-472 side, kept as history: rows 383 on its own branch; the c20-batch25 union's figure is re-read from the print. */
  /* D-491 side, kept as history: rows 382 on its own branch; the c20-batch25 union's figure is re-read from the print. */
  /* REC-211 side, kept as history: rows 381 -> 383 on its own branch (IC-273); the c20-batch23 union's figure is re-read from --strict's print. */
  /* REC-186 side, kept as history (c22-batch29: ours' rows: 436 kept; NUMBER TO RE-READ from --strict's print on the union): rows: 396, — REC-186 (2026-09-25, branch land/worker/REC-186): 395 -> 396, READ from this guard's print on the item's tree over origin/main 8bdf20e6. Whose growth: C-33.48 LAST_OWNER_CANNOT_LEAVE and its region is-leave-owner-floor in projectLeave. */
  /* D-513 side, kept as history: rows 394 on its own branch; the c20-batch27 union's figure is re-read from --strict's print. */
  /* D-463 side, kept as history: rows 393 on its own branch; the c20-batch27 union's figure is re-read from the print. */
  /* D-147 side, kept as history (c22-batch29: ours' rows: 436 kept; NUMBER TO RE-READ from --strict's print on the union): rows: 411, — D-147 (C-94 LIFECYCLE_CHECKS, 2026-09-25, branch land/worker/D-147): 400 -> 411, READ from this guard's own print on the item's tree over origin/main 964da679 — the new family, its eleven rows and codes, and its three regions (is-lifecycle-writable, is-lifecycle-grammar, is-lifecycle-due-cite). Every other key gated at slack 0 on the same run. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads regionLines at integration. */
  /* REC-203 side, kept as history (c22-batch29: ours' rows: 436 kept; NUMBER TO RE-READ from --strict's print on the union): rows: 403, — REC-203 (C-91 IDSPACE_CHECKS, 2026-09-25, branch land/worker/REC-203): MOVED 400 -> 403 from this guard's own --strict print on the item's tree over origin/main 964da679. Whose growth it absorbs: op=idmatch's ONE new family and its three refusals, each at its own DEC-49 REGION in store.mjs idMatch. regionLines is a property of the MERGED source, so CONDUCT re-reads it at integration. */
  /* D-547 side, kept as history: rows 396 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-217 side, kept as history: rows 398 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-448 side, kept as history: rows 406 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-468 side, kept as history: rows 396 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-214 side, kept as history: rows 400 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-549 side, kept as history: rows 396 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-512 side, kept as history: rows 396 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-205 side, kept as history: rows 392 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* c21-rec207-renumber side, kept as history: rows 401 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-530 side, kept as history: rows 396 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-454 side, kept as history: rows 396 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-150 side, kept as history: rows 409 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-520 side, kept as history: rows 396 on its own branch; the c22-batch29 union's figure is re-read from --strict's print. */
  /* D-134 side, kept as history: rows 409 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* MK-7 side, kept as history: rows 412 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* REC-147 side, kept as history: rows 407 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* REC-197 side, kept as history: rows 402 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-521b side, kept as history: rows 435 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-563 side, kept as history: rows 438 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-561 side, kept as history: rows 445 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-578 side, kept as history: rows 439 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-546 side, kept as history: rows 440 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-615 side, kept as history: rows 441 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-623 side, kept as history: rows 437 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-629 side, kept as history: rows 438 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-680 side, kept as history: rows 437 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* REC-215 side, kept as history: rows 437 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-419 side, kept as history: rows 441 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-689 side, kept as history: rows 437 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  /* D-695 side, kept as history: rows 438 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  /* D-670 side, kept as history: rows 437 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  /* D-688 side, kept as history: rows 502 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  /* D-628 side, kept as history: rows 442 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  rows: 519, /* CONDUCT #23 at the c23-batch30 union: 501 -> 519, READ from `node civicos-ui/check-refusal-codes.mjs --strict`'s print on the merged tree (HEAD 645f8c41); never a branch's figure, never added by hand. */ /* CONDUCT #22 at the c22-batch29 union: 436 -> 501, the union's figure READ from `node civicos-ui/check-refusal-codes.mjs --strict`'s print on the merged tree (`ratchet: rows ... measured 501`). */ /* CONDUCT #21 at c21-batch28: 400 -> 436, the union's figure READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch27: 395 -> 400, the union's figure (D-463, D-513 and the batch) READ from --strict's print on the committed merge. */ /* CONDUCT #20 at c20-batch25: 394 -> 395, D-510's C-86.1 (the PROMOTED_TYPE_CHECKS family and its region) over the union, READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch25: 392 -> 394, D-472's C-48.8/C-48.9 and their two regions over the union, READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch25: 391 -> 392, D-491's C-28.16 (and its is-capture-request region) over the batch24c union, READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch23: 389 -> 391, REC-211's arrivals over the c20-batch22 union, READ from this guard's --strict print on the merged tree. */ /* CONDUCT #20 at c20-batch22: the UNION of D-507 (381 -> 387) and D-508 (381 -> 383), READ from this guard's --strict print on the merged tree. */ /* D-507 (C-82.2..C-82.7, the six STATEMENT_ACK_* refusals that reached a member untranslated, 2026-09-24, branch land/worker/D-507): MOVED 381 -> 387 from this guard's own --strict print on the item's tree over origin/main 68fecb8d. Six new rows in the EXISTING family STATEMENT_ACK_CHECKS (no new family, so `families` does not move). ZERO of the move is pre-existing slack: the PRISTINE origin/main 68fecb8d, run in this worktree before any edit, printed exit 0 with EVERY ratchet key at slack 0. */  /* CONDUCT #20 (c20-batch14, 2026-09-24): 379 -> 381, RE-READ from this guard's own `--strict` print on the union of this branch with `origin/main` @ 454a02bc (c20-batch17 and c20-batch18). Whose growth it absorbs: D-484's two new rows, C-33.40 NO_BASIS and C-33.41 NO_CITATION in ACT_SHAPE_CHECKS. Never either side's figure and never added by hand. */ /* D-484 (C-33.40 NO_BASIS, C-33.41 NO_CITATION consolidated behind one governed helper each, 2026-09-24, branch land/worker/D-484): MOVED 368 -> 370 from this guard's own --strict print on the item's tree over origin/main 16fe1e7f. ZERO pre-existing slack: the PRISTINE origin/main 16fe1e7f, run before any edit in this worktree, printed exit 0 with EVERY ratchet key at slack 0. the two new ACT_SHAPE_CHECKS rows. */ /* CONDUCT #20 (c20-batch14, 2026-09-24): 372 -> 379, RE-READ from this guard's own `--strict` print on the committed union of c20-batch11fix and c20-integ1b — never either branch's figure and never added by hand. Whose growth it absorbs: the SEVEN new rows are D-64's `RENDER_CAPTURE_CHECKS` family (C-83.1..7), carried in from c20-integ1b. Both branches had re-done the same c19-batch10 + `main` union, so each branch's own figure was short of the union's. */ /* CONDUCT #20 (c20-integ1, 2026-09-24): 369 -> 377, RE-READ from this guard's own --strict print on the committed union (D-64 renumbered C-82 -> C-83, REC-184, and origin/main @ 548eb2c5 merged over c19-batch10) — never added by hand, and never either side's figure. Whose growth it absorbs: +7 D-64 (C-83.1..7) and +1 the base merge — IC-246's C-82.1 came in from origin/main @ 548eb2c5, which the batch10 floor of 369 did not hold. */ /* D-64 (C-83 RENDER_CAPTURE_CHECKS, 2026-09-24, cloud session WORKER D-64 (CONDUCT #19), branch land/worker/D-64): MOVED 323 -> 330 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: seven rows, C-83.1..C-83.7. ZERO of the move is pre-existing slack: every other key gated at slack 0 on the same run, and the move is exactly this item's own. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration. */ /* CONDUCT #19 (c19-batch11, 2026-09-24): REC-192 @ 2e102941 (C-71.8, C-71.9; IC-248; the branch read 325 on main) merged over the batch; RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* REC-192 (the version arm of op=partitionindependence, 2026-09-24, cloud session WORKER REC-192 (CONDUCT #19), branch land/worker/REC-192): MOVED 323 -> 325, the figure this guard PRINTED on the item's tree over origin/main 15b2a4c0: two new rows in PARTITION_INDEPENDENCE_CHECKS (C-71.8 TWO_SUBJECTS, C-71.9 NO_SUCH_VERSION). ZERO of the move is pre-existing slack: the same guard over pristine origin/main printed exactly 323. Integrator: RE-READ from this guard's print on the committed merge. */ /* D-456 (C-78 NAMESPACE_CHECKS, 2026-09-23, cloud session WORKER D-456 (CONDUCT #18), branch land/worker/D-456): MOVED 322 -> 323 from this guard's own --strict print on the item's tree over origin/main a8f6094a: one new family, one row (NAMESPACE_UNKNOWN), one governed region (namespaceGate > is-namespace-gate, 5 lines), one code judged and compared. ZERO of the move is pre-existing slack: the same guard on a8f6094a before the change printed 322, every key gated at slack 0. */ /* CONDUCT #19 (c19-batch11, 2026-09-24): c19-unionfix @ 7b52ab9b (at 368) merged into c19-batch10 (at 369); RE-READ from this guard's --strict print on the merged tree, never added by hand. Both sides' comments carried below. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): REC-189 (C-32.19 MACHINE_CANNOT_SET_RISK_TIER, minted C-32.18) merged over the batch; 368 -> 369 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): D-461 (C-78.2 NAMESPACE_PINNED) merged over the batch; 367 -> 368 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* D-461 (C-78.2 NAMESPACE_PINNED, 2026-09-24, cloud session WORKER D-461 (CONDUCT #19)): MOVED 323 -> 324 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: one new row in the existing NAMESPACE_CHECKS family and one new governed REGION (pinnedNamespaceGate > is-pinned-namespace-gate). ZERO of the move is pre-existing slack: the same guard on 15b2a4c0 gated every key at slack 0. */ /* c19-unionfix (c19-batch9, 2026-09-24): 367 -> 368, re-read from this guard's --strict print on the union with IC-246: its ONE new family STATEMENT_ACK_CHECKS (C-82), ONE row and code STATEMENT_ACK_DOCUMENTS_OVER_BOUND, ONE region `is-statement-ack-documents-bound` in acknowledgeStatement. */ /* CONDUCT #18 (c18-batch8, 2026-09-23): D-394 merged over the batch; 353 -> 356 RE-READ from this guard's --strict print on the committed merge 8477cdb5. The new members are D-394's; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-126 (C-75 and C-76, the per-item weight) merged over the batch; 347 -> 353 RE-READ from this guard's --strict print on the committed merge 6ab139df. The new members are D-126 (C-75 and C-76, the per-item weight)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-149 (C-70, discoverable or hidden) merged over the batch; 343 -> 347 RE-READ from this guard's --strict print on the committed merge f4be4c07. The new members are REC-149 (C-70, discoverable or hidden)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-122 (C-74, the on-point choice) merged over the batch; 340 -> 343 RE-READ from this guard's --strict print on the committed merge e1d34ee0. The new members are REC-122 (C-74, the on-point choice)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-149 (C-73 and C-32.18, the governing laws) merged over the batch; 334 -> 340 RE-READ from this guard's --strict print on the committed merge 8086d64f. The new members are D-149 (C-73 and C-32.18, the governing laws)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-148 (C-72, the fee quote) merged over the batch; 326 -> 334 RE-READ from this guard's --strict print on the committed merge c81aec68. The new members are D-148 (C-72, the fee quote)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-164 merged over c17-batch6; 322 -> 326 RE-READ from this guard's --strict print on the committed merge 84443ad4. The new members are REC-164's (C-64.4..C-64.7, the group's name and domain acts); neither side's figure is the sum. */ /* CONDUCT #17 (c17-batch5, 2026-09-23): REC-161 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-161 (C-71 PARTITION_INDEPENDENCE_CHECKS, 2026-09-23, cloud session WORKER REC-161 (CONDUCT #17)): MOVED 309 -> 316 from this guard's own print on the item's tree over origin/main 02603e88: its seven rows, C-71.1..C-71.7. ZERO of the move is pre-existing slack: the same guard over a pristine 02603e88 worktree printed 309, every key gated at slack 0. */ /* CONDUCT #17 (c17-batch4, 2026-09-23): D-179 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-179 (C-53.13 CAPTURE_HELD_BY_ANOTHER_BUNDLE, 2026-09-23, branch land/worker/D-179): MOVED 305 -> 306 from this guard's own print on the item's tree: the one new row, C-53.13 CAPTURE_HELD_BY_ANOTHER_BUNDLE in TESTIMONY_CHECKS. ZERO pre-existing slack — every other key gated at 0 on the same run, and the move is exactly this item's own. */ /* CONDUCT #17 (c17-batch3, 2026-09-23): D-278 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-278 (C-68 + C-69, 2026-09-23, branch land/worker/D-278): MOVED 305 -> 310 from this guard's own print on the item's tree, every other ratchet key gated. five new rows: C-68.1-.4 and C-69.1. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): origin/main (c16-batch7) merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-168 (C-33.39 RETIRED_NOT_CITABLE in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-ab0b1ebe531cb428b): MOVED 302 -> 303 from this guard's own print on the item's tree: the one new row, C-33.39 RETIRED_NOT_CITABLE in ACT_SHAPE_CHECKS. ZERO pre-existing slack — the pristine HEAD (0e7cc03e) sources, swapped in and restored by sha256 AND cmp, printed 302, every key gated.  — CONDUCT #16 merged it onto REC-175/REC-176, which moved this key from the same base; the figure is re-read from the merged print. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): REC-177 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-177 (C-22.16 AI_RUN_BOUND_NO_ALLOWANCE in AI_RUN_CHECKS, 2026-09-23, worktree agent-ad7020ce0605889f4), RE-READ ON THE MERGE onto origin/main 0e5f7054 (which carries REC-175's and D-168's moves of this key from the same base): MOVED 303 -> 304 from this guard's own print on the merged tree: the one new row, C-22.16. ZERO of the move is pre-existing slack: every other key gated at slack 0 on the same run. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): REC-179 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-179 (C-66.5 SURFACED_BY_REWRITTEN in SURFACE_CHECKS, one new region promote > is-promote-surfaced-by, 2026-09-23, worktree agent-a368de25bd347b32f): MOVED 303 -> 304 from this guard's own print on the item's tree MERGED with origin/main 0e5f7054 (REC-175 landed first and moved this key from the same base, so this figure is re-read, never added), every other ratchet key gated. ZERO of the move is pre-existing slack: the merged tree without this item printed 303. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): D-440 and D-420 each moved this key by their own row from 0e5f7054; the union's value is RE-READ from this guard's print on the committed merge. */ /* D-420 (C-45.12 CONTENT_EXTENT_NO_IMAGE_PAINTED in CONTENT_EXTENT_CHECKS, 2026-09-23, cloud session WORKER D-420 (CONDUCT #16)): MOVED 303 -> 304 from this guard's own print on the item's tree over origin/main 0e5f7054, every other ratchet key gated at slack 0 on the same run. One new row, C-45.12. ZERO of the move is pre-existing slack: the baseline run on 0e5f7054 printed exactly the old floor, so the move is this item's own. D-440 moves this key from the same base; CONDUCT re-reads the figure from the merged print. */ /* D-440 (C-45.11 CONTENT_EXTENT_NOT_A_CONTAINER, 2026-09-23, worktree agent-aa6d775965f554582), RE-READ ON ITS MERGE with origin/main 0e5f7054 (which carries REC-175's moves from the same base): MOVED 303 -> 304 from this guard's own print on the merged tree, every other ratchet key gated. The one new row. ZERO pre-existing slack. */ /* REC-175 (C-33.38 FILE_DIGEST_MISMATCH in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-a6f377d2e96aa24af): MOVED 301 -> 302 from this guard's own print on the item's tree over origin/main 14faa089, every other ratchet key gated at slack 0 on the same run. One new row, C-33.38. ZERO of the move is pre-existing slack: the baseline run on 14faa089 printed exactly the old floor, so the move is this item's own.  — CONDUCT #16 merged it with REC-172, which moved this key from the same base; the figure is re-read from the merged print. */ /* REC-172 (C-22.15 AI_RUN_BOUND_UNKNOWN in AI_RUN_CHECKS, 2026-09-23, worktree agent-ad79857cb007927b7): MOVED 300 -> 301 from this guard's own print on the item's tree: the one new row. ZERO pre-existing slack — the pristine HEAD (91913d6b) sources printed 300, every key gated.  — CONDUCT #16 merged it with UI-81, which moved this key from the same base; the figure is re-read from the merged print. */ /* UI-81 (C-44.2 FINDING_IN_SEVERAL_CASES in CASE_DERIVATION_CHECKS, 2026-09-23, worktree agent-a306ff29faf082ef3), RE-READ ON THE MERGE onto origin/main 91913d6b (which carries REC-169's moves): MOVED 300 -> 301 from this guard's own print on the merged tree, every other ratchet key gated. One new row, C-44.2. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* REC-169 (C-22.13/C-22.14 in AI_RUN_CHECKS, 2026-09-23, worktree agent-a30f15e61a2d24169): MOVED 298 -> 300 from this guard's own print on the item's tree: the two new AI_RUN_CHECKS rows. The move is exactly this item's own additions, read off the per-site line; every other key gated on the same run. */ /* D-85 (C-66 SURFACE_CHECKS, 2026-09-23, worktree agent-a09e49cc337fc2200): MOVED 294 -> 298 from this guard's own print on the item's tree, every other ratchet key gated. Its four rows, C-66.1..C-66.4 (SURFACE_NO_RUN, SURFACE_RUN_NOT_RUNNING, SURFACE_NO_BOUND, SURFACE_BOUND_REACHED). ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* REC-165 (C-27.18 SUGGEST_RUN_NOT_RUNNING, C-27.19 SUGGEST_OUTSIDE_RUN_CONTEXT, 2026-09-22, worktree agent-a6e92d28647553f52): MOVED 291 -> 293 from this guard's own print on the item's tree. Every floor here is gated at slack 0 (M0-79), so main measured 291 and the +2 is this item's two rows. */ /* D-436 (C-64 INSTANCE_GROUP_CHECKS, 2026-09-21, worktree agent-a6dd0a0a3a3a6cc10): MOVED 288 -> 291 from this guard's own GREEN print on the item's committed tree 2f09b7b6. Its three rows, C-64.1 to C-64.3. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* D-158 (C-63 SIGNER_ENROLMENT_CHECKS, 2026-09-20, worktree d158-conduct8): MOVED 285 -> 288 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE origin/main sources (e1aa2eee) with this item's three plane files (store.mjs, setup.mjs, bio-checks.mjs) swapped out and restored by sha256 AND cmp: pristine printed 286, so 1 of the move is PRE-EXISTING SLACK and +2 is this item's (C-63.1 SIGNER_MEMBER_NOT_ENROLLED and C-63.2 SIGNER_MEMBER_NOT_ACTIVE). */    /* D-270 (C-38.7/C-38.8 + C-61 REQUIRED_ARGUMENT_CHECKS, 2026-09-19, worktree d270-record): MOVED 219 -> 285 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE origin/main sources (98209ce2) with this item's two plane files swapped out and restored by sha256 AND cmp: pristine printed 282, so 63 of the move is PRE-EXISTING SLACK and +3 is this item's (C-38.7, C-38.8, C-61.1). */    /* REC-86 (IC-123, C-50 NARROW_CHECKS, 2026-09-18, worktree agent-a3fbd59a3fef1a961): MOVED 205 -> 219 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE HEAD sources (92f4c64e) with the item's four plane files swapped out and restored by sha256 + cmp: pristine printed 208, so 3 of the move is PRE-EXISTING slack and +11 (C-50.1..11) is this item's. */ /* MOVED 174 -> 176, 2026-08-10, worktree agent-a36b6782b06f5a651 (CASE-3), READ
  /* D-508 side, kept as history: */  /* D-508 (C-85 KNOCK_CHECKS, 2026-09-24, cloud session WORKER D-508 (CONDUCT #20), branch land/worker/D-508): MOVED 381 -> 383 from this guard's own --strict print on the item's tree over origin/main 68fecb8d0. Whose growth it absorbs: the two new rows C-85.1 RATE_IP and C-85.2 RATE_GLOBAL. ZERO of the move is pre-existing slack: the PRISTINE origin/main 68fecb8d0, run in this worktree before any edit, printed exit 0 with EVERY ratchet key at slack 0. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads regionLines at integration — and D-507 moves the same catalogue in parallel. */ /* CONDUCT #20 (c20-batch14, 2026-09-24): 379 -> 381, RE-READ from this guard's own `--strict` print on the union of this branch with `origin/main` @ 454a02bc (c20-batch17 and c20-batch18). Whose growth it absorbs: D-484's two new rows, C-33.40 NO_BASIS and C-33.41 NO_CITATION in ACT_SHAPE_CHECKS. Never either side's figure and never added by hand. */ /* D-484 (C-33.40 NO_BASIS, C-33.41 NO_CITATION consolidated behind one governed helper each, 2026-09-24, branch land/worker/D-484): MOVED 368 -> 370 from this guard's own --strict print on the item's tree over origin/main 16fe1e7f. ZERO pre-existing slack: the PRISTINE origin/main 16fe1e7f, run before any edit in this worktree, printed exit 0 with EVERY ratchet key at slack 0. the two new ACT_SHAPE_CHECKS rows. */ /* CONDUCT #20 (c20-batch14, 2026-09-24): 372 -> 379, RE-READ from this guard's own `--strict` print on the committed union of c20-batch11fix and c20-integ1b — never either branch's figure and never added by hand. Whose growth it absorbs: the SEVEN new rows are D-64's `RENDER_CAPTURE_CHECKS` family (C-83.1..7), carried in from c20-integ1b. Both branches had re-done the same c19-batch10 + `main` union, so each branch's own figure was short of the union's. */ /* CONDUCT #20 (c20-integ1, 2026-09-24): 369 -> 377, RE-READ from this guard's own --strict print on the committed union (D-64 renumbered C-82 -> C-83, REC-184, and origin/main @ 548eb2c5 merged over c19-batch10) — never added by hand, and never either side's figure. Whose growth it absorbs: +7 D-64 (C-83.1..7) and +1 the base merge — IC-246's C-82.1 came in from origin/main @ 548eb2c5, which the batch10 floor of 369 did not hold. */ /* D-64 (C-83 RENDER_CAPTURE_CHECKS, 2026-09-24, cloud session WORKER D-64 (CONDUCT #19), branch land/worker/D-64): MOVED 323 -> 330 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: seven rows, C-83.1..C-83.7. ZERO of the move is pre-existing slack: every other key gated at slack 0 on the same run, and the move is exactly this item's own. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration. */ /* CONDUCT #19 (c19-batch11, 2026-09-24): REC-192 @ 2e102941 (C-71.8, C-71.9; IC-248; the branch read 325 on main) merged over the batch; RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* REC-192 (the version arm of op=partitionindependence, 2026-09-24, cloud session WORKER REC-192 (CONDUCT #19), branch land/worker/REC-192): MOVED 323 -> 325, the figure this guard PRINTED on the item's tree over origin/main 15b2a4c0: two new rows in PARTITION_INDEPENDENCE_CHECKS (C-71.8 TWO_SUBJECTS, C-71.9 NO_SUCH_VERSION). ZERO of the move is pre-existing slack: the same guard over pristine origin/main printed exactly 323. Integrator: RE-READ from this guard's print on the committed merge. */ /* D-456 (C-78 NAMESPACE_CHECKS, 2026-09-23, cloud session WORKER D-456 (CONDUCT #18), branch land/worker/D-456): MOVED 322 -> 323 from this guard's own --strict print on the item's tree over origin/main a8f6094a: one new family, one row (NAMESPACE_UNKNOWN), one governed region (namespaceGate > is-namespace-gate, 5 lines), one code judged and compared. ZERO of the move is pre-existing slack: the same guard on a8f6094a before the change printed 322, every key gated at slack 0. */ /* CONDUCT #19 (c19-batch11, 2026-09-24): c19-unionfix @ 7b52ab9b (at 368) merged into c19-batch10 (at 369); RE-READ from this guard's --strict print on the merged tree, never added by hand. Both sides' comments carried below. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): REC-189 (C-32.19 MACHINE_CANNOT_SET_RISK_TIER, minted C-32.18) merged over the batch; 368 -> 369 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): D-461 (C-78.2 NAMESPACE_PINNED) merged over the batch; 367 -> 368 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* D-461 (C-78.2 NAMESPACE_PINNED, 2026-09-24, cloud session WORKER D-461 (CONDUCT #19)): MOVED 323 -> 324 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: one new row in the existing NAMESPACE_CHECKS family and one new governed REGION (pinnedNamespaceGate > is-pinned-namespace-gate). ZERO of the move is pre-existing slack: the same guard on 15b2a4c0 gated every key at slack 0. */ /* c19-unionfix (c19-batch9, 2026-09-24): 367 -> 368, re-read from this guard's --strict print on the union with IC-246: its ONE new family STATEMENT_ACK_CHECKS (C-82), ONE row and code STATEMENT_ACK_DOCUMENTS_OVER_BOUND, ONE region `is-statement-ack-documents-bound` in acknowledgeStatement. */ /* CONDUCT #18 (c18-batch8, 2026-09-23): D-394 merged over the batch; 353 -> 356 RE-READ from this guard's --strict print on the committed merge 8477cdb5. The new members are D-394's; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-126 (C-75 and C-76, the per-item weight) merged over the batch; 347 -> 353 RE-READ from this guard's --strict print on the committed merge 6ab139df. The new members are D-126 (C-75 and C-76, the per-item weight)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-149 (C-70, discoverable or hidden) merged over the batch; 343 -> 347 RE-READ from this guard's --strict print on the committed merge f4be4c07. The new members are REC-149 (C-70, discoverable or hidden)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-122 (C-74, the on-point choice) merged over the batch; 340 -> 343 RE-READ from this guard's --strict print on the committed merge e1d34ee0. The new members are REC-122 (C-74, the on-point choice)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-149 (C-73 and C-32.18, the governing laws) merged over the batch; 334 -> 340 RE-READ from this guard's --strict print on the committed merge 8086d64f. The new members are D-149 (C-73 and C-32.18, the governing laws)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-148 (C-72, the fee quote) merged over the batch; 326 -> 334 RE-READ from this guard's --strict print on the committed merge c81aec68. The new members are D-148 (C-72, the fee quote)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-164 merged over c17-batch6; 322 -> 326 RE-READ from this guard's --strict print on the committed merge 84443ad4. The new members are REC-164's (C-64.4..C-64.7, the group's name and domain acts); neither side's figure is the sum. */ /* CONDUCT #17 (c17-batch5, 2026-09-23): REC-161 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-161 (C-71 PARTITION_INDEPENDENCE_CHECKS, 2026-09-23, cloud session WORKER REC-161 (CONDUCT #17)): MOVED 309 -> 316 from this guard's own print on the item's tree over origin/main 02603e88: its seven rows, C-71.1..C-71.7. ZERO of the move is pre-existing slack: the same guard over a pristine 02603e88 worktree printed 309, every key gated at slack 0. */ /* CONDUCT #17 (c17-batch4, 2026-09-23): D-179 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-179 (C-53.13 CAPTURE_HELD_BY_ANOTHER_BUNDLE, 2026-09-23, branch land/worker/D-179): MOVED 305 -> 306 from this guard's own print on the item's tree: the one new row, C-53.13 CAPTURE_HELD_BY_ANOTHER_BUNDLE in TESTIMONY_CHECKS. ZERO pre-existing slack — every other key gated at 0 on the same run, and the move is exactly this item's own. */ /* CONDUCT #17 (c17-batch3, 2026-09-23): D-278 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-278 (C-68 + C-69, 2026-09-23, branch land/worker/D-278): MOVED 305 -> 310 from this guard's own print on the item's tree, every other ratchet key gated. five new rows: C-68.1-.4 and C-69.1. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): origin/main (c16-batch7) merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-168 (C-33.39 RETIRED_NOT_CITABLE in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-ab0b1ebe531cb428b): MOVED 302 -> 303 from this guard's own print on the item's tree: the one new row, C-33.39 RETIRED_NOT_CITABLE in ACT_SHAPE_CHECKS. ZERO pre-existing slack — the pristine HEAD (0e7cc03e) sources, swapped in and restored by sha256 AND cmp, printed 302, every key gated.  — CONDUCT #16 merged it onto REC-175/REC-176, which moved this key from the same base; the figure is re-read from the merged print. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): REC-177 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-177 (C-22.16 AI_RUN_BOUND_NO_ALLOWANCE in AI_RUN_CHECKS, 2026-09-23, worktree agent-ad7020ce0605889f4), RE-READ ON THE MERGE onto origin/main 0e5f7054 (which carries REC-175's and D-168's moves of this key from the same base): MOVED 303 -> 304 from this guard's own print on the merged tree: the one new row, C-22.16. ZERO of the move is pre-existing slack: every other key gated at slack 0 on the same run. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): REC-179 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-179 (C-66.5 SURFACED_BY_REWRITTEN in SURFACE_CHECKS, one new region promote > is-promote-surfaced-by, 2026-09-23, worktree agent-a368de25bd347b32f): MOVED 303 -> 304 from this guard's own print on the item's tree MERGED with origin/main 0e5f7054 (REC-175 landed first and moved this key from the same base, so this figure is re-read, never added), every other ratchet key gated. ZERO of the move is pre-existing slack: the merged tree without this item printed 303. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): D-440 and D-420 each moved this key by their own row from 0e5f7054; the union's value is RE-READ from this guard's print on the committed merge. */ /* D-420 (C-45.12 CONTENT_EXTENT_NO_IMAGE_PAINTED in CONTENT_EXTENT_CHECKS, 2026-09-23, cloud session WORKER D-420 (CONDUCT #16)): MOVED 303 -> 304 from this guard's own print on the item's tree over origin/main 0e5f7054, every other ratchet key gated at slack 0 on the same run. One new row, C-45.12. ZERO of the move is pre-existing slack: the baseline run on 0e5f7054 printed exactly the old floor, so the move is this item's own. D-440 moves this key from the same base; CONDUCT re-reads the figure from the merged print. */ /* D-440 (C-45.11 CONTENT_EXTENT_NOT_A_CONTAINER, 2026-09-23, worktree agent-aa6d775965f554582), RE-READ ON ITS MERGE with origin/main 0e5f7054 (which carries REC-175's moves from the same base): MOVED 303 -> 304 from this guard's own print on the merged tree, every other ratchet key gated. The one new row. ZERO pre-existing slack. */ /* REC-175 (C-33.38 FILE_DIGEST_MISMATCH in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-a6f377d2e96aa24af): MOVED 301 -> 302 from this guard's own print on the item's tree over origin/main 14faa089, every other ratchet key gated at slack 0 on the same run. One new row, C-33.38. ZERO of the move is pre-existing slack: the baseline run on 14faa089 printed exactly the old floor, so the move is this item's own.  — CONDUCT #16 merged it with REC-172, which moved this key from the same base; the figure is re-read from the merged print. */ /* REC-172 (C-22.15 AI_RUN_BOUND_UNKNOWN in AI_RUN_CHECKS, 2026-09-23, worktree agent-ad79857cb007927b7): MOVED 300 -> 301 from this guard's own print on the item's tree: the one new row. ZERO pre-existing slack — the pristine HEAD (91913d6b) sources printed 300, every key gated.  — CONDUCT #16 merged it with UI-81, which moved this key from the same base; the figure is re-read from the merged print. */ /* UI-81 (C-44.2 FINDING_IN_SEVERAL_CASES in CASE_DERIVATION_CHECKS, 2026-09-23, worktree agent-a306ff29faf082ef3), RE-READ ON THE MERGE onto origin/main 91913d6b (which carries REC-169's moves): MOVED 300 -> 301 from this guard's own print on the merged tree, every other ratchet key gated. One new row, C-44.2. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* REC-169 (C-22.13/C-22.14 in AI_RUN_CHECKS, 2026-09-23, worktree agent-a30f15e61a2d24169): MOVED 298 -> 300 from this guard's own print on the item's tree: the two new AI_RUN_CHECKS rows. The move is exactly this item's own additions, read off the per-site line; every other key gated on the same run. */ /* D-85 (C-66 SURFACE_CHECKS, 2026-09-23, worktree agent-a09e49cc337fc2200): MOVED 294 -> 298 from this guard's own print on the item's tree, every other ratchet key gated. Its four rows, C-66.1..C-66.4 (SURFACE_NO_RUN, SURFACE_RUN_NOT_RUNNING, SURFACE_NO_BOUND, SURFACE_BOUND_REACHED). ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* REC-165 (C-27.18 SUGGEST_RUN_NOT_RUNNING, C-27.19 SUGGEST_OUTSIDE_RUN_CONTEXT, 2026-09-22, worktree agent-a6e92d28647553f52): MOVED 291 -> 293 from this guard's own print on the item's tree. Every floor here is gated at slack 0 (M0-79), so main measured 291 and the +2 is this item's two rows. */ /* D-436 (C-64 INSTANCE_GROUP_CHECKS, 2026-09-21, worktree agent-a6dd0a0a3a3a6cc10): MOVED 288 -> 291 from this guard's own GREEN print on the item's committed tree 2f09b7b6. Its three rows, C-64.1 to C-64.3. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* D-158 (C-63 SIGNER_ENROLMENT_CHECKS, 2026-09-20, worktree d158-conduct8): MOVED 285 -> 288 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE origin/main sources (e1aa2eee) with this item's three plane files (store.mjs, setup.mjs, bio-checks.mjs) swapped out and restored by sha256 AND cmp: pristine printed 286, so 1 of the move is PRE-EXISTING SLACK and +2 is this item's (C-63.1 SIGNER_MEMBER_NOT_ENROLLED and C-63.2 SIGNER_MEMBER_NOT_ACTIVE). */    /* D-270 (C-38.7/C-38.8 + C-61 REQUIRED_ARGUMENT_CHECKS, 2026-09-19, worktree d270-record): MOVED 219 -> 285 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE origin/main sources (98209ce2) with this item's two plane files swapped out and restored by sha256 AND cmp: pristine printed 282, so 63 of the move is PRE-EXISTING SLACK and +3 is this item's (C-38.7, C-38.8, C-61.1). */    /* REC-86 (IC-123, C-50 NARROW_CHECKS, 2026-09-18, worktree agent-a3fbd59a3fef1a961): MOVED 205 -> 219 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE HEAD sources (92f4c64e) with the item's four plane files swapped out and restored by sha256 + cmp: pristine printed 208, so 3 of the move is PRE-EXISTING slack and +11 (C-50.1..11) is this item's. */ /* MOVED 174 -> 176, 2026-08-10, worktree agent-a36b6782b06f5a651 (CASE-3), READ
  ...
  /* CONDUCT #19 (c19-batch9, 2026-09-24): D-162 (the THEME family, renumbered C-74 -> C-81) merged over the batch; RE-READ from this guard's --strict print on the merged tree. */
  /* D-513 side, kept as history: census 699 on its own branch; the c20-batch27 union's figure is re-read from --strict's print. */
  /* D-463 side, kept as history: census 698 on its own branch; the c20-batch27 union's figure is re-read from the print. */
  /* D-510 side, kept as history: census 695 on its own branch; the c20-batch25 union's figure is re-read from the print. */
  /* D-472 side, kept as history: census 696 on its own branch; the c20-batch25 union's figure is re-read from the print. */
  /* D-491 side, kept as history: census 695 on its own branch; the c20-batch25 union's figure is re-read from the print. */
  /* REC-186 side, kept as history (c22-batch29: ours' census: 729 kept; NUMBER TO RE-READ from --strict's print on the union): census: 701, — REC-186 (2026-09-25, branch land/worker/REC-186): 700 -> 701, READ from this guard's print on the item's tree over origin/main 8bdf20e6. Whose growth: C-33.48 LAST_OWNER_CANNOT_LEAVE and its region is-leave-owner-floor in projectLeave. */
  /* D-147 side, kept as history (c22-batch29: ours' census: 729 kept; NUMBER TO RE-READ from --strict's print on the union): census: 716, — D-147 (C-94 LIFECYCLE_CHECKS, 2026-09-25, branch land/worker/D-147): 705 -> 716, READ from this guard's own print on the item's tree over origin/main 964da679 — the new family, its eleven rows and codes, and its three regions (is-lifecycle-writable, is-lifecycle-grammar, is-lifecycle-due-cite). Every other key gated at slack 0 on the same run. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads regionLines at integration. */
  /* REC-203 side, kept as history (c22-batch29: ours' census: 729 kept; NUMBER TO RE-READ from --strict's print on the union): census: 708, — REC-203 (C-91 IDSPACE_CHECKS, 2026-09-25, branch land/worker/REC-203): MOVED 705 -> 708 from this guard's own --strict print on the item's tree over origin/main 964da679. Whose growth it absorbs: op=idmatch's ONE new family and its three refusals, each at its own DEC-49 REGION in store.mjs idMatch. regionLines is a property of the MERGED source, so CONDUCT re-reads it at integration. */
  /* D-547 side, kept as history: census 701 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-217 side, kept as history: census 703 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-468 side, kept as history: census 701 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-214 side, kept as history: census 705 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-512 side, kept as history: census 701 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-205 side, kept as history: census 697 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* c21-rec207-renumber side, kept as history: census 706 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-530 side, kept as history: census 701 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-454 side, kept as history: census 701 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-150 side, kept as history: census 714 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-520 side, kept as history: census 701 on its own branch; the c22-batch29 union's figure is re-read from --strict's print. */
  /* D-134 side, kept as history: census 706 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* MK-7 side, kept as history: census 717 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* REC-147 side, kept as history: census 712 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* REC-197 side, kept as history: census 707 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-521b side, kept as history: census 728 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-563 side, kept as history: census 731 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-561 side, kept as history: census 732 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-578 side, kept as history: census 732 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-546 side, kept as history: census 733 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-615 side, kept as history: census 734 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-629 side, kept as history: census 731 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-680 side, kept as history: census 730 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* REC-215 side, kept as history: census 730 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-419 side, kept as history: census 734 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-689 side, kept as history: census 730 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  /* D-695 side, kept as history: census 731 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  /* D-670 side, kept as history: census 730 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  /* D-688 side, kept as history: census 781 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  /* D-628 side, kept as history: census 735 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  census: 797, /* CONDUCT #23 at the c23-batch30 union: 780 -> 797, READ from `node civicos-ui/check-refusal-codes.mjs --strict`'s print on the merged tree (HEAD 645f8c41); never a branch's figure, never added by hand. */ /* CONDUCT #22 at the c22-batch29 union: 729 -> 780, the union's figure READ from `node civicos-ui/check-refusal-codes.mjs --strict`'s print on the merged tree (`ratchet: census ... measured 780`). */ /* CONDUCT #21 at c21-batch28: 705 -> 729, the union's figure READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch27: 700 -> 705, the union's figure (D-463, D-513 and the batch) READ from --strict's print on the committed merge. */ /* CONDUCT #20 at c20-batch25: 699 -> 700, D-510's C-86.1 (the PROMOTED_TYPE_CHECKS family and its region) over the union, READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch25: 697 -> 699, D-472's C-48.8/C-48.9 and their two regions over the union, READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch25: 696 -> 697, D-491's C-28.16 (and its is-capture-request region) over the batch24c union, READ from --strict's print on the merged tree. */ /* REC-211 (IC-273, 2026-09-24, branch land/worker/REC-211): 694 -> 696, MOVED from this guard's own print on this item's REBASED, COMMITTED tree over `origin/main` @ 58293bf31 — never by adding to the number here. ZERO PRE-EXISTING SLACK, measured rather than assumed: that pristine `origin/main` was checked out into a scratch worktree OUTSIDE this one and run before any edit was compared, and printed exit 0 with EVERY ratchet key at slack 0, so the whole move is this item's. Whose growth it absorbs: the same two rows reaching the census. */ /* CONDUCT #20 (c20-batch14, 2026-09-24): 693 -> 694, RE-READ from this guard's own `--strict` print on the union of this branch with `origin/main` @ 454a02bc (c20-batch17 and c20-batch18). Whose growth it absorbs: one new code enters the plane census with D-484's pair (one of the two was already spelled in src). Never either side's figure and never added by hand. */ /* REC-193 (2026-09-24, land/worker/REC-193): MOVED 682 -> 683 from this guard's own print on the item's tree over origin/main 0fdef669: the ONE new code, STATEMENT_ACK_AUTHOR_UNDETERMINED, minted in src/store.mjs acknowledgeStatement beside D-150's five STATEMENT_ACK_* codes (a draft that records no author for its statement is refused rather than attributed to its last editor). ZERO of the move is pre-existing slack, MEASURED: this guard run over the PRISTINE origin/main src/store.mjs, swapped in and restored by sha256 AND cmp (b23856ca...5dfc, 3,166,538 B), printed census 682 and untranslated 301, every key gated at slack 0. */ /* CONDUCT #20 (c20-batch14, 2026-09-24): 686 -> 693, RE-READ from this guard's own `--strict` print on the committed union of c20-batch11fix and c20-integ1b — never either branch's figure and never added by hand. Whose growth it absorbs: the SEVEN new codes are D-64's C-83.1..7, the same seven. Both branches had re-done the same c19-batch10 + `main` union, so each branch's own figure was short of the union's. */ /* CONDUCT #20 (c20-integ1, 2026-09-24): 683 -> 691, RE-READ from this guard's own --strict print on the committed union (D-64 renumbered C-82 -> C-83, REC-184, and origin/main @ 548eb2c5 merged over c19-batch10) — never added by hand, and never either side's figure. Whose growth it absorbs: +7 D-64's codes and +1 IC-246's STATEMENT_ACK_DOCUMENTS_OVER_BOUND, arriving with origin/main. */ /* D-64 (C-83 RENDER_CAPTURE_CHECKS, 2026-09-24, cloud session WORKER D-64 (CONDUCT #19), branch land/worker/D-64): MOVED 632 -> 639 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: the seven RENDER_* codes. ZERO of the move is pre-existing slack: every other key gated at slack 0 on the same run, and the move is exactly this item's own. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration. */ /* CONDUCT #19 (c19-batch11, 2026-09-24): REC-192 @ 2e102941 (C-71.8, C-71.9; IC-248; the branch read 634 on main) merged over the batch; RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* REC-192 (the version arm of op=partitionindependence, 2026-09-24, cloud session WORKER REC-192 (CONDUCT #19), branch land/worker/REC-192): MOVED 632 -> 634, the figure this guard PRINTED on the item's tree over origin/main 15b2a4c0: the same two codes, now minted in src/store.mjs. ZERO of the move is pre-existing slack: the same guard over pristine origin/main printed exactly 632. Integrator: RE-READ from this guard's print on the committed merge. */ /* D-456 (C-78 NAMESPACE_CHECKS, 2026-09-23, cloud session WORKER D-456 (CONDUCT #18), branch land/worker/D-456): MOVED 631 -> 632 from this guard's own --strict print on the item's tree over origin/main a8f6094a: one new family, one row (NAMESPACE_UNKNOWN), one governed region (namespaceGate > is-namespace-gate, 5 lines), one code judged and compared. ZERO of the move is pre-existing slack: the same guard on a8f6094a before the change printed 631, every key gated at slack 0. */ /* CONDUCT #19 (c19-batch11, 2026-09-24): c19-unionfix @ 7b52ab9b (at 682) merged into c19-batch10 (at 683); RE-READ from this guard's --strict print on the merged tree, never added by hand. Both sides' comments carried below. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): REC-189 (C-32.19 MACHINE_CANNOT_SET_RISK_TIER, minted C-32.18) merged over the batch; 682 -> 683 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): D-461 (C-78.2 NAMESPACE_PINNED) merged over the batch; 681 -> 682 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* D-461 (C-78.2 NAMESPACE_PINNED, 2026-09-24, cloud session WORKER D-461 (CONDUCT #19)): MOVED 632 -> 633 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: one new row in the existing NAMESPACE_CHECKS family and one new governed REGION (pinnedNamespaceGate > is-pinned-namespace-gate). ZERO of the move is pre-existing slack: the same guard on 15b2a4c0 gated every key at slack 0. */ /* c19-unionfix (c19-batch9, 2026-09-24): 681 -> 682, re-read from this guard's --strict print on the union with IC-246: its ONE new family STATEMENT_ACK_CHECKS (C-82), ONE row and code STATEMENT_ACK_DOCUMENTS_OVER_BOUND, ONE region `is-statement-ack-documents-bound` in acknowledgeStatement. */ /* CONDUCT #18 (c18-batch8, 2026-09-23): D-394 merged over the batch; 667 -> 670 RE-READ from this guard's --strict print on the committed merge 8477cdb5. The new members are D-394's; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-126 (C-75 and C-76, the per-item weight) merged over the batch; 662 -> 667 RE-READ from this guard's --strict print on the committed merge 6ab139df. The new members are D-126 (C-75 and C-76, the per-item weight)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-149 (C-70, discoverable or hidden) merged over the batch; 658 -> 662 RE-READ from this guard's --strict print on the committed merge f4be4c07. The new members are REC-149 (C-70, discoverable or hidden)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-122 (C-74, the on-point choice) merged over the batch; 655 -> 658 RE-READ from this guard's --strict print on the committed merge e1d34ee0. The new members are REC-122 (C-74, the on-point choice)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-149 (C-73 and C-32.18, the governing laws) merged over the batch; 648 -> 655 RE-READ from this guard's --strict print on the committed merge 8086d64f. The new members are D-149 (C-73 and C-32.18, the governing laws)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-148 (C-72, the fee quote) merged over the batch; 640 -> 648 RE-READ from this guard's --strict print on the committed merge c81aec68. The new members are D-148 (C-72, the fee quote)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-150 merged over REC-164; 635 -> 640 RE-READ from this guard's --strict print on the committed merge 4e45ca5d. The new members are D-150's five STATEMENT_ACK_* codes; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-164 merged over c17-batch6; 631 -> 635 RE-READ from this guard's --strict print on the committed merge 84443ad4. The new members are REC-164's (C-64.4..C-64.7, the group's name and domain acts); neither side's figure is the sum. */ /* CONDUCT #17 (c17-batch5, 2026-09-23): REC-161 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-161 (C-71 PARTITION_INDEPENDENCE_CHECKS, 2026-09-23, cloud session WORKER REC-161 (CONDUCT #17)): MOVED 618 -> 625 from this guard's own print on the item's tree over origin/main 02603e88: the seven PARTITION_INDEPENDENCE_* codes. ZERO of the move is pre-existing slack: the same guard over a pristine 02603e88 worktree printed 618, every key gated at slack 0. */ /* CONDUCT #17 (c17-batch4, 2026-09-23): D-179 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-179 (C-53.13 CAPTURE_HELD_BY_ANOTHER_BUNDLE, 2026-09-23, branch land/worker/D-179): MOVED 614 -> 615 from this guard's own print on the item's tree: the one new code, CAPTURE_HELD_BY_ANOTHER_BUNDLE. ZERO pre-existing slack — every other key gated at 0 on the same run, and the move is exactly this item's own. */ /* CONDUCT #17 (c17-batch3, 2026-09-23): D-278 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-278 (C-68 + C-69, 2026-09-23, branch land/worker/D-278): MOVED 614 -> 619 from this guard's own print on the item's tree, every other ratchet key gated. five new codes: EVIDENCE_STORAGE_NOT_CONFIGURED, BOOTSTRAP_CREDENTIAL_UNSET/_PUBLISHED/_MISMATCH, UNKNOWN_OP. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): origin/main (c16-batch7) merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-168 (C-33.39 RETIRED_NOT_CITABLE in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-ab0b1ebe531cb428b): MOVED 611 -> 612 from this guard's own print on the item's tree: the one new code, RETIRED_NOT_CITABLE, minted in src/store.mjs cite. ZERO pre-existing slack — the pristine HEAD (0e7cc03e) sources, swapped in and restored by sha256 AND cmp, printed 611, every key gated.  — CONDUCT #16 merged it onto REC-175/REC-176, which moved this key from the same base; the figure is re-read from the merged print. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): REC-177 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-177 (C-22.16 AI_RUN_BOUND_NO_ALLOWANCE in AI_RUN_CHECKS, 2026-09-23, worktree agent-ad7020ce0605889f4), RE-READ ON THE MERGE onto origin/main 0e5f7054 (which carries REC-175's and D-168's moves of this key from the same base): MOVED 612 -> 613 from this guard's own print on the merged tree: AI_RUN_BOUND_NO_ALLOWANCE, the one new code. ZERO of the move is pre-existing slack: every other key gated at slack 0 on the same run. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): REC-179 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-179 (C-66.5 SURFACED_BY_REWRITTEN in SURFACE_CHECKS, one new region promote > is-promote-surfaced-by, 2026-09-23, worktree agent-a368de25bd347b32f): MOVED 612 -> 613 from this guard's own print on the item's tree MERGED with origin/main 0e5f7054 (REC-175 landed first and moved this key from the same base, so this figure is re-read, never added), every other ratchet key gated. ZERO of the move is pre-existing slack: the merged tree without this item printed 612. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): D-440 and D-420 each moved this key by their own row from 0e5f7054; the union's value is RE-READ from this guard's print on the committed merge. */ /* D-420 (C-45.12 CONTENT_EXTENT_NO_IMAGE_PAINTED in CONTENT_EXTENT_CHECKS, 2026-09-23, cloud session WORKER D-420 (CONDUCT #16)): MOVED 612 -> 613 from this guard's own print on the item's tree over origin/main 0e5f7054, every other ratchet key gated at slack 0 on the same run. One new code minted in the `is-content-extent` region: CONTENT_EXTENT_NO_IMAGE_PAINTED. ZERO of the move is pre-existing slack: the baseline run on 0e5f7054 printed exactly the old floor, so the move is this item's own. D-440 moves this key from the same base; CONDUCT re-reads the figure from the merged print. */ /* D-440 (C-45.11, 2026-09-23, worktree agent-aa6d775965f554582), RE-READ ON ITS MERGE with origin/main 0e5f7054: MOVED 612 -> 613 from this guard's own print on the merged tree. The one new code. ZERO pre-existing slack. */ /* REC-175 (C-33.38 FILE_DIGEST_MISMATCH in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-a6f377d2e96aa24af): MOVED 610 -> 611 from this guard's own print on the item's tree over origin/main 14faa089, every other ratchet key gated at slack 0 on the same run. One new code minted in src/store.mjs promote: FILE_DIGEST_MISMATCH. ZERO of the move is pre-existing slack: the baseline run on 14faa089 printed exactly the old floor, so the move is this item's own.  — CONDUCT #16 merged it with REC-172, which moved this key from the same base; the figure is re-read from the merged print. */ /* REC-172 (2026-09-23, worktree agent-ad79857cb007927b7): MOVED 610 -> 611 from this guard's own print: AI_RUN_BOUND_UNKNOWN, the one new code. The pristine HEAD (91913d6b) printed 610, so ZERO of the move is pre-existing slack. */ /* REC-169 (C-22.13/C-22.14 in AI_RUN_CHECKS, 2026-09-23, worktree agent-a30f15e61a2d24169): MOVED 608 -> 610 from this guard's own print on the item's tree: the two new codes, AI_RUN_CONSUME_INVALID and AI_RUN_BOUND_PLANE_COUNTED. The move is exactly this item's own additions, read off the per-site line; every other key gated on the same run. */ /* D-85 (C-66 SURFACE_CHECKS, 2026-09-23, worktree agent-a09e49cc337fc2200): MOVED 604 -> 608 from this guard's own print on the item's tree, every other ratchet key gated. Four new codes minted in src/store.mjs #surfacingGate. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* REC-165 (2026-09-22, worktree agent-a6e92d28647553f52): MOVED 601 -> 603 from this guard's own print (the commit-at-HEAD figure). The two codes minted in src/store.mjs suggestVersion: SUGGEST_RUN_NOT_RUNNING, SUGGEST_OUTSIDE_RUN_CONTEXT. Slack 0 on main (M0-79), so the +2 is this item's. */ /* D-436 (C-64 INSTANCE_GROUP_CHECKS, 2026-09-21, worktree agent-a6dd0a0a3a3a6cc10): MOVED 598 -> 601 from this guard's own GREEN print on the item's committed tree 2f09b7b6. Three new codes minted in src/store.mjs: GROUP_UNDETERMINED, GROUP_SLUG_MALFORMED, GROUP_ALREADY_RECORDED. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* D-158 (C-63 SIGNER_ENROLMENT_CHECKS, 2026-09-20, worktree d158-conduct8): MOVED 595 -> 598 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE origin/main sources (e1aa2eee) with this item's three plane files (store.mjs, setup.mjs, bio-checks.mjs) swapped out and restored by sha256 AND cmp: pristine printed 596, so 1 of the move is PRE-EXISTING SLACK and +2 is this item's (the two new codes). */    /* D-270 (C-38.7/C-38.8 + C-61 REQUIRED_ARGUMENT_CHECKS, 2026-09-19, worktree d270-record): MOVED 517 -> 595 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE origin/main sources (98209ce2) with this item's two plane files swapped out and restored by sha256 AND cmp: pristine printed 592, so 75 of the move is PRE-EXISTING SLACK and +3 is this item's (the three new codes). */    /* REC-86 (IC-123, C-50 NARROW_CHECKS, 2026-09-18, worktree agent-a3fbd59a3fef1a961): MOVED 490 -> 517 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE HEAD sources (92f4c64e) with the item's four plane files swapped out and restored by sha256 + cmp: pristine printed 506, so 16 of the move is PRE-EXISTING slack and +11 is this item's. */ /* MOVED 433 -> 437, 2026-08-10, worktree agent-a36b6782b06f5a651 (CASE-3), from
  ...
  /* CONDUCT #19 (c19-batch9, 2026-09-24): D-162 (the THEME family, renumbered C-74 -> C-81) merged over the batch; RE-READ from this guard's --strict print on the merged tree. */
  /* D-510 side, kept as history: reach 434 on its own branch; the c20-batch25 union's figure is re-read from the print. */
  /* D-472 side, kept as history: reach 435 on its own branch; the c20-batch25 union's figure is re-read from the print. */
  /* D-491 side, kept as history: reach 434 on its own branch; the c20-batch25 union's figure is re-read from the print. */
  /* REC-211 side, kept as history: reach 433 -> 435 on its own branch (IC-273); the c20-batch23 union's figure is re-read from --strict's print. */
  /* REC-186 side, kept as history (c22-batch29: ours' reach: 488 kept; NUMBER TO RE-READ from --strict's print on the union): reach: 448, — REC-186 (2026-09-25, branch land/worker/REC-186): 447 -> 448, READ from this guard's print on the item's tree over origin/main 8bdf20e6. Whose growth: C-33.48 LAST_OWNER_CANNOT_LEAVE and its region is-leave-owner-floor in projectLeave. */
  /* D-513 side, kept as history: reach 446 on its own branch; the c20-batch27 union's figure is re-read from --strict's print. */
  /* D-463 side, kept as history: reach 445 on its own branch; the c20-batch27 union's figure is re-read from the print. */
  /* D-147 side, kept as history (c22-batch29: ours' reach: 488 kept; NUMBER TO RE-READ from --strict's print on the union): reach: 463, — D-147 (C-94 LIFECYCLE_CHECKS, 2026-09-25, branch land/worker/D-147): 452 -> 463, READ from this guard's own print on the item's tree over origin/main 964da679 — the new family, its eleven rows and codes, and its three regions (is-lifecycle-writable, is-lifecycle-grammar, is-lifecycle-due-cite). Every other key gated at slack 0 on the same run. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads regionLines at integration. */
  /* REC-203 side, kept as history (c22-batch29: ours' reach: 488 kept; NUMBER TO RE-READ from --strict's print on the union): reach: 455, — REC-203 (C-91 IDSPACE_CHECKS, 2026-09-25, branch land/worker/REC-203): MOVED 452 -> 455 from this guard's own --strict print on the item's tree over origin/main 964da679. Whose growth it absorbs: op=idmatch's ONE new family and its three refusals, each at its own DEC-49 REGION in store.mjs idMatch. regionLines is a property of the MERGED source, so CONDUCT re-reads it at integration. */
  /* D-547 side, kept as history: reach 448 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-217 side, kept as history: reach 450 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-448 side, kept as history: reach 458 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-468 side, kept as history: reach 448 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-214 side, kept as history: reach 452 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-549 side, kept as history: reach 448 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-512 side, kept as history: reach 448 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-205 side, kept as history: reach 444 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* c21-rec207-renumber side, kept as history: reach 453 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-530 side, kept as history: reach 448 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-454 side, kept as history: reach 448 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-150 side, kept as history: reach 461 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-520 side, kept as history: reach 448 on its own branch; the c22-batch29 union's figure is re-read from --strict's print. */
  /* D-134 side, kept as history: reach 461 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* M0-148 side, kept as history: reach 457 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* MK-7 side, kept as history: reach 464 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* REC-147 side, kept as history: reach 459 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* REC-197 side, kept as history: reach 454 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-521b side, kept as history: reach 487 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-563 side, kept as history: reach 490 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-561 side, kept as history: reach 496 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-578 side, kept as history: reach 491 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-546 side, kept as history: reach 492 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-615 side, kept as history: reach 493 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-623 side, kept as history: reach 489 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-629 side, kept as history: reach 490 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-680 side, kept as history: reach 489 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* REC-215 side, kept as history: reach 489 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-419 side, kept as history: reach 493 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-542 side, kept as history (c22-batch30: ours' reach kept; NUMBER TO RE-READ from --strict's print on the union): reach: 595, — D-542 / D-562 (2026-09-25, branch land/worker/D-542): MOVED 488 -> 595 from this guard's own --strict print on the item's tree over origin/main 5e8a65a8. Whose growth: R5 BY OP and R6 PUBLIC OP, the two rules this item adds — 107 codes that no row, no app.html literal and no mock put in reach, each minted on an op app.html calls or a stranger can call; named on the `IN REACH ONLY BY OP` line. A widening of the instrument, not of the plane. */
  /* D-689 side, kept as history: reach 489 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  /* D-695 side, kept as history: reach 490 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  /* D-670 side, kept as history: reach 489 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  /* D-688 side, kept as history: reach 558 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  /* D-628 side, kept as history: reach 494 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  reach: 675, /* CONDUCT #23 at the c23-batch30 union: 557 -> 675, READ from `node civicos-ui/check-refusal-codes.mjs --strict`'s print on the merged tree (HEAD 645f8c41); never a branch's figure, never added by hand. */ /* CONDUCT #22 at the c22-batch29 union: 488 -> 557, the union's figure READ from `node civicos-ui/check-refusal-codes.mjs --strict`'s print on the merged tree (`ratchet: reach ... measured 557`). */ /* CONDUCT #21 at c21-batch28: 452 -> 488, the union's figure READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch27: 447 -> 452, the union's figure (D-463, D-513 and the batch) READ from --strict's print on the committed merge. */ /* CONDUCT #20 at c20-batch25: 446 -> 447, D-510's C-86.1 (the PROMOTED_TYPE_CHECKS family and its region) over the union, READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch25: 444 -> 446, D-472's C-48.8/C-48.9 and their two regions over the union, READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch25: 443 -> 444, D-491's C-28.16 (and its is-capture-request region) over the batch24c union, READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch23: 441 -> 443, REC-211's arrivals over the c20-batch22 union, READ from this guard's --strict print on the merged tree. */ /* CONDUCT #20 at c20-batch22: the UNION of D-507 (433 -> 439) and D-508 (433 -> 435), READ from this guard's --strict print on the merged tree. */ /* D-507 (C-82.2..C-82.7, the six STATEMENT_ACK_* refusals that reached a member untranslated, 2026-09-24, branch land/worker/D-507): MOVED 433 -> 439 from this guard's own --strict print on the item's tree over origin/main 68fecb8d. The same six, now R1 family rows; two of them (STATEMENT_ACK_BY_ITS_AUTHOR, STATEMENT_ACK_NO_STATEMENT) were in arm B's OBSERVED-ONLY set and therefore NOT in reach before, so the move is +6 and not +4. ZERO of the move is pre-existing slack: the PRISTINE origin/main 68fecb8d, run in this worktree before any edit, printed exit 0 with EVERY ratchet key at slack 0. */  /* CONDUCT #20 (c20-batch14, 2026-09-24): 432 -> 433, RE-READ from this guard's own `--strict` print on the union of this branch with `origin/main` @ 454a02bc (c20-batch17 and c20-batch18). Whose growth it absorbs: one more code a surface can receive, D-484's. Never either side's figure and never added by hand. */ /* D-484 (C-33.40 NO_BASIS, C-33.41 NO_CITATION consolidated behind one governed helper each, 2026-09-24, branch land/worker/D-484): MOVED 421 -> 422 from this guard's own --strict print on the item's tree over origin/main 16fe1e7f. ZERO pre-existing slack: the PRISTINE origin/main 16fe1e7f, run before any edit in this worktree, printed exit 0 with EVERY ratchet key at slack 0. R1 gains two rows; only NO_CITATION is a new code IN REACH, because NO_BASIS was already reached through R3 (conclude-act.test.mjs feeds it) and the union does not double-count it. +2 rows, +1 reach, and the difference is the measurement rather than an error. */ /* CONDUCT #20 (c20-batch14, 2026-09-24): 425 -> 432, RE-READ from this guard's own `--strict` print on the committed union of c20-batch11fix and c20-integ1b — never either branch's figure and never added by hand. Whose growth it absorbs: the SEVEN are D-64's C-83.1..7, each reachable through `op=acquire`'s render arm. Both branches had re-done the same c19-batch10 + `main` union, so each branch's own figure was short of the union's. */ /* CONDUCT #20 (c20-integ1, 2026-09-24): 422 -> 430, RE-READ from this guard's own --strict print on the committed union (D-64 renumbered C-82 -> C-83, REC-184, and origin/main @ 548eb2c5 merged over c19-batch10) — never added by hand, and never either side's figure. Whose growth it absorbs: +7 D-64 and +1 IC-246, the same two sources. */ /* D-64 (C-83 RENDER_CAPTURE_CHECKS, 2026-09-24, cloud session WORKER D-64 (CONDUCT #19), branch land/worker/D-64): MOVED 376 -> 383 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: the seven family rows (R1). ZERO of the move is pre-existing slack: every other key gated at slack 0 on the same run, and the move is exactly this item's own. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration. */ /* CONDUCT #19 (c19-batch11, 2026-09-24): REC-192 @ 2e102941 (C-71.8, C-71.9; IC-248; the branch read 378 on main) merged over the batch; RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* REC-192 (the version arm of op=partitionindependence, 2026-09-24, cloud session WORKER REC-192 (CONDUCT #19), branch land/worker/REC-192): MOVED 376 -> 378, the figure this guard PRINTED on the item's tree over origin/main 15b2a4c0: the two new rows (R1). ZERO of the move is pre-existing slack: the same guard over pristine origin/main printed exactly 376. Integrator: RE-READ from this guard's print on the committed merge. */ /* D-456 (C-78 NAMESPACE_CHECKS, 2026-09-23, cloud session WORKER D-456 (CONDUCT #18), branch land/worker/D-456): MOVED 375 -> 376 from this guard's own --strict print on the item's tree over origin/main a8f6094a: one new family, one row (NAMESPACE_UNKNOWN), one governed region (namespaceGate > is-namespace-gate, 5 lines), one code judged and compared. ZERO of the move is pre-existing slack: the same guard on a8f6094a before the change printed 375, every key gated at slack 0. */ /* CONDUCT #19 (c19-batch11, 2026-09-24): c19-unionfix @ 7b52ab9b (at 421) merged into c19-batch10 (at 422); RE-READ from this guard's --strict print on the merged tree, never added by hand. Both sides' comments carried below. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): REC-189 (C-32.19 MACHINE_CANNOT_SET_RISK_TIER, minted C-32.18) merged over the batch; 421 -> 422 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): D-461 (C-78.2 NAMESPACE_PINNED) merged over the batch; 420 -> 421 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* D-461 (C-78.2 NAMESPACE_PINNED, 2026-09-24, cloud session WORKER D-461 (CONDUCT #19)): MOVED 376 -> 377 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: one new row in the existing NAMESPACE_CHECKS family and one new governed REGION (pinnedNamespaceGate > is-pinned-namespace-gate). ZERO of the move is pre-existing slack: the same guard on 15b2a4c0 gated every key at slack 0. */ /* c19-unionfix (c19-batch9, 2026-09-24): 420 -> 421, re-read from this guard's --strict print on the union with IC-246: its ONE new family STATEMENT_ACK_CHECKS (C-82), ONE row and code STATEMENT_ACK_DOCUMENTS_OVER_BOUND, ONE region `is-statement-ack-documents-bound` in acknowledgeStatement. */ /* CONDUCT #18 (c18-batch8, 2026-09-23): D-394 merged over the batch; 406 -> 409 RE-READ from this guard's --strict print on the committed merge 8477cdb5. The new members are D-394's; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-126 (C-75 and C-76, the per-item weight) merged over the batch; 400 -> 406 RE-READ from this guard's --strict print on the committed merge 6ab139df. The new members are D-126 (C-75 and C-76, the per-item weight)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-149 (C-70, discoverable or hidden) merged over the batch; 396 -> 400 RE-READ from this guard's --strict print on the committed merge f4be4c07. The new members are REC-149 (C-70, discoverable or hidden)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-122 (C-74, the on-point choice) merged over the batch; 393 -> 396 RE-READ from this guard's --strict print on the committed merge e1d34ee0. The new members are REC-122 (C-74, the on-point choice)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-149 (C-73 and C-32.18, the governing laws) merged over the batch; 387 -> 393 RE-READ from this guard's --strict print on the committed merge 8086d64f. The new members are D-149 (C-73 and C-32.18, the governing laws)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-148 (C-72, the fee quote) merged over the batch; 379 -> 387 RE-READ from this guard's --strict print on the committed merge c81aec68. The new members are D-148 (C-72, the fee quote)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-164 merged over c17-batch6; 375 -> 379 RE-READ from this guard's --strict print on the committed merge 84443ad4. The new members are REC-164's (C-64.4..C-64.7, the group's name and domain acts); neither side's figure is the sum. */ /* CONDUCT #17 (c17-batch5, 2026-09-23): REC-161 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-161 (C-71 PARTITION_INDEPENDENCE_CHECKS, 2026-09-23, cloud session WORKER REC-161 (CONDUCT #17)): MOVED 362 -> 369 from this guard's own print on the item's tree over origin/main 02603e88: the seven C-71 rows. ZERO of the move is pre-existing slack: the same guard over a pristine 02603e88 worktree printed 362, every key gated at slack 0. */ /* CONDUCT #17 (c17-batch4, 2026-09-23): D-179 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-179 (C-53.13 CAPTURE_HELD_BY_ANOTHER_BUNDLE, 2026-09-23, branch land/worker/D-179): MOVED 358 -> 359 from this guard's own print on the item's tree: R1: the one new family row. ZERO pre-existing slack — every other key gated at 0 on the same run, and the move is exactly this item's own. */ /* CONDUCT #17 (c17-batch3, 2026-09-23): D-278 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-278 (C-68 + C-69, 2026-09-23, branch land/worker/D-278): MOVED 358 -> 363 from this guard's own print on the item's tree, every other ratchet key gated. the five new codes, reached as R1 family rows. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): origin/main (c16-batch7) merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-168 (C-33.39 RETIRED_NOT_CITABLE in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-ab0b1ebe531cb428b): MOVED 355 -> 356 from this guard's own print on the item's tree: the new row reaches by R1 (a family row). ZERO pre-existing slack — the pristine HEAD (0e7cc03e) sources, swapped in and restored by sha256 AND cmp, printed 355, every key gated.  — CONDUCT #16 merged it onto REC-175/REC-176, which moved this key from the same base; the figure is re-read from the merged print. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): REC-177 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-177 (C-22.16 AI_RUN_BOUND_NO_ALLOWANCE in AI_RUN_CHECKS, 2026-09-23, worktree agent-ad7020ce0605889f4), RE-READ ON THE MERGE onto origin/main 0e5f7054 (which carries REC-175's and D-168's moves of this key from the same base): MOVED 356 -> 357 from this guard's own print on the merged tree: the new row's R1 reach. ZERO of the move is pre-existing slack: every other key gated at slack 0 on the same run. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): REC-179 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-179 (C-66.5 SURFACED_BY_REWRITTEN in SURFACE_CHECKS, one new region promote > is-promote-surfaced-by, 2026-09-23, worktree agent-a368de25bd347b32f): MOVED 356 -> 357 from this guard's own print on the item's tree MERGED with origin/main 0e5f7054 (REC-175 landed first and moved this key from the same base, so this figure is re-read, never added), every other ratchet key gated. ZERO of the move is pre-existing slack: the merged tree without this item printed 356. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): D-440 and D-420 each moved this key by their own row from 0e5f7054; the union's value is RE-READ from this guard's print on the committed merge. */ /* D-420 (C-45.12 CONTENT_EXTENT_NO_IMAGE_PAINTED in CONTENT_EXTENT_CHECKS, 2026-09-23, cloud session WORKER D-420 (CONDUCT #16)): MOVED 356 -> 357 from this guard's own print on the item's tree over origin/main 0e5f7054, every other ratchet key gated at slack 0 on the same run. Its one R1 family row. ZERO of the move is pre-existing slack: the baseline run on 0e5f7054 printed exactly the old floor, so the move is this item's own. D-440 moves this key from the same base; CONDUCT re-reads the figure from the merged print. */ /* D-440 (C-45.11, 2026-09-23, worktree agent-aa6d775965f554582), RE-READ ON ITS MERGE with origin/main 0e5f7054: MOVED 356 -> 357 from this guard's own print on the merged tree — the new row's R1 reach. ZERO pre-existing slack. */ /* REC-175 (C-33.38 FILE_DIGEST_MISMATCH in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-a6f377d2e96aa24af): MOVED 354 -> 355 from this guard's own print on the item's tree over origin/main 14faa089, every other ratchet key gated at slack 0 on the same run. Its one R1 family row. ZERO of the move is pre-existing slack: the baseline run on 14faa089 printed exactly the old floor, so the move is this item's own.  — CONDUCT #16 merged it with REC-172, which moved this key from the same base; the figure is re-read from the merged print. */ /* REC-172 (2026-09-23, worktree agent-ad79857cb007927b7): MOVED 353 -> 354 from this guard's own print: the new row's R1 reach. The pristine HEAD (91913d6b) printed 353: ZERO pre-existing slack.  — CONDUCT #16 merged it with UI-81, which moved this key from the same base; the figure is re-read from the merged print. */ /* UI-81 (C-44.2 FINDING_IN_SEVERAL_CASES in CASE_DERIVATION_CHECKS, 2026-09-23, worktree agent-a306ff29faf082ef3), RE-READ ON THE MERGE onto origin/main 91913d6b (which carries REC-169's moves): MOVED 353 -> 354 from this guard's own print on the merged tree, every other ratchet key gated. The new row, in reach by R1 and arriving TRANSLATED; app.html keys on the refusal's `cases[]` and names no code, so R2 did not move. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* REC-169 (C-22.13/C-22.14 in AI_RUN_CHECKS, 2026-09-23, worktree agent-a30f15e61a2d24169): MOVED 351 -> 353 from this guard's own print on the item's tree: both codes are received by a committed suite (rec169-consume.test.mjs). The move is exactly this item's own additions, read off the per-site line; every other key gated on the same run. */ /* D-85 (C-66 SURFACE_CHECKS, 2026-09-23, worktree agent-a09e49cc337fc2200): MOVED 347 -> 351 from this guard's own print on the item's tree, every other ratchet key gated. The four rows, in reach by R1. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* REC-165 (2026-09-22, worktree agent-a6e92d28647553f52): MOVED 344 -> 346 from this guard's own print (the commit-at-HEAD figure). The two new C-27 rows, in reach by R1, both arriving TRANSLATED. Slack 0 on main (M0-79), so the +2 is this item's. */ /* D-436 (C-64 INSTANCE_GROUP_CHECKS, 2026-09-21, worktree agent-a6dd0a0a3a3a6cc10): MOVED 341 -> 344 from this guard's own GREEN print on the item's committed tree 2f09b7b6. The three rows, in reach by R1. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* D-158 (C-63 SIGNER_ENROLMENT_CHECKS, 2026-09-20, worktree d158-conduct8): MOVED 338 -> 341 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE origin/main sources (e1aa2eee) with this item's three plane files (store.mjs, setup.mjs, bio-checks.mjs) swapped out and restored by sha256 AND cmp: pristine printed 339, so 1 of the move is PRE-EXISTING SLACK and +2 is this item's (the two new codes, both arriving TRANSLATED). */    /* D-270 (C-38.7/C-38.8 + C-61 REQUIRED_ARGUMENT_CHECKS, 2026-09-19, worktree d270-record): MOVED 272 -> 338 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE origin/main sources (98209ce2) with this item's two plane files swapped out and restored by sha256 AND cmp: pristine printed 335, so 63 of the move is PRE-EXISTING SLACK and +3 is this item's (the three new codes, all three arriving TRANSLATED). */    /* REC-86 (IC-123, C-50 NARROW_CHECKS, 2026-09-18, worktree agent-a3fbd59a3fef1a961): MOVED 258 -> 272 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE HEAD sources (92f4c64e) with the item's four plane files swapped out and restored by sha256 + cmp: pristine printed 261, so 3 of the move is PRE-EXISTING slack and +11 is this item's. */ /* MOVED 227 -> 229, 2026-08-10, worktree agent-a36b6782b06f5a651 (CASE-3), from
  /* D-508 side, kept as history: */  /* D-508 (C-85 KNOCK_CHECKS, 2026-09-24, cloud session WORKER D-508 (CONDUCT #20), branch land/worker/D-508): MOVED 433 -> 435 from this guard's own --strict print on the item's tree over origin/main 68fecb8d0. Whose growth it absorbs: the two new C-85 rows; both arrive TRANSLATED, so reachGap does not move. ZERO of the move is pre-existing slack: the PRISTINE origin/main 68fecb8d0, run in this worktree before any edit, printed exit 0 with EVERY ratchet key at slack 0. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads regionLines at integration — and D-507 moves the same catalogue in parallel. */ /* CONDUCT #20 (c20-batch14, 2026-09-24): 432 -> 433, RE-READ from this guard's own `--strict` print on the union of this branch with `origin/main` @ 454a02bc (c20-batch17 and c20-batch18). Whose growth it absorbs: one more code a surface can receive, D-484's. Never either side's figure and never added by hand. */ /* D-484 (C-33.40 NO_BASIS, C-33.41 NO_CITATION consolidated behind one governed helper each, 2026-09-24, branch land/worker/D-484): MOVED 421 -> 422 from this guard's own --strict print on the item's tree over origin/main 16fe1e7f. ZERO pre-existing slack: the PRISTINE origin/main 16fe1e7f, run before any edit in this worktree, printed exit 0 with EVERY ratchet key at slack 0. R1 gains two rows; only NO_CITATION is a new code IN REACH, because NO_BASIS was already reached through R3 (conclude-act.test.mjs feeds it) and the union does not double-count it. +2 rows, +1 reach, and the difference is the measurement rather than an error. */ /* CONDUCT #20 (c20-batch14, 2026-09-24): 425 -> 432, RE-READ from this guard's own `--strict` print on the committed union of c20-batch11fix and c20-integ1b — never either branch's figure and never added by hand. Whose growth it absorbs: the SEVEN are D-64's C-83.1..7, each reachable through `op=acquire`'s render arm. Both branches had re-done the same c19-batch10 + `main` union, so each branch's own figure was short of the union's. */ /* CONDUCT #20 (c20-integ1, 2026-09-24): 422 -> 430, RE-READ from this guard's own --strict print on the committed union (D-64 renumbered C-82 -> C-83, REC-184, and origin/main @ 548eb2c5 merged over c19-batch10) — never added by hand, and never either side's figure. Whose growth it absorbs: +7 D-64 and +1 IC-246, the same two sources. */ /* D-64 (C-83 RENDER_CAPTURE_CHECKS, 2026-09-24, cloud session WORKER D-64 (CONDUCT #19), branch land/worker/D-64): MOVED 376 -> 383 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: the seven family rows (R1). ZERO of the move is pre-existing slack: every other key gated at slack 0 on the same run, and the move is exactly this item's own. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration. */ /* CONDUCT #19 (c19-batch11, 2026-09-24): REC-192 @ 2e102941 (C-71.8, C-71.9; IC-248; the branch read 378 on main) merged over the batch; RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* REC-192 (the version arm of op=partitionindependence, 2026-09-24, cloud session WORKER REC-192 (CONDUCT #19), branch land/worker/REC-192): MOVED 376 -> 378, the figure this guard PRINTED on the item's tree over origin/main 15b2a4c0: the two new rows (R1). ZERO of the move is pre-existing slack: the same guard over pristine origin/main printed exactly 376. Integrator: RE-READ from this guard's print on the committed merge. */ /* D-456 (C-78 NAMESPACE_CHECKS, 2026-09-23, cloud session WORKER D-456 (CONDUCT #18), branch land/worker/D-456): MOVED 375 -> 376 from this guard's own --strict print on the item's tree over origin/main a8f6094a: one new family, one row (NAMESPACE_UNKNOWN), one governed region (namespaceGate > is-namespace-gate, 5 lines), one code judged and compared. ZERO of the move is pre-existing slack: the same guard on a8f6094a before the change printed 375, every key gated at slack 0. */ /* CONDUCT #19 (c19-batch11, 2026-09-24): c19-unionfix @ 7b52ab9b (at 421) merged into c19-batch10 (at 422); RE-READ from this guard's --strict print on the merged tree, never added by hand. Both sides' comments carried below. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): REC-189 (C-32.19 MACHINE_CANNOT_SET_RISK_TIER, minted C-32.18) merged over the batch; 421 -> 422 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): D-461 (C-78.2 NAMESPACE_PINNED) merged over the batch; 420 -> 421 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* D-461 (C-78.2 NAMESPACE_PINNED, 2026-09-24, cloud session WORKER D-461 (CONDUCT #19)): MOVED 376 -> 377 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: one new row in the existing NAMESPACE_CHECKS family and one new governed REGION (pinnedNamespaceGate > is-pinned-namespace-gate). ZERO of the move is pre-existing slack: the same guard on 15b2a4c0 gated every key at slack 0. */ /* c19-unionfix (c19-batch9, 2026-09-24): 420 -> 421, re-read from this guard's --strict print on the union with IC-246: its ONE new family STATEMENT_ACK_CHECKS (C-82), ONE row and code STATEMENT_ACK_DOCUMENTS_OVER_BOUND, ONE region `is-statement-ack-documents-bound` in acknowledgeStatement. */ /* CONDUCT #18 (c18-batch8, 2026-09-23): D-394 merged over the batch; 406 -> 409 RE-READ from this guard's --strict print on the committed merge 8477cdb5. The new members are D-394's; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-126 (C-75 and C-76, the per-item weight) merged over the batch; 400 -> 406 RE-READ from this guard's --strict print on the committed merge 6ab139df. The new members are D-126 (C-75 and C-76, the per-item weight)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-149 (C-70, discoverable or hidden) merged over the batch; 396 -> 400 RE-READ from this guard's --strict print on the committed merge f4be4c07. The new members are REC-149 (C-70, discoverable or hidden)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-122 (C-74, the on-point choice) merged over the batch; 393 -> 396 RE-READ from this guard's --strict print on the committed merge e1d34ee0. The new members are REC-122 (C-74, the on-point choice)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-149 (C-73 and C-32.18, the governing laws) merged over the batch; 387 -> 393 RE-READ from this guard's --strict print on the committed merge 8086d64f. The new members are D-149 (C-73 and C-32.18, the governing laws)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-148 (C-72, the fee quote) merged over the batch; 379 -> 387 RE-READ from this guard's --strict print on the committed merge c81aec68. The new members are D-148 (C-72, the fee quote)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-164 merged over c17-batch6; 375 -> 379 RE-READ from this guard's --strict print on the committed merge 84443ad4. The new members are REC-164's (C-64.4..C-64.7, the group's name and domain acts); neither side's figure is the sum. */ /* CONDUCT #17 (c17-batch5, 2026-09-23): REC-161 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-161 (C-71 PARTITION_INDEPENDENCE_CHECKS, 2026-09-23, cloud session WORKER REC-161 (CONDUCT #17)): MOVED 362 -> 369 from this guard's own print on the item's tree over origin/main 02603e88: the seven C-71 rows. ZERO of the move is pre-existing slack: the same guard over a pristine 02603e88 worktree printed 362, every key gated at slack 0. */ /* CONDUCT #17 (c17-batch4, 2026-09-23): D-179 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-179 (C-53.13 CAPTURE_HELD_BY_ANOTHER_BUNDLE, 2026-09-23, branch land/worker/D-179): MOVED 358 -> 359 from this guard's own print on the item's tree: R1: the one new family row. ZERO pre-existing slack — every other key gated at 0 on the same run, and the move is exactly this item's own. */ /* CONDUCT #17 (c17-batch3, 2026-09-23): D-278 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-278 (C-68 + C-69, 2026-09-23, branch land/worker/D-278): MOVED 358 -> 363 from this guard's own print on the item's tree, every other ratchet key gated. the five new codes, reached as R1 family rows. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): origin/main (c16-batch7) merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-168 (C-33.39 RETIRED_NOT_CITABLE in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-ab0b1ebe531cb428b): MOVED 355 -> 356 from this guard's own print on the item's tree: the new row reaches by R1 (a family row). ZERO pre-existing slack — the pristine HEAD (0e7cc03e) sources, swapped in and restored by sha256 AND cmp, printed 355, every key gated.  — CONDUCT #16 merged it onto REC-175/REC-176, which moved this key from the same base; the figure is re-read from the merged print. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): REC-177 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-177 (C-22.16 AI_RUN_BOUND_NO_ALLOWANCE in AI_RUN_CHECKS, 2026-09-23, worktree agent-ad7020ce0605889f4), RE-READ ON THE MERGE onto origin/main 0e5f7054 (which carries REC-175's and D-168's moves of this key from the same base): MOVED 356 -> 357 from this guard's own print on the merged tree: the new row's R1 reach. ZERO of the move is pre-existing slack: every other key gated at slack 0 on the same run. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): REC-179 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-179 (C-66.5 SURFACED_BY_REWRITTEN in SURFACE_CHECKS, one new region promote > is-promote-surfaced-by, 2026-09-23, worktree agent-a368de25bd347b32f): MOVED 356 -> 357 from this guard's own print on the item's tree MERGED with origin/main 0e5f7054 (REC-175 landed first and moved this key from the same base, so this figure is re-read, never added), every other ratchet key gated. ZERO of the move is pre-existing slack: the merged tree without this item printed 356. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): D-440 and D-420 each moved this key by their own row from 0e5f7054; the union's value is RE-READ from this guard's print on the committed merge. */ /* D-420 (C-45.12 CONTENT_EXTENT_NO_IMAGE_PAINTED in CONTENT_EXTENT_CHECKS, 2026-09-23, cloud session WORKER D-420 (CONDUCT #16)): MOVED 356 -> 357 from this guard's own print on the item's tree over origin/main 0e5f7054, every other ratchet key gated at slack 0 on the same run. Its one R1 family row. ZERO of the move is pre-existing slack: the baseline run on 0e5f7054 printed exactly the old floor, so the move is this item's own. D-440 moves this key from the same base; CONDUCT re-reads the figure from the merged print. */ /* D-440 (C-45.11, 2026-09-23, worktree agent-aa6d775965f554582), RE-READ ON ITS MERGE with origin/main 0e5f7054: MOVED 356 -> 357 from this guard's own print on the merged tree — the new row's R1 reach. ZERO pre-existing slack. */ /* REC-175 (C-33.38 FILE_DIGEST_MISMATCH in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-a6f377d2e96aa24af): MOVED 354 -> 355 from this guard's own print on the item's tree over origin/main 14faa089, every other ratchet key gated at slack 0 on the same run. Its one R1 family row. ZERO of the move is pre-existing slack: the baseline run on 14faa089 printed exactly the old floor, so the move is this item's own.  — CONDUCT #16 merged it with REC-172, which moved this key from the same base; the figure is re-read from the merged print. */ /* REC-172 (2026-09-23, worktree agent-ad79857cb007927b7): MOVED 353 -> 354 from this guard's own print: the new row's R1 reach. The pristine HEAD (91913d6b) printed 353: ZERO pre-existing slack.  — CONDUCT #16 merged it with UI-81, which moved this key from the same base; the figure is re-read from the merged print. */ /* UI-81 (C-44.2 FINDING_IN_SEVERAL_CASES in CASE_DERIVATION_CHECKS, 2026-09-23, worktree agent-a306ff29faf082ef3), RE-READ ON THE MERGE onto origin/main 91913d6b (which carries REC-169's moves): MOVED 353 -> 354 from this guard's own print on the merged tree, every other ratchet key gated. The new row, in reach by R1 and arriving TRANSLATED; app.html keys on the refusal's `cases[]` and names no code, so R2 did not move. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* REC-169 (C-22.13/C-22.14 in AI_RUN_CHECKS, 2026-09-23, worktree agent-a30f15e61a2d24169): MOVED 351 -> 353 from this guard's own print on the item's tree: both codes are received by a committed suite (rec169-consume.test.mjs). The move is exactly this item's own additions, read off the per-site line; every other key gated on the same run. */ /* D-85 (C-66 SURFACE_CHECKS, 2026-09-23, worktree agent-a09e49cc337fc2200): MOVED 347 -> 351 from this guard's own print on the item's tree, every other ratchet key gated. The four rows, in reach by R1. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* REC-165 (2026-09-22, worktree agent-a6e92d28647553f52): MOVED 344 -> 346 from this guard's own print (the commit-at-HEAD figure). The two new C-27 rows, in reach by R1, both arriving TRANSLATED. Slack 0 on main (M0-79), so the +2 is this item's. */ /* D-436 (C-64 INSTANCE_GROUP_CHECKS, 2026-09-21, worktree agent-a6dd0a0a3a3a6cc10): MOVED 341 -> 344 from this guard's own GREEN print on the item's committed tree 2f09b7b6. The three rows, in reach by R1. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* D-158 (C-63 SIGNER_ENROLMENT_CHECKS, 2026-09-20, worktree d158-conduct8): MOVED 338 -> 341 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE origin/main sources (e1aa2eee) with this item's three plane files (store.mjs, setup.mjs, bio-checks.mjs) swapped out and restored by sha256 AND cmp: pristine printed 339, so 1 of the move is PRE-EXISTING SLACK and +2 is this item's (the two new codes, both arriving TRANSLATED). */    /* D-270 (C-38.7/C-38.8 + C-61 REQUIRED_ARGUMENT_CHECKS, 2026-09-19, worktree d270-record): MOVED 272 -> 338 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE origin/main sources (98209ce2) with this item's two plane files swapped out and restored by sha256 AND cmp: pristine printed 335, so 63 of the move is PRE-EXISTING SLACK and +3 is this item's (the three new codes, all three arriving TRANSLATED). */    /* REC-86 (IC-123, C-50 NARROW_CHECKS, 2026-09-18, worktree agent-a3fbd59a3fef1a961): MOVED 258 -> 272 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE HEAD sources (92f4c64e) with the item's four plane files swapped out and restored by sha256 + cmp: pristine printed 261, so 3 of the move is PRE-EXISTING slack and +11 is this item's. */ /* MOVED 227 -> 229, 2026-08-10, worktree agent-a36b6782b06f5a651 (CASE-3), from
  ...
  /* CONDUCT #19 (c19-batch9, 2026-09-24): D-162 (the THEME family, renumbered C-74 -> C-81) merged over the batch; RE-READ from this guard's --strict print on the merged tree. */
  /* D-510 side, kept as history: governedSites 175 on its own branch; the c20-batch25 union's figure is re-read from the print. */
  /* D-472 side, kept as history: governedSites 176 on its own branch; the c20-batch25 union's figure is re-read from the print. */
  /* REC-211 side, kept as history: governedSites 174 -> 176 on its own branch (IC-273); the c20-batch23 union's figure is re-read from --strict's print. */
  /* REC-186 side, kept as history (c22-batch29: ours' governedSites: 215 kept; NUMBER TO RE-READ from --strict's print on the union): governedSites: 187, — REC-186 (2026-09-25, branch land/worker/REC-186): 186 -> 187, READ from this guard's print on the item's tree over origin/main 8bdf20e6. Whose growth: C-33.48 LAST_OWNER_CANNOT_LEAVE and its region is-leave-owner-floor in projectLeave. */
  /* D-513 side, kept as history: governedSites 186 on its own branch; the c20-batch27 union's figure is re-read from --strict's print. */
  /* D-463 side, kept as history: governedSites 185 on its own branch; the c20-batch27 union's figure is re-read from the print. */
  /* D-147 side, kept as history (c22-batch29: ours' governedSites: 215 kept; NUMBER TO RE-READ from --strict's print on the union): governedSites: 194, — D-147 (C-94 LIFECYCLE_CHECKS, 2026-09-25, branch land/worker/D-147): 191 -> 194, READ from this guard's own print on the item's tree over origin/main 964da679 — the new family, its eleven rows and codes, and its three regions (is-lifecycle-writable, is-lifecycle-grammar, is-lifecycle-due-cite). Every other key gated at slack 0 on the same run. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads regionLines at integration. */
  /* REC-203 side, kept as history (c22-batch29: ours' governedSites: 215 kept; NUMBER TO RE-READ from --strict's print on the union): governedSites: 194, — REC-203 (C-91 IDSPACE_CHECKS, 2026-09-25, branch land/worker/REC-203): MOVED 191 -> 194 from this guard's own --strict print on the item's tree over origin/main 964da679. Whose growth it absorbs: op=idmatch's ONE new family and its three refusals, each at its own DEC-49 REGION in store.mjs idMatch. regionLines is a property of the MERGED source, so CONDUCT re-reads it at integration. */
  /* D-547 side, kept as history: governedSites 187 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-217 side, kept as history: governedSites 189 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-448 side, kept as history: governedSites 197 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-468 side, kept as history: governedSites 187 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-214 side, kept as history: governedSites 188 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-549 side, kept as history: governedSites 187 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-512 side, kept as history: governedSites 187 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-205 side, kept as history: governedSites 184 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* c21-rec207-renumber side, kept as history: governedSites 185 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-530 side, kept as history: governedSites 187 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-150 side, kept as history: governedSites 196 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-134 side, kept as history: governedSites 199 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-589 side, kept as history: governedSites 193 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* MK-7 side, kept as history: governedSites 196 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* REC-147 side, kept as history: governedSites 198 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* REC-197 side, kept as history: governedSites 194 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-521b side, kept as history: governedSites 214 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-563 side, kept as history: governedSites 217 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-561 side, kept as history: governedSites 224 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-578 side, kept as history: governedSites 218 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-546 side, kept as history: governedSites 219 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-615 side, kept as history: governedSites 220 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-623 side, kept as history: governedSites 216 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-629 side, kept as history: governedSites 217 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-680 side, kept as history: governedSites 216 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* REC-215 side, kept as history: governedSites 217 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-419 side, kept as history: governedSites 217 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-689 side, kept as history: governedSites 216 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  /* D-695 side, kept as history: governedSites 217 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  /* D-688 side, kept as history: governedSites 264 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  /* D-628 side, kept as history: governedSites 221 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  governedSites: 278, /* CONDUCT #23 at the c23-batch30 union: 263 -> 278, READ from `node civicos-ui/check-refusal-codes.mjs --strict`'s print on the merged tree (HEAD 645f8c41); never a branch's figure, never added by hand. */ /* CONDUCT #22 at the c22-batch29 union: 215 -> 263, the union's figure READ from `node civicos-ui/check-refusal-codes.mjs --strict`'s print on the merged tree (`ratchet: governedSites ... measured 263`). */ /* CONDUCT #21 at c21-batch28: 191 -> 215, the union's figure READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch27: 186 -> 191, the union's figure (D-463, D-513 and the batch) READ from --strict's print on the committed merge. */ /* CONDUCT #20 at c20-batch25: 185 -> 186, D-510's C-86.1 (the PROMOTED_TYPE_CHECKS family and its region) over the union, READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch25: 183 -> 185, D-472's C-48.8/C-48.9 and their two regions over the union, READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch23: 181 -> 183, REC-211's arrivals over the c20-batch22 union, READ from this guard's --strict print on the merged tree. */ /* CONDUCT #20 at c20-batch22: the UNION of D-507 (174 -> 180) and D-508 (174 -> 175), READ from this guard's --strict print on the merged tree. */ /* D-507 (C-82.2..C-82.7, the six STATEMENT_ACK_* refusals that reached a member untranslated, 2026-09-24, branch land/worker/D-507): MOVED 174 -> 180 from this guard's own --strict print on the item's tree over origin/main 68fecb8d. Six new `where`s, every one a narrowed REGION rather than a whole function. ZERO of the move is pre-existing slack: the PRISTINE origin/main 68fecb8d, run in this worktree before any edit, printed exit 0 with EVERY ratchet key at slack 0. */  /* CONDUCT #20 (c20-batch14, 2026-09-24): 172 -> 174, RE-READ from this guard's own `--strict` print on the union of this branch with `origin/main` @ 454a02bc (c20-batch17 and c20-batch18). Whose growth it absorbs: D-484's two `where` spans. Never either side's figure and never added by hand. */ /* D-484 (C-33.40 NO_BASIS, C-33.41 NO_CITATION consolidated behind one governed helper each, 2026-09-24, branch land/worker/D-484): MOVED 168 -> 170 from this guard's own --strict print on the item's tree over origin/main 16fe1e7f. ZERO pre-existing slack: the PRISTINE origin/main 16fe1e7f, run before any edit in this worktree, printed exit 0 with EVERY ratchet key at slack 0. the two new `where`s. */ /* CONDUCT #20 (c20-batch14, 2026-09-24): 170 -> 172, RE-READ from this guard's own `--strict` print on the committed union of c20-batch11fix and c20-integ1b — never either branch's figure and never added by hand. Whose growth it absorbs: the TWO are D-64's `where` spans, `fetch > is-render-admit` and `fetch > is-render-result`. Both branches had re-done the same c19-batch10 + `main` union, so each branch's own figure was short of the union's. */ /* CONDUCT #20 (c20-integ1, 2026-09-24): 169 -> 172, RE-READ from this guard's own --strict print on the committed union (D-64 renumbered C-82 -> C-83, REC-184, and origin/main @ 548eb2c5 merged over c19-batch10) — never added by hand, and never either side's figure. Whose growth it absorbs: +2 D-64's two new `where` spans and +1 IC-246's `is-statement-ack-documents-bound`. */ /* D-64 (C-83 RENDER_CAPTURE_CHECKS, 2026-09-24, cloud session WORKER D-64 (CONDUCT #19), branch land/worker/D-64): MOVED 141 -> 143 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: two governed regions, fetch > is-render-admit and fetch > is-render-result. ZERO of the move is pre-existing slack: every other key gated at slack 0 on the same run, and the move is exactly this item's own. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration. */ /* CONDUCT #19 (c19-batch11, 2026-09-24): c19-unionfix @ 7b52ab9b (at 168) merged into c19-batch10 (at 169); RE-READ from this guard's --strict print on the merged tree, never added by hand. Both sides' comments carried below. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): REC-189 (C-32.19 MACHINE_CANNOT_SET_RISK_TIER, minted C-32.18) merged over the batch; 168 -> 169 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): D-461 (C-78.2 NAMESPACE_PINNED) merged over the batch; 167 -> 168 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* D-461 (C-78.2 NAMESPACE_PINNED, 2026-09-24, cloud session WORKER D-461 (CONDUCT #19)): MOVED 141 -> 142 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: one new row in the existing NAMESPACE_CHECKS family and one new governed REGION (pinnedNamespaceGate > is-pinned-namespace-gate). ZERO of the move is pre-existing slack: the same guard on 15b2a4c0 gated every key at slack 0. */ /* c19-unionfix (c19-batch9, 2026-09-24): 167 -> 168, re-read from this guard's --strict print on the union with IC-246: its ONE new family STATEMENT_ACK_CHECKS (C-82), ONE row and code STATEMENT_ACK_DOCUMENTS_OVER_BOUND, ONE region `is-statement-ack-documents-bound` in acknowledgeStatement. */ /* CONDUCT #18 (c18-batch8, 2026-09-23): D-394 merged over the batch; 159 -> 160 RE-READ from this guard's --strict print on the committed merge 8477cdb5. The new members are D-394's; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-126 (C-75 and C-76, the per-item weight) merged over the batch; 154 -> 159 RE-READ from this guard's --strict print on the committed merge 6ab139df. The new members are D-126 (C-75 and C-76, the per-item weight)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-149 (C-70, discoverable or hidden) merged over the batch; 151 -> 154 RE-READ from this guard's --strict print on the committed merge f4be4c07. The new members are REC-149 (C-70, discoverable or hidden)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-122 (C-74, the on-point choice) merged over the batch; 150 -> 151 RE-READ from this guard's --strict print on the committed merge e1d34ee0. The new members are REC-122 (C-74, the on-point choice)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-149 (C-73 and C-32.18, the governing laws) merged over the batch; 147 -> 150 RE-READ from this guard's --strict print on the committed merge 8086d64f. The new members are D-149 (C-73 and C-32.18, the governing laws)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-148 (C-72, the fee quote) merged over the batch; 144 -> 147 RE-READ from this guard's --strict print on the committed merge c81aec68. The new members are D-148 (C-72, the fee quote)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-164 merged over c17-batch6; 140 -> 144 RE-READ from this guard's --strict print on the committed merge 84443ad4. The new members are REC-164's (C-64.4..C-64.7, the group's name and domain acts); neither side's figure is the sum. */ /* CONDUCT #17 (c17-batch5, 2026-09-23): REC-161 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-161 (C-71 PARTITION_INDEPENDENCE_CHECKS, 2026-09-23, cloud session WORKER REC-161 (CONDUCT #17)): MOVED 135 -> 136 from this guard's own print on the item's tree over origin/main 02603e88: the one new region partitionIndependence > is-partition-independence. ZERO of the move is pre-existing slack: the same guard over a pristine 02603e88 worktree printed 135, every key gated at slack 0. */ /* CONDUCT #17 (c17-batch4, 2026-09-23): D-179 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-179 (C-53.13 CAPTURE_HELD_BY_ANOTHER_BUNDLE, 2026-09-23, branch land/worker/D-179): MOVED 134 -> 135 from this guard's own print on the item's tree: the new row's `where`, #testimonyFence > is-register-home. ZERO pre-existing slack — every other key gated at 0 on the same run, and the move is exactly this item's own. */ /* CONDUCT #17 (c17-batch3, 2026-09-23): D-278 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-278 (C-68 + C-69, 2026-09-23, branch land/worker/D-278): MOVED 134 -> 137 from this guard's own print on the item's tree, every other ratchet key gated. three new spans: storageAbsent > is-storage-absent, fetch > is-bootstrap-claim, fetch > is-unknown-op. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): origin/main (c16-batch7) merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-168 (C-33.39 RETIRED_NOT_CITABLE in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-ab0b1ebe531cb428b): MOVED 131 -> 132 from this guard's own print on the item's tree: one new governed site, the region is-cite-retired. ZERO pre-existing slack — the pristine HEAD (0e7cc03e) sources, swapped in and restored by sha256 AND cmp, printed 131, every key gated.  — CONDUCT #16 merged it onto REC-175/REC-176, which moved this key from the same base; the figure is re-read from the merged print. */ /* REC-179 (C-66.5 SURFACED_BY_REWRITTEN in SURFACE_CHECKS, one new region promote > is-promote-surfaced-by, 2026-09-23, worktree agent-a368de25bd347b32f): MOVED 132 -> 133 from this guard's own print on the item's tree MERGED with origin/main 0e5f7054 (REC-175 landed first and moved this key from the same base, so this figure is re-read, never added), every other ratchet key gated. ZERO of the move is pre-existing slack: the merged tree without this item printed 132. */ /* REC-175 (C-33.38 FILE_DIGEST_MISMATCH in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-a6f377d2e96aa24af): MOVED 131 -> 132 from this guard's own print on the item's tree over origin/main 14faa089, every other ratchet key gated at slack 0 on the same run. One new `where`, promote > is-promote-digest. ZERO of the move is pre-existing slack: the baseline run on 14faa089 printed exactly the old floor, so the move is this item's own. */ /* UI-81 (C-44.2 FINDING_IN_SEVERAL_CASES in CASE_DERIVATION_CHECKS, 2026-09-23, worktree agent-a306ff29faf082ef3), RE-READ ON THE MERGE onto origin/main 91913d6b (which carries REC-169's moves): MOVED 130 -> 131 from this guard's own print on the merged tree, every other ratchet key gated. One new span: #resolveOneCase > is-finding-in-several-cases. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* REC-169 (C-22.13/C-22.14 in AI_RUN_CHECKS, 2026-09-23, worktree agent-a30f15e61a2d24169): MOVED 129 -> 130 from this guard's own print on the item's tree: the one `where` both rows name, airun.mjs checkConsume. The move is exactly this item's own additions, read off the per-site line; every other key gated on the same run. */ /* D-85 (C-66 SURFACE_CHECKS, 2026-09-23, worktree agent-a09e49cc337fc2200): MOVED 128 -> 129 from this guard's own print on the item's tree, every other ratchet key gated. One new span: #surfacingGate > is-surface-run. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* REC-167 (C-65 CASE_CONCLUSION_CHECKS, 2026-09-22, worktree agent-a33d4bac9a4ff9d3a): MOVED 127 -> 128 from this guard's own print on the item's tree, every other ratchet key gated. One new span: ratifyCaseDocument > is-caseratify-conclusion-moved. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* D-436 (C-64 INSTANCE_GROUP_CHECKS, 2026-09-21, worktree agent-a6dd0a0a3a3a6cc10): MOVED 125 -> 127 from this guard's own GREEN print on the item's committed tree 2f09b7b6. Two new spans: #groupUndetermined > is-group-undetermined and instanceGroupSeed > is-instance-group-seed. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* D-158 (C-63 SIGNER_ENROLMENT_CHECKS, 2026-09-20, worktree d158-conduct8): MOVED 123 -> 125 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE origin/main sources (e1aa2eee) with this item's three plane files (store.mjs, setup.mjs, bio-checks.mjs) swapped out and restored by sha256 AND cmp: pristine printed 124, so 1 of the move is PRE-EXISTING SLACK and +1 is this item's (one new span, #signerMemberBar > is-signer-member-attesting). */    /* D-270 (C-38.7/C-38.8 + C-61 REQUIRED_ARGUMENT_CHECKS, 2026-09-19, worktree d270-record): MOVED 84 -> 123 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE origin/main sources (98209ce2) with this item's two plane files swapped out and restored by sha256 AND cmp: pristine printed 121, so 37 of the move is PRE-EXISTING SLACK and +2 is this item's (sessionOpGate > is-session-op-gate and requiredArgument > is-required-argument). */   /* REC-86 (IC-123, C-50 NARROW_CHECKS, 2026-09-18, worktree agent-a3fbd59a3fef1a961): MOVED 81 -> 84 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE HEAD sources (92f4c64e) with the item's four plane files swapped out and restored by sha256 + cmp: pristine printed 81, so 0 of the move is PRE-EXISTING slack and +3 (#narrowSource > is-narrow-source, narrow > is-narrow-extent, narrow > is-narrow-claim) is this item's. */ // spans named by a row's `where` — a function, or a region inside one.
  /* D-508 side, kept as history: */  /* D-508 (C-85 KNOCK_CHECKS, 2026-09-24, cloud session WORKER D-508 (CONDUCT #20), branch land/worker/D-508): MOVED 174 -> 175 from this guard's own --strict print on the item's tree over origin/main 68fecb8d0. Whose growth it absorbs: the ONE new `where` span, knock > is-knock-rate. ZERO of the move is pre-existing slack: the PRISTINE origin/main 68fecb8d0, run in this worktree before any edit, printed exit 0 with EVERY ratchet key at slack 0. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads regionLines at integration — and D-507 moves the same catalogue in parallel. */ /* CONDUCT #20 (c20-batch14, 2026-09-24): 172 -> 174, RE-READ from this guard's own `--strict` print on the union of this branch with `origin/main` @ 454a02bc (c20-batch17 and c20-batch18). Whose growth it absorbs: D-484's two `where` spans. Never either side's figure and never added by hand. */ /* D-484 (C-33.40 NO_BASIS, C-33.41 NO_CITATION consolidated behind one governed helper each, 2026-09-24, branch land/worker/D-484): MOVED 168 -> 170 from this guard's own --strict print on the item's tree over origin/main 16fe1e7f. ZERO pre-existing slack: the PRISTINE origin/main 16fe1e7f, run before any edit in this worktree, printed exit 0 with EVERY ratchet key at slack 0. the two new `where`s. */ /* CONDUCT #20 (c20-batch14, 2026-09-24): 170 -> 172, RE-READ from this guard's own `--strict` print on the committed union of c20-batch11fix and c20-integ1b — never either branch's figure and never added by hand. Whose growth it absorbs: the TWO are D-64's `where` spans, `fetch > is-render-admit` and `fetch > is-render-result`. Both branches had re-done the same c19-batch10 + `main` union, so each branch's own figure was short of the union's. */ /* CONDUCT #20 (c20-integ1, 2026-09-24): 169 -> 172, RE-READ from this guard's own --strict print on the committed union (D-64 renumbered C-82 -> C-83, REC-184, and origin/main @ 548eb2c5 merged over c19-batch10) — never added by hand, and never either side's figure. Whose growth it absorbs: +2 D-64's two new `where` spans and +1 IC-246's `is-statement-ack-documents-bound`. */ /* D-64 (C-83 RENDER_CAPTURE_CHECKS, 2026-09-24, cloud session WORKER D-64 (CONDUCT #19), branch land/worker/D-64): MOVED 141 -> 143 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: two governed regions, fetch > is-render-admit and fetch > is-render-result. ZERO of the move is pre-existing slack: every other key gated at slack 0 on the same run, and the move is exactly this item's own. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration. */ /* CONDUCT #19 (c19-batch11, 2026-09-24): c19-unionfix @ 7b52ab9b (at 168) merged into c19-batch10 (at 169); RE-READ from this guard's --strict print on the merged tree, never added by hand. Both sides' comments carried below. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): REC-189 (C-32.19 MACHINE_CANNOT_SET_RISK_TIER, minted C-32.18) merged over the batch; 168 -> 169 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): D-461 (C-78.2 NAMESPACE_PINNED) merged over the batch; 167 -> 168 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* D-461 (C-78.2 NAMESPACE_PINNED, 2026-09-24, cloud session WORKER D-461 (CONDUCT #19)): MOVED 141 -> 142 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: one new row in the existing NAMESPACE_CHECKS family and one new governed REGION (pinnedNamespaceGate > is-pinned-namespace-gate). ZERO of the move is pre-existing slack: the same guard on 15b2a4c0 gated every key at slack 0. */ /* c19-unionfix (c19-batch9, 2026-09-24): 167 -> 168, re-read from this guard's --strict print on the union with IC-246: its ONE new family STATEMENT_ACK_CHECKS (C-82), ONE row and code STATEMENT_ACK_DOCUMENTS_OVER_BOUND, ONE region `is-statement-ack-documents-bound` in acknowledgeStatement. */ /* CONDUCT #18 (c18-batch8, 2026-09-23): D-394 merged over the batch; 159 -> 160 RE-READ from this guard's --strict print on the committed merge 8477cdb5. The new members are D-394's; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-126 (C-75 and C-76, the per-item weight) merged over the batch; 154 -> 159 RE-READ from this guard's --strict print on the committed merge 6ab139df. The new members are D-126 (C-75 and C-76, the per-item weight)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-149 (C-70, discoverable or hidden) merged over the batch; 151 -> 154 RE-READ from this guard's --strict print on the committed merge f4be4c07. The new members are REC-149 (C-70, discoverable or hidden)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-122 (C-74, the on-point choice) merged over the batch; 150 -> 151 RE-READ from this guard's --strict print on the committed merge e1d34ee0. The new members are REC-122 (C-74, the on-point choice)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-149 (C-73 and C-32.18, the governing laws) merged over the batch; 147 -> 150 RE-READ from this guard's --strict print on the committed merge 8086d64f. The new members are D-149 (C-73 and C-32.18, the governing laws)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-148 (C-72, the fee quote) merged over the batch; 144 -> 147 RE-READ from this guard's --strict print on the committed merge c81aec68. The new members are D-148 (C-72, the fee quote)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-164 merged over c17-batch6; 140 -> 144 RE-READ from this guard's --strict print on the committed merge 84443ad4. The new members are REC-164's (C-64.4..C-64.7, the group's name and domain acts); neither side's figure is the sum. */ /* CONDUCT #17 (c17-batch5, 2026-09-23): REC-161 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-161 (C-71 PARTITION_INDEPENDENCE_CHECKS, 2026-09-23, cloud session WORKER REC-161 (CONDUCT #17)): MOVED 135 -> 136 from this guard's own print on the item's tree over origin/main 02603e88: the one new region partitionIndependence > is-partition-independence. ZERO of the move is pre-existing slack: the same guard over a pristine 02603e88 worktree printed 135, every key gated at slack 0. */ /* CONDUCT #17 (c17-batch4, 2026-09-23): D-179 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-179 (C-53.13 CAPTURE_HELD_BY_ANOTHER_BUNDLE, 2026-09-23, branch land/worker/D-179): MOVED 134 -> 135 from this guard's own print on the item's tree: the new row's `where`, #testimonyFence > is-register-home. ZERO pre-existing slack — every other key gated at 0 on the same run, and the move is exactly this item's own. */ /* CONDUCT #17 (c17-batch3, 2026-09-23): D-278 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-278 (C-68 + C-69, 2026-09-23, branch land/worker/D-278): MOVED 134 -> 137 from this guard's own print on the item's tree, every other ratchet key gated. three new spans: storageAbsent > is-storage-absent, fetch > is-bootstrap-claim, fetch > is-unknown-op. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): origin/main (c16-batch7) merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-168 (C-33.39 RETIRED_NOT_CITABLE in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-ab0b1ebe531cb428b): MOVED 131 -> 132 from this guard's own print on the item's tree: one new governed site, the region is-cite-retired. ZERO pre-existing slack — the pristine HEAD (0e7cc03e) sources, swapped in and restored by sha256 AND cmp, printed 131, every key gated.  — CONDUCT #16 merged it onto REC-175/REC-176, which moved this key from the same base; the figure is re-read from the merged print. */ /* REC-179 (C-66.5 SURFACED_BY_REWRITTEN in SURFACE_CHECKS, one new region promote > is-promote-surfaced-by, 2026-09-23, worktree agent-a368de25bd347b32f): MOVED 132 -> 133 from this guard's own print on the item's tree MERGED with origin/main 0e5f7054 (REC-175 landed first and moved this key from the same base, so this figure is re-read, never added), every other ratchet key gated. ZERO of the move is pre-existing slack: the merged tree without this item printed 132. */ /* REC-175 (C-33.38 FILE_DIGEST_MISMATCH in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-a6f377d2e96aa24af): MOVED 131 -> 132 from this guard's own print on the item's tree over origin/main 14faa089, every other ratchet key gated at slack 0 on the same run. One new `where`, promote > is-promote-digest. ZERO of the move is pre-existing slack: the baseline run on 14faa089 printed exactly the old floor, so the move is this item's own. */ /* UI-81 (C-44.2 FINDING_IN_SEVERAL_CASES in CASE_DERIVATION_CHECKS, 2026-09-23, worktree agent-a306ff29faf082ef3), RE-READ ON THE MERGE onto origin/main 91913d6b (which carries REC-169's moves): MOVED 130 -> 131 from this guard's own print on the merged tree, every other ratchet key gated. One new span: #resolveOneCase > is-finding-in-several-cases. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* REC-169 (C-22.13/C-22.14 in AI_RUN_CHECKS, 2026-09-23, worktree agent-a30f15e61a2d24169): MOVED 129 -> 130 from this guard's own print on the item's tree: the one `where` both rows name, airun.mjs checkConsume. The move is exactly this item's own additions, read off the per-site line; every other key gated on the same run. */ /* D-85 (C-66 SURFACE_CHECKS, 2026-09-23, worktree agent-a09e49cc337fc2200): MOVED 128 -> 129 from this guard's own print on the item's tree, every other ratchet key gated. One new span: #surfacingGate > is-surface-run. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* REC-167 (C-65 CASE_CONCLUSION_CHECKS, 2026-09-22, worktree agent-a33d4bac9a4ff9d3a): MOVED 127 -> 128 from this guard's own print on the item's tree, every other ratchet key gated. One new span: ratifyCaseDocument > is-caseratify-conclusion-moved. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* D-436 (C-64 INSTANCE_GROUP_CHECKS, 2026-09-21, worktree agent-a6dd0a0a3a3a6cc10): MOVED 125 -> 127 from this guard's own GREEN print on the item's committed tree 2f09b7b6. Two new spans: #groupUndetermined > is-group-undetermined and instanceGroupSeed > is-instance-group-seed. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* D-158 (C-63 SIGNER_ENROLMENT_CHECKS, 2026-09-20, worktree d158-conduct8): MOVED 123 -> 125 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE origin/main sources (e1aa2eee) with this item's three plane files (store.mjs, setup.mjs, bio-checks.mjs) swapped out and restored by sha256 AND cmp: pristine printed 124, so 1 of the move is PRE-EXISTING SLACK and +1 is this item's (one new span, #signerMemberBar > is-signer-member-attesting). */    /* D-270 (C-38.7/C-38.8 + C-61 REQUIRED_ARGUMENT_CHECKS, 2026-09-19, worktree d270-record): MOVED 84 -> 123 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE origin/main sources (98209ce2) with this item's two plane files swapped out and restored by sha256 AND cmp: pristine printed 121, so 37 of the move is PRE-EXISTING SLACK and +2 is this item's (sessionOpGate > is-session-op-gate and requiredArgument > is-required-argument). */   /* REC-86 (IC-123, C-50 NARROW_CHECKS, 2026-09-18, worktree agent-a3fbd59a3fef1a961): MOVED 81 -> 84 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE HEAD sources (92f4c64e) with the item's four plane files swapped out and restored by sha256 + cmp: pristine printed 81, so 0 of the move is PRE-EXISTING slack and +3 (#narrowSource > is-narrow-source, narrow > is-narrow-extent, narrow > is-narrow-claim) is this item's. */ // spans named by a row's `where` — a function, or a region inside one.
  ...
  /* CONDUCT #19 (c19-batch9, 2026-09-24): D-162 (the THEME family, renumbered C-74 -> C-81) merged over the batch; RE-READ from this guard's --strict print on the merged tree. */
  /* D-510 side, kept as history: regions 158 on its own branch; the c20-batch25 union's figure is re-read from the print. */
  /* D-472 side, kept as history: regions 159 on its own branch; the c20-batch25 union's figure is re-read from the print. */
  /* REC-211 side, kept as history: regions 157 -> 159 on its own branch (IC-273); the c20-batch23 union's figure is re-read from --strict's print. */
  /* REC-186 side, kept as history (c22-batch29: ours' regions: 198 kept; NUMBER TO RE-READ from --strict's print on the union): regions: 170, — REC-186 (2026-09-25, branch land/worker/REC-186): 169 -> 170, READ from this guard's print on the item's tree over origin/main 8bdf20e6. Whose growth: C-33.48 LAST_OWNER_CANNOT_LEAVE and its region is-leave-owner-floor in projectLeave. */
  /* D-513 side, kept as history: regions 169 on its own branch; the c20-batch27 union's figure is re-read from --strict's print. */
  /* D-463 side, kept as history: regions 168 on its own branch; the c20-batch27 union's figure is re-read from the print. */
  /* D-147 side, kept as history (c22-batch29: ours' regions: 198 kept; NUMBER TO RE-READ from --strict's print on the union): regions: 177, — D-147 (C-94 LIFECYCLE_CHECKS, 2026-09-25, branch land/worker/D-147): 174 -> 177, READ from this guard's own print on the item's tree over origin/main 964da679 — the new family, its eleven rows and codes, and its three regions (is-lifecycle-writable, is-lifecycle-grammar, is-lifecycle-due-cite). Every other key gated at slack 0 on the same run. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads regionLines at integration. */
  /* REC-203 side, kept as history (c22-batch29: ours' regions: 198 kept; NUMBER TO RE-READ from --strict's print on the union): regions: 177, — REC-203 (C-91 IDSPACE_CHECKS, 2026-09-25, branch land/worker/REC-203): MOVED 174 -> 177 from this guard's own --strict print on the item's tree over origin/main 964da679. Whose growth it absorbs: op=idmatch's ONE new family and its three refusals, each at its own DEC-49 REGION in store.mjs idMatch. regionLines is a property of the MERGED source, so CONDUCT re-reads it at integration. */
  /* D-547 side, kept as history: regions 170 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-217 side, kept as history: regions 172 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-448 side, kept as history: regions 180 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-468 side, kept as history: regions 170 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-214 side, kept as history: regions 171 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-549 side, kept as history: regions 170 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-512 side, kept as history: regions 170 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-205 side, kept as history: regions 167 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* c21-rec207-renumber side, kept as history: regions 168 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-530 side, kept as history: regions 170 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-150 side, kept as history: regions 179 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-134 side, kept as history: regions 182 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-589 side, kept as history: regions 177 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* MK-7 side, kept as history: regions 179 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* REC-147 side, kept as history: regions 181 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* REC-197 side, kept as history: regions 177 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-521b side, kept as history: regions 197 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-563 side, kept as history: regions 200 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-561 side, kept as history: regions 207 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-578 side, kept as history: regions 201 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-546 side, kept as history: regions 202 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-615 side, kept as history: regions 203 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-623 side, kept as history: regions 199 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-629 side, kept as history: regions 200 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-680 side, kept as history: regions 199 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* REC-215 side, kept as history: regions 200 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-419 side, kept as history: regions 200 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-689 side, kept as history: regions 199 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  /* D-695 side, kept as history: regions 200 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  /* D-688 side, kept as history: regions 247 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  /* D-628 side, kept as history: regions 204 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  regions: 261, /* CONDUCT #23 at the c23-batch30 union: 246 -> 261, READ from `node civicos-ui/check-refusal-codes.mjs --strict`'s print on the merged tree (HEAD 645f8c41); never a branch's figure, never added by hand. */ /* CONDUCT #22 at the c22-batch29 union: 198 -> 246, the union's figure READ from `node civicos-ui/check-refusal-codes.mjs --strict`'s print on the merged tree (`ratchet: regions ... measured 246`). */ /* CONDUCT #21 at c21-batch28: 174 -> 198, the union's figure READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch27: 169 -> 174, the union's figure (D-463, D-513 and the batch) READ from --strict's print on the committed merge. */ /* CONDUCT #20 at c20-batch25: 168 -> 169, D-510's C-86.1 (the PROMOTED_TYPE_CHECKS family and its region) over the union, READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch25: 166 -> 168, D-472's C-48.8/C-48.9 and their two regions over the union, READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch23: 164 -> 166, REC-211's arrivals over the c20-batch22 union, READ from this guard's --strict print on the merged tree. */ /* CONDUCT #20 at c20-batch22: the UNION of D-507 (157 -> 163) and D-508 (157 -> 158), READ from this guard's --strict print on the merged tree. */ /* D-507 (C-82.2..C-82.7, the six STATEMENT_ACK_* refusals that reached a member untranslated, 2026-09-24, branch land/worker/D-507): MOVED 157 -> 163 from this guard's own --strict print on the item's tree over origin/main 68fecb8d. Six new DEC-49 REGION marker pairs in acknowledgeStatement, one per refusal. ZERO of the move is pre-existing slack: the PRISTINE origin/main 68fecb8d, run in this worktree before any edit, printed exit 0 with EVERY ratchet key at slack 0. */  /* CONDUCT #20 (c20-batch14, 2026-09-24): 155 -> 157, RE-READ from this guard's own `--strict` print on the union of this branch with `origin/main` @ 454a02bc (c20-batch17 and c20-batch18). Whose growth it absorbs: D-484's two new DEC-49 regions. Never either side's figure and never added by hand. */ /* D-484 (C-33.40 NO_BASIS, C-33.41 NO_CITATION consolidated behind one governed helper each, 2026-09-24, branch land/worker/D-484): MOVED 151 -> 153 from this guard's own --strict print on the item's tree over origin/main 16fe1e7f. ZERO pre-existing slack: the PRISTINE origin/main 16fe1e7f, run before any edit in this worktree, printed exit 0 with EVERY ratchet key at slack 0. the two new marker pairs, `is-act-no-basis` and `is-act-no-citation`. */ /* CONDUCT #20 (c20-batch14, 2026-09-24): 153 -> 155, RE-READ from this guard's own `--strict` print on the committed union of c20-batch11fix and c20-integ1b — never either branch's figure and never added by hand. Whose growth it absorbs: the TWO are D-64's two new DEC-49 regions, the same pair. Both branches had re-done the same c19-batch10 + `main` union, so each branch's own figure was short of the union's. */ /* CONDUCT #20 (c20-integ1, 2026-09-24): 152 -> 155, RE-READ from this guard's own --strict print on the committed union (D-64 renumbered C-82 -> C-83, REC-184, and origin/main @ 548eb2c5 merged over c19-batch10) — never added by hand, and never either side's figure. Whose growth it absorbs: +2 D-64 and +1 IC-246, the same three marker pairs. */ /* D-64 (C-83 RENDER_CAPTURE_CHECKS, 2026-09-24, cloud session WORKER D-64 (CONDUCT #19), branch land/worker/D-64): MOVED 124 -> 126 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: the same two regions. ZERO of the move is pre-existing slack: every other key gated at slack 0 on the same run, and the move is exactly this item's own. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration. */ /* CONDUCT #19 (c19-batch11, 2026-09-24): c19-unionfix @ 7b52ab9b (at 151) merged into c19-batch10 (at 152); RE-READ from this guard's --strict print on the merged tree, never added by hand. Both sides' comments carried below. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): REC-189 (C-32.19 MACHINE_CANNOT_SET_RISK_TIER, minted C-32.18) merged over the batch; 151 -> 152 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): D-461 (C-78.2 NAMESPACE_PINNED) merged over the batch; 150 -> 151 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* D-461 (C-78.2 NAMESPACE_PINNED, 2026-09-24, cloud session WORKER D-461 (CONDUCT #19)): MOVED 124 -> 125 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: one new row in the existing NAMESPACE_CHECKS family and one new governed REGION (pinnedNamespaceGate > is-pinned-namespace-gate). ZERO of the move is pre-existing slack: the same guard on 15b2a4c0 gated every key at slack 0. */ /* c19-unionfix (c19-batch9, 2026-09-24): 150 -> 151, re-read from this guard's --strict print on the union with IC-246: its ONE new family STATEMENT_ACK_CHECKS (C-82), ONE row and code STATEMENT_ACK_DOCUMENTS_OVER_BOUND, ONE region `is-statement-ack-documents-bound` in acknowledgeStatement. */ /* CONDUCT #18 (c18-batch8, 2026-09-23): D-394 merged over the batch; 142 -> 143 RE-READ from this guard's --strict print on the committed merge 8477cdb5. The new members are D-394's; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-126 (C-75 and C-76, the per-item weight) merged over the batch; 137 -> 142 RE-READ from this guard's --strict print on the committed merge 6ab139df. The new members are D-126 (C-75 and C-76, the per-item weight)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-149 (C-70, discoverable or hidden) merged over the batch; 134 -> 137 RE-READ from this guard's --strict print on the committed merge f4be4c07. The new members are REC-149 (C-70, discoverable or hidden)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-122 (C-74, the on-point choice) merged over the batch; 133 -> 134 RE-READ from this guard's --strict print on the committed merge e1d34ee0. The new members are REC-122 (C-74, the on-point choice)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-149 (C-73 and C-32.18, the governing laws) merged over the batch; 130 -> 133 RE-READ from this guard's --strict print on the committed merge 8086d64f. The new members are D-149 (C-73 and C-32.18, the governing laws)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-148 (C-72, the fee quote) merged over the batch; 127 -> 130 RE-READ from this guard's --strict print on the committed merge c81aec68. The new members are D-148 (C-72, the fee quote)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-164 merged over c17-batch6; 123 -> 127 RE-READ from this guard's --strict print on the committed merge 84443ad4. The new members are REC-164's (C-64.4..C-64.7, the group's name and domain acts); neither side's figure is the sum. */ /* CONDUCT #17 (c17-batch5, 2026-09-23): REC-161 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-161 (C-71 PARTITION_INDEPENDENCE_CHECKS, 2026-09-23, cloud session WORKER REC-161 (CONDUCT #17)): MOVED 118 -> 119 from this guard's own print on the item's tree over origin/main 02603e88: the one new region partitionIndependence > is-partition-independence. ZERO of the move is pre-existing slack: the same guard over a pristine 02603e88 worktree printed 118, every key gated at slack 0. */ /* CONDUCT #17 (c17-batch4, 2026-09-23): D-179 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-179 (C-53.13 CAPTURE_HELD_BY_ANOTHER_BUNDLE, 2026-09-23, branch land/worker/D-179): MOVED 117 -> 118 from this guard's own print on the item's tree: the one new REGION, is-register-home. ZERO pre-existing slack — every other key gated at 0 on the same run, and the move is exactly this item's own. */ /* CONDUCT #17 (c17-batch3, 2026-09-23): D-278 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-278 (C-68 + C-69, 2026-09-23, branch land/worker/D-278): MOVED 117 -> 120 from this guard's own print on the item's tree, every other ratchet key gated. the same three narrowed REGIONs. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): origin/main (c16-batch7) merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-168 (C-33.39 RETIRED_NOT_CITABLE in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-ab0b1ebe531cb428b): MOVED 114 -> 115 from this guard's own print on the item's tree: one new region, is-cite-retired. ZERO pre-existing slack — the pristine HEAD (0e7cc03e) sources, swapped in and restored by sha256 AND cmp, printed 114, every key gated.  — CONDUCT #16 merged it onto REC-175/REC-176, which moved this key from the same base; the figure is re-read from the merged print. */ /* REC-179 (C-66.5 SURFACED_BY_REWRITTEN in SURFACE_CHECKS, one new region promote > is-promote-surfaced-by, 2026-09-23, worktree agent-a368de25bd347b32f): MOVED 115 -> 116 from this guard's own print on the item's tree MERGED with origin/main 0e5f7054 (REC-175 landed first and moved this key from the same base, so this figure is re-read, never added), every other ratchet key gated. ZERO of the move is pre-existing slack: the merged tree without this item printed 115. */ /* REC-175 (C-33.38 FILE_DIGEST_MISMATCH in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-a6f377d2e96aa24af): MOVED 114 -> 115 from this guard's own print on the item's tree over origin/main 14faa089, every other ratchet key gated at slack 0 on the same run. One new REGION marker pair, is-promote-digest. ZERO of the move is pre-existing slack: the baseline run on 14faa089 printed exactly the old floor, so the move is this item's own. */ /* UI-81 (C-44.2 FINDING_IN_SEVERAL_CASES in CASE_DERIVATION_CHECKS, 2026-09-23, worktree agent-a306ff29faf082ef3): MOVED 113 -> 114 from this guard's own print on the item's tree, every other ratchet key gated. The same span, a narrowed REGION. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* D-85 (C-66 SURFACE_CHECKS, 2026-09-23, worktree agent-a09e49cc337fc2200): MOVED 112 -> 113 from this guard's own print on the item's tree, every other ratchet key gated. The same span, a narrowed REGION. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* REC-167 (C-65 CASE_CONCLUSION_CHECKS, 2026-09-22, worktree agent-a33d4bac9a4ff9d3a): MOVED 111 -> 112 from this guard's own print on the item's tree, every other ratchet key gated. The same span, a narrowed REGION. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* D-436 (C-64 INSTANCE_GROUP_CHECKS, 2026-09-21, worktree agent-a6dd0a0a3a3a6cc10): MOVED 109 -> 111 from this guard's own GREEN print on the item's committed tree 2f09b7b6. The same two spans, both narrowed REGIONS. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* D-158 (C-63 SIGNER_ENROLMENT_CHECKS, 2026-09-20, worktree d158-conduct8): MOVED 107 -> 109 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE origin/main sources (e1aa2eee) with this item's three plane files (store.mjs, setup.mjs, bio-checks.mjs) swapped out and restored by sha256 AND cmp: pristine printed 108, so 1 of the move is PRE-EXISTING SLACK and +1 is this item's (the same one region). */    /* D-270 (C-38.7/C-38.8 + C-61 REQUIRED_ARGUMENT_CHECKS, 2026-09-19, worktree d270-record): MOVED 70 -> 107 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE origin/main sources (98209ce2) with this item's two plane files swapped out and restored by sha256 AND cmp: pristine printed 105, so 35 of the move is PRE-EXISTING SLACK and +2 is this item's (the same two regions). */    /* REC-86 (IC-123, C-50 NARROW_CHECKS, 2026-09-18, worktree agent-a3fbd59a3fef1a961): MOVED 67 -> 70 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE HEAD sources (92f4c64e) with the item's four plane files swapped out and restored by sha256 + cmp: pristine printed 67, so 0 of the move is PRE-EXISTING slack and +3 is this item's. */ // + REC-69's `is-airuns-context` (ONE region, three codes, every one COMPARED —
  /* D-508 side, kept as history: */  /* D-508 (C-85 KNOCK_CHECKS, 2026-09-24, cloud session WORKER D-508 (CONDUCT #20), branch land/worker/D-508): MOVED 157 -> 158 from this guard's own --strict print on the item's tree over origin/main 68fecb8d0. Whose growth it absorbs: the ONE new region, knock > is-knock-rate. ZERO of the move is pre-existing slack: the PRISTINE origin/main 68fecb8d0, run in this worktree before any edit, printed exit 0 with EVERY ratchet key at slack 0. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads regionLines at integration — and D-507 moves the same catalogue in parallel. */ /* CONDUCT #20 (c20-batch14, 2026-09-24): 155 -> 157, RE-READ from this guard's own `--strict` print on the union of this branch with `origin/main` @ 454a02bc (c20-batch17 and c20-batch18). Whose growth it absorbs: D-484's two new DEC-49 regions. Never either side's figure and never added by hand. */ /* D-484 (C-33.40 NO_BASIS, C-33.41 NO_CITATION consolidated behind one governed helper each, 2026-09-24, branch land/worker/D-484): MOVED 151 -> 153 from this guard's own --strict print on the item's tree over origin/main 16fe1e7f. ZERO pre-existing slack: the PRISTINE origin/main 16fe1e7f, run before any edit in this worktree, printed exit 0 with EVERY ratchet key at slack 0. the two new marker pairs, `is-act-no-basis` and `is-act-no-citation`. */ /* CONDUCT #20 (c20-batch14, 2026-09-24): 153 -> 155, RE-READ from this guard's own `--strict` print on the committed union of c20-batch11fix and c20-integ1b — never either branch's figure and never added by hand. Whose growth it absorbs: the TWO are D-64's two new DEC-49 regions, the same pair. Both branches had re-done the same c19-batch10 + `main` union, so each branch's own figure was short of the union's. */ /* CONDUCT #20 (c20-integ1, 2026-09-24): 152 -> 155, RE-READ from this guard's own --strict print on the committed union (D-64 renumbered C-82 -> C-83, REC-184, and origin/main @ 548eb2c5 merged over c19-batch10) — never added by hand, and never either side's figure. Whose growth it absorbs: +2 D-64 and +1 IC-246, the same three marker pairs. */ /* D-64 (C-83 RENDER_CAPTURE_CHECKS, 2026-09-24, cloud session WORKER D-64 (CONDUCT #19), branch land/worker/D-64): MOVED 124 -> 126 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: the same two regions. ZERO of the move is pre-existing slack: every other key gated at slack 0 on the same run, and the move is exactly this item's own. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration. */ /* CONDUCT #19 (c19-batch11, 2026-09-24): c19-unionfix @ 7b52ab9b (at 151) merged into c19-batch10 (at 152); RE-READ from this guard's --strict print on the merged tree, never added by hand. Both sides' comments carried below. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): REC-189 (C-32.19 MACHINE_CANNOT_SET_RISK_TIER, minted C-32.18) merged over the batch; 151 -> 152 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): D-461 (C-78.2 NAMESPACE_PINNED) merged over the batch; 150 -> 151 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* D-461 (C-78.2 NAMESPACE_PINNED, 2026-09-24, cloud session WORKER D-461 (CONDUCT #19)): MOVED 124 -> 125 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: one new row in the existing NAMESPACE_CHECKS family and one new governed REGION (pinnedNamespaceGate > is-pinned-namespace-gate). ZERO of the move is pre-existing slack: the same guard on 15b2a4c0 gated every key at slack 0. */ /* c19-unionfix (c19-batch9, 2026-09-24): 150 -> 151, re-read from this guard's --strict print on the union with IC-246: its ONE new family STATEMENT_ACK_CHECKS (C-82), ONE row and code STATEMENT_ACK_DOCUMENTS_OVER_BOUND, ONE region `is-statement-ack-documents-bound` in acknowledgeStatement. */ /* CONDUCT #18 (c18-batch8, 2026-09-23): D-394 merged over the batch; 142 -> 143 RE-READ from this guard's --strict print on the committed merge 8477cdb5. The new members are D-394's; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-126 (C-75 and C-76, the per-item weight) merged over the batch; 137 -> 142 RE-READ from this guard's --strict print on the committed merge 6ab139df. The new members are D-126 (C-75 and C-76, the per-item weight)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-149 (C-70, discoverable or hidden) merged over the batch; 134 -> 137 RE-READ from this guard's --strict print on the committed merge f4be4c07. The new members are REC-149 (C-70, discoverable or hidden)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-122 (C-74, the on-point choice) merged over the batch; 133 -> 134 RE-READ from this guard's --strict print on the committed merge e1d34ee0. The new members are REC-122 (C-74, the on-point choice)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-149 (C-73 and C-32.18, the governing laws) merged over the batch; 130 -> 133 RE-READ from this guard's --strict print on the committed merge 8086d64f. The new members are D-149 (C-73 and C-32.18, the governing laws)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-148 (C-72, the fee quote) merged over the batch; 127 -> 130 RE-READ from this guard's --strict print on the committed merge c81aec68. The new members are D-148 (C-72, the fee quote)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-164 merged over c17-batch6; 123 -> 127 RE-READ from this guard's --strict print on the committed merge 84443ad4. The new members are REC-164's (C-64.4..C-64.7, the group's name and domain acts); neither side's figure is the sum. */ /* CONDUCT #17 (c17-batch5, 2026-09-23): REC-161 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-161 (C-71 PARTITION_INDEPENDENCE_CHECKS, 2026-09-23, cloud session WORKER REC-161 (CONDUCT #17)): MOVED 118 -> 119 from this guard's own print on the item's tree over origin/main 02603e88: the one new region partitionIndependence > is-partition-independence. ZERO of the move is pre-existing slack: the same guard over a pristine 02603e88 worktree printed 118, every key gated at slack 0. */ /* CONDUCT #17 (c17-batch4, 2026-09-23): D-179 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-179 (C-53.13 CAPTURE_HELD_BY_ANOTHER_BUNDLE, 2026-09-23, branch land/worker/D-179): MOVED 117 -> 118 from this guard's own print on the item's tree: the one new REGION, is-register-home. ZERO pre-existing slack — every other key gated at 0 on the same run, and the move is exactly this item's own. */ /* CONDUCT #17 (c17-batch3, 2026-09-23): D-278 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-278 (C-68 + C-69, 2026-09-23, branch land/worker/D-278): MOVED 117 -> 120 from this guard's own print on the item's tree, every other ratchet key gated. the same three narrowed REGIONs. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): origin/main (c16-batch7) merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-168 (C-33.39 RETIRED_NOT_CITABLE in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-ab0b1ebe531cb428b): MOVED 114 -> 115 from this guard's own print on the item's tree: one new region, is-cite-retired. ZERO pre-existing slack — the pristine HEAD (0e7cc03e) sources, swapped in and restored by sha256 AND cmp, printed 114, every key gated.  — CONDUCT #16 merged it onto REC-175/REC-176, which moved this key from the same base; the figure is re-read from the merged print. */ /* REC-179 (C-66.5 SURFACED_BY_REWRITTEN in SURFACE_CHECKS, one new region promote > is-promote-surfaced-by, 2026-09-23, worktree agent-a368de25bd347b32f): MOVED 115 -> 116 from this guard's own print on the item's tree MERGED with origin/main 0e5f7054 (REC-175 landed first and moved this key from the same base, so this figure is re-read, never added), every other ratchet key gated. ZERO of the move is pre-existing slack: the merged tree without this item printed 115. */ /* REC-175 (C-33.38 FILE_DIGEST_MISMATCH in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-a6f377d2e96aa24af): MOVED 114 -> 115 from this guard's own print on the item's tree over origin/main 14faa089, every other ratchet key gated at slack 0 on the same run. One new REGION marker pair, is-promote-digest. ZERO of the move is pre-existing slack: the baseline run on 14faa089 printed exactly the old floor, so the move is this item's own. */ /* UI-81 (C-44.2 FINDING_IN_SEVERAL_CASES in CASE_DERIVATION_CHECKS, 2026-09-23, worktree agent-a306ff29faf082ef3): MOVED 113 -> 114 from this guard's own print on the item's tree, every other ratchet key gated. The same span, a narrowed REGION. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* D-85 (C-66 SURFACE_CHECKS, 2026-09-23, worktree agent-a09e49cc337fc2200): MOVED 112 -> 113 from this guard's own print on the item's tree, every other ratchet key gated. The same span, a narrowed REGION. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* REC-167 (C-65 CASE_CONCLUSION_CHECKS, 2026-09-22, worktree agent-a33d4bac9a4ff9d3a): MOVED 111 -> 112 from this guard's own print on the item's tree, every other ratchet key gated. The same span, a narrowed REGION. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* D-436 (C-64 INSTANCE_GROUP_CHECKS, 2026-09-21, worktree agent-a6dd0a0a3a3a6cc10): MOVED 109 -> 111 from this guard's own GREEN print on the item's committed tree 2f09b7b6. The same two spans, both narrowed REGIONS. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* D-158 (C-63 SIGNER_ENROLMENT_CHECKS, 2026-09-20, worktree d158-conduct8): MOVED 107 -> 109 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE origin/main sources (e1aa2eee) with this item's three plane files (store.mjs, setup.mjs, bio-checks.mjs) swapped out and restored by sha256 AND cmp: pristine printed 108, so 1 of the move is PRE-EXISTING SLACK and +1 is this item's (the same one region). */    /* D-270 (C-38.7/C-38.8 + C-61 REQUIRED_ARGUMENT_CHECKS, 2026-09-19, worktree d270-record): MOVED 70 -> 107 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE origin/main sources (98209ce2) with this item's two plane files swapped out and restored by sha256 AND cmp: pristine printed 105, so 35 of the move is PRE-EXISTING SLACK and +2 is this item's (the same two regions). */    /* REC-86 (IC-123, C-50 NARROW_CHECKS, 2026-09-18, worktree agent-a3fbd59a3fef1a961): MOVED 67 -> 70 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE HEAD sources (92f4c64e) with the item's four plane files swapped out and restored by sha256 + cmp: pristine printed 67, so 0 of the move is PRE-EXISTING slack and +3 is this item's. */ // + REC-69's `is-airuns-context` (ONE region, three codes, every one COMPARED —
  ...
  /* CONDUCT #19 (c19-batch9, 2026-09-24): D-162 (the THEME family, renumbered C-74 -> C-81) merged over the batch; RE-READ from this guard's --strict print on the merged tree. */
  /* D-490 side, kept as history: regionLines 3755 on its own branch; the c20-batch25 union's figure is re-read from the print. */
  /* D-510 side, kept as history: regionLines 3761 on its own branch; the c20-batch25 union's figure is re-read from the print. */
  /* D-472 side, kept as history: regionLines 3804 on its own branch; the c20-batch25 union's figure is re-read from the print. */
  /* D-491 side, kept as history: regionLines 3770 on its own branch; the c20-batch25 union's figure is re-read from the print. */
  /* D-492 side, kept as history: regionLines 3747 -> 3765 on its own branch (IC-274); the c20-batch24 union's figure is re-read from --strict's print. */
  /* REC-211 side, kept as history: regionLines 3747 -> 3775 on its own branch (IC-273); the c20-batch23 union's figure is re-read from --strict's print. */
  /* REC-186 side, kept as history (c22-batch29: ours' regionLines: 4250 kept; NUMBER TO RE-READ from --strict's print on the union): regionLines: 3975, — REC-186 (2026-09-25, branch land/worker/REC-186): 3967 -> 3975, READ from this guard's print on the item's tree over origin/main 8bdf20e6. Whose growth: C-33.48 LAST_OWNER_CANNOT_LEAVE and its region is-leave-owner-floor in projectLeave (the region's lines, 8). */
  /* D-513 side, kept as history: regionLines 3890 on its own branch; the c20-batch27 union's figure is re-read from --strict's print. */
  /* D-463 side, kept as history: regionLines 3883 on its own branch; the c20-batch27 union's figure is re-read from the print. */
  /* D-147 side, kept as history (c22-batch29: ours' regionLines: 4250 kept; NUMBER TO RE-READ from --strict's print on the union): regionLines: 4046, — D-147 (C-94 LIFECYCLE_CHECKS, 2026-09-25, branch land/worker/D-147): 4010 -> 4046, READ from this guard's own print on the item's tree over origin/main 964da679 — the new family, its eleven rows and codes, and its three regions (is-lifecycle-writable, is-lifecycle-grammar, is-lifecycle-due-cite). Every other key gated at slack 0 on the same run. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads regionLines at integration. */
  /* REC-203 side, kept as history (c22-batch29: ours' regionLines: 4250 kept; NUMBER TO RE-READ from --strict's print on the union): regionLines: 4035, — REC-203 (C-91 IDSPACE_CHECKS, 2026-09-25, branch land/worker/REC-203): MOVED 4010 -> 4035 from this guard's own --strict print on the item's tree over origin/main 964da679. Whose growth it absorbs: op=idmatch's ONE new family and its three refusals, each at its own DEC-49 REGION in store.mjs idMatch. regionLines is a property of the MERGED source, so CONDUCT re-reads it at integration. */
  /* D-547 side, kept as history: regionLines 3978 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-217 side, kept as history: regionLines 3992 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-448 side, kept as history: regionLines 4021 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-468 side, kept as history: regionLines 3992 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-214 side, kept as history: regionLines 3997 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-549 side, kept as history: regionLines 3972 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-512 side, kept as history: regionLines 3978 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-205 side, kept as history: regionLines 3880 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* c21-rec207-renumber side, kept as history: regionLines 3897 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-530 side, kept as history: regionLines 3975 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-454 side, kept as history: regionLines 3991 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-162 side, kept as history: regionLines 3991 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* REC-150 side, kept as history: regionLines 4080 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-520 side, kept as history: regionLines 3978 on its own branch; the c22-batch29 union's figure is re-read from --strict's print. */
  /* D-134 side, kept as history: regionLines 4057 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-589 side, kept as history: regionLines 4037 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* MK-7 side, kept as history: regionLines 4090 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* REC-147 side, kept as history: regionLines 4051 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* REC-197 side, kept as history: regionLines 4026 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-521b side, kept as history: regionLines 4241 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-563 side, kept as history: regionLines 4283 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-561 side, kept as history: regionLines 4296 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-374 side, kept as history: regionLines 4258 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* REC-204 side, kept as history: regionLines 4296 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-578 side, kept as history: regionLines 4293 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-546 side, kept as history: regionLines 4313 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-615 side, kept as history: regionLines 4327 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-623 side, kept as history: regionLines 4258 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-629 side, kept as history: regionLines 4258 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-626 side, kept as history: regionLines 4253 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-680 side, kept as history: regionLines 4264 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* REC-215 side, kept as history: regionLines 4259 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-419 side, kept as history: regionLines 4291 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-625 side, kept as history: regionLines 4251 on its own branch (4250 -> 4251: ONE line in `is-connection-choice`, the C-74.3 detail naming an EMPTY occurrence=); the c22-batch30 union's figure is re-read from the print. */
  /* D-689 side, kept as history: regionLines 4264 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  /* D-695 side, kept as history: regionLines 4270 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  /* D-670 side, kept as history: regionLines 4266 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  /* D-688 side, kept as history: regionLines 4707 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  /* D-628 side, kept as history: regionLines 4341 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  regionLines: 4928, /* CONDUCT #23 at the c23-batch30 union: 4705 -> 4928, READ from `node civicos-ui/check-refusal-codes.mjs --strict`'s print on the merged tree (HEAD 645f8c41); never a branch's figure, never added by hand. */ /* CONDUCT #22 at the c22-batch29 union: 4250 -> 4705, the union's figure READ from `node civicos-ui/check-refusal-codes.mjs --strict`'s print on the merged tree (`ratchet: regionLines ... measured 4705`). */ /* CONDUCT #21 at c21-batch28: 4010 -> 4250, the union's figure READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch27: 3967 -> 4010, the union's figure (D-463, D-513 and the batch) READ from --strict's print on the committed merge. */ /* CONDUCT #20 at c20-batch25: 3959 -> 3967, D-490's edit inside `fetch > is-render-admit` over the union, READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch25: 3945 -> 3959, D-510's C-86.1 (the PROMOTED_TYPE_CHECKS family and its region) over the union, READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch25: 3888 -> 3945, D-472's C-48.8/C-48.9 and their two regions over the union, READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch25: 3865 -> 3888, D-491's C-28.16 (and its is-capture-request region) over the batch24c union, READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch24: 3847 -> 3865, D-492's +18 (is-render-admit, is-render-result) over the c20-batch23 union, READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch23: 3801 -> 3847, REC-211's arrivals over the c20-batch22 union, READ from this guard's --strict print on the merged tree. */ /* CONDUCT #20 at c20-batch22: the UNION of D-507 (3747 -> 3797) and D-508 (3747 -> 3751), READ from this guard's --strict print on the merged tree. */ /* D-507 (C-82.2..C-82.7, the six STATEMENT_ACK_* refusals that reached a member untranslated, 2026-09-24, branch land/worker/D-507): MOVED 3747 -> 3797 from this guard's own --strict print on the item's tree over origin/main 68fecb8d. The 50 lines inside those six new regions. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration. ZERO of the move is pre-existing slack: the PRISTINE origin/main 68fecb8d, run in this worktree before any edit, printed exit 0 with EVERY ratchet key at slack 0. */  /* CONDUCT #20 (c20-batch14, 2026-09-24): 3731 -> 3747, RE-READ from this guard's own `--strict` print on the union of this branch with `origin/main` @ 454a02bc (c20-batch17 and c20-batch18). Whose growth it absorbs: the sixteen lines of D-484's two regions AS THE MERGED SOURCE holds them. Never either side's figure and never added by hand. */ /* D-484 (C-33.40 NO_BASIS, C-33.41 NO_CITATION consolidated behind one governed helper each, 2026-09-24, branch land/worker/D-484): MOVED 3593 -> 3609 from this guard's own --strict print on the item's tree over origin/main 16fe1e7f. ZERO pre-existing slack: the PRISTINE origin/main 16fe1e7f, run before any edit in this worktree, printed exit 0 with EVERY ratchet key at slack 0. the 16 lines inside those two regions. */ /* CONDUCT #20 (c20-batch14, 2026-09-24): 3663 -> 3731, RE-READ from this guard's own `--strict` print on the committed union of c20-batch11fix and c20-integ1b — never either branch's figure and never added by hand. Whose growth it absorbs: the 68 are the line counts of D-64's two new regions AS THE MERGED SOURCE holds them — a region's line count is a property of the merged file, which is why this one is re-read here and never carried from a branch. Both branches had re-done the same c19-batch10 + `main` union, so each branch's own figure was short of the union's. */ /* CONDUCT #20 (c20-integ1, 2026-09-24): 3604 -> 3681, RE-READ from this guard's own --strict print on the committed union (D-64 renumbered C-82 -> C-83, REC-184, and origin/main @ 548eb2c5 merged over c19-batch10) — never added by hand, and never either side's figure. Whose growth it absorbs: +68 inside D-64's two regions and +9 inside IC-246's, matching both items' own printed moves (D-64 3299->3367, c19-unionfix 3584->3593) summed over a tree that holds both. */ /* D-64 (C-83 RENDER_CAPTURE_CHECKS, 2026-09-24, cloud session WORKER D-64 (CONDUCT #19), branch land/worker/D-64): MOVED 3299 -> 3367 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: is-render-admit 48L + is-render-result 20L, as the per-site line prints them. ZERO of the move is pre-existing slack: every other key gated at slack 0 on the same run, and the move is exactly this item's own. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration. */ /* CONDUCT #19 (c19-batch11, 2026-09-24): REC-192 @ 2e102941 (C-71.8, C-71.9; IC-248; the branch read 3349 on main) merged over the batch; RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* REC-192 (the version arm of op=partitionindependence, 2026-09-24, cloud session WORKER REC-192 (CONDUCT #19), branch land/worker/REC-192): MOVED 3299 -> 3349, the figure this guard PRINTED on the item's tree over origin/main 15b2a4c0: the version arm written INSIDE is-partition-independence (the partition arm moved one block in beside it). ZERO of the move is pre-existing slack: the same guard over pristine origin/main printed exactly 3299. Integrator: RE-READ from this guard's print on the committed merge. */ /* D-456 (C-78 NAMESPACE_CHECKS, 2026-09-23, cloud session WORKER D-456 (CONDUCT #18), branch land/worker/D-456): MOVED 3294 -> 3299 from this guard's own --strict print on the item's tree over origin/main a8f6094a: one new family, one row (NAMESPACE_UNKNOWN), one governed region (namespaceGate > is-namespace-gate, 5 lines), one code judged and compared. ZERO of the move is pre-existing slack: the same guard on a8f6094a before the change printed 3294, every key gated at slack 0. */ /* CONDUCT #19 (c19-batch11, 2026-09-24): c19-unionfix @ 7b52ab9b (at 3593) merged into c19-batch10 (at 3604); RE-READ from this guard's --strict print on the merged tree, never added by hand. Both sides' comments carried below. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): BOB #32's risk-tier ruling (is-machine-set-risk-tier, one clause) merged over the batch; 3602 -> 3604 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): REC-189 (C-32.19 MACHINE_CANNOT_SET_RISK_TIER, minted C-32.18) merged over the batch; 3589 -> 3602 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): D-461 (C-78.2 NAMESPACE_PINNED) merged over the batch; 3584 -> 3589 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* D-461 (C-78.2 NAMESPACE_PINNED, 2026-09-24, cloud session WORKER D-461 (CONDUCT #19)): MOVED 3299 -> 3304 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: one new row in the existing NAMESPACE_CHECKS family and one new governed REGION (pinnedNamespaceGate > is-pinned-namespace-gate). ZERO of the move is pre-existing slack: the same guard on 15b2a4c0 gated every key at slack 0. */ /* c19-unionfix (c19-batch9, 2026-09-24): 3584 -> 3593, re-read from this guard's --strict print on the union with IC-246: its ONE new family STATEMENT_ACK_CHECKS (C-82), ONE row and code STATEMENT_ACK_DOCUMENTS_OVER_BOUND, ONE region `is-statement-ack-documents-bound` in acknowledgeStatement. */ /* CONDUCT #19 (c19-batch9, 2026-09-24): REC-159 merged over the batch — its custodial regions — 3508 -> 3511 RE-READ from --strict on the merged tree. */ /* CONDUCT #18 (c18-batch8, 2026-09-23): D-394 merged over the batch; 3483 -> 3503 RE-READ from this guard's --strict print on the committed merge 8477cdb5. The new members are D-394's; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-126 (C-75 and C-76, the per-item weight) merged over the batch; 3437 -> 3483 RE-READ from this guard's --strict print on the committed merge 6ab139df. The new members are D-126 (C-75 and C-76, the per-item weight)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-149 (C-70, discoverable or hidden) merged over the batch; 3414 -> 3437 RE-READ from this guard's --strict print on the committed merge f4be4c07. The new members are REC-149 (C-70, discoverable or hidden)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-122 (C-74, the on-point choice) merged over the batch; 3392 -> 3414 RE-READ from this guard's --strict print on the committed merge e1d34ee0. The new members are REC-122 (C-74, the on-point choice)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-149 (C-73 and C-32.18, the governing laws) merged over the batch; 3350 -> 3392 RE-READ from this guard's --strict print on the committed merge 8086d64f. The new members are D-149 (C-73 and C-32.18, the governing laws)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-148 (C-72, the fee quote) merged over the batch; 3321 -> 3350 RE-READ from this guard's --strict print on the committed merge c81aec68. The new members are D-148 (C-72, the fee quote)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-164 merged over c17-batch6; 3294 -> 3321 RE-READ from this guard's --strict print on the committed merge 84443ad4. The new members are REC-164's (C-64.4..C-64.7, the group's name and domain acts); neither side's figure is the sum. */ /* CONDUCT #17 (c17-batch5, 2026-09-23): REC-161 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-161 (C-71 PARTITION_INDEPENDENCE_CHECKS, 2026-09-23, cloud session WORKER REC-161 (CONDUCT #17)): MOVED 3175 -> 3262 from this guard's own print on the item's tree over origin/main 02603e88: the new region, 87 lines. ZERO of the move is pre-existing slack: the same guard over a pristine 02603e88 worktree printed 3175, every key gated at slack 0. */ /* CONDUCT #17 (c17-batch4, 2026-09-23): D-179 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-179 (C-53.13 CAPTURE_HELD_BY_ANOTHER_BUNDLE, 2026-09-23, branch land/worker/D-179): MOVED 3151 -> 3163 from this guard's own print on the item's tree: the new region is-register-home, 12 lines as printed on the per-site line; no other region moved. ZERO pre-existing slack — every other key gated at 0 on the same run, and the move is exactly this item's own. */ /* CONDUCT #17 (c17-batch4, 2026-09-23): D-125 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-125 (2026-09-23, land/worker/D-125): MOVED 3151 -> 3153 from this guard's own print on the item's tree over origin/main 91bcea6b, every other ratchet key gated at slack 0 on the same run: queueMute > is-mute-class now refuses an OBLIGATION by class for BOTH the kind and the item form (the test reads PERSONALLY_MUTABLE_CLASSES and the refusal names `item` or `kind`), +2 lines. ZERO pre-existing slack: the baseline run on 91bcea6b printed exactly 3151. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration. */ /* CONDUCT #17 (c17-batch3, 2026-09-23): D-278 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-278 (C-68 + C-69, 2026-09-23, branch land/worker/D-278): MOVED 3151 -> 3169 from this guard's own print on the item's tree, every other ratchet key gated. the lines inside those three new regions. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): origin/main (c16-batch7) merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-168 (C-33.39 RETIRED_NOT_CITABLE in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-ab0b1ebe531cb428b): MOVED 3112 -> 3130 from this guard's own print on the item's tree: the lines inside the new region is-cite-retired. ZERO pre-existing slack — the pristine HEAD (0e7cc03e) sources, swapped in and restored by sha256 AND cmp, printed 3112, every key gated.  — CONDUCT #16 merged it onto REC-175/REC-176, which moved this key from the same base; the figure is re-read from the merged print. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): REC-179 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-179 (C-66.5 SURFACED_BY_REWRITTEN in SURFACE_CHECKS, one new region promote > is-promote-surfaced-by, 2026-09-23, worktree agent-a368de25bd347b32f): MOVED 3123 -> 3130 from this guard's own print on the item's tree MERGED with origin/main 0e5f7054 (REC-175 landed first and moved this key from the same base, so this figure is re-read, never added), every other ratchet key gated. ZERO of the move is pre-existing slack: the merged tree without this item printed 3123. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): D-440 and D-420 each moved this key by their own row from 0e5f7054; the union's value is RE-READ from this guard's print on the committed merge. */ /* D-420 (C-45.12 CONTENT_EXTENT_NO_IMAGE_PAINTED in CONTENT_EXTENT_CHECKS, 2026-09-23, cloud session WORKER D-420 (CONDUCT #16)): MOVED 3123 -> 3129 from this guard's own print on the item's tree over origin/main 0e5f7054, every other ratchet key gated at slack 0 on the same run. 6 lines inside checkContentExtent > is-content-extent (273L -> 279L: the page-form call and its comment) — A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration. ZERO of the move is pre-existing slack: the baseline run on 0e5f7054 printed exactly the old floor, so the move is this item's own. D-440 moves this key from the same base; CONDUCT re-reads the figure from the merged print. */ /* D-440 (C-45.11, 2026-09-23, worktree agent-aa6d775965f554582), RE-READ ON ITS MERGE with origin/main 0e5f7054: MOVED 3123 -> 3134 from this guard's own print on the merged tree — the eleven lines of the new arm inside checkContentExtent > is-content-extent. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration. ZERO pre-existing slack. */ /* REC-175 (C-33.38 FILE_DIGEST_MISMATCH in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-a6f377d2e96aa24af): MOVED 3112 -> 3123 from this guard's own print on the item's tree over origin/main 14faa089, every other ratchet key gated at slack 0 on the same run. The 11 lines inside is-promote-digest. ZERO of the move is pre-existing slack: the baseline run on 14faa089 printed exactly the old floor, so the move is this item's own. */ /* UI-81 (C-44.2 FINDING_IN_SEVERAL_CASES in CASE_DERIVATION_CHECKS, 2026-09-23, worktree agent-a306ff29faf082ef3): MOVED 3104 -> 3112 from this guard's own print on the item's tree, every other ratchet key gated. 8 lines, as the per-site line prints them — and A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* D-85 (C-66 SURFACE_CHECKS, 2026-09-23, worktree agent-a09e49cc337fc2200): MOVED 3072 -> 3104 from this guard's own print on the item's tree, every other ratchet key gated. 32 lines, as the per-site line prints them — and A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* REC-168 (2026-09-23, worktree agent-a960b19c9e4696303): MOVED 3043 -> 3072 from this guard's own print on the item's tree: all 29 lines inside captureRequest > is-capture-request (91L -> 120L: the run's sight, the principal relay, the status refusal and their comments). Slack 0 on main (M0-79). A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration. */ /* REC-165 (2026-09-22, worktree agent-a6e92d28647553f52): MOVED 2991 -> 3030 from this guard's own print on the item's tree: all 39 lines inside suggestVersion > is-suggest-shape (the run's sight, principal relay, status and BOB #28's context check, with their comments). Slack 0 on main (M0-79). A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration. */ /* D-436 (C-64 INSTANCE_GROUP_CHECKS, 2026-09-21, worktree agent-a6dd0a0a3a3a6cc10): MOVED 2973 -> 2991 from this guard's own GREEN print on the item's committed tree 2f09b7b6. 5 + 13 lines, as the per-site line prints them. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* D-158 (C-63 SIGNER_ENROLMENT_CHECKS, 2026-09-20, worktree d158-conduct8): MOVED 2952 -> 2973 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE origin/main sources (e1aa2eee) with this item's three plane files (store.mjs, setup.mjs, bio-checks.mjs) swapped out and restored by sha256 AND cmp: pristine printed 2963, so 11 of the move is PRE-EXISTING SLACK and +10 is this item's (the 10 lines inside is-signer-member-attesting — AND A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration). */    /* D-270 (C-38.7/C-38.8 + C-61 REQUIRED_ARGUMENT_CHECKS, 2026-09-19, worktree d270-record): MOVED 2215 -> 2952 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE origin/main sources (98209ce2) with this item's two plane files swapped out and restored by sha256 AND cmp: pristine printed 2910, so 695 of the move is PRE-EXISTING SLACK and +42 is this item's (+29 is-session-op-gate, +7 is-required-argument, +6 on fetch > is-admission whose header comment is CORRECTED because the session gate has left it — and A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration). */   /* REC-86 (IC-123, C-50 NARROW_CHECKS, 2026-09-18, worktree agent-a3fbd59a3fef1a961): MOVED 2035 -> 2215 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE HEAD sources (92f4c64e) with the item's four plane files swapped out and restored by sha256 + cmp: pristine printed 2094, so 59 of the move is PRE-EXISTING slack and +121 — AND regionLines IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration is this item's. */ /* MOVED 1407 -> 1425 BY REC-69'S REPLAY, 2026-08-09, RE-READ FROM A GREEN RUN
  /* D-508 side, kept as history: */  /* D-508 (C-85 KNOCK_CHECKS, 2026-09-24, cloud session WORKER D-508 (CONDUCT #20), branch land/worker/D-508): MOVED 3747 -> 3751 from this guard's own --strict print on the item's tree over origin/main 68fecb8d0. Whose growth it absorbs: the 4 lines inside knock > is-knock-rate. ZERO of the move is pre-existing slack: the PRISTINE origin/main 68fecb8d0, run in this worktree before any edit, printed exit 0 with EVERY ratchet key at slack 0. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads regionLines at integration — and D-507 moves the same catalogue in parallel. */ /* CONDUCT #20 (c20-batch14, 2026-09-24): 3731 -> 3747, RE-READ from this guard's own `--strict` print on the union of this branch with `origin/main` @ 454a02bc (c20-batch17 and c20-batch18). Whose growth it absorbs: the sixteen lines of D-484's two regions AS THE MERGED SOURCE holds them. Never either side's figure and never added by hand. */ /* D-484 (C-33.40 NO_BASIS, C-33.41 NO_CITATION consolidated behind one governed helper each, 2026-09-24, branch land/worker/D-484): MOVED 3593 -> 3609 from this guard's own --strict print on the item's tree over origin/main 16fe1e7f. ZERO pre-existing slack: the PRISTINE origin/main 16fe1e7f, run before any edit in this worktree, printed exit 0 with EVERY ratchet key at slack 0. the 16 lines inside those two regions. */ /* CONDUCT #20 (c20-batch14, 2026-09-24): 3663 -> 3731, RE-READ from this guard's own `--strict` print on the committed union of c20-batch11fix and c20-integ1b — never either branch's figure and never added by hand. Whose growth it absorbs: the 68 are the line counts of D-64's two new regions AS THE MERGED SOURCE holds them — a region's line count is a property of the merged file, which is why this one is re-read here and never carried from a branch. Both branches had re-done the same c19-batch10 + `main` union, so each branch's own figure was short of the union's. */ /* CONDUCT #20 (c20-integ1, 2026-09-24): 3604 -> 3681, RE-READ from this guard's own --strict print on the committed union (D-64 renumbered C-82 -> C-83, REC-184, and origin/main @ 548eb2c5 merged over c19-batch10) — never added by hand, and never either side's figure. Whose growth it absorbs: +68 inside D-64's two regions and +9 inside IC-246's, matching both items' own printed moves (D-64 3299->3367, c19-unionfix 3584->3593) summed over a tree that holds both. */ /* D-64 (C-83 RENDER_CAPTURE_CHECKS, 2026-09-24, cloud session WORKER D-64 (CONDUCT #19), branch land/worker/D-64): MOVED 3299 -> 3367 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: is-render-admit 48L + is-render-result 20L, as the per-site line prints them. ZERO of the move is pre-existing slack: every other key gated at slack 0 on the same run, and the move is exactly this item's own. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration. */ /* CONDUCT #19 (c19-batch11, 2026-09-24): REC-192 @ 2e102941 (C-71.8, C-71.9; IC-248; the branch read 3349 on main) merged over the batch; RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* REC-192 (the version arm of op=partitionindependence, 2026-09-24, cloud session WORKER REC-192 (CONDUCT #19), branch land/worker/REC-192): MOVED 3299 -> 3349, the figure this guard PRINTED on the item's tree over origin/main 15b2a4c0: the version arm written INSIDE is-partition-independence (the partition arm moved one block in beside it). ZERO of the move is pre-existing slack: the same guard over pristine origin/main printed exactly 3299. Integrator: RE-READ from this guard's print on the committed merge. */ /* D-456 (C-78 NAMESPACE_CHECKS, 2026-09-23, cloud session WORKER D-456 (CONDUCT #18), branch land/worker/D-456): MOVED 3294 -> 3299 from this guard's own --strict print on the item's tree over origin/main a8f6094a: one new family, one row (NAMESPACE_UNKNOWN), one governed region (namespaceGate > is-namespace-gate, 5 lines), one code judged and compared. ZERO of the move is pre-existing slack: the same guard on a8f6094a before the change printed 3294, every key gated at slack 0. */ /* CONDUCT #19 (c19-batch11, 2026-09-24): c19-unionfix @ 7b52ab9b (at 3593) merged into c19-batch10 (at 3604); RE-READ from this guard's --strict print on the merged tree, never added by hand. Both sides' comments carried below. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): BOB #32's risk-tier ruling (is-machine-set-risk-tier, one clause) merged over the batch; 3602 -> 3604 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): REC-189 (C-32.19 MACHINE_CANNOT_SET_RISK_TIER, minted C-32.18) merged over the batch; 3589 -> 3602 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): D-461 (C-78.2 NAMESPACE_PINNED) merged over the batch; 3584 -> 3589 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* D-461 (C-78.2 NAMESPACE_PINNED, 2026-09-24, cloud session WORKER D-461 (CONDUCT #19)): MOVED 3299 -> 3304 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: one new row in the existing NAMESPACE_CHECKS family and one new governed REGION (pinnedNamespaceGate > is-pinned-namespace-gate). ZERO of the move is pre-existing slack: the same guard on 15b2a4c0 gated every key at slack 0. */ /* c19-unionfix (c19-batch9, 2026-09-24): 3584 -> 3593, re-read from this guard's --strict print on the union with IC-246: its ONE new family STATEMENT_ACK_CHECKS (C-82), ONE row and code STATEMENT_ACK_DOCUMENTS_OVER_BOUND, ONE region `is-statement-ack-documents-bound` in acknowledgeStatement. */ /* CONDUCT #19 (c19-batch9, 2026-09-24): REC-159 merged over the batch — its custodial regions — 3508 -> 3511 RE-READ from --strict on the merged tree. */ /* CONDUCT #18 (c18-batch8, 2026-09-23): D-394 merged over the batch; 3483 -> 3503 RE-READ from this guard's --strict print on the committed merge 8477cdb5. The new members are D-394's; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-126 (C-75 and C-76, the per-item weight) merged over the batch; 3437 -> 3483 RE-READ from this guard's --strict print on the committed merge 6ab139df. The new members are D-126 (C-75 and C-76, the per-item weight)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-149 (C-70, discoverable or hidden) merged over the batch; 3414 -> 3437 RE-READ from this guard's --strict print on the committed merge f4be4c07. The new members are REC-149 (C-70, discoverable or hidden)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-122 (C-74, the on-point choice) merged over the batch; 3392 -> 3414 RE-READ from this guard's --strict print on the committed merge e1d34ee0. The new members are REC-122 (C-74, the on-point choice)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-149 (C-73 and C-32.18, the governing laws) merged over the batch; 3350 -> 3392 RE-READ from this guard's --strict print on the committed merge 8086d64f. The new members are D-149 (C-73 and C-32.18, the governing laws)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-148 (C-72, the fee quote) merged over the batch; 3321 -> 3350 RE-READ from this guard's --strict print on the committed merge c81aec68. The new members are D-148 (C-72, the fee quote)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-164 merged over c17-batch6; 3294 -> 3321 RE-READ from this guard's --strict print on the committed merge 84443ad4. The new members are REC-164's (C-64.4..C-64.7, the group's name and domain acts); neither side's figure is the sum. */ /* CONDUCT #17 (c17-batch5, 2026-09-23): REC-161 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-161 (C-71 PARTITION_INDEPENDENCE_CHECKS, 2026-09-23, cloud session WORKER REC-161 (CONDUCT #17)): MOVED 3175 -> 3262 from this guard's own print on the item's tree over origin/main 02603e88: the new region, 87 lines. ZERO of the move is pre-existing slack: the same guard over a pristine 02603e88 worktree printed 3175, every key gated at slack 0. */ /* CONDUCT #17 (c17-batch4, 2026-09-23): D-179 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-179 (C-53.13 CAPTURE_HELD_BY_ANOTHER_BUNDLE, 2026-09-23, branch land/worker/D-179): MOVED 3151 -> 3163 from this guard's own print on the item's tree: the new region is-register-home, 12 lines as printed on the per-site line; no other region moved. ZERO pre-existing slack — every other key gated at 0 on the same run, and the move is exactly this item's own. */ /* CONDUCT #17 (c17-batch4, 2026-09-23): D-125 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-125 (2026-09-23, land/worker/D-125): MOVED 3151 -> 3153 from this guard's own print on the item's tree over origin/main 91bcea6b, every other ratchet key gated at slack 0 on the same run: queueMute > is-mute-class now refuses an OBLIGATION by class for BOTH the kind and the item form (the test reads PERSONALLY_MUTABLE_CLASSES and the refusal names `item` or `kind`), +2 lines. ZERO pre-existing slack: the baseline run on 91bcea6b printed exactly 3151. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration. */ /* CONDUCT #17 (c17-batch3, 2026-09-23): D-278 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-278 (C-68 + C-69, 2026-09-23, branch land/worker/D-278): MOVED 3151 -> 3169 from this guard's own print on the item's tree, every other ratchet key gated. the lines inside those three new regions. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): origin/main (c16-batch7) merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-168 (C-33.39 RETIRED_NOT_CITABLE in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-ab0b1ebe531cb428b): MOVED 3112 -> 3130 from this guard's own print on the item's tree: the lines inside the new region is-cite-retired. ZERO pre-existing slack — the pristine HEAD (0e7cc03e) sources, swapped in and restored by sha256 AND cmp, printed 3112, every key gated.  — CONDUCT #16 merged it onto REC-175/REC-176, which moved this key from the same base; the figure is re-read from the merged print. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): REC-179 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-179 (C-66.5 SURFACED_BY_REWRITTEN in SURFACE_CHECKS, one new region promote > is-promote-surfaced-by, 2026-09-23, worktree agent-a368de25bd347b32f): MOVED 3123 -> 3130 from this guard's own print on the item's tree MERGED with origin/main 0e5f7054 (REC-175 landed first and moved this key from the same base, so this figure is re-read, never added), every other ratchet key gated. ZERO of the move is pre-existing slack: the merged tree without this item printed 3123. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): D-440 and D-420 each moved this key by their own row from 0e5f7054; the union's value is RE-READ from this guard's print on the committed merge. */ /* D-420 (C-45.12 CONTENT_EXTENT_NO_IMAGE_PAINTED in CONTENT_EXTENT_CHECKS, 2026-09-23, cloud session WORKER D-420 (CONDUCT #16)): MOVED 3123 -> 3129 from this guard's own print on the item's tree over origin/main 0e5f7054, every other ratchet key gated at slack 0 on the same run. 6 lines inside checkContentExtent > is-content-extent (273L -> 279L: the page-form call and its comment) — A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration. ZERO of the move is pre-existing slack: the baseline run on 0e5f7054 printed exactly the old floor, so the move is this item's own. D-440 moves this key from the same base; CONDUCT re-reads the figure from the merged print. */ /* D-440 (C-45.11, 2026-09-23, worktree agent-aa6d775965f554582), RE-READ ON ITS MERGE with origin/main 0e5f7054: MOVED 3123 -> 3134 from this guard's own print on the merged tree — the eleven lines of the new arm inside checkContentExtent > is-content-extent. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration. ZERO pre-existing slack. */ /* REC-175 (C-33.38 FILE_DIGEST_MISMATCH in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-a6f377d2e96aa24af): MOVED 3112 -> 3123 from this guard's own print on the item's tree over origin/main 14faa089, every other ratchet key gated at slack 0 on the same run. The 11 lines inside is-promote-digest. ZERO of the move is pre-existing slack: the baseline run on 14faa089 printed exactly the old floor, so the move is this item's own. */ /* UI-81 (C-44.2 FINDING_IN_SEVERAL_CASES in CASE_DERIVATION_CHECKS, 2026-09-23, worktree agent-a306ff29faf082ef3): MOVED 3104 -> 3112 from this guard's own print on the item's tree, every other ratchet key gated. 8 lines, as the per-site line prints them — and A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* D-85 (C-66 SURFACE_CHECKS, 2026-09-23, worktree agent-a09e49cc337fc2200): MOVED 3072 -> 3104 from this guard's own print on the item's tree, every other ratchet key gated. 32 lines, as the per-site line prints them — and A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* REC-168 (2026-09-23, worktree agent-a960b19c9e4696303): MOVED 3043 -> 3072 from this guard's own print on the item's tree: all 29 lines inside captureRequest > is-capture-request (91L -> 120L: the run's sight, the principal relay, the status refusal and their comments). Slack 0 on main (M0-79). A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration. */ /* REC-165 (2026-09-22, worktree agent-a6e92d28647553f52): MOVED 2991 -> 3030 from this guard's own print on the item's tree: all 39 lines inside suggestVersion > is-suggest-shape (the run's sight, principal relay, status and BOB #28's context check, with their comments). Slack 0 on main (M0-79). A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration. */ /* D-436 (C-64 INSTANCE_GROUP_CHECKS, 2026-09-21, worktree agent-a6dd0a0a3a3a6cc10): MOVED 2973 -> 2991 from this guard's own GREEN print on the item's committed tree 2f09b7b6. 5 + 13 lines, as the per-site line prints them. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* D-158 (C-63 SIGNER_ENROLMENT_CHECKS, 2026-09-20, worktree d158-conduct8): MOVED 2952 -> 2973 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE origin/main sources (e1aa2eee) with this item's three plane files (store.mjs, setup.mjs, bio-checks.mjs) swapped out and restored by sha256 AND cmp: pristine printed 2963, so 11 of the move is PRE-EXISTING SLACK and +10 is this item's (the 10 lines inside is-signer-member-attesting — AND A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration). */    /* D-270 (C-38.7/C-38.8 + C-61 REQUIRED_ARGUMENT_CHECKS, 2026-09-19, worktree d270-record): MOVED 2215 -> 2952 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE origin/main sources (98209ce2) with this item's two plane files swapped out and restored by sha256 AND cmp: pristine printed 2910, so 695 of the move is PRE-EXISTING SLACK and +42 is this item's (+29 is-session-op-gate, +7 is-required-argument, +6 on fetch > is-admission whose header comment is CORRECTED because the session gate has left it — and A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration). */   /* REC-86 (IC-123, C-50 NARROW_CHECKS, 2026-09-18, worktree agent-a3fbd59a3fef1a961): MOVED 2035 -> 2215 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE HEAD sources (92f4c64e) with the item's four plane files swapped out and restored by sha256 + cmp: pristine printed 2094, so 59 of the move is PRE-EXISTING slack and +121 — AND regionLines IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration is this item's. */ /* MOVED 1407 -> 1425 BY REC-69'S REPLAY, 2026-08-09, RE-READ FROM A GREEN RUN
  ...
  /* CONDUCT #19 (c19-batch9, 2026-09-24): D-162 (the THEME family, renumbered C-74 -> C-81) merged over the batch; RE-READ from this guard's --strict print on the merged tree. */
  /* D-510 side, kept as history: codesChecked 431 on its own branch; the c20-batch25 union's figure is re-read from the print. */
  /* D-472 side, kept as history: codesChecked 431 on its own branch; the c20-batch25 union's figure is re-read from the print. */
  /* D-491 side, kept as history: codesChecked 430 on its own branch; the c20-batch25 union's figure is re-read from the print. */
  /* REC-211 side, kept as history: codesChecked 429 -> 433 on its own branch (IC-273); the c20-batch23 union's figure is re-read from --strict's print. */
  /* REC-186 side, kept as history (c22-batch29: ours' codesChecked: 493 kept; NUMBER TO RE-READ from --strict's print on the union): codesChecked: 447, — REC-186 (2026-09-25, branch land/worker/REC-186): 446 -> 447, READ from this guard's print on the item's tree over origin/main 8bdf20e6. Whose growth: C-33.48 LAST_OWNER_CANNOT_LEAVE and its region is-leave-owner-floor in projectLeave. */
  /* D-513 side, kept as history: codesChecked 447 on its own branch; the c20-batch27 union's figure is re-read from --strict's print. */
  /* D-463 side, kept as history: codesChecked 443 on its own branch; the c20-batch27 union's figure is re-read from the print. */
  /* D-147 side, kept as history (c22-batch29: ours' codesChecked: 493 kept; NUMBER TO RE-READ from --strict's print on the union): codesChecked: 466, — D-147 (C-94 LIFECYCLE_CHECKS, 2026-09-25, branch land/worker/D-147): 454 -> 466, READ from this guard's own print on the item's tree over origin/main 964da679 — the new family, its eleven rows and codes, and its three regions (is-lifecycle-writable, is-lifecycle-grammar, is-lifecycle-due-cite). Every other key gated at slack 0 on the same run. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads regionLines at integration. */
  /* REC-203 side, kept as history (c22-batch29: ours' codesChecked: 493 kept; NUMBER TO RE-READ from --strict's print on the union): codesChecked: 460, — REC-203 (C-91 IDSPACE_CHECKS, 2026-09-25, branch land/worker/REC-203): MOVED 454 -> 460 from this guard's own --strict print on the item's tree over origin/main 964da679. Whose growth it absorbs: op=idmatch's ONE new family and its three refusals, each at its own DEC-49 REGION in store.mjs idMatch. regionLines is a property of the MERGED source, so CONDUCT re-reads it at integration. */
  /* D-547 side, kept as history: codesChecked 448 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-217 side, kept as history: codesChecked 449 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-448 side, kept as history: codesChecked 457 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-468 side, kept as history: codesChecked 447 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-214 side, kept as history: codesChecked 451 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-549 side, kept as history: codesChecked 447 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-512 side, kept as history: codesChecked 447 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-205 side, kept as history: codesChecked 443 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* c21-rec207-renumber side, kept as history: codesChecked 451 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-530 side, kept as history: codesChecked 447 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-454 side, kept as history: codesChecked 448 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-162 side, kept as history: codesChecked 447 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* REC-150 side, kept as history: codesChecked 463 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-520 side, kept as history: codesChecked 447 on its own branch; the c22-batch29 union's figure is re-read from --strict's print. */
  /* D-134 side, kept as history: codesChecked 463 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* MK-7 side, kept as history: codesChecked 466 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* REC-147 side, kept as history: codesChecked 461 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* REC-197 side, kept as history: codesChecked 456 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-521b side, kept as history: codesChecked 492 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-563 side, kept as history: codesChecked 497 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-561 side, kept as history: codesChecked 502 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-374 side, kept as history: codesChecked 494 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* REC-204 side, kept as history: codesChecked 499 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-578 side, kept as history: codesChecked 499 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-546 side, kept as history: codesChecked 501 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-615 side, kept as history: codesChecked 503 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-623 side, kept as history: codesChecked 495 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-629 side, kept as history: codesChecked 496 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-680 side, kept as history: codesChecked 494 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* REC-215 side, kept as history: codesChecked 494 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-419 side, kept as history: codesChecked 498 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-689 side, kept as history: codesChecked 494 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  /* D-695 side, kept as history: codesChecked 495 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  /* D-670 side, kept as history: codesChecked 495 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  /* D-628 side, kept as history: codesChecked 505 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  codesChecked: 595, /* CONDUCT #23 at the c23-batch30 union: 565 -> 595, READ from `node civicos-ui/check-refusal-codes.mjs --strict`'s print on the merged tree (HEAD 645f8c41); never a branch's figure, never added by hand. */ /* CONDUCT #22 at the c22-batch29 union: 493 -> 565, the union's figure READ from `node civicos-ui/check-refusal-codes.mjs --strict`'s print on the merged tree (`ratchet: codesChecked ... measured 565`). */ /* CONDUCT #21 at c21-batch28: 454 -> 493, the union's figure READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch27: 446 -> 454, the union's figure (D-463, D-513 and the batch) READ from --strict's print on the committed merge. */ /* CONDUCT #20 at c20-batch25: 444 -> 446, D-510's C-86.1 (the PROMOTED_TYPE_CHECKS family and its region) over the union, READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch25: 442 -> 444, D-472's C-48.8/C-48.9 and their two regions over the union, READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch25: 441 -> 442, D-491's C-28.16 (and its is-capture-request region) over the batch24c union, READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch23: 437 -> 441, REC-211's arrivals over the c20-batch22 union, READ from this guard's --strict print on the merged tree. */ /* CONDUCT #20 at c20-batch22: the UNION of D-507 (429 -> 435) and D-508 (429 -> 431), READ from this guard's --strict print on the merged tree. */ /* D-507 (C-82.2..C-82.7, the six STATEMENT_ACK_* refusals that reached a member untranslated, 2026-09-24, branch land/worker/D-507): MOVED 429 -> 435 from this guard's own --strict print on the item's tree over origin/main 68fecb8d. Six codes now COMPARED against a row at a governed site. ZERO of the move is pre-existing slack: the PRISTINE origin/main 68fecb8d, run in this worktree before any edit, printed exit 0 with EVERY ratchet key at slack 0. */  /* CONDUCT #20 (c20-batch14, 2026-09-24): 425 -> 429, RE-READ from this guard's own `--strict` print on the union of this branch with `origin/main` @ 454a02bc (c20-batch17 and c20-batch18). Whose growth it absorbs: four more codes compared at a family row, D-484's two among them. Never either side's figure and never added by hand. */ /* D-484 (C-33.40 NO_BASIS, C-33.41 NO_CITATION consolidated behind one governed helper each, 2026-09-24, branch land/worker/D-484): MOVED 414 -> 418 from this guard's own --strict print on the item's tree over origin/main 16fe1e7f. ZERO pre-existing slack: the PRISTINE origin/main 16fe1e7f, run before any edit in this worktree, printed exit 0 with EVERY ratchet key at slack 0. two codes compared at each of the two new sites (`reason:` and `code:`, both string literals at the site, which is what makes them comparable at all). */ /* CONDUCT #20 (c20-batch14, 2026-09-24): 418 -> 425, RE-READ from this guard's own `--strict` print on the committed union of c20-batch11fix and c20-integ1b — never either branch's figure and never added by hand. Whose growth it absorbs: the SEVEN are D-64's C-83.1..7 compared at their family rows. Both branches had re-done the same c19-batch10 + `main` union, so each branch's own figure was short of the union's. */ /* CONDUCT #20 (c20-integ1, 2026-09-24): 415 -> 423, RE-READ from this guard's own --strict print on the committed union (D-64 renumbered C-82 -> C-83, REC-184, and origin/main @ 548eb2c5 merged over c19-batch10) — never added by hand, and never either side's figure. Whose growth it absorbs: +7 D-64 and +1 IC-246. */ /* D-64 (C-83 RENDER_CAPTURE_CHECKS, 2026-09-24, cloud session WORKER D-64 (CONDUCT #19), branch land/worker/D-64): MOVED 365 -> 372 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: the seven codes, each compared against its row. ZERO of the move is pre-existing slack: every other key gated at slack 0 on the same run, and the move is exactly this item's own. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration. */ /* CONDUCT #19 (c19-batch11, 2026-09-24): REC-192 @ 2e102941 (C-71.8, C-71.9; IC-248; the branch read 367 on main) merged over the batch; RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* REC-192 (the version arm of op=partitionindependence, 2026-09-24, cloud session WORKER REC-192 (CONDUCT #19), branch land/worker/REC-192): MOVED 365 -> 367, the figure this guard PRINTED on the item's tree over origin/main 15b2a4c0: the two new refusals compared against their rows at is-partition-independence. ZERO of the move is pre-existing slack: the same guard over pristine origin/main printed exactly 365. Integrator: RE-READ from this guard's print on the committed merge. */ /* D-456 (C-78 NAMESPACE_CHECKS, 2026-09-23, cloud session WORKER D-456 (CONDUCT #18), branch land/worker/D-456): MOVED 364 -> 365 from this guard's own --strict print on the item's tree over origin/main a8f6094a: one new family, one row (NAMESPACE_UNKNOWN), one governed region (namespaceGate > is-namespace-gate, 5 lines), one code judged and compared. ZERO of the move is pre-existing slack: the same guard on a8f6094a before the change printed 364, every key gated at slack 0. */ /* CONDUCT #19 (c19-batch11, 2026-09-24): c19-unionfix @ 7b52ab9b (at 414) merged into c19-batch10 (at 415); RE-READ from this guard's --strict print on the merged tree, never added by hand. Both sides' comments carried below. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): REC-189 (C-32.19 MACHINE_CANNOT_SET_RISK_TIER, minted C-32.18) merged over the batch; 414 -> 415 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): D-461 (C-78.2 NAMESPACE_PINNED) merged over the batch; 413 -> 414 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* D-461 (C-78.2 NAMESPACE_PINNED, 2026-09-24, cloud session WORKER D-461 (CONDUCT #19)): MOVED 365 -> 366 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: one new row in the existing NAMESPACE_CHECKS family and one new governed REGION (pinnedNamespaceGate > is-pinned-namespace-gate). ZERO of the move is pre-existing slack: the same guard on 15b2a4c0 gated every key at slack 0. */ /* c19-unionfix (c19-batch9, 2026-09-24): 413 -> 414, re-read from this guard's --strict print on the union with IC-246: its ONE new family STATEMENT_ACK_CHECKS (C-82), ONE row and code STATEMENT_ACK_DOCUMENTS_OVER_BOUND, ONE region `is-statement-ack-documents-bound` in acknowledgeStatement. */ /* CONDUCT #18 (c18-batch8, 2026-09-23): D-394 merged over the batch; 398 -> 401 RE-READ from this guard's --strict print on the committed merge 8477cdb5. The new members are D-394's; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-126 (C-75 and C-76, the per-item weight) merged over the batch; 390 -> 398 RE-READ from this guard's --strict print on the committed merge 6ab139df. The new members are D-126 (C-75 and C-76, the per-item weight)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-149 (C-70, discoverable or hidden) merged over the batch; 386 -> 390 RE-READ from this guard's --strict print on the committed merge f4be4c07. The new members are REC-149 (C-70, discoverable or hidden)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-122 (C-74, the on-point choice) merged over the batch; 383 -> 386 RE-READ from this guard's --strict print on the committed merge e1d34ee0. The new members are REC-122 (C-74, the on-point choice)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-149 (C-73 and C-32.18, the governing laws) merged over the batch; 376 -> 383 RE-READ from this guard's --strict print on the committed merge 8086d64f. The new members are D-149 (C-73 and C-32.18, the governing laws)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-148 (C-72, the fee quote) merged over the batch; 368 -> 376 RE-READ from this guard's --strict print on the committed merge c81aec68. The new members are D-148 (C-72, the fee quote)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-164 merged over c17-batch6; 364 -> 368 RE-READ from this guard's --strict print on the committed merge 84443ad4. The new members are REC-164's (C-64.4..C-64.7, the group's name and domain acts); neither side's figure is the sum. */ /* CONDUCT #17 (c17-batch5, 2026-09-23): REC-161 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-161 (C-71 PARTITION_INDEPENDENCE_CHECKS, 2026-09-23, cloud session WORKER REC-161 (CONDUCT #17)): MOVED 349 -> 358 from this guard's own print on the item's tree over origin/main 02603e88: the nine refusal sites in the new region. ZERO of the move is pre-existing slack: the same guard over a pristine 02603e88 worktree printed 349, every key gated at slack 0. */ /* CONDUCT #17 (c17-batch4, 2026-09-23): D-179 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-179 (C-53.13 CAPTURE_HELD_BY_ANOTHER_BUNDLE, 2026-09-23, branch land/worker/D-179): MOVED 345 -> 346 from this guard's own print on the item's tree: the one code compared at is-register-home. ZERO pre-existing slack — every other key gated at 0 on the same run, and the move is exactly this item's own. */ /* CONDUCT #17 (c17-batch3, 2026-09-23): D-278 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-278 (C-68 + C-69, 2026-09-23, branch land/worker/D-278): MOVED 345 -> 350 from this guard's own print on the item's tree, every other ratchet key gated. the five new codes compared at their sites. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): origin/main (c16-batch7) merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-168 (C-33.39 RETIRED_NOT_CITABLE in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-ab0b1ebe531cb428b): MOVED 339 -> 341 from this guard's own print on the item's tree: the codes compared inside the new region, as the per-site walk counts them. ZERO pre-existing slack — the pristine HEAD (0e7cc03e) sources, swapped in and restored by sha256 AND cmp, printed 339, every key gated.  — CONDUCT #16 merged it onto REC-175/REC-176, which moved this key from the same base; the figure is re-read from the merged print. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): REC-177 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-177 (C-22.16 AI_RUN_BOUND_NO_ALLOWANCE in AI_RUN_CHECKS, 2026-09-23, worktree agent-ad7020ce0605889f4), RE-READ ON THE MERGE onto origin/main 0e5f7054 (which carries REC-175's and D-168's moves of this key from the same base): MOVED 341 -> 342 from this guard's own print on the merged tree: checkConsume compares 8 codes where it compared 7 (`checkConsume 77L (8 judged, 8 code(s) checked)`). ZERO of the move is pre-existing slack: every other key gated at slack 0 on the same run. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): REC-179 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-179 (C-66.5 SURFACED_BY_REWRITTEN in SURFACE_CHECKS, one new region promote > is-promote-surfaced-by, 2026-09-23, worktree agent-a368de25bd347b32f): MOVED 341 -> 342 from this guard's own print on the item's tree MERGED with origin/main 0e5f7054 (REC-175 landed first and moved this key from the same base, so this figure is re-read, never added), every other ratchet key gated. ZERO of the move is pre-existing slack: the merged tree without this item printed 341. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): D-440 and D-420 each moved this key by their own row from 0e5f7054; the union's value is RE-READ from this guard's print on the committed merge. */ /* D-420 (C-45.12 CONTENT_EXTENT_NO_IMAGE_PAINTED in CONTENT_EXTENT_CHECKS, 2026-09-23, cloud session WORKER D-420 (CONDUCT #16)): MOVED 341 -> 342 from this guard's own print on the item's tree over origin/main 0e5f7054, every other ratchet key gated at slack 0 on the same run. The one literal code the region now compares (32 -> 33 at is-content-extent). ZERO of the move is pre-existing slack: the baseline run on 0e5f7054 printed exactly the old floor, so the move is this item's own. D-440 moves this key from the same base; CONDUCT re-reads the figure from the merged print. */ /* D-440 (C-45.11, 2026-09-23, worktree agent-aa6d775965f554582), RE-READ ON ITS MERGE with origin/main 0e5f7054: MOVED 341 -> 342 from this guard's own print on the merged tree — the one new literal compared against its row. ZERO pre-existing slack. */ /* REC-175 (C-33.38 FILE_DIGEST_MISMATCH in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-a6f377d2e96aa24af): MOVED 334 -> 336 from this guard's own print on the item's tree over origin/main 14faa089, every other ratchet key gated at slack 0 on the same run. The codes compared inside is-promote-digest, as the per-site line printed them (2 for 1 judged). ZERO of the move is pre-existing slack: the baseline run on 14faa089 printed exactly the old floor, so the move is this item's own.  — CONDUCT #16 merged it with REC-172, which moved this key from the same base; the figure is re-read from the merged print. */ /* REC-172 (2026-09-23, worktree agent-ad79857cb007927b7): MOVED 333 -> 338 from this guard's own print: checkConsume compares 7 codes where it compared 2 (pristine HEAD 91913d6b printed `checkConsume 17L (2 judged, 2 code(s) checked)` and 333; this tree prints 67L, 7 and 7) — the map refusal, the list's two, the unknown key and `lease`, every one a literal inside the one governed span. The move is exactly this item's; every other key gated on the same run.  — CONDUCT #16 merged it with UI-81, which moved this key from the same base; the figure is re-read from the merged print. */ /* UI-81 (C-44.2 FINDING_IN_SEVERAL_CASES in CASE_DERIVATION_CHECKS, 2026-09-23, worktree agent-a306ff29faf082ef3), RE-READ ON THE MERGE onto origin/main 91913d6b (which carries REC-169's moves): MOVED 333 -> 334 from this guard's own print on the merged tree, every other ratchet key gated. 1 code compared at the new span, as printed. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* REC-169 (C-22.13/C-22.14 in AI_RUN_CHECKS, 2026-09-23, worktree agent-a30f15e61a2d24169): MOVED 331 -> 333 from this guard's own print on the item's tree: checkConsume compares 2 codes, as printed. The move is exactly this item's own additions, read off the per-site line; every other key gated on the same run. */ /* D-85 (C-66 SURFACE_CHECKS, 2026-09-23, worktree agent-a09e49cc337fc2200): MOVED 327 -> 331 from this guard's own print on the item's tree, every other ratchet key gated. 4 codes compared at the new span, as printed. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* REC-168 (2026-09-23, worktree agent-a960b19c9e4696303): MOVED 326 -> 327 from this guard's own print: is-capture-request compares 8 where it compared 7 — the status refusal's own CAPTURE_REQUEST_NO_RUN literal. The relayed AI_RUN_NOT_PRINCIPAL is judged and NOT compared (a variable code; its literal stays at runPrincipalGate). Slack 0 on main (M0-79). */ /* REC-165 (2026-09-22, worktree agent-a6e92d28647553f52): MOVED 323 -> 325 from this guard's own print: the two literal codes is-suggest-shape now compares (SUGGEST_RUN_NOT_RUNNING, SUGGEST_OUTSIDE_RUN_CONTEXT). The relayed AI_RUN_NOT_PRINCIPAL is judged and NOT compared (a variable code; its literal stays at runPrincipalGate). Slack 0 on main (M0-79). */ /* D-436 (C-64 INSTANCE_GROUP_CHECKS, 2026-09-21, worktree agent-a6dd0a0a3a3a6cc10): MOVED 319 -> 323 from this guard's own GREEN print on the item's committed tree 2f09b7b6. 2 + 2 codes compared at the two spans, as printed. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* D-158 (C-63 SIGNER_ENROLMENT_CHECKS, 2026-09-20, worktree d158-conduct8): MOVED 316 -> 319 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE origin/main sources (e1aa2eee) with this item's three plane files (store.mjs, setup.mjs, bio-checks.mjs) swapped out and restored by sha256 AND cmp: pristine printed 317, so 1 of the move is PRE-EXISTING SLACK and +2 is this item's (the two codes the new region compares). */    /* D-270 (C-38.7/C-38.8 + C-61 REQUIRED_ARGUMENT_CHECKS, 2026-09-19, worktree d270-record): MOVED 218 -> 316 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE origin/main sources (98209ce2) with this item's two plane files swapped out and restored by sha256 AND cmp: pristine printed 313, so 95 of the move is PRE-EXISTING SLACK and +3 is this item's (+3 net: the four new region codes less the one that left is-admission). */   /* REC-86 (IC-123, C-50 NARROW_CHECKS, 2026-09-18, worktree agent-a3fbd59a3fef1a961): MOVED 201 -> 218 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE HEAD sources (92f4c64e) with the item's four plane files swapped out and restored by sha256 + cmp: pristine printed 204, so 3 of the move is PRE-EXISTING slack and +14 (the eleven C-50 codes, plus three the moved regions compare) is this item's. */ // + REC-69's THREE (C-36.1..3), all COMPARED: the region's `refusal` helper sits
  /* D-508 side, kept as history: */  /* D-508 (C-85 KNOCK_CHECKS, 2026-09-24, cloud session WORKER D-508 (CONDUCT #20), branch land/worker/D-508): MOVED 429 -> 431 from this guard's own --strict print on the item's tree over origin/main 68fecb8d0. Whose growth it absorbs: the two codes compared inside knock > is-knock-rate. ZERO of the move is pre-existing slack: the PRISTINE origin/main 68fecb8d0, run in this worktree before any edit, printed exit 0 with EVERY ratchet key at slack 0. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads regionLines at integration — and D-507 moves the same catalogue in parallel. */ /* CONDUCT #20 (c20-batch14, 2026-09-24): 425 -> 429, RE-READ from this guard's own `--strict` print on the union of this branch with `origin/main` @ 454a02bc (c20-batch17 and c20-batch18). Whose growth it absorbs: four more codes compared at a family row, D-484's two among them. Never either side's figure and never added by hand. */ /* D-484 (C-33.40 NO_BASIS, C-33.41 NO_CITATION consolidated behind one governed helper each, 2026-09-24, branch land/worker/D-484): MOVED 414 -> 418 from this guard's own --strict print on the item's tree over origin/main 16fe1e7f. ZERO pre-existing slack: the PRISTINE origin/main 16fe1e7f, run before any edit in this worktree, printed exit 0 with EVERY ratchet key at slack 0. two codes compared at each of the two new sites (`reason:` and `code:`, both string literals at the site, which is what makes them comparable at all). */ /* CONDUCT #20 (c20-batch14, 2026-09-24): 418 -> 425, RE-READ from this guard's own `--strict` print on the committed union of c20-batch11fix and c20-integ1b — never either branch's figure and never added by hand. Whose growth it absorbs: the SEVEN are D-64's C-83.1..7 compared at their family rows. Both branches had re-done the same c19-batch10 + `main` union, so each branch's own figure was short of the union's. */ /* CONDUCT #20 (c20-integ1, 2026-09-24): 415 -> 423, RE-READ from this guard's own --strict print on the committed union (D-64 renumbered C-82 -> C-83, REC-184, and origin/main @ 548eb2c5 merged over c19-batch10) — never added by hand, and never either side's figure. Whose growth it absorbs: +7 D-64 and +1 IC-246. */ /* D-64 (C-83 RENDER_CAPTURE_CHECKS, 2026-09-24, cloud session WORKER D-64 (CONDUCT #19), branch land/worker/D-64): MOVED 365 -> 372 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: the seven codes, each compared against its row. ZERO of the move is pre-existing slack: every other key gated at slack 0 on the same run, and the move is exactly this item's own. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration. */ /* CONDUCT #19 (c19-batch11, 2026-09-24): REC-192 @ 2e102941 (C-71.8, C-71.9; IC-248; the branch read 367 on main) merged over the batch; RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* REC-192 (the version arm of op=partitionindependence, 2026-09-24, cloud session WORKER REC-192 (CONDUCT #19), branch land/worker/REC-192): MOVED 365 -> 367, the figure this guard PRINTED on the item's tree over origin/main 15b2a4c0: the two new refusals compared against their rows at is-partition-independence. ZERO of the move is pre-existing slack: the same guard over pristine origin/main printed exactly 365. Integrator: RE-READ from this guard's print on the committed merge. */ /* D-456 (C-78 NAMESPACE_CHECKS, 2026-09-23, cloud session WORKER D-456 (CONDUCT #18), branch land/worker/D-456): MOVED 364 -> 365 from this guard's own --strict print on the item's tree over origin/main a8f6094a: one new family, one row (NAMESPACE_UNKNOWN), one governed region (namespaceGate > is-namespace-gate, 5 lines), one code judged and compared. ZERO of the move is pre-existing slack: the same guard on a8f6094a before the change printed 364, every key gated at slack 0. */ /* CONDUCT #19 (c19-batch11, 2026-09-24): c19-unionfix @ 7b52ab9b (at 414) merged into c19-batch10 (at 415); RE-READ from this guard's --strict print on the merged tree, never added by hand. Both sides' comments carried below. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): REC-189 (C-32.19 MACHINE_CANNOT_SET_RISK_TIER, minted C-32.18) merged over the batch; 414 -> 415 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): D-461 (C-78.2 NAMESPACE_PINNED) merged over the batch; 413 -> 414 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* D-461 (C-78.2 NAMESPACE_PINNED, 2026-09-24, cloud session WORKER D-461 (CONDUCT #19)): MOVED 365 -> 366 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: one new row in the existing NAMESPACE_CHECKS family and one new governed REGION (pinnedNamespaceGate > is-pinned-namespace-gate). ZERO of the move is pre-existing slack: the same guard on 15b2a4c0 gated every key at slack 0. */ /* c19-unionfix (c19-batch9, 2026-09-24): 413 -> 414, re-read from this guard's --strict print on the union with IC-246: its ONE new family STATEMENT_ACK_CHECKS (C-82), ONE row and code STATEMENT_ACK_DOCUMENTS_OVER_BOUND, ONE region `is-statement-ack-documents-bound` in acknowledgeStatement. */ /* CONDUCT #18 (c18-batch8, 2026-09-23): D-394 merged over the batch; 398 -> 401 RE-READ from this guard's --strict print on the committed merge 8477cdb5. The new members are D-394's; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-126 (C-75 and C-76, the per-item weight) merged over the batch; 390 -> 398 RE-READ from this guard's --strict print on the committed merge 6ab139df. The new members are D-126 (C-75 and C-76, the per-item weight)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-149 (C-70, discoverable or hidden) merged over the batch; 386 -> 390 RE-READ from this guard's --strict print on the committed merge f4be4c07. The new members are REC-149 (C-70, discoverable or hidden)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-122 (C-74, the on-point choice) merged over the batch; 383 -> 386 RE-READ from this guard's --strict print on the committed merge e1d34ee0. The new members are REC-122 (C-74, the on-point choice)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-149 (C-73 and C-32.18, the governing laws) merged over the batch; 376 -> 383 RE-READ from this guard's --strict print on the committed merge 8086d64f. The new members are D-149 (C-73 and C-32.18, the governing laws)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-148 (C-72, the fee quote) merged over the batch; 368 -> 376 RE-READ from this guard's --strict print on the committed merge c81aec68. The new members are D-148 (C-72, the fee quote)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-164 merged over c17-batch6; 364 -> 368 RE-READ from this guard's --strict print on the committed merge 84443ad4. The new members are REC-164's (C-64.4..C-64.7, the group's name and domain acts); neither side's figure is the sum. */ /* CONDUCT #17 (c17-batch5, 2026-09-23): REC-161 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-161 (C-71 PARTITION_INDEPENDENCE_CHECKS, 2026-09-23, cloud session WORKER REC-161 (CONDUCT #17)): MOVED 349 -> 358 from this guard's own print on the item's tree over origin/main 02603e88: the nine refusal sites in the new region. ZERO of the move is pre-existing slack: the same guard over a pristine 02603e88 worktree printed 349, every key gated at slack 0. */ /* CONDUCT #17 (c17-batch4, 2026-09-23): D-179 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-179 (C-53.13 CAPTURE_HELD_BY_ANOTHER_BUNDLE, 2026-09-23, branch land/worker/D-179): MOVED 345 -> 346 from this guard's own print on the item's tree: the one code compared at is-register-home. ZERO pre-existing slack — every other key gated at 0 on the same run, and the move is exactly this item's own. */ /* CONDUCT #17 (c17-batch3, 2026-09-23): D-278 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-278 (C-68 + C-69, 2026-09-23, branch land/worker/D-278): MOVED 345 -> 350 from this guard's own print on the item's tree, every other ratchet key gated. the five new codes compared at their sites. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): origin/main (c16-batch7) merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-168 (C-33.39 RETIRED_NOT_CITABLE in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-ab0b1ebe531cb428b): MOVED 339 -> 341 from this guard's own print on the item's tree: the codes compared inside the new region, as the per-site walk counts them. ZERO pre-existing slack — the pristine HEAD (0e7cc03e) sources, swapped in and restored by sha256 AND cmp, printed 339, every key gated.  — CONDUCT #16 merged it onto REC-175/REC-176, which moved this key from the same base; the figure is re-read from the merged print. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): REC-177 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-177 (C-22.16 AI_RUN_BOUND_NO_ALLOWANCE in AI_RUN_CHECKS, 2026-09-23, worktree agent-ad7020ce0605889f4), RE-READ ON THE MERGE onto origin/main 0e5f7054 (which carries REC-175's and D-168's moves of this key from the same base): MOVED 341 -> 342 from this guard's own print on the merged tree: checkConsume compares 8 codes where it compared 7 (`checkConsume 77L (8 judged, 8 code(s) checked)`). ZERO of the move is pre-existing slack: every other key gated at slack 0 on the same run. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): REC-179 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-179 (C-66.5 SURFACED_BY_REWRITTEN in SURFACE_CHECKS, one new region promote > is-promote-surfaced-by, 2026-09-23, worktree agent-a368de25bd347b32f): MOVED 341 -> 342 from this guard's own print on the item's tree MERGED with origin/main 0e5f7054 (REC-175 landed first and moved this key from the same base, so this figure is re-read, never added), every other ratchet key gated. ZERO of the move is pre-existing slack: the merged tree without this item printed 341. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): D-440 and D-420 each moved this key by their own row from 0e5f7054; the union's value is RE-READ from this guard's print on the committed merge. */ /* D-420 (C-45.12 CONTENT_EXTENT_NO_IMAGE_PAINTED in CONTENT_EXTENT_CHECKS, 2026-09-23, cloud session WORKER D-420 (CONDUCT #16)): MOVED 341 -> 342 from this guard's own print on the item's tree over origin/main 0e5f7054, every other ratchet key gated at slack 0 on the same run. The one literal code the region now compares (32 -> 33 at is-content-extent). ZERO of the move is pre-existing slack: the baseline run on 0e5f7054 printed exactly the old floor, so the move is this item's own. D-440 moves this key from the same base; CONDUCT re-reads the figure from the merged print. */ /* D-440 (C-45.11, 2026-09-23, worktree agent-aa6d775965f554582), RE-READ ON ITS MERGE with origin/main 0e5f7054: MOVED 341 -> 342 from this guard's own print on the merged tree — the one new literal compared against its row. ZERO pre-existing slack. */ /* REC-175 (C-33.38 FILE_DIGEST_MISMATCH in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-a6f377d2e96aa24af): MOVED 334 -> 336 from this guard's own print on the item's tree over origin/main 14faa089, every other ratchet key gated at slack 0 on the same run. The codes compared inside is-promote-digest, as the per-site line printed them (2 for 1 judged). ZERO of the move is pre-existing slack: the baseline run on 14faa089 printed exactly the old floor, so the move is this item's own.  — CONDUCT #16 merged it with REC-172, which moved this key from the same base; the figure is re-read from the merged print. */ /* REC-172 (2026-09-23, worktree agent-ad79857cb007927b7): MOVED 333 -> 338 from this guard's own print: checkConsume compares 7 codes where it compared 2 (pristine HEAD 91913d6b printed `checkConsume 17L (2 judged, 2 code(s) checked)` and 333; this tree prints 67L, 7 and 7) — the map refusal, the list's two, the unknown key and `lease`, every one a literal inside the one governed span. The move is exactly this item's; every other key gated on the same run.  — CONDUCT #16 merged it with UI-81, which moved this key from the same base; the figure is re-read from the merged print. */ /* UI-81 (C-44.2 FINDING_IN_SEVERAL_CASES in CASE_DERIVATION_CHECKS, 2026-09-23, worktree agent-a306ff29faf082ef3), RE-READ ON THE MERGE onto origin/main 91913d6b (which carries REC-169's moves): MOVED 333 -> 334 from this guard's own print on the merged tree, every other ratchet key gated. 1 code compared at the new span, as printed. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* REC-169 (C-22.13/C-22.14 in AI_RUN_CHECKS, 2026-09-23, worktree agent-a30f15e61a2d24169): MOVED 331 -> 333 from this guard's own print on the item's tree: checkConsume compares 2 codes, as printed. The move is exactly this item's own additions, read off the per-site line; every other key gated on the same run. */ /* D-85 (C-66 SURFACE_CHECKS, 2026-09-23, worktree agent-a09e49cc337fc2200): MOVED 327 -> 331 from this guard's own print on the item's tree, every other ratchet key gated. 4 codes compared at the new span, as printed. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* REC-168 (2026-09-23, worktree agent-a960b19c9e4696303): MOVED 326 -> 327 from this guard's own print: is-capture-request compares 8 where it compared 7 — the status refusal's own CAPTURE_REQUEST_NO_RUN literal. The relayed AI_RUN_NOT_PRINCIPAL is judged and NOT compared (a variable code; its literal stays at runPrincipalGate). Slack 0 on main (M0-79). */ /* REC-165 (2026-09-22, worktree agent-a6e92d28647553f52): MOVED 323 -> 325 from this guard's own print: the two literal codes is-suggest-shape now compares (SUGGEST_RUN_NOT_RUNNING, SUGGEST_OUTSIDE_RUN_CONTEXT). The relayed AI_RUN_NOT_PRINCIPAL is judged and NOT compared (a variable code; its literal stays at runPrincipalGate). Slack 0 on main (M0-79). */ /* D-436 (C-64 INSTANCE_GROUP_CHECKS, 2026-09-21, worktree agent-a6dd0a0a3a3a6cc10): MOVED 319 -> 323 from this guard's own GREEN print on the item's committed tree 2f09b7b6. 2 + 2 codes compared at the two spans, as printed. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* D-158 (C-63 SIGNER_ENROLMENT_CHECKS, 2026-09-20, worktree d158-conduct8): MOVED 316 -> 319 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE origin/main sources (e1aa2eee) with this item's three plane files (store.mjs, setup.mjs, bio-checks.mjs) swapped out and restored by sha256 AND cmp: pristine printed 317, so 1 of the move is PRE-EXISTING SLACK and +2 is this item's (the two codes the new region compares). */    /* D-270 (C-38.7/C-38.8 + C-61 REQUIRED_ARGUMENT_CHECKS, 2026-09-19, worktree d270-record): MOVED 218 -> 316 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE origin/main sources (98209ce2) with this item's two plane files swapped out and restored by sha256 AND cmp: pristine printed 313, so 95 of the move is PRE-EXISTING SLACK and +3 is this item's (+3 net: the four new region codes less the one that left is-admission). */   /* REC-86 (IC-123, C-50 NARROW_CHECKS, 2026-09-18, worktree agent-a3fbd59a3fef1a961): MOVED 201 -> 218 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE HEAD sources (92f4c64e) with the item's four plane files swapped out and restored by sha256 + cmp: pristine printed 204, so 3 of the move is PRE-EXISTING slack and +14 (the eleven C-50 codes, plus three the moved regions compare) is this item's. */ // + REC-69's THREE (C-36.1..3), all COMPARED: the region's `refusal` helper sits
  ...
  /* CONDUCT #19 (c19-batch9, 2026-09-24): the security train (D-456, D-447) merged over the batch; RE-READ from this guard's --strict print on the committed merge. */
  /* D-513 side, kept as history: outcomeReturns 168 on its own branch; the c20-batch27 union's figure is re-read from --strict's print. */
  /* D-463 side, kept as history: outcomeReturns 166 on its own branch; the c20-batch27 union's figure is re-read from the print. */
  /* D-510 side, kept as history: outcomeReturns 164 on its own branch; the c20-batch25 union's figure is re-read from the print. */
  /* D-472 side, kept as history: outcomeReturns 165 on its own branch; the c20-batch25 union's figure is re-read from the print. */
  /* REC-186 side, kept as history (c22-batch29: ours' outcomeReturns: 186 kept; NUMBER TO RE-READ from --strict's print on the union): outcomeReturns: 169, — REC-186 (2026-09-25, branch land/worker/REC-186): 168 -> 169, READ from this guard's print on the item's tree over origin/main 8bdf20e6. Whose growth: C-33.48 LAST_OWNER_CANNOT_LEAVE and its region is-leave-owner-floor in projectLeave. */
  /* REC-203 side, kept as history (c22-batch29: ours' outcomeReturns: 186 kept; NUMBER TO RE-READ from --strict's print on the union): outcomeReturns: 175, — REC-203 (C-91 IDSPACE_CHECKS, 2026-09-25, branch land/worker/REC-203): MOVED 172 -> 175 from this guard's own --strict print on the item's tree over origin/main 964da679. Whose growth it absorbs: op=idmatch's ONE new family and its three refusals, each at its own DEC-49 REGION in store.mjs idMatch. regionLines is a property of the MERGED source, so CONDUCT re-reads it at integration. */
  /* D-547 side, kept as history: outcomeReturns 169 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-468 side, kept as history: outcomeReturns 169 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-214 side, kept as history: outcomeReturns 173 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-549 side, kept as history: outcomeReturns 169 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-512 side, kept as history: outcomeReturns 169 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-205 side, kept as history: outcomeReturns 166 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* c21-rec207-renumber side, kept as history: outcomeReturns 168 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-530 side, kept as history: outcomeReturns 169 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-520 side, kept as history: outcomeReturns 169 on its own branch; the c22-batch29 union's figure is re-read from --strict's print. */
  /* D-589 side, kept as history: outcomeReturns 166 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* MK-7 side, kept as history: outcomeReturns 175 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-563 side, kept as history: outcomeReturns 188 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-561 side, kept as history: outcomeReturns 195 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-578 side, kept as history: outcomeReturns 189 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-546 side, kept as history: outcomeReturns 190 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-615 side, kept as history: outcomeReturns 191 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-623 side, kept as history: outcomeReturns 187 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-629 side, kept as history: outcomeReturns 188 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* REC-215 side, kept as history: outcomeReturns 187 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-419 side, kept as history: outcomeReturns 191 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-689 side, kept as history: outcomeReturns 187 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  /* D-695 side, kept as history: outcomeReturns 188 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  /* D-628 side, kept as history: outcomeReturns 192 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  outcomeReturns: 220, /* CONDUCT #23 at the c23-batch30 union: 205 -> 220, READ from `node civicos-ui/check-refusal-codes.mjs --strict`'s print on the merged tree (HEAD 645f8c41); never a branch's figure, never added by hand. */ /* CONDUCT #22 at the c22-batch29 union: 186 -> 205, the union's figure READ from `node civicos-ui/check-refusal-codes.mjs --strict`'s print on the merged tree (`ratchet: outcomeReturns ... measured 205`). */ /* CONDUCT #21 at c21-batch28: 172 -> 186, the union's figure READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch27: 168 -> 172, the union's figure (D-463, D-513 and the batch) READ from --strict's print on the committed merge. */ /* CONDUCT #20 at c20-batch25: 167 -> 168, D-510's C-86.1 (the PROMOTED_TYPE_CHECKS family and its region) over the union, READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch25: 165 -> 167, D-472's C-48.8/C-48.9 and their two regions over the union, READ from --strict's print on the merged tree. */ /* REC-211 (IC-273, 2026-09-24, branch land/worker/REC-211): 163 -> 165, MOVED from this guard's own print on this item's REBASED, COMMITTED tree over `origin/main` @ 58293bf31 — never by adding to the number here. ZERO PRE-EXISTING SLACK, measured rather than assumed: that pristine `origin/main` was checked out into a scratch worktree OUTSIDE this one and run before any edit was compared, and printed exit 0 with EVERY ratchet key at slack 0, so the whole move is this item's. Whose growth it absorbs: the two new return-position outcomes, one per region. */ /* CONDUCT #20 (c20-batch14, 2026-09-24): 161 -> 163, RE-READ from this guard's own `--strict` print on the union of this branch with `origin/main` @ 454a02bc (c20-batch17 and c20-batch18). Whose growth it absorbs: two more outcomes read at a governed site, D-484's. Never either side's figure and never added by hand. */ /* D-484 (C-33.40 NO_BASIS, C-33.41 NO_CITATION consolidated behind one governed helper each, 2026-09-24, branch land/worker/D-484): MOVED 152 -> 154 from this guard's own --strict print on the item's tree over origin/main 16fe1e7f. ZERO pre-existing slack: the PRISTINE origin/main 16fe1e7f, run before any edit in this worktree, printed exit 0 with EVERY ratchet key at slack 0. the two helpers' single return each. */ /* CONDUCT #20 (c20-integ1, 2026-09-24): 154 -> 161, RE-READ from this guard's own --strict print on the committed union (D-64 renumbered C-82 -> C-83, REC-184, and origin/main @ 548eb2c5 merged over c19-batch10) — never added by hand, and never either side's figure. Whose growth it absorbs: +7 D-64's return-position outcomes; IC-246 adds none. */ /* D-64 (C-83 RENDER_CAPTURE_CHECKS, 2026-09-24, cloud session WORKER D-64 (CONDUCT #19), branch land/worker/D-64): MOVED 141 -> 148 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: the seven refusal returns in the two regions. ZERO of the move is pre-existing slack: every other key gated at slack 0 on the same run, and the move is exactly this item's own. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): REC-189 (C-32.19 MACHINE_CANNOT_SET_RISK_TIER, minted C-32.18) merged over the batch; 153 -> 154 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): D-461 (C-78.2 NAMESPACE_PINNED) merged over the batch; 152 -> 153 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* D-461 (C-78.2 NAMESPACE_PINNED, 2026-09-24, cloud session WORKER D-461 (CONDUCT #19)): MOVED 141 -> 142 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: one new row in the existing NAMESPACE_CHECKS family and one new governed REGION (pinnedNamespaceGate > is-pinned-namespace-gate). ZERO of the move is pre-existing slack: the same guard on 15b2a4c0 gated every key at slack 0. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-126 (C-75 and C-76, the per-item weight) merged over the batch; 149 -> 151 RE-READ from this guard's --strict print on the committed merge 6ab139df. The new members are D-126 (C-75 and C-76, the per-item weight)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-149 (C-70, discoverable or hidden) merged over the batch; 148 -> 149 RE-READ from this guard's --strict print on the committed merge 16400d39. The new members are REC-149 (C-70, discoverable or hidden)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-149 (C-73 and C-32.18, the governing laws) merged over the batch; 141 -> 148 RE-READ from this guard's --strict print on the committed merge 8086d64f. The new members are D-149 (C-73 and C-32.18, the governing laws)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-164 merged over c17-batch6; 140 -> 141 RE-READ from this guard's --strict print on the committed merge 84443ad4. The new members are REC-164's (C-64.4..C-64.7, the group's name and domain acts); neither side's figure is the sum. */ /* D-278 (C-68 + C-69, 2026-09-23, branch land/worker/D-278): MOVED 135 -> 140 from this guard's own print on the item's tree, every other ratchet key gated. five new return-position refusals at the three spans. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own. */ /* D-168 (C-33.39 RETIRED_NOT_CITABLE in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-ab0b1ebe531cb428b): MOVED 132 -> 133 from this guard's own print on the item's tree: the one new return-position refusal in is-cite-retired. ZERO pre-existing slack — the pristine HEAD (0e7cc03e) sources, swapped in and restored by sha256 AND cmp, printed 132, every key gated.  — CONDUCT #16 merged it onto REC-175/REC-176, which moved this key from the same base; the figure is re-read from the merged print. */ /* REC-176 (C-67.1 SNAP_KEY_TAKEN in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-af684c758de00aae7): MOVED 132 -> 133 from this guard's own print on the item's tree: is-promote-snapkey's one return. ZERO pre-existing slack — every other key gated at 0 on the same run, and the move is exactly this item's own, read off the per-site line.  — CONDUCT #16 merged it with REC-175, which moved this key from the same base; the figure is re-read from the merged print. */ /* REC-175 (C-33.38 FILE_DIGEST_MISMATCH in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-a6f377d2e96aa24af): MOVED 132 -> 133 from this guard's own print on the item's tree over origin/main 14faa089, every other ratchet key gated at slack 0 on the same run. The one return-position outcome in is-promote-digest. ZERO of the move is pre-existing slack: the baseline run on 14faa089 printed exactly the old floor, so the move is this item's own. */ /* REC-169 (C-22.13/C-22.14 in AI_RUN_CHECKS, 2026-09-23, worktree agent-a30f15e61a2d24169): MOVED 131 -> 132 from this guard's own print on the item's tree: aiRunOpen's new seed refusal return. The move is exactly this item's own additions, read off the per-site line; every other key gated on the same run. */ /* D-85 (C-66 SURFACE_CHECKS, 2026-09-23, worktree agent-a09e49cc337fc2200): MOVED 130 -> 131 from this guard's own print on the item's tree, every other ratchet key gated. The new span's relayed AI_RUN_NOT_PRINCIPAL return (the gate's code, field by field, never a spread). ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */   /* REC-168 (2026-09-23, worktree agent-a960b19c9e4696303): MOVED 129 -> 130 from this guard's own print on the item's tree; the two new returns in is-capture-request moved it by one, and WHICH of them the walk counts as the new outcome was not traced here (stated, not guessed). Slack 0 on main (M0-79). */ /* REC-165 (2026-09-22, worktree agent-a6e92d28647553f52): MOVED 128 -> 129 from this guard's own print on the item's tree; the three new returns in is-suggest-shape moved it by one, and WHICH of them the walk counts as the new outcome was not traced here (stated, not guessed). Slack 0 on main (M0-79). */ /* D-436 AT INTEGRATION (CONDUCT #11, 2026-09-21, merged onto origin/main 8d2ba50f): MOVED 127 -> 128 from this guard's own print on the MERGED tree, where M0-79's slack gate FAILED naming it (floor 127, measured 128). The branch's base (ce830340) floored this figure at 98 with slack, so the branch never moved it; M0-79 moved main's to 127 at slack 0 (45064d8f, and nothing but docs/ changed from there to 8d2ba50f). So ZERO of the move is pre-existing slack and +1 is D-436's one new return-position outcome at a governed site. */
  ...
  /* CONDUCT #19 (c19-batch9, 2026-09-24): D-162 (the THEME family, renumbered C-74 -> C-81) merged over the batch; RE-READ from this guard's --strict print on the merged tree. */
  /* D-510 side, kept as history: refusalsJudged 427 on its own branch; the c20-batch25 union's figure is re-read from the print. */
  /* D-472 side, kept as history: refusalsJudged 428 on its own branch; the c20-batch25 union's figure is re-read from the print. */
  /* D-491 side, kept as history: refusalsJudged 427 on its own branch; the c20-batch25 union's figure is re-read from the print. */
  /* REC-211 side, kept as history: refusalsJudged 426 -> 428 on its own branch (IC-273); the c20-batch23 union's figure is re-read from --strict's print. */
  /* REC-186 side, kept as history (c22-batch29: ours' refusalsJudged: 482 kept; NUMBER TO RE-READ from --strict's print on the union): refusalsJudged: 441, — REC-186 (2026-09-25, branch land/worker/REC-186): 440 -> 441, READ from this guard's print on the item's tree over origin/main 8bdf20e6. Whose growth: C-33.48 LAST_OWNER_CANNOT_LEAVE and its region is-leave-owner-floor in projectLeave. */
  /* D-513 side, kept as history: refusalsJudged 439 on its own branch; the c20-batch27 union's figure is re-read from --strict's print. */
  /* D-463 side, kept as history: refusalsJudged 438 on its own branch; the c20-batch27 union's figure is re-read from the print. */
  /* D-147 side, kept as history (c22-batch29: ours' refusalsJudged: 482 kept; NUMBER TO RE-READ from --strict's print on the union): refusalsJudged: 457, — D-147 (C-94 LIFECYCLE_CHECKS, 2026-09-25, branch land/worker/D-147): 445 -> 457, READ from this guard's own print on the item's tree over origin/main 964da679 — the new family, its eleven rows and codes, and its three regions (is-lifecycle-writable, is-lifecycle-grammar, is-lifecycle-due-cite). Every other key gated at slack 0 on the same run. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads regionLines at integration. */
  /* REC-203 side, kept as history (c22-batch29: ours' refusalsJudged: 482 kept; NUMBER TO RE-READ from --strict's print on the union): refusalsJudged: 448, — REC-203 (C-91 IDSPACE_CHECKS, 2026-09-25, branch land/worker/REC-203): MOVED 445 -> 448 from this guard's own --strict print on the item's tree over origin/main 964da679. Whose growth it absorbs: op=idmatch's ONE new family and its three refusals, each at its own DEC-49 REGION in store.mjs idMatch. regionLines is a property of the MERGED source, so CONDUCT re-reads it at integration. */
  /* D-547 side, kept as history: refusalsJudged 441 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-217 side, kept as history: refusalsJudged 443 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-448 side, kept as history: refusalsJudged 451 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-468 side, kept as history: refusalsJudged 441 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-214 side, kept as history: refusalsJudged 445 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-549 side, kept as history: refusalsJudged 441 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-512 side, kept as history: refusalsJudged 441 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-205 side, kept as history: refusalsJudged 437 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* c21-rec207-renumber side, kept as history: refusalsJudged 446 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-530 side, kept as history: refusalsJudged 441 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-454 side, kept as history: refusalsJudged 442 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* REC-162 side, kept as history: refusalsJudged 441 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* REC-150 side, kept as history: refusalsJudged 454 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-520 side, kept as history: refusalsJudged 441 on its own branch; the c22-batch29 union's figure is re-read from --strict's print. */
  /* D-134 side, kept as history: refusalsJudged 454 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-589 side, kept as history: refusalsJudged 440 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* MK-7 side, kept as history: refusalsJudged 457 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* REC-147 side, kept as history: refusalsJudged 452 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* REC-197 side, kept as history: refusalsJudged 447 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-521b side, kept as history: refusalsJudged 481 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-563 side, kept as history: refusalsJudged 484 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-561 side, kept as history: refusalsJudged 491 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-374 side, kept as history: refusalsJudged 483 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* REC-204 side, kept as history: refusalsJudged 488 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-578 side, kept as history: refusalsJudged 485 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-546 side, kept as history: refusalsJudged 486 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-615 side, kept as history: refusalsJudged 487 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-623 side, kept as history: refusalsJudged 483 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-629 side, kept as history: refusalsJudged 484 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-680 side, kept as history: refusalsJudged 483 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* REC-215 side, kept as history: refusalsJudged 483 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-419 side, kept as history: refusalsJudged 487 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  /* D-689 side, kept as history: refusalsJudged 483 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  /* D-695 side, kept as history: refusalsJudged 484 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  /* D-670 side, kept as history: refusalsJudged 484 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  /* D-628 side, kept as history: refusalsJudged 488 on its own branch; the c23-batch30 union's figure is re-read from the print. */
  refusalsJudged: 573, /* CONDUCT #23 at the c23-batch30 union: 549 -> 573, READ from `node civicos-ui/check-refusal-codes.mjs --strict`'s print on the merged tree (HEAD 645f8c41); never a branch's figure, never added by hand. */ /* CONDUCT #22 at the c22-batch29 union: 482 -> 549, the union's figure READ from `node civicos-ui/check-refusal-codes.mjs --strict`'s print on the merged tree (`ratchet: refusalsJudged ... measured 549`). */ /* CONDUCT #21 at c21-batch28: 445 -> 482, the union's figure READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch27: 440 -> 445, the union's figure (D-463, D-513 and the batch) READ from --strict's print on the committed merge. */ /* CONDUCT #20 at c20-batch25: 439 -> 440, D-510's C-86.1 (the PROMOTED_TYPE_CHECKS family and its region) over the union, READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch25: 437 -> 439, D-472's C-48.8/C-48.9 and their two regions over the union, READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch25: 436 -> 437, D-491's C-28.16 (and its is-capture-request region) over the batch24c union, READ from --strict's print on the merged tree. */ /* CONDUCT #20 at c20-batch23: 434 -> 436, REC-211's arrivals over the c20-batch22 union, READ from this guard's --strict print on the merged tree. */ /* CONDUCT #20 at c20-batch22: the UNION of D-507 (426 -> 432) and D-508 (426 -> 428), READ from this guard's --strict print on the merged tree. */ /* D-507 (C-82.2..C-82.7, the six STATEMENT_ACK_* refusals that reached a member untranslated, 2026-09-24, branch land/worker/D-507): MOVED 426 -> 432 from this guard's own --strict print on the item's tree over origin/main 68fecb8d. Six return-position outcomes now graded inside a governed span; `outcomeReturns` does NOT move (163 -> 163), because the walk already read all six — what changed is that they are now judged. ZERO of the move is pre-existing slack: the PRISTINE origin/main 68fecb8d, run in this worktree before any edit, printed exit 0 with EVERY ratchet key at slack 0. */  /* CONDUCT #20 (c20-batch14, 2026-09-24): 424 -> 426, RE-READ from this guard's own `--strict` print on the union of this branch with `origin/main` @ 454a02bc (c20-batch17 and c20-batch18). Whose growth it absorbs: D-484's two, graded as refusals. Never either side's figure and never added by hand. */ /* D-484 (C-33.40 NO_BASIS, C-33.41 NO_CITATION consolidated behind one governed helper each, 2026-09-24, branch land/worker/D-484): MOVED 413 -> 415 from this guard's own --strict print on the item's tree over origin/main 16fe1e7f. ZERO pre-existing slack: the PRISTINE origin/main 16fe1e7f, run before any edit in this worktree, printed exit 0 with EVERY ratchet key at slack 0. each of those two returns is graded a refusal (`ok: false`). */ /* CONDUCT #20 (c20-batch14, 2026-09-24): 417 -> 424, RE-READ from this guard's own `--strict` print on the committed union of c20-batch11fix and c20-integ1b — never either branch's figure and never added by hand. Whose growth it absorbs: the SEVEN are D-64's C-83.1..7 graded as refusals. Both branches had re-done the same c19-batch10 + `main` union, so each branch's own figure was short of the union's. */ /* CONDUCT #20 (c20-integ1, 2026-09-24): 414 -> 422, RE-READ from this guard's own --strict print on the committed union (D-64 renumbered C-82 -> C-83, REC-184, and origin/main @ 548eb2c5 merged over c19-batch10) — never added by hand, and never either side's figure. Whose growth it absorbs: +7 D-64 and +1 IC-246. */ /* D-64 (C-83 RENDER_CAPTURE_CHECKS, 2026-09-24, cloud session WORKER D-64 (CONDUCT #19), branch land/worker/D-64): MOVED 365 -> 372 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: the seven refusals judged. ZERO of the move is pre-existing slack: every other key gated at slack 0 on the same run, and the move is exactly this item's own. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration. */ /* CONDUCT #19 (c19-batch11, 2026-09-24): REC-192 @ 2e102941 (C-71.8, C-71.9; IC-248; the branch read 367 on main) merged over the batch; RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* REC-192 (the version arm of op=partitionindependence, 2026-09-24, cloud session WORKER REC-192 (CONDUCT #19), branch land/worker/REC-192): MOVED 365 -> 367, the figure this guard PRINTED on the item's tree over origin/main 15b2a4c0: the two new refusals graded at is-partition-independence. ZERO of the move is pre-existing slack: the same guard over pristine origin/main printed exactly 365. Integrator: RE-READ from this guard's print on the committed merge. */ /* D-456 (C-78 NAMESPACE_CHECKS, 2026-09-23, cloud session WORKER D-456 (CONDUCT #18), branch land/worker/D-456): MOVED 364 -> 365 from this guard's own --strict print on the item's tree over origin/main a8f6094a: one new family, one row (NAMESPACE_UNKNOWN), one governed region (namespaceGate > is-namespace-gate, 5 lines), one code judged and compared. ZERO of the move is pre-existing slack: the same guard on a8f6094a before the change printed 364, every key gated at slack 0. */ /* CONDUCT #19 (c19-batch11, 2026-09-24): c19-unionfix @ 7b52ab9b (at 413) merged into c19-batch10 (at 414); RE-READ from this guard's --strict print on the merged tree, never added by hand. Both sides' comments carried below. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): REC-189 (C-32.19 MACHINE_CANNOT_SET_RISK_TIER, minted C-32.18) merged over the batch; 413 -> 414 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): D-461 (C-78.2 NAMESPACE_PINNED) merged over the batch; 412 -> 413 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* D-461 (C-78.2 NAMESPACE_PINNED, 2026-09-24, cloud session WORKER D-461 (CONDUCT #19)): MOVED 365 -> 366 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: one new row in the existing NAMESPACE_CHECKS family and one new governed REGION (pinnedNamespaceGate > is-pinned-namespace-gate). ZERO of the move is pre-existing slack: the same guard on 15b2a4c0 gated every key at slack 0. */ /* c19-unionfix (c19-batch9, 2026-09-24): 412 -> 413, re-read from this guard's --strict print on the union with IC-246: its ONE new family STATEMENT_ACK_CHECKS (C-82), ONE row and code STATEMENT_ACK_DOCUMENTS_OVER_BOUND, ONE region `is-statement-ack-documents-bound` in acknowledgeStatement. */ /* CONDUCT #18 (c18-batch8, 2026-09-23): D-394 merged over the batch; 397 -> 400 RE-READ from this guard's --strict print on the committed merge 8477cdb5. The new members are D-394's; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-126 (C-75 and C-76, the per-item weight) merged over the batch; 391 -> 397 RE-READ from this guard's --strict print on the committed merge 6ab139df. The new members are D-126 (C-75 and C-76, the per-item weight)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-149 (C-70, discoverable or hidden) merged over the batch; 386 -> 391 RE-READ from this guard's --strict print on the committed merge f4be4c07. The new members are REC-149 (C-70, discoverable or hidden)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-122 (C-74, the on-point choice) merged over the batch; 383 -> 386 RE-READ from this guard's --strict print on the committed merge e1d34ee0. The new members are REC-122 (C-74, the on-point choice)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-149 (C-73 and C-32.18, the governing laws) merged over the batch; 376 -> 383 RE-READ from this guard's --strict print on the committed merge 8086d64f. The new members are D-149 (C-73 and C-32.18, the governing laws)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-148 (C-72, the fee quote) merged over the batch; 368 -> 376 RE-READ from this guard's --strict print on the committed merge c81aec68. The new members are D-148 (C-72, the fee quote)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-164 merged over c17-batch6; 364 -> 368 RE-READ from this guard's --strict print on the committed merge 84443ad4. The new members are REC-164's (C-64.4..C-64.7, the group's name and domain acts); neither side's figure is the sum. */ /* CONDUCT #17 (c17-batch5, 2026-09-23): REC-161 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-161 (C-71 PARTITION_INDEPENDENCE_CHECKS, 2026-09-23, cloud session WORKER REC-161 (CONDUCT #17)): MOVED 349 -> 358 from this guard's own print on the item's tree over origin/main 02603e88: the nine refusal sites in the new region. ZERO of the move is pre-existing slack: the same guard over a pristine 02603e88 worktree printed 349, every key gated at slack 0. */ /* CONDUCT #17 (c17-batch4, 2026-09-23): D-179 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-179 (C-53.13 CAPTURE_HELD_BY_ANOTHER_BUNDLE, 2026-09-23, branch land/worker/D-179): MOVED 345 -> 346 from this guard's own print on the item's tree: the one refusal returned at is-register-home. ZERO pre-existing slack — every other key gated at 0 on the same run, and the move is exactly this item's own. */ /* CONDUCT #17 (c17-batch3, 2026-09-23): D-278 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-278 (C-68 + C-69, 2026-09-23, branch land/worker/D-278): MOVED 345 -> 350 from this guard's own print on the item's tree, every other ratchet key gated. the same five refusals judged. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): origin/main (c16-batch7) merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-168 (C-33.39 RETIRED_NOT_CITABLE in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-ab0b1ebe531cb428b): MOVED 342 -> 343 from this guard's own print on the item's tree: the one new refusal judged in is-cite-retired. ZERO pre-existing slack — the pristine HEAD (0e7cc03e) sources, swapped in and restored by sha256 AND cmp, printed 342, every key gated.  — CONDUCT #16 merged it onto REC-175/REC-176, which moved this key from the same base; the figure is re-read from the merged print. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): REC-177 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-177 (C-22.16 AI_RUN_BOUND_NO_ALLOWANCE in AI_RUN_CHECKS, 2026-09-23, worktree agent-ad7020ce0605889f4), RE-READ ON THE MERGE onto origin/main 0e5f7054 (which carries REC-175's and D-168's moves of this key from the same base): MOVED 343 -> 344 from this guard's own print on the merged tree: the same new refusal at checkConsume (7 -> 8 judged). ZERO of the move is pre-existing slack: every other key gated at slack 0 on the same run. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): REC-179 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-179 (C-66.5 SURFACED_BY_REWRITTEN in SURFACE_CHECKS, one new region promote > is-promote-surfaced-by, 2026-09-23, worktree agent-a368de25bd347b32f): MOVED 343 -> 344 from this guard's own print on the item's tree MERGED with origin/main 0e5f7054 (REC-175 landed first and moved this key from the same base, so this figure is re-read, never added), every other ratchet key gated. ZERO of the move is pre-existing slack: the merged tree without this item printed 343. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): D-440 and D-420 each moved this key by their own row from 0e5f7054; the union's value is RE-READ from this guard's print on the committed merge. */ /* D-420 (C-45.12 CONTENT_EXTENT_NO_IMAGE_PAINTED in CONTENT_EXTENT_CHECKS, 2026-09-23, cloud session WORKER D-420 (CONDUCT #16)): MOVED 343 -> 344 from this guard's own print on the item's tree over origin/main 0e5f7054, every other ratchet key gated at slack 0 on the same run. That outcome, graded a refusal (32 -> 33 judged at is-content-extent). ZERO of the move is pre-existing slack: the baseline run on 0e5f7054 printed exactly the old floor, so the move is this item's own. D-440 moves this key from the same base; CONDUCT re-reads the figure from the merged print. */ /* D-440 (C-45.11, 2026-09-23, worktree agent-aa6d775965f554582), RE-READ ON ITS MERGE with origin/main 0e5f7054: MOVED 343 -> 344 from this guard's own print on the merged tree — the one new refusal. ZERO pre-existing slack. */ /* REC-175 (C-33.38 FILE_DIGEST_MISMATCH in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-a6f377d2e96aa24af): MOVED 337 -> 338 from this guard's own print on the item's tree over origin/main 14faa089, every other ratchet key gated at slack 0 on the same run. That outcome, graded a refusal. ZERO of the move is pre-existing slack: the baseline run on 14faa089 printed exactly the old floor, so the move is this item's own.  — CONDUCT #16 merged it with REC-172, which moved this key from the same base; the figure is re-read from the merged print. */ /* REC-172 (2026-09-23, worktree agent-ad79857cb007927b7): MOVED 336 -> 341 from this guard's own print: the same five refusals at checkConsume (2 -> 7 judged), pristine HEAD 91913d6b printing 336. ZERO pre-existing slack.  — CONDUCT #16 merged it with UI-81, which moved this key from the same base; the figure is re-read from the merged print. */ /* UI-81 (C-44.2 FINDING_IN_SEVERAL_CASES in CASE_DERIVATION_CHECKS, 2026-09-23, worktree agent-a306ff29faf082ef3), RE-READ ON THE MERGE onto origin/main 91913d6b (which carries REC-169's moves): MOVED 336 -> 337 from this guard's own print on the merged tree, every other ratchet key gated. 1 refusal judged at the new span, as printed. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* REC-169 (C-22.13/C-22.14 in AI_RUN_CHECKS, 2026-09-23, worktree agent-a30f15e61a2d24169): MOVED 333 -> 336 from this guard's own print on the item's tree: checkConsume's 2 refusals and aiRunOpen's relayed seed refusal (judged, a variable code, not compared). The move is exactly this item's own additions, read off the per-site line; every other key gated on the same run. */ /* D-85 (C-66 SURFACE_CHECKS, 2026-09-23, worktree agent-a09e49cc337fc2200): MOVED 328 -> 333 from this guard's own print on the item's tree, every other ratchet key gated. 5 refusals judged at the new span, as printed: its four codes and the relayed C-22.12. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* REC-168 (2026-09-23, worktree agent-a960b19c9e4696303): MOVED 326 -> 328 from this guard's own print: is-capture-request judges 9 where it judged 7 — the status refusal and the principal relay (stated `ok: false`, never spread, so its verdict is readable). Slack 0 on main (M0-79). */ /* REC-165 (2026-09-22, worktree agent-a6e92d28647553f52): MOVED 322 -> 325 from this guard's own print: is-suggest-shape judges 12 where it judged 9 — the two new refusals and the principal relay (stated `ok: false`, never spread, so its verdict is readable). Slack 0 on main (M0-79). */ /* D-436 (C-64 INSTANCE_GROUP_CHECKS, 2026-09-21, worktree agent-a6dd0a0a3a3a6cc10): MOVED 319 -> 322 from this guard's own GREEN print on the item's committed tree 2f09b7b6. 1 + 2 refusals judged at the two spans, as printed. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* D-158 (C-63 SIGNER_ENROLMENT_CHECKS, 2026-09-20, worktree d158-conduct8): MOVED 316 -> 319 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE origin/main sources (e1aa2eee) with this item's three plane files (store.mjs, setup.mjs, bio-checks.mjs) swapped out and restored by sha256 AND cmp: pristine printed 317, so 1 of the move is PRE-EXISTING SLACK and +2 is this item's (the same two, on codesChecked's arithmetic). */    /* D-270 (C-38.7/C-38.8 + C-61 REQUIRED_ARGUMENT_CHECKS, 2026-09-19, worktree d270-record): MOVED 220 -> 316 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE origin/main sources (98209ce2) with this item's two plane files swapped out and restored by sha256 AND cmp: pristine printed 313, so 93 of the move is PRE-EXISTING SLACK and +3 is this item's (+3 net, on codesChecked's arithmetic). */   /* REC-86 (IC-123, C-50 NARROW_CHECKS, 2026-09-18, worktree agent-a3fbd59a3fef1a961): MOVED 203 -> 220 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE HEAD sources (92f4c64e) with the item's four plane files swapped out and restored by sha256 + cmp: pristine printed 206, so 3 of the move is PRE-EXISTING slack and +14 is this item's. */ /* +3 by REC-69 (C-36.1..3 inside `is-airuns-context`), 2026-08-09, from the
  /* D-508 side, kept as history: */  /* D-508 (C-85 KNOCK_CHECKS, 2026-09-24, cloud session WORKER D-508 (CONDUCT #20), branch land/worker/D-508): MOVED 426 -> 428 from this guard's own --strict print on the item's tree over origin/main 68fecb8d0. Whose growth it absorbs: the two refusals graded inside knock > is-knock-rate. ZERO of the move is pre-existing slack: the PRISTINE origin/main 68fecb8d0, run in this worktree before any edit, printed exit 0 with EVERY ratchet key at slack 0. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads regionLines at integration — and D-507 moves the same catalogue in parallel. */ /* CONDUCT #20 (c20-batch14, 2026-09-24): 424 -> 426, RE-READ from this guard's own `--strict` print on the union of this branch with `origin/main` @ 454a02bc (c20-batch17 and c20-batch18). Whose growth it absorbs: D-484's two, graded as refusals. Never either side's figure and never added by hand. */ /* D-484 (C-33.40 NO_BASIS, C-33.41 NO_CITATION consolidated behind one governed helper each, 2026-09-24, branch land/worker/D-484): MOVED 413 -> 415 from this guard's own --strict print on the item's tree over origin/main 16fe1e7f. ZERO pre-existing slack: the PRISTINE origin/main 16fe1e7f, run before any edit in this worktree, printed exit 0 with EVERY ratchet key at slack 0. each of those two returns is graded a refusal (`ok: false`). */ /* CONDUCT #20 (c20-batch14, 2026-09-24): 417 -> 424, RE-READ from this guard's own `--strict` print on the committed union of c20-batch11fix and c20-integ1b — never either branch's figure and never added by hand. Whose growth it absorbs: the SEVEN are D-64's C-83.1..7 graded as refusals. Both branches had re-done the same c19-batch10 + `main` union, so each branch's own figure was short of the union's. */ /* CONDUCT #20 (c20-integ1, 2026-09-24): 414 -> 422, RE-READ from this guard's own --strict print on the committed union (D-64 renumbered C-82 -> C-83, REC-184, and origin/main @ 548eb2c5 merged over c19-batch10) — never added by hand, and never either side's figure. Whose growth it absorbs: +7 D-64 and +1 IC-246. */ /* D-64 (C-83 RENDER_CAPTURE_CHECKS, 2026-09-24, cloud session WORKER D-64 (CONDUCT #19), branch land/worker/D-64): MOVED 365 -> 372 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: the seven refusals judged. ZERO of the move is pre-existing slack: every other key gated at slack 0 on the same run, and the move is exactly this item's own. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads it at integration. */ /* CONDUCT #19 (c19-batch11, 2026-09-24): REC-192 @ 2e102941 (C-71.8, C-71.9; IC-248; the branch read 367 on main) merged over the batch; RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* REC-192 (the version arm of op=partitionindependence, 2026-09-24, cloud session WORKER REC-192 (CONDUCT #19), branch land/worker/REC-192): MOVED 365 -> 367, the figure this guard PRINTED on the item's tree over origin/main 15b2a4c0: the two new refusals graded at is-partition-independence. ZERO of the move is pre-existing slack: the same guard over pristine origin/main printed exactly 365. Integrator: RE-READ from this guard's print on the committed merge. */ /* D-456 (C-78 NAMESPACE_CHECKS, 2026-09-23, cloud session WORKER D-456 (CONDUCT #18), branch land/worker/D-456): MOVED 364 -> 365 from this guard's own --strict print on the item's tree over origin/main a8f6094a: one new family, one row (NAMESPACE_UNKNOWN), one governed region (namespaceGate > is-namespace-gate, 5 lines), one code judged and compared. ZERO of the move is pre-existing slack: the same guard on a8f6094a before the change printed 364, every key gated at slack 0. */ /* CONDUCT #19 (c19-batch11, 2026-09-24): c19-unionfix @ 7b52ab9b (at 413) merged into c19-batch10 (at 414); RE-READ from this guard's --strict print on the merged tree, never added by hand. Both sides' comments carried below. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): REC-189 (C-32.19 MACHINE_CANNOT_SET_RISK_TIER, minted C-32.18) merged over the batch; 413 -> 414 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): D-461 (C-78.2 NAMESPACE_PINNED) merged over the batch; 412 -> 413 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* D-461 (C-78.2 NAMESPACE_PINNED, 2026-09-24, cloud session WORKER D-461 (CONDUCT #19)): MOVED 365 -> 366 from this guard's own --strict print on the item's tree over origin/main 15b2a4c0: one new row in the existing NAMESPACE_CHECKS family and one new governed REGION (pinnedNamespaceGate > is-pinned-namespace-gate). ZERO of the move is pre-existing slack: the same guard on 15b2a4c0 gated every key at slack 0. */ /* c19-unionfix (c19-batch9, 2026-09-24): 412 -> 413, re-read from this guard's --strict print on the union with IC-246: its ONE new family STATEMENT_ACK_CHECKS (C-82), ONE row and code STATEMENT_ACK_DOCUMENTS_OVER_BOUND, ONE region `is-statement-ack-documents-bound` in acknowledgeStatement. */ /* CONDUCT #18 (c18-batch8, 2026-09-23): D-394 merged over the batch; 397 -> 400 RE-READ from this guard's --strict print on the committed merge 8477cdb5. The new members are D-394's; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-126 (C-75 and C-76, the per-item weight) merged over the batch; 391 -> 397 RE-READ from this guard's --strict print on the committed merge 6ab139df. The new members are D-126 (C-75 and C-76, the per-item weight)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-149 (C-70, discoverable or hidden) merged over the batch; 386 -> 391 RE-READ from this guard's --strict print on the committed merge f4be4c07. The new members are REC-149 (C-70, discoverable or hidden)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-122 (C-74, the on-point choice) merged over the batch; 383 -> 386 RE-READ from this guard's --strict print on the committed merge e1d34ee0. The new members are REC-122 (C-74, the on-point choice)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-149 (C-73 and C-32.18, the governing laws) merged over the batch; 376 -> 383 RE-READ from this guard's --strict print on the committed merge 8086d64f. The new members are D-149 (C-73 and C-32.18, the governing laws)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): D-148 (C-72, the fee quote) merged over the batch; 368 -> 376 RE-READ from this guard's --strict print on the committed merge c81aec68. The new members are D-148 (C-72, the fee quote)'s; neither side's figure is the sum. */ /* CONDUCT #18 (c17-batch7, 2026-09-23): REC-164 merged over c17-batch6; 364 -> 368 RE-READ from this guard's --strict print on the committed merge 84443ad4. The new members are REC-164's (C-64.4..C-64.7, the group's name and domain acts); neither side's figure is the sum. */ /* CONDUCT #17 (c17-batch5, 2026-09-23): REC-161 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-161 (C-71 PARTITION_INDEPENDENCE_CHECKS, 2026-09-23, cloud session WORKER REC-161 (CONDUCT #17)): MOVED 349 -> 358 from this guard's own print on the item's tree over origin/main 02603e88: the nine refusal sites in the new region. ZERO of the move is pre-existing slack: the same guard over a pristine 02603e88 worktree printed 349, every key gated at slack 0. */ /* CONDUCT #17 (c17-batch4, 2026-09-23): D-179 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-179 (C-53.13 CAPTURE_HELD_BY_ANOTHER_BUNDLE, 2026-09-23, branch land/worker/D-179): MOVED 345 -> 346 from this guard's own print on the item's tree: the one refusal returned at is-register-home. ZERO pre-existing slack — every other key gated at 0 on the same run, and the move is exactly this item's own. */ /* CONDUCT #17 (c17-batch3, 2026-09-23): D-278 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-278 (C-68 + C-69, 2026-09-23, branch land/worker/D-278): MOVED 345 -> 350 from this guard's own print on the item's tree, every other ratchet key gated. the same five refusals judged. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): origin/main (c16-batch7) merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* D-168 (C-33.39 RETIRED_NOT_CITABLE in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-ab0b1ebe531cb428b): MOVED 342 -> 343 from this guard's own print on the item's tree: the one new refusal judged in is-cite-retired. ZERO pre-existing slack — the pristine HEAD (0e7cc03e) sources, swapped in and restored by sha256 AND cmp, printed 342, every key gated.  — CONDUCT #16 merged it onto REC-175/REC-176, which moved this key from the same base; the figure is re-read from the merged print. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): REC-177 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-177 (C-22.16 AI_RUN_BOUND_NO_ALLOWANCE in AI_RUN_CHECKS, 2026-09-23, worktree agent-ad7020ce0605889f4), RE-READ ON THE MERGE onto origin/main 0e5f7054 (which carries REC-175's and D-168's moves of this key from the same base): MOVED 343 -> 344 from this guard's own print on the merged tree: the same new refusal at checkConsume (7 -> 8 judged). ZERO of the move is pre-existing slack: every other key gated at slack 0 on the same run. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): REC-179 merged over the batch; value RE-READ from this guard's print on the committed merge. */ /* REC-179 (C-66.5 SURFACED_BY_REWRITTEN in SURFACE_CHECKS, one new region promote > is-promote-surfaced-by, 2026-09-23, worktree agent-a368de25bd347b32f): MOVED 343 -> 344 from this guard's own print on the item's tree MERGED with origin/main 0e5f7054 (REC-175 landed first and moved this key from the same base, so this figure is re-read, never added), every other ratchet key gated. ZERO of the move is pre-existing slack: the merged tree without this item printed 343. */ /* CONDUCT #17 (c17-batch1, 2026-09-23): D-440 and D-420 each moved this key by their own row from 0e5f7054; the union's value is RE-READ from this guard's print on the committed merge. */ /* D-420 (C-45.12 CONTENT_EXTENT_NO_IMAGE_PAINTED in CONTENT_EXTENT_CHECKS, 2026-09-23, cloud session WORKER D-420 (CONDUCT #16)): MOVED 343 -> 344 from this guard's own print on the item's tree over origin/main 0e5f7054, every other ratchet key gated at slack 0 on the same run. That outcome, graded a refusal (32 -> 33 judged at is-content-extent). ZERO of the move is pre-existing slack: the baseline run on 0e5f7054 printed exactly the old floor, so the move is this item's own. D-440 moves this key from the same base; CONDUCT re-reads the figure from the merged print. */ /* D-440 (C-45.11, 2026-09-23, worktree agent-aa6d775965f554582), RE-READ ON ITS MERGE with origin/main 0e5f7054: MOVED 343 -> 344 from this guard's own print on the merged tree — the one new refusal. ZERO pre-existing slack. */ /* REC-175 (C-33.38 FILE_DIGEST_MISMATCH in ACT_SHAPE_CHECKS, 2026-09-23, worktree agent-a6f377d2e96aa24af): MOVED 337 -> 338 from this guard's own print on the item's tree over origin/main 14faa089, every other ratchet key gated at slack 0 on the same run. That outcome, graded a refusal. ZERO of the move is pre-existing slack: the baseline run on 14faa089 printed exactly the old floor, so the move is this item's own.  — CONDUCT #16 merged it with REC-172, which moved this key from the same base; the figure is re-read from the merged print. */ /* REC-172 (2026-09-23, worktree agent-ad79857cb007927b7): MOVED 336 -> 341 from this guard's own print: the same five refusals at checkConsume (2 -> 7 judged), pristine HEAD 91913d6b printing 336. ZERO pre-existing slack.  — CONDUCT #16 merged it with UI-81, which moved this key from the same base; the figure is re-read from the merged print. */ /* UI-81 (C-44.2 FINDING_IN_SEVERAL_CASES in CASE_DERIVATION_CHECKS, 2026-09-23, worktree agent-a306ff29faf082ef3), RE-READ ON THE MERGE onto origin/main 91913d6b (which carries REC-169's moves): MOVED 336 -> 337 from this guard's own print on the merged tree, every other ratchet key gated. 1 refusal judged at the new span, as printed. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* REC-169 (C-22.13/C-22.14 in AI_RUN_CHECKS, 2026-09-23, worktree agent-a30f15e61a2d24169): MOVED 333 -> 336 from this guard's own print on the item's tree: checkConsume's 2 refusals and aiRunOpen's relayed seed refusal (judged, a variable code, not compared). The move is exactly this item's own additions, read off the per-site line; every other key gated on the same run. */ /* D-85 (C-66 SURFACE_CHECKS, 2026-09-23, worktree agent-a09e49cc337fc2200): MOVED 328 -> 333 from this guard's own print on the item's tree, every other ratchet key gated. 5 refusals judged at the new span, as printed: its four codes and the relayed C-22.12. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* REC-168 (2026-09-23, worktree agent-a960b19c9e4696303): MOVED 326 -> 328 from this guard's own print: is-capture-request judges 9 where it judged 7 — the status refusal and the principal relay (stated `ok: false`, never spread, so its verdict is readable). Slack 0 on main (M0-79). */ /* REC-165 (2026-09-22, worktree agent-a6e92d28647553f52): MOVED 322 -> 325 from this guard's own print: is-suggest-shape judges 12 where it judged 9 — the two new refusals and the principal relay (stated `ok: false`, never spread, so its verdict is readable). Slack 0 on main (M0-79). */ /* D-436 (C-64 INSTANCE_GROUP_CHECKS, 2026-09-21, worktree agent-a6dd0a0a3a3a6cc10): MOVED 319 -> 322 from this guard's own GREEN print on the item's committed tree 2f09b7b6. 1 + 2 refusals judged at the two spans, as printed. ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ /* D-158 (C-63 SIGNER_ENROLMENT_CHECKS, 2026-09-20, worktree d158-conduct8): MOVED 316 -> 319 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE origin/main sources (e1aa2eee) with this item's three plane files (store.mjs, setup.mjs, bio-checks.mjs) swapped out and restored by sha256 AND cmp: pristine printed 317, so 1 of the move is PRE-EXISTING SLACK and +2 is this item's (the same two, on codesChecked's arithmetic). */    /* D-270 (C-38.7/C-38.8 + C-61 REQUIRED_ARGUMENT_CHECKS, 2026-09-19, worktree d270-record): MOVED 220 -> 316 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE origin/main sources (98209ce2) with this item's two plane files swapped out and restored by sha256 AND cmp: pristine printed 313, so 93 of the move is PRE-EXISTING SLACK and +3 is this item's (+3 net, on codesChecked's arithmetic). */   /* REC-86 (IC-123, C-50 NARROW_CHECKS, 2026-09-18, worktree agent-a3fbd59a3fef1a961): MOVED 203 -> 220 from this guard's own GREEN print on the item's tree. Split, measured by running this guard over the PRISTINE HEAD sources (92f4c64e) with the item's four plane files swapped out and restored by sha256 + cmp: pristine printed 206, so 3 of the move is PRE-EXISTING slack and +14 is this item's. */ /* +3 by REC-69 (C-36.1..3 inside `is-airuns-context`), 2026-08-09, from the
  ...
  vocabularyTerms: 118, /* CONDUCT #23 at the c23-batch30 union: 113 -> 118, READ from `node civicos-ui/check-refusal-codes.mjs --strict`'s print on the merged tree (HEAD 645f8c41); never a branch's figure, never added by hand. */ /* CONDUCT #22 at the c22-batch29 union: 112 -> 113, the union's figure READ from `node civicos-ui/check-refusal-codes.mjs --strict`'s print on the merged tree (`ratchet: vocabularyTerms ... measured 113`). */ /* D-516 (2026-09-24, worktree /home/user/bio on land/worker/D-516, cloud): MOVED 110 -> 112 from
     ...
     to `MEANING_MISSING_ROW_CAUSES` in `airun.mjs`, one term per level, because BOB #33 ruled (2026-09-24 17:58Z) that
     inside the stored watermark's one-second band the observation-log readers STATE undetermined rather than picking a
     cause they cannot tell. ZERO of the move is pre-existing slack: the run that printed 112 is the same run that printed
     every other key at slack 0, and the old floor plus this item's two is exactly 112. */ /* D-125 (2026-09-23): FALLS 111 -> 110 from this guard's own print, AND THE FALL IS NOT SLACK: queuestate.mjs `MUTE_REFUSAL_DETAIL` lost its FINDING term because BOB #26 RULED (2026-09-22, NOTIFICATIONS.md "MARKED AS HANDLED") that a member may mute a FINDING for themselves, so no member can receive that sentence any more; the vocabulary still stands with its OBLIGATION term. */ /* D-85 (C-66 SURFACE_CHECKS, 2026-09-23, worktree agent-a09e49cc337fc2200): MOVED 110 -> 111 from this guard's own print on the item's tree, every other ratchet key gated. RUN_BOUNDS gained `surfaces` (§11 item 5, rule 2). ZERO of the move is pre-existing slack: the printed figure is exactly the old floor plus this item's own, read off the guard's per-site line. */ // + REC-69's TWO `RUN_CONTEXTS` terms (inquiry, project). M0-79: 64 -> 110, see `vocabularies`.
  ...
  /* D-549 side, kept as history: untranslated 291 on its own branch; the c21-batch28 union's figure is re-read from the print. */
  /* D-134 side, kept as history: untranslated 284 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-561 side, kept as history: untranslated 274 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-623 side, kept as history: untranslated 279 on its own branch; the c22-batch30 union's figure is re-read from the print. */
  untranslated: 265, /* CONDUCT #23 at the c23-batch30 union: 266 -> 265, READ from `node civicos-ui/check-refusal-codes.mjs --strict`'s print on the merged tree (HEAD 645f8c41); never a branch's figure, never added by hand. FELL by one: a closure, not slack. */ /* CONDUCT #22 at the c22-batch29 union: 280 -> 266, the union's figure READ from `node civicos-ui/check-refusal-codes.mjs --strict`'s print on the merged tree (`ratchet: untranslated ... measured 266`). It FELL: the batch's rows translated more codes than its landings minted bare (arm F: 266 to partition). */ /* CONDUCT #21 at c21-batch28: 281 -> 280, the union's figure READ from --strict's print on the merged tree. */ /* D-448 (C-87 REVIEW_COPY_CHECKS, 2026-09-24, cloud session WORKER D-448 (SCHEDULER #19), branch land/worker/D-448): MOVED 292 -> 281 from this guard's own --strict print on the item's tree over origin/main 9f8b69e6. Whose growth it absorbs: a FLOOR THAT FALLS, and its reason at the site: it falls by exactly the eleven codes this item TRANSLATED, which leave arm F's partition (F6 166 -> 156, F4 100 -> 99 for NO_REVIEW_COPY). That is the sweep closing eleven codes, not the walk losing sight of them: arm F's partition still sums exactly and names all eleven as gone. ZERO of the move is pre-existing slack — the PRISTINE origin/main 9f8b69e6, run in this worktree BEFORE any edit, printed exit 0 with EVERY ratchet key at slack 0 (log kept as the item's baseline). A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads regionLines at integration. */ /* CONDUCT #20 at c20-batch22: the UNION of D-507 (300 -> 294) and D-508 (300 -> 298), READ from this guard's --strict print on the merged tree. */ /* D-507 (C-82.2..C-82.7, 2026-09-24, branch land/worker/D-507): FELL 300 -> 294 from this guard's own --strict print on the item's tree over origin/main 68fecb8d. **THIS FLOOR FALLING IS THE ITEM'S PAYLOAD AND NOT SLACK, and the reason is written here because a falling floor read without one is indistinguishable from a walk that went blind.** The six STATEMENT_ACK_* codes each gained a canned translation, so arm F's F6 partition — out of reach, one site — falls 172 -> 166 by exactly those six and EVERY other partition is unchanged (F1=2 F2=1 F3=18 F4=102 F5=5, summing with F6=166 to 294). The census did NOT move (694 -> 694): the six codes already existed and were already minted at one site each; what they lacked was a row. ZERO pre-existing slack: the PRISTINE origin/main 68fecb8d, run in this worktree before any edit, printed exit 0 with every ratchet key at slack 0 and untranslated measured at exactly 300. */  /* D-484 (C-33.40 NO_BASIS, C-33.41 NO_CITATION consolidated behind one governed helper each, 2026-09-24, branch land/worker/D-484): FELL 301 -> 299 from this guard's own --strict print on the item's tree over origin/main 16fe1e7f. ZERO pre-existing slack: the PRISTINE origin/main 16fe1e7f, run before any edit in this worktree, printed exit 0 with EVERY ratchet key at slack 0.
  /* D-508 side, kept as history: */  /* D-508 (C-85 KNOCK_CHECKS, 2026-09-24, cloud session WORKER D-508 (CONDUCT #20), branch land/worker/D-508): MOVED 300 -> 298 from this guard's own --strict print on the item's tree over origin/main 68fecb8d0. Whose growth it absorbs: FALLS by exactly the two codes D-508 translated, RATE_IP and RATE_GLOBAL, which leave arm F's F4 MULTI-SITE partition (102 -> 100). A floor that FALLS needs its reason at the site: this one falls because the sweep CLOSED two codes, not because the walk lost sight of them — arm F's own partition sums to 298 and names both as gone. ZERO of the move is pre-existing slack: the PRISTINE origin/main 68fecb8d0, run in this worktree before any edit, printed exit 0 with EVERY ratchet key at slack 0. A REGION'S LINE COUNT IS A PROPERTY OF THE MERGED SOURCE, so CONDUCT re-reads regionLines at integration — and D-507 moves the same catalogue in parallel. */ /* D-484 (C-33.40 NO_BASIS, C-33.41 NO_CITATION consolidated behind one governed helper each, 2026-09-24, branch land/worker/D-484): FELL 301 -> 299 from this guard's own --strict print on the item's tree over origin/main 16fe1e7f. ZERO pre-existing slack: the PRISTINE origin/main 16fe1e7f, run before any edit in this worktree, printed exit 0 with EVERY ratchet key at slack 0.
  ...
  /* D-485 (arm H, 2026-09-25, branch land/worker/D-485): SET from this guard's own print on the item's tree over
     origin/main 964da679 — the arm is new, so there is no prior figure and no slack to split. `r4Suites` is arm H's
     CORPUS (29 suites construct Miniflare over the plane's source, directly or through meaning-arms.walks.mjs, AND drive
     app.html): an empty corpus passes a zero gate for free, which is why the corpus is floored and not only the yield.
     `r4Pane` is R4 itself: 12 census codes a real-plane suite asserts, positively, in a pane. */
  r4Suites: 47, /* CONDUCT #23 at the c23-batch30 union: 42 -> 47, READ from `node civicos-ui/check-refusal-codes.mjs --strict`'s print on the merged tree (HEAD 645f8c41); never a branch's figure, never added by hand. */ /* CONDUCT #22 at the c22-batch29 union: 29 -> 42, the union's figure READ from `node civicos-ui/check-refusal-codes.mjs --strict`'s print on the merged tree (`ratchet: r4Suites ... measured 42`). */
  r4Pane: 16, /* CONDUCT #23 at the c23-batch30 union: 15 -> 16, READ from `node civicos-ui/check-refusal-codes.mjs --strict`'s print on the merged tree (HEAD 645f8c41); never a branch's figure, never added by hand. */ /* CONDUCT #22 at the c22-batch29 union: 12 -> 15, the union's figure READ from `node civicos-ui/check-refusal-codes.mjs --strict`'s print on the merged tree (`ratchet: r4Pane ... measured 15`). */
  /* UI-75 side, kept as history: r3Fed 81 on its own branch. */ /* UI-75 (2026-09-25, branch land/worker/UI-75): MOVED 80 -> 81 from this guard's own print, a REAL WIDENING of the kind D-433 says should raise this floor. THE CODE IS PARTITION_INDEPENDENCE_UNREADABLE (REC-161's C-71): `civicos-ui/test/elicitation.test.mjs` §7a has the record REFUSE the read-back's `op=partitionindependence` read in that code, and asserts the page renders NOT TOLD rather than clean. ESTABLISHED BEFORE MOVING: the same guard, `--strict`, on a PRISTINE worktree of this item's base (origin/main 964da679, in the session scratchpad) printed `r3Fed measured 80 · slack 0 · gated` and exited 0, so the +1 is this landing's. */
  /* UI-91 side, kept as history: r3Fed 81 on its own branch. */ /* UI-91 (2026-09-25, branch land/worker/UI-91): MOVED 80 -> 81 from this guard's own print at the COMMIT that adds civicos-ui/test/onpoint-choice.test.mjs, a REAL WIDENING of the kind D-433 says should raise this floor: that suite FEEDS CONNECTION_CHOICE_NOT_A_MEMBER (C-74.1, REC-122's) to the document page's connection display through a real plane and asserts its canned translation rendered by `actRefusalHtml`. ESTABLISHED BEFORE MOVING: the same harness on this item's base (8bdf20e6, before the suite was committed) reported no r3Fed failure, i.e. 80 at slack 0, so the +1 is this landing's. */ /* CONDUCT #20 (c20-batch14, 2026-09-24): 75 -> 80, RE-READ from this guard's own `--strict` print on the union of this branch with `origin/main` @ 454a02bc (c20-batch17 and c20-batch18). Whose growth it absorbs: UI-90's FIVE — D-149's MACHINE_CANNOT_SET_LAWS, NO_LAWS, BAD_LAW_LEVEL, BAD_CITATION and TOO_MANY_LAWS, each now FED to a surface by `civicos-ui/test/action-page.test.mjs` §8, a real widening in the direction D-433 says should raise this floor. Never either side's figure and never added by hand. */ /* UI-90 (2026-09-24, branch land/worker/UI-90): MOVED 73 -> 78 from this guard's own print, a REAL WIDENING of exactly the kind D-433 says should raise this floor — FIVE more codes are handed to a surface, not five more recognised by the walk. THE CODES ARE D-149's, fed by `civicos-ui/test/action-page.test.mjs` §8, which drives `op=actionlaws` through the action page for the first time: MACHINE_CANNOT_SET_LAWS, NO_LAWS, BAD_LAW_LEVEL, BAD_CITATION and TOO_MANY_LAWS all reach the member through `actRefusalHtml`, and the BAD_CITATION arm renders one end to end. ESTABLISHED BEFORE MOVING, as the FAIL text beside this table demands: the same guard run on a PRISTINE checkout of this item's base (d536f834, a scratch worktree outside the tree) prints `r3Fed measured 73 · slack 0 · gated` and exits 0, so the +5 is this landing's and is not a floor that was already stale. `untranslated` did not move (301 -> 301) and no ceiling moved. PRIOR: UI-84 (2026-09-24, branch land/worker/UI-84): MOVED 72 -> 73 from this guard's own print, and it is a REAL WIDENING in the direction D-433 says should raise this floor — one more code is now handed to a surface, not one more recognised by the walk. THE CODE IS UNKNOWN_OP (C-69.1, D-278's dispatch miss): refusal-translation-surface.test.mjs gained an arm that builds that refusal in the shape D-278 sends it — the row's own `translation`, IMPORTED from DISPATCH_CHECKS and never retyped — and drives it through `actRefusalHtml`, so the canned sentence a member would read is asserted at a surface for the first time. ESTABLISHED BEFORE MOVING, as the FAIL text beside this table demands: the same guard run on a PRISTINE checkout of this item's base (548eb2c5, a scratch worktree) prints `r3Fed measured 72 · slack 0 · gated` and exits 0, so the +1 is this landing's and is not a floor that was already stale. `untranslated` did not move (301 -> 301) and no ceiling moved. PRIOR: D-126 (2026-09-23, branch land/worker/D-126) MOVED 71 -> 72 from this guard's own print at the COMMIT that adds civicos-ui/test/queue-peritem.test.mjs (D-257: the working tree read it before the file was committed) — that suite FEEDS NOT_YOURS into the queue surface through a real plane. */ /* CONDUCT #20 (c20-batch13, 2026-09-24): c20-batch12 merged over c20-batch11fix; 75 RE-READ from this guard's --strict print on the MERGED tree, never arithmetic on either side. TWO CHAINS MET HERE and NEITHER side's figure is the union's: this side stood at 74 (D-126, UI-86, D-291 and their batch merges, below) and c20-batch12 stood at 73 (D-126 and UI-84, below). Both chains are kept in full because each names a DIFFERENT code reaching a surface, and a chain dropped at a merge is how a floor stops being a measurement. */ /* UI-84 (2026-09-24, branch land/worker/UI-84): MOVED 72 -> 73 from this guard's own print ON ITS OWN BASE, and it is a REAL WIDENING in the direction D-433 says should raise this floor — one more code is now handed to a surface, not one more recognised by the walk. THE CODE IS UNKNOWN_OP (C-69.1, D-278's dispatch miss): refusal-translation-surface.test.mjs gained an arm that builds that refusal in the shape D-278 sends it — the row's own `translation`, IMPORTED from DISPATCH_CHECKS and never retyped — and drives it through `actRefusalHtml`, so the canned sentence a member would read is asserted at a surface for the first time. ESTABLISHED BEFORE MOVING, as the FAIL text beside this table demands: the same guard run on a PRISTINE checkout of that item's base (548eb2c5, a scratch worktree) prints `r3Fed measured 72 · slack 0 · gated` and exits 0, so the +1 is that landing's and is not a floor that was already stale. `untranslated` did not move (301 -> 301) and no ceiling moved. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): D-291 (resolve-set.test.mjs feeds NO_SHA; read on the COMMITTED merge d1283d0d9, D-257: r3Fed is gated on the files in the commit) merged over the batch; 73 -> 74 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* D-291 (2026-09-24, branch land/worker/D-291): MOVED 72 -> 73 from this guard's own print at the COMMIT that adds civicos-ui/test/resolve-set.test.mjs, which FEEDS NO_SHA (op=resolve's own refusal of the one doctored candidate its header states). ALL of that move is D-291's: the same guard at the merge commit caf59910 (main + D-126) printed 72. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): UI-86 (the queue's item mute feeds BAD_KIND; D-126 and UI-86 each moved 71 -> 72 from the same base) merged over the batch; 72 -> 73 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* D-126 (2026-09-23, branch land/worker/D-126): MOVED 71 -> 72 from this guard's own print at the COMMIT that adds civicos-ui/test/queue-peritem.test.mjs (D-257: the working tree read it before the file was committed) — the suite FEEDS NOT_YOURS into the queue surface through a real plane. */
  /* D-134 side, kept as history: r3Fed 89 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* M0-148 side, kept as history: r3Fed 77 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-561 side, kept as history: r3Fed 83 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* UI-112 side, kept as history: r3Fed 83 on its own branch; the c22-batch30 union's figure is re-read from the print. UI-112 (2026-09-25, branch land/worker/UI-112): MOVED 82 -> 83 from this guard's own --strict print at the COMMIT that adds civicos-ui/test/onpoint-occurrence.test.mjs, a REAL WIDENING: that suite FEEDS CONNECTION_CHOICE_OCCURRENCE_UNNAMED (C-74.4, D-454) to the on-point chooser through a real plane and asserts its canned translation (imported from CONNECTION_CHOICE_CHECKS) is what the member reads. ESTABLISHED BEFORE MOVING: the same guard on a PRISTINE worktree of this item's base (origin/main 5e8a65a8) prints `r3Fed measured 82 · slack 0 · gated`, so the +1 is this landing's. */
  /* UI-120 side, kept as history: r3Fed 87 on its own branch; the c22-batch30 union's figure is re-read from the print. UI-120 (2026-09-25, branch land/worker/UI-120): MOVED 82 -> 87 from this guard's own print at the COMMIT that adds civicos-ui/test/content-crop-surface.test.mjs, a REAL WIDENING of the kind D-433 says should raise this floor: that suite FEEDS all five of C-99's codes (CROP_NO_PDF_MEMBER, CROP_NOT_A_PAGE_IMAGE, CROP_NOT_DERIVABLE, CROP_MEMBER_SILENT, CROP_CAPTURE_MISMATCH; D-419's) to the inquiry page's crop control through a real plane, asserting each row's translation (imported from CONTENT_CROP_CHECKS) is what the member reads. ESTABLISHED BEFORE MOVING: the same guard `--strict` on a PRISTINE worktree of this item's base (land/worker/D-419 @ 914bb380) prints `r3Fed measured 82 · slack 0 · gated` and exits 0, so the +5 is this landing's. */
  r3Fed: 98, /* CONDUCT #23 at the c23-batch30 union: 91 -> 98, READ from `node civicos-ui/check-refusal-codes.mjs --strict`'s print on the merged tree (HEAD 645f8c41); never a branch's figure, never added by hand. */ /* CONDUCT #22 at the c22-batch29 union: 82 -> 91, the union's figure READ from `node civicos-ui/check-refusal-codes.mjs --strict`'s print on the merged tree (`ratchet: r3Fed ... measured 91`). ONE key for the batch: D-134's 89, M0-148's 77 and D-561's 83 were each their own branch's. */ /* CONDUCT #21 at c21-batch28: ONE key — UI-91 and UI-96 each moved 80 -> 81 for its own suite and both lines merged, a duplicate key; the union's 82 READ from --strict's print. */ /* UI-96 (2026-09-25, branch land/worker/UI-96): MOVED 80 -> 81 from this guard's own print at the COMMIT that adds civicos-ui/test/version-notice-surface.test.mjs, a REAL WIDENING of the kind D-433 says should raise this floor: that suite FEEDS VERSION_NOTICE_NO_CONTENT (C-80, D-394) to the cross-version notice surface through a real plane, asserting the row's own translation (imported from VERSION_NOTICE_CHECKS) is what the member reads. ESTABLISHED BEFORE MOVING: the same guard on a PRISTINE worktree of this item's base (origin/main 8bdf20e6) prints `r3Fed measured 80 · slack 0 · gated`, so the +1 is this landing's. */ /* CONDUCT #20 (c20-batch14, 2026-09-24): 75 -> 80, RE-READ from this guard's own `--strict` print on the union of this branch with `origin/main` @ 454a02bc (c20-batch17 and c20-batch18). Whose growth it absorbs: UI-90's FIVE — D-149's MACHINE_CANNOT_SET_LAWS, NO_LAWS, BAD_LAW_LEVEL, BAD_CITATION and TOO_MANY_LAWS, each now FED to a surface by `civicos-ui/test/action-page.test.mjs` §8, a real widening in the direction D-433 says should raise this floor. Never either side's figure and never added by hand. */ /* UI-90 (2026-09-24, branch land/worker/UI-90): MOVED 73 -> 78 from this guard's own print, a REAL WIDENING of exactly the kind D-433 says should raise this floor — FIVE more codes are handed to a surface, not five more recognised by the walk. THE CODES ARE D-149's, fed by `civicos-ui/test/action-page.test.mjs` §8, which drives `op=actionlaws` through the action page for the first time: MACHINE_CANNOT_SET_LAWS, NO_LAWS, BAD_LAW_LEVEL, BAD_CITATION and TOO_MANY_LAWS all reach the member through `actRefusalHtml`, and the BAD_CITATION arm renders one end to end. ESTABLISHED BEFORE MOVING, as the FAIL text beside this table demands: the same guard run on a PRISTINE checkout of this item's base (d536f834, a scratch worktree outside the tree) prints `r3Fed measured 73 · slack 0 · gated` and exits 0, so the +5 is this landing's and is not a floor that was already stale. `untranslated` did not move (301 -> 301) and no ceiling moved. PRIOR: UI-84 (2026-09-24, branch land/worker/UI-84): MOVED 72 -> 73 from this guard's own print, and it is a REAL WIDENING in the direction D-433 says should raise this floor — one more code is now handed to a surface, not one more recognised by the walk. THE CODE IS UNKNOWN_OP (C-69.1, D-278's dispatch miss): refusal-translation-surface.test.mjs gained an arm that builds that refusal in the shape D-278 sends it — the row's own `translation`, IMPORTED from DISPATCH_CHECKS and never retyped — and drives it through `actRefusalHtml`, so the canned sentence a member would read is asserted at a surface for the first time. ESTABLISHED BEFORE MOVING, as the FAIL text beside this table demands: the same guard run on a PRISTINE checkout of this item's base (548eb2c5, a scratch worktree) prints `r3Fed measured 72 · slack 0 · gated` and exits 0, so the +1 is this landing's and is not a floor that was already stale. `untranslated` did not move (301 -> 301) and no ceiling moved. PRIOR: D-126 (2026-09-23, branch land/worker/D-126) MOVED 71 -> 72 from this guard's own print at the COMMIT that adds civicos-ui/test/queue-peritem.test.mjs (D-257: the working tree read it before the file was committed) — that suite FEEDS NOT_YOURS into the queue surface through a real plane. */ /* CONDUCT #20 (c20-batch13, 2026-09-24): c20-batch12 merged over c20-batch11fix; 75 RE-READ from this guard's --strict print on the MERGED tree, never arithmetic on either side. TWO CHAINS MET HERE and NEITHER side's figure is the union's: this side stood at 74 (D-126, UI-86, D-291 and their batch merges, below) and c20-batch12 stood at 73 (D-126 and UI-84, below). Both chains are kept in full because each names a DIFFERENT code reaching a surface, and a chain dropped at a merge is how a floor stops being a measurement. */ /* UI-84 (2026-09-24, branch land/worker/UI-84): MOVED 72 -> 73 from this guard's own print ON ITS OWN BASE, and it is a REAL WIDENING in the direction D-433 says should raise this floor — one more code is now handed to a surface, not one more recognised by the walk. THE CODE IS UNKNOWN_OP (C-69.1, D-278's dispatch miss): refusal-translation-surface.test.mjs gained an arm that builds that refusal in the shape D-278 sends it — the row's own `translation`, IMPORTED from DISPATCH_CHECKS and never retyped — and drives it through `actRefusalHtml`, so the canned sentence a member would read is asserted at a surface for the first time. ESTABLISHED BEFORE MOVING, as the FAIL text beside this table demands: the same guard run on a PRISTINE checkout of that item's base (548eb2c5, a scratch worktree) prints `r3Fed measured 72 · slack 0 · gated` and exits 0, so the +1 is that landing's and is not a floor that was already stale. `untranslated` did not move (301 -> 301) and no ceiling moved. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): D-291 (resolve-set.test.mjs feeds NO_SHA; read on the COMMITTED merge d1283d0d9, D-257: r3Fed is gated on the files in the commit) merged over the batch; 73 -> 74 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* D-291 (2026-09-24, branch land/worker/D-291): MOVED 72 -> 73 from this guard's own print at the COMMIT that adds civicos-ui/test/resolve-set.test.mjs, which FEEDS NO_SHA (op=resolve's own refusal of the one doctored candidate its header states). ALL of that move is D-291's: the same guard at the merge commit caf59910 (main + D-126) printed 72. */ /* CONDUCT #19 (c19-batch10, 2026-09-24): UI-86 (the queue's item mute feeds BAD_KIND; D-126 and UI-86 each moved 71 -> 72 from the same base) merged over the batch; 72 -> 73 RE-READ from this guard's --strict print on the merged tree, never added by hand. */ /* D-126 (2026-09-23, branch land/worker/D-126): MOVED 71 -> 72 from this guard's own print at the COMMIT that adds civicos-ui/test/queue-peritem.test.mjs (D-257: the working tree read it before the file was committed) — the suite FEEDS NOT_YOURS into the queue surface through a real plane. */
  /* D-542 / D-562 (2026-09-25, branch land/worker/D-542): SET from this guard's own print on the item's tree over
     origin/main 5e8a65a8 — the rules are new, so there is no prior figure and no slack to split. */
  r5Op: 409, /* CONDUCT #23 at the c23-batch30 union: 335 -> 409, READ from `node civicos-ui/check-refusal-codes.mjs --strict`'s print on the merged tree (HEAD 645f8c41); never a branch's figure, never added by hand. D-542's arm (R5, by op) on the union's app.html and plane. */
  r6Public: 33,
};

/* M0-148: the codes R3 reaches ONLY by following a binding, PINNED BY NAME — the
   `arm B / M0-148` print is the only source this list may be moved to. */
const R3_FOLLOWED_PIN = ["REQUIRED_ARGUMENT_MISSING"];   /* M0-148: SET from the print on a23fe7ee + this item */
...
const CEILING = {
  /* D-561 side, kept as history: reachGap 38 on its own branch; the c22-batch29 union's figure is re-read from the print. */
  /* D-542 side, kept as history (c22-batch30: ours' reachGap kept; NUMBER TO RE-READ from --strict's print on the union): reachGap: 146, — D-542 / D-562 (2026-09-25, branch land/worker/D-542): ROSE 39 -> 146 from this guard's own --strict print on the item's tree over origin/main 5e8a65a8, the one direction this ceiling may rise honestly (D-485's rule): THE INSTRUMENT GAINED SIGHT and the subject did not get worse. All 107 are codes R5/R6 put in reach for the first time, and NOT ONE of them carries a canned translation — the whole of the rise, and the finding this item was told to mint: D-641 carries them (each gets a DEC-49 row, or is established not to cross the wire and the walk narrowed at its op). The 39 are unchanged. May only FALL from here. */
  reachGap: 143, /* CONDUCT #23 at the c23-batch30 union: 43 -> 143, READ from `node civicos-ui/check-refusal-codes.mjs --strict`'s print on the merged tree (HEAD 645f8c41); never a branch's figure, never added by hand. ROSE, and only because THE INSTRUMENT GAINED SIGHT (D-485's rule, the one direction this ceiling may rise honestly): D-542/D-562's R5 (by op) and R6 (public op) put 107 untranslated codes in reach for the first time (its branch: 39 -> 146 over 5e8a65a8, kept as history above), each OWED by D-641. The subject did not get worse: `untranslated` FELL 266 -> 265 on the same run. The union's 143 is the print (43 on main, plus D-542's sighted codes, less those the batch translated), not a sum. May only FALL from here. */ /* CONDUCT #22 at the c22-batch29 union: 44 -> 43, READ from --strict's print on the merged tree (`arm B: RATCHET — 43 of 557 codes in reach`). Against M0-148's 44 (its branch's list, re-printed at 07f38280): NOT_PUBLISHED left (D-561's C-98.8 translates it) and NO_REVIEW_COPY left (C-87.1, D-448, on main but not on M0-148's base); NO_PROJECT_SCOPE ENTERED — the instrument gained sight (UI-110's suite), the subject did not get worse; owed by D-623 (arm H's R4_OWED). 44 - 2 + 1 = 43, and the figure is the print, not that sum. Against origin/main's 39 the union's ceiling ROSE by 4, each an instrument gaining sight (M0-148's NOT_CONCLUDED and NO_REVIEW_COPY, D-485's three, this NO_PROJECT_SCOPE) less the codes the batch translated. */ /* M0-148 (2026-09-25, branch land/worker/M0-148): ROSE 42 -> 44 from this guard's own print on the item's tree, the one direction this ceiling may rise honestly (D-485's rule): the INSTRUMENT GAINED SIGHT and the subject did not get worse. The two are NOT_CONCLUDED and NO_REVIEW_COPY (see reach), already handed to a member by preauth-vocabulary's mocks and already untranslated. Each is OWED by name: NO_REVIEW_COPY by D-448 (integrated, not yet on main at 964da679), NOT_CONCLUDED by D-599 (minted by this item). Pristine a23fe7ee printed 42 at slack 0. */ /* D-485 (arm H, 2026-09-25, branch land/worker/D-485): ROSE 39 -> 42 from this guard's own print on the item's tree over origin/main 964da679, and it is the ONE direction this ceiling may rise honestly — the INSTRUMENT GAINED SIGHT, the subject did not get worse. The three are BAD_REQUIRED, NO_JUSTIFICATION and NO_KIND: already shown to a member off the real plane (intent-write.test.mjs's #pg-pf, #rel-pf and #ent-pf panes) and already untranslated before this landing, and invisible to R1-R3. Each is owed by name in arm H's R4_OWED and by D-590, which lowers this ceiling as it pays them. ZERO pre-existing slack: the pristine 964da679 printed reachGap 39 at slack 0. */ /* D-484 (C-33.40 NO_BASIS, C-33.41 NO_CITATION consolidated behind one governed helper each, 2026-09-24, branch land/worker/D-484): FELL 40 -> 39 from this guard's own --strict print on the item's tree over origin/main 16fe1e7f. ZERO pre-existing slack: the PRISTINE origin/main 16fe1e7f, run before any edit in this worktree, printed exit 0 with EVERY ratchet key at slack 0. NO_BASIS was one of the 40 codes in reach with no
  ...
  /* D-688 side, kept as history: multiSiteCodes 54 on its own branch; the c23-batch30 union's figure is re-read from the print. */
...
const SLACK = {
  ...
  reach:                { bound: 0, why: "R1, R2, R3 and R4 (D-485), and R5 and R6 (D-542/D-562), each move by an edit to the catalog, app.html, a committed suite or bio-plane/src, in the landing that makes it; gated on the figure in the commit at HEAD (D-257)" },
  r4Suites:             { bound: 0, why: "a real-plane UI suite is added or removed by the landing that writes it, and a suite leaving the corpus lets arm H's zero gate pass for free (D-485); gated on the suites in the commit at HEAD (D-257)" },
  r4Pane:               { bound: 0, why: "a suite starts or stops asserting a code in a pane in the landing that edits it, so the floor moves in that landing (D-485); gated on the suites in the commit at HEAD (D-257)" },
  r5Op:                 { bound: 0, why: "a surface starts or stops calling an op, or the plane starts or stops minting a code on one, in the landing that edits app.html or bio-plane/src (D-542); gated on the files in the commit at HEAD (D-257)" },
  r6Public:             { bound: 0, why: "an op becomes or stops being public, or starts or stops minting a code, in the landing that edits OPS or bio-plane/src (D-562); gated on the files in the commit at HEAD (D-257)" },
...
/* ============================================================== D-433, 2026-09-19
   ...
   WHAT THIS MATCHER CAN AND CANNOT SEE, stated rather than left to be found:
     ...
     - A code fed through a VARIABLE (`const R = "X"; … reason: R`) — CORRECTED
       2026-09-25 by M0-148, never exempted. This line said the walk could not see
       one, and half of that was wrong: a LOCAL literal's declaration is itself
       fed-shaped, so `const R = "X"` always counted. The half that was right is
       the one that cost a code: a value arriving through an IMPORTED binding, or
       derived at runtime, never appears as a literal in the suite at all
       (REQUIRED_ARGUMENT_MISSING through `plane-refusal-wire.mjs`, UI-84's
       finding). The binding pass below follows both; what IT cannot see is
       stated at `loadSuiteHelpers`.
     - It reads the suite with COMMENTS BLANKED (M0-148): a code named in a note
       is not a code handed to a surface.
...
function partitionSuiteLiterals(raw, helpers = new Map()) {
  const fed = new Set(), observed = new Set();
  /* M0-148 (UI-100's F1): COMMENTS FIRST. This walk harvested every quoted
     SCREAMING_SNAKE token in the suite's TEXT, comments included, and counted a
     code named only in a note as one the suite hands to a surface — measured by
     UI-100 at 81 against 80, and it is why `publishedcase.test.mjs` had to name a
     code UNQUOTED in its own comment to keep this figure honest. Blanked through
     the estate's one lexer (`stripComments`, walkfloor.mjs), as D-485's arm H
     reads suites: length and newlines are preserved, so every offset below still
     addresses the same place, and string literals are KEPT, because a fed code IS
     a string literal. Regex literals are blanked too — a code inside `/…/` is a
     pattern a suite matches against, never one it hands over. */
  ...
  /* M0-148: A CODE FED THROUGH A BINDING. The literal pass above is blind to a
     code that reaches the surface as an IDENTIFIER — and since UI-84 and UI-100
     that is the estate's PREFERRED spelling, not a corner: `plane-refusal-wire.mjs`
     reads C-61.1's code out of `index.mjs` at the site that mints it, so
     `preauth-vocabulary.test.mjs` feeds REQUIRED_ARGUMENT_MISSING to its mock as
     `say(VERIFY_REFUSAL_WIRE)` and the text of the suite never holds the code.
     The walk undercounted by exactly the codes the estate derives most carefully.
     So the walk follows a binding to its VALUE:
       - an IMPORT from a sibling helper in civicos-ui/test/ (`import { … } from
         "./x.mjs"`, or `const { … } = await import("./x.mjs")`), resolved by the
         helper's EVALUATED export (`helpers`, loaded once by `loadSuiteHelpers`):
         a string export is its own value; a function export carries the codes of
         the string exports its body names (one level, stated below);
       - a LOCAL `const`/`let`/`var` whose initialiser names a carrier or a code
         literal, to a fixpoint — `const W = requiredArgumentWire("verify")`
         (a code reaching a local from a literal is already FED by the literal
         pass, so that half moves no figure; it lets a chain continue).
     An occurrence of a carrier is then classified as a literal is, but STRICTER,
     because this pass ADDS to reach and the literal pass may only remove from it:
     an occurrence anywhere inside an expectation call, or beside a comparison,
     is OBSERVED; only an occurrence outside both is FED. The occurrences that
     DECLARE a binding (its import, its own name, a carrier at the head of its
     initialiser) are neither — they carry a value, they do not hand one over. Identifiers are read over the source with
     strings blanked and `${…}` kept (`strip`, D-301's mode), so a binding's name
     inside a sentence is not a use. */
  const fedLiteral = new Set(fed);
  const code = strip(raw, { strings: true, keepInterpolations: true });
  const carriers = new Map();          // local name -> Set<code>
  const via = new Map();               // local name -> how it was resolved, for the print
  const unresolved = [];
  const declSpans = [];
  const specs = (list, sep) => list.split(",").map(x => x.trim()).filter(Boolean).map(x => {
    const [a, b] = x.split(sep).map(y => y.trim());
    return { exported: a, local: b || a };
  });
  const bind = (list, sep, spec, at, len) => {
    declSpans.push([at, at + len]);
    const h = helpers.get(spec);
    if (!h) return;                    // not a sibling helper this walk evaluates
    for (const { exported, local } of specs(list, sep)) {
      if (!/^[A-Za-z_$][\w$]*$/.test(local)) continue;
      if (h.refused) { unresolved.push(`${local} (${spec}: ${h.refused})`); continue; }
      const codes = h.codes.get(exported);
      if (codes && codes.size) { carriers.set(local, new Set(codes)); via.set(local, `${spec}#${exported}`); }
    }
  };
  for (const m of src.matchAll(/\bimport\s*\{([^}]*)\}\s*from\s*["'](\.\/[^"']+)["']/g))
    bind(m[1], /\s+as\s+/, m[2], m.index, m[0].length);
  for (const m of src.matchAll(/\b(?:const|let|var)\s*\{([^}]*)\}\s*=\s*await\s+import\(\s*["'](\.\/[^"']+)["']\s*\)/g))
    bind(m[1], ":", m[2], m.index, m[0].length);
  /* LOCAL BINDINGS: the initialiser runs to the first `;` or newline at depth 0. */
  const locals = [];
  for (const m of code.matchAll(/\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=(?!=)/g)) {
    ...
    locals.push({ name: m[1], at: m.index, end: i, rhs: [m.index + m[0].length, i] });
  }
  const idRe = (n) => new RegExp(`(?<![\\w$.])${n.replace(/\$/g, "\\$")}(?![\\w$])`, "g");
  for (let grew = true, pass = 0; grew && pass < 16; pass++) {
    grew = false;
    for (const l of locals) {
      const [a, b] = l.rhs, got = new Set(carriers.get(l.name) || []), before = got.size;
      for (const [n, cs] of carriers) if (n !== l.name && idRe(n).test(code.slice(a, b))) for (const c of cs) got.add(c);
      for (const lm of src.slice(a, b).matchAll(/["'`]([A-Z][A-Z0-9_]{2,})["'`]/g)) if (CODE_RE.test(lm[1])) got.add(lm[1]);
      if (got.size > before) {
        carriers.set(l.name, got); grew = true;
        if (!via.has(l.name)) via.set(l.name, "a local binding");
      }
    }
  }
  /* WHAT A DECLARATION IS. The binding's own NAME is not a use, and neither is a
     carrier standing at the HEAD of an initialiser (`const W = requiredArgumentWire(…)`,
     `= await X`): that builds a value, it hands nothing over. A carrier NESTED inside
     the initialiser — an argument, an object member — IS a use, classified where it
     stands, because that is where the estate hands a mock over:
     `const p5 = await openMock(mockFor({ answer: unknownOpWire(…) }))` (case-frozen-
     pair) feeds the surface inside the initialiser, and reading it as propagation
     alone left the code to be judged by `p5`'s later use in an `ok(…)`. */
  for (const l of locals) {
    if (!carriers.has(l.name)) continue;
    const nameAt = code.indexOf(l.name, l.at);
    declSpans.push([nameAt, nameAt + l.name.length]);
    const head = /^\s*(?:await\s+)?([A-Za-z_$][\w$]*)/.exec(code.slice(l.rhs[0], l.rhs[1]));
    if (head && carriers.has(head[1]) && head[1] !== l.name) {
      const hAt = l.rhs[0] + head[0].length - head[1].length;
      declSpans.push([hAt, hAt + head[1].length]);
    }
  }
  const inDecl = (at) => declSpans.some(([a, b]) => at >= a && at < b);
  const followed = new Map();          // code -> the binding it was followed through
  for (const [n, cs] of carriers) {
    for (const m of code.matchAll(idRe(n))) {
      if (inDecl(m.index)) continue;
      const before = code.slice(Math.max(0, m.index - 40), m.index);
      const after = code.slice(m.index + n.length, m.index + n.length + 40);
      if (/[{,]\s*$/.test(before) && /^\s*:/.test(after)) continue;       // an object KEY, not a use
      const isObs = inObsCall(m.index) || /[!=]==?\s*$/.test(before) || /^\s*[!=]==?/.test(after);
      for (const c of cs) {
        if (isObs) { observed.add(c); continue; }
        if (!fed.has(c)) followed.set(c, `${n} <- ${via.get(n)}`);
        fed.add(c);
      }
    }
  }
  ...
  return { fed, observed, followed, unresolved, fedLiteral };
}

/* M0-148: THE SIBLING HELPERS A SUITE IMPORTS, EVALUATED ONCE. A helper's export
   is resolved by IMPORTING it — the value the suite receives, not a reading of
   its text — because the export that matters (`REQUIRED_ARGUMENT_CODE`) is
   itself DERIVED from `index.mjs` by a regex, and no static reading of the
   helper can say what that regex returns. A helper that BOOTS something (a
   Miniflare plane, a child process, a listener) is NOT evaluated here: the guard
   must not start a plane to count a code. Such a helper's bindings are printed
   as UNRESOLVED, by name, every run, and so is a helper whose import THROWS
   (`plane-refusal-wire.mjs` throws at import when the plane stops decorating —
   and then the codes it carried leave R3 and the pin below names them).
   WHAT IT CANNOT SEE: a function export carries the codes of the STRING exports
   its body names, one level — a function reaching a code through another
   function, a local helper, or an object export is not followed; a local
   binding initialised by a CALL of a suite-local function
   (`const X = planeCaseNotPublished()`) is not evaluated; destructuring a
   carrier (`const { reason } = W`) is not followed. Each is an UNDERCOUNT, the
   direction a floor catches. */
async function loadSuiteHelpers(suites) {
  const helpers = new Map();
  for (const s of suites) {
    const src = stripComments(fs.readFileSync(path.join(TESTDIR, s), "utf8"));
    for (const m of src.matchAll(/(?:\bfrom\s*|\bimport\(\s*)["'](\.\/[^"'\/]+\.mjs)["']/g)) {
      const spec = m[1];
      if (helpers.has(spec) || /\.(?:test|control)\.mjs$/.test(spec)) continue;
      const abs = path.join(TESTDIR, spec);
      if (!fs.existsSync(abs)) continue;
      const hsrc = stripComments(fs.readFileSync(abs, "utf8"));
      if (/\bminiflare\b|\bchild_process\b|\.listen\s*\(/i.test(hsrc)) {
        helpers.set(spec, { refused: "boots a plane or a process; not evaluated by this guard", codes: new Map() });
        continue;
      }
      let mod;
      try { mod = await import("file://" + abs); }
      catch (e) {
        helpers.set(spec, { refused: `THREW at import: ${String(e && e.message || e).split("\n")[0].slice(0, 160)}`, codes: new Map() });
        continue;
      }
      const codes = new Map(), strs = new Map();
      for (const [k, v] of Object.entries(mod))
        if (typeof v === "string" && CODE_RE.test(v)) { strs.set(k, v); codes.set(k, new Set([v])); }
      for (const [k, v] of Object.entries(mod)) {
        if (typeof v !== "function") continue;
        const body = strip(String(v), { strings: true, keepInterpolations: true });
        const got = new Set();
        for (const [n, c] of strs) if (new RegExp(`(?<![\\w$.])${n}(?![\\w$])`).test(body)) got.add(c);
        if (got.size) codes.set(k, got);
      }
      helpers.set(spec, { refused: null, codes });
    }
  }
  return helpers;
}
...
/* ============================================================
   ARM H — REACH OBSERVED THROUGH THE REAL PLANE (D-485, 2026-09-25)
   ============================================================

   R1-R3 put a code in reach when the CATALOGUE holds it, the SURFACE's text
   names it, or a harness MOCK feeds it. None of them sees a code the surface
   RECEIVES FROM THE REAL PLANE, which is the only way most codes ever reach a
   member — and D-433 (rightly) took the suites that assert such codes out of
   R3, because a code a suite asserts against is not a code a MOCK handed the
   surface. So the one class of evidence that is closest to a member's own
   experience — a real-plane UI suite that drove app.html against the plane's
   own source under Miniflare and SAW THE CODE IN A PANE — counted toward
   nothing. **The receipt is `NO_CITATION`: `intent-write.test.mjs` read it in
   the `#rel-pf` and `#pg-dis-pf` panes, off the real plane, for months, while
   this guard's sentence "every code a surface can receive carries a canned
   translation" was false of it** (UI-83's finding; D-484 closed the instance,
   this arm closes the class).

   R4 — a code is IN REACH when a REAL-PLANE UI SUITE OBSERVES IT IN A PANE:
     REAL-PLANE: the suite constructs `new Miniflare(` itself, IN CODE (a
       constructor spelled inside a string is a fixture's text), or imports a
       helper in `civicos-ui/test/` that does (discovered, never listed —
       `meaning-arms.walks.mjs` is one today).
     A UI SUITE: it drives the surface (imports `./extract.mjs` or reads
       `app.html`).
     OBSERVES IT IN A PANE: a POSITIVE regex-literal test, `/…CODE…/.test(S)`,
       whose regex names the code and whose subject S is none of —
         NEGATED   `!/CODE/.test(S)`: the suite proved the code is ABSENT;
         WIRE      `JSON.stringify(…)` or a `.reason`/`.code`/`.error`/`.check`
                   read: the PLANE's answer, not a pane (the fact D-433 split out);
         SOURCE    an identifier bound, to a fixpoint, to `readFileSync` or
                   `appScript(`: the surface's SOURCE TEXT, which is R2's
                   question, not a pane.
       `.includes("CODE")` on a subject is read by the same rules (its yield is
       printed; it is 0 today).
     Intersected with the plane census, like R2 and R3 — `/UNRATED/` and
     `/INQ-2026/` name no refusal.

   THE GATE IS ZERO, NOT A RATCHET, and that is the row's acceptance: a code a
   member was SHOWN off the real plane is inside DEC-49 by its own sentence, so
   every R4 code must resolve to a canned translation (T1 or T2) or this guard
   exits 1 naming the code, the suite and the pane. It also enters `reach`, so
   arm B's gap and arm F's partition see it.

   WHAT THIS ARM CAN AND CANNOT SEE, stated because it is load-bearing:
     - It reads suite TEXT, lexed by the estate's one lexer (`stripComments`,
       walkfloor.mjs), so a regex quoted in a COMMENT is not an observation.
     - It cannot see a code observed through a VARIABLE regex, a string
       `.indexOf`, a DOM query that returns the code, or a pane the suite
       reads via a helper and asserts on elsewhere with a non-literal. Those
       are UNDER-reads: the safe direction for a gate at zero is the other one,
       and every contributing observation is PRINTED so a reader can check
       each against its suite.
     - Its PANE/WIRE/SOURCE split is by the subject's SPELLING. A subject that
       is really the wire but spelled as a bare variable is read as a pane,
       which OVER-reads reach — the direction that can only demand a
       translation, never excuse one.
     - It says nothing about a code a real-plane suite never drives. D-542
       (reach-by-op) is that arm; it is not this one. */
const MINIFLARE_CTOR = /\bnew\s+Miniflare\s*\(/;
const DRIVES_SURFACE = /["']\.\/extract\.mjs["']|app\.html/;
const WIRE_SUBJECT = /^JSON\.stringify\s*\(|\.(?:reason|code|error|check)\s*$/;
const H_CODE = /(?<![A-Za-z0-9_])([A-Z][A-Z0-9_]{2,})(?![A-Za-z0-9_])/g;
...
/* The identifiers bound to SOURCE TEXT, to a fixpoint: `const APP = fs.readFileSync(…)`,
   then `const REGION = APP.slice(…)`, and so on. */
function sourceIdents(code) {
  const decls = [...code.matchAll(/\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*([^;\n]*)/g)];
  const ids = new Set();
  for (let grew = true; grew;) {
    grew = false;
    for (const [, id, rhs] of decls) {
      if (ids.has(id)) continue;
      if (/\breadFileSync\s*\(|\bappScript\s*\(/.test(rhs) || [...ids].some(x => new RegExp(`(?<![\\w$.])${x.replace(/\$/g, "\\$")}(?![\\w$])`).test(rhs))) {
        ids.add(id); grew = true;
      }
    }
  }
  return ids;
...
/* Every observation in ONE suite: { code, kind, subject, at } for each code named
   in a positive-or-negated regex test or `.includes("CODE")`. `kind` is PANE,
   NEGATED, WIRE or SOURCE; only PANE enters reach. */
function paneObservations(src) {
  const lexed = stripComments(src);
  const srcIds = sourceIdents(lexed);
  const out = [];
  const classify = (negated, subject) => {
    const lead = (subject.match(/^[A-Za-z_$][\w$]*/) || [""])[0];
    return negated ? "NEGATED" : WIRE_SUBJECT.test(subject) ? "WIRE" : srcIds.has(lead) ? "SOURCE" : "PANE";
  };
  const line = at => src.slice(0, at).split("\n").length;
  /* A REGEX LITERAL IS WHAT THE LEXER BLANKED WHILE THE `.test(` AFTER IT SURVIVED:
     inside a comment the lexer blanks the `.test(` too, so a quoted example is not
     an observation. */
  for (const m of src.matchAll(/(!\s*)?\/((?:\\.|\[(?:\\.|[^\]\\\n])*\]|[^/\\\n\[])+)\/[a-z]*\.test\(/g)) {
    const reStart = m.index + (m[1] ? m[1].length : 0);
    const callAt = m.index + m[0].length - ".test(".length;
    if (lexed[reStart] !== " " || lexed.slice(callAt, callAt + 6) !== ".test(") continue;
    const open = m.index + m[0].length - 1, close = matchParen(src, open);
    if (close < 0) continue;
    const subject = src.slice(open + 1, close).trim();
    for (const c of m[2].matchAll(H_CODE))
      out.push({ code: c[1], kind: classify(!!m[1], subject), subject, at: line(m.index), shape: "regex" });
  }
  for (const m of lexed.matchAll(/(!\s*)?([A-Za-z_$][\w$.]*(?:\([^()\n]*\))?)\.includes\(\s*"([A-Z][A-Z0-9_]{2,})"\s*\)/g))
    out.push({ code: m[3], kind: classify(!!m[1], m[2]), subject: m[2], at: line(m.index), shape: "includes" });
  return out;
}

/* THE FIRST RUN OF THIS ARM WAS RED, AND THE ROW'S PREMISE WAS HALF TRUE. D-485's row
   said its arm would read RED only until D-484 translated `NO_CITATION`. D-484 did, and
   the arm still named FIVE more codes a real-plane suite reads in a pane with no canned
   translation (measured 2026-09-25 over origin/main 964da679):
       BAD_REQUIRED       intent-write #pg-pf         one site  (arm F: F5)
       NO_JUSTIFICATION   intent-write #rel-pf        one site  (arm F: F5)
       NO_KIND            intent-write #ent-pf        one site  (arm F: F5)
       NOT_CONCERNED      intent-write #pg-th-pf      MULTI-SITE (arm F: F4)
       NO_REASON          conclude-reading, intent-write #pg-dis-pf, queue-peritem  MULTI-SITE (F4)
   Three of them were not in reach at all before this arm (the reach grew 452 -> 455 and
   the gap 39 -> 42 by exactly those three); two were already in arm B's ratcheted gap.
   Translating them is catalogue work in the plane's area, and the multi-site two need
   D-550's one-mint-site consolidation first, so it is its own defect, D-590, placed by
   SCHEDULER. Until it lands, the five are OWED BY NAME, and the name is load-bearing:
     - a code in R4 with no translation that is NOT on this list FAILS (the gate is zero
       for everything else — this is what the row's control arms: strip NO_CITATION's
       translation and the arm fails naming it);
     - a code on this list that is now TRANSLATED, or no longer in R4, FAILS as STALE, so
       the list can only shrink, in the landing that pays the debt.
   It is a list of debts with their row, not an exemption: every run prints it. */
/* c22-batch29 union (CONDUCT #22, 2026-09-25): NO_PROJECT_SCOPE ADDED, owed by D-623 (spawned, fixing it). This guard's
   --strict print on the merged tree named it `NO TRANSLATION, owed by NOBODY (failed) · seen at
   queue-projectscope.test.mjs:461 (unent(kept))`: UI-110's new real-plane suite shows the member the plane's
   `op=proposedispose` refusal (store.mjs proposeDispose: a project-scoped disposition sent without its project) in the
   queue's kept-item pane, and the code has no catalogue row. The code was receivable before; the INSTRUMENT GAINED SIGHT of it, the subject did not get worse. */
const R4_OWED = new Map([
  ["BAD_REQUIRED", "D-590"], ["NO_JUSTIFICATION", "D-590"], ["NO_KIND", "D-590"],
  ["NOT_CONCERNED", "D-590"], ["NO_REASON", "D-590"],
  /* NO_PROJECT_SCOPE DISCHARGED by D-623 at c22-batch30 (CONDUCT #22, 2026-09-25): D-623 gives it catalogue row
     C-33.49 (ACT_SHAPE_CHECKS, renumbered from its branch's C-33.48) and its canned translation at both
     `op=proposedispose` sites through one helper, `actNoProjectScope`; the debt is paid, so it leaves this list in the
     landing that paid it, as the list's own rule requires. reachGap moves with it — re-read from --strict's print. */
]);

function armH(census, translated, rows) {
  /* TRANSLATED MEANS A SENTENCE EXISTS, NOT THAT A ROW DOES. Arm B's `translated` map
     records EVERY family row, with or without a `translation` — arm A is what fails a row
     that lacks one. FOUND BY THIS ARM'S OWN NEGATIVE CONTROL (the row's: strip
     NO_CITATION's translation): over that map arm H stayed SILENT, because NO_CITATION
     still had a row, and the guard went red only through arm A. So arm H asks the row for
     its sentence; a surface table's entry is proved total by arm D and counts as it is. */
  const rowSentence = new Map(rows.map(r => [r.code, r.translation]));
  const carries = c => translated.has(c) && (!rowSentence.has(c)
    || (typeof rowSentence.get(c) === "string" && rowSentence.get(c).trim().length > 0));
  const all = fs.readdirSync(TESTDIR).filter(f => f.endsWith(".mjs")).sort();
  const helpers = all.filter(f => !f.endsWith(".test.mjs")
    && MINIFLARE_CTOR.test(strip(fs.readFileSync(path.join(TESTDIR, f), "utf8"))));
  const pane = new Set(), paneRepro = new Set();
  const where = new Map();                 // code -> [ "suite:line <subject>" ]
  const kinds = { PANE: 0, NEGATED: 0, WIRE: 0, SOURCE: 0 }, shapes = { regex: 0, includes: 0 };
  const suites = [], perSuite = [];
  for (const s of all.filter(f => f.endsWith(".test.mjs"))) {
    const src = fs.readFileSync(path.join(TESTDIR, s), "utf8");
    const code = stripComments(src);
    /* THE CONSTRUCTOR IS READ IN CODE ONLY — strings blanked as well as comments. FOUND BY THIS
       ARM'S OWN CONTROL RUN (the driver's PRECONDITION went red): `refusal-codes.test.mjs` builds
       its fixture suites in TEMPLATE LITERALS that spell `new Miniflare(`, and read with strings
       kept it joined the corpus (29 -> 30) as a "real-plane suite" that runs no plane. */
    const real = MINIFLARE_CTOR.test(strip(src)) || helpers.some(h => code.includes(`./${h}`));
    if (!real || !DRIVES_SURFACE.test(code)) continue;
    suites.push(s);
    const committed = inCommit(path.join(TESTDIR, s));
    const seen = new Set();
    for (const o of paneObservations(src)) {
      if (!census.union.has(o.code)) continue;
      kinds[o.kind]++; shapes[o.shape]++;
      if (o.kind !== "PANE") continue;
      seen.add(o.code); pane.add(o.code); if (committed) paneRepro.add(o.code);
      if (!where.has(o.code)) where.set(o.code, []);
      where.get(o.code).push(`${s}:${o.at} (${o.subject.slice(0, 48)})`);
    }
    if (seen.size) perSuite.push({ suite: s, committed, codes: [...seen].sort() });
  }
  const suitesRepro = suites.filter(s => inCommit(path.join(TESTDIR, s))).length;

  MEASURE("r4Suites", suitesRepro, "real-plane UI suites (Miniflare over the plane's source, driving app.html) in the commit "
    + "at HEAD — ARM H's corpus (D-485)", suites.length);
  if (suitesRepro < FLOOR.r4Suites)
    FAIL(`ARM H (D-485): ${suitesRepro} real-plane UI suite(s) in the commit at HEAD, floor ${FLOOR.r4Suites}. THE ARM `
       + `LOST ITS CORPUS — a suite stopped constructing Miniflare or driving app.html, or this walk stopped recognising `
       + `one. Establish which before moving the floor: an empty corpus passes the zero gate below for free.`);
  MEASURE("r4Pane", paneRepro.size, "codes a real-plane UI suite in the commit at HEAD OBSERVES IN A PANE — R4 (D-485)",
    pane.size);
  if (paneRepro.size < FLOOR.r4Pane)
    FAIL(`ARM H (D-485): R4 is ${paneRepro.size} code(s) a real-plane suite observes in a pane, floor ${FLOOR.r4Pane}. `
       + `THE WALK LOST SIGHT of an observation — a suite stopped asserting the code, or this reader stopped reading `
       + `the shape. Establish which before moving the floor.`);

  const untranslated = [...pane].filter(c => !carries(c)).sort();
  const owed = untranslated.filter(c => R4_OWED.has(c));
  for (const [c, row] of R4_OWED) {
    if (carries(c))
      FAIL(`ARM H (D-485): R4_OWED names ${c} (owed by ${row}) but it now HAS a canned translation — `
         + `${translated.get(c)}. Remove it from R4_OWED in the landing that paid it: a debt list that keeps a paid `
         + `debt is one that can hide a new one.`);
    else if (!pane.has(c))
      FAIL(`ARM H (D-485): R4_OWED names ${c} (owed by ${row}) but no real-plane suite observes it in a pane any `
         + `more. Remove it from R4_OWED, and establish whether a suite stopped asserting it or this walk went blind.`);
  }
  for (const c of untranslated.filter(c => !R4_OWED.has(c)))
    FAIL(`ARM H (D-485): ${c} REACHES A MEMBER THROUGH THE REAL PLANE and has NO CANNED TRANSLATION — a real-plane `
       + `UI suite read it in a pane at ${where.get(c).join("; ")}. DEC-49 is not optional about this: a code a `
       + `surface is SHOWN is a code a surface can receive. Give it a row in a \`*_CHECKS\` family in `
       + `bio-plane/checks/bio-checks.mjs (the plane's translation, one place, every surface) — not wording at the `
       + `call site.`);

  NOTE(`arm H / D-485: REAL-PLANE REACH — ${suites.length} real-plane UI suite(s) (${suitesRepro} ${HEAD_SAYS}; `
     + `floor ${FLOOR.r4Suites}; Miniflare helpers: ${helpers.join(", ") || "none"}) · observations of a census code: `
     + `PANE ${kinds.PANE}, NEGATED ${kinds.NEGATED}, WIRE ${kinds.WIRE}, SOURCE ${kinds.SOURCE} (only PANE is reach) · `
     + `by shape: regex ${shapes.regex}, includes ${shapes.includes} · R4 ${pane.size} code(s), ${paneRepro.size} from `
     + `suites ${HEAD_SAYS}, floor ${FLOOR.r4Pane} · ${untranslated.length - owed.length ? `${untranslated.length - owed.length} UNTRANSLATED and not owed (failed)` : "EVERY one translated but the owed"}`
     + ` · OWED BY NAME ${owed.length}: ${owed.map(c => `${c} (${R4_OWED.get(c)})`).join(", ") || "none"}`);
  for (const c of [...pane].sort())
    NOTE(`arm H / D-485:   ${c} — ${carries(c) ? translated.get(c) : `NO TRANSLATION, owed by ${R4_OWED.get(c) || "NOBODY (failed)"}`} · seen at ${where.get(c).join("; ")}`);
  return { pane, paneRepro };
}
...
function armB(rows, census, surfaceTables, helpers) {
  ...
  const r3PerSuite = [];
  const R3followed = new Map(), R3unresolved = [], R3literalRepro = new Set();   // M0-148
  for (const s of suites) {
    const committed = inCommit(path.join(TESTDIR, s));
    const { fed, observed, followed, unresolved, fedLiteral } =
      partitionSuiteLiterals(fs.readFileSync(path.join(TESTDIR, s), "utf8"), helpers);
    for (const [c, how] of followed) if (census.union.has(c)) {
      if (!R3followed.has(c)) R3followed.set(c, []);
      R3followed.get(c).push(`${s} (${how})`);
    }
    for (const u of unresolved) R3unresolved.push(`${s}: ${u}`);
    if (committed) for (const c of fedLiteral) R3literalRepro.add(c);
  ...
  /* ============================================================== D-542 / D-562, 2026-09-25
     R5 BY OP and R6 PUBLIC OP. R2 asks whether the surface SPELLS a code; a surface
     that renders the plane's own `detail` (UI-68's review copy) spells none, and TEN
     of D-448's eleven codes sat out of reach for a day while a member could meet
     them on it. So the walk also asks what the surface CALLS: a code minted on an op
     `app.html` calls through a request helper is in reach (R5), and so is a code
     minted on an op a stranger can call, `classes: null` in `OPS` (R6, D-562 — the
     member who meets it is whoever asked). How "minted on an op" is read, and what
     the walk cannot see, is `reach-by-op.mjs`'s header, and its blind spots are
     PRINTED below every run. (R4 is D-485's arm H pane rule,
     SEEN IN A PANE; it joins reach after arm H runs, below.) Intersected with the census, as R2 and R3 are.
     Both are FLOORED on the figure from files in the commit at HEAD (D-257): a code
     counts there only when a file minting it on that op is committed. */
  const byOp = reachByOp({ planeDir: PLANE, appPath: APP });
  const R5 = new Map(), R6 = new Map(), R5repro = new Set(), R6repro = new Set();
  const opRule = (ops, into, repro) => {
    for (const op of ops)
      for (const [c, files] of byOp.opCodes.get(op) || new Map()) {
        if (!census.union.has(c)) continue;
        if (!into.has(c)) into.set(c, new Set());
        into.get(c).add(op);
        if ([...files].some(f => inCommit(path.join(PLANE_SRC, f)))) repro.add(c);
      }
  };
  if (byOp.ok) {
    opRule(byOp.surface.called.keys(), R5, R5repro);
    opRule(byOp.publicOps, R6, R6repro);
  }

  const reach = new Set([...R1, ...R2, ...R3, ...R5.keys(), ...R6.keys()]);
  ...
  const reachRepro = new Set([...R1, ...R2, ...R3repro, ...R5repro, ...R6repro]);
  ...
  /* D-485 — R4, THE REACH A REAL-PLANE SUITE SAW IN A PANE. Joined here, after the
     translations are known, because arm H gates its codes at ZERO against them;
     from here on R4 is reach like R1-R3, so the gap and arm F's partition see it. */
  const H = armH(census, translated, rows);
  for (const c of H.pane) reach.add(c);
  for (const c of H.paneRepro) reachRepro.add(c);
  ...
  MEASURE("reach", reachRepro.size, "codes a surface can receive, R1 + R2 + R3 FED + R4 SEEN (D-485) + R5 + R6 (D-542), in the commit at HEAD (the "
  ...
  /* D-542 / D-562: R5 and R6 FLOORED IN THEIR OWN RIGHT, for D-433's reason — the
     union cannot see either move, because most of what they hold is also a family
     row (R1). A by-op walk that stops resolving a helper, an op or a call falls
     HERE rather than nowhere. */
  const r5Size = byOp.ok ? R5repro.size : 0, r6Size = byOp.ok ? R6repro.size : 0;
  MEASURE("r5Op", r5Size, "codes minted on an op app.html calls, from files in the commit at HEAD (D-542's R5)", R5.size);
  if (r5Size < FLOOR.r5Op)
    FAIL(`R5 BY OP is ${r5Size} code(s) minted on an op the surface calls, floor is ${FLOOR.r5Op}. THE BY-OP WALK `
       + `LOST SIGHT${byOp.ok ? "" : ` (it could not read the dispatch: ${byOp.why})`} — and the total reach cannot `
       + `tell you, because a code dropped here is usually still a family row. Establish which of the surface's `
       + `helpers, the dispatch table or the call graph stopped resolving before moving the floor.`);
  MEASURE("r6Public", r6Size, "codes minted on a PUBLIC op (`classes: null`), from files in the commit at HEAD (D-562's R6)",
    R6.size);
  if (r6Size < FLOOR.r6Public)
    FAIL(`R6 PUBLIC OP is ${r6Size} code(s) minted on an op a stranger can call, floor is ${FLOOR.r6Public}. `
       + `THE WALK LOST SIGHT of a public op's codes${byOp.ok ? "" : ` (${byOp.why})`}.`);
  ...
  /* M0-148: THE CODES FED THROUGH A BINDING, printed with the binding each was
     followed through, and the ones fed ONLY that way PINNED BY NAME. A count
     alone cannot say which hand-off went missing when the resolution breaks —
     R3 falls by one and the floor's FAIL names no code — so the codes this walk
     can reach ONLY by following a binding are held in `R3_FOLLOWED_PIN`, and a
     pinned code no suite in the commit at HEAD feeds any more FAILS BY NAME. The
     pin is a ratchet by name: a code newly reached only by following FAILS until
     it is added from THIS print, and one that leaves (a suite starts feeding it
     as a literal, or stops feeding it) FAILS until it is removed — so the pin is
     always the printed set and never a hand-kept list. */
  for (const [c, how] of [...R3followed].sort())
    NOTE(`arm B / M0-148: FOLLOWED ${c} — ${how.join("; ")}`);
  if (R3unresolved.length)
    NOTE(`arm B / M0-148: UNRESOLVED binding(s), NOT followed and so NOT counted (an undercount, named): `
       + R3unresolved.join(" · "));
  const onlyFollowed = [...R3repro].filter(c => !R3literalRepro.has(c)).sort();
  NOTE(`arm B / M0-148: ${onlyFollowed.length} code(s) FED ONLY THROUGH A BINDING by a suite ${HEAD_SAYS}: `
     + `${onlyFollowed.join(", ") || "(none)"} · pinned ${R3_FOLLOWED_PIN.length}: ${R3_FOLLOWED_PIN.join(", ") || "(none)"}`);
  for (const c of R3_FOLLOWED_PIN)
    if (!R3repro.has(c))
      FAIL(`${c} is PINNED as fed to a surface through a binding (M0-148), and NO suite ${HEAD_SAYS} feeds it now. `
         + `Either the suite stopped handing it over, or THIS WALK STOPPED FOLLOWING THE BINDING — read the `
         + `UNRESOLVED and FOLLOWED lines above: a helper whose import threw, or a binding renamed out of the walk's `
         + `sight, drops the code here while the suite still feeds it. Establish which before touching the pin.`);
    else if (R3literalRepro.has(c))
      FAIL(`${c} is PINNED as fed ONLY through a binding (M0-148), but a suite ${HEAD_SAYS} now feeds it as a `
         + `LITERAL too. Remove it from R3_FOLLOWED_PIN — the pin holds exactly the printed set.`);
  for (const c of onlyFollowed)
    if (!R3_FOLLOWED_PIN.includes(c))
      FAIL(`${c} is fed to a surface ONLY through a binding (M0-148: ${(R3followed.get(c) || []).join("; ")}) and is `
         + `not in R3_FOLLOWED_PIN. Add it from this print, so the day the binding stops resolving the failure `
         + `names it instead of reporting R3 one lower.`);

  /* D-542: THE BY-OP WALK'S OWN LINES — its corpus, what it could not resolve, each
     surface op with the codes it carries, and the codes ONLY R5/R6 put in reach. */
  if (!byOp.ok)
    NOTE(`arm B / D-542: BY-OP WALK DID NOT RUN — ${byOp.why}. R5 and R6 are EMPTY and their floors decide.`);
  else {
    const sOps = [...byOp.surface.called.keys()].sort();
    NOTE(`arm B / D-542: R5 BY OP ${R5.size} code(s) over ${sOps.length} op(s) app.html calls through `
       + `${byOp.surface.helpers.length} request helper(s) [${byOp.surface.helpers.join(", ")}] · R6 PUBLIC OP `
       + `${R6.size} code(s) over ${byOp.publicOps.size} op(s) with \`classes: null\` · ${byOp.opCodes.size} of `
       + `${byOp.ops.size} ops mint a code the walk can attribute`);
    NOTE(`arm B / D-542: CANNOT SEE — ${byOp.surface.dynamic.length} helper call(s) with a COMPUTED op, `
       + `not resolved and not scored: ${byOp.surface.dynamic.join(" · ") || "none"}; ${byOp.surface.notAnOp.length} `
       + `quoted name(s) handed to a helper that OPS does not hold${byOp.surface.notAnOp.length ? `: ${byOp.surface.notAnOp.join(", ")}` : ""}; `
       + `${byOp.unrouted.length} op(s) neither routed to the store nor handled by name in index.mjs`
       + `${byOp.unrouted.length ? `: ${byOp.unrouted.join(", ")}` : ""}. The generic request path (auth, namespaces) `
       + `is attributed to no op.`);
    for (const op of sOps) {
      const codes = [...(byOp.opCodes.get(op) || new Map()).keys()].filter(c => census.union.has(c)).sort();
      if (codes.length) NOTE(`arm B / D-542:   ${op}${byOp.publicOps.has(op) ? " (public)" : ""} — ${codes.length} [${codes.join(", ")}]`);
    }
    for (const op of [...byOp.publicOps].filter(o => !byOp.surface.called.has(o)).sort()) {
      const codes = [...(byOp.opCodes.get(op) || new Map()).keys()].filter(c => census.union.has(c)).sort();
      if (codes.length) NOTE(`arm B / D-562:   ${op} (public, no surface call) — ${codes.length} [${codes.join(", ")}]`);
    }
    const only = [...new Set([...R5.keys(), ...R6.keys()])]
      .filter(c => !R1.has(c) && !R2.has(c) && !R3.has(c)).sort();
    NOTE(`arm B / D-542: IN REACH ONLY BY OP ${only.length} code(s) (no row, no surface literal, no mock) — `
       + `${only.map(c => `${c}${translated.has(c) ? "" : " UNTRANSLATED"} <- ${[...new Set([...(R5.get(c) || []), ...(R6.get(c) || [])])].sort().join("|")}`).join(", ") || "none"}`);
  }

  NOTE(`arm B: REACH ${reach.size} codes — R1 family rows ${R1.size}, R2 named by app.html ${R2.size}, `
     + `R3 FED by a harness mock ${R3.size}, R5 by op ${R5.size}, R6 public op ${R6.size} (R2/R3/R5/R6 intersected with the plane census; D-433: `
     + `observed-only codes excluded), R4 SEEN IN A PANE by a real-plane suite ${H.pane.size} (arm H, D-485) · `
...
function armC(rows) {
  ...
  const unclassified = [];
  const wholeMinusRegions = [];  // D-589 — whole-function sites that EXCLUDED a claimed nested region
  ...
  for (const [key, site] of sites) {
    ...
    if (site.region) {
      ...
      /* D-589 — A WHOLE-FUNCTION SPAN DOES NOT RE-JUDGE A CLAIMED REGION INSIDE IT. See `nestedRegionsIn`. */
      const nested = nestedRegionsIn(src, fnBody, site, claimedRegions);
      body = nested.body; nestedLines = nested.lines;
      if (nested.names.length) wholeMinusRegions.push(`${site.fn} − ${nested.names.join(", ")}`);
    }
    bodiesRead++;
  ...
  NOTE(`arm C: NESTED — ${wholeMinusRegions.length} whole-function site(s) judged WITHOUT the claimed region(s) `
     + `inside them, each region judged once, by its own rows (D-589)`
     + `${wholeMinusRegions.length ? `: ${wholeMinusRegions.join(" · ")}` : ""}`);
...
/* D-589 — THE CLAIMED REGIONS NESTED IN A WHOLE-FUNCTION SPAN, BLANKED OUT OF IT.
 *
 * A region is a governed site of its own: its rows are the ones its refusals owe codes to. Before this, a
 * whole-function `where` over the same function judged the region's lines a SECOND time, against the
 * function's rows, so a region written inside a function that another family still names whole FAILED every
 * refusal it held by name — measured by REC-207's first draft in `aiRunOpen`, where the region was right and
 * the guard was not. The two sites disagreed about who governs the same lines; the innermost claim now does,
 * and every line is judged exactly once.
 *
 * Only a region some row CLAIMS for this same file and function is taken out — an unclaimed marker is an
 * orphan and fails below, and a claimed region whose markers are malformed fails at its own site through
 * `regionSpan`; here it is left IN the whole span (a malformed region is never a reason to judge less). The
 * span taken out is marker to END marker inclusive, and it is blanked to spaces with its newlines kept, so every
 * line number the arm prints stays true. `lines` is how many lines were taken out, so `bodyLines` counts
 * only what this site actually judged. */
function nestedRegionsIn(src, fnBody, site, claimedRegions) {
  const fnStart = src.indexOf(fnBody.text);
  let text = fnBody.text, lines = 0;
  const names = [];
  if (fnStart < 0) return { body: fnBody, lines, names };
  for (const m of fnBody.text.matchAll(ANY_REGION_MARKER)) {
    if (m[1]) continue;                                          // an END marker
    const name = m[2];
    if (!claimedRegions.has(`${site.file}::${name}`)) continue;
    const esc = name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const ends = [...fnBody.text.matchAll(REGION_END(esc))];
    const starts = [...fnBody.text.matchAll(REGION_START(esc))];
    if (ends.length !== 1 || starts.length !== 1 || ends[0].index <= m.index) continue;
    const close = fnBody.text.indexOf(CLOSE_COMMENT, ends[0].index);
    if (close < 0) continue;
    const s = m.index, e = close + 2;
    const cut = text.slice(s, e);
    text = text.slice(0, s) + cut.replace(/[^\n]/g, " ") + text.slice(e);
    lines += cut.split("\n").length - 1;
    names.push(name);
  }
  return { body: { text, startLine: fnBody.startLine }, lines, names };
}
...
function functionBody(src, fn) {
  ...
  /* D-448 — `static` IS PART OF A METHOD'S HEADER AND THIS READER COULD NOT SEE PAST IT, so a `where`
     naming a static method resolved to nothing and arm C FAILED with "could not find function". It failed
     loudly rather than quietly, which is why this is a gap and not a hole — but the effect was that a
     refusal minted inside a static helper could never be GOVERNED, and D-448 is the first family whose
     mints are static on purpose: `#noReviewCopy` and `#notReviewOwner` are the review copy's two dead
     answers, built from NO argument precisely so their bytes cannot vary with anything the caller sent or
     anything the record holds, and `this` would hand them the record. Making them instance methods to
     satisfy this regex would have traded a real property of the plane for a limitation of the instrument.
     THE WIDENING ONLY EVER ADDS MATCHES — it matches strictly more headers, never different ones — and the
     one way that could change an EXISTING resolution is a class holding both a `static foo()` and an
     instance `foo()`, where the first match would flip. MEASURED across every file this walk reads
     (`bio-plane/src/*.mjs`, 106 static methods): ZERO names carry both forms, so no site this guard already
     resolved resolves anywhere new. Arm (f) of the control arms this landing. */
  const re = new RegExp(`(?:^|\\n)\\s*(?:export\\s+)?(?:static\\s+)?(?:async\\s+)?(?:function\\s+)?${esc}\\s*\\(`, "g");
...
const MULTI_SITE_CLOSED = new Map([
  ...
  /* The three below were DECLARED by CONDUCT #21 at c21-batch28, where arm G first met the branches that made them
     (D-549, D-448 and D-454 were each cut before D-550 and never ran this arm). Each was READ at its sites; none
     raises the ceiling, which stays at 59. */
  ...
  /* DECLARED at the c22-batch29 union (CONDUCT #22, 2026-09-25), where arm G first met D-147's LIFECYCLE_CHECKS, D-134's
     CUSTODIAL_CHECKS and REC-162's split of SESSION_ROLE_CANNOT_REACH_OP — each built on a base without arm G. Every
     site below was READ on the merged tree before it was declared; LIFECYCLE_TEXT_UNWRITABLE was read too and is NOT
     one condition, so it is a CANDIDATE below, not a closure. (The five QUOTE_* candidates below have the SAME
     has()/refusal() shape as these nine and were left as D-550 left them — judging them is not this union's.) */
  ["STAGE_NOT_OF_DIRECTION", "D-147: minted once, in store.mjs actionCorrespond's is-lifecycle-grammar region; the other literal is the adjacent `if (has(\"STAGE_NOT_OF_DIRECTION\"))` that READS the code of the catalogue's own lifecycleFindings finding to pick it — RATE_IP's shape, one condition (C-94's finding of that name)"],
  ["FOLLOWS_NO_ENTRY", "D-147: minted once, in store.mjs actionCorrespond's is-lifecycle-grammar region; the other literal is the adjacent `if (has(\"FOLLOWS_NO_ENTRY\"))` that READS the code of the catalogue's own lifecycleFindings finding to pick it — RATE_IP's shape, one condition (C-94's finding of that name)"],
  ["APPEAL_NAMES_NO_DECISION", "D-147: minted once, in store.mjs actionCorrespond's is-lifecycle-grammar region; the other literal is the adjacent `if (has(\"APPEAL_NAMES_NO_DECISION\"))` that READS the code of the catalogue's own lifecycleFindings finding to pick it — RATE_IP's shape, one condition (C-94's finding of that name)"],
  ["OUTCOME_NOT_ON_RECEIVED", "D-147: minted once, in store.mjs actionCorrespond's is-lifecycle-grammar region; the other literal is the adjacent `if (has(\"OUTCOME_NOT_ON_RECEIVED\"))` that READS the code of the catalogue's own lifecycleFindings finding to pick it — RATE_IP's shape, one condition (C-94's finding of that name)"],
  ["OUTCOME_NOT_IN_VOCABULARY", "D-147: minted once, in store.mjs actionCorrespond's is-lifecycle-grammar region; the other literal is the adjacent `if (has(\"OUTCOME_NOT_IN_VOCABULARY\"))` that READS the code of the catalogue's own lifecycleFindings finding to pick it — RATE_IP's shape, one condition (C-94's finding of that name)"],
  ["DECISION_WITHOUT_OUTCOME", "D-147: minted once, in store.mjs actionCorrespond's is-lifecycle-grammar region; the other literal is the adjacent `if (has(\"DECISION_WITHOUT_OUTCOME\"))` that READS the code of the catalogue's own lifecycleFindings finding to pick it — RATE_IP's shape, one condition (C-94's finding of that name)"],
  ["FEE_ESTIMATE_WITHOUT_QUOTE", "D-147: minted once, in store.mjs actionCorrespond's is-lifecycle-grammar region; the other literal is the adjacent `if (has(\"FEE_ESTIMATE_WITHOUT_QUOTE\"))` that READS the code of the catalogue's own lifecycleFindings finding to pick it — RATE_IP's shape, one condition (C-94's finding of that name)"],
  ["DUE_HALF_STATED", "D-147: minted once, in store.mjs actionCorrespond's is-lifecycle-grammar region; the other literal is the adjacent `if (has(\"DUE_HALF_STATED\"))` that READS the code of the catalogue's own lifecycleFindings finding to pick it — RATE_IP's shape, one condition (C-94's finding of that name)"],
  ["DUE_NOT_A_DATE", "D-147: minted once, in store.mjs actionCorrespond's is-lifecycle-grammar region; the other literal is the adjacent `if (has(\"DUE_NOT_A_DATE\"))` that READS the code of the catalogue's own lifecycleFindings finding to pick it — RATE_IP's shape, one condition (C-94's finding of that name)"],
  ["NOT_AN_ADMIN", "D-134: four sites — #custodialBar, memberCaps, adminEndorse, adminRemove — and each is the CALLER not being an active administrator (the one TARGET case was split to TARGET_NOT_AN_ADMIN); C-96.1's sentence (`the account asking is not one of them`) is true at all four, and dec49Decorate attaches it to the three bare returns on the way out"],
  ...
  /* D-688 (2026-09-25, WORKER D-688): the five QUOTE_* candidates D-550 left and the c22-batch29 union named as the SAME
     has()/refusal() shape as D-147's nine. Each READ at its two literals in store.mjs actionCorrespond's
     is-quote-grammar region on 95fe7bc7: one `if (has("X"))` READING the code of `quoteFindings`' own finding and the
     `return refusal("X", …)` on the next line minting it; no literal of any of the five in index.mjs. One condition
     each (C-72's finding of that name), so each is DECLARED closed and the ceiling falls by five. */
  ["QUOTE_NOT_ON_RECEIVED", "D-148: minted once, in store.mjs actionCorrespond's is-quote-grammar region; the other literal is the adjacent `if (has(\"QUOTE_NOT_ON_RECEIVED\"))` that READS the code of the catalogue's own quoteFindings finding to pick it — RATE_IP's shape, one condition (C-72.1's finding of that name; judged by D-688)"],
  ["QUOTE_AMOUNT_NOT_A_NUMBER", "D-148: minted once, in store.mjs actionCorrespond's is-quote-grammar region; the other literal is the adjacent `if (has(\"QUOTE_AMOUNT_NOT_A_NUMBER\"))` that READS the code of the catalogue's own quoteFindings finding to pick it — RATE_IP's shape, one condition (C-72.2's finding of that name; judged by D-688)"],
  ["QUOTE_NO_CURRENCY", "D-148: minted once, in store.mjs actionCorrespond's is-quote-grammar region; the other literal is the adjacent `if (has(\"QUOTE_NO_CURRENCY\"))` that READS the code of the catalogue's own quoteFindings finding to pick it — RATE_IP's shape, one condition (C-72.3's finding of that name; judged by D-688)"],
  ["QUOTE_ANSWERS_NO_SENT", "D-148: minted once, in store.mjs actionCorrespond's is-quote-grammar region; the other literal is the adjacent `if (has(\"QUOTE_ANSWERS_NO_SENT\"))` that READS the code of the catalogue's own quoteFindings finding to pick it — RATE_IP's shape, one condition (C-72.4's finding of that name; judged by D-688)"],
  ["QUOTE_REVISES_NO_QUOTE", "D-148: minted once, in store.mjs actionCorrespond's is-quote-grammar region; the other literal is the adjacent `if (has(\"QUOTE_REVISES_NO_QUOTE\"))` that READS the code of the catalogue's own quoteFindings finding to pick it — RATE_IP's shape, one condition (C-72.5's finding of that name; judged by D-688)"],
]);
const MULTI_SITE_CANDIDATES = new Set([
  ...
  "PROJECT_ACT_NOT_A_PARTICIPANT", /* D-688: the five QUOTE_* judged one condition each and DECLARED in MULTI_SITE_CLOSED */
  "RATIFY_FINDING_NOT_IN_A_RATIFIED_CASE", "RATIFY_NOT_EVIDENCE_OF_A_RATIFIED_CASE",
  ...
  /* LIFECYCLE_TEXT_UNWRITABLE left this set by D-688 (2026-09-25): the c22-batch29 union read it as TWO conditions under
     one code; D-688 split the token case to LIFECYCLE_TOKEN_MALFORMED (C-94.12, region is-lifecycle-token), so each
     code is now minted at ONE site with a sentence true of it. */
...
const suiteHelpers = await loadSuiteHelpers(
  fs.readdirSync(TESTDIR).filter(f => f.endsWith(".test.mjs")).sort());
const { reach, translated, gap } = armB(rows, census, surfaceTables, suiteHelpers);
armC(rows);
await armE();
